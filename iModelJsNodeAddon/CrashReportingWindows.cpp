...
void JsInterop::InitializeCrashReporting(CrashReportingConfig const& cfg)
    ...
    for (auto& prop : GetCrashReportPropertiesFromConfig(cfg))
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
void JsInterop::SetCrashReportProperty(Utf8CP key, Utf8CP value)
    {
    if (nullptr == s_customCrashProperties)
        return;

    auto wkey = std::wstring(WString(key, true).c_str());

    if (value == nullptr)
        {
        s_customCrashProperties->erase(wkey);
        }
    else
        {
        auto wvalue = std::wstring(WString(value, true).c_str());
        (*s_customCrashProperties)[wkey] = wvalue;
        }
    }
...
std::map<std::string, std::string> JsInterop::GetCrashReportProperties()
    ...
    if (nullptr == s_customCrashProperties)
        return props;
    for (auto prop : *s_customCrashProperties)
        props[Utf8String(prop.first)] = Utf8String(prop.second);
    return props;
    }
