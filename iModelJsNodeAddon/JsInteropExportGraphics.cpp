...
#define LOG (NativeLogging::CategoryLogger("ExportGraphics"))
...
public:
    struct CachedEntry
        ...
        CachedEntry(ColorDef c, bool twoSided, RenderMaterialId matId, DgnTextureId texId, DgnSubCategoryId subCat, DgnGeometryClass gc)
            : color(c), isTwoSided(twoSided), materialId(matId), textureId(texId), subCategoryId(subCat), geometryClass(gc) { }
        };
    ...
    struct CachedLineString
        ...
        CachedLineString(ColorDef c, DgnSubCategoryId subCat, DgnGeometryClass gc)
            : color(c), subCategoryId(subCat), geometryClass(gc) { }
        };
    bvector<CachedLineString>               m_cachedLineStrings;
    bool                                    m_gotBadPolyface;
    bool                                    m_generateLines;
    double                                  m_minLineStyleComponentSize;
    double                                  m_decimationTolerance;

    struct LineStyleGeometryCacheEntry
        {
        IGeometryPtr        m_inputGeometry;
        PolyfaceHeaderPtr   m_polyface;

        LineStyleGeometryCacheEntry(IGeometryR ig, PolyfaceHeaderR pf) : m_inputGeometry(&ig), m_polyface(&pf) { }
        };
...
virtual bool _ProcessCurveVector(CurveVectorCR curve, bool filled, SimplifyGraphic& sg) override
    ...
    if (curve.IsAnyRegionType())
        return false; // throw back to polyface processing if can be closed shape
    if (!m_generateLines)
        return true;
    ...
    if (!transform.IsIdentity())
        transform.Multiply(&strokePoints[0], (int)strokePoints.size());
    ...
    DgnGeometryClass geometryClass = sg.GetCurrentGeometryParams().GetGeometryClass();
    ColorDef color = sg.GetCurrentGraphicParams().GetFillColor();
    for (auto& cached : m_cachedLineStrings)
        {
        if (cached.color == color && cached.subCategoryId == subCategoryId && cached.geometryClass == geometryClass)
            {
    ...
    if (outIndices == nullptr)
        {
        m_cachedLineStrings.emplace_back(color, subCategoryId, geometryClass);
...
void ResolveMeshDisplayProps(SimplifyGraphic& sg, PolyfaceQueryCR pf, MeshDisplayProps& result)
    ...
    result.geometryClass = sg.GetCurrentGeometryParams().GetGeometryClass();
...
bool _ProcessPolyface(PolyfaceQueryCR pfIn, bool filled, SimplifyGraphic& sg) override {
  auto result = JsInterop::ProcessPolyface(pfIn, true, [&](PolyfaceQueryCR pf) {
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
bool ProcessPolyfaceAfterValidation(PolyfaceQueryCR pfQuery, bool filled, SimplifyGraphic& sg)
    {
    ...
    for (auto& entry : m_cachedEntries)
        ...
        if (entry.color != displayProps.color ||
            ...
            entry.geometryClass != displayProps.geometryClass)
            continue;
    ...
    if (exportMesh == nullptr)
        {
        m_cachedEntries.emplace_back(displayProps.color, displayProps.isTwoSided, displayProps.materialId, textureId, displayProps.subCategoryId, displayProps.geometryClass);
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
// 3D linestyles will send through many, many instances of the same geometry. Some
...
PolyfaceHeaderCP FindCachedLineStyleGeometry(IGeometryCR geometry)
    {
    BeAssert(IsDrawingStyledCurveVector());
    for (auto const& cacheEntry : m_lineStyleGeometryCache)
        {
        if (cacheEntry.m_inputGeometry->IsSameStructureAndGeometry(geometry))
            return cacheEntry.m_polyface.get();
        }
    return nullptr;
    }

PolyfaceHeaderR AddToCachedLineStyleGeometry(IGeometryR geometry, PolyfaceHeaderR pf)
    {
    BeAssert(IsDrawingStyledCurveVector());
    m_lineStyleGeometryCache.emplace_back(geometry, pf);
...
void _AddCurveVector(CurveVectorCR curves, bool isFilled) override
    {
    if (!m_exportGraphicsProcessor.IsDrawingStyledCurveVector() || !curves.IsAnyRegionType())
        {
        T_Super::_AddCurveVector(curves, isFilled);
        return;
        }
    // IGeometry functions want Ptr objects so we have to clone even when just checking cache.
    IGeometryPtr geometryPtr = IGeometry::Create(curves.Clone());
    if (!geometryPtr.IsValid())
        {
        BeAssert(false);
        return;
        }
    PolyfaceHeaderCP cachedPolyface = m_exportGraphicsProcessor.FindCachedLineStyleGeometry(*geometryPtr.get());
    if (!cachedPolyface)
        {
        IPolyfaceConstructionPtr pfBuilder = IPolyfaceConstruction::Create(*GetScaledFacetOptions());
        pfBuilder->AddRegion(curves);
        PolyfaceHeaderPtr clientMesh = pfBuilder->GetClientMeshPtr();
        if (clientMesh.IsValid())
            cachedPolyface = &m_exportGraphicsProcessor.AddToCachedLineStyleGeometry(*geometryPtr.get(), *clientMesh.get());
        }
    if (cachedPolyface)
        m_processor._ProcessPolyface(*cachedPolyface, false, *this);
    }
...
void _AddBSplineSurface(MSBsplineSurfaceCR surface) override
    {
    if (!m_exportGraphicsProcessor.IsDrawingStyledCurveVector())
        {
        T_Super::_AddBSplineSurface(surface);
        return;
        }
    // IGeometry functions want Ptr objects so we have to clone even when just checking cache.
    IGeometryPtr geometryPtr = IGeometry::Create(surface.Clone());
    if (!geometryPtr.IsValid())
        {
        BeAssert(false);
        return;
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
void _AddSolidPrimitive(ISolidPrimitiveCR solid) override
    {
    if (!m_exportGraphicsProcessor.IsDrawingStyledCurveVector())
        {
        T_Super::_AddSolidPrimitive(solid);
        return;
        }
    // IGeometry functions want Ptr objects so we have to clone even when just checking cache.
    IGeometryPtr geometryPtr = IGeometry::Create(solid.Clone());
    if (!geometryPtr.IsValid())
        {
        BeAssert(false);
        return;
    ...
    if (!cachedPolyface)
        {
        IPolyfaceConstructionPtr pfBuilder = IPolyfaceConstruction::Create(*GetScaledFacetOptions());
        pfBuilder->AddSolidPrimitive(solid);
        PolyfaceHeaderPtr clientMesh = pfBuilder->GetClientMeshPtr();
        if (clientMesh.IsValid())
            cachedPolyface = &m_exportGraphicsProcessor.AddToCachedLineStyleGeometry(*geometryPtr.get(), *clientMesh.get());
        }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
void _AddPolyface(PolyfaceQueryCR meshData, bool isFilled) override
    {
    const size_t minPointCount = 100; // Arbitrary tolerance to avoid setting up decimation for small meshes
    if (m_exportGraphicsProcessor.m_decimationTolerance == 0.0 || meshData.GetPointCount() < minPointCount)
        {
        T_Super::_AddPolyface(meshData, isFilled);
        return;
        }

    const double minCompressionRatio = 0.9; // Only tested after most of the expensive work is done, so not going to be picky
    PolyfaceHeaderPtr decimated = meshData.ClusteredVertexDecimate(m_exportGraphicsProcessor.m_decimationTolerance, minCompressionRatio);
    if (decimated.IsValid())
        T_Super::_AddPolyface(*decimated, isFilled); // Still call superclass so it can resolve IncompatiblePolyfacePreference
    else
        T_Super::_AddPolyface(meshData, isFilled);
    }
...
void _AddSubGraphic(Render::GraphicBuilderR graphic, DgnGeometryPartId partId, TransformCR transform, Render::GeometryParamsR geomParams) override
    {
    if (!m_instances)
        return T_Super::_AddSubGraphic(graphic, partId, transform, geomParams);
    ...
    rec.geometryClass = geomParams.GetGeometryClass();
...
static void convertPartInstances(Napi::Env& env, Napi::Array& napiArray, DgnElementId partInstanceId,
    ...
    for (auto& rec : instances)
        ...
        napiDisplayProps.Set("geometryClass", Napi::Number::New(env, static_cast<uint8_t>(rec.geometryClass)));
...
static bvector<std::unique_ptr<ExportGraphicsJob>> Create(DgnDbR db, Napi::Object const& exportProps)
    {
    ...
    if (napiMinLineStyleComponentSize.IsNumber())
        minLineStyleComponentSize = napiMinLineStyleComponentSize.DoubleValue();

    double decimationTolerance = 0.0;
    Napi::Number napiDecimationTolerance = exportProps.Get("decimationTol").As<Napi::Number>();
    ...
    for (uint32_t i = 0; i < elementIdArray.Length(); ++i)
        ...
        if (onLineGraphicsCb.IsFunction())
            job->m_onLineGraphicsCbRef = Napi::Persistent(onLineGraphicsCb);
        if (napiPartArray.IsArray())
            job->m_napiPartArrayRef = Napi::Persistent(napiPartArray);
...
void Finish(Napi::Env& env)
    ...
    if (m_caughtException)
        ...
        LOG.errorv(errorMsg, m_elementId.GetValueUnchecked());
    ...
    Napi::String elementIdString = createIdString(env, m_elementId);

    Napi::Function onGraphicsCb = m_onGraphicsCbRef.Value();
    if (onGraphicsCb.IsFunction())
        {
    ...
    Napi::Function onLineGraphicsCb = m_onLineGraphicsCbRef.Value();
    if (onLineGraphicsCb.IsFunction())
        {
    ...
    Napi::Array napiPartArray = m_napiPartArrayRef.Value();
    if (napiPartArray.IsArray() && !m_instances.empty())
        {
        convertPartInstances(env, napiPartArray, m_elementId, m_instances);
        }
    }
};
...
uint64_t getIdFromNapiValue(Napi::Value const& napiVal)
    {
