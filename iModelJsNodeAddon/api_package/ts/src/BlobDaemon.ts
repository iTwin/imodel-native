...
export interface DaemonProps {
  ...
  /** whether to create a shell to run daemon. */
  shell?: boolean;
}

/** The name and key for a blob container for the BlockCache daemon */
export interface BlobContainerProps extends DaemonProps {
...
export interface BlobDbProps extends BlobContainerProps {
...
export class BlobDaemon {
  ...
  private static daemonDir(props: DaemonProps) {
  ...
  public static getDbFileName(props: BlobDbProps) {
   ...
   * @return `{ result, errMsg }` the meaning of result varies by command, but 0 always means success. For `attach`, negative values are `DaemonAttachError`, positive numbers below 400 are `SQLITE_xxx` errors, and
   * values 400 and above are HTTP errors. `errMsg` is blank on success, but an english message useful for diagnostics otherwise.
   */
  public static command(command: BlobDaemonCommand, args: BlobDbProps): { result: DaemonAttachError | DbResult | number, errMsg: string } {
    const fullArgs = { ...args };
