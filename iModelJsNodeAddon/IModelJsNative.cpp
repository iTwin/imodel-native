...
struct NativeECSqlRowReader : BeObjectWrap<NativeECSqlRowReader>
    ...
    private:
        ...
        void Parse(BeJsValue const& requestObj, Env env)
            {
            //parse requestObj into m_request
            if(!requestObj.isObject())
                THROW_JS_IMODEL_NATIVE_EXCEPTION(env,"ECSqlRowReader constructor's third argument must be an object.", IModelJsNativeErrorKey::BadArg);
            JsOptions& jsonOptions = m_queryReader->getAdaptor().GetOptions();
            if (requestObj.isBoolMember("abbreviateBlobs")) 
                {
                const bool setAbbreviateBlobs = requestObj["abbreviateBlobs"].asBool();
                jsonOptions.SetAbbreviateBlobs(setAbbreviateBlobs);
                }
            if (requestObj.isBoolMember("convertClassIdsToClassNames")) 
                {
                const bool setConvertClassIdsToClassNames = requestObj["convertClassIdsToClassNames"].asBool();
                jsonOptions.SetConvertClassIdsToClassNames(setConvertClassIdsToClassNames);
                }
            }
    public:
        NativeECSqlRowReader(NapiInfoCR info) : BeObjectWrap<NativeECSqlRowReader>(info)
            {
            REQUIRE_ARGUMENT_ANY_OBJ(0, ecdb);
            REQUIRE_ARGUMENT_STRING(1, ecsql);
            REQUIRE_ARGUMENT_ANY_OBJ(2, requestObj);
            ECDb const* ecdb = ecdb.As<Napi::External<ECDb>>().Data();
            if (m_ecdb == nullptr)
                THROW_JS_TYPE_EXCEPTION("Invalid first arg for NativeECSqlRowReader constructor. ECDb must not be nullptr");
            m_queryReader = std::make_unique<ECSSqlRowReader>(ecdb, ecsql.As<Napi::String>().Utf8Value().c_str());
            Parse(requestObj, info.Env());
            }
        ...
        static void Init(Napi::Env& env, Napi::Object exports)
            ...
            InstanceMethod("reset", &NativeECSqlRowReader::Reset)});
        ...
        Napi::Value Step(NapiInfoCR info)
            {
                BeJsValue row;
                return m_queryReader->Step(row) ? NativeECSqlValue::New(info.Env(), row, m_queryReader->getAdaptor().GetECDb()) : info.Env().Undefined();
            }
    };
