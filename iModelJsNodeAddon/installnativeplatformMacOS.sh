#!/bin/zsh

function invalidiModelJsDir {
  echo Set the environment variable imodeljsDir, or pass an argument, that points to an imodeljs directory.
  exit
}

if [[ -z "${OutRoot}" ]]; then
  echo The OutRoot environment variable is not set.
  exit
fi

if [[ "" == $1 ]]; then
  if [[ -z "${imodeljsDir}" ]]; then invalidiModelJsDir; fi

  export tempDir=$imodeljsDir
else
  export tempDir=$1
fi

cp $OutRoot/MacOSX64/packages/imodeljs-native/*          $tempDir/core/backend/node_modules/@bentley/imodeljs-native
cp -r $OutRoot/MacOSX64/packages/imodeljs-n_10-darwin-x64 $tempDir/core/backend/node_modules/@bentley/imodeljs-native
