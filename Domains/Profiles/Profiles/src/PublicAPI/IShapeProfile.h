...
public:
    struct CreateParams : T_Super::CreateParams
        {
        DECLARE_PROFILES_CREATE_PARAMS_BASE_METHODS (IShapeProfile)
    ...
    public:
        //! @beginGroup
        double flangeWidth = 0.0; //!< Extent of the width (flange length). @details Defined parallel to the x axis of the position coordinate system.
        double depth = 0.0; //!< Extent of the depth (web length). @details Defined parallel to the y axis of the position coordinate system.
        double flangeThickness = 0.0; //!< Flange thickness of the I-shape. @details Both, the upper and the lower flanges have the same thickness and they are centred on the y-axis of the position coordinate system.
        double webThickness = 0.0; //!< Web thickness of the I-shape. @details The web is centred on the x-axis and the y-axis of the position coordinate system.
        //! @endGroup
        ...
        Angle flangeSlope = Angle::FromRadians (0.0); //!< Slope of the lower faces of the top flange and of the upper faces of the bottom flange. @details Non-zero in case of tapered flanges, 0 in case of parallel flanges, default 0 if not specified.
        //! @endGroup
        };

protected:
    explicit IShapeProfile (CreateParams const& params); //!< @private
...
public:
    ...
    //! Creates an instance of IShapeProfile.
    //! @param params CreateParams used to populate instance properties.
    //! @return Instance of IShapeProfile.
...
public:
    //! @beginGroup
    PROFILES_EXPORT double GetFlangeWidth() const; //!< Gets the value of @ref CreateParams.flangeWidth "FlangeWidth"
    ...
    PROFILES_EXPORT double GetFlangeThickness() const; //!< Gets the value of @ref CreateParams.flangeThickness "FlangeThickness"
    ...
    PROFILES_EXPORT double GetFilletRadius() const; //!< Gets the value of @ref CreateParams.filletRadius "FilletRadius"
    PROFILES_EXPORT void SetFilletRadius (double value); //!< Sets the value for @ref CreateParams.filletRadius "FilletRadius"

    PROFILES_EXPORT double GetFlangeEdgeRadius() const; //!< Gets the value of @ref CreateParams.flangeEdgeRadius "FlangeEdgeRadius"
    ...
    PROFILES_EXPORT Angle GetFlangeSlope() const; //!< Gets the value of @ref CreateParams.flangeSlope "FlangeSlope"
