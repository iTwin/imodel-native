...
TEST_F(EnergyPerformanceAspectTestFixture, Create)
    ...
    double rating = 99.99f;

    EnergyPerformanceAspectPtr aspect = EnergyPerformanceAspect::Create(rating);
    ...
    EnergyPerformanceAspectCPtr aspect2 = EnergyPerformanceAspect::GetCP(*object);
    ASSERT_TRUE(aspect2.IsValid());

    ASSERT_EQ(rating, aspect2->GetRating());
