...
protected:
    ROADRAILALIGNMENT_EXPORT bool ComputeOffsetFillet (const double& offsetDist, const double& radius, bvector<CurveCurve::FilletDetail> &arcs);

public:
    // override primary offset checks with real data
    ROADRAILALIGNMENT_EXPORT void SetPrimaryOffsets (const double& right, const double& left);
    // override secondary offset checks with real data
    ROADRAILALIGNMENT_EXPORT void SetSecondaryOffsets (const double& right, const double& left);

    // compute the necessary geometry to create an entrance intersection
    // offsets should be signed
    ROADRAILALIGNMENT_EXPORT bool ComputePotentialFillets (const double& primaryOffset, const double& secondaryOffset, const double& radius, bvector<CurveCurve::FilletDetail> &arcs);
    // calculate the actual intersection point
    ROADRAILALIGNMENT_EXPORT bool ComputeIntersectionPoint (DPoint3dR pt, double * primaryStation = nullptr, double * secondaryStation = nullptr, bool allowExtension = true);

    // accessor for primary road
    ROADRAILALIGNMENT_EXPORT AlignmentPairPtr PrimaryRoad () { return m_primaryRoad; }
    // accessor for secondary road
    ROADRAILALIGNMENT_EXPORT AlignmentPairPtr SecondaryRoad () { return m_secondaryRoad; }
...
public:
    ...
    ROADRAILALIGNMENT_EXPORT DPoint3d Point () { return m_point; }
    ROADRAILALIGNMENT_EXPORT DVec3d SecondaryTangentAtIntersect() { return m_secondaryTangentAtIntersect; }
    ROADRAILALIGNMENT_EXPORT ExtensionType ExtensionType () { return m_extensionType; }
    ROADRAILALIGNMENT_EXPORT double PrimaryStation () { return m_primaryStation; }
    ROADRAILALIGNMENT_EXPORT double SecondaryStation () { return m_secondaryStation; }
    ROADRAILALIGNMENT_EXPORT double ExtendedAmount() { return m_extendedAmount; }
...
public:
    ...
    ROADRAILALIGNMENT_EXPORT void WantZDifferences (const bool& bVal) { m_computeZs = bVal; }
    // add computed station values to info
    ROADRAILALIGNMENT_EXPORT void WantStations (const bool& bVal) { m_computeStations = bVal; }
    // allow primary start to be extended
    ROADRAILALIGNMENT_EXPORT void SetAllowExtendPrimaryStart (const bool& bVal) { m_extendPrimaryStart = bVal; }
    // allow primary end to be extended
    ROADRAILALIGNMENT_EXPORT void SetAllowExtendPrimaryEnd (const bool& bVal) { m_extendPrimaryEnd = bVal; }
    // allow secondary start to be extended
    ROADRAILALIGNMENT_EXPORT void SetAllowExtendSecondaryStart (const bool& bVal) { m_extendSecondaryStart = bVal; }
    // allow secondary end to be extended
    ROADRAILALIGNMENT_EXPORT void SetAllowExtendSecondaryEnd (const bool& bVal) { m_extendSecondaryEnd = bVal; }
    ...
    ROADRAILALIGNMENT_EXPORT size_t Compute (bvector<AlignmentIntersectionInfo>& potentialIntersections);

    ROADRAILALIGNMENT_EXPORT bool ComputeClosest (DPoint3dCR referencePt, AlignmentIntersectionInfoR info);
    ROADRAILALIGNMENT_EXPORT bool ComputeClosest(DPoint3dCR referencePt, const double maxDistance, AlignmentIntersectionInfoR info);
    // return the intersection that is at the closest station along the given alignment (primary or secondary)
    ROADRAILALIGNMENT_EXPORT bool ComputeClosestToStation(double referenceStation, bool alongPrimary, AlignmentIntersectionInfoR info);

public:
    ROADRAILALIGNMENT_EXPORT static AlignmentIntersectionPtr Create (AlignmentPairCP primary, AlignmentPairCP secondary);
