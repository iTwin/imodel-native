...
function showErrorAndExit(msg, alsoShowSupported) {
  console.log("ERROR - ECSchemaOps cannot be installed.");
  console.log(msg);
  if (alsoShowSupported) {
    console.log("ECSchemaOps runs on:");
...
function checkSupportedPlatform() {
  ...
  if (!knownPlatform)
    showErrorAndExit(`ECSchemaOps does not run on the ${process.platform} platform.`, true);

  if (!knownPlatform.architectures.find(a => a === process.arch))
    showErrorAndExit(`ECSchemaOps does not run on the ${process.arch} version of ${knownPlatform.description}. ECSchemaOps runs on ${knownPlatform.architectures.join()} only.`);
}
...
const installDir = path.join(os.tmpdir(), "install-ecschema-ops-native");
