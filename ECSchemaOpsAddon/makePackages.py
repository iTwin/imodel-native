...
# @param inputProductdir The path to the Product that contains the ingredients, e.g., D:\imodel02\out\Winx64\product\ECSchemaOpsAddon-Windows
...
# @param sourceDir The source directory, i.e., %SrcRoot%ECSchemaOpsAddon
...
def generate_package_for_platform(outdirParent, inputProductdir, nodeOS, nodeCPU, packageVersion, sourceDir):
    ...
    outputpackagename = 'ecschema-ops-' + nodeOS + '-' + nodeCPU
...
# Generate ecschema-ops-nodeaddonapi
# @param outdirParent The path to the output package's parent directory
# @param parentSourceDir The ECSchemaOpsAddon source directory, i.e., %SrcRoot%ECSchemaOpsAddon
# @param packageVersion The semantic version number for the generated package
# @return the full path to the generated package directory
def generate_imodeljs_native_platform_api(outdirParent, parentSourceDir, packageVersion):

    outputpackagedir = os.path.join(outdirParent, 'ecschema-ops')
    ...
    filesToCopy = ['installNativePlatform.js', 'loadNativePlatform.js']

    for fileToCopy in filesToCopy:
        shutil.copyfile(os.path.join(apiSourceDir, fileToCopy), os.path.join(outputpackagedir, fileToCopy))

    shutil.copyfile(os.path.join(os.environ['BuildContext'], 'Delivery', 'ECSchemaOpsNative.d.ts'), os.path.join(outputpackagedir, 'ECSchemaOpsNative.d.ts'))
    shutil.copyfile(os.path.join(os.environ['BuildContext'], 'Delivery', 'ECSchemaOpsNative.js'), os.path.join(outputpackagedir, 'ECSchemaOpsNative.js'))
