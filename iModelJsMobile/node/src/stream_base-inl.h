...
inline int StreamBase::Shutdown(v8::Local<v8::Object> req_wrap_obj) {
  ...
  if (msg != nullptr) {
    req_wrap_obj->Set(env->context(), env->error_string(), OneByteString(env->isolate(), msg)).Check();
    ClearError();
  }
...
inline void StreamReq::Done(int status, const char* error_str) {
  ...
  if (error_str != nullptr) {
    async_wrap->object()->Set(env->context(), env->error_string(),
                              OneByteString(env->isolate(), error_str)).Check();
  }
