...
This Example
--------------------------
This example demonstrates the use of Bitmap Viewports in Vortex
...
#include "..\include\BitmapTool.h"		// Draw to bitmap and save
#include "..\include\VortexRender.h"	// Vortex OpenGL rendering

// Main Entry Point
int main(int argc, char* argv[])
{
	VortexExampleApp example( std::string("Bitmap Example") );

	if (example.initializeApplication(argc, argv, false))
	{
		example.addTool(new FileTool);
		example.addTool(new ShaderTool);
		example.addTool(new BitmapTool);				// see BitmapToo.cpp for draw to bitmap code
