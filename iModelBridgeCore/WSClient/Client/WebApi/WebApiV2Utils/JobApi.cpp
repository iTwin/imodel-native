...
* @bsimethod
+--------------------------------------------------------------------------------------*/
AsyncTaskPtr<HttpJobResult> JobApi::ExecuteViaJob(HttpRequest request, WSInfoCR info, IWSRepositoryClient::JobOptionsPtr options, ICancellationTokenPtr ct) const
    {
...
AsyncTaskPtr<HttpJobResult> JobApi::ExecuteViaJob
    ...
    return requestTask->Then([=] (HttpResponse& httpResponse)
        {
        if (!isJobApiEnabled)
            {
...
ICancellationTokenPtr ct
    ...
    HttpRequest jobRequest = m_configuration->GetHttpClient().CreateRequest(jobUrl, "GET");
...
std::shared_ptr<HttpJobResult> finalResultOut
    ...
    return jobRequest.PerformAsync()->Then(thread, [=] (HttpResponse& jobResponse)
        {
...
* @bsimethod                                                julius.cepukenas   12/2017
+--------------------------------------------------------------------------------------*/
AsyncTaskPtr<HttpResponse> JobApi::DeleteJob(Utf8StringCR jobUrl, ICancellationTokenPtr ct) const
    {
    HttpRequest request = m_configuration->GetHttpClient().CreateRequest(jobUrl, "DELETE");
