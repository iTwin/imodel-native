...
* @bsimethod
+--------------------------------------------------------------------------------------*/
CacheNodeKey CachedResponseManager::SavePage(CachedResponseInfoCR info, uint64_t page, Utf8StringCR cacheTag, bool isPartial)
    {
    CacheNodeKey pageKey = FindPage(info, page);
    if (!pageKey.IsValid())
        {
        pageKey = InsertPage(info, page, cacheTag, isPartial);
        }
    else
        {
...
* @bsimethod
+--------------------------------------------------------------------------------------*/
CacheNodeKey CachedResponseManager::InsertPage(CachedResponseInfoCR info, uint64_t page, Utf8StringCR cacheTag, bool isPartial)
    {
...
* @bsimethod
+--------------------------------------------------------------------------------------*/
BentleyStatus CachedResponseManager::UpdatePage(ECInstanceId pageId, Utf8StringCR cacheTag, bool isPartial)
    {
    ...
    statement->BindBoolean(3, isPartial);
    statement->BindId(4, pageId);
...
BentleyStatus CachedResponseManager::SaveInfo(CachedResponseInfoR info)
    ...
    if (!info.IsCached())
        ...
        {
        return BE_SQLITE_OK == m_responseUpdater.Get().Update(ECDbHelper::ECInstanceIdFromJsonInstance(info.GetJsonData()), info.GetJsonData()) ? SUCCESS : ERROR;
        }
    }
...
BentleyStatus CachedResponseManager::InsertInfo(CachedResponseInfoR info)
    ...
    instance[CLASS_CachedResponseInfo_PROPERTY_Parent][ECJsonUtilities::json_navId()] = info.GetKey().GetParent().GetInstanceId().ToHexStr();
    instance[CLASS_CachedResponseInfo_PROPERTY_Holder][ECJsonUtilities::json_navId()] = info.GetKey().GetHolder().GetInstanceId().ToHexStr();
...
const InstanceCacheHelper::CachedInstances& instances
    ...
    CacheNodeKey pageKey = SavePage(info, page, cacheTag, instances.HasPartialInstances());
