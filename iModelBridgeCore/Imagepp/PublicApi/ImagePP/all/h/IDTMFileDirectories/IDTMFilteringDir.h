//:>--------------------------------------------------------------------------------------+
//:>
//:>     $Source: PublicApi/ImagePP/all/h/IDTMFileDirectories/IDTMFilteringDir.h $
//:>
...
public:
    struct Options : public UserOptions
        {
        _HDLLg virtual          ~Options                           () = 0;

        _HDLLg FilterType       GetType                            () const;

    protected:
        _HDLLg explicit         Options                            (FilterType              pi_Type);
    private:
    ...
    * Filtering accessors
    ...
    _HDLLg bool                 IsFiltered                         (TileID                  pi_ID) const;
    _HDLLg void                 SetFiltered                        (TileID                  pi_ID,
                                                                    bool                        pi_Filtered);
...
public:
    struct Options : public FilteringDir::Options
        {
        _HDLLg explicit         Options                            ();
    private:
        virtual bool            _SaveTo                            (FilteringDir&               pi_rDir) const override;
        };

    _HDLLg virtual              ~DumbFilteringHandler              ();

    _HDLLg static bool          IsCompatibleWith                   (const FilteringDir&         pi_rDir);

    _HDLLg static Ptr           CreateFrom                         (FilteringDir*               pi_pDir);
