...
//-----------------------------------------------------------------------------
// Quantize palette definition for R8G8B8
//-----------------------------------------------------------------------------
...
struct HRPQuantizedPaletteR8G8B8Node
    {
    // is the node a leaf
    bool  IsLeaf;
    ...
    // this is an octree, so there are 8 possible childs to each node
    struct HRPQuantizedPaletteR8G8B8Node* pChild[8];
...
class HRPQuantizedPaletteR8G8B8 : public HRPQuantizedPalette
    {
public:
...
protected:
    ...
    HRPQuantizedPaletteR8G8B8(const HRPQuantizedPaletteR8G8B8& pi_rObj);
    HRPQuantizedPaletteR8G8B8& operator=(const HRPQuantizedPaletteR8G8B8& pi_rObj);
...
private:
    ...
    typedef list<HRPQuantizedPaletteR8G8B8Node*> NodeList;
    ...
    uint32_t        GetColors( const struct HRPQuantizedPaletteR8G8B8Node*  pTree,
                               HRPPixelPalette*                             po_pPixelPalette,
                               uint32_t                                     EntryIndex,
                               HRPHistogram*                                po_pHistogram) const;

    void            PairLeaves(HRPQuantizedPaletteR8G8B8Node* pi_pNode);

    void            DeleteTree(struct HRPQuantizedPaletteR8G8B8Node* pi_pNode);
