//:>--------------------------------------------------------------------------------------+
//:>
//:>     $Source: PublicApi/ImagePP/all/h/HTGFFAttributeManager.hpp $
//:>
...
inline bool AttributeManager::Get  (AttributeID     pi_ID,
    ...
    {
    HSTATICASSERT(sizeof(uint32_t) == sizeof(uint32_t));
    return GetFile().GetField(pi_ID, reinterpret_cast<uint32_t*>(&po_rData));
    }
...
inline bool AttributeManager::Set  (AttributeID     pi_ID,
    ...
    {
    return GetFile().SetField(pi_ID, static_cast<uint32_t>(pi_rData));
    }
...
inline bool AttributeManager::Set  (AttributeID     pi_ID,
    ...
    {
    return GetFile().SetField(pi_ID, pi_rData.c_str());
    }
...
template<>
inline bool AttributeManager::Set  (AttributeID     pi_ID,
                                    const uint32_t*    pi_pData,
                                    size_t          pi_Size)
    {
    HSTATICASSERT(sizeof(uint32_t) == sizeof(uint32_t));
    return Set(pi_ID, reinterpret_cast<const uint32_t*>(pi_pData), pi_Size);
    }
