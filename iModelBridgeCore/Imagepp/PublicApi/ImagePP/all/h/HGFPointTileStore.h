//:>--------------------------------------------------------------------------------------+
//:>
//:>     $Source: PublicApi/ImagePP/all/h/HGFPointTileStore.h $
//:>
...
public:
    ...
    virtual HPMBlockID StoreNewBlock (POINT* DataTypeArray, size_t countData)
        ...
        if (!m_tileHandler->AddPoints(newNodeID, MyArray))
            ...
            throw HFCFileException("Unable to obtain file name ... IDTMFile::File API should be modified.");
            }
    ...
    virtual size_t StoreHeader (HGFPointNodeHeader<EXTENT>* header, HPMBlockID blockID)
        ...
        m_tileHandler->GetDir().SetResolution (ConvertBlockID(blockID), header->m_level);
...
public:
    ...
    virtual HPMBlockID StoreNewBlock (SPATIAL* DataTypeArray, size_t countData)
        ...
        std::transform(DataTypeArray, DataTypeArray + countData, ImagePP::AccumulateIter(FeatureTotalPointQty),
