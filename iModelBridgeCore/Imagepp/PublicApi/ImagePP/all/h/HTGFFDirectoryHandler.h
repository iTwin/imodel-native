//:>--------------------------------------------------------------------------------------+
//:>
//:>     $Source: PublicApi/ImagePP/all/h/HTGFFDirectoryHandler.h $
//:>
...
+---------------+---------------+---------------+---------------+---------------+------*/
class DirectoryHandler : public HFCShareableObject<DirectoryHandler>
    {
public:
    ...
    _HDLLg virtual              ~DirectoryHandler              () = 0;

    _HDLLg bool                 Save                           ();

protected:
    _HDLLg explicit             DirectoryHandler               ();
    ...
    _HDLLg AttributeManager&    AttributeMgr                   (Directory&                      pi_rDir);

    _HDLLg const PacketManagerBase&
                                PacketMgr                      (const Directory&                pi_rDir) const;
    _HDLLg PacketManagerBase&   PacketMgr                      (Directory&                      pi_rDir);
    ...
    _HDLLg SubDirManagerBase&   SubDirMgr                      (Directory&                      pi_rDir);
...
class SingleDirHandler : public SingleDirHandlerConcretePart
    {
private:
    typedef HFCPtr<DirT>  DirPtr;

public:
    // Provide these convenience typedefs
    typedef HFCPtr<HandlerT>    Ptr;
    typedef HFCPtr<HandlerT>    CPtr;
