...
public:
    ...
    /**----------------------------------------------------------------------------
     ...
     amount of DataType instances managed below this limit.
    -----------------------------------------------------------------------------*/
    HPMCountLimitedPool(HPMMemoryMgr* memoryManager, size_t countLimit)
        {
        m_countLimit = countLimit;
        m_totalUsedCount = 0;
    ...
    -----------------------------------------------------------------------------*/
    bool Allocate (size_t Count, const HPMCountLimitedPoolItem<DataType>* poolItem)
        {
    ...
    -----------------------------------------------------------------------------*/
    bool Reallocate (size_t NewCount, const HPMCountLimitedPoolItem<DataType>* poolItem)
        {
        ...
        memcpy((void*)newMemory, poolItem->GetMemory(), min (NewCount, poolItem->GetMemoryCount()) *sizeof(DataType));
        m_totalUsedCount -= poolItem->GetMemoryCount();

        if (m_memoryManager != NULL)
            m_memoryManager->FreeMemory (reinterpret_cast<HPMCountLimitedPool*>(poolItem->GetMemory()), poolItem->GetMemoryCount() * sizeof (DataType));
        else
    ...
    /**----------------------------------------------------------------------------
        ...
        pool management.
    -----------------------------------------------------------------------------*/
    bool Free (const HPMCountLimitedPoolItem<DataType>* poolItem)
        {
...
// end critical section
    ...
    /**----------------------------------------------------------------------------
        ...
        memory.
    -----------------------------------------------------------------------------*/
    bool RemoveItem (const HPMCountLimitedPoolItem<DataType>* poolItem)
        {
        if (poolItem->GetPoolManaged())
            {
// Critical section
