...
public:
    ...
    HGFDecompositionException()
    {
    }
    virtual         ~HGFDecompositionException()
    {
    }
    HGFDecompositionException&   operator=(const HGFDecompositionException& pi_rObj)
    {
    return *this;
    }
    HGFDecompositionException(const HGFDecompositionException& pi_rObj)
    {
    }
    };
...
/*---------------------------------------------------------------------------------**//**
    ...
    Do not make a method out of it since externally the HGF2DShape does not
    know of HGF2DPolySegment
+---------------+---------------+---------------+---------------+---------------+------*/
/*static*/ HGF2DShape* HGF2DPolygonOfSegments::CreateShapeFromAutoCrossingPolySegment(const HGF2DPolySegment& pi_rAutoCrossingPolySegment)
    {
    ...
    ----------------------------------------------------------------------------*/
    list<HFCPtr<HGF2DPolySegment> > ListOfPolySegments;
    ...
    list<HFCPtr<HGF2DPolySegment> >::iterator Itr = ListOfPolySegments.begin();
    list<HFCPtr<HGF2DPolySegment> >::iterator MasterItr;
    ...
    for ( ; !MasterFound && Itr != ListOfPolySegments.end(); ++Itr)
        ...
        HFCPtr<HGF2DPolygonOfSegments> pFirstPolygon = new HGF2DPolygonOfSegments(**Itr);
        ...
        for ( ; !MasterFound && SecondItr != ListOfPolySegments.end(); ++SecondItr)
            {
            // Create a polygon of segments
            HFCPtr<HGF2DPolygonOfSegments> pSecondPolygon = new HGF2DPolygonOfSegments(**SecondItr);

            // Check spatial relation of second to first
            if (HGF2DShape::S_IN == pFirstPolygon->CalculateSpatialPositionOf(*pSecondPolygon))
                {
                MasterFound = true;
                MasterItr = Itr;
                }
            else if (HGF2DShape::S_IN == pSecondPolygon->CalculateSpatialPositionOf(*pFirstPolygon))
                {
                MasterFound = true;
                MasterItr = SecondItr;
                }
    ...
    if (MasterFound)
        ...
        HAutoPtr<HGF2DHoledShape> pNewHoledShape(new HGF2DHoledShape(HGF2DPolygonOfSegments(**MasterItr)));
        ...
        for ( ; Itr != ListOfPolySegments.end(); ++Itr)
            ...
            if (Itr != MasterItr)
                {
                pNewHoledShape->AddHole(HGF2DPolygonOfSegments(**Itr));
                }
            }
        ...
        HAutoPtr<HGF2DComplexShape> pComplexShape(new HGF2DComplexShape());

        list<HFCPtr<HGF2DPolySegment> >::iterator Itr = ListOfPolySegments.begin();
        for ( ; Itr != ListOfPolySegments.end(); ++Itr)
            {
            pComplexShape->AddShape(HGF2DPolygonOfSegments(**Itr));
            }
...
//    if (pi_rSelfLiteExtent.IsPointOutterIn(MidPoint, pi_Tolerance))
    if (((X > XMin) || HDOUBLE_EQUAL(X, XMin, pi_Tolerance)) &&
        ...
        MyPartPosition = HGF2DShape::S_OUT;
    ...
    if (MyPartPosition == HGF2DShape::S_ON)
        {

        // We create a full fledged segment
        HGF2DSegment MySegment(pi_rStartPoint, pi_rEndPoint);
...
/** -----------------------------------------------------------------------------
    ...
    -----------------------------------------------------------------------------
*/
HGF2DPolygonOfSegments*  HGF2DPolygonOfSegments::AllocateParallelCopy(double pi_rOffset,
                                                                  HGF2DVector::ArbitraryDirection pi_DirectionToRight) const
    ...
    HGF2DLiteLine MyStartLine(m_PolySegment.GetStartPoint(), PerpendicularBearing);

    HAutoPtr<HGF2DPolySegment> pPolyParallel(m_PolySegment.AllocateParallelCopy(pi_rOffset, pi_DirectionToRight, &MyStartLine, &MyStartLine));

    // Make sure the polysegment closes
    pPolyParallel->AdjustEndPointTo(pPolyParallel->GetStartPoint());

    // Create new polygon of segments
    HAutoPtr<HGF2DPolygonOfSegments> pNewPolygon(new HGF2DPolygonOfSegments(*pPolyParallel));
...
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::Clip(const HGF2DRectangle& pi_rRectangle) const
    {
    HINVARIANTS;

    HGF2DShape*  pResultShape = NULL;
...
/** -----------------------------------------------------------------------------
    ...
    -----------------------------------------------------------------------------
*/
bool HGF2DPolygonOfSegments::RepresentsARectangle() const
    {
    HINVARIANTS;
    ...
    if (m_PolySegment.GetSize() == 5)
        ...
        for (; NextItr != m_PolySegment.m_Points.end() && AllSegmentsAreAligned ; ++NextItr)
            ...
            HGFBearing SegmentBearing = (HGF2DPosition(*NextItr) -
                                         HGF2DPosition(*Itr)).CalculateBearing();

            // Obtain trigonometric value of bearing
            double TrigoBearing = CalculateNormalizedTrigoValue(SegmentBearing.GetAngle());
...
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::Simplify()
    {
    HINVARIANTS;
...
/** -----------------------------------------------------------------------------
    ...
    @return A dynamically allocated HGF2DRectangle containing the rectangle
            equivalent to self.
    ...
    -----------------------------------------------------------------------------
*/
HGF2DRectangle* HGF2DPolygonOfSegments::GenerateCorrespondingRectangle() const
    {
    HINVARIANTS;
    ...
    HGF2DRectangle*  pNewRectangle = new HGF2DRectangle(GetExtent());
...
//-----------------------------------------------------------------------------
HGF2DPolygonOfSegments::HGF2DPolygonOfSegments(const HGF2DRectangle& pi_rRectangle)
    : HGF2DSimpleShape(),
      m_PolySegment(),
    ...
    // Append positions
    m_PolySegment.AppendPoint(HGF2DPosition(XMin, YMin));
    m_PolySegment.AppendPoint(HGF2DPosition(XMin, YMax));
    m_PolySegment.AppendPoint(HGF2DPosition(XMax, YMax));
    m_PolySegment.AppendPoint(HGF2DPosition(XMax, YMin));
    m_PolySegment.AppendPoint(HGF2DPosition(XMin, YMin));
...
//-----------------------------------------------------------------------------
HGF2DPolygonOfSegments::HGF2DPolygonOfSegments(const HGF2DPolySegment& pi_rPolySegment)
    : HGF2DSimpleShape(),
    ...
    HGF2DVector::SetAutoToleranceActive(m_PolySegment.IsAutoToleranceActive());
    HGF2DVector::SetTolerance(m_PolySegment.GetTolerance());
...
//-----------------------------------------------------------------------------
HGF2DPolygonOfSegments::HGF2DPolygonOfSegments(size_t  pi_BufferLength,
                                           double pi_aBuffer[])
    : HGF2DSimpleShape(),
      m_RotationDirectionUpToDate(false),
...
//-----------------------------------------------------------------------------
HGF2DPolygonOfSegments& HGF2DPolygonOfSegments::operator=(const HGF2DPolygonOfSegments& pi_rObj)
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
HFCPtr<HGF2DLinear> HGF2DPolygonOfSegments::GetLinear() const
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
HGF2DPosition HGF2DPolygonOfSegments::CalculateClosestPoint(const HGF2DPosition& pi_rPoint) const
    {
    HINVARIANTS;
...
// at specified point
//-----------------------------------------------------------------------------
bool HGF2DPolygonOfSegments::AreContiguousAt(const HGF2DVector& pi_rVector,
...
//-----------------------------------------------------------------------------
HGF2DLiteExtent HGF2DPolygonOfSegments::GetExtent() const
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::Scale(double pi_ScaleFactor, const HGF2DPosition& pi_rScaleOrigin)
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::Rotate(double pi_Angle, const HGF2DPosition& pi_rRotationOrigin)
    {
    HINVARIANTS;
...
// around the given location
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::Scale(double              pi_ScaleFactorX,
    ...
    for(size_t Index = 0 ; Index < m_PolySegment.GetSize() ; Index++)
        {
        m_PolySegment.m_Points[Index][HGF2DPosition::X] += pi_ScaleFactorX * (ScaleOriginX - m_PolySegment.GetPoint(Index)[HGF2DPosition::X]);
        m_PolySegment.m_Points[Index][HGF2DPosition::Y] += pi_ScaleFactorY * (ScaleOriginY - m_PolySegment.GetPoint(Index)[HGF2DPosition::Y]);
        }
...
//-----------------------------------------------------------------------------
double HGF2DPolygonOfSegments::CalculateRawArea() const
    {
    HINVARIANTS;
...
/** -----------------------------------------------------------------------------
    ...
    -----------------------------------------------------------------------------
*/
bool HGF2DPolygonOfSegments::IsConvex() const
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
HGFBearing HGF2DPolygonOfSegments::CalculateBearing(const HGF2DPosition& pi_rPositionPoint,
    ...
    if ((pi_rPositionPoint.IsEqualTo(m_PolySegment.GetPoint(m_PolySegment.GetSize() - 1),  GetTolerance())) &&
        (pi_Direction == HGF2DVector::ALPHA))
        {
        // The given point is the end point of polygon ... special processing

        // Obtain bearing from last linear in complex
        HGF2DSegment TempSegment(m_PolySegment.GetPoint(m_PolySegment.GetSize() - 2),
                               m_PolySegment.GetPoint(m_PolySegment.GetSize() - 1));
...
//-----------------------------------------------------------------------------
double HGF2DPolygonOfSegments::CalculateAngularAcceleration(const HGF2DPosition& pi_rPositionPoint,
                                                                          HGF2DVector::ArbitraryDirection pi_Direction) const
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
bool HGF2DPolygonOfSegments::IsPointOn(const HGF2DPosition& pi_rPoint,
                                     HGF2DVector::ExtremityProcessing pi_ExtremitityProcessing,
                                     double pi_Tolerance) const
    {
...
//-----------------------------------------------------------------------------
bool HGF2DPolygonOfSegments::IsPointIn(const HGF2DPosition& pi_rPoint, double pi_Tolerance) const
    {
    HINVARIANTS;
    ...
    HGF2DShape::SpatialPosition ThePointSpatialPosition;

    ThePointSpatialPosition = CalculateSpatialPositionOfPositionSameUnits(pi_rPoint, Tolerance);

    return(ThePointSpatialPosition == HGF2DShape::S_IN);
...
//-----------------------------------------------------------------------------
HGF2DShape::SpatialPosition HGF2DPolygonOfSegments::CalculateSpatialPositionOfPositionSameUnits(const HGF2DPosition& pi_rPoint,
        double pi_Tolerance) const
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
size_t HGF2DPolygonOfSegments::ObtainContiguousnessPoints(const HGF2DVector& pi_rVector,
    ...
    for(size_t Index = 1 ; Index < m_PolySegment.GetSize() ; Index++)
        {
        // Create the segment
        HGF2DSegment    CurrentSegment(m_PolySegment.GetPoint(Index - 1), m_PolySegment.GetPoint(Index));
        ...
        if (CurrentSegment.AreContiguous(pi_rVector))
            ...
            if (CurrentSegment.ObtainContiguousnessPoints(pi_rVector, &TempPoints) != 0)
                ...
                HGF2DPositionCollection::const_iterator MyPointIterator = TempPoints.begin();
...
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::ObtainContiguousnessPointsAt(const HGF2DVector& pi_rVector,
    ...
    if ((*po_pFirstContiguousnessPoint).IsEqualTo(m_PolySegment.GetPoint(0), GetTolerance()))
        {
        // The contiguousness region may possibly extent in last component linear ...

        // We obtain all contiguousness points
        HGF2DPositionCollection     AllContiguousnessPoints;
...
//-----------------------------------------------------------------------------
bool HGF2DPolygonOfSegments::Crosses(const HGF2DVector& pi_rVector) const
    {
    HINVARIANTS;
    ...
    if (m_PolySegment.GetSize() > 0)
        {
        if ((pi_rVector.GetClassID() == HGF2DPolygonOfSegments::CLASS_ID) ||
            (pi_rVector.GetClassID() == HGF2DRectangle::CLASS_ID))
            {
            //TR 217810
            HAutoPtr<HGF2DPolygonOfSegments> pPolygonOfSegments;

            if (pi_rVector.GetClassID() == HGF2DRectangle::CLASS_ID)
                {
                pPolygonOfSegments = new HGF2DPolygonOfSegments((const HGF2DRectangle&) pi_rVector);
                }
            else
                {
                pPolygonOfSegments = new HGF2DPolygonOfSegments((const HGF2DPolygonOfSegments&) pi_rVector);
                }

            Answer = CrossesPolygonOfSegments(*pPolygonOfSegments);
            }
        else
            {
            ...
            if (!Answer)
                {
                HGF2DPosition   TheEndPoint(m_PolySegment.GetPoint(0));
...
//-----------------------------------------------------------------------------
// CrossesPolygonOfSegments
// Test if two polygons cross.
//-----------------------------------------------------------------------------
bool HGF2DPolygonOfSegments::CrossesPolygonOfSegments(const HGF2DPolygonOfSegments& pi_rPolygon) const
    {
    bool Answer = false;
    ...
    if (GetExtent().OutterOverlaps(pi_rPolygon.GetExtent(), GetTolerance()))
        ...
        HGF2DShape::SpatialPosition LastSelfPosition = S_ON;
        HGF2DShape::SpatialPosition LastGivenPosition = S_ON;
        HGF2DShape::SpatialPosition TempPosition;
                ...
                if ((LastSelfPosition == S_IN && TempPosition == S_OUT) ||
                    ...
                    if (++SelfItr != m_PolySegment.m_Points.end())
                        ...
                        if ((LastSelfPosition == S_IN && TempPosition == S_OUT) ||
                            ...
                            {
                            // Note the current point position
                            if (LastSelfPosition == S_ON)
                                LastSelfPosition = TempPosition;
                            }
                        }
        ...
        if (!Answer && !(LastSelfPosition == S_ON && LastGivenPosition == S_ON) &&
            ...
            Answer = m_PolySegment.CrossesPolySegment(pi_rPolygon.m_PolySegment);
            ...
            if (!Answer)
                {
                HGF2DPosition TheEndPoint(pi_rPolygon.m_PolySegment.GetPoint(0));
...
//-----------------------------------------------------------------------------
size_t HGF2DPolygonOfSegments::Intersect(const HGF2DVector& pi_rVector, HGF2DPositionCollection* po_pCrossPoints) const
    {
    HINVARIANTS;
    ...
    if (m_PolySegment.GetSize() > 0)
        ...
        HGF2DPosition   AutoClosingPoint(m_PolySegment.GetPoint(0));
    ...
    switch (pi_Operation)
        ...
        case HGF2DPolygonOfSegments::INTERSECT:
            ...
            if (pi_rListOfShapes.size() == 0)
                ...
                if (pi_rListOfShapes.size() == 0)
                    ...
                    // we take either one
                    // It may also occur that the shapes are disjoint but very closely flirting
                    // Resolution of the two cases is performed by checking if all points are on.
                    if (AllOnPoints)
                        pi_rListOfShapes.push_back(static_cast<HGF2DPolygonOfSegments*>(pi_rPolygon.Clone()));
                    else
                        pi_rListOfShapes.push_back(new HGF2DVoidShape());
                    }
                }
            break;
        ...
        case HGF2DPolygonOfSegments::UNION:
            ...
            if (pi_rListOfShapes.size() == 0)
                ...
                if (SelfSpatialPosition == HGF2DShape::S_IN)
                    {
                    ...
                    pi_rListOfShapes.push_back(static_cast<HGF2DPolygonOfSegments*>(Clone()));
                    }
                }
...
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::DifferentiateFromShape(const HGF2DShape& pi_rShape) const
    {
    HINVARIANTS;
    ...
    // We separate the process depending on the complexity of given shape
        ...
        {
        if(pi_rShape.GetShapeType() == HGF2DRectangle::CLASS_ID)
            pMyResultShape = DifferentiateFromPolygon(HGF2DPolygonOfSegments(*(HGF2DRectangle*)(&pi_rShape)));
        else if (pi_rShape.GetShapeType() == HGF2DPolygonOfSegments::CLASS_ID)
            pMyResultShape = DifferentiateFromPolygon((*(HGF2DPolygonOfSegments*)(&pi_rShape)));
        else
            pMyResultShape = pi_rShape.DifferentiateShape(*this);
        }
...
//-----------------------------------------------------------------------------
// DifferentiateFromPolygon
// PRIVATE METHOD
// This method create a new shape as the difference between self and given.
// The coordinate system of both shapes must be the same
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::DifferentiateFromPolygon(const HGF2DPolygonOfSegments& pi_rPolygon) const
    {
    HINVARIANTS;
    ...
    if (!GetExtent().InnerOverlaps(pi_rPolygon.GetExtent(), MIN(GetTolerance(), pi_rPolygon.GetTolerance())))
        ...
        if (pi_rPolygon.RepresentsARectangle())
            pMyResultShape = (HGF2DShape*)pi_rPolygon.GenerateCorrespondingRectangle();
        else
            pMyResultShape = static_cast<HGF2DShape*>(pi_rPolygon.Clone());
        }
    else
        {
        ...
        if (InteractsWith(pi_rPolygon, &SelfPolyPoints, &GivenPolyPoints, HGF2DPolygonOfSegments::DIFFFROM))
            {
            // The shape interact with each other ... long process
            pMyResultShape = DifferentiateFromCrossingPolygon(pi_rPolygon, SelfPolyPoints, GivenPolyPoints);
            }
        else
            {
            // Even though their extent is not disjoint, they may still be disjoint
            // We therefore evaluate their spatial position
            HGF2DShape::SpatialPosition     TheGivenPosition = CalculateSpatialPositionOfNonCrossingPolygonOfSegments(pi_rPolygon);
            ...
            if ((TheGivenPosition == HGF2DShape::S_IN) || (TheGivenPosition == HGF2DShape::S_ON))
                ...
                HGF2DShape::SpatialPosition     TheSelfPosition = pi_rPolygon.CalculateSpatialPositionOfNonCrossingPolygonOfSegments(*this);
                ...
                if (TheSelfPosition == HGF2DShape::S_OUT)
                    ...
                    HGF2DHoledShape* pMyResultHoledShape = new HGF2DHoledShape(pi_rPolygon);
...
//-----------------------------------------------------------------------------
// DifferentiateFromCrossingPolygon
// PRIVATE METHOD
// This method create a new shape as the difference between self and given.
// The coordinate system of both shapes must be the same
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::DifferentiateFromCrossingPolygon(const HGF2DPolygonOfSegments& pi_rPolygon,
                                                                 const HGF2DPositionCollection& pi_rPoly1,
    ...
    if (MyListOfPolygons.size() == 0)
        ...
        pMyResultShape = new HGF2DComplexShape(MyListOfPolygons);
        }
    else
        {
        pMyResultShape = static_cast<HGF2DShape*>((*MyListOfPolygons.begin())->Clone());
        }

    // Destroy list
    HGF2DShape::HoleList::iterator    MyIterator = MyListOfPolygons.begin();
...
// The coordinate system of both shapes must be the same
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::DifferentiateShape(const HGF2DShape& pi_rShape) const
    {
    HINVARIANTS;
    ...
    // We separate the process depending on the complexity of given shape
        ...
        {
        if (pi_rShape.GetShapeType() == HGF2DRectangle::CLASS_ID)
            pMyResultShape = DifferentiatePolygon(HGF2DPolygonOfSegments(*(HGF2DRectangle*)(&pi_rShape)));
        else if (pi_rShape.GetShapeType() == HGF2DPolygonOfSegments::CLASS_ID)
            pMyResultShape = DifferentiatePolygon((*(HGF2DPolygonOfSegments*)(&pi_rShape)));
        else
            pMyResultShape = pi_rShape.DifferentiateFromShape(*this);
        }
...
//-----------------------------------------------------------------------------
// DifferentiatePolygon
// PRIVATE METHOD
// This method create a new shape as the difference between self and given.
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::DifferentiatePolygon(const HGF2DPolygonOfSegments& pi_rPolygon) const
    {
    HINVARIANTS;
    ...
    if (!GetExtent().InnerOverlaps(pi_rPolygon.GetExtent(), MIN(GetTolerance(), pi_rPolygon.GetTolerance())))
        ...
        if (InteractsWith(pi_rPolygon, &SelfPolyPoints, &GivenPolyPoints, HGF2DPolygonOfSegments::DIFF))
            {
            // The shape intersect each other ... long process
            pMyResultShape = DifferentiateCrossingPolygon(pi_rPolygon, SelfPolyPoints, GivenPolyPoints);
            }
        else
            {
            // Even though their extent is not disjoint, they may still be disjoint
            ...
            if (TheGivenPosition == HGF2DShape::S_ON)
                ...
                HGF2DHoledShape* pMyResultHoledShape = new HGF2DHoledShape(*this);
...
//-----------------------------------------------------------------------------
// DifferentiateCrossingPolygon
// PRIVATE METHOD
// This method create a new shape as the difference between self and given.
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::DifferentiateCrossingPolygon(const HGF2DPolygonOfSegments& pi_rPolygon,
                                                             const HGF2DPositionCollection& pi_rPoly1,
                                                             const HGF2DPositionCollection& pi_rPoly2) const
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::IntersectShape(const HGF2DShape& pi_rShape) const
    {
    HINVARIANTS;
    ...
    // We separate the process depending on the complexity of given shape
        ...
        {
        if (pi_rShape.GetShapeType() == HGF2DRectangle::CLASS_ID)
            pMyResultShape = IntersectPolygon(HGF2DPolygonOfSegments(*(HGF2DRectangle*)(&pi_rShape)));
        else if (pi_rShape.GetShapeType() == HGF2DPolygonOfSegments::CLASS_ID)
            pMyResultShape = IntersectPolygon((*(HGF2DPolygonOfSegments*)(&pi_rShape)));
        else
            pMyResultShape = pi_rShape.IntersectShape(*this);
        }
...
// The coordinate system of both shapes must be the same
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::IntersectPolygon(const HGF2DPolygonOfSegments& pi_rPolygon) const
    {
    HINVARIANTS;
    ...
    if (!GetExtent().InnerOverlaps(pi_rPolygon.GetExtent(), MIN(GetTolerance(), pi_rPolygon.GetTolerance())))
        ...
        if (InteractsWith(pi_rPolygon, &SelfPolyPoints, &GivenPolyPoints, HGF2DPolygonOfSegments::INTERSECT, true, &ContiguousInteraction))
            {
            // The shape intersect each other ... long process
            // We append contiguousness points if any
            pMyResultShape = IntersectCrossingPolygon(pi_rPolygon, SelfPolyPoints, GivenPolyPoints);
            }
        else
            {
            // Even though their extent is not disjoint, they may still be disjoint
            // We therefore evaluate their spatial position
            HGF2DShape::SpatialPosition     TheGivenPosition;
            ...
            if (ContiguousInteraction)
                ...
                TheGivenPosition = CalculateSpatialPositionOfNonCrossingPolygonOfSegments(pi_rPolygon);
            ...
            if ((TheGivenPosition == HGF2DShape::S_IN) || (TheGivenPosition == HGF2DShape::S_ON))
            ...
            else if (TheGivenPosition == HGF2DShape::S_OUT)
                {
                // The given shape is OUT, but this does not imply that self is also OUT ... check
                HGF2DShape::SpatialPosition     TheSelfPosition;
                if (ContiguousInteraction)
                    TheSelfPosition = pi_rPolygon.CalculateSpatialPositionOf(*this);
                else
                    TheSelfPosition = pi_rPolygon.CalculateSpatialPositionOfNonCrossingPolygonOfSegments(*this);
                ...
                else if (TheSelfPosition == HGF2DShape::S_IN)
                    {
                    // Self cannot be PARTIALY_IN nor ON the given
                    // (since the given is not PARTIALY_IN nor ON self), then
...
//-----------------------------------------------------------------------------
// IntersectCrossingPolygon
// PRIVATE METHOD
// This method create a new shape as the intersection between self and given.
// The coordinate system of both shapes must be the same
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::IntersectCrossingPolygon(const HGF2DPolygonOfSegments& pi_rPolygon,
                                                         const HGF2DPositionCollection& pi_rPoly1,
                                                         const HGF2DPositionCollection& pi_rPoly2) const
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::UnifyShape(const HGF2DShape& pi_rShape) const
    {
    HINVARIANTS;
    ...
    // We separate the process depending on the complexity of given shape
        ...
        {
        if (pi_rShape.GetShapeType() == HGF2DRectangle::CLASS_ID)
            pMyResultShape = UnifyPolygon(HGF2DPolygonOfSegments(*(HGF2DRectangle*)(&pi_rShape)));
        else if (pi_rShape.GetShapeType() == HGF2DPolygonOfSegments::CLASS_ID)
            pMyResultShape = UnifyPolygon((*(HGF2DPolygonOfSegments*)(&pi_rShape)));
        else
            pMyResultShape = pi_rShape.UnifyShape(*this);
        }
...
// This method create a new shape as the union between self and given.
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::UnifyPolygon(const HGF2DPolygonOfSegments& pi_rPolygon) const
    {
    HINVARIANTS;
    ...
    if (!GetExtent().OutterOverlaps(pi_rPolygon.GetExtent(), MIN(GetTolerance(), pi_rPolygon.GetTolerance())) && !AreContiguous(pi_rPolygon))
        ...
        if (InteractsWith(pi_rPolygon, &SelfPolyPoints, &GivenPolyPoints, HGF2DPolygonOfSegments::UNION))
            {
            // The shape intersect each other ... long process
            // We append contiguousness points if any
            pMyResultShape = UnifyCrossingPolygon(pi_rPolygon, SelfPolyPoints, GivenPolyPoints);
            }
        else
            {
...
//-----------------------------------------------------------------------------
// UnifyCrossingPolygon
// PRIVATE METHOD
// This method create a new shape as the union between self and given.
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::UnifyCrossingPolygon(const HGF2DPolygonOfSegments& pi_rPolygon,
                                                     const HGF2DPositionCollection& pi_rPoly1,
                                                     const HGF2DPositionCollection& pi_rPoly2) const
    {
    HINVARIANTS;
...
//    HPRECONDITION(Crosses(pi_rPolygon) || AreContiguous(pi_rPolygon));
    ...
    if (SpecialProcessing)
        ...
        if (MyListOfPolygons.size() > 1)
            ...
            HGF2DShape::SimpleShapeList::iterator MyShapeIterator = MyListOfPolygons.begin();

            // The loop should never go past the end of the list
            while (!Found && MyShapeIterator != MyListOfPolygons.end())
            	{
                HGF2DShape::SimpleShapeList::iterator MyOtherShapeIterator = MyListOfPolygons.begin();
            ...
            if (Found)
                ...
                // Create holed shape
                HGF2DHoledShape* pMyResultHoledShape = new HGF2DHoledShape(**MyShapeIterator);

                // Add all holes and detroy shapes at the same time
                HGF2DShape::SimpleShapeList::iterator MyFinalShapeIterator = MyListOfPolygons.begin();
...
void HGF2DPolygonOfSegments::SuperScan(const HGF2DPolygonOfSegments&  pi_rGiven,
    ...
    *po_pAllOn = false;
    ...
    HGF2DLiteExtent     MySelfExtent = GetExtent();
    HGF2DLiteExtent     MyGivenExtent = pi_rGiven.GetExtent();
    ...
    do
        ...
        if ((MyPartPosition == HGF2DShape::S_OUT && pi_WantInPtsOfShape1) ||
            ...
            HGF2DPolygonOfSegments* pMyNewPoly = new HGF2DPolygonOfSegments();
...
//                             pi_rGiven);
            ...
            if (!IrregularShapes)
                ...
                if (pMyNewPoly->m_PolySegment.m_Points.size() > 2)
                    ...
                    if (pMyNewPoly->m_PolySegment.m_Points[0] != CurrentPoint) // (pMyNewPoly->m_PolySegment.GetPoint(0) != pMyNewPoly->m_PolySegment.GetPoint(m_PolySegment.GetSize() - 1))
                        {
    ...
    if (IrregularShapes || pi_rListOfShapes.size()== 0)
        ...
        HGF2DShape::SimpleShapeList::iterator Itr;
            ...
            do
                ...
                CurrentPoint = (*(apPoints[ShapeIndex]))[Index];
                    ...
                    if (((Flags[ShapeIndex][Index] == USED) && !CurrentPoint.IsEqualTo(MyStartPoint, Tolerance))
                        || ((MyPartPosition == HGF2DShape::S_OUT && PolyIn[ShapeIndex]) ||
                            (MyPartPosition == HGF2DShape::S_IN && !PolyIn[ShapeIndex])))
                ...
                if (CurrentPoint.IsEqualTo(MyStartPoint, Tolerance))
                    ...
                    if (ShapeIndex == StartShape && StartIndex == Index)
                        ...
                        if (((Flags[ShapeIndex][Index3] == USED) && !CurrentPoint3.IsEqualTo(MyStartPoint, Tolerance))
                            ...
                            StopLoop = (MyPartPosition == HGF2DShape::S_ON);
...
//-----------------------------------------------------------------------------
bool HGF2DPolygonOfSegments::InteractsWithSameUnits(const HGF2DPolygonOfSegments& pi_rPolygon,
    ...
    if ((SelfPolyCW && CalculateRotationDirection() == HGF2DSimpleShape::CW) ||
        (!SelfPolyCW && CalculateRotationDirection() == HGF2DSimpleShape::CCW))
        {
    ...
    if ((GivenPolyCW && pi_rPolygon.CalculateRotationDirection() == HGF2DSimpleShape::CW) ||
        (!GivenPolyCW && pi_rPolygon.CalculateRotationDirection() == HGF2DSimpleShape::CCW))
        {
...
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::InteractsWithSameUnitsContiguousProcessing(
    ...
    if (!(ContiguousnessPoints[0].IsEqualTo(ContiguousnessPoints[1], Tolerance)))
        ...
        if (((*PrevSelfItr) - (ContiguousnessPoints[0])).CalculateLength() <
            ((*PrevSelfItr) - (ContiguousnessPoints[1])).CalculateLength())
            {
        ...
        // For the given segment, the order of contiguousness points is unknown
        // it must be determined
        if (((*PrevGivenItr) - (ContiguousnessPoints[0])).CalculateLength() <
            ((*PrevGivenItr) - (ContiguousnessPoints[1])).CalculateLength())
            {
...
//-----------------------------------------------------------------------------
bool HGF2DPolygonOfSegments::AreAdjacent(const HGF2DVector& pi_rVector) const
    {
    HINVARIANTS;
...
// This method checks if the polygon is contiguous with given vector.
//-----------------------------------------------------------------------------
bool HGF2DPolygonOfSegments::AreContiguous(const HGF2DVector& pi_rVector) const
    {
...
// in text format
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::PrintState(ostream& po_rOutput) const
    {
#ifdef __HMR_PRINTSTATE

    HGF2DSimpleShape::PrintState(po_rOutput);

    HDUMP0("BEGIN Dumping a HGF2DPolygonOfSegments object\n");
    po_rOutput << "BEGIN Dumping a HGF2DPolygonOfSegments object" << endl;
    ...
    HDUMP0("END (HGF2DPolygonOfSegments)\n");
    po_rOutput << "END (HGF2DPolygonOfSegments)" << endl;
#endif
    }
...
// segments
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::Drop(HGF2DPositionCollection* po_pPoints,
...
// active
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::ResetTolerance()
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::SetTolerance(double pi_Tolerance)
    {
    HINVARIANTS;
...
// This method sets the tolerance for polysegment
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::SetStrokeTolerance(const HFCPtr<HGFLiteTolerance> & pi_Tolerance)
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::SetAutoToleranceActive(bool pi_ActiveAutoTolerance)
    {
    HINVARIANTS;
    ...
    // Set tolerance to polysegment
    HGF2DVector::SetAutoToleranceActive(pi_ActiveAutoTolerance);
...
//-----------------------------------------------------------------------------
// CalculateSpatialPositionOfPolygonOfSegments
// PRIVATE
// This method calculates the spatial position of current polygon
// compared to given
//-----------------------------------------------------------------------------
HGF2DShape::SpatialPosition HGF2DPolygonOfSegments::CalculateSpatialPositionOfPolygonOfSegments(const HGF2DPolygonOfSegments& pi_rPolygon) const
    {
    HINVARIANTS;
    ...
    if (GetExtent().OutterOverlaps(pi_rPolygon.GetExtent(), MIN(GetTolerance(), pi_rPolygon.GetTolerance())))
        ...
        if (CrossesPolygonOfSegments(pi_rPolygon))
            {
            ...
            ThePosition = CalculateSpatialPositionOfNonCrossingPolygonOfSegments(pi_rPolygon);
            }
        }
...
//-----------------------------------------------------------------------------
// CalculateSpatialPositionOfNonCrossingPolygonOfSegments
// PRIVATE
// This method calculates the spatial position of current polygon
// compared to given
//-----------------------------------------------------------------------------
HGF2DShape::SpatialPosition HGF2DPolygonOfSegments::CalculateSpatialPositionOfNonCrossingPolygonOfSegments(const HGF2DPolygonOfSegments& pi_rPolygon) const
    {
    HINVARIANTS;
    ...
    if (GetExtent().OutterOverlaps(pi_rPolygon.GetExtent(), Tolerance))
        ...
        for (ThePosition = HGF2DShape::S_ON; Itr != pi_rPolygon.m_PolySegment.m_Points.end() && ThePosition == HGF2DShape::S_ON ; ++Itr)
        ...
        if (ThePosition == HGF2DShape::S_ON)
            ...
            // Obtain the position of each point until IN or OUT is obtained
            HGF2DPositionCollection::const_iterator Itr = m_PolySegment.m_Points.begin();
            for (; Itr != m_PolySegment.m_Points.end() && ThePosition == HGF2DShape::S_ON ; ++Itr)
            ...
            if (ThePosition != HGF2DShape::S_ON)
                ...
                ThePosition = (ThePosition == HGF2DShape::S_OUT ? HGF2DShape::S_IN : HGF2DShape::S_OUT);
                }
            else
                {
                ...
                if (HDOUBLE_EQUAL(SelfArea, GivenArea, GetTolerance() * GetTolerance()))
                    ...
                    ThePosition = (SelfArea > GivenArea ? HGF2DShape::S_IN : HGF2DShape::S_OUT);
                }
            }
        }
...
// CalculateSpatialPositionOfPositionSegment2
//-----------------------------------------------------------------------------
HGF2DShape::SpatialPosition HGF2DPolygonOfSegments::CalculateSpatialPositionOfPositionSegment(const HGF2DPosition& pi_rPoint,
        const HGF2DPosition& pi_rStartPoint,
        const HGF2DPosition& pi_rEndPoint,
        double              pi_Tolerance) const
    {
...
//-----------------------------------------------------------------------------
HGF2DPolygonOfSegments::PointUsage* HGF2DPolygonOfSegments::InsertAutoContiguousPoints(HGF2DPolySegment& pio_rPolySegment) const
    {
    // Allocate iterators
                ...
                if (Result)
                    ...
                    if (TheOtherLiteSegment.AreContiguous(TheLiteSegment))
                        ...
                        if (!(ContiguousnessPoints[0].IsEqualTo(ContiguousnessPoints[1], Tolerance)))
                            ...
                            // it must be determined
                            if (((*PrevItr) - (ContiguousnessPoints[0])).CalculateLength() <
                                ((*PrevItr) - (ContiguousnessPoints[1])).CalculateLength())
                                {
...
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::InsertAutoFlirtPoints(HGF2DPositionCollection& pio_rPoints) const
    {
    HINVARIANTS;
...
//-----------------------------------------------------------------------------
bool HGF2DPolygonOfSegments::ChangeToShape(const HGF2DPolygonOfSegments* const* apPoly,
                                           const HGF2DPositionCollection**  apPoints,
                                           HGF2DLiteExtent* PolyExtents,
                                           PointUsage** Flags,
                                           bool* PolyIn,
    ...
    do
        ...
        if (Index < (apPoints[ShapeIndex]->size() - 1))
            ...
            PointIsCorrect = ((OnAccepted) || (MyPartPosition != HGF2DShape::S_ON)) &&
                             (!((MyPartPosition == HGF2DShape::S_OUT && PolyIn[ShapeIndex]) ||
                                (MyPartPosition == HGF2DShape::S_IN && !PolyIn[ShapeIndex])));
...
//-----------------------------------------------------------------------------
void HGF2DPolygonOfSegments::Rasterize(HGFScanLines& pio_rScanlines) const
    {
...
// @bsimethod                                                   2014/06
//-----------------------------------------------------------------------------
HFCPtr<HGF2DShape> HGF2DPolygonOfSegments::AllocTransformDirect(const HGF2DTransfoModel& pi_rModel) const
    {
    HINVARIANTS;
    ...
    if (IsEmpty())
        ...
        {
        pResultShape = new HGF2DPolygonOfSegments(*this);
        }
    else
        {
        HFCPtr<HGF2DLinear> pMyPrimaryResultLinear = static_cast<HGF2DLinear*>(m_PolySegment.AllocPolySegmentTransformDirect(pi_rModel));
        ...
        HFCPtr<HGF2DPolySegment> pMyPolySegment = static_cast<HGF2DPolySegment*>(&*(pMyPrimaryResultLinear));

        // Check if there are at least 3 segments in result. Removal of points can occur as a result
        // of high or low scale factor
        if (pMyPolySegment->GetSize() < 3)
            ...
            HGF2DPosition   AreaOrigin = pMyPolySegment->GetPoint(pMyPolySegment->GetSize() -2);
            ...
            if (HDOUBLE_EQUAL(fabs(pMyPolySegment->CalculateRayArea(AreaOrigin)), 0.0,
                ...
                if (pMyPolySegment->IsAutoContiguous())
                    ...
                    // We have a special case where before transformation two lines
                    ...
                    pResultShape = AllocateComplexShapeFromAutoContiguousPolySegment(*pMyPolySegment);

                    }
                // We must now make sure if the polysegment autocrosses itself. This will typically occur
                // in the rare cases we have a model with a vanishing line and the shape passes through
                // this vanishing line.
                    ...
                    pResultShape = HGF2DPolygonOfSegments::CreateShapeFromAutoCrossingPolySegment(*pMyPolySegment);
                    }
                else
                    {
                    // Result linear is valid ...
                    // we generate a polygon with it and return it
                    pResultShape = new HGF2DPolygonOfSegments(*pMyPolySegment);
                    }
                }
            }

        // &&AR Is this pertinent and valid ... should not the stroke tolerance be scaled locally.
...
//-----------------------------------------------------------------------------
HGF2DShape* HGF2DPolygonOfSegments::AllocateComplexShapeFromAutoContiguousPolySegment(const HGF2DPolySegment& pi_rPolySegment) const
    {
    // The provided polysegment MUST be autocontiguous and this autocontiguousness
    // must not be the result of contiguousness needles
    HDEBUGCODE(HGF2DPolySegment TempPolySegment(pi_rPolySegment));
    ...
    HGF2DPolySegment WorkPolySegment(pi_rPolySegment);
    ...
    do
        ...
        HGF2DPolySegment NewPolySegment;
        ...
        pResultComplex->AddShape(HGF2DPolygonOfSegments(NewPolySegment));
    ...
    if (pResultComplex->GetShapeList().size() == 1)
        ...
        pResultShape = static_cast<HGF2DShape*>((*pResultComplex->GetShapeList().begin())->Clone());
        }
