...
// in text format
//-----------------------------------------------------------------------------
void HVE2DVector::PrintState(ostream& po_rOutput) const
    {
#ifdef __HMR_PRINTSTATE
    po_rOutput << "BEGIN Dumping a HVE2DVector object" << endl;
    HDUMP0("BEGIN Dumping a HVE2DVector object\n");
    ...
    HDUMP0(DumString);
    HDUMP0("\n");
    po_rOutput << "END (HVE2DVector)" << endl;
    HDUMP0("END (HVE2DVector)\n");
#endif
    }
...
// linear
//-----------------------------------------------------------------------------
HGFBearing HVE2DVector::CalculatePerpendicularBearingAt(const HGF2DLocation& pi_rPoint,
    ...
    HGFBearing AlphaBearing = CalculateBearing(pi_rPoint, HVE2DVector::ALPHA);
    HGFBearing BetaBearing = CalculateBearing(pi_rPoint, HVE2DVector::BETA);

    HGFBearing ResultBearing;
    double AngleToApply;

    if (pi_DirectionToRight == HVE2DVector::ALPHA)
        {
...
/** -----------------------------------------------------------------------------
    ...
    -----------------------------------------------------------------------------
*/
bool HVE2DVector::IsConnectedBy(const HVE2DLinear& pi_rLinear) const
    {
    bool   Answer;

    // Check if they have the same coordinate system
    if (GetCoordSys() == pi_rLinear.GetCoordSys())
        Answer = (IsPointOnSCS(pi_rLinear.GetStartPoint()) ||
                  IsPointOnSCS(pi_rLinear.GetEndPoint()));
    else
        Answer = (IsPointOn(pi_rLinear.GetStartPoint()) || IsPointOn(pi_rLinear.GetEndPoint()));
...
/** -----------------------------------------------------------------------------
    ...
    @param pi_rObject Constant reference to a HVE2DVector to find cross points with.

    @param pi_rTestPoint A constant reference to an HGF2DLocation which must
                         be located on the self vector at which resolution
                         of possible intersection is needed.

    @param pi_rNextPoint A constant reference to an HGF2DLocation which if
            ...
            if (AreContiguousAtAndGet(pi_rVector,
                    ...
                    if (!pi_rVector.IsAtAnExtremity(FirstContiguousnessPoint, Tolerance) &&
                        ...
                        double  Acc0A = pi_rVector.
                                                        CalculateAngularAcceleration(FirstContiguousnessPoint,
                                                                                     HVE2DVector::ALPHA);
                        double  Acc0B = pi_rVector.CalculateAngularAcceleration(FirstContiguousnessPoint, HVE2DVector::BETA);
                        double  Acc2A = CalculateAngularAcceleration(FirstContiguousnessPoint, HVE2DVector::ALPHA);
                        double  Acc2B = CalculateAngularAcceleration(FirstContiguousnessPoint, HVE2DVector::BETA);
                ...
                bool   FirstAnswer = (B3MinusB0 < B1MinusB0) ||
                                      (((B3MinusB0 == B1MinusB0) || (B3MinusB0 == 0.0)) &&
                                       (CalculateAngularAcceleration(pi_rTestPoint, HVE2DVector::BETA) <
                                        pi_rVector.CalculateAngularAcceleration(pi_rTestPoint, HVE2DVector::BETA)));

                bool   SecondAnswer = ((B2MinusB0 < B1MinusB0)) ||
                                       (((B2MinusB0 == B1MinusB0) || (B2MinusB0 == 0.0)) &&
                                        (CalculateAngularAcceleration(pi_rTestPoint, HVE2DVector::ALPHA) <
                                         pi_rVector.CalculateAngularAcceleration(pi_rTestPoint, HVE2DVector::BETA)));
...
/** -----------------------------------------------------------------------------
    ...
    @param pi_rPoint Constant reference to an HGF2DLocation which constains
    ...
    -----------------------------------------------------------------------------
*/
bool HVE2DVector::TendsToTheLeftOfSCS(const HVE2DVector&   pi_rVector,
                                       const HGF2DLocation& pi_rPoint,
                                       ArbitraryDirection   pi_Direction) const
    {
    // They must have the same coordinate system
    HPRECONDITION(GetCoordSys() == pi_rVector.GetCoordSys());
    ...
    HGFBearing    ReferenceBearing = pi_rVector.CalculateBearing(pi_rPoint, HVE2DVector::BETA);
    ...
    // Check if not equal
        ...
        // Otherwise check if equal acceleration
            ...
            HGF2DLocation   FirstPoint(GetCoordSys());
            HGF2DLocation   SecondPoint(GetCoordSys());
            ObtainContiguousnessPointsAt(pi_rVector,
                                         pi_rPoint,
                                         &FirstPoint,
                                         &SecondPoint);
            ...
            if (FirstPoint.IsEqualTo(pi_rPoint, GetTolerance()))
                ...
                ReferenceBearing = pi_rVector.CalculateBearing(SecondPoint, HVE2DVector::BETA);
                ...
                // Calculate new reference bearing
                ReferenceBearing = pi_rVector.CalculateBearing(FirstPoint, HVE2DVector::BETA);
                ...
                if (AngleDiff2 < PI)
                    ...
                    {
                    // They have equal bearing ... obtain accelerations
                    double PointAcc2 = CalculateAngularAcceleration(FirstPoint, pi_Direction);

                    double RefAcc2 = pi_rVector.CalculateAngularAcceleration(FirstPoint, HVE2DVector::BETA);
...
//-----------------------------------------------------------------------------
bool HVE2DVector::AreContiguousAtAndGet(const HVE2DVector& pi_rVector,
                                         const HGF2DLocation& pi_rPoint,
...
HFCPtr<HGFTolerance> HVE2DVector::GetStrokeTolerance() const
    ...
    return new HGFTolerance (*m_pStrokeTolerance);
    }
