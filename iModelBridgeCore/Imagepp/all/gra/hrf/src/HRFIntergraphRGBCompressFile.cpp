//:>--------------------------------------------------------------------------------------+
//:>
//:>     $Source: all/gra/hrf/src/HRFIntergraphRGBCompressFile.cpp $
//:>
//:>  $Copyright: (c) 2014 Bentley Systems, Incorporated. All rights reserved. $
//:>
//:>+--------------------------------------------------------------------------------------
// Class HRFIntergraphRGBCompressFile
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// HRFRGBCompressBlockCapabilities
//-----------------------------------------------------------------------------
class HRFRGBCompressBlockCapabilities : public HRFRasterFileCapabilities
    {
public:
    // Constructor
    HRFRGBCompressBlockCapabilities()
...
//-----------------------------------------------------------------------------
// HRFRGBCompressCodecCapabilities
//-----------------------------------------------------------------------------
class HRFRGBCompressCodecCapabilities : public  HRFRasterFileCapabilities
    {
public:
    // Constructor
    HRFRGBCompressCodecCapabilities()
        ...
        Add(new HRFCodecCapability(HFC_READ_WRITE_CREATE,
                                   HCDCodecRGBRLE8::CLASS_ID,
                                   new HRFRGBCompressBlockCapabilities()));
        }
    };
...
//-----------------------------------------------------------------------------
HRFRGBCompressCapabilities::HRFRGBCompressCapabilities()
    : HRFRasterFileCapabilities()
    {
    // PixelTypeV24R8G8B8
    ...
    pPixelTypeV24R8G8B8 = new HRFPixelTypeCapability(HFC_READ_WRITE_CREATE,
                                                     ...
                                                     new HRFRGBCompressCodecCapabilities());
...
HFC_IMPLEMENT_SINGLETON(HRFIntergraphRGBCompressCreator)

//-----------------------------------------------------------------------------
// Creator
// Compress capabilities instance member definition
//-----------------------------------------------------------------------------

HRFIntergraphRGBCompressCreator::HRFIntergraphRGBCompressCreator()
    : HRFIntergraphFile::Creator(HRFIntergraphRGBCompressFile::CLASS_ID)
    {
...
WString HRFIntergraphRGBCompressCreator::GetLabel() const
    {
    HFCResourceLoader* stringLoader = HFCResourceLoader::GetInstance();
    return stringLoader->GetString(IDS_FILEFORMAT_RGBCompressed);  // Intergraph RGB compress File Format
    }
...
WString HRFIntergraphRGBCompressCreator::GetExtensions() const
    {
...
HFCPtr<HRFRasterFile> HRFIntergraphRGBCompressCreator::Create(const HFCPtr<HFCURL>& pi_rpURL,
    ...
    HFCPtr<HRFRasterFile> pFile = new HRFIntergraphRGBCompressFile(pi_rpURL, pi_AccessMode, pi_Offset);
    HASSERT(pFile != 0);
...
bool HRFIntergraphRGBCompressCreator::IsKindOfFile(const HFCPtr<HFCURL>& pi_rpURL,
                                                    uint64_t             pi_Offset) const
    {
    HPRECONDITION(pi_rpURL != 0);
    ...
    (const_cast<HRFIntergraphRGBCompressCreator*>(this))->SharingControlCreate(pi_rpURL);
    ...
    if (pFile != 0 && pFile->GetLastExceptionID() == NO_EXCEPTION)
        ...
        if (HeaderTypeCode == 0x0908)
            ...
            if ((DataTypeCode == 27) && (pi_Offset || !IsMultiPage(*pFile, (WordToFollow + 2)/256)) )
                Result = true;
            else
                {
                if (DataTypeCode == 65)
...
WRAPUP:
    ...
    HASSERT(!(const_cast<HRFIntergraphRGBCompressCreator*>(this))->m_pSharingControl->IsLocked());
    (const_cast<HRFIntergraphRGBCompressCreator*>(this))->m_pSharingControl = 0;
...
//-----------------------------------------------------------------------------
const HFCPtr<HRFRasterFileCapabilities>& HRFIntergraphRGBCompressCreator::GetCapabilities()
    {
    if (m_pCapabilities == 0)
        m_pCapabilities  = new HRFRGBCompressCapabilities();
...
// allow to Open an image file
//-----------------------------------------------------------------------------
HRFIntergraphRGBCompressFile::HRFIntergraphRGBCompressFile(const HFCPtr<HFCURL>& pi_rURL,
                                                           HFCAccessMode         pi_AccessMode,
                                                           uint64_t             pi_Offset)
...
//-----------------------------------------------------------------------------
void HRFIntergraphRGBCompressFile::CreateDescriptors()
    {
    ...
    // for (UInt32 Page=0; Page < CalcNumberOfPage(); Page++)
        ...
        uint32_t BgColor              = 0;
        uint32_t* ScrColorComponentPtr = (uint32_t*)&m_IntergraphHeader.IBlock2.use[0];
...
//-----------------------------------------------------------------------------
uint64_t HRFIntergraphRGBCompressFile::GetFileCurrentSize() const
    {
...
//-----------------------------------------------------------------------------
const HFCPtr<HRFRasterFileCapabilities>& HRFIntergraphRGBCompressFile::GetCapabilities () const
    {
    return HRFIntergraphRGBCompressCreator::GetInstance()->GetCapabilities();
    }
