...
WString HRFImgRGBCreator::GetLabel() const
    ...
    return stringLoader->GetString(IDS_FILEFORMAT_ImgRGB); //ImgRGB File Format
    }
...
bool HRFImgRGBCreator::IsKindOfFile(const HFCPtr<HFCURL>& pi_rpURL,
    ...
    // Read .a file
    if (pFile->Read((void*)aBuffer,  4) != 4 || sscanf((char*)aBuffer, "%u", &Width) != 1  || Width == 0) // validate if it is in 1..9999 range
        goto WRAPUP;

    if (pFile->Read((void*)aBuffer,  4) != 4 || sscanf((char*)aBuffer, "%u", &Height) != 1 || Height == 0)// validate if it is in 1..9999 range
        goto WRAPUP;
...
HRFImgRGBFile::~HRFImgRGBFile()
    ...
    if ((GetAccessMode().m_HasCreateAccess) && (CountPages() < 1))
        ...
        HFCFileManager::GetInstance()->Remove((HFCPtr<HFCURLFile>&)m_pRedFileURL);
        HFCFileManager::GetInstance()->Remove((HFCPtr<HFCURLFile>&)m_pGreenFileURL);
        HFCFileManager::GetInstance()->Remove((HFCPtr<HFCURLFile>&)m_pBlueFileURL);
        }
    }
...
void HRFImgRGBFile::OpenFiles()
    ...
    if (!m_IsOpen)
        {
        // Open main file
        m_pImgRGBFile = HFCBinStream::Instanciate(CreateCombinedURLAndOffset(GetURL(), m_Offset), GetAccessMode());

        ThrowFileExceptionIfError(m_pImgRGBFile, GetURL()->GetURL());
        ...
        m_pRedFile   = HFCBinStream::Instanciate(CreateCombinedURLAndOffset(m_pRedFileURL,   m_Offset), GetAccessMode());
        ThrowFileExceptionIfError(m_pRedFile, m_pRedFileURL->GetURL());

        m_pGreenFile = HFCBinStream::Instanciate(CreateCombinedURLAndOffset(m_pGreenFileURL, m_Offset), GetAccessMode());
        ThrowFileExceptionIfError(m_pGreenFile, m_pGreenFileURL->GetURL());

        m_pBlueFile  = HFCBinStream::Instanciate(CreateCombinedURLAndOffset(m_pBlueFileURL,  m_Offset), GetAccessMode());
        ThrowFileExceptionIfError(m_pBlueFile, m_pBlueFileURL->GetURL());
...
void HRFImgRGBFile::GetFileHeaderFromFile()
    ...
    // Read Width
    m_pImgRGBFile->Read((void*)aBuffer, 4);
    sscanf((char*)aBuffer, "%u", &(m_FileHeader.m_Width));

    // Read Height
    m_pImgRGBFile->Read((void*)aBuffer, 4);
    sscanf((char*)aBuffer, "%u", &(m_FileHeader.m_Height));
...
void HRFImgRGBFile::SetFileHeaderToFile()
    ...
    sprintf ((char*)aBuffer, "%4u%4u", m_FileHeader.m_Width, m_FileHeader.m_Height);

    // Write to main file
    m_pImgRGBFile->Write((void*)aBuffer, 12);
    }
