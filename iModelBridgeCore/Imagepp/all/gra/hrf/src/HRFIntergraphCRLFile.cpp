...
//-----------------------------------------------------------------------------
// HRFCRLBlockCapabilities
//-----------------------------------------------------------------------------
class HRFCRLBlockCapabilities : public HRFRasterFileCapabilities
    {
public:
    // Constructor
    HRFCRLBlockCapabilities()
...
//-----------------------------------------------------------------------------
// HRFCRLCodecCapabilities
//-----------------------------------------------------------------------------
class HRFCRLCodecCapabilities : public  HRFRasterFileCapabilities
    {
public:
    // Constructor
    HRFCRLCodecCapabilities()
        ...
        Add(new HRFCodecCapability(HFC_READ_WRITE_CREATE,
                                   HCDCodecCRL8::CLASS_ID,
                                   new HRFCRLBlockCapabilities()));
        }
    };
...
// HRFCRLCapabilities
//-----------------------------------------------------------------------------
HRFCRLCapabilities::HRFCRLCapabilities()
    : HRFRasterFileCapabilities()
    {
...
HFC_IMPLEMENT_SINGLETON(HRFIntergraphCRLCreator)
...
HRFIntergraphCRLCreator::HRFIntergraphCRLCreator()
    : HRFIntergraphFile::Creator(HRFIntergraphCRLFile::CLASS_ID)
    {
...
HFCPtr<HRFRasterFile> HRFIntergraphCRLCreator::Create(const HFCPtr<HFCURL>& pi_rpURL,
    ...
    HFCPtr<HRFRasterFile> pFile = new HRFIntergraphCRLFile(pi_rpURL, pi_AccessMode, pi_Offset);
    HASSERT(pFile != 0);
...
bool HRFIntergraphCRLCreator::IsKindOfFile(const HFCPtr<HFCURL>& pi_rpURL,
                                            uint64_t             pi_Offset) const
    {
    HPRECONDITION(pi_rpURL != 0);
    ...
    (const_cast<HRFIntergraphCRLCreator*>(this))->SharingControlCreate(pi_rpURL);
...
WRAPUP:
    ...
    HASSERT(!(const_cast<HRFIntergraphCRLCreator*>(this))->m_pSharingControl->IsLocked());
    (const_cast<HRFIntergraphCRLCreator*>(this))->m_pSharingControl = 0;

    return Result;
    }

//-----------------------------------------------------------------------------
// Create or get the singleton capabilities of an Intergraph CRL file.
//-----------------------------------------------------------------------------

const HFCPtr<HRFRasterFileCapabilities>& HRFIntergraphCRLCreator::GetCapabilities()
    {
    if (m_pCapabilities == 0)
        m_pCapabilities  = new HRFCRLCapabilities();
...
//-----------------------------------------------------------------------------
uint64_t HRFIntergraphCRLFile::GetFileCurrentSize() const
    {
...
// allow to Open an image file
//-----------------------------------------------------------------------------
HRFIntergraphCRLFile::HRFIntergraphCRLFile(const HFCPtr<HFCURL>& pi_rURL,
                                           HFCAccessMode         pi_AccessMode,
                                           uint64_t             pi_Offset)
...
//-----------------------------------------------------------------------------
void HRFIntergraphCRLFile::CreateDescriptors()
    {
...
//-----------------------------------------------------------------------------
const HFCPtr<HRFRasterFileCapabilities>& HRFIntergraphCRLFile::GetCapabilities () const
    {
    return HRFIntergraphCRLCreator::GetInstance()->GetCapabilities();
    }
