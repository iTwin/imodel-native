...
struct ConverterV64R16G16B16A16_V64R16G16B16A16 : public HRPPixelConverter
    ...
    void Convert(const void* pi_pSourceRawData, void* pio_pDestRawData) const
        ...
        memcpy(pio_pDestRawData, pi_pSourceRawData, 8);
        };
    ...
    virtual void Compose(const void* pi_pSourceRawData, void* pio_pDestRawData) const
        ...
        if(pSrc[3] != 0)
            ...
            if(pDest[3] == 0 || pSrc[3] == 0xFFFF)
                ...
                unsigned short PremultDestColor = (pDest[3] * pDest[0]) / 0xFFFF;
                pDest[0] = ((pSrc[3] * (pSrc[0] - PremultDestColor)) / 0xFFFF) + PremultDestColor;

                PremultDestColor = (pDest[3] * pDest[1]) / 0xFFFF;
                pDest[1] = ((pSrc[3] * (pSrc[1] - PremultDestColor)) / 0xFFFF) + PremultDestColor;

                PremultDestColor = (pDest[3] * pDest[2]) / 0xFFFF;
                pDest[2] = ((pSrc[3] * (pSrc[2] - PremultDestColor)) / 0xFFFF) + PremultDestColor;

                // Adst' = MaxAlpha - (( (MaxAlpha - Asrc) * (MaxAlpha - Adst) ) / MaxAlpha
                // --> Transparency percentages are multiplied
                pDest[3] = 0xFFFF - (((0xFFFF - pSrc[3]) * (0xFFFF - pDest[3])) / 0xFFFF);
                }
            }
        };
        ...
        for(size_t i(0); i < pi_PixelsCount*4; i+=4)
            ...
            if(pSrc[i+3] != 0)
                ...
                if(pDest[i+3] == 0 || pSrc[i+3] == 0xFFFF)
                    ...
                    // premul = ((Adst * Cdst)/MaxAlpha)
                    // Cdst'  = ((Asrc * (Csrc - premul)) / MaxAlpha) + premul
                    unsigned short PremultDestColor = (pDest[i+3] * pDest[i]) / 0xFFFF;
                    pDest[i] = ((pSrc[i+3] * (pSrc[i] - PremultDestColor)) / 0xFFFF) + PremultDestColor;

                    PremultDestColor = (pDest[i+3] * pDest[i+1]) / 0xFFFF;
                    pDest[i+1] = ((pSrc[i+3] * (pSrc[i+1] - PremultDestColor)) / 0xFFFF) + PremultDestColor;

                    PremultDestColor = (pDest[i+3] * pDest[i+2]) / 0xFFFF;
                    pDest[i+2] = ((pSrc[i+3] * (pSrc[i+2] - PremultDestColor)) / 0xFFFF) + PremultDestColor;

                    // Adst' = MaxAlpha - (( (MaxAlpha - Asrc) * (MaxAlpha - Adst) ) / MaxAlpha
                    // --> Transparency percentages are multiplied
                    pDest[i+3] = 0xFFFF - (((0xFFFF - pSrc[i+3]) * (0xFFFF - pDest[i+3])) / 0xFFFF);
                    }
                }
            }
...
struct ConverterV24R8G8B8_V64R16G16B16A16 : public HRPPixelConverter
    ...
    void Convert(const void* pi_pSourceRawData, void* pio_pDestRawData) const
        ...
        pDest[2] = CONVERT_8BIT_TO_16BIT(pSrc[2]);
        pDest[3] = 0xFFFF;
        }
...
struct ConverterV32R8G8B8A8_V64R16G16B16A16 : public HRPPixelConverter
    ...
    void Convert(const void* pi_pSourceRawData, void* pio_pDestRawData) const
        ...
        pDest[1] = CONVERT_8BIT_TO_16BIT(pSrc[1]);
        pDest[2] = CONVERT_8BIT_TO_16BIT(pSrc[2]);
        pDest[3] = CONVERT_8BIT_TO_16BIT(pSrc[3]);
        };
    ...
    virtual void Compose(const void* pi_pSourceRawData, void* pio_pDestRawData) const
        ...
        if(pV32Src[3] != 0)
            ...
            if(pV64Dest[3] == 0 || pV32Src[3] == 0xFF)
                ...
                // premul = ((Adst * Cdst)/MaxAlpha)
                // Cdst'  = ((Asrc * (Csrc - premul)) / MaxAlpha) + premul
                unsigned short PremultDestColor = (pV64Dest[3] * pV64Dest[0]) / 0xFFFF;
                pV64Dest[0] = ((V64Src[3] * (V64Src[0] - PremultDestColor)) / 0xFFFF) + PremultDestColor;

                PremultDestColor = (pV64Dest[3] * pV64Dest[1]) / 0xFFFF;
                pV64Dest[1] = ((V64Src[3] * (V64Src[1] - PremultDestColor)) / 0xFFFF) + PremultDestColor;

                PremultDestColor = (pV64Dest[3] * pV64Dest[2]) / 0xFFFF;
                pV64Dest[2] = ((V64Src[3] * (V64Src[2] - PremultDestColor)) / 0xFFFF) + PremultDestColor;

                // Adst' = MaxAlpha - (( (MaxAlpha - Asrc) * (MaxAlpha - Adst) ) / MaxAlpha
                // --> Transparency percentages are multiplied
                pV64Dest[3] = 0xFFFF - (((0xFFFF - V64Src[3]) * (0xFFFF - pV64Dest[3])) / 0xFFFF);
                }
            }
        };
...
struct ConverterV64R16G16B16A16_V24R8G8B8 : public HRPPixelConverter
    ...
    void Convert(const void* pi_pSourceRawData, void* pio_pDestRawData) const
        ...
        pDest[0] = CONVERT_16BIT_TO_8BIT(pSrc[0]);
        pDest[1] = CONVERT_16BIT_TO_8BIT(pSrc[1]);
        pDest[2] = CONVERT_16BIT_TO_8BIT(pSrc[2]);
        };
    ...
    virtual void Compose(const void* pi_pSourceRawData, void* pio_pDestRawData) const
        ...
        pDst[0] = CONVERT_16BIT_TO_8BIT((pSrc[3] * (pSrc[0] - CONVERT_8BIT_TO_16BIT(pDst[0]))) / 0xFFFF) + pDst[0];
        pDst[1] = CONVERT_16BIT_TO_8BIT((pSrc[3] * (pSrc[1] - CONVERT_8BIT_TO_16BIT(pDst[1]))) / 0xFFFF) + pDst[1];
        pDst[2] = CONVERT_16BIT_TO_8BIT((pSrc[3] * (pSrc[2] - CONVERT_8BIT_TO_16BIT(pDst[2]))) / 0xFFFF) + pDst[2];
        };
    ...
    virtual void Compose(const void* pi_pSourceRawData, void* pio_pDestRawData, size_t pi_PixelsCount) const
        ...
        for(size_t i(0); i < pi_PixelsCount*3; i+=3)
            {
            // ((SrcAlpha * (Src - Dst)) / MaxAlpha) + Dst
            pDst[i]   = CONVERT_16BIT_TO_8BIT((pSrc[3] * (pSrc[0] - CONVERT_8BIT_TO_16BIT(pDst[i])))   / 0xFFFF) + pDst[i];
            pDst[i+1] = CONVERT_16BIT_TO_8BIT((pSrc[3] * (pSrc[1] - CONVERT_8BIT_TO_16BIT(pDst[i+1]))) / 0xFFFF) + pDst[i+1];
            pDst[i+2] = CONVERT_16BIT_TO_8BIT((pSrc[3] * (pSrc[2] - CONVERT_8BIT_TO_16BIT(pDst[i+2]))) / 0xFFFF) + pDst[i+2];
...
struct ConverterV64R16G16B16A16_V32R8G8B8A8 : public HRPPixelConverter
    ...
    void Convert(const void* pi_pSourceRawData, void* pio_pDestRawData) const
        ...
        pDest[1] = CONVERT_16BIT_TO_8BIT(pSrc[1]);
        pDest[2] = CONVERT_16BIT_TO_8BIT(pSrc[2]);
        pDest[3] = CONVERT_16BIT_TO_8BIT(pSrc[3]);
        }
...
struct ConverterV64R16G16B16A16_V48R16G16B16 : public HRPPixelConverter
    ...
    virtual void Compose(const void* pi_pSourceRawData, void* pio_pDestRawData) const
        ...
        // ((SrcAlpha * (Src - Dst)) / MaxAlpha) + Dst
        pDst[0] = ((pSrc[3] * (pSrc[0] - pDst[0])) / 0xFFFF) + pDst[0];
        pDst[1] = ((pSrc[3] * (pSrc[1] - pDst[1])) / 0xFFFF) + pDst[1];
        pDst[2] = ((pSrc[3] * (pSrc[2] - pDst[2])) / 0xFFFF) + pDst[2];
        };
    ...
    virtual void Compose(const void* pi_pSourceRawData, void* pio_pDestRawData, size_t pi_PixelsCount) const
        ...
        for(size_t i(0); i < pi_PixelsCount*3; i+=3)
            {
            // ((SrcAlpha * (Src - Dst)) / MaxAlpha) + Dst
            pDst[i]   = ((pSrc[3] * (pSrc[0] - pDst[i]))   / 0xFFFF) + pDst[i];
            pDst[i+1] = ((pSrc[3] * (pSrc[1] - pDst[i+1])) / 0xFFFF) + pDst[i+1];
            pDst[i+2] = ((pSrc[3] * (pSrc[2] - pDst[i+2])) / 0xFFFF) + pDst[i+2];
