//:>--------------------------------------------------------------------------------------+
//:>
//:>     $Source: all/gra/HTiff/src/HTGFFFile.cpp $
//:>
...
struct File::Impl
    ...
    explicit                        Impl                       (const WString&                  pi_rInputFilePath,
        ...
        if (m_file.GetFilePtr()->GetLastExceptionID() != NO_EXCEPTION)
        ...
        if (pi_rAccessMode.m_HasCreateAccess)
            ...
            if (version > pi_version)
                throw runtime_error("Stored file version is greater than actual file version!");
            }
...
File::File (const char*                     pi_rInputFilePath,
    ...
    BeStringUtilities::LocaleCharToWChar(InputStr,pi_rInputFilePath,codePage);
