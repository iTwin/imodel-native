//:>--------------------------------------------------------------------------------------+
//:>
//:>     $Source: all/net/hcs/test/SocketTest/ClientThread.h $
//:>
...
public:
    ...
    virtual void Go()
        {
        ostringstream Host;
        Host << "iip://" << m_Host << ":" << m_Port;
        while (!m_rStopEvent.WaitUntilSignaled(0))
            {
            try
                {
                m_pConnection = new HCSSocketConnection(Host.str());
                if (m_pConnection->Connect(string(""), string(""), 3000))
                    {
                    ostringstream Request;
                    uint32_t Value = 10.0 * (double)rand() / (double)RAND_MAX;
                    Request << Value << s_Marker;

                    // send the Request to the server
                    m_pConnection->Send((const Byte*)Request.str().data(), Request.str().size());
...
private:
    ...
    HAutoPtr<HCSSocketConnection> m_pConnection;
