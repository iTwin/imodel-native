/*--------------------------------------------------------------------------------------+
|
|     $Source: ext/MatrixFromTiePts/src/projtran.c $
|
...
**      transformation of coordinate using a  projective
**
**===========================================================================*/
...
**
**  ALAIN LAPIERRE          Original version          3/1/93
**===========================================================================*/
...
void trf_transfoCoordProj2D(int32_t            mapping,
    ...
    {
    double yprod[3];
    trf_transfoCoordPrepProj2D(mapping, coef3D, ptIn, yprod);
    trf_transfoCoordDoProj2D(mapping, coef3D, ptIn, ptOut, yprod);
    }
...
void trf_transfoCoordPrepProj2D(int32_t            mapping,
    ...
    yprod[0]= coef3D->coef[mapping][XIND][5] * normY + 1.0;
    yprod[1]= coef3D->coef[mapping][XIND][2] * normY +
              coef3D->coef[mapping][XIND][3];
    yprod[2]= coef3D->coef[mapping][YIND][2] * normY +
...
void trf_transfoCoordDoProj2D(int32_t          mapping,
    ...
    div   = coef3D->coef[mapping][XIND][4] * normX + yprod[0];

    ptOut->x = ((coef3D->coef[mapping][XIND][1] * normX + yprod[1]) / div) + coef3D->coef[mappingTran][XIND][0];
    ptOut->y = ((coef3D->coef[mapping][YIND][1] * normX + yprod[2]) / div) + coef3D->coef[mappingTran][YIND][0];
    ptOut->z = 0.0;
    }
...
/*fh=========================================================================
**
**  trf_transfoCoordDoProjScanLine2D
**
                                                                                         ...
                                                                                         double               *yprod)
{
    ...
    int32_t pt;
        ...
        for(pt=0; pt < scanLineLength; pt++, scanLineOriginX += scanLinePixelSizeX )
        {
                ...
                ptOut[pt].x= ((coef3D->coef[mapping][XIND][1] * normX + yprod[1]) / div) + coef3D->coef[mappingTran][XIND][0];
                ptOut[pt].y= ((coef3D->coef[mapping][YIND][1] * normX + yprod[2]) / div) + coef3D->coef[mappingTran][YIND][0];
        }
}
                                   ...
                                   TRANSFO_RSP_COLOR_INFO *pi_pColorInfo)
{
    ...
    */
    ...
    CoefX4 = pi_pCoef3D->coef[pi_Mapping][XIND][4] * pio_pBufferInfo->m_InputImagePixelSize;
    CoefTranX0 = (pi_pCoef3D->coef[MappingTran][XIND][0] -
            pio_pBufferInfo->m_InputImageOrigin.x) /
            pio_pBufferInfo->m_InputImagePixelSize;
    CoefTranY0 = (pi_pCoef3D->coef[MappingTran][YIND][0] -
            pio_pBufferInfo->m_InputImageOrigin.y) /
            pio_pBufferInfo->m_InputImagePixelSize;
    YProd0 = pi_pYprod[0] * pio_pBufferInfo->m_InputImagePixelSize;
    YProd1 = pi_pYprod[1];
    YProd2 = pi_pYprod[2];
    ...
    */
        ...
        for(iPixel=0; iPixel < pi_ScanLineLength; iPixel++)
    {
        ...
        */
        NormX = OutputPixelCoordUserX - CoefX0;
                Div   = CoefX4 * NormX + YProd0;

        InputPixelCoordUserX =  ((CoefX1 * NormX + YProd1) / Div) + CoefTranX0;
        InputPixelCoordUserY =  ((CoefY1 * NormX + YProd2) / Div) + CoefTranY0;
