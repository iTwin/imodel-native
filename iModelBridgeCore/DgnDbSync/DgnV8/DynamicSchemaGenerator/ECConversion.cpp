...
static DgnV8Api::XAttributeHandlerId   s_schemaStoredHiddenHandlerId(22271, 2);
static DgnV8Api::XAttributeHandlerId   s_schemaExternalHiddenHandlerId(22271, 3);
static DgnV8Api::XAttributeHandlerId   s_intrinsicInstanceHandlerId(22294, 3);

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    Paul.Connelly   12/13
+---------------+---------------+---------------+---------------+---------------+------*/
bool IsIntrinsicInstance(Bentley::ElementRefP elemRef, UInt32 localId)
    {
    DgnV8Api::XAttributeHandle xa(elemRef, s_intrinsicInstanceHandlerId, localId);
    if (xa.IsValid() && sizeof(UInt16) <= xa.GetSize())
        return true;
    else
        return false;
    }
...
//---------------+---------------+---------------+---------------+---------------+-------
void FindIntrinsicClassesOnElement(Bentley::ElementRefP rootElement, DgnV8Api::ElementECClassInfo& classes)
    {
    DgnV8Api::XAttributeHandle primaryXattr;
    DgnV8Api::TempSchemaCache tsc;
    ECObjectsV8::SchemaNameClassNamePair primaryPair;
    bool foundPrimary = false;
    DgnV8Api::ElementHandle eh(rootElement);
    bool primaryIsIdentifyingItemType = false;
    for (DgnV8Api::ElementHandle::XAttributeIter iter = eh.GetXAttributeIter(); iter.IsValid(); iter.ToNext())
        {
        auto handlerId = iter.GetHandlerId();
        if (handlerId.GetMajorId() != 22271 || s_schemaStoredHiddenHandlerId == handlerId || s_schemaExternalHiddenHandlerId == handlerId || !IsIntrinsicInstance(eh.GetElementRef(), iter.GetId()))
            continue;

        DgnV8Api::ECXAttributesReader reader(iter, eh.GetDgnModelP(), false, tsc, nullptr);
        if (!reader.IsValid())
            continue;

        ECObjectsV8::SchemaNameClassNamePair info;
        reader.GetSchemaName(info.m_schemaName);
        reader.GetClassName(info.m_className);

        bool isPrimary = classes.size() == 0;
        classes.Insert(info, isPrimary);
        }
    }
