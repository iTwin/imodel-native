...
* @bsimethod                                    Sam.Wilson                      08/16
+---------------+---------------+---------------+---------------+---------------+------*/
Utf8String Converter::SheetsComputeViewAttachmentName(DgnModelId parentSheetId, DgnAttachmentCR v8DgnAttachment) const
    {
    // We must come up with a unique code value for this view a unique code.
    // The following will not collide with:
    //  - any code generated by other bridge jobs, because it is scoped to this bridge job's definition model.
    //  - the code of any view based on a V8 name, because we are using a character that is illegal in V8. 
    //  - any other generated definition because this will be paired with BIS_CODESPEC_ViewDefinition.
    //  - any other generated view, because it uses the ElementId of the BIM sheet and the ElementId of the V8 attachment on the (original) sheet.
    // Also note that this code will be used only for ViewDefinitions that are marked PRIVATE.
    return Utf8PrintfString(":%llx:%llx", (uint64_t)parentSheetId.GetValue(), (uint64_t)v8DgnAttachment.GetElementId());
    }
