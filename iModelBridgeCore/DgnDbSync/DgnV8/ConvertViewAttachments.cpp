...
BentleyStatus Converter::GetRangeFromSectionView(DRange3dR range, DgnAttachmentCR v8Attachment, ResolvedModelMapping const& sheetModelMapping, bool isSheetProxyGraphics)
    ...
    //  Note that, because of SheetUnnestAttachments, we know that every section-cut attachment is a direct attachment to the sheet. So, its proxygraphics are always in the CS of the sheet.
    if (!v8Attachment.GetDgnModelP()->Is3d())
        return BSIERROR;

    BeAssert(v8Attachment.GetDgnModelP()->Is3d() && "SheetUnnestAttachments must guarantee that all section-cut attachments are directly attached to the sheet");
