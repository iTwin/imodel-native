...
BentleyStatus Converter::GetRangeFromSectionView(DRange3dR range, DgnAttachmentCR v8Attachment, ResolvedModelMapping const& sheetModelMapping, bool isSheetProxyGraphics)
    ...
    //  Note that we might or might not be a direct attachment of the section cut to the sheet. We might have:
    //  V8 sheet -[sectionview]-> 3ddesign
    // or 
    //  V8 sheet -> V8 sectiondrawing -[sectionview]-> 3ddesign
    // 
    //  We also have to be careful when figuring out what transform to use, because there are 2 cases:
    //  1. Proxy graphics are in BIM Sheet:Model coordinates.
    //      v8Attachment corresponds to a BIM DrawingModel that the caller just generated by extracting proxy graphics from the attached V8 model. 
    //      Therefore, we know that the BIM DrawingModel's graphics are already in the BIM SheetModel's coordinate system. That's what the isSheetProxyGraphics flag tells us. 
    //      Note that v8Attachment might be a direct attachment of a section cut, or v8Attachment might be an attachment of a V8 drawing attaches the section cut.
    //      (NEEDS WORK: In theory, the section could even be in a nested attachment, but the GetDrawingGeneratorAttachment doesn't support that.) 
    //      In any case, the isSheetProxyGraphics means that the graphics in the BIM DrawingModel were harvested in the context of the current BIM sheet and are in its CS.
    //  2. Proxy graphics are in BIM DrawingModel coordinates.
    //      v8Attachment corresponds to a BIM DrawingModel that contains extracted proxy graphics. That DrawingModel was populated earlier, in some other context in the converter. 
    //      The DrawingModel's graphics are in its own coordinate system. We still want to use the named view that generated the section in order to compute the range of 
    //      the BIM DrawingModel's graphics, but we must remember when transforming from V8 to BIM that they are in the CS of the BIM drawing, not the BIM sheet.

    DgnV8ModelR attachedV8Model = *v8Attachment.GetDgnModelP();

    DgnV8Api::DgnAttachment const* nvAttachment;
    
    // See if this attachment is itself a section-like attachment to a 3D model.
    if (attachedV8Model.Is3d())
        {
        //  This could be a case of a direct attachment of a section.
        if (!isSheetProxyGraphics)
            return BSIERROR;
        nvAttachment = &v8Attachment;   // The input attachment is the section attachment that was used to generate the proxy graphics
        }
    else
        {
        //  This could be case of an indrect attachment of a section view. See if this attachment is to a drawing model that was generated from a section-like attachment to a 3D design. 
        nvAttachment = GetDrawingGeneratorAttachment(v8Attachment);
        if (nullptr == nvAttachment)
            return BSIERROR;
        }

    // Use the section-like NamedView to get the view geometry
    auto nv = nvAttachment->GetNamedView();
    if (!nv.IsValid())
        return BSIERROR;
    ...
    //  Transform that range to the V8 sheet's coordinate system.
    //  TRICKY: 
    //      isSheetProxyGraphics == true
    //          If we know that the converted proxy graphics are in the CS of the BIM sheet, then we want to get the NV's range into the CS of the V8 sheet.
    //          nvAttachment might or might not be a direct attachment of the section cut to the sheet. We might have:
    //            V8 sheet -[sectionview]-> 3ddesign
    //          or 
    //            V8 sheet -> V8 sectiondrawing -[sectionview]-> 3ddesign
    //          We call mdlRefFile_getTransformToParent to get the all-in transform from the NV to the V8 sheet.
    //      isSheetProxyGraphics == false
    //          If we know that the converted proxy graphics are in the CS of a drawing model, then we want the NV's range in the CS of the V8 drawing model
    //          which used the NV to cut the section. So, we call DgnAttachment::GetTransformToParent in that case.
    Bentley::Transform toV8Parent;
    if (isSheetProxyGraphics)
        Bentley::mdlRefFile_getTransformToParent(&toV8Parent, nvAttachment, nullptr);
    else
        nvAttachment->GetTransformToParent(toV8Parent, true);
    ...
    //          and the extract the normal min/max range points from that.
    Bentley::DPoint3d nvBoxCorners[8];
    nvRange.Get8Corners(nvBoxCorners);
    
    auto v8SectionRange = Bentley::DRange3d::DRange3d::From(nvBoxCorners, 8);

    // ... and flatten it.
    //auto flattenTrans = Bentley::Transform::FromIdentity();
    //flattenTrans.form3d[2][2] = 0.0;
    //flattenTrans.Multiply(&nvRange.low, 2);

    if (!attachedV8Model.Is3d())
        {
        // Make sure the range includes the annotations in the drawing, too.
        AddV8ModelToRange(v8SectionRange, attachedV8Model);
        }

    // Get the view range into BIM Sheet:Model coordinates
    //  TRICKY: Consider the same two cases when inferring what CS the converted proxy graphics are in.
    Transform toBim;
    if (isSheetProxyGraphics)
        toBim = sheetModelMapping.GetTransform();
    else
        toBim = ComputeUnitsScaleTransform(attachedV8Model);
