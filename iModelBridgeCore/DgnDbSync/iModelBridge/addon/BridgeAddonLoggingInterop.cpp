...
* @bsimethod                                    Sam.Wilson                      02/18
+---------------+---------------+---------------+---------------+---------------+------*/
void BridgeNative::JsInterop::LogMessage(Utf8CP category, NativeLogging::SEVERITY sev, Utf8CP msg)
{
    if (BridgeNative::jsInterop_getLogger().IsEmpty() || !IsMainThread())
    {
        BridgeNative::deferLogging(category, sev, msg);
        return;
    }

    BridgeNative::doDeferredLogging();
    BridgeNative::logMessageToJs(category, sev, msg);
}

/*---------------------------------------------------------------------------------**/ /**
* @bsimethod                                    Sam.Wilson                      02/18
+---------------+---------------+---------------+---------------+---------------+------*/
bool BridgeNative::JsInterop::IsSeverityEnabled(Utf8CP category, NativeLogging::SEVERITY sev)
{
    if (BridgeNative::jsInterop_getLogger().IsEmpty() || !IsMainThread())
        return true;

    BridgeNative::doDeferredLogging();
    return BridgeNative::callIsLogLevelEnabledJs(category, sev);
}
...
NativeLogging::ILogger &JsInterop::GetNativeLogger()
{
    ...
    if (nullptr == s_nativeLogger)
        s_nativeLogger = NativeLogging::LoggingManager::GetLogger("iModelBridgeApiServer"); // This is thread-safe. The assignment is atomic, and GetLogger will always return the same value for a given key anyway.
