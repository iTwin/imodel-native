...
public:
    ...
    void AdjustLineStyleScale(Render::GeometryParams& params, DgnV8Api::DgnAttachment const* refP);
    ...
    //! Convert levels in v8 element that is found on a drawing (used to preconvert drawing element levels).
    void ConvertDrawingLevels(DgnV8EhCR v8eh);
...
protected:
    ...
    //! @{
    BentleyStatus DropElementRefersToElementsIndices();
    BentleyStatus RecreateElementRefersToElementsIndices();
...
struct ConvertToDgnDbElementExtension : DgnV8Api::Handler::Extension
{
    ...
    virtual void _UpdateResourceDefinitions (iMBridgeDocPropertiesAccessor& accessor) {}
