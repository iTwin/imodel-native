...

/*%-----------------------------------------------------------------------------
 FUNC: aecDTM_findAffectedTriangles
 DESC: Finds all the triangles that are affected by adding a point at the
       input location to the input terrain model.  A list of triangles
       stored in a long array) is returned.
 HIST: Original - tmi 01-Nov-1990
 MISC:
 KEYW: DTM FIND TRIANGLES AFFECT LIST
-----------------------------------------------------------------------------%*/
...
{
  struct CIVdtmtin *tmpP, *tinP;
  struct CIVdtmpnt pnt, *p1P, *p2P, *p3P;
  int sts = SUCCESS, rpt, sid;
  long sid1, sid2;

  tinP = *triPP;
  *triPP = (struct CIVdtmtin *)0;
  *ntinlstP = 0L;
  memset ( &pnt, 0, sizeof(struct CIVdtmpnt) );
  DTMDPOINTTOPOINT ( srfP, locP[0], &pnt );

  if ( aecDTM_findTriangle ( &tinP, 0, &rpt, &sid, srfP, locP ) == SUCCESS )
    if ( rpt != -1 )
      sts = aecDTM_getPointNeighbors ( (long *)0, (long **)0, ntinlstP, tinlstPP, (long *)0, (long **)0, srfP, *(&(tinP->p1)+rpt), tinP, 0 );
    else if ( ! sid )
    {
      if ( ( sts = aecDTM_triangleStack ( ntinlstP, tinlstPP, tinP ) ) == SUCCESS )
        if ( ( sts = aecDTM_delaunayTriangleCheck ( srfP, &pnt, tinP, tinP->p2, tinP->p1, ntinlstP, tinlstPP ) ) == SUCCESS )
          if ( ( sts = aecDTM_delaunayTriangleCheck ( srfP, &pnt, tinP, tinP->p3, tinP->p2, ntinlstP, tinlstPP ) ) == SUCCESS )
            sts = aecDTM_delaunayTriangleCheck ( srfP, &pnt, tinP, tinP->p1, tinP->p3, ntinlstP, tinlstPP );
    }
    else
    {
      if ( sid == 1 )
        p1P = tinP->p2,  p2P = tinP->p3,  p3P = tinP->p1;
      else if ( sid == 2 )
        p1P = tinP->p3,  p2P = tinP->p1,  p3P = tinP->p2;
      else
        p1P = tinP->p1,  p2P = tinP->p2,  p3P = tinP->p3;

      if ( ( sts = aecDTM_triangleStack ( ntinlstP, tinlstPP, tinP ) ) == SUCCESS )
        if ( ( sts = aecDTM_delaunayTriangleCheck ( srfP, &pnt, tinP, p2P, p1P, ntinlstP, tinlstPP ) ) == SUCCESS )
          if ( ( sts = aecDTM_delaunayTriangleCheck ( srfP, &pnt, tinP, p3P, p2P, ntinlstP, tinlstPP ) ) == SUCCESS )
          {
            tmpP = *( &(tinP->n12) + sid - 1 );
            if ( tmpP )
            {
              aecDTM_getTriangleSideIndex ( &sid1, &sid2, tinP, tmpP, FALSE );
              tinP = tmpP;
              sid = (int)sid2;
              if ( sid == 1 )
                p1P = tinP->p2,  p2P = tinP->p3,  p3P = tinP->p1;
              ...
              if ( ( sts = aecDTM_triangleStack ( ntinlstP, tinlstPP, tinP ) ) == SUCCESS )
                if ( ( sts = aecDTM_delaunayTriangleCheck ( srfP, &pnt, tinP, p2P, p1P, ntinlstP, tinlstPP ) ) == SUCCESS )
                  sts = aecDTM_delaunayTriangleCheck ( srfP, &pnt, tinP, p3P, p2P, ntinlstP, tinlstPP );
            }
          }
    }
