...
cleanup:
    if (zeroSlopePointsIndexP != nullptr)
        {
        free(zeroSlopePointsIndexP); zeroSlopePointsIndexP = nullptr;
        }
    bcdtmList_nullTptrValuesDtmObject(dtmP);
    bcdtmList_nullSptrValuesDtmObject(dtmP);
    /*
...
*/
    {
    int            ret = DTM_SUCCESS, dbg = DTM_TRACE_VALUE(0), cdbg = DTM_CHECK_VALUE(0);
    long           ap, np, sp, numRemoved, startPoint;
    bool           belowSump = true;
    long           expansionTime;
    double         elevation;
    DTM_SUMP_LINES *slP;
...
|                                                                    |
+-------------------------------------------------------------------*/
static int bcdtmDrainage_calculateVolumeSurfaceToPlaneDtmObject
(
    BC_DTM_OBJ* dtmP,
    double     elevation,
    double&     cutP,
    ...
    /*
    ** This Function Calculates The Tin Volume above and below The elevation
    */
    {
    long   p1, p2, p3, clc;
    bool voidFlag;
    double cutTriangle,fillTriangle,areaTriangle,fillArea;
    /*
    ** Initialise Variables
    */
    cutP = fillP = areaP = 0.0 ;
    /*
    ** Scan Cyclic List and Extract each Traingle
    */
    for( p1 = 0 ; p1 < dtmP->numPoints ; ++p1 )
        {
        if( ( clc = nodeAddrP(dtmP,p1)->cPtr) != dtmP->nullPtr )
            ...
            while( clc != dtmP->nullPtr )
                ...
                if( p2 > p1 && p3 > p1 && nodeAddrP(dtmP,p3)->hPtr != p1 )
                    ...
                    if( ! voidFlag )
                        {
                        bcdtmTinVolume_prismToFlatPlaneDtmObject(dtmP,p1,p2,p3,elevation,cutTriangle,fillTriangle,areaTriangle, fillArea) ;
                        cutP  = cutP  + cutTriangle ;
                        fillP = fillP + fillTriangle ;
                        areaP = areaP + areaTriangle ;
                        }
                    }
                clc = clistAddrP(dtmP,clc)->nextPtr ;
                p2  = p3 ;
...
|                                                                    |
+-------------------------------------------------------------------*/
static int bcdtmDrainage_appendBermToPondDtmObject
(
    BC_DTM_OBJ  *pondSideSlopesP,     /* ==> Pointer To Pond DTM  Object      */
    bool        isBermFillOnly,       /* ==> Calculate Berm In Fill Only      */
    BC_DTM_OBJ  *fillTinP,            /* ==> Pointer To Fill Tin              */
    double      bermSlope,            /* ==> Slope Of Berm                    */
    double      bermWidth,            /* ==> Width Of Berm                    */
    bool        isCrown,              /* ==> Flag To Indicate A Crown On Berm */
    double      crownWidth,           /* ==> Width Of Crown Component Of Berm */
    double      cornerStrokeTolerance /* ==> Corner Stroke Tolerance          */
)
    /*
    ** This Function Appends A Berm To A Pond
    */
    {
    int                  ret=DTM_SUCCESS,dbg=0 ;
    long                 n,drapeFlag,numHullPts=0,numDataObjects,sideSlopeTableSize,fillSideSlopeTableSize ;
    bool                 process;
    double               z ;
    DPoint3d             *p3d,*hullPtsP=nullptr ;
    DTMUserTag         userTag ;
    DTMFeatureId       featureId ;
    DTM_SIDE_SLOPE_TABLE *sstP,*arP,*farP,*larP,*sideSlopeTableP=nullptr,*fillSideSlopeTableP=nullptr,*tempTableP=nullptr ;
    BC_DTM_OBJ           *pondDataP=nullptr,*pondTinP=nullptr,**dataObjectsPP=nullptr ;
    ...
    */
    if( dbg )
        ...
        bcdtmWrite_message(0,0,0,"bermWidth        = %8.4lf",bermWidth) ;
        bcdtmWrite_message(0,0,0,"isCrown          = %8ld",isCrown) ;
        bcdtmWrite_message(0,0,0,"crownWidth       = %8.4lf",crownWidth) ;
        bcdtmWrite_message(0,0,0,"cornerStokeTol   = %8.4lf",cornerStrokeTolerance) ;
        bcdtmWrite_geopakDatFileFromDtmObject(pondSideSlopesP,L"berm00.dat") ;
        }
    /*
    ** Extract Hull From Data Object
    */
    if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(pondSideSlopesP,DTMFeatureType::Hull,&userTag,&featureId,&hullPtsP,&numHullPts) ) goto errexit ;
    if( dbg ) bcdtmWrite_message(0,0,0,"numHullPts = %6ld",numHullPts) ;
    if( dbg && isBermFillOnly )
        {
        for( p3d = hullPtsP ; p3d < hullPtsP + numHullPts ; ++p3d )
            {
            if( bcdtmDrape_pointDtmObject(fillTinP,p3d->x,p3d->y,&z,&drapeFlag)) goto errexit ;
            if     ( drapeFlag == 0 ) bcdtmWrite_message(0,0,0,"Hull Point[%8ld] ** %12.5lf %12.5lf %10.4lf ** External To Fill Tin",(long)(p3d-hullPtsP),p3d->x,p3d->y,p3d->z) ;
            else if( drapeFlag == 2 ) bcdtmWrite_message(0,0,0,"Hull Point[%8ld] ** %12.5lf %12.5lf %10.4lf ** In Fill Tin Void",(long)(p3d-hullPtsP),p3d->x,p3d->y,p3d->z) ;
            else
                {
                if( fabs(z-p3d->z) < 0.0001 ) z = p3d->z ;
                if     ( z < p3d->z ) bcdtmWrite_message(0,0,0,"Hull Point[%8ld] ** %12.5lf %12.5lf %10.4lf ** %10.4lf ** In Fill",(long)(p3d-hullPtsP),p3d->x,p3d->y,p3d->z,z) ;
                else if( z > p3d->z ) bcdtmWrite_message(0,0,0,"Hull Point[%8ld] ** %12.5lf %12.5lf %10.4lf ** %10.4lf ** In Cut",(long)(p3d-hullPtsP),p3d->x,p3d->y,p3d->z,z) ;
                else                  bcdtmWrite_message(0,0,0,"Hull Point[%8ld] ** %12.5lf %12.5lf %10.4lf ** %10.4lf ** On Surface",(long)(p3d-hullPtsP),p3d->x,p3d->y,p3d->z,z) ;
                }
            }
        }
    /*
    ** Calculate A Fill Side Slope Table For Subsequent Berm Calculations
    */
    fillSideSlopeTableSize = numHullPts ;
    fillSideSlopeTableP = ( DTM_SIDE_SLOPE_TABLE * ) malloc ( fillSideSlopeTableSize * sizeof(DTM_SIDE_SLOPE_TABLE )) ;
    if( fillSideSlopeTableP == nullptr )
        {
        bcdtmWrite_message(1,0,0,"Memory Allocation Failure") ;
        goto errexit ;
        }
    if( dbg ) bcdtmWrite_message(0,0,0,"Size Of Side Slope Table Before Cut/Fill Transitions = %6ld",fillSideSlopeTableSize) ;
    for( sstP = fillSideSlopeTableP , p3d = hullPtsP ; sstP < fillSideSlopeTableP + fillSideSlopeTableSize ; ++sstP , ++p3d )
        {
        sstP->slopeToTin = nullptr ;
        sstP->cutFillTin = fillTinP ;
        sstP->radialStartPoint.x = p3d->x ;
        sstP->radialStartPoint.y = p3d->y ;
        sstP->radialStartPoint.z = p3d->z ;
        sstP->cutSlope           = 0.0 ;
        sstP->fillSlope          = 0.0 ;
        sstP->radialOption       = 1 ;
        sstP->radialStatus       = 1 ;
        sstP->sideSlopeOption    = 3 ;
        if( isCrown ) sstP->toHorizOffset = (bermWidth-crownWidth) / 2.0 ;
        else          sstP->toHorizOffset = bermWidth     ;
        if( isBermFillOnly )  sstP->cutFillOption = 2 ;
        else                  sstP->cutFillOption = 0 ;
        sstP->isForceSlope       = 1 ;
        sstP->forcedSlope        = bermSlope ;
        sstP->useSlopeTable      = 0 ;
        sstP->isRadialDir        = 0 ;
        sstP->toElev             = 0.0 ;
        }
    if( dbg ) bcdtmWrite_message(0,0,0,"Size Of Side Slope Table After  Cut/Fill Transitions = %6ld",fillSideSlopeTableSize) ;
    /*
    ** If Is Berm Fill Only Find And Set First Side Slope Point To Active Radial
    */
    if( isBermFillOnly )
        {
        /*
        ** Scan To First Inactive Radial
        */
        if( dbg ) bcdtmWrite_message(0,0,0,"Berm Fill Only ** Scanning To First Active Radial") ;
        farP = nullptr ;
        for( sstP = fillSideSlopeTableP ; sstP < fillSideSlopeTableP + fillSideSlopeTableSize && farP == nullptr ; ++sstP )
            {
            if( sstP->radialStatus == 0 ) farP = sstP ;
            }
        /*
        **  Scan To First Active Radial
        */
        if( farP != nullptr )
            {
            larP = farP ;
            farP = nullptr ;
            for( sstP = larP ; sstP < fillSideSlopeTableP + fillSideSlopeTableSize && farP == nullptr ; ++sstP )
                {
                if( sstP->radialStatus == 1 ) farP = sstP ;
                }
            /*
            **     Copy Side Slope Table To Temporary Table
            */
            if( farP != nullptr )
                {
                tempTableP = ( DTM_SIDE_SLOPE_TABLE * ) malloc ( fillSideSlopeTableSize * sizeof(DTM_SIDE_SLOPE_TABLE )) ;
                if( tempTableP == nullptr ) { bcdtmWrite_message(1,0,0,"Memory Allocation Failure") ; goto errexit ; }
                /*
                **        Copy From First Active Radial To Second Last Entry In Table
                */
                for( sstP = farP , larP = tempTableP ; sstP < fillSideSlopeTableP + fillSideSlopeTableSize - 1 ; ++sstP , ++larP )
                    {
                    *larP = *sstP ;
                    }
                /*
                **        Copy From Start Of Table To Entry Prior To First Active Radial
                */
                for( sstP = fillSideSlopeTableP ; sstP < farP ; ++sstP , ++larP )
                    {
                    *larP = *sstP ;
                    }
                /*
                **        Set Last Table Entry Equal To First
                */
                *larP =  *fillSideSlopeTableP ;
                /*
                **        Set Table Pointers
                */
                free(fillSideSlopeTableP) ;
                fillSideSlopeTableP = tempTableP ;
                tempTableP = nullptr ;
                }
            }
        }
    ...
    */
    if( hullPtsP != nullptr ) { free(hullPtsP) ; hullPtsP = nullptr ; }
    /*
    ** Calculate Fill Side Slopes ** Need To Determine Cut/Fill Transition Points
    */
    if( isBermFillOnly )
        {
        if( dbg ) bcdtmWrite_message(0,0,0,"Creating Berm Fill Side Slopes") ;
        if( bcdtmSideSlope_createSideSlopesForSideSlopeTableDtmObject(&fillSideSlopeTableP,&fillSideSlopeTableSize,1,nullptr,0,1,1,cornerStrokeTolerance,0.00001,nullptr,0,nullUserTag,nullUserTag,&dataObjectsPP,&numDataObjects) ) goto errexit ;
        /*
        **  Delete Data Objects
        */
        for( n = 0 ; n < numDataObjects ; ++n )
            {
            pondDataP = *(dataObjectsPP+n) ;
            if( bcdtmObject_destroyDtmObject(&pondDataP)) goto errexit ;
            }
        if( dataObjectsPP   != nullptr ) { free(dataObjectsPP) ; dataObjectsPP = nullptr ; }
        /*
        ** Mark Inactive Radials Not In Fill By Draping On Fill Tin
        ** Added 27/01/2005 RobC
        */
        for( sstP = fillSideSlopeTableP ; sstP < fillSideSlopeTableP + fillSideSlopeTableSize ; ++sstP  )
            {
            if( sstP->radialStatus )
                {
                if( bcdtmDrape_pointDtmObject(fillTinP,sstP->radialStartPoint.x,sstP->radialStartPoint.y,&z,&drapeFlag)) goto errexit ;
                if( drapeFlag == 0 || drapeFlag == 2 ) sstP->radialStatus = 0 ;
                else
                    {
                    if( fabs(z-sstP->radialStartPoint.z) < 0.0001 ) z = sstP->radialStartPoint.z ;
                    if( z >= sstP->radialStartPoint.z ) sstP->radialStatus = 0 ;
                    }
                }
            }
        }
    /*
    ** Scan Fill Side Slopes Table For Radials In Fill
    */
    farP = fillSideSlopeTableP ;
    process = true ;
    while( process == true )
        {
        process = false ;
        /*
        **   Get First Active Radial
        */
        while ( farP < fillSideSlopeTableP + fillSideSlopeTableSize && ! farP->radialStatus ) ++farP ;
        /*
        **   Get Last Active Radial
        */
        larP = farP + 1  ;
        while ( larP < fillSideSlopeTableP + fillSideSlopeTableSize &&   larP->radialStatus ) ++larP ;
        --larP ;
        /*
        **   Calculate Berm For Active Radials
        */
        if( farP < larP )
            {
            process = true ;
            if( dbg )
                {
                bcdtmWrite_message(0,0,0,"Number Of Active Radials = %6ld",(long)(larP-farP)+1) ;
                bcdtmWrite_message(0,0,0,"First Radial = %6ld ** %10.4lf %10.4lf %10.4lf",(long)(farP-fillSideSlopeTableP),farP->radialStartPoint.x,farP->radialStartPoint.y,farP->radialStartPoint.z) ;
                bcdtmWrite_message(0,0,0,"Last  Radial = %6ld ** %10.4lf %10.4lf %10.4lf",(long)(larP-fillSideSlopeTableP),larP->radialStartPoint.x,larP->radialStartPoint.y,larP->radialStartPoint.z) ;
                }
            /*
            **      Allocate Memory For Side Slope Table
            */
            sideSlopeTableSize = (long)(larP-farP) + 1 ;
            ...
            */
            if( dbg ) bcdtmWrite_message(0,0,0,"Populating Side Slope Table") ;
            for( sstP = sideSlopeTableP , arP = farP ; sstP < sideSlopeTableP + sideSlopeTableSize ; ++sstP , ++arP )
                {
                sstP->slopeToTin = nullptr ;
                sstP->cutFillTin = nullptr ;
                sstP->radialStartPoint.x = arP->radialStartPoint.x ;
                sstP->radialStartPoint.y = arP->radialStartPoint.y ;
                sstP->radialStartPoint.z = arP->radialStartPoint.z ;
                sstP->cutSlope           = 0.0 ;
                sstP->fillSlope          = 0.0 ;
                sstP->radialOption       = 1 ;
                sstP->sideSlopeOption    = 3 ;
            ...
            /*
            **      Calculate Berm Side Slopes
            */
            if( dbg ) bcdtmWrite_message(0,0,0,"Creating Berm Side Slopes") ;
            if( bcdtmSideSlope_createSideSlopesForSideSlopeTableDtmObject(&sideSlopeTableP,&sideSlopeTableSize,1,nullptr,0,1,1,cornerStrokeTolerance,0.00001,nullptr,0,nullUserTag,nullUserTag,&dataObjectsPP,&numDataObjects) ) goto errexit ;
            pondDataP = *dataObjectsPP ;
            if( dbg ) bcdtmObject_reportStatisticsDtmObject(pondDataP) ;
            /*
            **      Set External Berm If No Crown
            */
            if( ! isCrown )
                {
                if( dbg ) bcdtmWrite_message(0,0,0,"Setting External Berm For No Crown") ;
                if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(pondDataP,DTMFeatureType::SlopeToe,&userTag,&featureId,&hullPtsP,&numHullPts))  goto errexit ;
                if( bcdtmObject_storeDtmFeatureInDtmObject(pondDataP,DTMFeatureType::Breakline,-10030,1,&nullFeatureId,hullPtsP,numHullPts)) goto errexit ;
                if( hullPtsP != nullptr ) { free(hullPtsP) ; hullPtsP = nullptr ; }
                }
            /*
            **      Append Berm Side Slopes To Pond Data Object
            */
            if( bcdtmObject_appendDtmObject(pondSideSlopesP,pondDataP)) goto errexit ;
            /*
            **      Free Memory
            */
            if( dataObjectsPP != nullptr ) { free(dataObjectsPP) ; dataObjectsPP = nullptr ; }
            if( ! isBermFillOnly && pondDataP != nullptr ) bcdtmObject_destroyDtmObject(&pondDataP) ;
            /*
            **      Append Crown To Pond
            */
            if( isCrown )
                {
                if( dbg ) bcdtmWrite_message(0,0,0,"Creating Crown Side Slopes") ;
                /*
                **         Extract Hull or Slope Toe From Data Object
                */
                if( ! isBermFillOnly )  { if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(pondSideSlopesP,DTMFeatureType::Hull,&userTag,&featureId,&hullPtsP,&numHullPts)) goto errexit ; }
                else                    { if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(pondDataP,DTMFeatureType::SlopeToe,&userTag,&featureId,&hullPtsP,&numHullPts))  goto errexit ; }
                /*
                **         If Crown Width is Zero Just Add Hull From Above As A Crown Break Line
                */
                if( crownWidth == 0.0 )
                    {
                    if( bcdtmObject_storeDtmFeatureInDtmObject(pondSideSlopesP,DTMFeatureType::Breakline,-10020,1,&nullFeatureId,hullPtsP,numHullPts)) goto errexit ;
                    if( bcdtmObject_storeDtmFeatureInDtmObject(pondSideSlopesP,DTMFeatureType::Hull,nullUserTag,1,&nullFeatureId,hullPtsP,numHullPts)) goto errexit ;
                    if( isBermFillOnly ) if( bcdtmObject_storeDtmFeatureInDtmObject(pondDataP,DTMFeatureType::SlopeToe,nullUserTag,1,&nullFeatureId,hullPtsP,numHullPts)) goto errexit ;
                    }
                /*
                **         If Crown Width Is Greater Than Zero Calculate Side Slopes For Crown
                */
                if( crownWidth > 0.0 )
                    {
                    /*
                    ...
                    */
                    if( ! isBermFillOnly ) bcdtmData_deleteAllOccurrencesOfDtmFeatureTypeDtmObject(pondSideSlopesP,DTMFeatureType::Hull) ;
                    /*
                    **            Allocate Memory For Crown Side Slope Table
                    */
                    sideSlopeTableSize = numHullPts ;
                    sideSlopeTableP = ( DTM_SIDE_SLOPE_TABLE * ) malloc ( sideSlopeTableSize * sizeof(DTM_SIDE_SLOPE_TABLE )) ;
                    if( sideSlopeTableP == nullptr ) { bcdtmWrite_message(1,0,0,"Memory Allocation Failure") ; goto errexit ; }
                    /*
                    **           Populate Side Slope Table For Crown Calculation
                    */
                    for( sstP = sideSlopeTableP , p3d = hullPtsP ; sstP < sideSlopeTableP + sideSlopeTableSize ; ++sstP , ++p3d )
                        {
                        sstP->slopeToTin = sstP->cutFillTin = nullptr ;
                        sstP->radialStartPoint.x = p3d->x ;
                        sstP->radialStartPoint.y = p3d->y ;
                        sstP->radialStartPoint.z = p3d->z ;
                        sstP->cutSlope           = 0.0 ;
                    ...
                    /*
                    **            Calculate Crown Side Slopes
                    */
                    if( bcdtmSideSlope_createSideSlopesForSideSlopeTableDtmObject(&sideSlopeTableP,&sideSlopeTableSize,1,nullptr,0,1,1,cornerStrokeTolerance,0.00001,nullptr,0,nullUserTag,nullUserTag,&dataObjectsPP,&numDataObjects) ) goto errexit ;
                    pondDataP = *dataObjectsPP ;
                    /*
                    **            Add Outside Of Crwown
                    */
                    if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(pondDataP,DTMFeatureType::SlopeToe,&userTag,&featureId,&hullPtsP,&numHullPts))  goto errexit ;
                    if( bcdtmObject_storeDtmFeatureInDtmObject(pondDataP,DTMFeatureType::Breakline,-10021,1,&nullFeatureId,hullPtsP,numHullPts)) goto errexit ;
                    if( hullPtsP != nullptr ) { free(hullPtsP) ; hullPtsP = nullptr ; }
                    /*
                    **            Append Crown Side Slopes To Pond Data Object
                    */
                    if( bcdtmObject_appendDtmObject(pondSideSlopesP,pondDataP)) goto errexit ;
                    /*
                    **            Free Memory
                ...
                if( dbg ) bcdtmWrite_geopakDatFileFromDtmObject(pondSideSlopesP,L"berm01.dat") ;
                /*
                **         Add External Component Of Berm
                */
                if( dbg ) bcdtmWrite_message(0,0,0,"Calculating Exterior Berm") ;
                if( ! isBermFillOnly )  { if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(pondSideSlopesP,DTMFeatureType::Hull,&userTag,&featureId,&hullPtsP,&numHullPts) ) goto errexit ; }
                else                    { if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(pondDataP,DTMFeatureType::SlopeToe,&userTag,&featureId,&hullPtsP,&numHullPts) ) goto errexit ; }
                ...
                **         Free Memory
                */
                if( isBermFillOnly && pondDataP != nullptr ) if( bcdtmObject_destroyDtmObject(&pondDataP)) goto errexit ;
                /*
                **         Remove Boundary Polygon
                */
                bcdtmData_deleteAllOccurrencesOfDtmFeatureTypeDtmObject(pondSideSlopesP,DTMFeatureType::Hull) ;
                /*
                **         Allocate Memory For Side Slope Table
                */
                sideSlopeTableSize = numHullPts ;
                ...
                */
                ...
                for( sstP = sideSlopeTableP , p3d = hullPtsP ; sstP < sideSlopeTableP + sideSlopeTableSize ; ++sstP , ++p3d )
                    ...
                    sstP->radialOption       = 1 ;
                    sstP->sideSlopeOption    = 3 ;
                    sstP->toHorizOffset      = (bermWidth-crownWidth) / 2.0 ;
                    sstP->cutFillOption      = 0 ;
                    sstP->isForceSlope       = 1 ;
                    sstP->forcedSlope        = -bermSlope ;
                    sstP->useSlopeTable      = 0 ;
                ...
                /*
                **        Calculate External Berm Side Slopes
                */
                if( bcdtmSideSlope_createSideSlopesForSideSlopeTableDtmObject(&sideSlopeTableP,&sideSlopeTableSize,1,nullptr,0,1,1,cornerStrokeTolerance,0.00001,nullptr,0,nullUserTag,nullUserTag,&dataObjectsPP,&numDataObjects) ) goto errexit ;
                pondDataP = *dataObjectsPP ;
                /*
                **        Extract Slope Toes For External Berm Boundary
                */
                if( hullPtsP != nullptr ) { free(hullPtsP) ; hullPtsP = nullptr ; }
                if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(pondDataP,DTMFeatureType::SlopeToe,&userTag,&featureId,&hullPtsP,&numHullPts))  goto errexit ;
                if( bcdtmObject_storeDtmFeatureInDtmObject(pondDataP,DTMFeatureType::Breakline,-10030,1,&nullFeatureId,hullPtsP,numHullPts)) goto errexit ;
                if( hullPtsP != nullptr ) { free(hullPtsP) ; hullPtsP = nullptr ; }
                /*
                **        Append External Berm Side Slopes To Pond Data Object
                */
                if( dbg ) bcdtmWrite_message(0,0,0,"Appending External Berm Side Slopes To Pond") ;
                if( bcdtmObject_appendDtmObject(pondSideSlopesP,pondDataP)) goto errexit ;
                if( dbg ) bcdtmWrite_geopakDatFileFromDtmObject(pondSideSlopesP,L"berm02.dat") ;
                /*
                **        Free Memory
                */
                if( dataObjectsPP   != nullptr ) { free(dataObjectsPP) ; dataObjectsPP = nullptr ; }
            ...
            /*
            **     For Berm Fill Only Remove Boundary Polygons And Get New Hull By Triangulating
            */
            if( isBermFillOnly )
                {
                if( pondDataP == nullptr ) if( bcdtmObject_createDtmObject(&pondDataP) ) goto errexit ;
                bcdtmData_changeAllOccurrencesOfDtmFeatureTypeDtmObject(pondSideSlopesP,DTMFeatureType::Hull,DTMFeatureType::Breakline) ;
                if( bcdtmObject_cloneDtmObject(pondSideSlopesP,&pondTinP)) goto errexit ;
                pondTinP->ppTol = pondTinP->plTol = 0.0 ;
                if( bcdtmObject_triangulateDtmObject(pondTinP)) goto errexit ;
                if( bcdtmList_removeNoneFeatureHullLinesDtmObject(pondTinP)) goto errexit ;
                if( bcdtmList_extractHullDtmObject(pondTinP,&hullPtsP,&numHullPts)) goto errexit ;
                if( bcdtmObject_storeDtmFeatureInDtmObject(pondSideSlopesP,DTMFeatureType::Hull,nullUserTag,1,&nullFeatureId,hullPtsP,numHullPts)) goto errexit ;
                bcdtmObject_destroyDtmObject(&pondTinP) ;
                }
            }
        /*
        **  Reset Active Radial Pointers
        */
        farP = larP + 1 ;
        }
    /*
...
cleanup :
    ...
    if( fillSideSlopeTableP != nullptr ) free(fillSideSlopeTableP) ;
    if( dataObjectsPP   != nullptr ) { free(dataObjectsPP) ; dataObjectsPP = nullptr ; }
    if( pondDataP       != nullptr ) bcdtmObject_destroyDtmObject(&pondDataP) ;
    if( pondTinP        != nullptr ) bcdtmObject_destroyDtmObject(&pondTinP) ;
    if( hullPtsP        != nullptr ) { free(hullPtsP)   ; hullPtsP   = nullptr ; }
    if( tempTableP      != nullptr ) { free(tempTableP) ; tempTableP = nullptr ; }
    /*
    ** Job Completed
    */
    if( dbg && ret == DTM_SUCCESS ) bcdtmWrite_geopakDatFileFromDtmObject(pondSideSlopesP,L"afterberm.dat") ;
    if( dbg && ret == DTM_SUCCESS )  bcdtmWrite_message(0,0,0,"Appending Berm To Pond Completed") ;
    if( dbg && ret == DTM_ERROR   )  bcdtmWrite_message(0,0,0,"Appending Berm To Pond Error") ;
    return(ret) ;
    /*
    ** Error Exit
    */
...
|                                                                    |
+-------------------------------------------------------------------*/
static int bcdtmDrainage_designPondToATargetVolumeDtmObject
(
    DPoint3d         *pondPtsP,               /* ==> Defining Array Of DPoint3d Points For Pond                                         */
    long        numPondPts,              /* ==> Number Of DPoint3d Pond Pts                                                        */
    DTMPondDesignMethod outsideInsideOpt,        /* ==> Flag To Indicate outsideInsideOpt Or Outside 1 = Outside 2 = outsideInsideOpt */
    double      designVolume,            /* ==> Target Volume                                                                 */
    double      pondSlope,               /* ==> Slope Of Pond Sides                                                           */
    double      freeBoard,               /* ==> Height Of Free Board                                                          */
    bool        isBerm,                  /* ==> Flag To Indicate A Berm , true For Berm , false For No Berm                   */
    bool        isBermFillOnly,          /* ==> Calculate Berm Only In Fill                                                   */
    BC_DTM_OBJ  *fillTinP,               /* ==> Fill Tin                                                                      */
    double      bermSlope,               /* ==> Slope Of Berm                                                                 */
    double      bermWidth,               /* ==> Width Of Berm                                                                 */
    bool        isCrown,                 /* ==> Flag To Indicate A Crown On Berm, true For Crown false For No Crown           */
    double      crownWidth,              /* ==> Width Of Crown Component Of Berm                                              */
    double      cornerStrokeTolerance,   /* ==> Corner Stroke Tolerance                                                       */
    BC_DTM_OBJ  **designedPondPP,        /* <== Untriangulated DTM Object For Designed Pond                                                  */
    DTMPondResult& designedPondFlagP,      /* <== Flag To Indicate Design Results                                               */
    double      *designedPondElevationP, /* <== Elevation At Which Volume Calculations Were Performed For Designed Pond       */
    double      *designedPondVolumeP     /* <== Design Pond Volume Calculated At Pond Elevation                               */
)
    {
    int                   ret=DTM_SUCCESS,dbg=0 ;
    long                  loop=0,sideSlopeTableSize,numDataObjects=0,binaryFlag,process ;
    long                  numStart,numEnd,numFeaturePts,numHullPts,numJoinUserTags=0 ;
    double                zMin=0.0,zMax=0.0,zIncAmount=0.0,pZmin,pZmax,Cut,Balance,Area,pondElevation,pondVolume=0.0,lastPondVolume,lastPondElevation ;
    DPoint3d              *p3dP,*featurePtsP=nullptr,*hullPtsP=nullptr ;
    BC_DTM_OBJ            *pondDataP=nullptr,*pondTinP=nullptr,*pondSideSlopesP=nullptr,**pondObjectsPP=nullptr,*tempP=nullptr ;
    ...
    ** Write Entry Message
    */
    if( dbg ) bcdtmWrite_message(0,0,0,"Designing Pond To A Target Volume") ;
    /*
    ** Initialise
    */
    designedPondFlagP                            = DTMPondResult::TargetObtained   ;
    *designedPondElevationP                       = 0.0 ;
    *designedPondVolumeP                          = 0.0 ;
    /*
    ** Determine Min Max Values                   For Pond Boundary
    */
    pZmin = pZmax = pondPtsP->z                   ;
    for( p3dP = pondPtsP +                        1 ; p3dP < pondPtsP + numPondPts ; ++p3dP )
        {
        if( p3dP->z < pZmin ) pZmin = p3dP->z ;
        if( p3dP->z > pZmax ) pZmax = p3dP->z ;
        }
    if( dbg ) bcdtmWrite_message(0,0,0,"Pond Boundary z Min = %10.4lf z Max = %10.4lf",pZmin,pZmax) ;
    /*
    ** Create Side Slope Table For Pond Design
    */
    sideSlopeTableSize = numPondPts ;
    sideSlopeTableP    = ( DTM_SIDE_SLOPE_TABLE * ) malloc ( sideSlopeTableSize * sizeof(DTM_SIDE_SLOPE_TABLE )) ;
    if( sideSlopeTableP == nullptr ) { bcdtmWrite_message(1,0,0,"Memory Allocation Failure") ; goto errexit ; }
    /*
    ** Populate Side Slope Table
    */
    for( sstP = sideSlopeTableP , p3dP = pondPtsP ; sstP < sideSlopeTableP + sideSlopeTableSize ; ++sstP , ++p3dP )
        {
        sstP->slopeToTin = sstP->cutFillTin = nullptr ;
        sstP->radialStartPoint.x = p3dP->x ;
        sstP->radialStartPoint.y = p3dP->y ;
        sstP->radialStartPoint.z = p3dP->z ;
        if( outsideInsideOpt == DTMPondDesignMethod::BottomUp ) { sstP->cutSlope = pondSlope ; sstP->fillSlope = 0.0 ; }
        else                                                    { sstP->cutSlope = 0.0 ; sstP->fillSlope = -pondSlope ; }
        sstP->radialOption       = 1 ;
        sstP->sideSlopeOption    = 2 ;
        sstP->cutFillOption      = 0 ;
        sstP->isForceSlope       = 0 ;
        sstP->forcedSlope        = 0.0 ;
        sstP->useSlopeTable      = 0 ;
    ...
    /*
    ** Set Parameters For Bottum Up - Calculate External Side Slopes
    */
    if( outsideInsideOpt == DTMPondDesignMethod::BottomUp )
        {
        zMin = pZmax + freeBoard  ;
        zMax = zMin + 1000.0 ;
        zIncAmount = 10.0 ;
        }
    /*
    ** Set Parameters For Top Down - Calculate Internal Side Slopes
    */
    else if( outsideInsideOpt == DTMPondDesignMethod::TopDown )
        {
        zMax = pZmin - freeBoard  ;
        zMin = zMax - 1000.0 ;
        zIncAmount = -10.0 ;
        }
    /*
    ** Set Volume Iteration Parameters
    */
    binaryFlag = 0 ;
    lastPondVolume = 0.0 ;
    lastPondElevation = 0.0 ;
    /*
    **   Iteratively Calculate Ponds Until Design Volume Reached
    */
    process = 1 ;
    while ( process && loop <= 100 )
        ...
        ++loop ;
        /*
        **  Delete Pond DTM Objects
        */
        if( pondDataP != nullptr ) bcdtmObject_destroyDtmObject(&pondDataP) ;
        if( pondTinP  != nullptr ) bcdtmObject_destroyDtmObject(&pondTinP) ;
        /*
        **  Set Elevation To Calculate Side Slope
        */
        if( ! binaryFlag ) pondElevation = zMin + zIncAmount ;
        else               pondElevation = ( zMin + zMax ) / 2.0 ;
        /*
        **  Calculate Side Slopes
        */
        if( dbg ) bcdtmWrite_message(0,0,0,"Designing Pond To Elevation = %10.4lf",pondElevation) ;
        for( sstP = sideSlopeTableP ; sstP < sideSlopeTableP + sideSlopeTableSize ; ++sstP ) sstP->toElev = pondElevation ;
        if( dbg ) bcdtmWrite_message(0,0,0,"Calculating Side Slopes") ;
        if( bcdtmSideSlope_createSideSlopesForSideSlopeTableDtmObject(&sideSlopeTableP,&sideSlopeTableSize,(long)outsideInsideOpt,nullptr,0,1,1,cornerStrokeTolerance,0.00001,nullptr,0,nullUserTag,-10000,&pondObjectsPP,&numDataObjects) ) goto errexit ;
        if( dbg ) bcdtmWrite_message(0,0,0,"Calculating Side Slopes Completed") ;
        pondDataP = *pondObjectsPP   ;
        if( dbg == 2 ) bcdtmWrite_geopakDatFileFromDtmObject(pondDataP,L"volumePond.dat") ;
        /*
        **  Get And Join Break Lines Representing Pond Element
        */
        if( bcdtmObject_createDtmObject(&tempP)) goto errexit ;
        bcdtmObject_setPointMemoryAllocationParametersDtmObject(tempP,pondDataP->numPoints,pondDataP->numPoints) ;
        if( bcdtmData_moveTaggedDtmFeatureTypeOccurrencesToDtmObject(pondDataP,tempP,DTMFeatureType::Breakline,-10000)) goto errexit ;
        if( bcdtmJoin_dtmFeatureTypeDtmObject(tempP,0.0,DTMFeatureType::Breakline,DTMFeatureType::Breakline,&numStart,&numEnd,&joinUserTagsP,&numJoinUserTags) ) goto errexit ;
        if( dbg ) bcdtmWrite_message(0,0,0,"pond boundary ** numStart = %6ld numEnd = %6ld",numStart,numEnd) ;
        if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(tempP,DTMFeatureType::Breakline,&userTag,&featureId,&featurePtsP,&numFeaturePts) ) goto errexit ;
        /*
        **  Get And Restore Hull Points
        */
        if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(pondDataP,DTMFeatureType::Hull,&userTag,&featureId,&hullPtsP,&numHullPts) ) goto errexit ;
        if( bcdtmObject_storeDtmFeatureInDtmObject(pondDataP,DTMFeatureType::Hull,nullUserTag,1,&nullFeatureId,hullPtsP,numHullPts)) goto errexit ;
        /*
        **  Set Break Lines In Data Object To Represent Bottom And Top Of Pond
        */
        if( outsideInsideOpt == DTMPondDesignMethod::BottomUp )
            {
            if( bcdtmObject_storeDtmFeatureInDtmObject(pondDataP,DTMFeatureType::Breakline,-10000,1,&nullFeatureId,featurePtsP,numFeaturePts)) goto errexit ;
            if( bcdtmObject_storeDtmFeatureInDtmObject(pondDataP,DTMFeatureType::Breakline,-10010,1,&nullFeatureId,hullPtsP,numHullPts)) goto errexit ;
            }
        else if( outsideInsideOpt == DTMPondDesignMethod::TopDown )
            {
            if( bcdtmObject_storeDtmFeatureInDtmObject(pondDataP,DTMFeatureType::Breakline,-10010,1,&nullFeatureId,hullPtsP,numHullPts)) goto errexit ;
            bcdtmObject_initialiseDtmObject(tempP) ;
            if( bcdtmData_moveTaggedDtmFeatureTypeOccurrencesToDtmObject(pondDataP,tempP,DTMFeatureType::Hole,-nullUserTag)) goto errexit ;
            if( bcdtmJoin_dtmFeatureTypeDtmObject(tempP,0.0,DTMFeatureType::Hole,DTMFeatureType::Breakline,&numStart,&numEnd,&joinUserTagsP,&numJoinUserTags) ) goto errexit ;
            if( dbg ) bcdtmWrite_message(0,0,0,"pond bottom   ** numStart = %6ld numEnd = %6ld",numStart,numEnd) ;
            if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(tempP,DTMFeatureType::Breakline,&userTag,&featureId,&featurePtsP,&numFeaturePts) ) goto errexit ;
            if( dbg ) bcdtmWrite_message(0,0,0,"numFeaturePts = %6ld",numFeaturePts) ;
            while ( numFeaturePts > 0 )
                {
                if( bcdtmObject_storeDtmFeatureInDtmObject(pondDataP,DTMFeatureType::Breakline,-10000,1,&nullFeatureId,featurePtsP,numFeaturePts)) goto errexit ;
                if( bcdtmData_moveToP3DArrayDtmFeatureTypeDtmObject(tempP,DTMFeatureType::Breakline,&userTag,&featureId,&featurePtsP,&numFeaturePts) ) goto errexit ;
                }
        ...
        /*
        **  Remove Unwanted Feature Types From Data Object
        */
        bcdtmData_deleteAllOccurrencesOfDtmFeatureTypeDtmObject(pondDataP,DTMFeatureType::GraphicBreak) ;
        /*
        **  Free Memory
        */
        if( hullPtsP    != nullptr )  { free(hullPtsP)    ; hullPtsP    = nullptr ; }
        if( featurePtsP != nullptr )  { free(featurePtsP) ; featurePtsP = nullptr ; }
        if( tempP       != nullptr )    bcdtmObject_destroyDtmObject(&tempP) ;
        /*
        **  Take A Copy Of Pond Side Slopes For Volume Calculations
        */
        if( pondTinP != nullptr ) bcdtmObject_destroyDtmObject(&pondTinP) ;
        if( pondSideSlopesP != nullptr ) bcdtmObject_destroyDtmObject(&pondSideSlopesP) ;
        if( bcdtmObject_cloneDtmObject(pondDataP,&pondSideSlopesP)) goto errexit ;
        if( bcdtmObject_cloneDtmObject(pondDataP,&pondTinP)) goto errexit ;
        /*
        **  Free memory
        */
        ...
        */
        pondTinP->ppTol = pondTinP->plTol = 0.0 ;
        bcdtmObject_triangulateDtmObject(pondTinP) ;
        if( dbg ) bcdtmWrite_toFileDtmObject(pondTinP,L"pondTin.bcdtm") ;
        /*
        ** Calculate Volume
        */
        if( outsideInsideOpt == DTMPondDesignMethod::BottomUp ) if( bcdtmDrainage_calculateVolumeSurfaceToPlaneDtmObject(pondTinP,pondElevation-freeBoard,Cut,pondVolume,Balance,Area) != DTM_SUCCESS ) goto errexit ;
        if( outsideInsideOpt == DTMPondDesignMethod::TopDown )  if( bcdtmDrainage_calculateVolumeSurfaceToPlaneDtmObject(pondTinP,pZmin-freeBoard,Cut,pondVolume,Balance,Area) != DTM_SUCCESS ) goto errexit ;
        if( outsideInsideOpt == DTMPondDesignMethod::BottomUp ) *designedPondElevationP = pondElevation-freeBoard ;
        if( outsideInsideOpt == DTMPondDesignMethod::TopDown )  *designedPondElevationP = pondElevation ;
        *designedPondVolumeP = pondVolume ;
        if( dbg ) bcdtmWrite_message(0,0,0,"Loop = %2ld binaryFlag = %1ld Pond Volume = %20.4lf Design Volume = %20.4lf",loop,binaryFlag,pondVolume,designVolume) ;
        /*
        **  Check Volume Calculated Against Design Volume
        */
        if( fabs(pondVolume-designVolume) < 0.1 ) process = 0 ;
        else
            {
            if( pondVolume <  designVolume )
                {
                if( ! binaryFlag )
                    {
                    /*
                    **           Check For Side Slopes Not Limiting Properly
                    */
                    if( lastPondVolume > pondVolume && outsideInsideOpt == DTMPondDesignMethod::TopDown )
                        {
                        binaryFlag = 1 ;
                        zMin = pondElevation ;
                        }
                    else  zIncAmount = zIncAmount * 2.0 ;
                    }
                else
                    {
                    if( outsideInsideOpt == DTMPondDesignMethod::BottomUp ) zMin = pondElevation ;
                    else                        zMax = pondElevation ;
                    }
                }
            else
                {
                binaryFlag = 1 ;
                if( outsideInsideOpt == DTMPondDesignMethod::BottomUp ) zMax = pondElevation ;
                else                        zMin = pondElevation ;
                /*
                **        Check For Side Slopes Not Limiting Properly
                */
                if( zMin >= zMax  )
                    {
                    if( outsideInsideOpt == DTMPondDesignMethod::BottomUp ) *designedPondElevationP = lastPondElevation-freeBoard ;
                    if( outsideInsideOpt == DTMPondDesignMethod::TopDown )  *designedPondElevationP = lastPondElevation ;
                    *designedPondVolumeP    = lastPondVolume ;
                    process = 0 ;
                    }
                }
            /*
            **     Check Termination Of Pond Iteration
            */
            if( outsideInsideOpt == DTMPondDesignMethod::BottomUp )
                {
                if( fabs(pondElevation-pZmax-freeBoard ) < 0.001 ) { process = 0 ; designedPondFlagP = DTMPondResult::TargetVolumeLessThanPondBoundary ; }
                else if( binaryFlag && fabs(zMin-zMax) < 0.00001 )   process = 0 ;
                }
            if( outsideInsideOpt == DTMPondDesignMethod::TopDown )
                {
                if( fabs(pondElevation-pZmin-freeBoard) < 0.001   ) { process = 0 ; designedPondFlagP = DTMPondResult::TargetVolumeLessThanPondBoundary ; }
                else if( ! binaryFlag && fabs(lastPondVolume-pondVolume ) < 0.1 && fabs(pondVolume-designVolume) > 20.0  ) { process = 0 ; designedPondFlagP = DTMPondResult::TargetVolumeExceedsPondMaxVolume ; }
                else if( binaryFlag &&  fabs(zMin-zMax) < 0.00001 ) { process = 0 ; designedPondFlagP = DTMPondResult::TargetObtained ; }
                }
            }
        /*
        ** Set Last Pond Volume
        */
        lastPondVolume = pondVolume ;
        lastPondElevation = pondElevation ;
        }
    /*
    ** Append Berm To Pond
    */
    if( isBerm )
        {
        if( dbg ) bcdtmWrite_message(0,0,0,"Calculating Berm") ;
        if( bcdtmDrainage_appendBermToPondDtmObject(pondSideSlopesP,isBermFillOnly,fillTinP,bermSlope,bermWidth,isCrown,crownWidth,cornerStrokeTolerance)) goto errexit ;
        }
    /*
    ** Remove UnWanted Side Slope Features
    */
    bcdtmData_changeAllOccurrencesOfDtmFeatureTypeDtmObject(pondSideSlopesP,DTMFeatureType::Hole,DTMFeatureType::Breakline) ;
    // bcdtmData_deleteAllOccurrencesOfDtmFeatureTypeDtmObject(pondSideSlopesP,DTMFeatureType::SlopeToe) ;
    bcdtmData_deleteAllOccurrencesOfDtmFeatureTypeDtmObject(pondSideSlopesP,DTMFeatureType::GraphicBreak) ;
    bcdtmData_compactUntriangulatedFeatureTableDtmObject(pondSideSlopesP);
    /*
    ** Set Designed Pond
    */
    *designedPondPP = pondSideSlopesP ;
    pondSideSlopesP = nullptr ;
    /*
    ** Clean Up
    */
cleanup :
    ...
    ** Job Completed
    */
    if( dbg && ! ret )  bcdtmWrite_message(0,0,0,"Designing Pond To A Target Volume Completed") ;
    if( dbg &&   ret )  bcdtmWrite_message(0,0,0,"Designing Pond To A Target Volume Error") ;
    return(ret) ;
    /*
    ** Error Exit
    */
...
|                                                                    |
+-------------------------------------------------------------------*/
static int bcdtmDrainage_designPondToATargetElevationDtmObject
(
    DPoint3d*  pondPtsP,               /* ==> Defining Array Of DPoint3d Points For Pond                                        */
    long       numPondPts,              /* ==> Number Of DPoint3d Pond Pts                                                       */
    DTMPondDesignMethod outsideInsideOpt,        /* ==> Flag To Indicate Inside Or Outside 1 = Outside 2 = Inside                    */
    ...
    double     crownWidth,              /* ==> Width Of Crown Component Of Berm                                             */
    double     cornerStrokeTolerance,   /* ==> Corner Stroke Tolerance                                                      */
    BC_DTM_OBJ **designedPondPP,        /* <== Data Object For Designed Pond                                                */
    DTMPondResult& designedPondFlagP,      /* <== Flag To Indicate Design Results                                              */
    double     *designedPondElevationP, /* <== Elevation At Which Volume Calculations Were Performed For Designed Pond      */
    double     *designedPondVolumeP     /* <== Design Pond Volume Calculated At Pond Elevation                              */
)
    {
    int      ret=DTM_SUCCESS,dbg=0;
    long     sideSlopeTableSize,numDataObjects=0,numFeaturePts,numHullPts,numStart,numEnd,numJoinUserTags=0 ;
    double   pZmin,pZmax,cut,balance,area,pondVolume ;
    DPoint3d      *p3dP,*featurePtsP=nullptr,*hullPtsP=nullptr ;
    BC_DTM_OBJ  **dataObjectsPP=nullptr,*pondSideSlopesP=nullptr,*pondDataP=nullptr,*pondTinP=nullptr,*tempP=nullptr ;
    DTM_SIDE_SLOPE_TABLE *sstP,*sideSlopeTableP=nullptr ;
    DTMUserTag  userTag ;
    DTMFeatureId featureId ;
    DTM_JOIN_USER_TAGS *joinUserTagsP=nullptr ;
    ...
    ** Write Entry Message
    */
    if( dbg ) bcdtmWrite_message(0,0,0,"Designing Pond To A Target Elevation") ;
    /*
    ** Initialise
    */
    designedPondFlagP      = DTMPondResult::TargetObtained   ;
    ...
    */
    ...
    if( dbg ) bcdtmWrite_message(0,0,0,"Pond Boundary z Min = %10.4lf z Max = %10.4lf",pZmin,pZmax) ;
    /*
    ** Check Elevation Is Correct For Side Of Pond
    */
    if( outsideInsideOpt == DTMPondDesignMethod::BottomUp && elevation <= pZmax ) { bcdtmWrite_message(0,0,0,"Target Elevation Less Than Maximum Pond Boundary z Value") ; goto errexit ; }
    if( outsideInsideOpt == DTMPondDesignMethod::TopDown && elevation >= pZmin ) { bcdtmWrite_message(0,0,0,"Target Elevation Greater Than Minimum Pond Boundary z Value") ; goto errexit ; }
    /*
    ** Create Side Slope Table For Pond Design
    */
    sideSlopeTableSize = numPondPts ;
    ...
    */
    for( sstP = sideSlopeTableP , p3dP = pondPtsP ; sstP < sideSlopeTableP + sideSlopeTableSize ; ++sstP , ++p3dP )
        ...
        sstP->useSlopeTable      = 0 ;
        sstP->isRadialDir        = 0 ;
        sstP->toElev             = elevation ;
        }
    /*
    **  Calculate Pond To Target Elevation
    */
    if( dbg ) bcdtmWrite_message(0,0,0,"Designing Pond To Elevation = %10.4lf",elevation) ;
    if( bcdtmSideSlope_createSideSlopesForSideSlopeTableDtmObject(&sideSlopeTableP,&sideSlopeTableSize,(long)outsideInsideOpt,nullptr,0,1,1,cornerStrokeTolerance,0.00001,nullptr,0,nullUserTag,-10000,&dataObjectsPP,&numDataObjects) ) goto errexit ;
    pondDataP = *dataObjectsPP ;
    if( dbg ) bcdtmWrite_geopakDatFileFromDtmObject(pondDataP,L"elevationPond.dat") ;
    /*
    ** Get And Join Break Lines Representing Pond Element
    */
    if( bcdtmObject_createDtmObject(&tempP)) goto errexit ;
    ...
    ** Remove Unwanted Feature Types From Data Object
    */
    if( bcdtmData_deleteAllOccurrencesOfDtmFeatureTypeDtmObject(pondDataP,DTMFeatureType::GraphicBreak) ) goto errexit ;
    /*
    ** Take A Copy Of Pond Side Slopes For Volume Calculations
    */
    if( bcdtmObject_cloneDtmObject(pondDataP,&pondSideSlopesP)) goto errexit ;
    /*
    ** Create Tin For Pond To Determine Pond Volume
    */
    if( bcdtmObject_cloneDtmObject(pondDataP,&pondTinP)) goto errexit ;
    pondTinP->ppTol = pondTinP->plTol = 0.0 ;
    if( bcdtmObject_triangulateDtmObject(pondTinP)) goto errexit ;
    ...
    ** Calculate Volume
    */
    if( outsideInsideOpt == DTMPondDesignMethod::BottomUp ) elevation = elevation - freeBoard ;
    else                        elevation = pZmin - freeBoard ;
    if( bcdtmDrainage_calculateVolumeSurfaceToPlaneDtmObject(pondTinP,elevation,cut,pondVolume,balance,area) != DTM_SUCCESS ) goto errexit ;
    *designedPondElevationP = elevation  ;
    *designedPondVolumeP    = pondVolume ;
    if( dbg ) bcdtmWrite_message(0,0,0,"Elevation = %10.4lf Pond Volume = %12.4lf",elevation,pondVolume) ;
    /*
    ** Free Memory
    */
    if( hullPtsP        != nullptr ) { free(hullPtsP)        ; hullPtsP    = nullptr     ; }
    ...
    */
    if( isBerm )
        {
        if( dbg ) bcdtmWrite_geopakDatFileFromDtmObject(pondSideSlopesP,L"pondsideslopes.dat") ;
        if( bcdtmDrainage_appendBermToPondDtmObject(pondSideSlopesP,isBermFillOnly,fillTinP,bermSlope,bermWidth,isCrown,crownWidth,cornerStrokeTolerance)) goto errexit ;
        }
    /*
    ** Remove UnWanted Side Slope Features
...
cleanup :
    ...
    ** Job Completed
    */
    if( dbg && ! ret )  bcdtmWrite_message(0,0,0,"Designing Pond To A Target Elevation Completed") ;
    if( dbg &&   ret )  bcdtmWrite_message(1,0,0,"Designing Pond To A Target Elevation Error") ;
    return(ret) ;
    /*
    ** Error Exit
    */
...
/*==============================================================================*
* @memo   Designs A Pond To A Defined Volume Or Target Elevation
* @doc    Designs A Pond To A Defined Volume Or Target Elevation
* @notes
* &notes  1. For Target Volumes A Non Level Pond Boundary Has An Inherent Volume.
* @notes  2. The requested target volume may be less than the inherent volume
* @notes  3. When desining inside the Pond boundary the target volume may be greater than
* @notes     the maximum volume of the pond
*
* @author Rob Cormack 8 April 2003 rob@geopak.com
* @param  *pondObjectP     <== Pond Object With Break Lines
* @param  *pondFlagP       <== Target Volume Only - Flag To Indicate Design Results
*                          == 0  Target Volume Obtained
*                          == 1  Target Volume Less Than Inherent Volume Of Pond Boundary
*                          == 2  When Desiging Inside The Target Volume Exceeds The Maximum Volume Of The Pond
* @param  *pondElevationP  <== Elevation At Which Volume Calculations Were Performed For Designed Pond
* @param  *pondVolumeP     <== Design Pond Volume Calculated At Pond Elevation
* @param  *pondPtsP        ==> Defining Array Of DPoint3d Points For Pond
* @param  numPondPts       ==> Number Of DPoint3d Pond Pts
* @param  outsideInsideOpt ==> Flag To Indicate Inside Or Outside Of Pond Points <1 = Outside 2 = Inside>
* @param  volElevOpt       ==> Flag To Indicate A Target Volume Or Elevation <1 = Volume 2 = Elevation>
* @param  volume           ==> Target Volume
* @param  elevation        ==> Target Elevation
* @param  pondSlope        ==> Slope Of Pond Sides
* @param  freeBoard        ==> Height Of Free Board
* @param  isBerm           ==> Flag To Include A Berm < true For Berm , false For No Berm >
* @param  bermSlope        ==> Slope Of Berm
* @param  bermWidtn        ==> Width Of Berm
* @param  isCrown          ==> Flag To Include A Crown On Berm < true For Crown, false For No Crown >
* @param  crownWidth       ==> Width Of Crown Component Of Berm
* @param  isBermFillOnly   ==> Flag To Calculate A Berm Only In Fill Ares  <true,false >
* @param  *fillTinP        ==> Pointer To Fill Tin Object
* @param  fillSlope        ==> Fill Slope From Berm To Tin Object
* @return DTM_SUCCESS or DTM_ERROR
* @version
* @see None
*===============================================================================*/
int bcdtmDrainage_designPondToATargetVolumeOrElevationDtmObject
(
    BC_DTM_OBJ* pondObjectP,                                    /* <== Pond Object With Break Lines                                                 */
    DTMPondResult&       pondFlagP,                             /* <== Target Volume Only - Flag To Indicate Design Results                         */
                                                                /*  == 0  Target Volume Obtained                                                    */
                                                                /*  == 1  Target Volume Less Than Inherent Volume Of Pond Boundary                  */
                                                                /*  == 2  When Desiging Inside The Target Volume Exceeds The Maximum Volume Of The Pond  */
    double*              pondElevationP,                        /* <== Elevation At Which Volume Calculations Were Performed For Designed Pond      */
    double*              pondVolumeP,                           /* <== Design Pond Volume Calculated At Pond Elevation                              */
    DPoint3d*            pondPtsP,                              /* ==> Defining Array Of DPoint3d Points For Pond                                        */
    long                 numPondPts,                            /* ==> Number Of DPoint3d Pond Pts                                                       */
    DTMPondDesignMethod  outsideInsideOpt,                      /* ==> Flag To Indicate Inside Or Outside Of Pond Pts <1 = Outside,2 = Inside>      */
    DTMPondTarget        volElevOpt,                            /* ==> Flag To Indicate A Target Volume Or Elevation <1 = Volume ,2 = Elevation>    */
    double               volume,                                /* ==> Target Volume                                                                */
    double               elevation,                             /* ==> Target Elevation                                                             */
    double               pondSlope,                             /* ==> Slope Of Pond Sides                                                          */
    double               freeBoard,                             /* ==> Height Of Free Board                                                         */
    bool                 isBerm,                                /* ==> Flag To Include A Berm  <true For Berm , false For No Berm >                 */
    double               bermSlope,                             /* ==> Slope Of Berm                                                                */
    double               bermWidth,                             /* ==> Width Of Berm                                                                */
    bool                 isCrown,                               /* ==> Flag To Include A Crown On Berm < true For Crown false For No Crown>         */
    double               crownWidth,                            /* ==> Width Of Crown Component Of Berm                                             */
    double               cornerStrokeTolerance,                 /* ==> Corner Stroke Tolerance                                               */
    bool                 isBermFillOnly,                        /* ==> Flag To Calculate A Berm Only In Fill Ares  <true,false >                    */
    BC_DTM_OBJ*          fillTinP,                              /* ==> Pointer To Fill Tin Object                                                   */
    double               fillSlope                              /* ==> Fill Slope From Berm To Tin Object                                           */
)
    {
    int                     ret = DTM_SUCCESS, dbg = DTM_TRACE_VALUE(0);
    long                    numLocalPts, start = bcdtmClock();
    DTMDirection            direction;
    double                  area;
    DPoint3dP               p3d;
    ...
    */
    if (dbg)
        {
        bcdtmWrite_message(0, 0, 0, "Design Pond By Volume Or Target Elevation");
        bcdtmWrite_message(0, 0, 0, "pondObjectP      = %p", pondObjectP);
        bcdtmWrite_message(0, 0, 0, "pondPtsP          = %p", pondPtsP);
        bcdtmWrite_message(0, 0, 0, "numPondPts       = %6ld", numPondPts);
        bcdtmWrite_message(0, 0, 0, "outsideInsideOpt = %6ld", outsideInsideOpt);
        bcdtmWrite_message(0, 0, 0, "volElevOpt       = %6ld", volElevOpt);
        bcdtmWrite_message(0, 0, 0, "volume           = %10.4lf", volume);
        bcdtmWrite_message(0, 0, 0, "elevation        = %10.4lf", elevation);
        bcdtmWrite_message(0, 0, 0, "pondSlope        = %10.4lf", pondSlope);
        bcdtmWrite_message(0, 0, 0, "freeBoard        = %10.4lf", freeBoard);
        bcdtmWrite_message(0, 0, 0, "isBerm           = %6ld", isBerm);
        bcdtmWrite_message(0, 0, 0, "bermSlope        = %10.4lf", bermSlope);
        bcdtmWrite_message(0, 0, 0, "bermWidth        = %10.4lf", bermWidth);
        bcdtmWrite_message(0, 0, 0, "isCrown          = %6ld", isCrown);
        bcdtmWrite_message(0, 0, 0, "crownWidth       = %10.4lf", crownWidth);
        bcdtmWrite_message(0, 0, 0, "Corner Stroke    = %10.4lf", cornerStrokeTolerance);
        bcdtmWrite_message(0, 0, 0, "isBermFillOnly   = %6ld", isBermFillOnly);
        bcdtmWrite_message(0, 0, 0, "fillTinP         = %p", fillTinP);
        bcdtmWrite_message(0, 0, 0, "fillSlope        = %10.4lf", fillSlope);
        if (dbg == 2)
            {
            for (p3d = pondPtsP; p3d < pondPtsP + numPondPts; ++p3d)
                {
                bcdtmWrite_message(0, 0, 0, "Pond Point[%6ld] = %10.4lf %10.4lf %10.4lf", (long)(p3d - pondPtsP), p3d->x, p3d->y, p3d->z);
                }
            }
        }
    /*
    ...
    */
    if (pondObjectP == nullptr)
        {
        bcdtmWrite_message(1, 0, 0, "Pond Object Not Initialised"); goto errexit;
        }
    if (bcdtmObject_testForValidDtmObject(pondObjectP)) goto errexit;
    if (pondPtsP == nullptr || numPondPts <= 3)
        {
        bcdtmWrite_message(1, 0, 0, "Pond Points Not Defined"); goto errexit;
        }
    if (outsideInsideOpt != DTMPondDesignMethod::BottomUp && outsideInsideOpt != DTMPondDesignMethod::TopDown)
        {
        bcdtmWrite_message(1, 0, 0, "outsideInside Option Value Illegal"); goto errexit;
        }
    if (volElevOpt != DTMPondTarget::Elevation && volElevOpt != DTMPondTarget::Volume)
        {
        bcdtmWrite_message(1, 0, 0, "Volume Elevation Option Value Illegal"); goto errexit;
        }
    if (volElevOpt == DTMPondTarget::Volume && volume <= 0.0)
        {
        bcdtmWrite_message(1, 0, 0, "Volume Value Illegal"); goto errexit;
        }
    if (pondSlope == 0.0)
        {
        bcdtmWrite_message(1, 0, 0, "Pond Side Slope Value Illegal"); goto errexit;
        }
    if (freeBoard < 0.0)
        {
        bcdtmWrite_message(1, 0, 0, "Free Board Value Illegal"); goto errexit;
        }
    if (isBerm != true && isBerm != false)
        {
        bcdtmWrite_message(1, 0, 0, "Is Berm Value is Illegal"); goto errexit;
        }
    if (isCrown != true && isCrown != false)
        {
        bcdtmWrite_message(1, 0, 0, "Is Crown Value is Illegal"); goto errexit;
        }
    if (pondSlope < 0.0) pondSlope = -pondSlope;
    // if( bermSlope < 0.0 ) bermSlope = -bermSlope ;
    if (!isBerm)
        {
        isBermFillOnly = 0; fillTinP = nullptr;
        }
    if (isBermFillOnly && fillTinP == nullptr)
        {
        bcdtmWrite_message(1, 0, 0, "No Fill Tin Provided For Berm Fill Only"); goto errexit;
        }
    if (!isBermFillOnly) fillTinP = nullptr;
    if (cornerStrokeTolerance < 0.0) cornerStrokeTolerance = 100.0;
    /*
    ** Check Pond Closes
    */
    if (pondPtsP->x != (pondPtsP + numPondPts - 1)->x || pondPtsP->y != (pondPtsP + numPondPts - 1)->y)
        {
        bcdtmWrite_message(1, 0, 0, "Pond Element Does Not Close");
        pondPtsP->x = (pondPtsP + numPondPts - 1)->x;
        pondPtsP->y = (pondPtsP + numPondPts - 1)->y;
        }
    /*
    ** Make A Local Copy Of The Pond Pts
    */
    numLocalPts = numPondPts;
    localPts.resize(numLocalPts);
    memcpy(localPts.data(), pondPtsP, numLocalPts * sizeof(DPoint3d));
    /*
    ** Calculate Area And Direction Pond Polygon
    */
    bcdtmMath_getPolygonDirectionP3D(localPts.data(), numLocalPts, &direction, &area);
    /*
    ** Set Pond Polygon AntiClockwise
    */
    if (direction == DTMDirection::Clockwise) bcdtmMath_reversePolygonDirectionP3D(localPts.data(), numLocalPts);
    /*
    ** Set Pond Element Up If Top Down With A Free Board
    */
    /*
    Commented Out 29 September 2004 RobC
     if( outsideInsideOpt == 2 && freeBoard > 0.0 )
       {
        if( bcdtmDrainage_adjustPondElementToFreeBoardDtmObject(&localPts,&numLocalPts,freeBoard,pondSlope,cornerStrokeTolerance) != DTM_SUCCESS ) goto errexit ;
       }
    */
    /*
    ** Design Pond
    */
    if (volElevOpt == DTMPondTarget::Volume) if (bcdtmDrainage_designPondToATargetVolumeDtmObject(localPts.data(), numLocalPts, outsideInsideOpt, volume, pondSlope, freeBoard, isBerm, isBermFillOnly, fillTinP, bermSlope, bermWidth, isCrown, crownWidth, cornerStrokeTolerance, &pondDtmP, pondFlagP, pondElevationP, pondVolumeP) != DTM_SUCCESS) goto errexit;
    if (volElevOpt == DTMPondTarget::Elevation) if (bcdtmDrainage_designPondToATargetElevationDtmObject(localPts.data(), numLocalPts, outsideInsideOpt, elevation, pondSlope, freeBoard, isBerm, isBermFillOnly, fillTinP, bermSlope, bermWidth, isCrown, crownWidth, cornerStrokeTolerance, &pondDtmP, pondFlagP, pondElevationP, pondVolumeP) != DTM_SUCCESS) goto errexit;
    /*
    ** Write Design Pond Data
    */
    if (dbg && pondDtmP != nullptr)
        {
        bcdtmWrite_message(0, 0, 0, "Pond Elevation = %10.4lf ** Pond Volume = %15.4lf", *pondElevationP, *pondVolumeP);
        bcdtmWrite_geopakDatFileFromDtmObject(pondDtmP, L"pondDesign.dat");
        bcdtmObject_reportStatisticsDtmObject(pondDtmP);
        }
    /*
    ** Append Design Pond To Users Pond Object
    */
    if (pondDtmP != nullptr) if (bcdtmObject_appendDtmObject(pondObjectP, pondDtmP)) goto errexit;
    /*
    ** Clean Up
    */
cleanup:
    ...
    ** Job Completed
    */
    if (dbg) bcdtmWrite_message(0, 0, 0, "Time To Create Pond = %8.3lf Seconds", bcdtmClock_elapsedTime(bcdtmClock(), start));
    if (dbg && ret == DTM_SUCCESS)  bcdtmWrite_message(0, 0, 0, "Design Pond By Volume Or Target Elevation Completed");
    if (dbg && ret != DTM_SUCCESS)  bcdtmWrite_message(0, 0, 0, "Design Pond By Volume Or Target Elevation Error");
    return(ret);
    /*
    ** Error Exit
    */
