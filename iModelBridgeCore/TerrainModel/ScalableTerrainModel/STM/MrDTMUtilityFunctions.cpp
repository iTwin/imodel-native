/*--------------------------------------------------------------------------------------+
|
|     $Source: ScalableTerrainModel/STM/MrDTMUtilityFunctions.cpp $
...
int GetShapeInFrontOfProjectivePlane(vector<DPoint3d>&       shapeInFrontOfProjectivePlane, 
    ...
    HVE2DSegment            borderSegment(pDummyCoordSys);
    ...
    for (int segmentInd = 0; segmentInd < 4; segmentInd++)
        ...
        borderSegment.SetStartPoint(borderSegmentFirstPt);
        borderSegment.SetEndPoint(borderSegmentsecondPt);
...
int TriangulateDTM(Bentley::TerrainModel::DTMPtr&                     dtmPtr,
    ...
    bcdtmObject_setTriangulationParametersDtmObject(dtmObjP,
                                                    Bentley::MrDTM::IMrDTMQueryParameters::GetToleranceTriangulationParam(),
                                                    Bentley::MrDTM::IMrDTMQueryParameters::GetToleranceTriangulationParam(),
                                                    mrDTMQueryParamsPtr->GetEdgeOptionTriangulationParam() + 1,
                                                    mrDTMQueryParamsPtr->GetMaxSideLengthTriangulationParam());
    int status;
