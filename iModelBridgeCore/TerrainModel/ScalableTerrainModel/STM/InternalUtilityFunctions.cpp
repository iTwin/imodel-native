/*--------------------------------------------------------------------------------------+
|
|     $Source: ScalableTerrainModel/STM/InternalUtilityFunctions.cpp $
...
(
 ...
 double* maxLatitude
    ...
    StatusInt status = GCSPtr->GetMathematicalDomain(gcsGeoDomain);

    // Convert geo points to Image++ points
    for (int idx = 0 ; idx < gcsGeoDomain.size() ; idx++)
        myPoints.push_back(HGF2DCoord<double>(gcsGeoDomain[idx].longitude, gcsGeoDomain[idx].latitude));
