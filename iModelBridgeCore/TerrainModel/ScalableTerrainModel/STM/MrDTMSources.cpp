/*--------------------------------------------------------------------------------------+
|
|     $Source: ScalableTerrainModel/STM/MrDTMSources.cpp $
|    $RCSfile: MrDTMSources.cpp,v $
|   $Revision: 1.27 $
...
const WChar* IDTMDgnModelSource::GetPath() const
    {
    static StatusInt STATUS;
    return m_impl.GetPath(STATUS).c_str();
    }

const WChar* IDTMDgnModelSource::GetPath (StatusInt& status) const
    {
    return m_impl.GetPath(status).c_str();
    }
