/*--------------------------------------------------------------------------------------+
|
|     $Source: ScalableTerrainModel/PublicAPI/ScalableTerrainModel/IMrDTMStream.hpp $
|    $RCSfile: IMrDTMStream.hpp,v $
|   $Revision: 1.6 $
...
bool WriteArray (BinaryOStream& stream, const ValueT* array, size_t size)
    ...
    assert(sizeInBytes <= (numeric_limits<UInt32>::max)());

    const UInt32 sizeInBytesField = static_cast<UInt32>(sizeInBytes);
