/*--------------------------------------------------------------------------------------+
|
|     $Source: ScalableTerrainModel/GeoCoords/GCSWktParsing.cpp $
...
bool ExtractLocalCS(const WKTSection&       wktSection,
    ...
    const ULong localDatumType = wcstoul(localDatumTypeParameter.strBegin(), &localDatumTypeEnd, 10);
...
bool ExtractParameter (const WKTSection&    wktSection,
    ...
    WChar* rowEnd = 0;
    const ULong row = wcstoul(&name[4], &rowEnd, 10);

    WChar* colEnd = 0;
    const ULong col = wcstoul(&name[6], &colEnd, 10);
