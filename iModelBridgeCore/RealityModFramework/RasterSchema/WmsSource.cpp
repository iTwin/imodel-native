...
Utf8String WmsSource::BuildTileUrl(TileId const& tileId)
    ...
    tileUrl.Sprintf("%s?VERSION=%s&REQUEST=GetMap&LAYERS=%s&STYLES=%s&%s=%s&BBOX=%f,%f,%f,%f&WIDTH=%d&HEIGHT=%d&FORMAT=%s", 
        m_properties.m_url.c_str(), m_properties.m_version.c_str(), m_properties.m_layers.c_str(), m_properties.m_styles.c_str(), 
        m_properties.m_csType.c_str(), m_properties.m_csLabel.c_str(), 
        minX, minY, maxX, maxY,
        GetTileSizeX(tileId), GetTileSizeY(tileId), m_properties.m_format.c_str());
...
//         }
    ...
    if(!m_properties.m_vendorSpecific.empty())
        {
        tileUrl.append("&");
        tileUrl.append(m_properties.m_vendorSpecific);
        }
