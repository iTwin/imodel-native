...
void Dispatch(WorkOverlord* lord)
    ...
    while (s_keepRunning)
        ...
        if (s_innactiveUsers.size() == 0)
            {
            innactiveUserMutex.unlock();
            Sleep(2000);
            continue;
            }
...
void Stats::PrintStats()
    ...
    statMutex.lock();
    system("cls");
    std::cout << "Type      Success    Failure   minTime   maxTime   avgTime" << std::endl;
    std::cout << "Spatial " << opStats[OperationType::SPATIAL]->success << "      " << opStats[OperationType::SPATIAL]->failure << "       ";
    std::cout << opStats[OperationType::SPATIAL]->minTime << "      " << opStats[OperationType::SPATIAL]->maxTime << "      " << opStats[OperationType::SPATIAL]->avgTime << std::endl;

    std::cout << "PackId " << opStats[OperationType::PACKID]->success << "      " << opStats[OperationType::PACKID]->failure << "       ";
    std::cout << opStats[OperationType::PACKID]->minTime << "      " << opStats[OperationType::PACKID]->maxTime << "      " << opStats[OperationType::SPATIAL]->avgTime << std::endl;

    std::cout << "Package " << opStats[OperationType::PACKFILE]->success << " " << opStats[OperationType::PACKFILE]->failure << "       ";
    std::cout << opStats[OperationType::PACKFILE]->minTime << "      " << opStats[OperationType::PACKFILE]->maxTime << "      " << opStats[OperationType::SPATIAL]->avgTime << std::endl;
    statMutex.unlock();
    }
