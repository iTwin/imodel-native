...
template<class POINT, class EXTENT> class ScalableMesh2DDelaunayMesher : public ISMPointIndexMesher<POINT, EXTENT>
{
    ...
    private:
        ...
        void CreateGraph(HFCPtr<SMMeshIndexNode<POINT, EXTENT>>& node, const DPoint3d* points, int nbPoints, const int32_t* ptsIndice, int nbPtsIndice) const;
        void SelectPointsToStitch(std::vector<DPoint3d>& stitchedPoints, HFCPtr<SMMeshIndexNode<POINT, EXTENT> > node, MTGGraph* meshGraph, MTGMask featureMask, EXTENT neighborExt, vector<int>* pointsToDestPointsMap) const;
