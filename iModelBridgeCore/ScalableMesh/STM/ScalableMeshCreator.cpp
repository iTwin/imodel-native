...
StatusInt IScalableMeshCreator::Impl::CreateDataIndex (HFCPtr<MeshIndexType>&                                    pDataIndex, 
    ...
    ISMPointIndexMesher<PointType, PointIndexExtentType>* pMesher3d =
        ...
        if (!isSingleFile)
            ...
            pStreamingTileStore = new StreamingStoreType(account, streamingFilePath, StreamingStoreType::SMStoreDataType::POINTS, (SCM_COMPRESSION_DEFLATE == m_compressionType), true);
            // SM_NEEDS_WORKS : layerID 
            pStreamingIndiceTileStore = new StreamingIndiceStoreType(account, streamingFilePath, StreamingIndiceStoreType::SMStoreDataType::INDICES, (SCM_COMPRESSION_DEFLATE == m_compressionType));
            pStreamingUVTileStore = new StreamingUVStoreType(account, streamingFilePath, StreamingUVStoreType::SMStoreDataType::UVS, (SCM_COMPRESSION_DEFLATE == m_compressionType));
            pStreamingUVsIndicesTileStore = new StreamingIndiceStoreType(account, streamingFilePath, StreamingIndiceStoreType::SMStoreDataType::UVINDICES, (SCM_COMPRESSION_DEFLATE == m_compressionType));
