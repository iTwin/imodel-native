...
BENTLEY_SM_EXPORT void CreateGraphFromIndexBuffer(MTGGraph* graph, const int32_t* buffer, int count, int pointCount, bvector<int>& componentContours, const DPoint3d* points);

BENTLEY_SM_EXPORT void MergeGraphs(MTGGraph * destGraphP, std::vector<DPoint3d>& destPoints, MTGGraph * srcGraphP, std::vector<DPoint3d>& inPoints, DPoint3d minCorner, DPoint3d maxCorner, std::vector<int>& pointToDestPointsMap, bvector<int>& componentContours);
...
BENTLEY_SM_EXPORT void GetGraphExteriorBoundary(MTGGraph* graphP, bvector<bvector<DPoint3d>>& bound, const DPoint3d* points, bool pruneTriangleInnerLoops = false);
...
BENTLEY_SM_EXPORT void ExtractMeshIndicesFromGraph(std::vector<int32_t>& indices, MTGGraph* graphP);

BENTLEY_SM_EXPORT size_t FastCountNodesAroundFace(MTGGraph* graphP, MTGNodeId id);

BENTLEY_SM_EXPORT void PrintGraph(Utf8String path, Utf8String name, MTGGraph* graphP);

size_t CountExteriorFaces(MTGGraph* graphP);

BENTLEY_SM_EXPORT void UntieLoopsFromPolygon(bvector<DPoint3d>& polygon);

BENTLEY_SM_EXPORT bool RemoveKnotsFromGraph(MTGGraph* graphP, std::vector<DPoint3d>& ptsToModify);

void PrintGraphWithPointInfo(Utf8String path, Utf8String name, MTGGraph* graphP, const DPoint3d* pts, const size_t npts);

BENTLEY_SM_EXPORT void ResolveUnmergedBoundaries(MTGGraph * graphP);

void RecomputeExterior(MTGGraph * graphP);

void RemoveOverhangsFromPolygon(bvector<DPoint3d>& polygon);

BENTLEY_SM_EXPORT bool HasOverlapWithNeighborsXY(MTGGraph* graphP, MTGNodeId boundaryId, const DPoint3d* pts);

BENTLEY_SM_EXPORT void TagFeatureEdges(MTGGraph* graphP, DTMFeatureType tagValue, size_t featureSize, const int32_t* featureData, bool tagEnds = true);

BENTLEY_SM_EXPORT void ReadFeatureTags(MTGGraph * graphP, std::vector<int>& pointToDestPointsMap, bvector<bvector<int32_t>>& features, std::map<int,int>& componentForPoints);

BENTLEY_SM_EXPORT void ReadFeatureTags(MTGGraph * graphP, std::vector<int>& pointToDestPointsMap, bvector<bvector<int32_t>>& features);
