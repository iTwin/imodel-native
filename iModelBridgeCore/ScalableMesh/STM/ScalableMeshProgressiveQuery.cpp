...
QueryPlanner* ScalableMeshProgressiveQueryEngine::GetQueryPlanner(const RequestedQuery& queryObject)
{
    ...
    void QueryProcessor::LoadNodeDisplayData(IScalableMeshCachedDisplayNodePtr& meshNodePtr, HFCPtr<SMPointIndexNode<DPoint3d, Extent3dType>>& visibleNode, bool loadTexture, const bset<uint64_t>& clipVisibilities, IScalableMeshPtr& scalableMeshPtr, IScalableMeshDisplayCacheManagerPtr& displayCacheManagerPtr)
        ...
        if (!inLoading)
            ...
            TRACEPOINT(EventType::EVT_CREATE_DISPLAY_LOAD, visibleNode->GetBlockID().m_integerID, (uint64_t)-1, smNode->GetSingleTextureID(), -1, (uint64_t)meshNode, -1)
...
threadId);
        ...
        StatusInt QueryProcessor::GetUpdatedOverviewNodes(bvector<IScalableMeshCachedDisplayNodePtr>& overviewNodes, int queryId)
        {
            ...
            while (queryItr != queryItrEnd)
            {
                ...
                {
                    overviewNodes.clear();
                    for (size_t threadIter = 0; threadIter < (*queryItr)->m_updatedOverviewNodes.size(); threadIter++)
                    {
                        (*queryItr)->m_updatedOverviewNodeMutexes[threadIter].lock();
#ifdef DISPLAYLOG                 
                        /*
                        fprintf(logger.GetFile(), "threadId %i : nb found nodes : %I64d\n", threadIter, (*queryItr)->m_foundMeshNodes[threadIter].size());
                        fflush(logger.GetFile());
                        */
#endif
                        overviewNodes.insert(overviewNodes.end(), (*queryItr)->m_updatedOverviewNodes[threadIter].begin(), (*queryItr)->m_updatedOverviewNodes[threadIter].end());
                        (*queryItr)->m_updatedOverviewNodeMutexes[threadIter].unlock();
                    }
...
void ScalableMeshProgressiveQueryEngine::PreloadOverview(HFCPtr<SMPointIndexNode<DPoint3d, Extent3dType>>& node, IScalableMesh* sMesh)
    ...
    TRACEPOINT(EventType::EVT_CREATE_DISPLAY_OVR_PRELOAD, node->GetBlockID().m_integerID, (uint64_t)-1, smNode->GetSingleTextureID(), -1, (uint64_t)meshNode.get(), -1)
...
size_t threadId, IScalableMeshPtr* scalableMeshPtr, IScalableMeshDisplayCacheManagerPtr* displayCacheManagerPtr)
           ...
           TRACEPOINT(EventType::START_NEWQUERY_SEARCHNODES, threadId, (uint64_t)-1, -1, -1, elapsedSearch, m_nodesToSearch->GetNodes().size() - m_nodeToSearchCurrentInd)
            ...
            TRACEPOINT(EventType::START_NEWQUERY_FOUNDNODES, threadId, (uint64_t)-1, -1, -1, elapsedFound, nNodes)
            double elapsed = ((double)clock() - startT) / CLOCKS_PER_SEC * 1000.0;
            TRACEPOINT(EventType::START_NEWQUERY, threadId, (uint64_t)-1, -1, -1, elapsed, nNodes)
            }
...
BentleyStatus ScalableMeshProgressiveQueryEngine::_GetOverviewNodes(bvector<BENTLEY_NAMESPACE_NAME::ScalableMesh::IScalableMeshCachedDisplayNodePtr>& meshNodes,
    ...
    if (requestedQueryP != 0)
        ...
        if (s_useIntermediateOverviews)
        {
            ...
            TRACEPOINT(EventType::QUERY_NUMBER_OF_OVERVIEWS, queryId, (uint64_t)-1, -1, -1, (int)newPreviewNodes.size(), 0)
