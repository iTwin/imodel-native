/*--------------------------------------------------------------------------------------+
|
|     $Source: STM/ScalableMeshRasterDraping.cpp $
|    $RCSfile: ScalableMeshRasterDraping.cpp,v $
|   $Revision: 1.7 $
...
public :
    ...
    friend class ScalableMeshRasterDraping;
...
/*----------------------------------------------------------------------------+
|ScalableMeshRasterDraping::ScalableMeshRasterDraping
+----------------------------------------------------------------------------*/
ScalableMeshRasterDraping* ScalableMeshRasterDraping::Create(const AString& pi_rFileNameDescriptors)
    {
    ScalableMeshRasterDraping* pScalableMeshRasterDraping = 0;
  
    try
        {
        pScalableMeshRasterDraping = new ScalableMeshRasterDraping(pi_rFileNameDescriptors);

        //The mosaic was not created, likely because no valid raster file 
        //was found.


        if (pScalableMeshRasterDraping->GetImageppOpaqueData()->GetDrapingMosaic() == 0)
            {
            delete pScalableMeshRasterDraping;
            pScalableMeshRasterDraping = 0;
            }
        }
    catch (HFCException&)
        {
    ...
    return pScalableMeshRasterDraping;
    }

/*----------------------------------------------------------------------------+
|ScalableMeshRasterDraping::ScalableMeshRasterDraping
+----------------------------------------------------------------------------*/
ScalableMeshRasterDraping::ScalableMeshRasterDraping(const AString& pi_rFileNameDescriptors)
    {    
...
/*----------------------------------------------------------------------------+
|ScalableMeshRasterDraping::CreateDrapingMosaic
+----------------------------------------------------------------------------*/
#ifdef __HMR_UNICODE
...
void ScalableMeshRasterDraping::CreateTextureBitmapForCurrentView(vector<TextureDescription>&  rTextures,    
                 ...
                 //       pTextureBitmap->Clear();
                  ...
                  //      pTextureBitmap->CopyFrom(pSourceRaster, copyFromOptions);
                        ...
                        if (OutputBitmapBuffer == true)
                            ...
                            sprintf(TempBuffer, "file://F:\\MyDoc\\SS2 - Iteration 7\\ScalableMesh\\Visualization - Analysis\\Draping\\Texture%i_%i.bmp", BitmapInd, textureInd++);
    ...
    if (pEffectiveArea->IsEmpty() == false)
        ...
        if (ScaleX != Scale)
            ...
            {    
                ...
                sprintf(TempBuffer, "file://F:\\MyDoc\\SS2 - Iteration 7\\ScalableMesh\\Visualization - Analysis\\Draping\\ChessTexture.bmp");
...
void ScalableMeshRasterDraping::ComputePlaneNormal(DVec3d& rNormalVec,
                                            DPoint3d*    pFencePt,
                                            int     nbFencePt)
    {
...
void ScalableMeshRasterDraping::ComputeDifferentResAreas(vector<CopyFromAreaInfo>& rCopyFromAreas,    
...
double ScalableMeshRasterDraping::ComputeMinPixelSizeRequired(double&           po_rMinBitmapSizeInUor,
...
void ScalableMeshRasterDraping::ComputeDifferentResAreasUor(DMatrix4dCP pi_pRootToViewMatrix,
                                                  double      pi_xMin,
                                                  double      pi_xMax,
                                                  double      pi_yMin,
                                                  double      pi_yMax,
...
double ScalableMeshRasterDraping::ComputeMinPixelSizeRequiredUor(double&     po_rMaxBitmapSize,
...
void ScalableMeshRasterDraping::ComputeDestinationExtentFromSourceExtent(double&     po_dstXMin,
                                                                  double&     po_dstXMax,
                                                                  double&     po_dstYMin,
                                                                  double&     po_dstYMax,      
...
void ScalableMeshRasterDraping::ReleaseLastTextureBitmap()
    {
...
/*----------------------------------------------------------------------------+
|static ScalableMeshRasterDraping::LocateWorldClusterInDGNWorld
|
|NOTE : Function mostly based on int HGFUSWorldCluster::Synchronize() in RasterLib. The RasterLib version
|       should be used if we decide that ScalableMesh.dll is linking the RasterLib.
|
+----------------------------------------------------------------------------*/
void ScalableMeshRasterDraping::LocateWorldClusterInCurrentModel(double meterPerMasterUnits,
...
const ImageppOpaqueData* ScalableMeshRasterDraping::GetImageppOpaqueData() const
    {
