...
bool OpenResultFile(BeXmlNodeP pRootNode, FILE*& pResultFile)
    ...
    if (status == BEXML_Success)
        ...
        if (status == BEXML_Success && timestampVal.CompareTo(L"yes") == 0)
            ...
            WString name; 
            WString ext;
            BeFileName::ParseName(&dev, &dir, &name, &ext, timestampVal.c_str());

            std::time_t t = std::time(nullptr);
            wchar_t timeStamp[1000];
            std::wcsftime(timeStamp, sizeof(timeStamp), L"_%H_%M_%S__%d_%m_%y", std::localtime(&t));
            
            resultFileName = dev + dir + name + WString(timeStamp) + ext;
            }
