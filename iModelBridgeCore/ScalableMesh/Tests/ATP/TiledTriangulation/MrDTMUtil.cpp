...
+---------------+---------------+---------------+---------------+---------------+------*/
/*WString GetFileNameFor (const IMrDTMDataRef& dataRef)
    {
        
    const DgnDocumentPtr docPtr = dataRef.GetMrDTMDocument();
    WString fileNameStr;
    if (0 != docPtr.get())
        return docPtr->GetFileName();

    // Try with the moniker
    const DgnDocumentMonikerPtr monikerPtr( dataRef.GetFileMoniker());   

    return (0 == monikerPtr.get()) ? L"" : monikerPtr->GetShortDisplayName();
    }*/

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                   Mathieu.St-Pierre  09/2012
+---------------+---------------+---------------+---------------+---------------+------*/
//std::thread::id main_thread_id = std::this_thread::get_id();

/*bool IsProcessTerminatedByUser()
    {
    //Required because the triangulation algo can call this function in a thread differing from the main and 
    //mdlSystem_getChar can crash when not called in main thread. 
    if (main_thread_id == std::this_thread::get_id())
        {
        return (0 != mdlSystem_extendedAbortRequested()) || (ESC_KEY == mdlSystem_getChar());
        }
    else
        {
        return false;
...
//=======================================================================================
// @bsimethod                                                   Mathieu.St-Pierre 08/13
//=======================================================================================
/*int AddClipOnQuery(IScalableMeshQueryPtr&         queryPtr,
                   const vector<ClipInfo>& clips)
    {
    vector<ClipInfo>::const_iterator clipIter(clips.begin());
    vector<ClipInfo>::const_iterator clipIterEnd(clips.end());

    while (clipIter != clipIterEnd)
        {
        vector<DPoint3d> tempClipPts(clipIter->m_region);

        queryPtr->AddClip(const_cast<DPoint3d*>(&tempClipPts.front()), (int)clipIter->m_region.size(), clipIter->m_isMask);
...
// @bsimethod                                                   Mathieu.St-Pierre 08/13
//=======================================================================================
/*int GetClipForQuery(vector<ClipInfo>& clips, ElementHandleCR eeh)
    {
    RefCountedPtr<DTMDataRef> DTMDataRef;
    ...
    if ((mrRef->GetClipActivation() == true) && (mrRef->GetNbClips() > 0))
        {
        ...
        for (int clipInd = 0; clipInd < mrRef->GetNbClips(); clipInd++)
            {
            ClipInfo clipInfo;

            status = mrRef->GetClip(clipPointsP, numberOfClipPoints, clipInfo.m_isMask, clipInd);

            assert(clipPointsP != 0);

            bsiTransform_multiplyDPoint3dArrayInPlace(&invertTrsf, clipPointsP, numberOfClipPoints);

            for (int clipPtInd = 0; clipPtInd < numberOfClipPoints; clipPtInd++)
                {
                clipInfo.m_region.push_back(clipPointsP[clipPtInd]);
                }
...
* @bsimethod                                                   Mathieu.St-Pierre  09/2012
+---------------+---------------+---------------+---------------+---------------+------*/
//TR 353212 - Temporary fix, the real fix should be that the query returns that the user aborted the process,
//            but that requires a lot of modifcations in the platform.
/*static bool s_queryProcessTerminatedByUser = false;

int CheckTriangulationStopCallback(DTMFeatureType dtmFeatureType)
    {
    if (IsProcessTerminatedByUser() == true)
        {
        s_queryProcessTerminatedByUser = true;
        return DTM_ERROR;
        }
...
/*StatusInt updateDTMPtrWithSTMDirect(DTMPtr& dtmPtr, const DPoint3d* regionPts, size_t nbPts, IScalableMeshPtr mrDTMPtr,
    ...
    if (fullResLinearQueryPtr != 0)
        ...
        if (maximumNbLinearFeaturePoints == NB_LINEAR_FEATURE_POINTS_REQUESTED_FULL_RESOLUTION)
            {
            unsigned int maxNbLinearFeaturePoints = (unsigned int)max(remainingPointCount, 11);
        ...
        if ((status = fullResLinearQueryPtr->Query(dtmPtr, regionPts, (int)nbPts, IScalableMeshQueryParametersPtr(mrDtmFullResLinearParametersPtr))) != SUCCESS)
            {
    ...
    if (fixResPointQueryPtr != 0)
        ...
        status = fixResPointQueryPtr->Query(singleResolutionViewDtmPtr, 0, 0, IScalableMeshQueryParametersPtr(mrDtmFixResqueryParamsPtr));
...
/*        mrDtmFullResQueryParam->SetMaximumNumberOfPoints((unsigned int)remainingPointCount);
        ...
        if ((status = fullResQueryPtr->Query(dtmPtr, regionPts, (unsigned int)nbPts, IScalableMeshQueryParametersPtr(mrDtmFullResQueryParam))) != SUCCESS)
            {
            bool isSet = SetTriangulationTerminationCallback(0);
            assert(true == isSet);
            return status;
...
+---------------+---------------+---------------+---------------+---------------+------*/
/*StatusInt updateDTMPtrWithSTMDirect(DTMPtr& dtmPtr, const DPoint3d* regionPts, size_t nbPts, ElementHandleCR eeh,
                                    bool applyTriangulation, size_t& remainingPointCount, bool applyClip,
                                    double decimationFactorForPointFeatures, unsigned int maximumNbLinearFeaturePoints)
    {
    RefCountedPtr<DTMDataRef> DTMDataRef;
    if (DTMElementHandlerManager::GetDTMDataRef (DTMDataRef, eeh) != SUCCESS)
    ...
    IMrDTMDataRef& mrRef = static_cast<IMrDTMDataRef&>(*DTMDataRef);

    if (mrRef.GetMrDTMState() == SCM_STATE_EMPTY)
        return ERROR;

    IScalableMeshPtr mrDTMPtr(dynamic_cast<IScalableMesh*>(DTMDataRef->GetDTMStorage(DTMDataRefPurpose::GetMrDtm)));

    if (mrDTMPtr == 0)
        return ERROR;

    int edgeMethod;
    mrRef.GetEdgeMethod(edgeMethod);
    ...
    return updateDTMPtrWithSTMDirect(dtmPtr, regionPts, nbPts, mrDTMPtr,
                                     applyTriangulation, remainingPointCount, clips, applyClip,
                                     edgeMethod, mrRef.GetEdgeMethodLengthInStorage(), drange, mrRef.GetDestinationGCS(),
