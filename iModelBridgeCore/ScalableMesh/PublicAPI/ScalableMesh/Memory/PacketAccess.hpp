/*--------------------------------------------------------------------------------------+
|
|     $Source: PublicAPI/ScalableMesh/Memory/PacketAccess.hpp $
...
template <typename T>
void NonConstPacketProxyBase<T>::Reserve(size_t newCapacity)
    {
    GetImpl().Reserve(newCapacity*sizeof(T));
    }


template <typename T>
void NonConstPacketProxyBase<T>::SetSize (size_t                  pi_Size)
    {
...
template <typename T>
void NonConstPacketProxyBase<T>::SetEnd (iterator pi_newEndIt)
    {
