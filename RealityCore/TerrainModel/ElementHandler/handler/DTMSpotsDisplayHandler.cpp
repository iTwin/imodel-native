...
//=======================================================================================
bool DTMElementSpotsDisplayHandler::_Draw (ElementHandleCR el, const ElementHandle::XAttributeIter& xAttr, DTMDrawingInfo& drawingInfo, ViewContextR context)
    {
    ...
    if (purpose == DrawPurpose::ChangedPre)
        ...
        if (DrawPurpose::RangeCalculation != purpose)
            {
            DTMElementSpotsHandler::DisplayParams params (xAttr);
            ...
            DTMStrokeForPoints stroker ( dtm, params, drawingInfo, xAttr.GetId(), adp.GetRescaleFactor (), [] (DTMStrokeForPoints& stroker, BcDTMP dtm, const DTMFenceParams& fenceParams, DTMFeatureCallback defaultCallback)
                {
                dtm->BrowseFeatures (DTMFeatureType::TinPoint, fenceParams, 10000, &stroker, defaultCallback);
                });
