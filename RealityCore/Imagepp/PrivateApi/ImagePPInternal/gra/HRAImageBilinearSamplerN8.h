...
/*----------------------------------------------------------------------------+
|struct HRAImageBilinearSamplerN8
...
struct HRAImageBilinearSamplerN8 : HRAImageSampler
{
public:
    // Use HRAImageSampler::CreateBilinear(...) to instanciate this class.
    friend HRAImageSampler;
	virtual ~HRAImageBilinearSamplerN8();
...
private:
    ...
    * @bsimethod                                                   Mathieu.Marchand  10/2014
    ...
    struct SamplerSurfaceVisitor : public SurfaceVisitor
        ...
        virtual ImagePPStatus _Visit(HRASampleRleSurface& surface) override { return IMAGEPP_STATUS_UnknownError; }
        ...
        virtual ImagePPStatus _Visit(HRAPacketRleSurface& surface) override { return IMAGEPP_STATUS_UnknownError; }

        // Only N8.
        virtual ImagePPStatus _Visit(HRAPacketN8Surface& surface) override { return Process(surface); }
        virtual ImagePPStatus _Visit(HRASampleN8Surface& surface) override { return Process(surface); }
    ...
    HRAImageBilinearSamplerN8(HFCPtr<HGF2DTransfoModel>& destToSrcTransfo);

    template<typename Surface_T>
    void Stretch_T(HRAImageSampleR outData, PixelOffset const& outOffset, Surface_T& inData, PixelOffset const& inOffset);
    ...
    vector<float> m_realPixelOffSet;        // Used by _Stretch.
    // ***** End Not-thread safe *****
};
