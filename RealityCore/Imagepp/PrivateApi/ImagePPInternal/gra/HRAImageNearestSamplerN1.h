...
/*----------------------------------------------------------------------------+
|struct HRAImageNearestSamplerN1
| This class is not thread safe and thus cannot be use by multiple thread at the 
| same time. See m_pixelOffSet members.  
+----------------------------------------------------------------------------*/
struct HRAImageNearestSamplerN1 : HRAImageSampler
    {
public:
    // Use HRAImageSampler::CreateNearestN1(...) to instanciate this class.
    friend HRAImageSampler;

    virtual ~HRAImageNearestSamplerN1();
...
private:
    ...
    * @bsimethod                                                   Mathieu.Marchand  10/2014
    ...
    struct SamplerSurfaceVisitor : public SurfaceVisitor
        ...
        virtual ImagePPStatus _Visit(HRASampleN1Surface& surface) override { return Process(surface); }
        virtual ImagePPStatus _Visit(HRAPacketN1Surface& surface) override { return Process(surface); }
    ...
    HRAImageNearestSamplerN1(HFCPtr<HGF2DTransfoModel>& destToSrcTransfo);
