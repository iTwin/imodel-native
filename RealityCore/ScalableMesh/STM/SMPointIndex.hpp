...
template<class POINT, class EXTENT> SMPointIndexNode<POINT, EXTENT>::SMPointIndexNode (size_t pi_SplitTreshold,
    ...
    //m_nodeHeader.m_geometricResolution = this->m_nodeHeader.m_textureResolution = static_cast<float>(sqrt(m_nodeHeader.m_nodeExtent.ExtentSquared()));
...
template<class POINT, class EXTENT>  HFCPtr<SMPointIndexNode<POINT, EXTENT> > SMPointIndexNode<POINT, EXTENT>::CreateNewNode(HPMBlockID blockID, bool isRootNode)
    {    
...
template<class POINT, class EXTENT> void SMPointIndexNode<POINT, EXTENT>::Load() const
    ...
    if (this == m_SMIndex->GetRootNode())
        ...
        typename CreatedNodeMap::iterator nodeIter(m_createdNodeMap->find(GetBlockID().m_integerID));

        if (nodeIter == m_createdNodeMap->end())
            {
            m_createdNodeMap->insert(std::pair<int64_t, HFCPtr<SMPointIndexNode<POINT, EXTENT>>>(GetBlockID().m_integerID, UNCONSTTHIS));
            }
        }
    ...
    if (!UNCONSTTHIS->m_nodeHeader.m_IsLeaf)
        ...
        if (!UNCONSTTHIS->m_nodeHeader.m_IsBranched)
            ...
            typename CreatedNodeMap::iterator nodeIter(m_createdNodeMap->find(UNCONSTTHIS->m_nodeHeader.m_SubNodeNoSplitID.m_integerID));

            if (nodeIter == m_createdNodeMap->end())
                {                
                UNCONSTTHIS->m_pSubNodeNoSplit = UNCONSTTHIS->CreateNewChildNode(UNCONSTTHIS->m_nodeHeader.m_SubNodeNoSplitID);
                m_createdNodeMap->insert(std::pair<int64_t, HFCPtr<SMPointIndexNode<POINT, EXTENT>>>(UNCONSTTHIS->m_nodeHeader.m_SubNodeNoSplitID.m_integerID, UNCONSTTHIS->m_pSubNodeNoSplit));                
                }
            else
                {
            ...
            for (size_t indexNode = 0 ; indexNode < m_nodeHeader.m_numberOfSubNodesOnSplit; indexNode++)
                ...
                typename CreatedNodeMap::iterator nodeIter(m_createdNodeMap->find(UNCONSTTHIS->m_nodeHeader.m_apSubNodeID[indexNode].m_integerID));
    ...
    if (m_loadNeighbors)
        ...
        for (size_t neighborPosIndex = 0; neighborPosIndex < MAX_NEIGHBORNODES_COUNT; neighborPosIndex++)
            ...
            for (size_t neigborIndex = 0; neigborIndex < UNCONSTTHIS->m_nodeHeader.m_apNeighborNodeID[neighborPosIndex].size(); neigborIndex++)
                ...
                typename CreatedNodeMap::iterator nodeIter(m_createdNodeMap->find(UNCONSTTHIS->m_nodeHeader.m_apNeighborNodeID[neighborPosIndex][neigborIndex].m_integerID));
                ...
                if (nodeIter == m_createdNodeMap->end())
                    ...
                    m_createdNodeMap->insert(std::pair<int64_t, HFCPtr<SMPointIndexNode<POINT, EXTENT>>>(UNCONSTTHIS->m_nodeHeader.m_apNeighborNodeID[neighborPosIndex][neigborIndex].m_integerID, UNCONSTTHIS->m_apNeighborNodes[neighborPosIndex].back()));
                    }
                else
                    {
    ...
    if (!IsParentSet())
        ...
        if (UNCONSTTHIS->m_nodeHeader.m_parentNodeID != ISMStore::GetNullNodeID())
            ...
            typename CreatedNodeMap::iterator nodeIter(m_createdNodeMap->find(UNCONSTTHIS->m_nodeHeader.m_parentNodeID.m_integerID));            

            if (nodeIter == m_createdNodeMap->end())
                {
                UNCONSTTHIS->SetParentNodePtr(UNCONSTTHIS->CreateNewNode(UNCONSTTHIS->m_nodeHeader.m_parentNodeID.m_integerID));
                m_createdNodeMap->insert(std::pair<int64_t, HFCPtr<SMPointIndexNode<POINT, EXTENT>>>(UNCONSTTHIS->m_nodeHeader.m_parentNodeID.m_integerID, UNCONSTTHIS->GetParentNodePtr()));                
                }
            else
                {
    ...
    if (IsLoaded())
        ...
        if (!UNCONSTTHIS->m_nodeHeader.m_IsLeaf)
            ...
            if (UNCONSTTHIS->m_pSubNodeNoSplit != NULL)
                ...
                typename CreatedNodeMap::iterator nodeIter(m_createdNodeMap->find(UNCONSTTHIS->m_pSubNodeNoSplit->GetBlockID().m_integerID));
                ...
                for (size_t indexNode = 0; indexNode < m_nodeHeader.m_numberOfSubNodesOnSplit; indexNode++)
                    ...
                    if (UNCONSTTHIS->m_apSubNodes[indexNode] != NULL)
                        ...
                        typename CreatedNodeMap::iterator nodeIter(m_createdNodeMap->find(UNCONSTTHIS->m_apSubNodes[indexNode]->GetBlockID().m_integerID));
  ...
  //         ((m_apNeighborNodes[parentNeighborInd].size() == 1) /*&& (m_apNeighborNodes[parentNeighborInd][0]->m_nodeHeader.m_level < m_apSubNodes[subNodeInd]->m_nodeHeader.m_level)*/));
    ...
    */
    ...
    if (m_apNeighborNodes[parentNeighborInd].size() == 1)
        {
        struct ParentNodeIDs
            {
            int neighborSubNodeID, subNodeNeighborID, neighborSubNodeNeighborID;
            };
        bvector<ParentNodeIDs> parentNeighborInfo;
        switch (parentNeighborInd)
            ...
            case 0:
                ...
                switch (subNodeInd)
                    {
                    case 0:
                        parentNeighborInfo.push_back({3,0,7});
                        break;
                    case 4:
                        parentNeighborInfo.push_back({ 7,-1,-1 });
                        break;
            ...
            case 1:
                ...
                switch (subNodeInd)
                    {
                    case 0:
                        parentNeighborInfo.push_back({ 2,1,6 });
                        parentNeighborInfo.push_back({ 3,2,5 });
                        break;
                    case 1:
                        parentNeighborInfo.push_back({ 2,0,7 });
                        parentNeighborInfo.push_back({ 3,1,6 });
                        break;
                    case 4:
                        parentNeighborInfo.push_back({ 6,0,-1 });
                        break;
                    case 5:
                        parentNeighborInfo.push_back({ 7,-1,-1 });
                        break;
            ...
            case 2:
                ...
                switch (subNodeInd)
                    {
                    case 1:
                        parentNeighborInfo.push_back({ 2,2,5 });
                        break;
                    case 5:
                        parentNeighborInfo.push_back({ 6,-1,-1 });
                        break;
            ...
            case 3:
                ...
                switch (subNodeInd)
                    {
                    case 0:
                        parentNeighborInfo.push_back({ 1,3,4 });
                        parentNeighborInfo.push_back({ 3,5,2 });
                        break;
                    case 2:
                        parentNeighborInfo.push_back({ 1,0,7 });
                        parentNeighborInfo.push_back({ 3,3,4 });
                        break;
                    case 4:
                        parentNeighborInfo.push_back({ 5,-1,-1 });
                        break;
                    case 6:
                        parentNeighborInfo.push_back({ 7,-1,-1 });
                        break;
            ...
            case 4:
                ...
                switch (subNodeInd)
                    {
                    case 1:
                        parentNeighborInfo.push_back({ 0,4,3 });
                        parentNeighborInfo.push_back({ 2,7,0 });
                        break;
                    case 3:
                        parentNeighborInfo.push_back({ 2,4,3 });
                        parentNeighborInfo.push_back({ 0,2,5 });
                        break;
                    case 5:
                        parentNeighborInfo.push_back({ 4,-1,-1 });
                        break;
                    case 7:
                        parentNeighborInfo.push_back({ 6,-1,-1 });
                        break;
            ...
            case 5:
                ...
                switch (subNodeInd)
                    {
                    case 2:
                        parentNeighborInfo.push_back({ 1,5,2 });
                        break;
                    case 6:
                        parentNeighborInfo.push_back({ 5,-1,-1 });
                        break;
            ...
            case 6:
                ...
                switch (subNodeInd)
                    {
                    case 2:
                        parentNeighborInfo.push_back({ 0,6,1 });
                        parentNeighborInfo.push_back({ 1,7,0 });
                        break;
                    case 3:
                        parentNeighborInfo.push_back({ 1,6,1 });
                        parentNeighborInfo.push_back({ 0,5,2 });
                        break;
                    case 6:
                        parentNeighborInfo.push_back({ 4,-1,-1 });
                        break;
            ...
            case 7:
                ...
                switch (subNodeInd)
                    {
                    case 3:
                        parentNeighborInfo.push_back({ 0,7,0 });
                        break;
                    case 7:
                        parentNeighborInfo.push_back({ 4,-1,-1 });
                        break;
            ...
            case 17:
                ...
                switch (subNodeInd)
                    {
                    case 4:
                        parentNeighborInfo.push_back({ 3,-1,-1 });
                        break;
            ...
            case 18:
                ...
                switch (subNodeInd)
                    {
                    case 4:
                        parentNeighborInfo.push_back({ 2,-1,-1 });
                        break;
            ...
            case 20:
                ...
                switch (subNodeInd)
                    {
                    case 4:
                        parentNeighborInfo.push_back({ 1,-1,-1 });
                        break;
            ...
            case 21:
                ...
                switch (subNodeInd)
                    {
                    case 4:
                        parentNeighborInfo.push_back({ 0,-1,-1 });
                        break;
        ...
        for(int i = 0; i < parentNeighborInfo.size(); i++)
            {
            auto neighborSubNode = m_apNeighborNodes[parentNeighborInd][0]->m_apSubNodes[parentNeighborInfo[i].neighborSubNodeID];
            auto& currentSubNodeNeighbor = m_apSubNodes[subNodeInd]->m_apNeighborNodes[parentNeighborInfo[i].subNodeNeighborID];
            if(m_apNeighborNodes[parentNeighborInd][0]->m_apSubNodes.size() >= parentNeighborInfo[i].neighborSubNodeID && neighborSubNode != 0)
                {
                //NEEDS_WORK_SM:Reactivate this
               BeAssert(neighborSubNode->m_nodeHeader.m_level == m_apSubNodes[subNodeInd]->m_nodeHeader.m_level);
               BeAssert(currentSubNodeNeighbor.size() == 0);

               currentSubNodeNeighbor.push_back(neighborSubNode);
                
                //NEEDS_WORK_SM:Reactivate this
                BeAssert(neighborSubNode->m_apNeighborNodes[parentNeighborInfo[i].neighborSubNodeNeighborID].size() == 0);

                neighborSubNode->m_apNeighborNodes[parentNeighborInfo[i].neighborSubNodeNeighborID].push_back(m_apSubNodes[subNodeInd]);
                }
            }
        }
    }
...
template<class POINT, class EXTENT> void SMPointIndexNode<POINT, EXTENT>::AdviseNeighborNodeChange(const HFCPtr<SMPointIndexNode<POINT, EXTENT> >& oldNeighborNode, HFCPtr<SMPointIndexNode<POINT, EXTENT> >& newNeighborNode)
    ...
    for (; neighborPosInd < MAX_NEIGHBORNODES_COUNT; neighborPosInd++)
        ...
        if (m_nodeHeader.m_apNeighborNodeID[neighborPosInd].size() > 0)
            {
            std::vector<HPMBlockID>::iterator neighborIdIter(m_nodeHeader.m_apNeighborNodeID[neighborPosInd].begin());
            std::vector<HPMBlockID>::iterator neighborIdIterEnd(m_nodeHeader.m_apNeighborNodeID[neighborPosInd].end());
...
template<class POINT, class EXTENT> void SMPointIndexNode<POINT, EXTENT>::Track3dPoints(size_t countPoints)
    ...
    if (!merged)
        {
        m_nodeHeader.m_3dPointsDescBins.push_back(typename SMIndexNodeHeaderBase<EXTENT>::RLC3dPoints(ptsPtr->size(), countPoints));
        }
    }
...
void SMPointIndexNode<POINT, EXTENT>::SetNodeExtent(const EXTENT& extent)
    ...
    //m_nodeHeader.m_geometricResolution = static_cast<float>(sqrt(m_nodeHeader.m_nodeExtent.ExtentSquared()));
    //if (!this->IsTextured())
    //    this->m_nodeHeader.m_textureResolution = m_nodeHeader.m_geometricResolution;
...
template<class POINT, class EXTENT> bool SMPointIndex<POINT, EXTENT>::Store()
    ...
    if (m_pRootNode != NULL)
        ...
        if (s_dropNodes)
            {                           
            DumpOctTree((char *)"D:\\MyDoc\\Scalable Mesh Iteration 8\\PartialUpdate\\Neighbor\\Log\\NodeAferCreationR.xml", false);                
