 ...
 * @bsimethod
 +---------------+---------------+---------------+---------------+---------------+------*/
static int filterCaller(void* tracker, Utf8CP tableName) {
    return (int)((ChangeTracker*)tracker)->_FilterTable(tableName);
}
...
DbResult Changes::Change::GetOperation(Utf8CP* tableName, int* nCols, DbOpcode* opcode, int* indirect) const {
    return (DbResult)sqlite3changeset_op(m_iter, tableName, nCols, (int*)opcode, indirect);
}
DbResult Changes::Change::GetPrimaryKeyColumns(Byte** cols, int* nCols) const {
    return (DbResult)sqlite3changeset_pk(m_iter, cols, nCols);
}
DbResult Changes::Change::GetFKeyConflicts(int* nConflicts) const {
    return (DbResult)sqlite3changeset_fk_conflicts(m_iter, nConflicts);
}
...
void Changes::Change::DumpCurrentValuesOfChangedColumns(Db const& db) const {
    ...
    for (int i = 0; i <= npcols; ++i) {
        ...
        if (!GetValue(i, Changes::Change::Stage::Old).IsValid() && !GetValue(i, Changes::Change::Stage::New).IsValid())
