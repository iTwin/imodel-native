...
ECN::ECClassId ecClassid
    ...
    /* Create instance */
    ECClassP ecClass;
    if (SUCCESS != m_ecSqlStatement.GetECDb()->GetEC().GetSchemaManager().GetECClass(ecClass, ecClassid))
        return nullptr;
...
ECN::IECInstancePtr ECInstanceECSqlSelectAdapter::GetInstance() const
    ...
    if (-1 != m_ecClassIdColumnIndex)
        ...
        m_ecSqlStatement.GetECDb()->GetEC().GetSchemaManager().GetECClass(ecClass, value.GetInt64());
        }
    else
        {
        for(int i=0; i < m_ecSqlStatement.GetColumnCount(); i++)
            ...
            if (columnInfo.IsGeneratedProperty())
                continue;
            ecClass = const_cast<ECN::ECClassP>(&(columnInfo.GetRootClass()));
            break;
            }
        }
...
bool isSource
    ...
    ECClassP endpointClass;
    m_ecSqlStatement.GetECDb()->GetEC().GetSchemaManager().GetECClass(endpointClass, endpointClassId);
