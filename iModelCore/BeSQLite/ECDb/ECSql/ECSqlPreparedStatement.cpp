...
//---------------------------------------------------------------------------------------
ECSqlPreparedStatement::ECSqlPreparedStatement (ECSqlType type, ECDbR ecdb, ECSqlEventManager& eventManager, ECSqlStatusContext& statusContext)
...
ECSqlStatus ECSqlPreparedStatement::ClearBindings ()
    ...
    if (nativeSqlStat != BE_SQLITE_OK)
        return GetStatusContextR ().SetError (&GetECDbR (), nativeSqlStat);
    else
        return ResetStatus ();
    }
...
ECSqlStepStatus ECSqlPreparedStatement::DoStep ()
    ...
    switch (nativeSqlStatus)
        ...
        default:
            {
            GetStatusContextR ().SetError (&GetECDbR (), nativeSqlStatus);
            return ECSqlStepStatus::Error;
            }
        };
    }
...
// @bsimethod                                                Krischan.Eberle        12/13
//---------------------------------------------------------------------------------------
ECSqlInsertPreparedStatement::ECSqlInsertPreparedStatement (ECDbR ecdb, ECSqlEventManager& eventManager, ECSqlStatusContext& statusContext)
...
ECSqlStepStatus ECSqlInsertPreparedStatement::Step (ECInstanceKey& instanceKey)
    ...
    if (stat == ECSqlStepStatus::Done)
        {
        if (checkModifiedRowCount && ecdb.GetModifiedRowCount () == 0)
            {
...
ECSqlStatus ECSqlInsertPreparedStatement::GenerateECInstanceIdAndBindToInsertStatement (ECInstanceId& generatedECInstanceId)
    ...
    auto& ecdb = GetECDbR ();
    auto dbStat = ecdb.GetECR ().GetECInstanceIdSequence ().GetNextValue<ECInstanceId> (generatedECInstanceId);
...
//---------------------------------------------------------------------------------------
ECSqlUpdatePreparedStatement::ECSqlUpdatePreparedStatement (ECDbR ecdb, ECSqlEventManager& eventManager, ECSqlStatusContext& statusContext)
...
// @bsimethod                                                Affan.Khan             04/14
//---------------------------------------------------------------------------------------
ECSqlDeletePreparedStatement::ECSqlDeletePreparedStatement (ECDbR ecdb, ECSqlEventManager& eventManager, ECSqlStatusContext& statusContext)
