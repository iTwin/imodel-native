...
public:
    ...
    static ECSqlStatus TryParseQualifiedPath(PropertyPath& resolvedPropertyPath, Utf8StringCR qualifedPath, ECDbR ecdb)
        ...
        auto targetClassP = ecdb.GetEC().GetSchemaManager().GetECClassResolver().GetECClass(schemaName, className);
        if (!targetClassP)
            {
        ...
        auto targetClassMap = ecdb.GetEC().GetECDbMap().GetClassMap(*targetClassP);
