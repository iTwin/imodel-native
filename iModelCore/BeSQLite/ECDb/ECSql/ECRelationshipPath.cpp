...
// @bsimethod                                    Ramanujam.Raman                 01 / 2014
//+---------------+---------------+---------------+---------------+---------------+------
ECClassP ECRelatedItemsDisplaySpecificationsCache::GetRelatedSpecificationsClass (ECDbR ecDb)
    {
    ECClassP customAttributeClass;
    ecDb.GetEC ().GetSchemaManager ().GetECClass (customAttributeClass, "Bentley_Standard_CustomAttributes", "RelatedItemsDisplaySpecifications");
...
BentleyStatus ECRelatedItemsDisplaySpecificationsCache::Initialize ()
    {
    ECClassP relatedSpecificationsClass = GetRelatedSpecificationsClass (m_ecDb);
    if (relatedSpecificationsClass == nullptr)
        return ERROR;

    ECSchemaList allSchemas;
    BentleyStatus smStatus = m_ecDb.GetEC ().GetSchemaManager ().GetECSchemas (allSchemas, false);
