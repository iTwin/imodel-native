...
ECSqlStatus ECSqlParseContext::TryResolveClass (shared_ptr<ClassNameExp::Info>& classNameExpInfo, Utf8StringCR schemaNameOrPrefix, Utf8StringCR className) 
    {
    auto const& store = m_ecdb.GetEC();
    ECClassP resolvedClass = store.GetSchemaManager().GetECClassResolver().GetECClass(schemaNameOrPrefix, className);
    ...
    auto map = store.GetECDbMap().GetClassMap(*resolvedClass);
