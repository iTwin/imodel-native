...
PrimitiveType primitiveType
    ...
    {
    auto const& primArraySystemClass = ctx.GetECSqlStatementR().GetPreparedStatementP ()->GetECDbR ().GetEC ().GetECDbMap ().GetClassForPrimitiveArrayPersistence(primitiveType);
    field = unique_ptr<ECSqlField> (
...
PropertyMapCR propertyMap
    ...
    auto structTypeMap = ecdb.GetEC ().GetECDbMap ().GetClassMap (*structType);
