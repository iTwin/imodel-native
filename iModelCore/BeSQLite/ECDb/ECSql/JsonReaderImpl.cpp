...
: m_ecDb (ecdb), m_statementCache (ecdb)
    {
    auto stat = m_ecDb.GetEC ().GetSchemaManager ().GetECClass (m_ecClass, ecClassId);
    BeAssert (stat == SUCCESS && m_ecClass != nullptr && "Could not retrieve class with specified id");
    m_isValid = stat == SUCCESS;
    }
...
const JsonECSqlSelectAdapter::FormatOptions& formatOptions
    ...
    ECClassP dgnElementClass;
    m_ecDb.GetEC ().GetSchemaManager ().GetECClass (dgnElementClass, "dgn", "Element");
    ...
    for (ECInstanceKeyMultiMapConstIterator classIdIter = instanceKeys.begin (); classIdIter != instanceKeys.end (); classIdIter = instanceIdIter)
        ...
        ECClassP ecRelatedClass = nullptr;
        m_ecDb.GetEC ().GetSchemaManager ().GetECClass (ecRelatedClass, ecRelatedClassId);
...
ECClassCP ECClassHelper::ResolveClass (Utf8CP schemaName, Utf8StringCR className, ECDbR ecDb, ECSchemaCP defaultSchema)
    ...
    if (schemaName != nullptr)
        ecDb.GetEC ().GetSchemaManager ().GetECClass (ecClass, schemaName, className.c_str ());
    else
        {
        ...
        ecDb.GetEC ().GetSchemaManager ().GetECClass (ecClass, defaultSchemaName.c_str (), className.c_str ());
        }
