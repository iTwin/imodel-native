...
ECN::ECClassCR ECDbMap::GetClassForPrimitiveArrayPersistence (PrimitiveType primitiveType) const
    {
    ECSchemaCP ecdbSystemSchema = ECDbSystemSchemaHelper::GetSchema (m_ecdb.GetEC ().GetSchemaManager ());
...
void ECDbMap::LoadAllMetadata () const
    ...
    ECClassP ecClass;
    auto& schemaManager = GetECDbR ().GetEC ().GetSchemaManager ();
    while (stmt.Step () == BE_SQLITE_ROW)
        {
        schemaManager.GetECClass (ecClass, (ECClassId)stmt.GetValueInt64 (0));
        if (ecClass != nullptr)
            GetClassMap (*ecClass);
        }
    }
...
CreateTableStatus ECDbMap::CreateOrUpdateRequiredTables ()
    ...
    for (; it != m_clustersByTable.end(); ++it)
        ...
        for (auto ecClassId : classesMappedToDbTable)
            {
            ECClassP ecClassP = nullptr;
            schemaManager.GetECClass (ecClassP, ecClassId);
            BeAssert (ecClassP != nullptr);
            if (ecClassP != nullptr)
                {
                auto classMapP = GetClassMap (*ecClassP);
