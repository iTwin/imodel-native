...
// @bsimethod                                 Krischan.Eberle                02/2014
//+---------------+---------------+---------------+---------------+---------------+-
MapStatus PropertyMapSystem::_FindOrCreateColumnsInTable (ClassMapR classMap)
    {
...
PropertyMapPtr PropertyMapECInstanceId::Create (ECDbSchemaManagerCR schemaManager, IClassMap const& classMap)
    ...
    classMap.GetTable ().GetPrimaryKeyTableConstraint ().GetColumns (columns);
    if (columns.empty ())
        {
    ...
    DbColumnPtr systemColumn = columns[0];
...
m_defaultConstraintClassId (defaultConstraintECClassId)
    ...
    if (table != nullptr)
        {
        table->SetAfterSetClassIdHook ([this] (DbColumnPtr& newClassIdCol)
            {
