...
BentleyStatus OpenCloseECDbConnection ()
    ...
    // ECDb API is exposed through this:
    ecdb.GetEC ();
...
BentleyStatus ECDbSchemaManagerGetSchema ()
    ...
    ECSchemaP fooSchema = nullptr;
    BentleyStatus stat = schemaManager.GetECSchema (fooSchema, "fooschema", false);
    if (SUCCESS != stat)
        {
...
BentleyStatus ECDbSchemaManagerGetClass ()
    ...
    // Retrieve an ECClass
    ECClassP addressClass = nullptr;
    BentleyStatus stat = schemaManager.GetECClass (addressClass, "fooschema", "Address");
    if (SUCCESS != stat)
        {
