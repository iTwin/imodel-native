...
RelationshipClassMap::RelationshipClassMap (ECRelationshipClassCR ecRelClass, ECDbMapCR ecDbMap, MapStrategy mapStrategy, bool setIsDirty)
    ...
    m_sourceConstraintMap (ecDbMap.GetECDbR ().GetEC ().GetSchemaManager (), ecRelClass.GetSource ()),
    m_targetConstraintMap (ecDbMap.GetECDbR ().GetEC ().GetSchemaManager (), ecRelClass.GetTarget ())
    {}
...
DbColumnPtr RelationshipClassMap::CreateConstraintColumn (Utf8CP columnName, bool addToTable)
    ...
    if (addToTable)
        GetTable ().GetColumnsR ().Add (column);
    else
        column->MakeVirtual ();
...
Utf8CP tableAlias
    ...
    {
    auto stat = ClassMap::NativeSqlConverterImpl::_GetWhereClause (whereClauseBuilder, ecsqlType, isPolymorphicClassExp, tableAlias);
...
MapStatus RelationshipClassEndTableMap::_InitializePart1 (ClassMapInfoCR classMapInfo, IClassMap const* parentClassMap)
    ...
    //Add prop maps for the source and target constraints
    return CreateConstraintPropMaps (thisEnd, defaultThisEndECClassId, otherEndECInstanceIdColumn, otherEndECClassIdColumn, defaultOtherEndECClassId);
    }
...
ECClassId defaultOtherEndClassId
    ...
    DbColumnPtrList pkColumns;
    persistenceEndTable.GetPrimaryKeyTableConstraint().GetColumns (pkColumns);
