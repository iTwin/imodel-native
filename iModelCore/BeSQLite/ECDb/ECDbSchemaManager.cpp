...
BentleyStatus ECDbSchemaManager::GetECSchemas (ECSchemaListR schemas, bool ensureAllClassesLoaded) const
    ...
    for (DbECSchemaKey schemaKey : schemaKeys)
        {
        ECSchemaP out;
        stat = GetECSchemaByKey (out, schemaKey, ensureAllClassesLoaded);
        if (stat != SUCCESS)
            return stat;
...
BentleyStatus ECDbSchemaManager::UpdateECSchema (SchemaImportContext const& context, ECDiffPtr& diff, ECSchemaCR ecSchema, CustomAttributeTrackerP tracker) const
    {
    ECSchemaP existingSchema = nullptr;
    Utf8String schemaName (ecSchema.GetName ().c_str ());
    auto stat = GetECSchema (existingSchema, schemaName.c_str (), true);
    if (SUCCESS != stat)
        {
        context.GetIssueListener ().Report (IImportIssueListener::Severity::Error,
...
* @bsimethod                                                    Affan.Khan        07/2012
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus ECDbSchemaManager::GetECSchema (ECSchemaP& ecSchema, Utf8CP schemaName, bool ensureAllClassesLoaded) const
    {
    return (m_ecReader->GetECSchema(ecSchema, schemaName, ensureAllClassesLoaded) == BE_SQLITE_ROW) ?
                        SUCCESS : ERROR;
    }
...
* @bsimethod                                                    Affan.Khan        07/2012
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus ECDbSchemaManager::GetECSchema (ECN::ECSchemaP& ecSchema, ECSchemaId schemaId, bool ensureAllClassesLoaded) const
    {
    return (m_ecReader->GetECSchema (ecSchema, schemaId, ensureAllClassesLoaded) == BE_SQLITE_ROW) ?
                    SUCCESS : ERROR;
    }
...
* @bsimethod                                                    Affan.Khan        07/2012
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus ECDbSchemaManager::GetECSchemaByKey (ECN::ECSchemaP& ecSchema, DbECSchemaKeyCR schemaKey, bool ensureAllClassesLoaded) const
    {
    return GetECSchema(ecSchema, schemaKey.GetECSchemaId(), ensureAllClassesLoaded);
    }
...
* @bsimethod                                                    Affan.Khan        06/2012
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus ECDbSchemaManager::GetECClass (ECClassP& ecClass, DbECClassKeyCR key) const
    {
    return GetECClass(ecClass, key.GetECClassId());
    }

/*---------------------------------------------------------------------------------------
* @bsimethod                                                    Affan.Khan        06/2012
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus ECDbSchemaManager::GetECClass (ECClassP& ecClass, ECClassId ecClassId) const
    {
    return (m_ecReader->GetECClass(ecClass, ecClassId) == BE_SQLITE_ROW) ? SUCCESS : ERROR;
    }

/*---------------------------------------------------------------------------------------
* @bsimethod                                                    Affan.Khan        06/2012
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus ECDbSchemaManager::GetECClass(ECClassP& ecClass, Utf8CP schemaNameOrPrefix, Utf8CP className, ResolveSchema resolveSchema) const // WIP_FNV: probably stays the same... though I expected this to look in memory, first
    {
    switch (resolveSchema)
        {
        case ResolveSchema::AutoDetect:
            ecClass = GetECClassResolver().GetECClass(schemaNameOrPrefix, className);
            return ecClass == NULL ? ERROR : SUCCESS;
        case ResolveSchema::BySchemaName:
            return (m_ecReader->GetECClassBySchemaName(ecClass, schemaNameOrPrefix, className) == BE_SQLITE_ROW) ? SUCCESS : ERROR;
        case ResolveSchema::BySchemaNamespacePrefix:
            return (m_ecReader->GetECClassBySchemaNameSpacePrefix(ecClass, schemaNameOrPrefix, className) == BE_SQLITE_ROW) ? SUCCESS : ERROR;
        default:
            return ERROR;
            break;
        }
...
+---------------+---------------+---------------+---------------+---------------+------*/
IECClassResolver& ECDbSchemaManager::GetECClassResolver() const
    {
    return *m_ecReader;
    }
...
ECSchemaPtr ECDbSchemaManager::_LocateSchema (SchemaKeyR key, SchemaMatchType matchType, ECSchemaReadContextR schemaContext)
    ...
    auto status = GetECSchema(ecSchema, schemaName.c_str(), true /* load all ECClasses */);
    if (SUCCESS != status || !ecSchema)
        return nullptr;

    if (ecSchema->GetSchemaKey().Matches(key, matchType))
        {
        schemaContext.GetCache().AddSchema(*ecSchema);
        return ecSchema;
        }
...
ECClassCP ECDbSchemaManager::_LocateClass (WCharCP schemaName, WCharCP className)
    ...
    GetECClass (ecClass, schemaUtf8.c_str(), classUtf8.c_str());
    return ecClass;
    }
...
BentleyStatus ECDbSchemaManager::EnsureDerivedClassesExist(ECN::ECClassCR ecClass) const
    ...
    for (ECClassId derivedClassId : derivedClassIds)
        {
        auto loadStatus = GetECClass(derivedClass, derivedClassId);
