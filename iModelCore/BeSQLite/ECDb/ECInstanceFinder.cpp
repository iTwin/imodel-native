...
ECSqlStatus ECInstanceFinder::QueryableRelationship::PrepareECSqlStatement (ECDbR ecDb)
    ...
    if (m_thisRelationshipEnd == ECRelationshipEnd_Source)
        ...
        relECSql.Sprintf ("SELECT ECInstanceId, TargetECClassId, TargetECInstanceId "
            "FROM %s.%s WHERE SourceECClassId = %lld AND SourceECInstanceId = ?", 
            relSchemaName.c_str(), relClassName.c_str(), m_thisClass->GetId());
        }
    else
        {
        relECSql.Sprintf ("SELECT ECInstanceId, SourceECClassId, SourceECInstanceId "
            "FROM %s.%s WHERE TargetECClassId = %lld AND TargetECInstanceId = ?",
            relSchemaName.c_str(), relClassName.c_str(), m_thisClass->GetId());
        }
...
void ECInstanceFinder::FindEndClasses (bset<ECClassId>& endClassIds, ECClassId relationshipClassId, ECRelationshipEnd relationshipEnd, ECDbR ecDb)
    ...
    while ((r = stmt->Step()) == BE_SQLITE_ROW)
        {
        ECClassId endClassId = stmt->GetValueInt64 (0);
...
DbResult ECInstanceFinder::FindRelationshipsOnEnd (QueryableRelationshipVector& queryableRelationships, ECClassId thisEndClassId, ECDbR ecDb)
    ...
    Utf8CP sql = 
        ...
        "    )"
        " SELECT DISTINCT ECRelationshipClass.ECClassId AS RelationshipId, ThisEndConstraintClass.ECRelationshipEnd As ThisEndIsTarget"
        " FROM ec_RelationshipConstraintClass ThisEndConstraintClass"
        " JOIN ec_Class ECRelationshipClass ON ThisEndConstraintClass.ECClassId == ECRelationshipClass.ECClassId"
        " JOIN ec_RelationshipConstraint ThisEndConstraint  ON (ThisEndConstraint.ECClassId == ECRelationshipClass.ECClassId AND ThisEndConstraint.ECRelationshipEnd == ThisEndConstraintClass.ECRelationshipEnd)"
    ...
    ECDbSchemaManagerCR ecDbSchemaManager = ecDb.GetEC ().GetSchemaManager ();

    ECClassP anyClass = nullptr;
    ecDbSchemaManager.GetECClass (anyClass, "Bentley_Standard_Classes", "AnyClass");
    if (anyClass != nullptr)
        {
    ...
    while (BE_SQLITE_ROW == (result = stmt->Step()))
        {
        ECClassId ecRelationshipClassId = stmt->GetValueInt64(0);
        ECClassP ecClass = nullptr;
        ecDbSchemaManager.GetECClass (ecClass, ecRelationshipClassId);
...
void ECInstanceFinder::DumpInstanceKeyMap (const ECInstanceKeyMultiMap& instanceKeyMultiMap, ECDbR ecDb)
    ...
    for (ECInstanceKeyMultiMapConstIterator classIdIter = instanceKeyMultiMap.begin(); classIdIter != instanceKeyMultiMap.end(); classIdIter = instanceIdIter)
        ...
        ECClassP ecClass = nullptr;
        ecDb.GetEC ().GetSchemaManager ().GetECClass (ecClass, classId);
...
int findRelatedDirections
    ...
    for (ECInstanceKeyMultiMapConstIterator classIdIter = seedInstanceKeyMap.begin(); classIdIter != seedInstanceKeyMap.end(); )
        ...
        for (QueryableRelationship& queryableRelationship : *queryableRelationshipsOnThisEnd)
            ...
            auto classMapP = m_ecDb.GetEC ().GetECDbMap ().GetClassMap (*classP);
            ...
            while (instanceIdIter != instanceIdRange.second)
                ...
                while ((stepStatus = statement->Step()) == ECSqlStepStatus::HasRow)
                    ...
                    if (relationshipInstanceKeyMap != nullptr)
                        ...
                        POSTCONDITION (relationshipClassId > 0 && relationshipInstanceId.IsValid (), ERROR);
...
uint8_t& currentDepth
    ...
    if (LOG.isSeverityEnabled (NativeLogging::LOG_TRACE))
        {
        LOG.tracev ("After a collection (depth %d) map, new instances added:", currentDepth);
    ...
    for (ECInstanceKeyMultiMapIterator iter = newSeedInstanceKeyMap.begin(); iter != newSeedInstanceKeyMap.end(); /* no increment due to modifications */)
        ...
        if (instanceKeyMap.end() != std::find (instanceKeyMap.begin(), instanceKeyMap.end(), instanceEntry))
            {
            ECClassP ecClass = nullptr;
            m_ecDb.GetEC ().GetSchemaManager ().GetECClass (ecClass, iter->first);
            LOG.warningv (L"Detected a relationship cycle with instance id %lld of class %ls", iter->second, 
