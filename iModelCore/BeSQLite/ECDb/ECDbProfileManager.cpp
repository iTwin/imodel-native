...
ECDbR ecdb
    ...
    ecdb.GetECR ().GetIdSequences (idSequences);
    auto stat = ResetIdSequences (ecdb, idSequences, ecdb.GetRepositoryId ());
...
//+---------------+---------------+---------------+---------------+---------------+-
//static
DbResult ECDbProfileManager::ResetIdSequences (ECDbR ecdb, bvector<BeRepositoryBasedIdSequenceCP> const& idSequences, BeRepositoryId repositoryId)
    {
    if (ecdb.IsReadonly ())
        {
        BeAssert (ecdb.IsReadonly ());
    ...
    for (BeRepositoryBasedIdSequenceCP idSequence : idSequences)
        {
        BeAssert (idSequence != nullptr);

        DbResult stat = idSequence->Reset (repositoryId);
    ...
    const NativeLogging::SEVERITY sev = NativeLogging::LOG_DEBUG;
    if (LOG.isSeverityEnabled (sev))
        {
        LOG.messagev (sev, "Id sequences were reset in ECDb file '%s' (Repository id %d) in %.4f seconds.", 
            ecdb.GetDbFileName (),
            repositoryId.GetValue (),
