        ...
        if (ecDbIndex->GetValue (v, BSCAP_Properties) == ECOBJECTS_STATUS_Success && !v.IsNull ())
            ...
            for (uint32_t j = 0; j < propertyCount && !errorEncountered; j++)
                ...
                if (ecDbIndex->GetValue (v, propertiesPropIdx, j) == ECOBJECTS_STATUS_Success)
                    ...
                    ECPropertyP prop = ecClass.GetPropertyP (v.GetString ());
                    if (!prop)
                        {
                        LOG.errorv (L"Rejecting index[%d] specified in ECDbClassHint on class %ls because property specified in index '%ls' doesn't exist in class", i, ecClass.GetFullName (), v.GetString ());
                        errorEncountered = true; // skip this index and continue with rest
                        break;
                        }

                    if (!prop->GetAsPrimitiveProperty ())
                        {
                        LOG.errorv (L"Rejecting index[%d] specified in ECDbClassHint on class %ls because specified property is not primitive.", i, ecClass.GetFullName ());
                        errorEncountered = true; // skip this index and continue with rest
                        break;
                        }
                    ...
                    switch (prop->GetAsPrimitiveProperty ()->GetType ())
                        {
                            ...
                            case PRIMITIVETYPE_IGeometry:
                                LOG.errorv (L"Rejecting user specified index[%d] specified in ECDbClassHint on class %ls because specified property type not supported. Supported types are String, Boolean, Integer, DateTime, Double, Binary, Point2d and Point3d", i, ecClass.GetFullName ());
                                errorEncountered = true; // skip this index and continue with rest
                                break;
                            default:
                                LOG.errorv (L"Rejecting user specified index[%d] specified in ECDbClassHint on class %ls because specified property type not supported. Supported types are String, Boolean, Integer, DateTime, Double and Binary", i, ecClass.GetFullName ());
                                errorEncountered = true; // skip this index and continue with rest
                                break;
                        }
