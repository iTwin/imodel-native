...
DbResult ECDbProfileUpgrader_1003::MapTransformationValueMapClass (ECDbR ecdb)
    {
    ECClassP tvmECClass = nullptr;
    if (SUCCESS != ecdb.GetEC ().GetSchemaManager ().GetECClass (tvmECClass, "Bentley_Standard_CustomAttributes", "TransformationValueMap"))
        return BE_SQLITE_OK;
...
DbResult ECDbProfileUpgrader_1002::UnmapUnsupportedStandardClasses (ECDbR ecdb)
    ...
    for (auto const& kvPair : candidateClassNames)
        ...
        for (auto const& className : classNames)
            ...
            const auto schemaStat = schemaManager.GetECClass (ecClass, schemaName, className.c_str ());
...
DbResult ECDbProfileECSchemaUpgrader::ImportProfileSchemas (ECDbR ecdb, bool updateSchema)
    ...
    context->AddSchemaLocater (ecdb.GetEC ().GetSchemaLocater ());
    ...
    auto importStat = ecdb.GetEC ().GetSchemaManager ().ImportECSchemas (context->GetCache (), options);
