...
void ImportCommand::RunImportSchema (ECSqlConsoleSession& session, BeFileNameCR ecschemaPath) const
    ...
    auto status = session.GetECDbR ().GetEC ().GetSchemaManager ().ImportECSchemas (context->GetCache (), options);
...
void ExportCommand::RunExportSchema (ECSqlConsoleSession& session, Utf8CP outFolderStr) const
    {
    auto const& schemaManager = session.GetECDbR ().GetEC().GetSchemaManager();
...
void PopulateCommand::_Run (ECSqlConsoleSession& session, vector<Utf8String> const& args) const
    ...
    if (argCount == 2)
        ...
        auto stat = session.GetECDbR ().GetEC().GetSchemaManager ().GetECSchema (schema, schemaName, true);
        if (stat != SUCCESS)
            {
        ...
        {
        auto stat = session.GetECDbR ().GetEC().GetSchemaManager ().GetECSchemas (schemaList,true);
        if (stat != SUCCESS)
            {
