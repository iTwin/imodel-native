...
**
** Alternative encryption schemes may be supported by replacing this
** entire file at compile time.
*/
...
static void bcvXorBuffers(u8 *out, u8 *a, u8 *b, int nByte){
  if( sizeof(a)==8
   ...
   && (b - (u8*)0)%8==0
    ...
    sqlite3_uint64 *z = (sqlite3_uint64*)b;
    while( nByte>8 ){
      *(x++) = *(y++) ^ *(z++);
      nByte -= 8;
    }
...
** use is specified by variableaNonce, which points to a 16-byte
** (BCV_LOCAL_KEYSIZE byte) buffer.
*/
int bcvEncrypt(
...
/*
** Decrypt buffer aData (size nData bytes) using key pKey. The nonce that
** was used when encrypting is specified by variableaNonce, which points to
** a 16-byte (BCV_LOCAL_KEYSIZE byte) buffer.
*/
int bcvDecrypt(
  BcvEncryptionKey *pKey,         /* Key to encrypt with */
  const u8 *aNonce,               /* 16 byte nonce value */
