...
# undef sqlite3_key
# undef sqlite3_rekey
# define sqlite3CodecAttach  sqlite3CodecAttach_aes128ofb
# define sqlite3CodecGetKey  sqlite3CodecGetKey_aes128ofb
# define sqlite3_key         sqlite3_key_aes128ofb
# define sqlite3_rekey       sqlite3_rekey_aes128ofb
#endif
...
*/
static const char zCopyright_AES128[] = 
...
*/
static void *sqliteCodecAes128(
  void *codecState,
  void *pageData,
  Pgno pgno,
  int op
){
  CodecAes128 *pCodec = codecState;
...
#if 0
  printf("CODEC %s%d pgno=%-4d op=%d\n",
         (op&4)==0 ? "decode" : "encode", op&1,
         pgno, op);
#endif
  ...
  */
  ...
  assert( pCodec->nonceSize + pCodec->nonceZero == AES_BLOCK_SZ - 4 );
  ...
  */  
  if( op&2 ){
    /* AES with a 128-bit key and block in OFB mode */
    unsigned int i;
    unsigned char *mask = pCodec->mask;
    u32 *rk = pCodec->key[op&1].keySchedule;

    rijndaelEncrypt128(rk, nonce, mask);
    for(i=16; i<pCodec->usable; i+=16){
      rijndaelEncrypt128(rk, &mask[i-16], &mask[i]);
    }
  }

#if 0
  printf("CODEC %s%d pgno=%-4d op=%d "
         "nonce(%d)=%02x%02x%02x%02x%02x%02x "
         "in=%02x%02x%02x%02x%02x%02x ",
     (op&4)==0 ? "decode" : "encode", op&1,
     pgno, op, pCodec->nonceSize,
     nonce[0], nonce[1], nonce[2], nonce[3], nonce[4], nonce[5],
     zData[0], zData[1], zData[2], zData[3], zData[4], zData[5]
  );
#endif
...
#if 0
  printf("out=%02x%02x%02x%02x%02x%02x\n",
...
*/
static void loadKey128(CodecAes128 *pCodec, const void *pKey, int nKey){
  if( pKey && nKey!=0 ){
    ...
    rijndaelKeySetupEnc128(pCodec->key[0].keySchedule, pCodec->key[0].repeat);
  }else{
...
*/
static void sqliteCodecAes128SizeChng(void *p, int pageSize, int nReserve){
  CodecAes128 *pCodec = (CodecAes128*)p;
  pCodec->pageSize = pageSize;
  pCodec->nonceSize = nReserve;
  pCodec->usable = pageSize - nReserve;
  pCodec->nonceZero = AES_BLOCK_SZ - sizeof(Pgno) - pCodec->nonceSize;
...
** Deallocate a codec.
*/
static void sqliteCodecAes128Free(void *p){
  CodecAes128 *pCodec = (CodecAes128*)p;
...
int sqlite3CodecAttach(
  ...
  if( pDb->pBt && (pPager = sqlite3BtreePager(pDb->pBt))!=0 ){
    pCodec = sqlite3_malloc( sizeof(CodecAes128) );
    if( pCodec==0 ){
      return SQLITE_NOMEM;
    }
    memset(pCodec, 0, sizeof(CodecAes128));
    /* 12 bytes of nonce for AES */
    sqlite3BtreeSetPageSize(pDb->pBt, 0, AES_BLOCK_SZ-sizeof(Pgno), 0);
    loadKey128(pCodec, pKey, nKey);
    pCodec->key[1] = pCodec->key[0];
    sqlite3PagerSetCodec(pPager,
         sqliteCodecAes128,
         sqliteCodecAes128SizeChng,
         sqliteCodecAes128Free,
         pCodec
    );
  }
  return SQLITE_OK;
...
void sqlite3CodecGetKey(sqlite3 *db, int iDb, void **ppKey, int *pnKey){
  ...
  pCodec = (CodecAes128*)sqlite3PagerGetCodec(pPager);
  if( pCodec==0 ){
    *pnKey = 0;
    *ppKey = 0;
  }else{
