...
struct BeBriefcaseBasedIdSequence final : NonCopyableClass {
   private:
    ...
    BE_SQLITE_EXPORT uint64_t GetCurrentInt64Value() const;
   ...
   public:
    ...
    TBeBriefcaseBasedId GetCurrentValue() const {
        return TBeBriefcaseBasedId(GetCurrentInt64Value());
    }
...
struct BeBriefcaseBasedIdSequenceManager final : NonCopyableClass {
   ...
   public:
    ...
    BeBriefcaseBasedIdSequence const& GetSequence(uint32_t sequenceKey) const {
        BeAssert(sequenceKey < (uint32_t)m_sequences.size());
        return m_sequences[(size_t)sequenceKey];
    }
