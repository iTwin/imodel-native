...
+---------------+---------------+---------------+---------------+---------------+------*/
struct IECClassResolver
    {
    public:
        virtual ECN::ECClassP GetECClass(ECN::ECClassId ecClassId)=0;
        virtual ECN::ECClassP GetECClass(Utf8StringCR schemaNameOrPrefix, Utf8String className)=0;
...
//!     ecSchemaContext->AddSchemaLocater(ecdb.GetEC().GetSchemaLocater());
...
public:
    ...
    //! @param[out] ecSchema The retrieved ECSchema
    ...
    ECDB_EXPORT BentleyStatus GetECSchema (ECN::ECSchemaP& ecSchema, Utf8CP schemaName, bool ensureAllClassesLoaded = true) const;
...
//__PUBLISH_SECTION_END__
    ECDB_EXPORT BentleyStatus GetECSchema (ECN::ECSchemaP& ecSchema, ECN::ECSchemaId schemaId, bool ensureAllClassesLoaded = true) const;
    ECDB_EXPORT BentleyStatus GetECSchemaByKey (ECN::ECSchemaP& ecSchema, DbECSchemaKeyCR schemaKey, bool ensureAllClassesLoaded = true) const;
...
//__PUBLISH_SECTION_START__
    ...
    //! @param[out] ecClass The retrieved ECClass
    ...
    //! @return BentleyStatus::SUCCESS or BentleyStatus::ERROR
    ECDB_EXPORT BentleyStatus GetECClass(ECN::ECClassP& ecClass, Utf8CP schemaNameOrPrefix, Utf8CP className, ResolveSchema resolveSchema = ResolveSchema::BySchemaName ) const;

    //! Gets the ECClass for the specified ECClassId.
    //! @param[out] ecClass The retrieved ECClass
    //! @param[in] ecClassId Id of the ECClass to retrieve
    //! @return BentleyStatus::SUCCESS or BentleyStatus::ERROR
    ECDB_EXPORT BentleyStatus GetECClass (ECN::ECClassP& ecClass, ECN::ECClassId ecClassId) const;
...
//__PUBLISH_SECTION_END__
    ECDB_EXPORT BentleyStatus GetECClass (ECN::ECClassP& ecClass, DbECClassKeyCR key) const;
    ...
    IECClassResolver& GetECClassResolver () const;
