...
public:
    ...
    BE_SQLITE_EXPORT DbResult ApplyChanges(DbR db, Rebase* rebase = nullptr, bool invert = false);
...
//=======================================================================================
struct DbSchemaChangeSet : ChangeSet {
    ...
    DbSchemaChangeSet(Utf8CP ddl = nullptr, int chunkSize = 4 * 1024) : ChangeSet(chunkSize) { Append(ddl); }
...
public:
    ChangeTracker(Utf8CP name = NULL) : m_name(name) {
        m_session = 0;
        m_db = 0;
        m_isTracking = false;
    }
    ...
    bool HasDbSchemaChanges() const { return !m_dbSchemaChanges._IsEmpty(); }
