...
struct RTree2dVal {
    ...
    void ToRange(DRange2dR range) const {
        range.low.x = m_minx;
        range.low.y = m_miny;
        range.high.x = m_maxx;
        range.high.y = m_maxy;
    }
    void ToRange(DRange3dR range) const {
        range.low.x = m_minx;
        range.low.y = m_miny;
        range.high.x = m_maxx;
        range.high.y = m_maxy;
    ...
    void Union(RTree2dVal const& other) {
...
struct RTree3dVal {
    ...
    void ToRangeR(DRange3dR range) const {
        range.low.x = m_minx;
        range.low.y = m_miny;
        range.low.z = m_minz;
        range.high.x = m_maxx;
    ...
    DRange3d ToRange() const {
        DRange3d range;
        ToRangeR(range);
        return range;
    }
    void FromRange(DRange3dCR range) {
        m_minx = range.low.x;
        m_maxx = range.high.x;
        m_miny = range.low.y;
        m_maxy = range.high.y;
    ...
    void Union(RTree3dVal const& other) {
        m_minx = std::min(m_minx, other.m_minx);
        m_miny = std::min(m_miny, other.m_miny);
