...
struct CloudCache {
    ...
    BE_SQLITE_EXPORT CloudResult InitCache(Utf8StringCR name, Utf8StringCR rootDir, int64_t cacheSize = 0, int nRequest = 0, int httpTimeout = 0, bool verboseCurlLog = false, bool compressBlocks = false);
...
public:
    ...
    BE_SQLITE_EXPORT CloudResult Init(CloudContainer const& container, int logLevel = 0, int nRequests = 0, int httpTimeout = 0, bool compressBlocks = false);
