...
DbModule::VirtualTable::Cursor::~Cursor() {
    delete m_ctx;
}
...
DbModule::VirtualTable::~VirtualTable() {
    delete m_ctx;
}
...
static int vtabConnect(sqlite3* db,
    ...
    auto rc = (int)module->Connect(vt, argc, argv);
    if (rc != SQLITE_OK) {
        if (vt) delete vt;
        return (int)rc;
    }
    *ppVtab = &vt->GetCallbackData()->base;
    sqlite3_declare_vtab(db, module->GetDeclaration().c_str());
