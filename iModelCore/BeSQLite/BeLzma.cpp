...
static void* allocFor7z(void* p, size_t size) {
    return malloc(size);
}
static void freeFor7z(void* p, void* address) {
    free(address);
}
...
BentleyStatus LzmaEncoder::LzmaParams::FromJson(BeJsConst v) {
    if (!v.isObject())
        return ERROR;
    if (v.isNumericMember("algo"))
        m_algo = static_cast<Algorithm>(v["algo"].asInt());
    if (v.isNumericMember("blockSize"))
        m_blockSize = v["blockSize"].asUInt64();
    if (v.isNumericMember("btMode"))
        m_btMode = static_cast<BtMode>(v["btMode"].asInt());
    if (v.isNumericMember("dictSize"))
        m_dictSize = v["dictSize"].asUInt();
    if (v.isNumericMember("fb"))
        m_fb = v["fb"].asInt();
    if (v.isNumericMember("lc"))
        m_lc = v["lc"].asInt();
    if (v.isNumericMember("level"))
        m_level = v["level"].asInt();
    if (v.isNumericMember("lp"))
        m_lp = v["lp"].asInt();
    if (v.isNumericMember("mc"))
        m_mc = v["mc"].asInt();
    if (v.isNumericMember("numBlockThreads"))
        m_numBlockThreads = v["numBlockThreads"].asInt();
    if (v.isNumericMember("numHashBytes"))
        m_numHashBytes = v["numHashBytes"].asInt();
    if (v.isNumericMember("numThreads"))
        m_numThreads = v["numThreads"].asInt();
    if (v.isNumericMember("numTotalThreads"))
        m_numTotalThreads = v["numTotalThreads"].asInt();
    if (v.isNumericMember("pb"))
        m_pb = v["pb"].asInt();
    if (v.isNumericMember("writeEndMark"))
        m_writeEndMark = v["writeEndMark"].asInt();
    Normalize();
    return SUCCESS;
}
...
struct LzmaEncoder::Impl {
   ...
   public:
    ...
    // @bsimethod
    //---------------------------------------------------------------------------------------
    Impl(LzmaEncoder::LzmaParams const& param) : m_inStream(nullptr), m_outStream(nullptr), m_supportRandomAccess(param.GetSupportRandomAccess()), m_progressImpl(nullptr), m_encHandle(0) {  // uint32_t dictionarySize, int level, int threads, int algo
