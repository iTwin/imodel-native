...
BentleyStatus SetupTestECDb(BeFileNameR ecdbPath, Utf8CP testECDbName)
{
    ...
    if (ecdb.GetEC().GetSchemaManager().ImportECSchemas(*schemaCache) != SUCCESS)
        return ERROR;
...
// @bsiclass                                                Krischan.Eberle      07/2014
//+===============+===============+===============+===============+===============+======
struct ECPersistenceTestInserter : TestInserter<ECPersistenceTestInserter>
{
private:
    std::map<ECN::ECClassCP, ECPersistencePtr> m_cache;
...
public:
    ECPersistenceTestInserter()
        : TestInserter("ECPersistence")
    {
    }
    ...
    bool DoPrepare(ECDbR ecdb, std::vector<ECN::ECClassCP> const& testClasses)
    {
        ...
        {
            //calls to GeECPersistence cache the per-class persistence and do all preparation
            auto persistence = ecdb.GetEC().GetECPersistence(nullptr, *testClass);
            if (persistence == nullptr)
                return false;

            m_cache[testClass] = persistence;
        }
    ...
    bool DoInsertInstance(IECInstanceR testInstance) const
    {
        ...
        auto& inserter = *it->second;
        ECInstanceId id;
        return inserter.Insert(&id, testInstance) == PERSISTENCE_Success;
    };
    ...
    TestInserter<TTestInserter>& testInserter,StopWatch& logTimer)
{
    ...
    if (!Utf8String::IsNullOrEmpty(testClassName))
    {
        ...
        ASSERT_EQ(SUCCESS, ecdb.GetEC().GetSchemaManager().GetECClass(testClass, testSchemaName, testClassName));
        testClasses.push_back(testClass);
    }
    else
    {
        ECSchemaP schema = nullptr;
        ASSERT_EQ(SUCCESS, ecdb.GetEC().GetSchemaManager().GetECSchema(schema, testSchemaName));
...
void RunPerformanceComparisonTest(Utf8CP testClassName, int numberOfInstancesPerClass,Utf8String TestDetails)
{
    ...
    //Create test files
    BeFileName ecpTestFile;
    BeFileName eciiTestFile;
    BeFileName ecsqlinsertTestFile;
    ASSERT_EQ(SUCCESS, SetupTestECDb(ecpTestFile, "ecpersistenceperformance.ecdb"));
   ...
   RunPerformanceComparison(ecsqlHasRun, ecsqlinsertInsertTiming, testSchemaName, testClassName, ecsqlinsertTestFile, numberOfInstancesPerClass, ecsqlTestInserter, ecsqlTestInserterLogTimer);
    ...
    //********** Run performance test with ECPersistence getting time zero have to take a look
    bool hasRun = false;
    double ecpInsertTiming = -1.0;
    ECPersistenceTestInserter ecpTestInserter;
    StopWatch ecpTestInserterLogTimer("ecpTestInserterLogTimer", false);
   // PerformanceTestingFrameWork performanceEcpTestInserter;
    Utf8String ecpTestInserterSchemaDetails(testSchemaName);
    Utf8String ecpTestInserterClassDetails(testClassName);
    Utf8PrintfString ecpTestInserterNumberOfClasses("%d", numberOfInstancesPerClass);
   RunPerformanceComparison(hasRun, ecpInsertTiming, testSchemaName, testClassName, ecpTestFile, numberOfInstancesPerClass, ecpTestInserter, ecpTestInserterLogTimer);
    ...
    { 
        ...
        LOG.infov("ECInstanceInserter Performance (percentage of ECPersistence): Insertion: %.1f%%",
            eciiInsertTiming / ecpInsertTiming * 100.0);
        EXPECT_TRUE(performancePercentageEcii.writeTodbForDouble(L"PerformanceTest.ecdb", eciiInsertTiming / ecpInsertTiming * 100.0, TestDetails+" Description ECInstanceInserter/ECPersistence",
            " ECInstanceInserter Performance (percentage of ECPersistence): Insertion: for Schema Name : " + ecpTestInserterSchemaDetails + " Class Name" + ecpTestInserterClassDetails + " Number of instances Per Class : " + ecpTestInserterNumberOfClasses, false));
    }
    if (ecsqlHasRun)
    {
        PerformanceTestingFrameWork performancePercentageEcSql;
        LOG.infov("ECSQL INSERT Performance (percentage of ECPersistence): Insertion: %.1f%%",
            ecsqlinsertInsertTiming / ecpInsertTiming * 100.0);
        EXPECT_TRUE(performancePercentageEcSql.writeTodbForDouble(L"PerformanceTest.ecdb", ecsqlinsertInsertTiming / ecpInsertTiming * 100.0, TestDetails +"Description ECSQL INSERT/ECPersistence",
            " ECSQL INSERT Performance (percentage of ECPersistence): Insertion: for Schema Name : " + ecpTestInserterSchemaDetails + " Class Name " + ecpTestInserterClassDetails + " Number of instances Per Class : " + ecpTestInserterNumberOfClasses, false));
