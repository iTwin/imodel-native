...
,StopWatch &totalStopWatch, Utf8String testDetails) const
    ...
    ECClassP testClass = GetTestClass (db);
...
+---------------+---------------+---------------+---------------+---------------+------*/
//static
ECClassP PerformanceECSqlStatementTestFixture::GetTestClass 
(
ECDbR testDb
)
    {
    ECSchemaP schema = nullptr;
    testDb.GetEC().GetSchemaManager().GetECSchema(schema, TestSchemaHelper::TESTSCHEMA_NAME);

    ECClassP testClass = schema->GetClassP (TestSchemaHelper::TESTCLASS_NAME);
