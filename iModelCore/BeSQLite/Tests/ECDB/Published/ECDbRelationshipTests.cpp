...
TEST(ECDbRelationships, testRelationshipKeys)
    ...
    ECSchemaP ecSchema=NULL;
    ASSERT_EQ(SUCCESS, ecdbr.GetEC().GetSchemaManager().GetECSchema(ecSchema, "UserWorkBench", true));
    ASSERT_TRUE(ecSchema!=NULL);

    ECRelationshipClassP areaTown = ecSchema->GetClassP(L"area_town")->GetRelationshipClassP();
    ...
    ECRelationshipClassP countryContinent = ecSchema->GetClassP(L"country_continent")->GetRelationshipClassP();
    ...
    ECRelationshipClassP houseUser = ecSchema->GetClassP(L"house_user")->GetRelationshipClassP();
