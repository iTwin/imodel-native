...
bool CreateSimpleCompany (ECDbR ecDb)
    ...
    auto importSchemaStatus = ecDb.GetEC().GetSchemaManager().ImportECSchemas (context->GetCache());
...
bool PopulateSimpleCompany (ECDbR ecDb)
    ...
    auto schemaStatus = ecDb.GetEC().GetSchemaManager().GetECSchema (schema, "SimpleCompany", true);
    if (schemaStatus != SUCCESS)
        return false;

    /* Import sample instances into Db */
    ECClassCP employeeClass = schema->GetClassP (L"Employee");
    ECClassCP managerClass = schema->GetClassP (L"Manager");
    ECClassCP subContractorClass = schema->GetClassP (L"SubContractor");
    ECRelationshipClassCP managerRelClass = schema->GetClassP (L"ManagerRelationship")->GetRelationshipClassCP();
    ECRelationshipClassCP subContractorRelClass = schema->GetClassP (L"SubContractorRelationship")->GetRelationshipClassCP();
...
TEST (FieldEngineer, RelationshipIssue)
    ...
    /* Import sample instances into Db */
    ECClassP cachedFileInfoClass;
    ecDb.GetEC().GetSchemaManager().GetECClass (cachedFileInfoClass, "DSCacheSchema", "CachedFileInfo");
    ASSERT_TRUE (cachedFileInfoClass != nullptr);

    ECClassP folderClass;
    ecDb.GetEC().GetSchemaManager().GetECClass (folderClass, "eB_PW_CommonSchema_WSB", "Folder");
    ASSERT_TRUE (folderClass != nullptr);

    ECClassP tmpClass;
    ecDb.GetEC().GetSchemaManager().GetECClass (tmpClass, "DSCacheJoinSchema", "CachedFileInfoRelationship");
