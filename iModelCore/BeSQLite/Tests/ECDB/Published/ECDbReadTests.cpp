...
TEST(ECDbInstances, WriteCalculatedECProperty)
{
    ...
    schemaReadContext->AddSchemaLocater(db.GetEC().GetSchemaLocater());
    ECSchemaPtr schema;
...
TEST(ECDbTest, createECDbWithArbitraryNumberOfECInstances)
{
    ...
    ECSchemaP ecschemap;
    ASSERT_EQ(SUCCESS, ecdbr.GetEC().GetSchemaManager().GetECSchema(ecschemap, "SimpleCompany", true));
    ECClassP employee = ecschemap->GetClassP(L"Employee");
