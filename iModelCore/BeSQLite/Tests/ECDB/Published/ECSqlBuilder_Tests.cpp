...
TEST (ECSqlBuilderTests, ECSqlSelectBuilder_IncompleteBuilder)
    ...
    sqlStr = ecsql.ToString ();
    EXPECT_STREQ ("SELECT  FROM ONLY [stco].[AAA]", sqlStr.c_str ()) << "Unexpected result for ECSqlSelectBuilder::ToString ()";
    }
...
TEST (ECSqlBuilderTests, ECSqlSelectBuilder_ToString)
    ...
    EXPECT_STREQ (" FROM ONLY [ecsql].[PSA]", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("").From (*psaClass, false);
    EXPECT_STREQ ("SELECT  FROM ONLY [ecsql].[PSA]", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("SELECT I, D FROM ONLY [ecsql].[PSA]", builder.ToString ().c_str ());

    //ECSQL entities are case sensitive
    builder = ECSqlSelectBuilder ();
    builder.Select ("I, D").From (*psaClass, false);
    EXPECT_STRNE ("SELECT i, D FROM ONLY [ecsql].[PSa]", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("SELECT I, D FROM [ecsql].[PSA]", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, D").From (*psaClass, "a", true);
    EXPECT_STREQ ("SELECT I, D FROM [ecsql].[PSA] a", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, D").From (*psaClass, "a", false);
    EXPECT_STREQ ("SELECT I, D FROM ONLY [ecsql].[PSA] a", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("a.I, a.D").From (*psaClass, "a", false);
    EXPECT_STREQ ("SELECT a.I, a.D FROM ONLY [ecsql].[PSA] a", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("SELECT * FROM ONLY [ecsql].[P] a", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.SelectAll ().From (*nonDomainStructWithPrims, false);
    EXPECT_STREQ ("SELECT * FROM ONLY [ecsql].[PStruct]", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("B, ECInstanceId, S").From (*psaClass, false);
    EXPECT_STREQ ("SELECT B, ECInstanceId, S FROM ONLY [ecsql].[PSA]", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("Dt_Array, P3D, PStruct_Array, P2D, PStructProp").From (*psaClass, false);
    EXPECT_STREQ ("SELECT Dt_Array, P3D, PStruct_Array, P2D, PStructProp FROM ONLY [ecsql].[PSA]", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("PStructProp.i, PStructProp.dtUtc").From (*psaClass, false);
    EXPECT_STREQ ("SELECT PStructProp.i, PStructProp.dtUtc FROM ONLY [ecsql].[PSA]", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("3.14").From (*psaClass, false);
    EXPECT_STREQ ("SELECT 3.14 FROM ONLY [ecsql].[PSA]", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("3.14 AS D, 3.14 AS BlaBla").From (*psaClass, true);
    EXPECT_STREQ ("SELECT 3.14 AS D, 3.14 AS BlaBla FROM [ecsql].[PSA]", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, Dt, S").From (*psaClass, "a", false).Where ("a.D < 3.14");
    EXPECT_STREQ ("SELECT I, Dt, S FROM ONLY [ecsql].[PSA] a WHERE a.D < 3.14", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, Dt, S").From (*psaClass, false).Where ("a.D < 3.14");
    EXPECT_STREQ ("SELECT I, Dt, S FROM ONLY [ecsql].[PSA] WHERE a.D < 3.14", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, Dt, S").From (*psaClass, false).Where ("ECInstanceId > 123");
    EXPECT_STREQ ("SELECT I, Dt, S FROM ONLY [ecsql].[PSA] WHERE ECInstanceId > 123", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, Dt, S").From (*psaClass, false).Where ("Dt = ?");
    EXPECT_STREQ ("SELECT I, Dt, S FROM ONLY [ecsql].[PSA] WHERE Dt = ?", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, Dt, S").From (*psaClass, false).Where ("Dt = :dt AND L > :lmin AND L <= :lmax");
    EXPECT_STREQ ("SELECT I, Dt, S FROM ONLY [ecsql].[PSA] WHERE Dt = :dt AND L > :lmin AND L <= :lmax", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, Dt, S").From (*psaClass, false).Where ("B = True OR B = False");
    EXPECT_STREQ ("SELECT I, Dt, S FROM ONLY [ecsql].[PSA] WHERE B = True OR B = False", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, S").From (*psaClass, false).Where ("Dt = DATE '2013-08-05'");
    EXPECT_STREQ ("SELECT I, S FROM ONLY [ecsql].[PSA] WHERE Dt = DATE '2013-08-05'", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, S").From (*psaClass, false).Where ("P3D = POINT3D (1.1, 2.2, 3.3)");
    EXPECT_STREQ ("SELECT I, S FROM ONLY [ecsql].[PSA] WHERE P3D = POINT3D (1.1, 2.2, 3.3)", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, S").From (*psaClass, false).OrderBy ("a.ECInstanceId");
    EXPECT_STREQ ("SELECT I, S FROM ONLY [ecsql].[PSA] ORDER BY a.ECInstanceId", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, S").From (*psaClass, false).OrderBy ("a.ECInstanceId ASC, a.I DESC");
    EXPECT_STREQ ("SELECT I, S FROM ONLY [ecsql].[PSA] ORDER BY a.ECInstanceId ASC, a.I DESC", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, S").From (*psaClass, false).Limit ("10");
    EXPECT_STREQ ("SELECT I, S FROM ONLY [ecsql].[PSA] LIMIT 10", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("I, S").From (*psaClass, false).Limit ("10", "5");
    EXPECT_STREQ ("SELECT I, S FROM ONLY [ecsql].[PSA] LIMIT 10 OFFSET 5", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("p.I, p.S").From (*psaClass, "p", false).Join (*pClass, "c", false).Using (*psaHasPRelClass);
    EXPECT_STREQ ("SELECT p.I, p.S FROM ONLY [ecsql].[PSA] p JOIN ONLY [ecsql].[P] c USING [ecsql].[PSAHasP]", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("SELECT p.I, p.S FROM ONLY [ecsql].[PSA] p JOIN ONLY [ecsql].[PSA] c USING [ecsql].[PSAHasPSA]", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("p.I, p.S").From (*psaClass, "p", false).Join (*psaClass, "c", false).Using (*psaHasPsaRelClass, JoinDirection::Forward);
    EXPECT_STREQ ("SELECT p.I, p.S FROM ONLY [ecsql].[PSA] p JOIN ONLY [ecsql].[PSA] c USING [ecsql].[PSAHasPSA] FORWARD", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("p.I, p.S").From (*psaClass, "p", false).Join (*psaClass, "c", false).Using (*psaHasPsaRelClass, JoinDirection::Reverse);
    EXPECT_STREQ ("SELECT p.I, p.S FROM ONLY [ecsql].[PSA] p JOIN ONLY [ecsql].[PSA] c USING [ecsql].[PSAHasPSA] REVERSE", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("SELECT p.I, p.S FROM ONLY [ecsql].[PSA] p JOIN ONLY [ecsql].[PSA] c USING [ecsql].[PSAHasP] WHERE c.ECInstanceId = ? AND p.S LIKE 'ma%'", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("p.I, p.S").From (*psaClass, "p", false).Join (*psaClass, "c", false).Using (*psaHasPsaRelClass).WhereSourceEndIs ("p", "?");
    EXPECT_STREQ ("SELECT p.I, p.S FROM ONLY [ecsql].[PSA] p JOIN ONLY [ecsql].[PSA] c USING [ecsql].[PSAHasPSA] WHERE SourceECInstanceId = ?", builder.ToString ().c_str ());

    builder = ECSqlSelectBuilder ();
    builder.Select ("p.I, p.S").From (*psaClass, "p", false).Join (*psaClass, "c", false).Using (*psaHasPsaRelClass).WhereTargetEndIs ("c", "?");
    EXPECT_STREQ ("SELECT p.I, p.S FROM ONLY [ecsql].[PSA] p JOIN ONLY [ecsql].[PSA] c USING [ecsql].[PSAHasPSA] WHERE TargetECInstanceId = ?", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("SELECT p.I, p.S FROM ONLY [ecsql].[PSA] p JOIN ONLY [ecsql].[PSA] c USING [ecsql].[PSAHasPSA] WHERE SourceECInstanceId = 123 AND TargetECInstanceId = 444", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("SELECT p.I, p.S FROM ONLY [ecsql].[PSA] p JOIN ONLY [ecsql].[P] c USING [ecsql].[PSAHasPSA] WHERE c.ECInstanceId = ? AND p.S LIKE 'ma%'", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("SELECT p.I, p.S FROM ONLY [ecsql].[PSA] p JOIN ONLY [ecsql].[PSA] c USING [ecsql].[PSAHasPSA] WHERE p.L = 10 ORDER BY c.ECInstanceId LIMIT 10 OFFSET 5", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("SELECT Bli? Bla! FROM ONLY [ecsql].[PSA] WHERE Blo knirsch 3.14", builder.ToString ().c_str ());
    }
...
TEST (ECSqlBuilderTests, ECSqlInsertBuilder_ToString)
    ...
    EXPECT_STREQ ("INSERT INTO [ecsql].[PSA]", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("INSERT INTO [ecsql].[PStruct] (i) VALUES (12)", builder.ToString ().c_str ());

    builder = ECSqlInsertBuilder ();
    builder.InsertInto (*psaClass).AddValue ("I", "?").AddValue ("L", "1023122");
    EXPECT_STREQ ("INSERT INTO [ecsql].[PSA] (I, L) VALUES (?, 1023122)", builder.ToString ().c_str ());

    builder = ECSqlInsertBuilder ();
    builder.InsertInto (*psaClass).AddValue ("I", "?").AddValue ("L", "1023122").AddValue ("D", "?");
    EXPECT_STREQ ("INSERT INTO [ecsql].[PSA] (I, L, D) VALUES (?, 1023122, ?)", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("INSERT INTO [ecsql].[PSA] (knirsch ? no, bli !) VALUES (he! , )", builder.ToString ().c_str ());
    }
...
TEST (ECSqlBuilderTests, ECSqlUpdateBuilder_ToString)
    ...
    EXPECT_STREQ ("UPDATE ONLY [ecsql].[PSA]", builder.ToString ().c_str ());

    builder = ECSqlUpdateBuilder ();
    builder.Update (*psaClass, false).AddSet ("I", "123").Where ("ECInstanceId > ?");
    EXPECT_STREQ ("UPDATE ONLY [ecsql].[PSA] SET I = 123 WHERE ECInstanceId > ?", builder.ToString ().c_str ());

    builder = ECSqlUpdateBuilder ();
    builder.Update (*nonDomainStructWithPrims, false).AddSet ("i", "123").Where ("l IN (?, ?)");
    EXPECT_STREQ ("UPDATE ONLY [ecsql].[PStruct] SET i = 123 WHERE l IN (?, ?)", builder.ToString ().c_str ());

    builder = ECSqlUpdateBuilder ();
    builder.Update (*psaClass, false).AddSet ("I", "123").AddSet ("L", "?").Where ("ECInstanceId > ?");
    EXPECT_STREQ ("UPDATE ONLY [ecsql].[PSA] SET I = 123, L = ? WHERE ECInstanceId > ?", builder.ToString ().c_str ());

    builder = ECSqlUpdateBuilder ();
    builder.Update (*psaClass, true).AddSet ("I", "123").AddSet ("L", "?").Where ("ECInstanceId > ?");
    EXPECT_STREQ ("UPDATE [ecsql].[PSA] SET I = 123, L = ? WHERE ECInstanceId > ?", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("UPDATE ONLY [ecsql].[PSA] SET knirsch ? no = he! , bli ! = $", builder.ToString ().c_str ());
    }
...
TEST (ECSqlBuilderTests, ECSqlDeleteBuilder_ToString)
    ...
    EXPECT_STREQ ("DELETE FROM ONLY [ecsql].[PSA]", builder.ToString ().c_str ());

    builder = ECSqlDeleteBuilder ();
    builder.DeleteFrom (*psaClass, true);
    EXPECT_STREQ ("DELETE FROM [ecsql].[PSA]", builder.ToString ().c_str ());

    builder = ECSqlDeleteBuilder ();
    builder.DeleteFrom (*psaClass, true).Where ("I = ? OR S LIKE 'he%'");
    EXPECT_STREQ ("DELETE FROM [ecsql].[PSA] WHERE I = ? OR S LIKE 'he%'", builder.ToString ().c_str ());

    builder = ECSqlDeleteBuilder ();
    builder.DeleteFrom (*psaClass, false).Where ("I = ? OR S LIKE 'he%'");
    EXPECT_STREQ ("DELETE FROM ONLY [ecsql].[PSA] WHERE I = ? OR S LIKE 'he%'", builder.ToString ().c_str ());

    builder = ECSqlDeleteBuilder ();
    builder.DeleteFrom (*nonDomainStructWithPrims, false).Where ("i = ? OR s LIKE 'he%'");
    EXPECT_STREQ ("DELETE FROM ONLY [ecsql].[PStruct] WHERE i = ? OR s LIKE 'he%'", builder.ToString ().c_str ());
    ...
    EXPECT_STREQ ("DELETE FROM ONLY [ecsql].[PSA] WHERE Blo knirsch 3.14", builder.ToString ().c_str ());
    }
