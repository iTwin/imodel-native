/*--------------------------------------------------------------------------------------+
|
|  $Source: Tests/ECDB/Published/ECDbBrownBag_Demo.cpp $
|
...
TEST (ECDbBrownBagDemo, CreateECDbFromScratch)
    ...
    BentleyStatus importStat = ecdb.GetEC ().GetSchemaManager ().ImportECSchemas (schemaContext->GetCache ());
...
TEST (ECDbBrownBagDemo, SchemaManager)
    ...
    ECDbSchemaManagerCR schemaManager = ecdb.GetEC ().GetSchemaManager ();

    //** Get ECSchema from ECDb file
    ECSchemaP schema = nullptr;
    schemaManager.GetECSchema (schema, "StartupCompany");
    ...
    schemaManager.GetECClass (employeeClass, "StartupCompany", "Employee");
...
TEST (ECDbBrownBagDemo, PopulateSampleECDb)
    ...
    ECSchemaP schema = nullptr;
    ecdb.GetEC ().GetSchemaManager ().GetECSchema (schema, "StartupCompany", true);
