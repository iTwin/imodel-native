...
ECSchemaReadContextPtr LocateECSchema (ECDbR ecDB, BeFileNameCR ecSchemaFile, ECSchemaPtr& ecSchema)
    ...
    if (schemaFullName.substr (extPos).CompareToI (ecSchemaExt) == 0)
        ...
        if (ECSchema::ParseSchemaFullName (schemaName, schemaMajor, schemaMinor, schemaFullName.substr (0, extPos)) == ECOBJECTS_STATUS_Success)
            ...
            contextPtr->AddSchemaLocater (ecDB.GetEC ().GetSchemaLocater ());
...
bool ImportECSchema (ECDbR ecDB, ECSchemaReadContextPtr contextPtr)
    ...
    if (contextPtr.IsValid ())
        {
        BentleyStatus state = ecDB.GetEC ().GetSchemaManager ().ImportECSchemas (contextPtr->GetCache ());
...
TEST(ECDbSchemas, UpdateExistingECSchemaWithNewProperties)
    ...
    auto importSchemaStatus = db.GetEC().GetSchemaManager().ImportECSchemas (*schemaCache12);
    ASSERT_EQ (SUCCESS, importSchemaStatus);
    ...
    importSchemaStatus = db.GetEC ().GetSchemaManager ().ImportECSchemas (*schemaCache11, ECDbSchemaManager::ImportOptions (true, true));
    ...
    importSchemaStatus = db.GetEC ().GetSchemaManager ().ImportECSchemas (*schemaCache13, ECDbSchemaManager::ImportOptions (true, true));
    ASSERT_EQ(SUCCESS, importSchemaStatus);
    db.CloseDb();
    db.OpenBeSQLiteDb(ecdbFileName.c_str(), Db::OpenParams(Db::OPEN_ReadWrite, DefaultTxn_Yes));

    ECSchemaP  updatedECSchema = nullptr;
    ASSERT_EQ(SUCCESS, db.GetEC().GetSchemaManager().GetECSchema(updatedECSchema, "TestSchema"));
...
TEST (ECDbSchemas, UpdatingSchemaShouldNotDeleteExistingRelationshipsOrIndexes)
    ...
    auto status = ecDb.GetEC ().GetSchemaManager ().ImportECSchemas (schemaCache, ECDbSchemaManager::ImportOptions (true, true));
...
TEST(ECDbSchemas, VerifyEmptyECSchemaCanBeRead)
    ...
    auto schemaStat = ecdb.GetEC().GetSchemaManager().ImportECSchemas(*cache);
    ASSERT_EQ (SUCCESS, schemaStat) << "Importing empty ECSchema failed";
    ecdb.GetEC().ClearCache();
    ECSchemaP stroedEmptySchema;
    schemaStat = ecdb.GetEC ().GetSchemaManager ().GetECSchema (stroedEmptySchema, "EmptyECSchema");
    ASSERT_EQ (SUCCESS, schemaStat) << "Getting empty ECSchema failed";
...
TEST(ECDbSchemas, CreateCloseOpenImport)
    ...
    ASSERT_EQ (SUCCESS, db.GetEC ().GetSchemaManager ().ImportECSchemas (schemaContext->GetCache (), ECDbSchemaManager::ImportOptions (false, false))) << "ImportECSchema should have imported successfully after closing and re-opening the database.";
...
TEST(ECDbSchemas, ImportSchemaAgainstExistingTableWithoutECInstanceIdColumn)
    ...
    BeTest::SetFailOnAssert (false);
        {
        ASSERT_EQ (ERROR, ecdb.GetEC ().GetSchemaManager ().ImportECSchemas (*testSchemaCache, ECDbSchemaManager::ImportOptions (false, false))) << "ImportECSchema is expected to return success for schemas with classes that map to an existing table.";
        }
...
TEST(ECDbSchemas, ImportSchemaAgainstExistingTableWithECInstanceIdColumn)
    ...
    ASSERT_EQ (SUCCESS, ecdb.GetEC ().GetSchemaManager ().ImportECSchemas (*testSchemaCache, 
...
TEST (ECDbSchemas, DeigoRelationshipTest)
    ...
    //now import test schema where the table already exists for the ECClass
    ASSERT_EQ (SUCCESS, ecdb.GetEC ().GetSchemaManager ().ImportECSchemas (ctx->GetCache(),
        ECDbSchemaManager::ImportOptions (false, false))) << "ImportECSchema is expected to return success for schemas with classes that map to an existing table.";


    ECClassP aCivilModel = nullptr, aDataSetModel = nullptr, aGeometricModel = nullptr, aCivilModelHasDataSetModel = nullptr;

    ecdb.GetEC ().GetSchemaManager ().GetECClass (aCivilModel, "DiegoSchema1", "CivilModel");
    ecdb.GetEC ().GetSchemaManager ().GetECClass (aDataSetModel, "DiegoSchema1", "DataSetModel");
    ecdb.GetEC ().GetSchemaManager ().GetECClass (aCivilModelHasDataSetModel, "DiegoSchema1", "CivilModelHasDataSetModel");
    ecdb.GetEC ().GetSchemaManager ().GetECClass (aGeometricModel, "DiegoSchema2", "GeometricModel");
...
TEST (ECDbSchemas, DeigoRelationshipTest2)
    ...
    ECClassP aCivilModel, aDataSetModel, aGeometricModel, aDesignStandardsModel, aCivilModel__DataSetModel;
    
    aCivilModel = aDataSetModel = aGeometricModel = aDesignStandardsModel = aCivilModel__DataSetModel = nullptr;

    auto& schemaManager = ecdb.GetEC ().GetSchemaManager ();
    schemaManager.GetECClass (aCivilModel, "HW_Bentley_Civil__Model_Base", "CivilModel");
    schemaManager.GetECClass (aDataSetModel, "HW_Bentley_Civil__Model_Base", "DataSetModel");
    schemaManager.GetECClass (aCivilModel__DataSetModel, "HW_Bentley_Civil__Model_Base", "CivilModel__DataSetModel");
    schemaManager.GetECClass (aGeometricModel, "HW_Bentley_Civil__Model_Geometry", "GeometricModel");
    schemaManager.GetECClass (aDesignStandardsModel, "HW_Bentley_Civil__Model_DesignStandards", "DesignStandardsModel");
...
TEST(ECDbSchemas, ImportSchemaWithRelationshipAgainstExistingTable)
    ...
    ASSERT_EQ (SUCCESS, ecdb.GetEC ().GetSchemaManager ().ImportECSchemas (*testSchemaCache, ECDbSchemaManager::ImportOptions (false, false))) << "ImportECSchema is expected to return success for schemas with classes that map to an existing table.";
...
TEST(ECDbSchemas, ReadCustomAttributesTest)
    ...
    Utf8String dbPath;
        ...
        auto importStat = db.GetEC ().GetSchemaManager ().ImportECSchemas (*testSchemaCache);
    ...
    ECSchemaP readSchema = nullptr;
    auto schemaStatus = db.GetEC ().GetSchemaManager ().GetECSchema (readSchema, Utf8String (testSchema->GetName().c_str ()).c_str ());
    ASSERT_EQ (SUCCESS, schemaStatus) << L"Could not read test schema from reopened ECDb file.";
...
TEST(ECDbSchemas, ImportSupplementalSchemas)
    ...
    ASSERT_EQ (BE_SQLITE_OK, stat) << "Creation of test ECDb file failed.";
    auto schemaStatus = db.GetEC ().GetSchemaManager ().ImportECSchemas (schemaContext->GetCache (),
    ...
    ECSchemaP startupCompanySchema = nullptr;
    schemaStatus = db.GetEC ().GetSchemaManager ().GetECSchema (startupCompanySchema, "StartupCompany");
    ASSERT_EQ (SUCCESS, schemaStatus);
    //ASSERT_EQ (startupCompanySchema->IsSupplemented(), true);
    ...
    //ECSchemaP openPlant3D = schemaContext->GetKnownSchemas().GetSchema(SchemaKey(L"OpenPlant_3D",1, 2));
    //ECSchemaP openPlant = schemaContext->GetKnownSchemas().GetSchema(SchemaKey(L"OpenPlant",1, 2));

    //ASSERT_TRUE(openPlant3D != nullptr);
    //ASSERT_TRUE(openPlant != nullptr);

    //ASSERT_EQ (openPlant3D->IsSupplemented(), true);
    //ASSERT_EQ (openPlant->IsSupplemented(), true);    
    }
...
TEST(ECDbSchemas, SystemSchemaTest)
   ...
   ECSchemaP startupCompanySchema = nullptr;
   auto schemaStatus = db.GetEC().GetSchemaManager().GetECSchema (startupCompanySchema, "StartupCompany");
   ASSERT_EQ (SUCCESS, schemaStatus);
   ECSchemaP ecdbSystemSchema = nullptr;
   schemaStatus = db.GetEC().GetSchemaManager().GetECSchema (ecdbSystemSchema, "ECDbSystem");
   ASSERT_EQ (SUCCESS, schemaStatus);
...
TEST (ECDbSchemas, ArrayPropertyTest)
    ...
    db.GetEC ().ClearCache ();
    ECSchemaP startupCompanySchema = nullptr;
    auto schemaStatus = db.GetEC ().GetSchemaManager ().GetECSchema (startupCompanySchema, "StartupCompany", true);
    ASSERT_EQ (SUCCESS, schemaStatus);
    ASSERT_TRUE (startupCompanySchema != nullptr);

    auto arrayTestClass = startupCompanySchema->GetClassP (L"ArrayTestclass");
...
TEST(ECDbSchemas, ECDbSchemaManagerAPITest)
    ...
    ECSchemaP openPlant3D = nullptr;
    ECDbSchemaManagerCR schemaManager = ecDbStore.GetSchemaManager();
    ///////////////////////Load StartupCompany//////////////////////////////
    StopWatch s0 ("Loading StartupCompany Schema From Database", true);
    EXPECT_EQ (SUCCESS, schemaManager.GetECSchema (openPlant3D, "StartupCompany", true /* Load full schemas*/));
    s0.Stop();
    ...
    for(int i=0; i < maxClassesToLoad; i++)
        ...
        EXPECT_EQ (SUCCESS, schemaManager.GetECClass (ecClass, key.GetECClassId ()));
        randomClassSW.Stop();
    ...
    for (DbECSchemaKey schemaKey : schemasInDb)
        {
         ...
         //EXPECT_TRUE ( schemaManager.ContainsECSchema(schemaKey) == true); //schema key
         //EXPECT_TRUE ( schemaManager.ContainsECSchema(schemaKey.GetECSchemaId()) == true); //schema Id
         //EXPECT_TRUE ( schemaManager.ContainsECSchema(SchemaKey(WString(schemaKey.GetName(), true).c_str(), schemaKey.GetVersionMajor(), schemaKey.GetVersionMinor())) == true); //ECN::SchemaKey
         //Check GetECCSchema() and all overloads. Full ecschema are loaded in this case
         outSchema = nullptr;
         EXPECT_EQ (SUCCESS, schemaManager.GetECSchemaByKey (outSchema, schemaKey));
         ...
         outSchema = nullptr;
         EXPECT_EQ (SUCCESS, schemaManager.GetECSchema (outSchema, ecSchemaId));
         EXPECT_TRUE(outSchema != nullptr);

         outSchema = nullptr;
         EXPECT_EQ (SUCCESS, schemaManager.GetECSchema (outSchema, schemaKey.GetName ()));
         EXPECT_TRUE(outSchema != nullptr);

         outSchema = nullptr;         
         EXPECT_EQ (ERROR, schemaManager.GetECSchema(outSchema, schemaKey.GetFullName().c_str()));
         EXPECT_TRUE(outSchema == nullptr);
         ...
         for (DbECClassKey classKey : classKeys)
             {
             outClass = nullptr;
             EXPECT_EQ (SUCCESS, schemaManager.GetECClass(outClass, classKey));
             EXPECT_TRUE(outClass != nullptr);
             outClass = nullptr;
             EXPECT_EQ (SUCCESS, schemaManager.GetECClass (outClass, classKey.GetECClassId ()));
             EXPECT_TRUE(outClass != nullptr);
             outClass = nullptr;
             EXPECT_EQ (SUCCESS, schemaManager.GetECClass (outClass, schemaKey.GetName (), classKey.GetName ()));
             EXPECT_TRUE(outClass != nullptr);
             outClass = nullptr;
             }
        }
...
TEST_F(ECDbSchemaFixture,TestDuplicateNamespacePrefix)
    ...
    auto status = db.GetEC ().GetSchemaManager ().ImportECSchemas (*cache, ECDbSchemaManager::ImportOptions (true, false));
    ASSERT_EQ (SUCCESS, status);
    db.GetEC().ClearCache();
    for(auto& x : data)
        {
        ...
        auto expected   = x[2];
        ECSchemaP out;
        auto schemaGetStatus = db.GetEC().GetSchemaManager().GetECSchema(out, Utf8String(name.c_str()).c_str(), true);
        ASSERT_EQ (SUCCESS, schemaGetStatus);
...
TEST(ECDbSchemas, Verify_TFS_14829_A)
   {
    ...
    ECDbTestUtility::ReadECSchemaFromDisk (ecSchema, schemaContext, L"StartupCompany.02.00.ecschema.xml");
    auto schemaStatus = db.GetEC ().GetSchemaManager ().ImportECSchemas (schemaContext->GetCache (), ECDbSchemaManager::ImportOptions (true, bUpdate));
    ASSERT_EQ (SUCCESS, schemaStatus);

    ECDbTestUtility::ReadECSchemaFromDisk (ecSchema, schemaContext, L"SimpleCompany.01.00.ecschema.xml");
    schemaStatus = db.GetEC ().GetSchemaManager ().ImportECSchemas (schemaContext->GetCache (), ECDbSchemaManager::ImportOptions (true, bUpdate));
    ASSERT_EQ (SUCCESS, schemaStatus);
...
TEST(ECDbSchemas, Verify_TFS_14829_B)
   {
    ...
    ECDbTestUtility::ReadECSchemaFromDisk (ecSchema, schemaContext, L"SimpleCompany.01.00.ecschema.xml");
    schemaStatus = db.GetEC().GetSchemaManager().ImportECSchemas (schemaContext->GetCache());
    ASSERT_EQ (SUCCESS, schemaStatus);
...
TEST (ECDbSchema, Replace_TableWithNullView_And_ViewsWithTablesTest)
    {
    // Save a test project
    ECDbTestProject saveTestProject;
    saveTestProject.Create ("StartupCompany.ecdb", L"StartupCompany.02.00.ecschema.xml", false);
    ...
    std::set<Utf8String> tables, views, recreatedTables;
    Statement stmt1;
    stat = stmt1.Prepare (db, "select SM.tbl_name from sqlite_master SM INNER  JOIN ec_ClassMap CM ON CM.[MapToDbTable] =  SM.tbl_name WHERE type in ('table')");
    ASSERT_EQ (stat, BE_SQLITE_OK);
    while (stmt1.Step () == BE_SQLITE_ROW)
        tables.insert (stmt1.GetValueText (0));

    stat = db.GetEC ().ReplaceEmptyTablesWithEmptyViews ();
    ASSERT_EQ (stat, BE_SQLITE_OK);


    Statement stmt2;
    stat = stmt2.Prepare (db, "select SM.tbl_name from sqlite_master SM INNER  JOIN ec_ClassMap CM ON CM.[MapToDbTable] =  SM.tbl_name WHERE type in ('view')");
    ASSERT_EQ (stat, BE_SQLITE_OK);
    ...
    for (auto& o : tables)
        ...
        if (views.find (o) == views.end ())
            {
            ASSERT_TRUE (false) << "Failed to replace table [" << o.c_str () << "] with a view";
            }
        }

    for (auto& o : views)
        {
        if (tables.find (o) == tables.end ())
            {
            ASSERT_TRUE (false) << "Found a view [" << o.c_str() << "] with no associated table ";
            }
        }

    stat = db.GetEC ().ReplaceViewsWithEmptyTables ();
    ASSERT_EQ (stat, BE_SQLITE_OK);

    Statement stmt3;
    stat = stmt3.Prepare (db, "select SM.tbl_name from sqlite_master SM INNER  JOIN ec_ClassMap CM ON CM.[MapToDbTable] =  SM.tbl_name WHERE type in ('table')");
    ASSERT_EQ (stat, BE_SQLITE_OK);
    while (stmt3.Step () == BE_SQLITE_ROW)
        recreatedTables.insert (stmt3.GetValueText (0));
    ...
    for (auto& o : tables)
        {
        if (recreatedTables.find (o) == recreatedTables.end ())
            {
            ASSERT_TRUE (false) << "Failed to recreate table [" << o.c_str () << "]";
            }
        }

    for (auto& o : recreatedTables)
        {
        if (tables.find (o) == tables.end ())
            {
            ASSERT_TRUE (false) << "Found a table [" << o.c_str () << "] with did not existed when schema was imported";
            }
        }
    }
//=====Affan/Assert/Test============================================================================================
