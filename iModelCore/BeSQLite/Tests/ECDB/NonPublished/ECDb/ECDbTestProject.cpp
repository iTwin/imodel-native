...
ECClassCP ECDbTestSchemaManager::GetClass (Utf8CP schemaName, Utf8CP className) const
    {
    ECClassP foundClass = nullptr;
    if (SUCCESS == GetSchemaManager ().GetECClass (foundClass, schemaName, className))
        return foundClass;
    else
        return nullptr;
    }
...
ECDbSchemaManagerCR ECDbTestSchemaManager::GetSchemaManager () const
    {
    return GetECDb ().GetEC ().GetSchemaManager ();
    }
...
void ECDbTestUtility::DumpECSchemaUsageStatistics (ECSchemaCR schema, ECDbR ecdb, bool dumpEmptyClasses)
    ...
    for (ECClassP ecClass : schema.GetClasses())
        ...
        ECPersistencePtr persistence = ecdb.GetEC ().GetECPersistence (&status, *ecClass);
        if (!persistence.IsValid() && status == PERSISTENCE_ClassIsNotMapped)
            {
