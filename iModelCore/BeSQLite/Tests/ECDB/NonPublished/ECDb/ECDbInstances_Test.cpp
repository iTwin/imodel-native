...
/*---------------------------------------------------------------------------------**//**
* @bsiclass                                     Affan.Khan                        03/13
+---------------+---------------+---------------+---------------+---------------+------*/
TEST(ECDbInstances, QueryFromView)
    {
    ECDbTestProject saveTestProject;
    saveTestProject.Create ("StartupCompany.ecdb", L"StartupCompany.02.00.ecschema.xml", false);
    auto const& store= saveTestProject.GetECDb().GetEC();

    ASSERT_EQ(BE_SQLITE_OK, store.ReplaceEmptyTablesWithEmptyViews());

    ECN::ECClassP classP = nullptr;
    ASSERT_EQ (SUCCESS, store.GetSchemaManager().GetECClass(classP,"StartupCompany","Asset"));

    ECSqlSelectBuilder selectBuilder;
    selectBuilder.From(*classP).SelectAll();
    ECSqlStatement ecStatement;        
    auto stat = ecStatement.Prepare(saveTestProject.GetECDb(), selectBuilder.ToString ().c_str ()); 
...
TEST(ECDbInstances, CreateAndImportSchemaThenInsertInstance)
    ...
    ASSERT_EQ (SUCCESS, db.GetEC ().GetSchemaManager ().ImportECSchemas (*schemaCache, ECDbSchemaManager::ImportOptions (true, true)));
...
TEST(ECDbInstances, UpdateArrayProperty)
    ...
    ECN::ECClassP testClass = nullptr;
    ASSERT_EQ (SUCCESS, db.GetEC().GetSchemaManager().GetECClass(testClass,"KitchenSink","TestClass"));
...
TEST(ECDbInstances, FindECInstances)
    ...
    ecDbStore.GetSchemaManager().GetECClassKeys (classKeys, "StartupCompany");
    for (DbECClassKey ecClassKey : classKeys)
        {
        ECClassP ecClass = nullptr;
        ecDbStore.GetSchemaManager().GetECClass (ecClass, ecClassKey.GetECClassId());  // WIP_FNV: start taking ECSchemaName, ECClassName, instead
...
TEST(ECDbInstances, SelectClause)
    ...
    ECClassP employee;
    auto sms = ecDbStore.GetSchemaManager().GetECClass(employee, "StartupCompany", "Employee");
    ASSERT_EQ (SUCCESS, sms);
...
TEST(ECDbInstances, AdapterCheckClassBeforeOperation)
    ...
    ECClassP project;
    sms = ecDbStore.GetSchemaManager().GetECClass(project, "StartupCompany", "Project");
    ASSERT_EQ(SUCCESS, sms);
    ...
    ECInstanceKey instanceKey;
    sms = inserter.Insert(instanceKey, *instance);
    EXPECT_EQ(ERROR, sms);
...
TEST(ECDbInstances, DomainCustomAttributeStructCombinations)
{
    ...
    ECClassP allTrue;
    auto sms = ecDbStore.GetSchemaManager().GetECClass(allTrue, "TryClassCombinations", "S_T_CA_T_D_T");
    ASSERT_EQ(SUCCESS, sms);
    IECInstancePtr instance;
    ...
    ECClassP Test1;
    sms = ecDbStore.GetSchemaManager().GetECClass(Test1, "TryClassCombinations", "S_T_CA_F_D_T");
    ASSERT_EQ(SUCCESS, sms);
    ...
    ECClassP Test2;
    sms = ecDbStore.GetSchemaManager().GetECClass(Test2, "TryClassCombinations", "S_F_CA_T_D_T");
    ASSERT_EQ(SUCCESS, sms);
