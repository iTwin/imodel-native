...
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, PrepareStatement)
    {
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, PrepareStatementWithBlobColumns)
    {
    BeFileName temporaryDir;
    BeTest::GetHost ().GetOutputRoot (temporaryDir);
    BeSQLiteLib::Initialize (temporaryDir);
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckEmptinessOnEmptyTablesWithPK)
    {
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckEmptinessOnEmptyTablesWithPKAndAdditionalIndex)
    {
    const int tableCount = 2000;
    const int rowCount = 0;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckEmptinessOnEmptyTablesNoIndexes)
    {
    const int tableCount = 2000;
    const int rowCount = 0;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckEmptinessOnTablesWithOneRowWithPK)
    {
    const int tableCount = 2000;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckEmptinessOnTablesWithOneRowWithPKAndAdditionalIndex)
    {
    const int tableCount = 2000;
    const int rowCount = 1;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckEmptinessOnTablesWithOneRowNoIndexes)
    {
    const int tableCount = 2000;
    const int rowCount = 1;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckEmptinessOnNonEmptyTablesWithPK)
    {
    const int tableCount = 2000;
    const int rowCount = 100;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckEmptinessOnNonEmptyTablesWithPKAndAdditionalIndex)
    {
    const int tableCount = 2000;
    const int rowCount = 100;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckEmptinessOnNonEmptyTablesNoIndexes)
    {
    const int tableCount = 2000;
    const int rowCount = 100;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckTableExistenceOfExistingTables)
    {
    const int tableCount = 2000;
    const int rowCount = 100;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, CheckTableExistenceOfNonExistingTables)
    {
    const int tableCount = 2000;
    const int rowCount = 100;
...
bool createIndices
    ...
    if (createIndices)
        {
        LOG.infov ("Dropping %d tables [%d rows; one index besides PK] with multiple calls took %.4f seconds.", tableCount, rowCount, timer.GetElapsedSeconds ());
        }
    else
        {
        LOG.infov ("Dropping %d tables [%d rows; no index besides PK] with multiple calls took %.4f seconds.", tableCount, rowCount, timer.GetElapsedSeconds ());
        }
    }
...
bool createIndices
    ...
    if (createIndices)
        {
        LOG.infov ("Dropping %d tables [%d rows; one index besides PK] with a single call took %.4f seconds.", tableCount, rowCount, timer.GetElapsedSeconds ());
        }
    else
        {
        LOG.infov ("Dropping %d tables [%d rows; no index besides PK] with a single call took %.4f seconds.", tableCount, rowCount, timer.GetElapsedSeconds ());
        }
    }
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, ExecuteDropEmptyTablesHavingIndicesWithMultipleCalls)
    {
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, ExecuteDropTablesHavingIndicesWithMultipleCalls)
    {
    const int tableCount = 5000;
    const int rowCount = 100;
    RunExecuteDropTablesWithMultipleCallsTest (tableCount, rowCount, true);
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, ExecuteDropEmptyTablesNotHavingIndicesWithMultipleCalls)
    {
    const int tableCount = 5000;
    const int rowCount = 0;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, ExecuteDropTablesNotHavingIndicesWithMultipleCalls)
    {
    const int tableCount = 5000;
    const int rowCount = 100;
    RunExecuteDropTablesWithMultipleCallsTest (tableCount, rowCount, false);
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, ExecuteDropEmptyTablesHavingIndicesWithSingleCall)
    {
    const int tableCount = 5000;
    const int rowCount = 0;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, ExecuteDropTablesHavingIndicesWithSingleCall)
    {
    const int tableCount = 5000;
    const int rowCount = 100;
    RunExecuteDropTablesWithSingleCallsTest (tableCount, rowCount, true);
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, ExecuteDropEmptyTablesNotHavingIndicesWithSingleCall)
    {
    const int tableCount = 5000;
    const int rowCount = 0;
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, ExecuteDropTablesNotHavingIndicesWithSingleCall)
    {
    const int tableCount = 5000;
    const int rowCount = 100;
    RunExecuteDropTablesWithSingleCallsTest (tableCount, rowCount, false);
...
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, ExecuteCreateEmptyViewWithSingleCall)
    {
    BeFileName temporaryDir;
    BeTest::GetHost ().GetOutputRoot (temporaryDir);
    BeSQLiteLib::Initialize (temporaryDir);
    ...
    timer.Stop ();
    db.CloseDb ();
    LOG.infov ("Creating %d empty views with a single call took %.4f seconds.", viewCount, timer.GetElapsedSeconds ());
    }

//---------------------------------------------------------------------------------------
// @bsiclass                                     Krischan.Eberle                  12/12
//+---------------+---------------+---------------+---------------+---------------+------
TEST(PerformanceSqlStatementTests, ExecuteCreateEmptyViewWithMultipleCalls)
    {
    BeFileName temporaryDir;
    BeTest::GetHost ().GetOutputRoot (temporaryDir);
    BeSQLiteLib::Initialize (temporaryDir);
    ...
    timer.Stop ();
    db.CloseDb ();
    LOG.infov ("Creating %d empty views with multiple calls took %.4f seconds.", viewCount, timer.GetElapsedSeconds ());
    }
