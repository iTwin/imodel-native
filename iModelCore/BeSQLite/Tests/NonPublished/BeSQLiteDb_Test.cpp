...
struct MyChangeSet : ChangeSet {
    virtual ConflictResolution _OnConflict(ConflictCause clause, Changes::Change iter) {
        BeAssert(false);
...
TEST_F(BeSQLiteDbTests, ChangeGroup_Filter) {
    ...
    ASSERT_EQ(BE_SQLITE_OK, ChangeGroup::FilterIf(cs, [&](Changes::Change const& change) { return getChangeTable(change).EqualsIAscii("t1"); }, t1Group));
    ...
    ASSERT_EQ(BE_SQLITE_OK, ChangeGroup::FilterIfElse(cs, [&](Changes::Change const& change) { return getChangeTable(change).EqualsIAscii("t2"); }, t2Group, t1t3Group));
