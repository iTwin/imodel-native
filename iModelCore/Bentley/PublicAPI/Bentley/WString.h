...
public:
    ...
    template<typename... Args> static int Swscanf_safe(const wchar_t* buffer, const wchar_t* format, Args&&... args) {
        BeAssert(std::wstring::npos == std::wstring(format).find(L"%s") && "%s is unsafe, do not use sscanf for that purpose");
...
public:
    ...
    template<typename... Args> static int Sscanf_safe(const char* const buffer, const char* const format, Args&&... args) {
        ...
        BeAssert(std::string::npos == std::string(format).find("%s") && "%s is unsafe, do not use sscanf for that purpose");
