...
// efficient compare-to code when using common Bentley string types // BENTLEY CHANGES
...
template <>
struct btree_key_compare_to_adapter<std::less<bastring> >
    : public btree_key_compare_to_tag {
  btree_key_compare_to_adapter() {}
  btree_key_compare_to_adapter(const std::less<bastring>&) {}
  btree_key_compare_to_adapter(
      const btree_key_compare_to_adapter<std::less<bastring> >&) {}
  int operator()(const bastring &a, const bastring &b) const {
    return a.compare(b);
  }
};

template <>
struct btree_key_compare_to_adapter<std::greater<bastring> >
    : public btree_key_compare_to_tag {
  btree_key_compare_to_adapter() {}
  btree_key_compare_to_adapter(const std::greater<bastring>&) {}
  btree_key_compare_to_adapter(
      const btree_key_compare_to_adapter<std::greater<bastring> >&) {}
  int operator()(const bastring &a, const bastring &b) const {
    return b.compare(a);
  }
};
template <>
struct btree_key_compare_to_adapter<std::less<bwstring> >
    : public btree_key_compare_to_tag {
  btree_key_compare_to_adapter() {}
  btree_key_compare_to_adapter(const std::less<bwstring>&) {}
  btree_key_compare_to_adapter(
      const btree_key_compare_to_adapter<std::less<bwstring> >&) {}
  int operator()(const bwstring &a, const bwstring &b) const {
    return a.compare(b);
  }
};

template <>
struct btree_key_compare_to_adapter<std::greater<bwstring> >
    : public btree_key_compare_to_tag {
  btree_key_compare_to_adapter() {}
  btree_key_compare_to_adapter(const std::greater<bwstring>&) {}
  btree_key_compare_to_adapter(
      const btree_key_compare_to_adapter<std::greater<bwstring> >&) {}
  int operator()(const bwstring &a, const bwstring &b) const {
    return b.compare(a);
  }
};
...
// A parameters structure for holding the type parameters for a bmap.
...
struct bmap_params
  ...
  typedef bpair<const Key, data_type> value_type;
  typedef bpair<Key, data_type> mutable_value_type;
...
class btree_node {
 ...
 public:
  ...
  void set_position(int v) { fields_.position = (typename base_fields::field_type) v; }

  // Getter/setter for the number of values stored in this node.
  int count() const { return fields_.count; }
  void set_count(int v) { fields_.count = (typename base_fields::field_type) v; }
...
struct btree_internal_locate_plain_compare {
  ...
  static bpair<Iter, int> dispatch(const K &k, const T &t, Iter iter) {
...
class btree : public Params::key_compare {
 ...
 public:
  ...
  // the btree. See bmap::operator[].
  template <typename ValuePointer>
  bpair<iterator,bool> insert_unique(const key_type &key, ValuePointer value);

  // Inserts a value into the btree only if it does not already exist. The
  // boolean return value indicates whether insertion succeeded or failed.
  bpair<iterator,bool> insert_unique(const value_type &v) {
 ...
 private:
  ...
  template <typename IterType>
  bpair<IterType, int> internal_locate(
      const key_type &key, IterType iter) const;
  template <typename IterType>
  bpair<IterType, int> internal_locate_plain_compare(
      const key_type &key, IterType iter) const;
  template <typename IterType>
  bpair<IterType, int> internal_locate_compare_to(
      const key_type &key, IterType iter) const;
...
#if REMOVED_DOESNT_COMPILE // BENTLEY
  static_assert(sizeof(key_compare_checker(key_compare_helper()(key_type(), key_type()))) == sizeof(big_), "key_comparison_function_must_return_bool");
#endif
...
bpair<typename btree<P>::iterator, bool>
btree<P>::insert_unique(const key_type &key, ValuePointer value) {
  if (empty()) {
    *mutable_root() = new_leaf_root_node(1);
  }

  bpair<iterator, int> res = internal_locate(key, iterator(root(), 0));
...
template <typename P> template <typename IterType>
inline bpair<IterType, int> btree<P>::internal_locate(
    const key_type &key, IterType iter) const {
  return internal_locate_type::dispatch(key, *this, iter);
}

template <typename P> template <typename IterType>
inline bpair<IterType, int> btree<P>::internal_locate_plain_compare(
    const key_type &key, IterType iter) const {
  for (;;) {
...
template <typename P> template <typename IterType>
inline bpair<IterType, int> btree<P>::internal_locate_compare_to(
    const key_type &key, IterType iter) const {
  for (;;) {
...
IterType btree<P>::internal_find_unique(
    const key_type &key, IterType iter) const {
  if (iter.node) {
    bpair<IterType, int> res = internal_locate(key, iter);
    if (res.second == kExactMatch) {
      return res.first;
    }
    if (!res.second) {
...
#endif  // BENTLEY_UTIL_BTREE_BTREE_H__
