...
struct DateTime
    ...
    public:
        ...
        BENTLEYDLL_EXPORT static DateTime FromJulianDay(uint64_t julianDayMsec, Info const& targetInfo);
        static BentleyStatus FromJulianDay(DateTime& dt, uint64_t julianDayMsec, Info const& targetInfo) {
            dt = FromJulianDay(julianDayMsec, targetInfo);
            return dt.IsValid() ? SUCCESS : ERROR;
        }
        ...
        //! @return SUCCESS if computation was successful. ERROR in case of errors, e.g.
        //!          if computation of local time zone offset failed.
        BENTLEYDLL_EXPORT static DateTime FromJulianDay(double julianDay, Info const& targetInfo);
        static BentleyStatus FromJulianDay(DateTime& dt, double julianDay, Info const& targetInfo) {
            dt = FromJulianDay(julianDay, targetInfo);
            return dt.IsValid() ? SUCCESS : ERROR;
        }
        ...
        BENTLEYDLL_EXPORT static DateTime FromUnixMilliseconds(uint64_t unixMilliseconds);
        static BentleyStatus FromUnixMilliseconds(DateTime& dt, uint64_t unixMilliseconds) {
            dt = FromUnixMilliseconds(unixMilliseconds);
            return dt.IsValid() ? SUCCESS: ERROR;
        }
        ...
        BENTLEYDLL_EXPORT DateTime ToUtc() const;
        BentleyStatus ToUtc(DateTime& out) const {
            out = ToUtc();
            return out.IsValid() ? SUCCESS : ERROR;
        }

        //! Converts this UTC DateTime to local time
        BENTLEYDLL_EXPORT DateTime ToLocalTime() const;
        BentleyStatus ToLocalTime(DateTime& localTime) const {
            localTime = ToLocalTime();
            return localTime.IsValid() ? SUCCESS : ERROR;
        }
        ...
        BENTLEYDLL_EXPORT Utf8String ToTimestampString() const;
        ...
        BENTLEYDLL_EXPORT static DateTime FromString(Utf8CP dateTimeIso8601);
        static BentleyStatus FromString(DateTime&dt, Utf8CP dateTimeIso8601) {
            dt =  FromString(dateTimeIso8601);
            return dt.IsValid() ? SUCCESS : ERROR;
        }
        static DateTime FromString(Utf8StringCR dateTimeIso8601) { return FromString(dateTimeIso8601.c_str()); }
