/*--------------------------------------------------------------------------------------+
|
|     $Source: PublicAPI/Bentley/bpool.h $
|
...
public:
  ...
  void free (void *const chunk)
  {
     BBOOST_ASSERT(used_list.count(chunk) == 1);
     BBOOST_ASSERT(free_list.count(chunk) == 0);
