/*--------------------------------------------------------------------------------------+
|
|     $Source: PublicAPI/Bentley/stdcxx/rw/_defs.h $
|
...
#  include <Bentley/stdcxx/rw/_config.h>
...
#  define _RWSTD_MINIMUM_STRING_CAPACITY   8U
#endif
...
#  define _RWSTD_NEW_CAPACITY(ignore, what, from)   \
      BC__RW::__rw_new_capacity (from, what)
#else
#  define _RWSTD_NEW_CAPACITY(T, what, from)   \
      BC__RW::__rw_new_capacity<T>(from, what)
#endif   // HP aCC
...
#  define BC__USING(ignore)              typedef void __rw_unused_type
...
#  ifdef __TURBOC__
#    define _RWSTD_SPECIALIZED_FRIEND(name)   (std::name)
...
#  define _RWSTD_ATOMIC_SWAP(x, y, mutex)                            \
          BC__RW::__rw_atomic_exchange  (x, y, mutex)
...
#    define _RWSTD_ATOMIC_IO_SWAP(x, y, mutex)                       \
            ((this->flags () & _RWSTD_IOS_NOLOCK) ?                  \
                BC__RW::__rw_ordinary_exchange  (x, y)                  \
             :  BC__RW::__rw_atomic_exchange  (x, y, mutex))
...
#    define _RWSTD_STRING_ATOMIC_PREINCREMENT(x, mutex)              \
            BC__RW::__rw_string_atomic_preincrement (x, mutex)

#    define _RWSTD_STRING_ATOMIC_PREDECREMENT(x, mutex)              \
            BC__RW::__rw_string_atomic_predecrement (x, mutex)

#    define _RWSTD_STRING_ATOMIC_SWAP(x, y, mutex)                   \
            BC__RW::__rw_string_atomic_exchange (x, y, mutex)
...
#error We should compile for thread-safety
...
#  define _RWSTD_ATOMIC_SWAP(x, y, ignore)  \
          BC__RW::__rw_atomic_exchange ((x), (y), false)
...
#  define BC__TRY           try
#  define BC__CATCH(ex)     catch (ex)
#  define BC__THROW(ex)     throw ex
#  define BC__RETHROW       throw
#else   // if defined (_RWSTD_NO_EXCEPTIONS)
#  define BC__TRY
#  define BC__CATCH(ignore)   while (0)
#  define BC__THROW(ignore)   ((void)0)
#  define BC__RETHROW         ((void)0)
...
#  define _RWSTD_REBIND(from, to) \
          std::allocator_interface < from, to >
...
#  define _STD_HAS_FACET(type, arg)    std::has_facet < type >(arg)
#  define _HAS_FACET(type, arg)        has_facet < type >(arg)

#  define _STD_USE_FACET(type, arg)    std::use_facet < type >(arg)
...
#  define _STD_HAS_FACET(type, arg)    std::has_facet (arg, (type*)(0))
#  define _HAS_FACET(type, arg)        has_facet (arg, (type*)(0))

#  define _STD_USE_FACET(type, arg)    std::use_facet (arg, (type*)(0))
...
// working around g++ 2.95.2 -fno-honor-std inability to use std::use_facet
   ...
   // use_facet is introduced into namespace BC__RW via a using declaration
...
#    define _RWSTD_USE_FACET(type, arg)   std::use_facet< type >(arg)
#  else
#    define _RWSTD_USE_FACET(type, arg)   use_facet< type >(arg)
#  endif
#else   // if defined (_RWSTD_NO_TEMPLATE_ON_RETURN_TYPE)
#  ifndef _RWSTD_NO_HONOR_STD
#    define _RWSTD_USE_FACET(type, arg)   std::use_facet(arg, (type*)(0))
#  else
...
#  define _RWSTD_VALUE_TYPE(iterT) \
       (typename std::iterator_traits< iterT >::value_type*)0
#  define _RWSTD_DIFFERENCE_TYPE(iterT)   \
       (typename std::iterator_traits< iterT >::difference_type*)0
...
#    define _RWSTD_ITERATOR_CATEGORY(iterT, ignore) \
         (const typename std::iterator_traits< iterT >::iterator_category&) \
...
#  define _RWSTD_REVERSE_ITERATOR(iterT, ign1, ign2, ign3) \
       std::reverse_iterator<iterT>

#else   // if defined (_RWSTD_NO_CLASS_PARTIAL_SPEC)

#  define _RWSTD_VALUE_TYPE(iterT) bc___value_type ((iterT*)0)
   // assume default ctor is defined and accessible
#  define _RWSTD_DIFFERENCE_TYPE(iterT)   \
       bc___distance_type (iterT ())
...
#  define _RWSTD_REVERSE_ITERATOR(iterT, Ref, Ptr, Tag)         \
       std::reverse_iterator<iterT, std::Tag, value_type,     \
...
#    define _RWSTD_ASSERT(expr)                                            \
     ((expr) ? (void)0 : BC__RW::BC__RW_ASSERT_FAIL (#expr, __FILE__, __LINE__, \
                                                __PRETTY_FUNCTION__))
...
#    define _RWSTD_ASSERT(expr)   \
     ((expr) ? (void)0 : BC__RW::BC__RW_ASSERT_FAIL (#expr, __FILE__, __LINE__, 0))
#  endif
...
#   define _RWSTD_ASSERT(expr) BeAssert(expr)
#endif   // _RWSTDDEBUG
...
// *** BENTLEY_CHANGE
#  define _RWSTD_REQUIRES(pred, EXPCLASS, args)   (pred) ? (void)0 : throw EXPCLASS (#args)
//#  define _RWSTD_REQUIRES(pred, args)   (pred) ? (void)0 : BC__RW::__rw_throw args
#else
   // working around a DEC cxx bug
#  define _RWSTD_REQUIRES(pred, args)   if (pred) ; else BC__RW::__rw_throw args
#endif
...
#  define BC__THROWS(type_id_list)   throw type_id_list
#else   // if _RWSTD_NO_EXCEPTIONS || _RWSTD_NO_EXCEPTION_SPECIFICATION
#  define BC__THROWS(ignore)         /* empty */
...
#  define BC__NEW_THROWS(args)     BC__THROWS (args)
#else
#  define BC__NEW_THROWS(ignore)   /* empty */
...
#  define _RWSTD_DECLARE_NOTHROW  BC__THROWS(())
#  define _RWSTD_DEFINE_NOTHROW   BC__THROWS(())
#endif   // _RWSTD_ATTR_NORETURN
...
#define _RWSTD_COMPILE_ASSERT(const_expr) \
        ((void)BC__RW::__rw_compile_assert<(const_expr)>::_C_ok)

// called for failed assertions
void _RWSTD_EXPORT
BC__RW_ASSERT_FAIL (const char*, const char*, int, const char*)
...
#  pragma does_not_return (BC__RW_ASSERT_FAIL)
#endif   // __SUNPRO_CC
...
#  define BC__DISTANCE(first, last, ignore) std::distance (first, last)
#else
// working around a SunPro 5.{3,4} bug (see PR #25958)
#  define BC__DISTANCE(first, last, type) BC__RW::__rw_distance (first, last, type())
#endif   // _RWSTD_NO_CLASS_PARTIAL_SPEC
...
#  define _RWSTD_ASSERT_RANGE(first, last) \
      _RWSTD_ASSERT (BC__RW::__rw_valid_range (first, last))
#  define _RWSTD_ASSERT_IN_RANGE(it, first, last) \
      _RWSTD_ASSERT (BC__RW::__rw_in_range (it, first, last))
#  define _RWSTD_ASSERT_DEREF(it) \
      _RWSTD_ASSERT (BC__RW::__rw_dereferenceable (it))
#else
...
#if defined(SNI)
#  define _RWSTD_USING_SNI(name) BC__USING(__SNI::name)
...
#  define _UNCAUGHT_EXCEPTION()   std::uncaught_exception()
...
#      define _RWSTD_DEFINE_TEMPLATE_FIRST(ignore)   0
#      define _RWSTD_DEFINE_TEMPLATE_LAST(ignore)    0
#    else   // if defined (BC__RWSTD_LIB_SRC)
#      define _RWSTD_DEFINE_TEMPLATE(name)   _RWSTD_INSTANTIATE ## name
#      ifndef _RWSTD_NO_EXPLICIT_INSTANTIATION_BEFORE_DEFINITION
#        define _RWSTD_DEFINE_TEMPLATE_FIRST(ignore) 0
#        define _RWSTD_DEFINE_TEMPLATE_LAST(name)    _RWSTD_INSTANTIATE ## name
...
#define _RWSTD_ISTREAMBUF_ITER(T)   \
    std::istreambuf_iterator<T, std::char_traits<T > >

#define _RWSTD_OSTREAMBUF_ITER(T)   \
    std::ostreambuf_iterator<T, std::char_traits<T > >
