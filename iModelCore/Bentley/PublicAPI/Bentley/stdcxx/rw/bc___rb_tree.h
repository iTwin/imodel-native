...
// BENTLEY_CHANGE
// We modified the _C_child member variable to make writing a Visual Studio visualizer easier.
// It's tricky, but we shrunk the array to [1] and put a like member next to it.
// When you crank up warnings in clang, it gets upset, so supress this warning here due to our trick.
...
struct  bc___rw_rb_tree_node
{
    ...
    typedef _RWSTD_REBIND (allocator_type,  bc___rw_rb_tree_node) _C_node_alloc_t;
    ...
    _C_link_t  _C_parent;
    // BENTLEY_CHANGE
    // Tthe array of 2 items does not work with Visual Studio visualizers for bmap, bset.  It needs left and right pointers.
    // _C_link_t  _C_child [2];   // left (0) and right (1) children
    union
        {
        _C_link_t _C_child[1];   // left (0) and right (1) children
        _C_link_t _C_child_left;
        };
    _C_link_t _C_child_right;
    // BENTLEY_CHANGE end
    _Val       _C_value;
...
public:
    ...
    typedef bc___rw_tree_iter<_TypeT, _DiffT, value_type*, value_type&, _C_node_t>
    _C_iterator;

    _C_link_t _C_node;

    bc___rw_tree_iter () { /* empty */ }

    // no copy ctor other than the one below is defined
    // will use a compiler generated one if bc___rw_tree_iter != _C_iterator
    bc___rw_tree_iter (const _C_iterator &__rhs)
        : _C_node (__rhs._C_node) { }

    template <class _Ptr, class _Ref>
    bc___rw_tree_iter (const bc___rw_tree_iter<_TypeT, _DiffT, _Ptr, _Ref, _Node>&
                    __rhs)
        : _C_node (__rhs._C_node) { }

    bc___rw_tree_iter (_C_link_t __lnk)
        : _C_node (__lnk) {}        

#ifdef SNI
    difference_type operator- (const bc___rw_tree_iter&) const {
        return 0;
    }
#endif
    
    bc___rw_tree_iter& operator++ () {
    ...
    bc___rw_tree_iter operator++ (int) {
        bc___rw_tree_iter __tmp (*this);
        return ++*this, __tmp;
    }
...
#define _RWSTD_TREE_ITER(n) \
        bc___rw_tree_iter <_TypeT, _DiffT, _Ptr##n, _Ref##n, _Node>
...
class bc___rb_tree : private _Alloc
{
private:
    
    typedef  bc___rw_rb_tree_node<_Alloc,_Val,_Key,_KeyOf> _C_node_t; 
...
private:
    ...
    typedef bc___rw_tree_iter<value_type, difference_type, pointer,
                              reference, _C_node_t>        _C_tree_iter;

    typedef bc___rw_tree_iter<value_type, difference_type, const_pointer,
...
public:
    ...
    typedef __rw_debug_iter <bc___rb_tree, _C_tree_iter, _C_tree_iter>
    iterator;

    typedef __rw_debug_iter <bc___rb_tree, _C_tree_citer, _C_tree_iter>
    const_iterator;
...
private:
    ...
    // allow _C_node_buf access to bc___rb_tree's private type(s)
...
public:
    ...
    bc___rb_tree (const key_compare& = key_compare (),
               const allocator_type& = allocator_type ());

    template<class _InputIter>
    bc___rb_tree (_InputIter __first, _InputIter __last,
    ...
    bc___rb_tree (const bc___rb_tree&);

    ~bc___rb_tree ();

    bc___rb_tree& operator= (const bc___rb_tree&);
    ...
    void swap (bc___rb_tree &__rhs) {
      if (get_allocator () == __rhs.get_allocator ()) {
          std::swap (_C_buf_list, __rhs._C_buf_list);
          std::swap (_C_free_list, __rhs._C_free_list);
          std::swap (_C_next_avail, __rhs._C_next_avail);
          std::swap (_C_last, __rhs._C_last);
          std::swap (_C_end, __rhs._C_end);
          std::swap (_C_size, __rhs._C_size);
          std::swap (_C_cmp, __rhs._C_cmp);
      }
      else {
          bc___rb_tree __tmp = *this;
          *this = __rhs;
          __rhs = __tmp;
      } 
    }

    void _C_insert (const value_type&, BENTLEY_BSTDCXX::bpair<iterator, bool>&, bool);

    BENTLEY_BSTDCXX::bpair<iterator, bool>
    insert (const value_type &__val, bool __dup) {
        BENTLEY_BSTDCXX::bpair<iterator, bool> __ret;
...
// MSVC 6.0 thinks S<const T*> is the same as S<T*>...
    ...
    const_iterator find (const key_type& __key) const {
        return _RWSTD_CONST_CAST (bc___rb_tree*, this)->find (__key);
    }
    ...
    const_iterator lower_bound (const key_type& __key) const {
        return _RWSTD_CONST_CAST (bc___rb_tree*, this)->lower_bound (__key);
    }

    iterator upper_bound (const key_type&);

    const_iterator upper_bound (const key_type& __key) const {
        return _RWSTD_CONST_CAST (bc___rb_tree*, this)->upper_bound (__key);
    }

    BENTLEY_BSTDCXX::bpair<iterator, iterator> equal_range (const key_type&);

    BENTLEY_BSTDCXX::bpair<const_iterator, const_iterator>
    equal_range (const key_type& __key) const {
        BENTLEY_BSTDCXX::bpair<iterator, iterator> __tmp =
            _RWSTD_CONST_CAST (bc___rb_tree*, this)->equal_range (__key);
        return BENTLEY_BSTDCXX::bpair<const_iterator, const_iterator>
...
template <class _Key, class _Val, class _KeyOf, class _Comp, class _Alloc>
inline bool
operator== (const bc___rb_tree<_Key, _Val, _KeyOf, _Comp, _Alloc>& __lhs,
            const bc___rb_tree<_Key, _Val, _KeyOf, _Comp, _Alloc>& __rhs)
{
    return    __lhs.size () == __rhs.size ()
           && std::equal (__lhs.begin (), __lhs.end (), __rhs.begin ());
}
...
template <class _Key, class _Val, class _KeyOf, class _Comp, class _Alloc>
inline void bc___rb_tree<_Key, _Val, _KeyOf, _Comp, _Alloc>::
...
template <class _Key, class _Val, class _KeyOf, class _Comp, class _Alloc>
inline typename bc___rb_tree<_Key, _Val, _KeyOf, _Comp, _Alloc>::size_type 
bc___rb_tree<_Key, _Val, _KeyOf, _Comp, _Alloc>::
count (const _Key& __k) const
{
    BENTLEY_BSTDCXX::bpair<const_iterator, const_iterator> __p = equal_range (__k);
    size_type __n = BC__DISTANCE (__p.first, __p.second, size_type);
    return __n;
}


#define _RWSTD_RB_TREE_ITER \
        typename bc___rb_tree<_Key, _Val, _KeyOf, _Comp, _Alloc>::iterator
                                               
template <class _Key, class _Val, class _KeyOf, class _Comp, class _Alloc>
inline BENTLEY_BSTDCXX::bpair<_RWSTD_RB_TREE_ITER , _RWSTD_RB_TREE_ITER >
bc___rb_tree<_Key, _Val, _KeyOf, _Comp, _Alloc>::
equal_range (const _Key& __k)
{
    return BENTLEY_BSTDCXX::bpair<iterator, iterator>(lower_bound (__k), upper_bound(__k));
}
