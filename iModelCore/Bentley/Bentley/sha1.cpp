...
void SHA1::Add(const void* data, size_t numBytes)
{
  ...
  BeAssert(m_bufferSize <= BlockSize);
  ...
  BeAssert((m_bufferSize + numBytes) < BlockSize);

  // keep remaining bytes in buffer
  while (numBytes > 0)
  {
    if (m_bufferSize >= BlockSize) // enable static anlyzer to see that there is no danger of buffer overrun
      {
      BeAssert(false);
      return;
      }
