...
PUSH_REVIEWED_STATIC_ANALYSIS_WARNING(28159)
...
Utf8String BeSystemInfo::GetDeviceId ()
    ...
    if (BeFileStatus::Success != status)
        {
        BentleyApi::NativeLogging::CategoryLogger("BeAssert").errorv ("Opening '%s' failed with error code %d", fname.c_str (), file.GetLastError ());
        return "";
        }
    ...
    if (BeFileStatus::Success != status)
        {
        BentleyApi::NativeLogging::CategoryLogger("BeAssert").errorv ("Reading '%s' failed with error code %d", fname.c_str (), file.GetLastError ());
        file.Close();
        return "";
        }
