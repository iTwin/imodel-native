/*--------------------------------------------------------------------------------------+
|
|     $Source: PublicAPI/Licensing/Connect/ConnectSignInManager.h $
|
...
#include "IConnectSignInManager.h"
#include "IImsClient.h"
#include "SamlToken.h"
#include "../Utils/UrlProvider.h"
...
struct ConnectSignInManager : IConnectSignInManager, std::enable_shared_from_this<ConnectSignInManager>
    ...
    public:
        ...
        LICENSING_EXPORT static ConnectSignInManagerPtr Create
            (
        ...
        LICENSING_EXPORT void Configure(Configuration config);
        ...
        LICENSING_EXPORT AsyncTaskPtr<SignInResult> SignInWithToken(SamlTokenPtr token, Utf8StringCR rpUri = nullptr);
        ...
        LICENSING_EXPORT AsyncTaskPtr<SignInResult> SignInWithCredentials(CredentialsCR credentials);
        //! Store sign-in information on disk so user would stay signed-in. Call after successful sign-in.
        LICENSING_EXPORT void FinalizeSignIn();

        //! Get user information stored in token
        LICENSING_EXPORT static UserInfo ReadUserInfo(SamlTokenCR token);

        //! Check if Connection Client is installed
        //! Should be called first to make sure the API will work correctly
        /*LICENSING_EXPORT bool IsConnectionClientInstalled();

        //! Uses Connection Client API to get a token to sign in with
        LICENSING_EXPORT AsyncTaskPtr<ConnectionClientTokenResult> GetConnectionClientToken(Utf8StringCR rpUri);

        //! Uses Connection Client API to listen to events fired by Connection Client
        LICENSING_EXPORT void StartConnectionClientListener();

        //! Check if Connection Client listener is started
        LICENSING_EXPORT bool IsConnectionClientListenerStarted();
		*/
        //! Get token's connect environment from local state
        LICENSING_EXPORT UrlProvider::Environment ReadConnectEnvironment();
    };
