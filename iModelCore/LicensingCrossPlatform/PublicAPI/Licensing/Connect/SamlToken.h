/*--------------------------------------------------------------------------------------+
|
|     $Source: PublicAPI/Licensing/Connect/SamlToken.h $
|
...
struct SamlToken : public ISecurityToken
    ...
    public:
        ...
        LICENSING_EXPORT SamlToken();
        //! Construct token from xml string
        LICENSING_EXPORT SamlToken(Utf8String token);
        //! Copy constructor
        LICENSING_EXPORT SamlToken(const SamlToken& other);
        //! Assignment operator
        LICENSING_EXPORT SamlToken& operator=(const SamlToken& other);
        //! Check whenver given token is empty
        LICENSING_EXPORT bool IsEmpty() const;
        //! Check whenver given token is valid xml and supported
        LICENSING_EXPORT bool IsSupported() const override;
        ...
        LICENSING_EXPORT bool IsValidAt(DateTimeCR dateTimeUtc) const;
        ...
        LICENSING_EXPORT bool IsValidNow(uint32_t offsetMinutes) const;
        //! Get token lifetime in minutes 
        LICENSING_EXPORT uint32_t GetLifetime() const;
        //! Get token attributes
        LICENSING_EXPORT BentleyStatus GetAttributes(bmap<Utf8String, Utf8String>& atributtesOut) const;
        //! Get base 64 encoded certificate
        LICENSING_EXPORT BentleyStatus GetX509Certificate(Utf8StringR certOut) const;
        //! Create string for authorization header
        LICENSING_EXPORT Utf8String ToAuthorizationString() const override;
        //! Return original token representation
        LICENSING_EXPORT Utf8StringCR AsString() const override;
        //! Compare contents of two tokens for equality
        LICENSING_EXPORT bool operator==(const ISecurityToken& other) const override;
    };
