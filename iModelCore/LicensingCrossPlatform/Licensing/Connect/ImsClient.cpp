/*--------------------------------------------------------------------------------------+
|
|     $Source: Licensing/Connect/ImsClient.cpp $
|
...
+---------------+---------------+---------------+---------------+---------------+------*/
void ImsClient::InitializeShared(ApplicationInfoPtr info, IHttpHandlerPtr httpHandler)
    {
...
* @bsimethod                                                    Vincas.Razma    02/2015
+---------------+---------------+---------------+---------------+---------------+------*/
ImsClient::ImsClient(ApplicationInfoPtr clientInfo, IHttpHandlerPtr httpHandler) :
...
* @bsimethod                                                    Vincas.Razma    02/2015
+---------------+---------------+---------------+---------------+---------------+------*/
ImsClientPtr ImsClient::Create(ApplicationInfoPtr clientInfo, IHttpHandlerPtr httpHandler)
    {
...
Utf8StringCR pastUsername
    ...
    Request request = client.CreatePostRequest(stsUrl);
    ...
    return request.PerformAsync()->Then<SamlTokenResult>([rpUri, stsUrl, pastUsername] (ResponseCR response)
        {
...
+---------------+---------------+---------------+---------------+---------------+------*/
bool ImsClient::IsLoginRedirect(ResponseCR response)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
Utf8String ImsClient::GetFederatedSignInUrl(ApplicationInfoCR info, Utf8StringCR windowsDomainName)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
Utf8String ImsClient::GetRelyingPartyUri(ApplicationInfoCR info)
    {
...
* @bsimethod                                                    Ron.Stewart     01/2016
+---------------+---------------+---------------+---------------+---------------+------*/
Utf8String ImsClient::GetClientRelyingPartyUriForWtrealm(ApplicationInfoCR info)
    {
...
Utf8String ImsClient::GetA2PUrl(Utf8String url, SamlTokenPtr token, Utf8String tokenGuid)
    ...
    HttpClient client(m_clientInfo, UrlProvider::GetSecurityConfigurator(m_httpHandler));
    Request request = client.CreatePostRequest(UrlProvider::Urls::ImsActiveSTSHelperService.Get() + "/json/RegisterToken");
    request.GetHeaders().SetAuthorization(authorization);
    request.GetHeaders().SetContentType(REQUESTHEADER_ContentType_ApplicationJson);
    request.SetRequestBody(HttpStringBody::Create(Json::FastWriter::ToString(params)));
    request.SetValidateCertificate(true); // Ensure secure connection when passing authentication information
