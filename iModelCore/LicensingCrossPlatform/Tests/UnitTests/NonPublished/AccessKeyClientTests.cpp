/*--------------------------------------------------------------------------------------+
|
|     $Source: Tests/UnitTests/NonPublished/AccessKeyClientTests.cpp $
|
...
#define TEST_MACHINE1           "first"
#define TEST_MACHINE2           "second"
...
AccessKeyClientImplPtr CreateWithAgnosticKeyTestClient(IPolicyProviderPtr policyProvider, IUlasProviderPtr ulasProvider, ILicensingDbPtr licensingDb, Utf8StringCR productId = TEST_PRODUCT_ID, Utf8StringCR ultimateId = TEST_ULTIMATE_ID, Utf8StringCR machinename = TEST_MACHINE1 )
{
	auto appInfo = std::make_shared<ApplicationInfo>(BeVersion(1, 0), machinename, productId);
