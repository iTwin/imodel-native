...
TEST_F(PolicyProviderTests, GetPolicy_Success)
    ...
    Utf8String certificateUrl = mockUrl + "/PolicyTokenSigningCertificate";
    ...
    GetMockHttpHandler().ForRequest(1, [=](Http::RequestCR request)
        {
        EXPECT_EQ(policyUrl, request.GetUrl());
        return MockHttpHandler::StubHttpResponse(policyToken); // return response with policy string
        });
    GetMockHttpHandler().ForRequest(2, [=](Http::RequestCR request)
        {
        EXPECT_EQ(certificateUrl, request.GetUrl());
        return MockHttpHandler::StubHttpResponse(certToken); // return response with cert string
        });
...
TEST_F(PolicyProviderTests, GetPolicy_GetPolicyFailure)
    ...
    GetMockHttpHandler().ForRequest(1, [=](Http::RequestCR request)
        {
        EXPECT_EQ(policyUrl, request.GetUrl());
        return MockHttpHandler::StubHttpFailureResponse(); // return failed response
        });
    GetMockHttpHandler().ForRequest(2, [=](Http::RequestCR request)
        {
        EXPECT_EQ(certificateUrl, request.GetUrl());
