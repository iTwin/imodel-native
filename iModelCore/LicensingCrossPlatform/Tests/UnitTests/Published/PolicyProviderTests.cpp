...
TEST_F(PolicyProviderTests, GetPolicyWithKey_Success)
    ...
    GetMockHttpHandler().ForRequest(1, [=](Http::RequestCR request)
		...
		if (request.GetUrl() == policyUrl)
		{
			...
			Json::Value request = Json::Reader::DoParse(requeststr);
			EXPECT_STREQ(TEST_DEVICE_ID, request["MachineName"].asCString());
			EXPECT_STREQ(TEST_DEVICE_SID, request["MachineSID"].asCString());
			return MockHttpHandler::StubHttpResponse(policyToken);
		}
        else if (request.GetUrl() == certUrl)
            return MockHttpHandler::StubHttpResponse(certToken);
...
TEST_F(PolicyProviderTests, GetPolicyWithKey_SuccessAgnostic)
{
	...
	GetMockHttpHandler().ForRequest(1, [=](Http::RequestCR request)
	{
		if (request.GetUrl() == policyUrl)
		{
			auto requeststr = request.GetRequestBody()->AsString();
			Json::Value request = Json::Reader::DoParse(requeststr);
			EXPECT_STREQ(TEST_ULT_ID, request["MachineName"].asCString());
			EXPECT_STREQ(TEST_ULT_ID, request["MachineSID"].asCString());
			return MockHttpHandler::StubHttpResponse(policyToken);
		}
		else if (request.GetUrl() == certUrl)
			return MockHttpHandler::StubHttpResponse(certToken);
	...
	// assert that we return a well formed policy
	const auto result = GetPolicyProvider().GetPolicyWithKey(accessKey, TEST_ULT_ID).get();
}
