...
folly::Future<BentleyStatus> UlasProviderMock::RealtimeTrackUsage(Utf8StringCR accessToken, int productId, Utf8StringCR featureString, Utf8StringCR deviceId, BeVersionCR version, Utf8StringCR projectId, UsageType usageType, Utf8StringCR correlationId)
    {
    m_realtimeTrackUsageCalls++;
    return folly::makeFuture(m_mockedRealtimeTrackUsage);
    }

folly::Future<BentleyStatus> UlasProviderMock::RealtimeMarkFeature(Utf8StringCR accessToken, FeatureEvent featureEvent, int productId, Utf8StringCR featureString, Utf8StringCR deviceId, UsageType usageType, Utf8StringCR correlationId)
    {
