...
TEST_F(SaasClientTests, SaasClientTrackUsage_Success)
    ...
    EXPECT_SUCCESS(client->TrackUsage(accessToken, version, projectId, "", UsageType::Production, "").get());
...
TEST_F(SaasClientTests, SaasClientMarkFeatureNoDataNoProject_Success)
    ...
    EXPECT_SUCCESS(client->MarkFeature(accessToken, featureEvent, "", UsageType::Production, "").get());
