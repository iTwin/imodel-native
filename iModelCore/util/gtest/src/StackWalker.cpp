...
#pragma warning(disable:4740) // ASM code suppresses optimization
...
#if defined (WIP_GTEST_STACKWALKER_DETAILS)
...
#endif // ifdef (WIP_GTEST_STACKWALKER_DETAILS)  
  }
  OnOutput(buffer);
}
...
void StackWalker::SetPrintFunction (pfOnOutCallback function)
{
...
{
  if (NULL != m_printFunction)
    (*m_printFunction)(buffer);
  else
    printf ("%s", buffer);
}
