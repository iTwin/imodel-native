...
TEST_F(ProfileTestFixture, CheckECDbProfileVersion)
    ...
    std::vector<std::pair<ProfileVersion, ProfileState>> expectedProfileStates {
            ...
            {ProfileVersion(4,0,0,5), ProfileState::Older(ProfileState::CanOpen::Readwrite, true)},
            {ProfileVersion(4,0,0,6), ProfileState::UpToDate()},
    ...
    std::vector<ProfileVersion> expectedOlderReadWriteAndUpgradable = {ProfileVersion(4,0,0,0), ProfileVersion(4,0,0,1), ProfileVersion(4,0,0,2), ProfileVersion(4,0,0,3), ProfileVersion(4,0,0,4), ProfileVersion(4,0,0,5)};
    ProfileVersion expectedUpToDate = ProfileVersion(4,0,0,6);
    std::vector<ProfileVersion> expectedNewerReadWrite = {ProfileVersion(4,0,0,7), ProfileVersion(4,0,1,0), ProfileVersion(4,0,1,3), ProfileVersion(4,0,2,0), ProfileVersion(4,0,10,99)};
...
TEST_F(ProfileTestFixture, TestFeatureTableWithProfileVersion4_0_0_6_NewDb)
    {
    ASSERT_EQ(DbResult::BE_SQLITE_OK, SetupECDb("ec_feature_table.ecdb"));

    EXPECT_FALSE(m_ecdb.GetECDbProfileVersion().IsEmpty());
    EXPECT_EQ(m_ecdb.GetECDbProfileVersion(), ProfileVersion(4, 0, 0, 6));
    EXPECT_TRUE(GetHelper().TableExists("ec_Feature")) << "ECDb feature table not found in ECDb file which was newly created.";
