...
#define PROFILETABLE_SELECT_ClassHasBaseClasses                                                                                                            \
    "SELECT s.Name, c.Name, baseS.Name, baseC.Name FROM ec_ClassHasBaseClasses bc JOIN ec_Class c ON bc.ClassId=c.Id JOIN ec_Schema s ON s.Id=c.SchemaId " \
...
#define PROFILETABLE_SELECT_Property                                                                                                                                                                                   \
    "SELECT s.Name, p.Name, p.DisplayLabel,p.Description,p.IsReadonly,p.Priority,p.Ordinal,p.Kind,p.PrimitiveType,p.PrimitiveTypeMinLength,p.PrimitiveTypeMaxLength,p.PrimitiveTypeMinValue,p.PrimitiveTypeMaxValue, " \
...
#define PROFILETABLE_SELECT_RelationshipConstraint                                                                                                                                                                          \
    "SELECT relS.Name, rel.Name, rc.RelationshipEnd,rc.MultiplicityLowerLimit,rc.MultiplicityUpperLimit,rc.IsPolymorphic,rc.RoleLabel,abstractConstraintS.Name, abstractConstraint.Name FROM ec_RelationshipConstraint rc " \
...
#define PROFILETABLE_SELECT_RelationshipConstraintClass                                                                                                           \
    "SELECT relS.Name,rel.Name,rc.RelationshipEnd,constraintS.Name,constraintC.Name FROM ec_RelationshipConstraintClass rcc "                                     \
...
#define PROFILETABLE_SELECT_PropertyPath                                                                                                                                               \
    "SELECT rootPropS.Name, rootPropC.Name, rootProp.Name, pp.AccessString FROM ec_PropertyPath pp "                                                                                   \
...
#define PROFILETABLE_SELECT_PropertyMap                                                                                                                                                                                                   \
    "SELECT s.Name, c.Name, rootPropS.Name,rootPropC.Name,rootProp.Name,t.Name,col.Name FROM ec_PropertyMap pm "                                                                                                                          \
...
#define PROFILETABLE_SELECT_Table                                                                                                                                                                                                    \
    "SELECT parentT.Name, t.Name, t.Type, exclusiveRootClassS.Name, exclusiveRootClass.Name, t.UpdatableViewName FROM ec_table t "                                                                                                   \
    ...
    "ORDER BY t.Name"
#define PROFILETABLE_SELECT_Column                                                                                                                                                                               \
    "SELECT t.Name, c.Name,c.Type,c.IsVirtual,c.Ordinal,c.NotNullConstraint,c.UniqueConstraint,c.CheckConstraint,c.DefaultConstraint,c.CollationConstraint,c.OrdinalInPrimaryKey,c.ColumnKind FROM ec_Column c " \
    "JOIN ec_Table t ON t.Id=c.TableId ORDER BY t.Name, c.Name"
#define PROFILETABLE_SELECT_Index                                                                                                            \
    "SELECT i.Name, t.Name, i.IsUnique,i.AddNotNullWhereExp,i.IsAutoGenerated,s.Name,c.Name,i.AppliesToSubclassesIfPartial FROM ec_Index i " \
    "JOIN ec_Table t ON t.Id=i.TableId JOIN ec_Class c ON c.Id=i.ClassId JOIN ec_Schema s ON s.Id=c.SchemaId ORDER BY i.Name"
#define PROFILETABLE_SELECT_IndexColumn                                 \
    "SELECT i.Name, c.Name, t.Name, ic.Ordinal FROM ec_IndexColumn ic " \
...
TEST_F(FileFormatCompatibilityTests, ProfileUpgrade) {
...

    {
        ...
        ASSERT_EQ(ECSqlStatus::Success, ecsqlStmt.Prepare(upgradedFile, "SELECT Name,VersionMajor,VersionWrite,VersionMinor FROM meta.ECSchemaDef WHERE Name IN ('CoreCustomAttributes','ECDbFileInfo','ECDbMeta','ECDbSystem', 'ECDbMap') ORDER BY Name"));
        ...
        while (BE_SQLITE_ROW == ecsqlStmt.Step()) {
            EXPECT_STREQ(testValues[rowCount].c_str(), ecsqlStmt.GetValueText(0)) << "Expected ExtendedTypeName for property: " << ecsqlStmt.GetValueText(1);
            rowCount++;
        }
        ASSERT_EQ(8, rowCount) << "Expected number of id properties in ECDbSystem schema";
        ecsqlStmt.Finalize();
...
TEST_F(FileFormatCompatibilityTests, ForwardCompatibilitySafeguards_UnknownMapStrategy) {
    ...
    ASSERT_EQ(BE_SQLITE_DONE, GetHelper().ExecuteInsertECSql(key1,
                                                             Utf8PrintfString("insert into ts.A(PropA,AToARelProp.Id,AToARelProp.RelECClassId,AToChildARelProp.Id,AToChildARelProp.RelECClassId,AToGrandChildARelProp.Id,AToGrandChildARelProp.RelECClassId,AToBRelProp.Id,AToBRelProp.RelECClassId) values ('PropA1', 0x1, %s, 0x1, %s, 0x1, %s, 0x1, %s)",
                                                                              aToAClass->GetId().ToHexStr().c_str(), aToChildAClass->GetId().ToHexStr().c_str(), aToGrandChildAClass->GetId().ToHexStr().c_str(), aToBClass->GetId().ToHexStr().c_str())
                                                                 .c_str()));
    ...
    ASSERT_EQ(BE_SQLITE_DONE, GetHelper().ExecuteInsertECSql(key2,
                                                             Utf8PrintfString("insert into ts.B(PropB,BToARelProp.Id,BToARelProp.RelECClassId,BToChildARelProp.Id,BToChildARelProp.RelECClassId,BToGrandChildARelProp.Id,BToGrandChildARelProp.RelECClassId) values ('PropB1', 0x1, %s, 0x1, %s, 0x1, %s)",
                                                                              bToAClass->GetId().ToHexStr().c_str(), bToChildAClass->GetId().ToHexStr().c_str(), bToGrandChildAClass->GetId().ToHexStr().c_str())
                                                                 .c_str()));
    ...
    for (const auto& classesToUpdate : std::vector<Utf8String>{"'A'", "'A', 'ChildA'", "'A', 'GrandChildA'", "'A', 'ChildA', 'GrandChildA'"}) {
        ...
        for (const auto& [testCaseNumber, selectStatement, sqlPrepareResult, sqlStepResult, expectedColumnNames] : TestCaseData{
                 ...
                 {2, "SELECT ECInstanceId, PropA, AToARelProp, AToChildARelProp, AToGrandChildARelProp, AToBRelProp FROM ts.A", ECSqlStatus::Success, BE_SQLITE_DONE, {"ECInstanceId", "PropA", "AToARelProp", "AToChildARelProp", "AToGrandChildARelProp", "AToBRelProp"}},
                 ...
                 {4, "SELECT ECInstanceId, PropA, PropChildA, AToARelProp, AToChildARelProp, AToGrandChildARelProp, AToBRelProp FROM ts.ChildA", ECSqlStatus::Success, BE_SQLITE_DONE, {"ECInstanceId", "PropA", "PropChildA", "AToARelProp", "AToChildARelProp", "AToGrandChildARelProp", "AToBRelProp"}},
                 ...
                 {6, "SELECT ECInstanceId, PropA, PropChildA, PropGrandChildA, AToARelProp, AToChildARelProp, AToGrandChildARelProp, AToBRelProp FROM ts.GrandChildA", ECSqlStatus::Success, BE_SQLITE_DONE, {"ECInstanceId", "PropA", "PropChildA", "PropGrandChildA", "AToARelProp", "AToChildARelProp", "AToGrandChildARelProp", "AToBRelProp"}},
    ...
    for (const auto& classesToUpdate : std::vector<Utf8String>{"'ChildA'", "'ChildA', 'GrandChildA'"}) {
        ...
        for (const auto& [testCaseNumber, selectStatement, sqlPrepareResult, sqlStepResult, expectedColumnNames] : TestCaseData{
                 ...
                 {26, "SELECT ECInstanceId, PropA, PropChildA, AToARelProp, AToChildARelProp, AToGrandChildARelProp, AToBRelProp FROM ts.ChildA", ECSqlStatus::Success, BE_SQLITE_DONE, {"ECInstanceId", "PropA", "PropChildA", "AToARelProp", "AToChildARelProp", "AToGrandChildARelProp", "AToBRelProp"}},
                 ...
                 {28, "SELECT ECInstanceId, PropA, PropChildA, PropGrandChildA, AToARelProp, AToChildARelProp, AToGrandChildARelProp, AToBRelProp FROM ts.GrandChildA", ECSqlStatus::Success, BE_SQLITE_DONE, {"ECInstanceId", "PropA", "PropChildA", "PropGrandChildA", "AToARelProp", "AToChildARelProp", "AToGrandChildARelProp", "AToBRelProp"}},
    ...
    for (const auto& [testCaseNumber, selectStatement, sqlPrepareResult, sqlStepResult, expectedColumnNames] : TestCaseData{
             ...
             {49, "SELECT ECInstanceId, PropA, PropChildA, PropGrandChildA, AToARelProp, AToChildARelProp, AToGrandChildARelProp, AToBRelProp FROM ts.GrandChildA", ECSqlStatus::Success, BE_SQLITE_DONE, {"ECInstanceId", "PropA", "PropChildA", "PropGrandChildA", "AToARelProp", "AToChildARelProp", "AToGrandChildARelProp", "AToBRelProp"}},
...
TEST_F(FileFormatCompatibilityTests, EC31KOQs) {
    ...
    ASSERT_EQ(ERROR, TestHelper::RunSchemaImport(SchemaItem(
        ...
        << "EC3.1 cannot handle ECnamed units";
