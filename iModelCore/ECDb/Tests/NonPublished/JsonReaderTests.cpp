...
TEST_F(JsonECSqlSelectAdapterTests, ReservedWordsCollisions) {
    ...
    ASSERT_EQ(BE_SQLITE_DONE, GetHelper().ExecuteInsertECSql(relKey, Utf8PrintfString("INSERT INTO ts.Rel(SourceECInstanceId, TargetECInstanceId, sourceClassName, targetClassName) VALUES(%s,%s,'source','target')",
                                                                                      fooKey.GetInstanceId().ToString().c_str(), fooKey.GetInstanceId().ToString().c_str())
                                                                         .c_str()));
...
TEST_F(JsonECSqlSelectAdapterTests, DataTypes) {
    ...
    bvector<Byte> blobVal = {112, 33, 1, 0, 22, 44, 88, 88, 88, 255, 0, 100};
    ...
    ECInstanceKey key;
    {
        ...
        ASSERT_EQ(ECSqlStatus::Success, stmt.BindBlob(2, blobVal.data(), (int)blobVal.size(), IECSqlBinder::MakeCopy::No));
...
TEST_F(JsonECSqlSelectAdapterTests, AbbreviateBlobs) {
    ...
    bvector<Byte> fullBlobVal = {112, 33, 1, 0, 22, 44, 88, 88, 88, 255, 0, 100};
    bvector<Byte> singleBlobVal = {56};
    bvector<Byte> emptyBlobVal;

    ECInstanceKey fullKey, singleKey, emptyKey, nullKey;
    ECSqlStatement stmt;

    ASSERT_EQ(ECSqlStatus::Success, stmt.Prepare(m_ecdb, "INSERT INTO ts.Foo(Bi) VALUES(?)"));
    ASSERT_EQ(ECSqlStatus::Success, stmt.BindBlob(1, fullBlobVal.data(), (int)fullBlobVal.size(), IECSqlBinder::MakeCopy::No));
    ASSERT_EQ(BE_SQLITE_DONE, stmt.Step(fullKey));
    stmt.Reset();
    stmt.ClearBindings();

    ASSERT_EQ(ECSqlStatus::Success, stmt.BindBlob(1, singleBlobVal.data(), (int)singleBlobVal.size(), IECSqlBinder::MakeCopy::No));
    ASSERT_EQ(BE_SQLITE_DONE, stmt.Step(singleKey));
    stmt.Reset();
    stmt.ClearBindings();

    ASSERT_EQ(ECSqlStatus::Success, stmt.BindZeroBlob(1, 0));
    ASSERT_EQ(BE_SQLITE_DONE, stmt.Step(emptyKey));
    stmt.Reset();
    stmt.ClearBindings();

    ASSERT_EQ(ECSqlStatus::Success, stmt.BindNull(1));
    ASSERT_EQ(BE_SQLITE_DONE, stmt.Step(nullKey));
    stmt.Reset();
    stmt.ClearBindings();
    stmt.Finalize();

    ASSERT_EQ(ECSqlStatus::Success, stmt.Prepare(m_ecdb, "SELECT Bi FROM ts.Foo WHERE ECInstanceId=?"));
    JsonECSqlSelectAdapter adapter(stmt);
    JsonDoc actualJson;

    // Blob larger than abbreviated length
    ASSERT_EQ(ECSqlStatus::Success, stmt.BindId(1, fullKey.GetInstanceId()));
    ASSERT_EQ(BE_SQLITE_ROW, stmt.Step());
    ...
    // Blob equal to abbreviated length
    ASSERT_EQ(ECSqlStatus::Success, stmt.BindId(1, singleKey.GetInstanceId()));
    ASSERT_EQ(BE_SQLITE_ROW, stmt.Step());
    ...
    // Blob smaller than abbreviated length
    ASSERT_EQ(ECSqlStatus::Success, stmt.BindId(1, emptyKey.GetInstanceId()));
    ASSERT_EQ(BE_SQLITE_ROW, stmt.Step());
