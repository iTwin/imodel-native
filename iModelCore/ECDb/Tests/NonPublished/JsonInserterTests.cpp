...
TEST_F(JsonInserterTests, InsertNavProps) {
...

    {
        expectedJsonStr.Sprintf(R"json({ "name" : "Child 1",
                                    "parent" : { "id" : "%s", "relClassName" : "TestSchema.FkRel" } })json",
...
TEST_F(JsonInserterTests, RoundTrip_InsertThenRead) {
    ...
    std::vector<std::tuple<ECClassCP, Utf8String, ECJsonInt64Format>> dataset{
        {psaClass, Utf8PrintfString(R"json({ "I" : 123,
                                    ...
                                    "S_Array" : ["Hello", "world"],
                                    "PStructProp" : { "i" : 123 }})json",
                                    pKeyStr.c_str()),
         ECJsonInt64Format::AsNumber},
        ...
        {psaClass, Utf8PrintfString(R"json({ "I" : 123,
                                    ...
                                    "PStructProp" : { "i" : 123 }})json",
                                    pKeyStr.c_str()),
         ECJsonInt64Format::AsDecimalString},
        ...
        {psaClass, Utf8PrintfString(R"json({ "I" : 123,
                                    ...
                                    "PStructProp" : { "i" : 123 }})json",
                                    pKeyStr.c_str()),
         ECJsonInt64Format::AsHexadecimalString},

        {linkTableRelClass, Utf8PrintfString(R"json({ "sourceId" : "%s", "targetId" : "%s" }")json", pKeyStr.c_str(), pKeyStr.c_str()), ECJsonInt64Format::AsDecimalString},

        {linkTableRelClass, Utf8PrintfString(R"json({ "sourceId" : "%s", "sourceClassName" : "ECSqlTest.P", "targetId" : "%s" , "targetClassName" : "ECSqlTest.P" }")json", pKeyStr.c_str(), pKeyStr.c_str()), ECJsonInt64Format::AsDecimalString},
...
struct PrimArrayJsonInserterTests : public ECDbTestFixture {
   ...
   protected:
    PrimArrayJsonInserterTests()
        : m_testDate(DateTime::GetCurrentTimeUtc()), m_testPoint2d(DPoint2d::From(POINTXVALUE, POINTYVALUE)), m_testPoint3d(DPoint3d::From(POINTXVALUE, POINTYVALUE, POINTZVALUE)), m_testGeometry(IGeometry::Create(ICurvePrimitive::CreateLine(DSegment3d::From(0.0, 0.0, 0.0, 1.0, 1.0, 1.0)))) {
