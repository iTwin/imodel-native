...
void ECDbMetaSchemaECSqlTestFixture::AssertClassDefs(ECSchemaCR expectedSchema) {
    ...
    ASSERT_EQ(ECSqlStatus::Success, classStatement.Prepare(m_ecdb,
                                                           "SELECT c.Name, c.* FROM meta.ECSchemaDef s "
...
void ECDbMetaSchemaECSqlTestFixture::AssertEnumerationDefs(ECSchemaCR expectedSchema) {
    ...
    ASSERT_EQ(ECSqlStatus::Success, enumStatement.Prepare(m_ecdb,
                                                          "SELECT e.Name, e.* FROM meta.ECSchemaDef s "
...
void ECDbMetaSchemaECSqlTestFixture::AssertKindOfQuantityDefs(ECSchemaCR expectedSchema) {
    ...
    ASSERT_EQ(ECSqlStatus::Success, statement.Prepare(m_ecdb,
                                                      "SELECT koq.Name, koq.* FROM meta.ECSchemaDef s "
...
void ECDbMetaSchemaECSqlTestFixture::AssertUnitSystemDefs(ECSchemaCR expectedSchema) {
    ECSqlStatement statement;
    ASSERT_EQ(ECSqlStatus::Success, statement.Prepare(m_ecdb,
                                                      "SELECT us.Name, us.* FROM meta.ECSchemaDef s "
...
void ECDbMetaSchemaECSqlTestFixture::AssertPhenomenonDefs(ECSchemaCR expectedSchema) {
    ECSqlStatement statement;
    ASSERT_EQ(ECSqlStatus::Success, statement.Prepare(m_ecdb,
                                                      "SELECT ph.Name, ph.* FROM meta.ECSchemaDef s "
...
void ECDbMetaSchemaECSqlTestFixture::AssertUnitDefs(ECSchemaCR expectedSchema) {
    ECSqlStatement statement;
    ASSERT_EQ(ECSqlStatus::Success, statement.Prepare(m_ecdb,
                                                      "SELECT u.Name, u.* FROM meta.ECSchemaDef s "
...
void ECDbMetaSchemaECSqlTestFixture::AssertFormatDefs(ECSchemaCR expectedSchema) {
    ECSqlStatement statement;
    ASSERT_EQ(ECSqlStatus::Success, statement.Prepare(m_ecdb,
                                                      "SELECT f.Name, f.* FROM meta.ECSchemaDef s "
...
void ECDbMetaSchemaECSqlTestFixture::AssertPropertyCategoryDefs(ECSchemaCR expectedSchema) {
    ECSqlStatement statement;
    ASSERT_EQ(ECSqlStatus::Success, statement.Prepare(m_ecdb,
                                                      "SELECT cat.Name, cat.* FROM meta.ECSchemaDef s "
...
void ECDbMetaSchemaECSqlTestFixture::AssertPropertyDefs(ECClassCR expectedClass) {
    ...
    ASSERT_EQ(ECSqlStatus::Success, propStatement.Prepare(m_ecdb,
                                                          "SELECT p.Name, p.* FROM meta.ECPropertyDef p "
...
TEST_F(ECDbMetaSchemaECSqlTestFixture, CustomAttributes) {
    ...
    {
        ECSqlStatement stmt;
        ASSERT_EQ(ECSqlStatus::Success, stmt.Prepare(m_ecdb, "SELECT [Instance] FROM meta.CustomAttribute WHERE [Class].[Id] IS (TestSchema.CAClass) LIMIT 1"));
        ASSERT_EQ(stmt.Step(), BE_SQLITE_ROW);
        ASSERT_STREQ(stmt.GetValueText(0), "<CAClass xmlns=\"TestSchema.01.00\">\n    <CAProp>Test</CAProp>\n</CAClass>\n");
    }

    {
        ECSqlStatement stmt;
        ASSERT_EQ(ECSqlStatus::Success, stmt.Prepare(m_ecdb, "SELECT Count(*) FROM meta.CustomAttribute ca JOIN meta.ECClassDef c USING meta.CustomAttributeClassHasInstance WHERE c.ECInstanceId IS (TestSchema.CAClass)"));
        ASSERT_EQ(stmt.Step(), BE_SQLITE_ROW);
...
TEST_F(ECDbMetaSchemaECSqlTestFixture, JsonCustomAttributeSubQuery) {
    ...
    // Same as before but using polymorphic IS match for caClass
    {
        ECSqlStatement stmt;
        ASSERT_EQ(ECSqlStatus::Success, stmt.Prepare(m_ecdb, R"stmt(
            SELECT e.Name
    ...
    // Same as before but using IS ONLY match for caClass
    {
        ECSqlStatement stmt;
        ASSERT_EQ(ECSqlStatus::Success, stmt.Prepare(m_ecdb, R"stmt(
            SELECT e.Name
    ...
    // Same as above but using IS syntax
    {
        ECSqlStatement stmt;
        ASSERT_EQ(ECSqlStatus::Success, stmt.Prepare(m_ecdb, R"stmt(
            SELECT json_extract(ca.Instance, '$.ClassAlias.Alias')
