...
TEST_F(JsonTests, json_tree) {
  ...
  if("join") {
        ...
        ASSERT_STREQ(nativeSQL,  "SELECT s.key,s.value,s.type,s.atom,s.parent,s.fullkey,s.path FROM (SELECT [Id] ECInstanceId,77 ECClassId,[doc] FROM [main].[ts_json_docs]) [json_docs],json_tree([json_docs].[doc]) s WHERE s.key='gravity'");
        stmt.BindText(1, test_data, IECSqlBinder::MakeCopy::No);
...
TEST_F(JsonTests, json_each) {
  ...
  if("join") {
        ...
        auto nativeSQL = stmt.GetNativeSql();
        ASSERT_STREQ(nativeSQL,  "SELECT s.key,s.value,s.type,s.atom,s.parent,s.fullkey,s.path FROM (SELECT [Id] ECInstanceId,77 ECClassId,[doc] FROM [main].[ts_json_docs]) [json_docs],json_each([json_docs].[doc]) s WHERE s.key='gravity'");
        stmt.BindText(1, test_data, IECSqlBinder::MakeCopy::No);
        ASSERT_EQ(BE_SQLITE_ROW, stmt.Step());
