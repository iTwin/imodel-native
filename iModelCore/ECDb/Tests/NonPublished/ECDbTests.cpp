...
TEST_F(ECDbTestFixture, ElementRefersToElementsExtendedUniqueIndex)
    ...
    const auto testSchema = R"xml(
        <?xml version='1.0' encoding='utf-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.2">
            <ECSchemaReference name="BisCore" version="1.0.%d" alias="bis"/>
            ...
            <ECEntityClass typeName="ElementGroup" displayLabel="Group of Elements" >
                <BaseClass>SpecificElement</BaseClass>
    ...
    const auto importECDbVersion = R"xml(
        <ECCustomAttributes>
    ...
    auto importSchemasTestAndReset = [this](const SchemaItem& schemaForSetup, const std::vector<SchemaItem>& schemasToImport, const bool expectedToFailInsert)
        ...
        for (const auto& memberPriorityValue : { 2, 0, 1, 0 })
            {
            // Create duplicate entries with the MemberPriority being the only difference
            statement.Finalize();

            statement.Prepare(m_ecdb, "INSERT INTO ts.GroupOfElements(SourceECClassId, SourceECInstanceId, TargetECClassId, TargetECInstanceId, MemberPriority) VALUES(?,?,?,?,?)");
            ...
            if (memberPriorityValue == 0)
                statement.BindNull(5);
            else
                statement.BindInt(5, memberPriorityValue);

            ECInstanceKey insert;
            if (expectedToFailInsert)
                {
                ASSERT_EQ(statement.Step(insert), BE_SQLITE_ERROR);
                statement.Finalize();
                m_ecdb.AbandonChanges();
                m_ecdb.CloseDb();
                return;
                }            
            ASSERT_EQ(statement.Step(insert), BE_SQLITE_DONE);
            }

        statement.Finalize();
        // Run a select statement to verify if the elements are grouped correctly
        statement.Prepare(m_ecdb, "select SourceECClassId, SourceECInstanceId, TargetECClassId, TargetECInstanceId, MemberPriority from ts.GroupOfElements where SourceECInstanceId=? and SourceECClassId=? ORDER BY MemberPriority");
        statement.BindId(1, firstGroup.GetInstanceId());
        statement.BindId(2, firstGroup.GetClassId());

        for (const auto& memberPriorityValue: { 0, 0, 1, 2 })
            {
            ...
            EXPECT_EQ(statement.GetValueDouble(4), memberPriorityValue);
            }
        statement.Finalize();
    ...
    importSchemasTestAndReset(SchemaItem(Utf8PrintfString(bisCoreCommon, 0, "", "")), {SchemaItem(Utf8PrintfString(bisCoreCommon, 1, importECDbVersion, memberPriorityProp)), SchemaItem(Utf8PrintfString(testSchema, 1))}, false);
    ...
    importSchemasTestAndReset(SchemaItem(Utf8PrintfString(bisCoreCommon, 0, importECDbVersion, memberPriorityProp)), {SchemaItem(Utf8PrintfString(testSchema, 0))}, false);

    // Test Case 3:
    // Setup ECDb with old BisCore without MemberPriority property, import the test schema.
    // New unique index will not be setup and inserts into GroupOfElements table should fail.
    importSchemasTestAndReset(SchemaItem(Utf8PrintfString(bisCoreCommon, 0, "", "")), {SchemaItem(Utf8PrintfString(testSchema, 0))}, true);
    }
