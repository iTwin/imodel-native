...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
BeFileName SharedSchemaDb::GetFileName() const
    {
    return m_fileName;
    }
...
InMemoryECDb::Ptr InMemoryECDb::CreateSnapshot(DbResult* outRc)
    ...
    dbPtr->CloseDb();
    rc = Db::Deserialize(buff, *dbPtr, DbDeserializeOptions::FreeOnClose | DbDeserializeOptions::Resizable, nullptr, [&](DbR db) { db.ResetBriefcaseId(BeBriefcaseId(0)); });
    if (rc == BE_SQLITE_OK)
        {
...
#ifdef TRACE_CS
    auto cancelTrace = GetTraceStmtEvent().AddListener([](TraceContext const& ctx, Utf8CP sql) { printf("[STMT] %s\n", ctx.GetExpandedSql().c_str()); });
#endif
