...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(ECSqlStatementTestFixture, SelectingInternalPropFromNavProp)
    {
...
*/
    ...
    if ("simple_select_subquery") {
        auto ecsql = R"(select Parent from (select * from (select * from ts.Element)) limit 2)";
        ECSqlStatement stmt;
        ASSERT_EQ(ECSqlStatus::Success, stmt.Prepare(m_ecdb, ecsql));
        ASSERT_EQ(1, stmt.GetColumnCount());
    ...
    if ("simple_select_cte_subquery_with_navigation_props_internal_prop") {
        auto ecsql = R"(select Parent.Id from (select * from (select * from ts.Element)) limit 2)";
        ECSqlStatement stmt;
        ASSERT_EQ(ECSqlStatus::Success, stmt.Prepare(m_ecdb, ecsql));
        ASSERT_EQ(1, stmt.GetColumnCount());
