...
TEST_F(JoinedTableTestFixture,  Disqualifying_PolymorphicFilter)
    ...
    prepareAndMatchSql("SELECT * FROM +ALL ts.Goo", {
        "[CHC_ts_Foo].[ClassId]=[ts_Foo].ECClassId AND [CHC_ts_Foo].[BaseClassId]=79"
        }, ECSqlStatus::Success);
...
TEST_F(JoinedTableTestFixture,  Disqualifying_PrimaryJoinTerm)
    ...
    prepareAndMatchSql("SELECT * FROM ts.Goo JOIN +[ts].[Doo] ON Goo.ECInstanceId=Doo.ECInstanceId", {
        "+[ts_Doo].[FooId]=[ts_Foo].[Id]"
        }, ECSqlStatus::Success);
    prepareAndMatchSql("SELECT * FROM (SELECT * FROM +ts.Doo)", {
        "+[ts_Doo].[FooId]=[ts_Foo].[Id]"
        }, ECSqlStatus::Success);
