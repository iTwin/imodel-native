...
TEST_F(JoinedTableTestFixture, ECSqlUpdateOptimization) {
    ...
    assertNativeSql(
        "UPDATE [ts_Base] SET [Code]=200 WHERE InVirtualSet(:_ecdb_ecsqlparam_id_col1,[Id]);"
        "UPDATE [ts_Sub] SET [SubNo]=300 WHERE InVirtualSet(:_ecdb_ecsqlparam_id_col1,[BaseId])",
        stmt);
    stmt.Finalize();
    ...
    expectedSql.Sprintf(
        "UPDATE [ts_Base] SET [Code]=200 WHERE [Id]=:_ecdb_ecsqlparam_ix1_col1 AND (ECClassId IN (SELECT ClassId FROM [main].ec_cache_ClassHierarchy WHERE BaseClassId=%s));"
    ...
    expectedSql.Sprintf(
        "UPDATE [ts_Base] SET [Code]=200 WHERE [ECClassId]=:_ecdb_ecsqlparam_ix1_col1 AND (ECClassId IN (SELECT ClassId FROM [main].ec_cache_ClassHierarchy WHERE BaseClassId=%s));"
...
TEST_F(JoinedTableTestFixture, AcrossMultipleSchemaImports) {
    ...
    ASSERT_EQ(SUCCESS, ImportSchema(SchemaItem("<?xml version='1.0' encoding='utf-8'?>"
                                               ...
                                               "    </ECEntityClass>"
                                               "</ECSchema>")))
        << "MapStrategy Option JoinedTablePerDirectSubclass (applied to subclasses) is expected to succeed";
    ...
    ASSERT_EQ(SUCCESS, ImportSchema(SchemaItem("<?xml version='1.0' encoding='utf-8'?>"
                                               ...
                                               "    </ECEntityClass>"
                                               "</ECSchema>")))
        << "MapStrategy Option JoinedTablePerDirectSubclass (applied to subclasses) is expected to be honored from base Class of Referred schema";
...
TEST_F(JoinedTableTestFixture, Disqualifying_PolymorphicFilter) {
    ...
    prepareAndMatchSql("SELECT * FROM +ALL ts.Goo", {SqlPrintfString("[CHC_ts_Foo].[ClassId]=[ts_Foo].ECClassId AND [CHC_ts_Foo].[BaseClassId]=%d", CLASS_ID(ts, Goo)).GetUtf8CP()}, ECSqlStatus::Success);

    prepareAndMatchSql("SELECT * FROM ALL ts.Goo", {SqlPrintfString("[CHC_ts_Foo].[ClassId]=[ts_Foo].ECClassId AND [CHC_ts_Foo].[BaseClassId]=%d", CLASS_ID(ts, Goo)).GetUtf8CP()}, ECSqlStatus::Success);

    prepareAndMatchSql("SELECT * FROM ts.Goo", {SqlPrintfString("[CHC_ts_Foo].[ClassId]=[ts_Foo].ECClassId AND [CHC_ts_Foo].[BaseClassId]=%d", CLASS_ID(ts, Goo)).GetUtf8CP()}, ECSqlStatus::Success);

    prepareAndMatchSql("SELECT * FROM ONLY ts.Goo", {SqlPrintfString("[ts_Foo].ECClassId=%d", CLASS_ID(ts, Goo)).GetUtf8CP()}, ECSqlStatus::Success);

    prepareAndMatchSql("SELECT * FROM +ONLY ts.Goo", {SqlPrintfString("+[ts_Foo].ECClassId=%d", CLASS_ID(ts, Goo)).GetUtf8CP()}, ECSqlStatus::Success);

    prepareAndMatchSql("SELECT * FROM ONLY ts.Doo", {SqlPrintfString("[ts_Foo].ECClassId=%d", CLASS_ID(ts, Doo)).GetUtf8CP()}, ECSqlStatus::Success);

    prepareAndMatchSql("SELECT * FROM +ONLY ts.Doo", {SqlPrintfString("+[ts_Foo].ECClassId=%d", CLASS_ID(ts, Doo)).GetUtf8CP()}, ECSqlStatus::Success);

    prepareAndMatchSql("SELECT * FROM ALL ts.Doo", {SqlPrintfString("[ts_Foo].ECClassId=%d", CLASS_ID(ts, Doo)).GetUtf8CP()}, ECSqlStatus::Success);
...
TEST_F(JoinedTableTestFixture, Disqualifying_PrimaryJoinTerm) {
    ...
    prepareAndMatchSql("SELECT * FROM ts.Goo JOIN +[ts].[Doo] ON Goo.ECInstanceId=Doo.ECInstanceId", {"+[ts_Doo].[FooId]=[ts_Foo].[Id]"}, ECSqlStatus::Success);
    prepareAndMatchSql("SELECT * FROM (SELECT * FROM +ts.Doo)", {"+[ts_Doo].[FooId]=[ts_Foo].[Id]"}, ECSqlStatus::Success);
    prepareAndMatchSql("SELECT * FROM (SELECT * FROM  ts.Goo JOIN +[ts].[Doo] ON Goo.ECInstanceId=Doo.ECInstanceId)", {"+[ts_Doo].[FooId]=[ts_Foo].[Id]"}, ECSqlStatus::Success);
    prepareAndMatchSql("SELECT * FROM (SELECT * FROM +ts.Goo JOIN +[ts].[Doo] ON Goo.ECInstanceId=Doo.ECInstanceId)", {"+[ts_Doo].[FooId]=[ts_Foo].[Id]", "+[ts_Goo].[FooId]=[ts_Foo].[Id]"}, ECSqlStatus::Success);
...
void JoinedTableTestFixture::VerifyInsertedInstance(ECDbR ecdb, Utf8CP ecsql, ECInstanceId sourceInstanceId, ECInstanceId targetInstanceId, ECClassId sourceClassId, ECClassId targetClassId) {
    ...
    ASSERT_EQ(sourceInstanceId.GetValue(), stmt.GetValueInt64(2)) << "Get SourceInstanceId failed : " << ecsql;
    ASSERT_EQ(sourceClassId.GetValue(), stmt.GetValueInt64(3)) << "Get SourceClassId failed : " << ecsql;
