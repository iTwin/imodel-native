...
TEST_F(ConcurrentQueryFixture, ConcurrentInstancePropertyAccessQueries)
    {
    ...
    // Concurrent instance query should succeed since experimental features were enabled globally with a PRAGMA statement
    const auto concurrentResponse1 = concurrentQueryManager.Enqueue(std::move(ECSqlRequest::MakeRequest("SELECT $ -> name FROM meta.ECClassDef WHERE Description='Relates the property to its PropertyCategory.'"))).Get();
    // Concurrent instance query should succeed since experimental featureswere enabled globally with a PRAGMA statement
    ...
    //Disable experimental features for non-concurrent ecsql queries with PRAGMA
    ...
    const auto concurrentResponse3 = concurrentQueryManager.Enqueue(std::move(ECSqlRequest::MakeRequest("SELECT $ -> name FROM meta.ECClassDef WHERE Description='Used to define a supplemental schema and its purpose' ECSQLOPTIONS ENABLE_EXPERIMENTAL_FEATURES"))).Get();
    // Concurrent instance query should fail since experimental features were disabled for this query request
    const auto concurrentResponse4 = concurrentQueryManager.Enqueue(std::move(ECSqlRequest::MakeRequest("SELECT $ -> name FROM meta.ECClassDef WHERE Description='Optional additional meta data for ECProperties of type DateTime.'"))).Get();
    // Concurrent instance query should succeed since experimental features were enabled for this query request
    ...
    EXPECT_EQ(concurrentResponse2->GetStatus(), QueryResponse::Status::Done);
    EXPECT_STREQ(concurrentResponse2->GetAsConst<ECSqlResponse>().asJsonString().c_str(), "[[\"SchemaReference\"]]");
    ...
    // Non-concurrent instance query (experimental feature) should fail since experimental features were disabled
...
TEST_F(ConcurrentQueryFixture, ConcurrentInstanceAccessQueries)
    {
    ASSERT_EQ(DbResult::BE_SQLITE_OK, SetupECDb("ExperimentalFeaturesConcurrentQueries.ecdb"));
    ...
    // Non-concurrent instance query (experimental feature) should succeed since experimental features are enabled
    ECSqlStatement experimentalStmt;
    EXPECT_EQ(ECSqlStatus::Success, experimentalStmt.Prepare(m_ecdb, "SELECT $ FROM meta.ECClassDef WHERE Description='Relates the property to its PropertyCategory.'"));
    EXPECT_EQ(experimentalStmt.Step(), BE_SQLITE_ROW);
    experimentalStmt.Finalize();

    // Execute all concurrent queries where experimental features are enabled/disabled individually per query
    // Concurrent instance query should succeed since experimental features were enabled globally with a PRAGMA statement
    EXPECT_EQ(concurrentQueryManager.Enqueue(std::move(ECSqlRequest::MakeRequest("SELECT $ FROM meta.ECClassDef WHERE Description='Relates the property to its PropertyCategory.'"))).Get()->GetStatus(), QueryResponse::Status::Done);
    // Concurrent instance query should succeed since experimental featureswere enabled globally with a PRAGMA statement
    EXPECT_EQ(concurrentQueryManager.Enqueue(std::move(ECSqlRequest::MakeRequest("SELECT $ FROM meta.ECClassDef WHERE Description='Reference to a schema including name and version'"))).Get()->GetStatus(), QueryResponse::Status::Done);

    //Disable experimental features for non-concurrent ecsql queries with PRAGMA
    ConcurrentQueryFixture::EnableECSqlExperimentalFeatures(m_ecdb, false);

    // Concurrent instance query should succeed since experimental features were enabled for this query request
    EXPECT_EQ(concurrentQueryManager.Enqueue(std::move(ECSqlRequest::MakeRequest("SELECT $ FROM meta.ECClassDef WHERE Description='Used to define a supplemental schema and its purpose' ECSQLOPTIONS ENABLE_EXPERIMENTAL_FEATURES"))).Get()->GetStatus(), QueryResponse::Status::Done);
    // Concurrent instance query should fail since experimental features were disabled for this query request
    EXPECT_EQ(concurrentQueryManager.Enqueue(std::move(ECSqlRequest::MakeRequest("SELECT $ FROM meta.ECClassDef WHERE Description='Optional additional meta data for ECProperties of type DateTime.'"))).Get()->GetStatus(), QueryResponse::Status::Error_ECSql_PreparedFailed);
    // Concurrent instance query should succeed since experimental features were enabled for this query request
    EXPECT_EQ(concurrentQueryManager.Enqueue(std::move(ECSqlRequest::MakeRequest("SELECT $ FROM meta.ECClassDef WHERE Description='KindOfQuantity' ECSQLOPTIONS enable_experimental_features"))).Get()->GetStatus(), QueryResponse::Status::Done);

    // Non-concurrent instance query (experimental feature) should fail since experimental features were disabled
    EXPECT_EQ(ECSqlStatus::InvalidECSql, experimentalStmt.Prepare(m_ecdb, "SELECT $ FROM meta.ECClassDef WHERE Description='Used to define a supplemental schema and its purpose'"));
    experimentalStmt.Finalize();
    }

TEST_F(ConcurrentQueryFixture, InstancePropertyQueryWithECSqlReader)
    {
    ASSERT_EQ(DbResult::BE_SQLITE_OK, SetupECDb("ExperimentalFeaturesConcurrentQueries.ecdb"));
    auto& concurrentQueryManager = ConcurrentQueryMgr::GetInstance(m_ecdb);
    ...
    ECSqlReader readerExperimentalFeaturesEnabled(concurrentQueryManager, "SELECT $ -> name FROM meta.ECClassDef WHERE Description='Relates the property to its PropertyCategory.' ECSQLOPTIONS ENABLE_EXPERIMENTAL_FEATURES");
    try
        {
        EXPECT_TRUE(readerExperimentalFeaturesEnabled.Next());
        EXPECT_EQ(readerExperimentalFeaturesEnabled.GetColumns().size(), 1U);
        EXPECT_EQ(readerExperimentalFeaturesEnabled.GetRow().Count(), 1U);
        EXPECT_STREQ(readerExperimentalFeaturesEnabled.GetRow()[0].ToString().c_str(), "\"PropertyHasCategory\"");
        }
    catch(std::runtime_error error) { EXPECT_STRNE(error.what(), errorMsg); }
    }

TEST_F(ConcurrentQueryFixture, InstanceAccessQueryWithECSqlReader)
    {
    ASSERT_EQ(DbResult::BE_SQLITE_OK, SetupECDb("ExperimentalFeaturesConcurrentQueries.ecdb"));
    auto& concurrentQueryManager = ConcurrentQueryMgr::GetInstance(m_ecdb);
    ...
    ECSqlReader readerExperimentalFeaturesEnabled(concurrentQueryManager, "SELECT $ FROM meta.ECClassDef WHERE Description='Relates the property to its PropertyCategory.' ECSQLOPTIONS ENABLE_EXPERIMENTAL_FEATURES");
    try
        {
        EXPECT_TRUE(readerExperimentalFeaturesEnabled.Next());
        EXPECT_EQ(readerExperimentalFeaturesEnabled.GetColumns().size(), 1U);
