...
struct TestHelper final {
   ...
   public:
    ...
    ECSqlStatus PrepareECSql(Utf8CP ecsql) const {
        ECSqlStatement stmt;
    ...
    std::vector<Utf8String> GetColumnNames(Utf8CP dbTableName) const {
        bvector<Utf8String> cols;
        m_ecdb.GetColumns(cols, dbTableName);
        return std::vector<Utf8String>(cols.begin(), cols.end());
    }
...
struct TestUtilities final {
   ...
   public:
    ...
    static BentleyStatus ParseJson(BeJsDocument& json, Utf8StringCR jsonStr) {
        ...
        return json.hasParseError() ? ERROR : SUCCESS;
    }
