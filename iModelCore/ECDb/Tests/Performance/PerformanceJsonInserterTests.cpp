...
TEST(PerformanceJsonInserter, InsertJsonCppUsingPresistanceAPI)
{
    ...
    LOGTODB(TEST_DETAILS, timer.GetElapsedSeconds(), "Inserting JsonCpp JSON objects into ECDb For repetitionCount: 1000", 1000);
}
...
TEST(PerformanceJsonInserter, InsertRapidJsonInsertJasonCppUsingPresistanceAPI)
{
    ...
    LOGTODB(TEST_DETAILS, rapidJasintimer.GetElapsedSeconds(), "Inserting RapidJson JSON objects into ECDb For repetitionCount: 1000", 1000);
}
