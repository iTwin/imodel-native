...
// @bsiclass
//=======================================================================================
struct PerformanceSharedTableTests : ECDbTestFixture {
   private:
    ...
    BentleyStatus SetupTestECDb() {
        ...
        if (mustCreateSeed) {
            ...
            for (int i = 0; i < s_initialInstanceCount; i++) {
                ...
                if (ECSqlStatus::Success != stmt.BindInt(2, i))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindText(3, txt.c_str(), IECSqlBinder::MakeCopy::No))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindInt(4, i))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindText(5, txt.c_str(), IECSqlBinder::MakeCopy::No))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindInt(6, i))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindText(7, txt.c_str(), IECSqlBinder::MakeCopy::No))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindInt(8, i))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindText(9, txt.c_str(), IECSqlBinder::MakeCopy::No))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindInt(10, i))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindText(11, txt.c_str(), IECSqlBinder::MakeCopy::No))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindInt(12, i))
                    return ERROR;
                if (ECSqlStatus::Success != stmt.BindText(13, txt.c_str(), IECSqlBinder::MakeCopy::No))
                    return ERROR;
...
BeFileName PerformanceSharedTableTests::s_seedFilePath;

//---------------------------------------------------------------------------------------
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(PerformanceSharedTableTests, CreateSeedFile) {
...
TEST_F(PerformanceSharedTableTests, Insert) {
    ...
    timer.Stop();
    LogTiming(timer, "ECSQL INSERT (SHAREDTABLE) ", s_opCount);
}
...
TEST_F(PerformanceSharedTableTests, Update) {
    ...
    timer.Stop();
    LogTiming(timer, "ECSQL UPDATE (SHAREDTABLE) ", s_opCount);
}
...
TEST_F(PerformanceSharedTableTests, Select) {
    ...
    timer.Stop();
    LogTiming(timer, "ECSQL SELECT (SHAREDTABLE) ", s_opCount);
}
