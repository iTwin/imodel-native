...
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(PerformancePrimArrayJsonVsECDTests, InsertJson)
    {
    const int rowCount = 100000;
    uint32_t arraySize = 10000;
...
// @bsimethod                                      Krischan.Eberle       07/2016
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(PerformancePrimArrayJsonVsECDTests, SelectJson)
    {
    const int rowCount = 100000;
    uint32_t arraySize = 10000;
...
// @bsimethod                                      Krischan.Eberle       07/2016
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(PerformancePrimArrayJsonVsECDTests, InsertECD)
    {
    const int rowCount = 100000;
    uint32_t arraySize = 10000;
...
// @bsimethod                                      Krischan.Eberle       07/2016
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(PerformancePrimArrayJsonVsECDTests, SelectECD)
    {
    const int rowCount = 100000;
    uint32_t arraySize = 10000;
...
BentleyStatus PerformancePrimArrayJsonVsECDTests::RunInsertJson(PrimitiveType arrayType, uint32_t arraySize, int rowCount)
    ...
    LogTiming(timer, "JSON - INSERT", arrayType, arraySize, rowCount);
    return SUCCESS;
    }
...
BentleyStatus PerformancePrimArrayJsonVsECDTests::RunSelectJson(PrimitiveType arrayType, uint32_t arraySize, int rowCount)
            ...
            switch (arrayType)
                {
                    ...
                    if (fb.size() != GetTestBlobSize() ||
                        memcmp(GetTestBlob(), fb.data(), GetTestBlobSize()) != 0)
                        return ERROR;
                    }
    ...
    stmt.Finalize();
    timer.Stop();
    LogTiming(timer, "JSON - SELECT", arrayType, arraySize, rowCount);
    return SUCCESS;
    }
...
BentleyStatus PerformancePrimArrayJsonVsECDTests::RunInsertECD(PrimitiveType arrayType, uint32_t arraySize, int rowCount)
    ...
    LogTiming(timer, "ECD - INSERT", arrayType, arraySize, rowCount);
    return SUCCESS;
    }
...
BentleyStatus PerformancePrimArrayJsonVsECDTests::RunSelectECD(PrimitiveType arrayType, uint32_t arraySize, int rowCount)
    ...
    stmt.Finalize();
    timer.Stop();
    LogTiming(timer, "ECD - SELECT", arrayType, arraySize, rowCount);
    return SUCCESS;
    }
