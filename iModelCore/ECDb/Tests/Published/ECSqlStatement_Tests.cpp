...
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F (ECSqlSelectTests, SelectQueriesOnDbGeneratedDuringBuild_04Plant)
    {
    ...
    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT TAG FROM appdw.EquipmentElementItem WHERE EQUIP_NO='50P-104B'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("50P-104B", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT COUNT(EQUIP_NO) FROM appdw.EquipmentElementItem WHERE INSUL_THK='2' AND DES_TEMP='0'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (6, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT DISTINCT INSULATION FROM appdw.PipingComponentElementItem"));
    ...
    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT AREA FROM appdw.PipingComponentElementItem WHERE MAIN_SIZE='10'"));
    ...
    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT EQUIP_NO FROM appdw.NozzleElementItem WHERE COMP_LEN=4.5 OR COMPONENT_ID='AT_EGXUTRMA_1A'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("50TW-102", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT NAME FROM appdw.PipelineElementItem WHERE NAME LIKE 'TRIM'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("TRIM", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT COMPONENT_ID FROM appdw.Equipment_PartElementItem WHERE EQUIP_NO IN(SELECT EQUIP_NO FROM appdw.Equipment_PartElementItem WHERE POSX=15798.99902)"));
    ...
    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT COMPONENT_ID FROM appdw.NozzleElementItem WHERE COMPONENT_ID LIKE '%_3G'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("AT_EGXUTRMA_3G", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT COUNT(EQUIP_NO) FROM appdw.EquipmentElementItem WHERE POSX BETWEEN 16300 AND 16400"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (2, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT COUNT(COMPONENT_ID) FROM appdw.Equipment_PartElementItem WHERE TAG IS NULL"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (0, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT DISTINCT MODULE_NAME FROM appdw.PipingComponentElementItem WHERE COMPONENT_ID IN(SELECT COMPONENT_ID FROM appdw.PipingComponentElementItem WHERE MAIN_SIZE IS NOT NULL AND MAIN_SIZE='10')"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("Base", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT SUM(POSX) FROM appdw.EquipmentElementItem WHERE INSUL_THK='2'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (95429, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT MAX(POSY) FROM appdw.EquipmentElementItem"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (5403.87402, stmt.GetValueDouble (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT AVG(POSZ) FROM appdw.EquipmentElementItem"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (239, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT COUNT(INSUL_THK) FROM appdw.EquipmentElementItem WHERE INSUL_THK=2"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (6, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT EQUIP_NO FROM ONLY appdw.EquipmentElementItem WHERE EQUIP_NO='50E-101A'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("50E-101A", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select COMPONENT_ID From appdw.NozzleElementItem where COMPONENT_ID ='AT_EGXUTRMA_1P' "));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("AT_EGXUTRMA_1P", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select COUNT(SPEC_TABLE) From appdw.PipingComponentElementItem where SPEC_TABLE Like 'ELBOW'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (147, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select COMPONENT_ID From appdw.NozzleElementItem where COMPONENT_ID Like '%A_3%'"));
    ...
    ASSERT_EQ (ECSqlStatus::InvalidECSql, stmt.Prepare (ecdb, "Select * FROM appdw.blabla")) << "No such table exists.";
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select POSX,POSY,POSZ From appdw.EquipmentElementItem WHERE EQUIP_NO='50TW-102'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
...
* @bsimethod                             Maha Nasir                         10/15
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F (ECSqlSelectTests, SelectQueriesOnDbGeneratedDuringBuild_79Main)
    {
    ECDbTestProject project;
    ECSqlStatement stmt;
    BeFileName dir;
    ...
    BeTest::GetHost ().GetDocumentsRoot (dir);
    dir.AppendToPath (L"DgnDb");
    dir.AppendToPath (L"79_Main.i.idgndb");
    if (dir.DoesPathExist ())
        {
        ASSERT_EQ (DbResult::BE_SQLITE_OK, project.Open (dir.GetNameUtf8 ().c_str (), Db::OpenParams (Db::OpenMode::Readonly)));
        }
    ...
    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select DISTINCT EQP_DESC From ams.EQUIP_MEQPElementItem Where EQP_DESC LIKE 'HEA%%'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("HEAT EXCHANGER", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select COUNT(SRC_CODE) From ams.EQUIP_MEQPElementItem Where SRC_CODE='FENCE'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (8, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select BOM_FLAG From ams.EQUIP_MEQPElementItem where CLIP_LENGTH=6.0572915077209473"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (1, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select EQP_NO From ams.EQUIP_MEQPElementItem where ELEMENT_ID>5000 ORDER BY EQP_NO ASC"));
    ...
    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select COUNT(BOM_FLAG) AS MyQuery From ams.EQUIP_MEQPElementItem where BOM_FLAG>=1"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (16, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select DISTINCT COMPTYPE From ams.EQUIP_MEQPElementItem Where COMPTYPE LIKE '%EQ%'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("MEQP", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select EQP_SEQ From ams.EQUIP_MEQPElementItem Where EQP_DESC='FEED PUMP' AND D_STATUS='IN_STUDY'"));
    ...
    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select SRC_CODE From ams.EQUIP_MEQPElementItem Where CLIP_LENGTH BETWEEN 1 AND 10"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("CLSLIB", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select ORIG_DES From ams.EQUIP_MEQPElementItem Where EQP_SEQ='TW' OR ELEMENT_ID='700'"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("Alan.Leonard", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT SPEC From ams.PIPE_PFLRElementItem Where FL_TYPE IN(Select FL_TYPE From ams.PIPE_PFLRElementItem Where EPREP1='BW')"));
    ...
    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select SUM(RATING) From ams.EQUIP_PNOZElementItem Where SIZE_1=10"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_EQ (750, stmt.GetValueInt (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "Select DISTINCT DATABASE From ams.EQUIP_PNOZElementItem Where SIZE_1 NOT BETWEEN 1 AND 19"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
    ASSERT_STREQ ("EQUIP", stmt.GetValueText (0));
    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "SELECT NOZ_NO From ams.EQUIP_PNOZElementItem Where LINENO =(Select LINENO From ams.EQUIP_PNOZElementItem Where EQP_TRN='0' AND EQP_NO='101')"));
    ASSERT_EQ (stmt.Step (), DbResult::BE_SQLITE_ROW);
...
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(ECSqlTestFixture, ECSqlStatement_ClassWithStructHavingStructArrayInsert)
    {
    const auto perClassRowCount = 0;
    // Create and populate a sample project
    auto& ecdb = SetUp("ecsqlstatementtests.ecdb", BeFileName(L"ECSqlTest.01.00.ecschema.xml"), ECDb::OpenParams(Db::OpenMode::ReadWrite), perClassRowCount);
...
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F(ECSqlTestFixture, ECSqlStatement_ClassWithStructHavingStructArrayInsertWithDotoperator)
    {
    const auto perClassRowCount = 0;
    // Create and populate a sample project
    auto& ecdb = SetUp("ecsqlstatementtests.ecdb", BeFileName(L"ECSqlTest.01.00.ecschema.xml"), ECDb::OpenParams(Db::OpenMode::ReadWrite), perClassRowCount);
