...
void MetaSchemaECSqlTestFixture::AssertClassDef(ECClassCR expectedClass, ECSqlStatement const& actualClassDefRow)
    ...
    for (int i = 0; i < colCount; i++)
        ...
        if (colName.EqualsI("SchemaId"))
            {
            ASSERT_EQ(expectedClass.GetSchema().GetId().GetValue(), val.GetValueNavigation (nullptr).GetValue()) << "ECClassDef.SchemaId";
...
void MetaSchemaECSqlTestFixture::AssertEnumerationDef(ECEnumerationCR expectedEnum, ECSqlStatement const& actualEnumerationDefRow)
    ...
    for (int i = 0; i < colCount; i++)
        ...
        if (colName.EqualsI("SchemaId"))
            {
            ASSERT_EQ(expectedEnum.GetSchema().GetId().GetValue(), val.GetValueNavigation (nullptr).GetValue()) << "ECEnumerationDef.SchemaId";
            continue;
            }
...
void MetaSchemaECSqlTestFixture::AssertKindOfQuantityDef(KindOfQuantityCR expectedKoq, ECSqlStatement const& actualKoqDefRow)
    ...
    for (int i = 0; i < colCount; i++)
        ...
        if (colName.EqualsI("SchemaId"))
            {
            ASSERT_EQ(expectedKoq.GetSchema().GetId().GetValue(), val.GetValueNavigation (nullptr).GetValue()) << "KindOfQuantityDef.SchemaId";
            continue;
            }
...
void MetaSchemaECSqlTestFixture::AssertPropertyDef(ECPropertyCR expectedProp, ECSqlStatement const& actualPropertyDefRow)
    ...
    for (int i = 0; i < colCount; i++)
        ...
        if (colName.EqualsI("ClassId"))
            {
            ASSERT_EQ(expectedProp.GetClass().GetId().GetValue(), val.GetValueNavigation (nullptr).GetValue()) << "ECPropertyDef.ClassId for " << expectedProp.GetClass().GetFullName() << "." << expectedProp.GetName().c_str();
            continue;
            }
        ...
        if (colName.EqualsI("EnumerationId"))
            ...
            if (expectedEnum != nullptr)
                ASSERT_EQ(expectedEnum->GetId().GetValue(), val.GetValueNavigation (nullptr).GetValue()) << "ECPropertyDef.EnumerationId for prim prop with enumeration for " << expectedProp.GetClass().GetFullName() << "." << expectedProp.GetName().c_str();
            else
        ...
        if (colName.EqualsI("StructClassId"))
            ...
            if (structProp != nullptr)
               ASSERT_EQ(structProp->GetType().GetId().GetValue(), val.GetValueNavigation (nullptr).GetValue()) << "ECPropertyDef.StructClassId for struct prop for " << expectedProp.GetClass().GetFullName() << "." << expectedProp.GetName().c_str();
            else if (structArrayProp != nullptr)
                ASSERT_EQ(structArrayProp->GetStructElementType()->GetId().GetValue(), val.GetValueNavigation (nullptr).GetValue()) << "ECPropertyDef.StructClassId for struct array prop for " << expectedProp.GetClass().GetFullName() << "." << expectedProp.GetName().c_str();
            else
        ...
        if (colName.EqualsI("KindOfQuantityId"))
            ...
            if (expectedKoq != nullptr)
                ASSERT_EQ(expectedKoq->GetId().GetValue(), val.GetValueNavigation (nullptr).GetValue()) << "ECPropertyDef.KindOfQuantityId for prim or prim array prop for " << expectedProp.GetClass().GetFullName() << "." << expectedProp.GetName().c_str();
            else
        ...
        if (colName.EqualsI("NavigationRelationshipClassId"))
            {
            if (navProp != nullptr)
                ASSERT_EQ(navProp->GetRelationshipClass()->GetId().GetValue(), val.GetValueNavigation (nullptr).GetValue()) << "ECPropertyDef.NavigationRelationshipClassId for nav prop for " << expectedProp.GetClass().GetFullName() << "." << expectedProp.GetName().c_str();
            else
