...
// @bsimethod                                     Krischan.Eberle                  04/13
//+---------------+---------------+---------------+---------------+---------------+------
void ECSqlAsserter::AssertPrepare(bool& prepareSucceeded, ECSqlTestItem const& testItem, ECSqlStatement& statement, PrepareECSqlExpectedResult const& expectedResult) const
    {
    ...
    ECSqlStatus stat = PrepareStatement(statement, ecsql, !expectedToSucceed);
    ...
    if (expectedToSucceed)
        ...
        ASSERT_NE(ECSqlStatus::Success, stat) << assertMessage << " " << ecsql;
        }
    }
...
void ECSqlSelectAsserter::AssertStep(ECSqlTestItem const& testItem, ECSqlStatement& statement, ResultCountECSqlExpectedResult const& expectedResult) const
    {
    const bool expectedToSucceed = expectedResult.IsExpectedToSucceed();
    DbResult stat = Step(statement, !expectedToSucceed);
    ...
    const int expectedColumnCount = expectedResult.GetExpectedColumnCount();
    const int actualColumnCount = statement.GetColumnCount();
    ...
    if (expectedResult.HasExpectedRowCount())
        {
        const int expectedRowCount = expectedResult.GetExpectedRowCount();
...
void ECSqlSelectAsserter::AssertColumnInfo(ECSqlTestItem const& testItem, ECSqlStatement const& statement, IECSqlValue const& value, ECTypeDescriptor const* parentDataType) const
    ...
    ECSqlPropertyPath const& propPath = columnInfo.GetPropertyPath();
    ECClassCR rootClass = columnInfo.GetRootClass();

    if (columnInfo.IsGeneratedProperty())
        {
        EXPECT_STREQ("DynamicECSqlSelectClause", rootClass.GetName().c_str()) << "IECSqlValue::GetColumnInfo().GetRootClass() is expected to return the anonymous class for generated properties.";
...
Utf8String ECSqlSelectAsserter::DataTypeToString(ECTypeDescriptor const& dataType)
    ...
    if (dataType.IsPrimitive())
        {
        //for literal NULLs in the select clause
        if ((int) dataType.GetPrimitiveType() == 0)
            return ""; //for literal NULLs in the select clause
