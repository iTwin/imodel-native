...
TEST(FieldEngineer, Workflow)
    ...
    ASSERT_EQ(BE_SQLITE_OK, updater.Update(ecInstanceId, currentObject)); // Uses ECInstanceId in the currentObject to find the entry to update
    ...
    ASSERT_EQ(BE_SQLITE_OK, inserter.Insert(currentObject));
    ...
    ASSERT_EQ(BE_SQLITE_OK, deleter.Delete(bronzeFishId));
    ...
    */
    ...
    ASSERT_TRUE(deleter.IsValid());
    ASSERT_EQ(BE_SQLITE_OK, deleter2.Delete(managerRelId));
