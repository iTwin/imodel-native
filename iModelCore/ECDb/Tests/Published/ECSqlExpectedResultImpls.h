/*--------------------------------------------------------------------------------------+
|
|     $Source: Tests/ECDB/Published/ECSqlExpectedResultImpls.h $
|
...
//=======================================================================================    
struct PrepareECSqlExpectedResult : public IECSqlExpectedResult
    {
public:
...
public:
    ...
    static std::unique_ptr<IECSqlExpectedResult> Create (Utf8CP expectedNativeSql, Category failingCategory = Category::Supported, Utf8CP description = nullptr);
    //! Initializes a new instance of the PrepareECSqlExpectedResult type.
    //! @param[in] expectedNativeSqlList List of expected native SQL string resulting from translation of ECSQL.
    //!            Pass empty list if the translation is not expected to succeed.
    static std::unique_ptr<IECSqlExpectedResult> Create (SqlListCR expectedNativeSqlList, Category failingCategory = Category::Supported, Utf8CP description = nullptr);
    //! Initializes a new instance of the PrepareECSqlExpectedResult type which indicates that the test
    //! is expected to fail
    //! @param[in] failingCategory Failure IECSqlBinder
    static std::unique_ptr<IECSqlExpectedResult> CreateFailing (Category failingCategory, Utf8CP description = nullptr);
    ...
    //! @param[in] failingCategory Failure IECSqlBinder
    static std::unique_ptr<IECSqlExpectedResult> CreateFailing (Utf8CP expectedNativeSql, Category failingCategory, Utf8CP description = nullptr);
...
// @bsiclass                                                 Krischan.Eberle     04/2013
//=======================================================================================    
struct ResultCountECSqlExpectedResult : public IECSqlExpectedResult
    {
private:
    int m_columnCount;
    int m_rowCount;
...
public:
    ...
    //! @param[in] expectedColumnCount Number of expected columns in result set
    //! @param[in] expectedRowCount Number of expected rows in result set. Pass -1 if row count test should be skipped
    static std::unique_ptr<IECSqlExpectedResult> Create (int expectedColumnCount, int expectedRowCount = -1);
    //! Initializes a new instance of the ResultCountECSqlExpectedResult type.
    //! @param[in] IECSqlBinder Expected result IECSqlBinder
    //! @param[in] expectedColumnCount Number of expected columns in result set
    //! @param[in] expectedRowCount Number of expected rows in result set. Pass -1 if row count test should be skipped
    static std::unique_ptr<IECSqlExpectedResult> Create (Category IECSqlBinder, Utf8CP description, int expectedColumnCount, int expectedRowCount = -1);
...
// @bsiclass                                                 Krischan.Eberle     04/2013
//=======================================================================================    
struct AffectedRowsECSqlExpectedResult : public IECSqlExpectedResult
    {
    private:
        int m_affectedRowCount;

        //! Initializes a new instance of the AffectedRowsECSqlExpectedResult type.
        //! @param[in] expectedAffectedRowCount Expected number of rows affected by non-Select ECSQL
        explicit AffectedRowsECSqlExpectedResult (int expectedAffectedRowCount);

        //! Initializes a new instance of the AffectedRowsECSqlExpectedResult type representing a test which is expected to fail
        //! @param[in] failingCategory Failure IECSqlBinder
        explicit AffectedRowsECSqlExpectedResult (Category failingCategory, Utf8CP description = nullptr);

    public:
        AffectedRowsECSqlExpectedResult (AffectedRowsECSqlExpectedResult const& rhs);
        AffectedRowsECSqlExpectedResult& operator= (AffectedRowsECSqlExpectedResult const& rhs);
        AffectedRowsECSqlExpectedResult (AffectedRowsECSqlExpectedResult&& rhs);
        AffectedRowsECSqlExpectedResult& operator= (AffectedRowsECSqlExpectedResult&& rhs);

        virtual ~AffectedRowsECSqlExpectedResult ();

        int GetExpectedAffectedRowCount () const;

        //! Initializes a new instance of the AffectedRowsECSqlExpectedResult type.
        //! @param[in] expectedAffectedRowCount Expected number of rows affected by non-Select ECSQL
        static std::unique_ptr<IECSqlExpectedResult> Create (int expectedAffectedRowCount);
        //! Initializes a new instance of the AffectedRowsECSqlExpectedResult type which indicates that the test
        //! is expected to fail
        //! @param[in] failingCategory Failure IECSqlBinder
        static std::unique_ptr<IECSqlExpectedResult> CreateFailing (Category failingCategory, Utf8CP description = nullptr);
    };
