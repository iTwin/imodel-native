...
TEST (JsonUpdaterTests, UpdateRelationshipProperty)
    ...
    ASSERT_EQ(BE_SQLITE_OK, ECDbTestUtility::CreateECDb (ecdb, nullptr, L"UpdateRelationshipProperty.ecdb"));
    ...
    ECSqlStatement stmt;
    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "INSERT INTO ts.A (P1) VALUES(?)"));


    ASSERT_EQ (ECSqlStatus::Success, stmt.BindInt (1, 111));
    ASSERT_EQ (ECSqlStepStatus::Done, stmt.Step (sourceKey));
    ...
    ASSERT_EQ (ECSqlStepStatus::Done, stmt.Step (targetKey));
    targetInstanceId = targetKey.GetECInstanceId ();

    stmt.Finalize ();

    ASSERT_EQ (ECSqlStatus::Success, stmt.Prepare (ecdb, "INSERT INTO ts.AHasA (SourceECInstanceId, TargetECInstanceId, Name) VALUES(?,?,'good morning')"));
    ...
    ASSERT_EQ (ECSqlStepStatus::Done, stmt.Step (relKey));
    relInstanceId = relKey.GetECInstanceId ();
    }

    ECClassP relClass = nullptr;
    ASSERT_EQ(BentleyStatus::SUCCESS, ecdb.GetEC().GetSchemaManager().GetECClass(relClass, "test", "AHasA"));
    ASSERT_TRUE (relClass != nullptr);
    JsonReader reader (ecdb, relClass->GetId ());
    ...
    JsonUpdater updater (ecdb, *relClass);
    ...
    */
    ...
    ASSERT_EQ (ECSqlStatus::Success, checkStmt.Prepare (ecdb, "SELECT NULL FROM ts.AHasA WHERE ECInstanceId=? AND Name=?"));
    ...
    ASSERT_EQ (ECSqlStepStatus::HasRow, checkStmt.Step ());
    checkStmt.Reset ();
