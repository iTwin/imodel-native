...
+===============+===============+===============+===============+===============+======*/
struct InstanceCRUDTests : public ECDbTestFixture
    {
    ECDb                        m_db;
...
bool InstanceCRUDTests::importSchema(WString schemaNameWithoutVerionAndExtension)
    ...
    auto importSchemaStatus = m_db.Schemas().ImportECSchemas(cache->GetSchemas());
...
TEST_F(InstanceCRUDTests, singleClassTest)
    ...
    ECSchemaPtr schemaPtr = ReadECSchemaFromDisk(schemaReadContext, m_db, BeFileName(schema));
