...
struct QueryRequest {
   ...
   public:
    ...
    QueryRequest& SetQuota(QueryQuota quota) noexcept {
        m_quota = quota;
        return *this;
    }
    QueryRequest& SetPriority(int32_t priority) noexcept {
        m_priority = priority;
        return *this;
    }
    QueryRequest& SetUsePrimaryConnection(bool usePrimary) {
        m_usePrimaryConn = usePrimary;
        return *this;
    }
    QueryRequest& SetRestartToken(std::string const& token) {
        m_restartToken = token;
        return *this;
    }
    QueryRequest& SetDelay(std::chrono::milliseconds t) noexcept {
        m_delay = t;
        return *this;
    }
...
struct BlobIORequest : public QueryRequest {
   ...
   public:
    ...
    BlobIORequest& SetRange(QueryLimit const& range) {
        m_range = range;
        return *this;
    }
...
class ECSqlParams final {
   ...
   public:
    ...
    ECSqlParams& BindBlob(std::string const& name, bvector<Byte> const& val) {
        m_params[name] = ECSqlParam(name, val);
        return *this;
    }
    ECSqlParams& BindBool(std::string const& name, bool val) {
        m_params[name] = ECSqlParam(name, val);
        return *this;
    }
    ECSqlParams& BindDouble(std::string const& name, double val) {
        m_params[name] = ECSqlParam(name, val);
        return *this;
    }
    ECSqlParams& BindId(std::string const& name, BeInt64Id val) {
        m_params[name] = ECSqlParam(name, val);
        return *this;
    }
    ECSqlParams& BindIdSet(std::string const& name, BeIdSet const& val) {
        m_params[name] = ECSqlParam(name, val);
        return *this;
    }
    ECSqlParams& BindInt(std::string const& name, int val) {
        m_params[name] = ECSqlParam(name, val);
        return *this;
    }
    ECSqlParams& BindLong(std::string const& name, int64_t val) {
        m_params[name] = ECSqlParam(name, val);
        return *this;
    }
    ECSqlParams& BindNull(std::string const& name) {
        m_params[name] = ECSqlParam(name);
        return *this;
    }
    ECSqlParams& BindPoint2d(std::string const& name, DPoint2dCR val) {
        m_params[name] = ECSqlParam(name, val);
        return *this;
    }
    ECSqlParams& BindPoint3d(std::string const& name, DPoint3dCR val) {
        m_params[name] = ECSqlParam(name, val);
        return *this;
    }
    ECSqlParams& BindString(std::string const& name, std::string const& val) {
        m_params[name] = ECSqlParam(name, val);
        return *this;
    }
    ECSqlParams& BindBlob(int index, bvector<Byte> const& val) {
        BindBlob(std::to_string(index), val);
        return *this;
    }
    ECSqlParams& BindBool(int index, bool val) {
        BindBool(std::to_string(index), val);
        return *this;
    }
    ECSqlParams& BindDouble(int index, double val) {
        BindDouble(std::to_string(index), val);
        return *this;
    }
    ECSqlParams& BindId(int index, BeInt64Id val) {
        BindId(std::to_string(index), val);
        return *this;
    }
    ECSqlParams& BindIdSet(int index, BeIdSet const& val) {
        BindIdSet(std::to_string(index), val);
        return *this;
    }
    ECSqlParams& BindInt(int index, int val) {
        BindInt(std::to_string(index), val);
        return *this;
    }
    ECSqlParams& BindLong(int index, int64_t val) {
        BindLong(std::to_string(index), val);
        return *this;
    }
    ECSqlParams& BindNull(int index) {
        BindNull(std::to_string(index));
        return *this;
    }
    ECSqlParams& BindPoint2d(int index, DPoint2dCR val) {
        BindPoint2d(std::to_string(index), val);
        return *this;
    }
    ECSqlParams& BindPoint3d(int index, DPoint3dCR val) {
        BindPoint3d(std::to_string(index), val);
        return *this;
    }
    ECSqlParams& BindString(int index, std::string const& val) {
        BindString(std::to_string(index), val);
        return *this;
    }
...
struct ECSqlRequest : public QueryRequest {
   ...
   public:
    ...
    ECSqlRequest& SetValueFmt(ECSqlValueFormat fmt) noexcept {
        m_valueFmt = fmt;
        return *this;
    }
    ECSqlRequest& SetLimit(QueryLimit limit) noexcept {
        m_limit = limit;
        return *this;
    }
    ECSqlRequest& SetAbbreviateBlobs(bool abbreviateBlobs) {
        m_abbreviateBlobs = abbreviateBlobs;
        return *this;
    }
    ECSqlRequest& SetSuppressLogErrors(bool suppressLogErrors) {
        m_suppressLogErrors = suppressLogErrors;
        return *this;
    }
    ECSqlRequest& SetIncludeMetaData(bool includeMetaData) {
        m_includeMetaData = includeMetaData;
        return *this;
    }
    ECSqlRequest& SetConvertClassIdsToClassNames(bool convertClassIdsToClassNames) {
        m_convertClassIdsToClassNames = convertClassIdsToClassNames;
        return *this;
    }
    ECSqlRequest& SetArgs(Json::Value const& args) {
        m_args.FromJs(args);
        return *this;
    }
    ECSqlRequest& SetArgs(ECSqlParams const& args) {
        m_args = args;
        return *this;
    }
...
struct QueryResponse : std::enable_shared_from_this<QueryResponse> {
    ...
    struct Stats {
       ...
       public:
        ...
        Stats(std::chrono::microseconds cpuTime, std::chrono::milliseconds totalTime, uint32_t memUsed, QueryQuota const& quota, std::chrono::milliseconds prepareTime) : m_cpuTime(cpuTime), m_totalTime(totalTime), m_memLimit(quota.MaxMemoryAllowed()), m_memUsed(memUsed), m_timeLimit(std::chrono::duration_cast<std::chrono::milliseconds>(quota.MaxTimeAllowed())), m_prepareTime(prepareTime) {}
        virtual ~Stats() {}
...
struct ConcurrentQueryMgr final {
    ...
    enum class ClearCacheOption { Yes,
                                  No };
    enum class DetachAttachDbs { Yes,
                                 No };
    struct Config final {
       ...
       public:
        ...
        Config& SetProgressOpCount(uint32_t progressOpCount) {
            m_progressOpCount = progressOpCount;
            return *this;
        }
        ...
        Config& SetQuota(QueryQuota const& quota) {
            m_quota = quota;
            return *this;
        }
        Config& SetWorkerThreadCount(uint32_t workerThreadCount) {
            m_workerThreadCount = workerThreadCount;
            return *this;
        }
        Config& SetRequestQueueSize(uint32_t requestQueueSize) {
            m_requestQueueSize = requestQueueSize;
            return *this;
        }
        Config& SetIgnorePriority(bool ignorePriority) {
            m_ignorePriority = ignorePriority;
            return *this;
        }
        Config& SetDoNotUsePrimaryConnToPrepare(bool doNotUsePrimaryConnToPrepare) {
            m_doNotUsePrimaryConnToPrepare = doNotUsePrimaryConnToPrepare;
            return *this;
        }
        Config& SetAutoShutdownWhenIdleForSeconds(std::chrono::seconds autoShutdownWhenIdleForSeconds) {
            m_autoShutdownWhenIdleForSeconds = autoShutdownWhenIdleForSeconds;
            return *this;
        }
        Config& SetStatementCacheSizePerWorker(uint32_t statementCacheSizePerWorker) {
            m_statementCacheSizePerWorker = statementCacheSizePerWorker;
            return *this;
        }
