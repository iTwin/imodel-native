...
struct ECSqlPropertyPath final {
   public:
    ...
    struct const_iterator {
        ...
        using value_type = EntryCP;
        using difference_type = std::ptrdiff_t;
        using pointer = EntryCP*;
        using reference = EntryCP&;
...
struct ECSqlColumnInfo final {
    ...
    struct RootClass final {
       ...
       public:
        ...
        Utf8StringCR GetTableSpace() const {
            BeAssert(IsValid());
            return m_tableSpace;
        }
   ...
   public:
    ...
    ECSqlColumnInfo(const ECSqlColumnInfo& rhs, bool isDynamic) : m_dataType(rhs.m_dataType),
    ...
    DateTime::Info const& GetDateTimeInfo() const {
        BeAssert((m_dataType.IsPrimitive() || m_dataType.IsPrimitiveArray()) && m_dataType.GetPrimitiveType() == ECN::PRIMITIVETYPE_DateTime);
    ...
    ECN::ECStructClassCP GetStructType() const {
        BeAssert((m_dataType.IsStruct() || m_dataType.IsStructArray()) && m_structType != nullptr);
        return m_structType;
    }
    ...
    //! Gets the ECProperty that the specified column originated from.
    //! In case the column is using an alias, this will point to the real property, while GetProperty will point to a generated property.
    //! @note When called from an IECSqlValue that represents an array @b element, this method will return nullptr.
    //! @return ECProperty backing the column represented by this info object.
    ECN::ECPropertyCP GetOriginProperty() const { return m_originProperty; }
