...
struct ClassMapStrategy {
   ...
   public:
    ...
    bool IsLinkTableRelationship() const {
        BeAssert(!IsEmpty());
        return IsRelationshipClass() && (IsTablePerHierarchy() || IsOwnTable());
    }
    bool IsTablePerHierarchy() const {
        BeAssert(!IsEmpty());
        return m_strategy == MapStrategy::TablePerHierarchy;
    }
    bool IsOwnTable() const {
        BeAssert(!IsEmpty());
        return m_strategy == MapStrategy::OwnTable;
    }
    bool IsExistingTable() const {
        BeAssert(!IsEmpty());
        return m_strategy == MapStrategy::ExistingTable;
    }
    bool IsForeignKeyRelationship() const {
        BeAssert(!IsEmpty());
        return m_strategy == MapStrategy::ForeignKeyRelationshipInSourceTable || m_strategy == MapStrategy::ForeignKeyRelationshipInTargetTable;
    }
    bool IsRelationshipClass() const {
        BeAssert(!IsEmpty());
        return m_class->IsRelationshipClass();
    }
    bool IsEntityClass() const {
        BeAssert(!IsEmpty());
        return m_class->IsEntityClass();
    }
    bool IsNotMapped() const {
        BeAssert(!IsEmpty());
        return m_strategy == MapStrategy::NotMapped;
    }
    bool IsEmpty() const { return m_class == nullptr; }
    ECN::ECClassCP GetClass() const {
        BeAssert(!IsEmpty());
        return m_class;
    }
    MapStrategy GetStrategy() const {
        BeAssert(!IsEmpty());
        return m_strategy;
    }
};
    ...
    struct SearchResults {
       ...
       public:
        ...
        SearchResults(
            ...
            std::map<ECN::ECClassId, std::vector<ForeignKeyRelation>>&& fkRels) : m_entityKeyMap(entities), m_linktableRelMap(linkTableRels), m_foreignKeyMap(fkRels) {}
...
struct DropSchemaResult {
   ...
   public:
    ...
    DropSchemaResult(DropSchemaResult&& rhs) : m_searchResults(std::move(rhs.m_searchResults)),
