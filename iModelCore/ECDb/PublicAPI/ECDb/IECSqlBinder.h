...
private:
    ...
    virtual ECSqlStatus _BindIdSet(std::shared_ptr<VirtualSet>) = 0;
...
public:
    ...
    ECSqlStatus BindVirtualSet(std::shared_ptr<VirtualSet> virtualSet) { return BindInt64((int64_t) virtualSet.get()); }

    //! Binds an IdSet to the SQL function @b InVirtualSet while holding it in memory.
    //! The parameter must be the first parameter in the InVirtualSet function.
    //! @param[in] shared pointer to virtualSet to bind
    //! @return ECSqlStatus::Success or error codes
    ECDB_EXPORT ECSqlStatus BindIdSet(std::shared_ptr<VirtualSet> virtualSet);
