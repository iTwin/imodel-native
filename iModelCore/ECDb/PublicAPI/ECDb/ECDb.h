...
struct ECSqlConfig {
    ...
    private:
        ...
        bool m_validateInsertValues;
        mutable std::unordered_map<OptimizationOptions, bool> m_optimisationOptionsMap;

    public:
        ECSqlConfig(): m_experimentalFeaturesEnabled(false), m_validateInsertValues(false) {
        ...
        bool IsInsertValueValidationEnabled() const { return m_validateInsertValues; }
        void SetInsertValueValidation(const bool v) { m_validateInsertValues = v; }
};
...
public:
    ...
    //! Depreciated, this is NOP and change change can only be detach if connection is closed.
    ...
    //! Get name of change cache file if any
    //! @param[in] changeCachePath Path to the Change Cache file will be returned in this parameter
    //! @return true if a cachefile was attached else return false.
    //! @see @ref ECDbChange
    ECDB_EXPORT bool TryGetChangeCacheFileName(BeFileNameR changeCachePath) const;
    ...
    ECDB_EXPORT BentleyStatus AddIssueListener(ECN::IIssueListener const& issueListener);
    ...
    ECDB_EXPORT void AddECDbCacheClearListener(IECDbCacheClearListener&);
