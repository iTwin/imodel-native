...
Utf8String DDLGenerator::GetColumnDDL(ECDbSqlColumn const& o)
    ...
    if (o.GetConstraint().GetCollation() != ECDbSqlColumn::Constraint::Collation::Default)
        sql.append(" COLLATE ").append(ECDbSqlColumn::Constraint::CollationToString(o.GetConstraint().GetCollation()));

    if (!o.GetConstraint().GetDefaultExpression().empty())
        sql.append(" DEFAULT (").append(o.GetConstraint().GetDefaultExpression()).append(")");
...
//---------------------------------------------------------------------------------------
//static 
ECDbSqlColumn::Type ECDbSqlColumn::PrimitiveTypeToColumnType(ECN::PrimitiveType type)
    {
    switch (type)
        {
...
//--------------------------------------------------------------------------------------
//static 
bool ECDbSqlColumn::IsCompatible(ECDbSqlColumn::Type target, ECDbSqlColumn::Type source)
    {
    switch (source)
        {
            case ECDbSqlColumn::Type::Any:
...
//---------------------------------------------------------------------------------------
ECDbSQLManager::ECDbSQLManager(ECDbR ecdb) : m_ecdb(ecdb), m_idGenerator(ecdb), m_defaultDb(*this), m_persistence(ecdb),
m_nullTable(nullptr), m_mapStorage(*this), m_loaded(false)
    {}
...
DbResult ECDbSqlPersistence::ReadTable(Statement& stmt, ECDbMapDb& o) const
    {
    ECDbTableId id = stmt.GetValueId<ECDbTableId>(0);
...
DbResult ECDbSqlPersistence::ReadColumns(ECDbSqlTable& o) const
    ...
    DbResult stat = stmt->BindId(1, o.GetId());
    if (stat != BE_SQLITE_OK)
        return stat;
...
DbResult ECDbSqlPersistence::ReadColumn(Statement& stmt, ECDbSqlTable& table, std::map<size_t, ECDbSqlColumn const*>& primaryKeys) const
    {
    ECDbColumnId id = stmt.GetValueId<ECDbColumnId>(0);
    Utf8CP name = stmt.GetValueText(1);
...
DbResult ECDbSqlPersistence::InsertForeignKey(ECDbSqlForeignKeyConstraint const& o) const
    ...
    stmt->BindId(2, o.GetForeignKeyTable().GetId());
    stmt->BindId(3, o.GetReferencedTable().GetId());
    if (!o.GetName().empty())
    ...
    for (size_t i = 0; i < o.Count(); i++)
        ...
        stmt->ClearBindings();
        stmt->BindId(1, o.GetId());
        stmt->BindId(2, o.GetFkColumns()[i]->GetId());
        stmt->BindId(3, o.GetReferencedTableColumns()[i]->GetId());
        stmt->BindInt(4, (int) i);
...
ECDbTableId ECDbBriefcaseBasedId::_NextTableId()
    ...
    return ECDbTableId(id.GetValue());
    }
...
ECDbColumnId ECDbBriefcaseBasedId::_NextColumnId()
    ...
    return ECDbColumnId(id.GetValue());
    }
...
ECDbIndexId ECDbBriefcaseBasedId::_NextIndexId()
    ...
    return ECDbIndexId(id.GetValue());
    }
...
ECDbConstraintId ECDbBriefcaseBasedId::_NextConstraintId()
    ...
    if (m_ecdb.GetECDbImplR().GetConstraintIdSequence().GetNextValue(id) != BE_SQLITE_OK)
        ...
        return ECDbConstraintId();
        }

    return ECDbConstraintId(id.GetValue());
    }
...
ECDbClassMapId ECDbBriefcaseBasedId::_NextClassMapId()
    ...
    if (m_ecdb.GetECDbImplR().GetClassMapIdSequence().GetNextValue(id) != BE_SQLITE_OK)
        {
        BeAssert(false && "Failed to generate new constraintid");
        return ECDbClassMapId();
        }

    return ECDbClassMapId(id.GetValue());
    }
...
ECDbPropertyPathId ECDbBriefcaseBasedId::_NextPropertyPathId()
    ...
    if (m_ecdb.GetECDbImplR().GetPropertyMapIdSequence().GetNextValue(id) != BE_SQLITE_OK)
        {
        BeAssert(false && "Failed to generate new constraintid");
        return ECDbPropertyPathId();
        }

    return ECDbPropertyPathId(id.GetValue());
    }
...
DbResult ECDbMapStorage::InsertPropertyMap(ECDbPropertyMapInfo const& o) const
    ...
    for (auto column : o.GetColumns())
        {
        stmt->BindId(1, o.GetClassMap().GetId());
        stmt->BindId(2, o.GetPropertyPath().GetId());
...
DbResult ECDbMapStorage::ReadPropertyMap(ECDbClassMapInfo& o) const
    ...
    while (stmt->Step() == BE_SQLITE_ROW)
        {
        ECDbPropertyPathId propertyPathId = stmt->GetValueId<ECDbPropertyPathId>(0);
...
ECDbPropertyMapInfo const* ECDbClassMapInfo::FindPropertyMap(ECN::ECPropertyId rootPropertyId, Utf8CP accessString) const
    {
    for (ECDbPropertyMapInfo const* kp : GetPropertyMaps(false))
        {
...
ECDbPropertyMapInfo* ECDbClassMapInfo::CreatePropertyMap(ECDbPropertyPath const& propertyPath)
    {
    std::unique_ptr<ECDbPropertyMapInfo> propertyMap = std::unique_ptr<ECDbPropertyMapInfo>(new ECDbPropertyMapInfo(*this, propertyPath));
    ECDbPropertyMapInfo* p = propertyMap.get();
...
ECDbPropertyMapInfo* ECDbClassMapInfo::CreatePropertyMap(ECN::ECPropertyId rootPropertyId, Utf8CP accessString, std::vector<ECDbSqlColumn const*> const& columns)
    {
    ECDbPropertyPath const* propertyPath = m_map.FindPropertyPath(rootPropertyId, accessString);
    if (propertyPath == nullptr)
        propertyPath = m_map.CreatePropertyPath(rootPropertyId, accessString);

    ECDbPropertyMapInfo* prop = CreatePropertyMap(*propertyPath);
