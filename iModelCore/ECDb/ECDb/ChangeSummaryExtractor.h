...
struct InstanceChange final {
   ...
   public:
    ...
    InstanceChange(ECInstanceId summaryId, ECInstanceKey const& keyOfChangedInstance, DbOpcode dbOpcode, bool isIndirect) : m_summaryId(summaryId), m_keyOfChangedInstance(keyOfChangedInstance), m_dbOpcode(dbOpcode), m_isIndirect(isIndirect) {}
...
struct ChangeSummaryExtractor final {
   private:
    enum class ExtractMode { InstancesOnly,
                             RelationshipInstancesOnly };
