...
public:
    ...
    public:
        ...
        void GetPrioritySortedList (std::vector<DbColumnCP>& prioritySortedList) const;
        };
...
private:
    ...
    bvector<NewClassIdColumnHook> m_afterSetClassIdHooks;
    DbIndexP             m_structArrayIndex;
    DbColumnPtrList m_systemKeys;
...
public:
    DbColumnPtrList const& GetSystemKeys () const { return m_systemKeys; }
    ...
    void AddAfterSetClassIdHook (NewClassIdColumnHook hook) { m_afterSetClassIdHooks.push_back (hook); }
...
struct DbTableConstraint : DbConstraint
    ...
    protected:
        ...
        DbTableConstraint();
    public:
        void GetColumns(bvector<DbColumnCP>& columns) const;
        void GetColumns(DbColumnPtrList& columns) const;

        bool AddColumn(DbColumnCR column);
        bool RemoveColumn(DbColumnCR column);
...
+---------------+---------------+---------------+---------------+---------------+------*/
struct DbForignKeyConstraint : DbTableConstraint
    {
    ...
    private:
        ...
        DbTableCP m_onTargetTable;
        DbColumnPtrList m_targetColumns;
    protected:
        DbForignKeyConstraint ();
        virtual bool _BuildSqlClause (Utf8StringR sql)override;
    ...
    public:
        ...
        void AddTargetColumn (DbColumnCR column);
        void SetOnDelete (ActionType cascade) { m_onDeleteAction = cascade; }
        void SetOnUpdate (ActionType cascade) { m_onUpdateAction = cascade; }
        void SetMatchType (MatchType matchType) { m_matchType = matchType; }
        void SetTargetTable (DbTableCR target) { m_onTargetTable = &target; }
        static DbForignKeyConstraintPtr Create ();
    };
/*---------------------------------------------------------------------------------------
* @bsimethod                                                    affan.khan      08/2012
+---------------+---------------+---------------+---------------+---------------+------*/
