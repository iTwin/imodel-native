...
BentleyStatus DbMap::DoMapSchemas(SchemaImportContext& ctx, bvector<ECN::ECSchemaCP> const& schemas) const
    ...
    for (auto& kvpair : ctx.GetClassMappingInfoCache())
        {
        if (SUCCESS != ClassMapper::MapUserDefinedIndexes(m_ecdb, *kvpair.first))
            return ERROR;
        }
...
BentleyStatus DbMap::CreateOrUpdateIndexesInDb(SchemaImportContext& ctx) const
    {
    //cache indexes on stack as we add new indexes to the master collection which would then invalidate the iterator
    ...
    //replicate user-defined indexes for other tables to which subclasses map (only for non-unique indexes or for unique indexes if no more than one non-virtual table is involved)
    std::map<Utf8String, DbIndexListCustomAttribute::DbIndex, CompareIUtf8Ascii> userDefinedIndexLookup;
    bset<ECClassId> classesAlreadyLookedUp;
    for (DbIndex const* index : indexes)
        ...
        if (storageDesc.GetHorizontalPartitions().size() == 1)
            continue;
        ...
        if (classesAlreadyLookedUp.find(classId) == classesAlreadyLookedUp.end())
            {
            DbIndexListCustomAttribute dbIndexListCA;
            bvector<DbIndexListCustomAttribute::DbIndex> dbIndexCAs;
            if (!ECDbMapCustomAttributeHelper::TryGetDbIndexList(dbIndexListCA, classMap->GetClass()) || SUCCESS != dbIndexListCA.GetIndexes(dbIndexCAs))
                {
                BeAssert(false && "Must have the DbIndexList CA because this is a user-defined index");
                return ERROR;
                }

            for (DbIndexListCustomAttribute::DbIndex const& dbIndexCA : dbIndexCAs)
                {
                userDefinedIndexLookup[dbIndexCA.GetName()] = dbIndexCA;
                }

            classesAlreadyLookedUp.insert(classId);
            }

        auto it = userDefinedIndexLookup.find(index->GetName());
        if (it == userDefinedIndexLookup.end())
            {
            BeAssert(false);
            return ERROR;
            }

        DbIndexListCustomAttribute::DbIndex const& baseClassIndexCA = it->second;

        DbTable const& indexTable = index->GetTable();
        for (Partition const& horizPartition : storageDesc.GetHorizontalPartitions())
            {
            DbTable const& partitionTable = horizPartition.GetTable();
            if (&indexTable == &partitionTable)
                continue;
            ...
            for (ECClassId derivedClassId : horizPartition.GetClassIds())
                ...
                Utf8String indexName(index->GetName());
                indexName.append("_").append(partitionTable.GetName());
                DbIndexListCustomAttribute::DbIndex subclassIndex(indexName, baseClassIndexCA.IsUnique(), baseClassIndexCA.GetProperties(), baseClassIndexCA.GetWhereClause());
                if (SUCCESS != ClassMapper::MapUserDefinedIndex(m_ecdb, *derivedClassMap, subclassIndex))
                    return ERROR;
                }
            }
        }
