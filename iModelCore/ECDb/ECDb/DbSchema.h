...
struct DbIndex final
    ...
    public:
        DbIndex(DbIndexId id, DbTable const& table, Utf8StringCR name, bool isUnique, std::vector<DbColumn const*> const& columns, bool addColsAreNotNullWhereExp, bool isAutoGenerated, ECN::ECClassId classId, bool appliesToSubclassesIfPartial)
            :m_id(id), m_name(name), m_table(table), m_isUnique(isUnique), m_columns(columns), m_addColsAreNotNullWhereExp(addColsAreNotNullWhereExp), m_isAutoGenerated(isAutoGenerated), m_classId(classId), m_appliesToSubclassesIfPartial(appliesToSubclassesIfPartial) { BeAssert(id.IsValid() && !name.empty() && !m_columns.empty()); }
...
public:
    ...
    DbIndex* CreateIndex(Utf8StringCR indexName, bool isUnique, std::vector<DbColumn const*> const&, bool addIsNotNullWhereExp, bool isAutoGenerated, ECN::ECClassId, bool applyToSubclassesIfPartial = true);
    DbIndex* AddIndexDef(DbIndexId, Utf8StringCR indexName, bool isUnique, std::vector<DbColumn const*> const&, bool addIsNotNullWhereExp, bool isAutoGenerated, ECN::ECClassId, bool applyToSubclassesIfPartial);
...
public:
    struct TableCollection final
        ...
        public:
            ...
            void ClearCache() const { m_tableNamesById.clear(); m_tableMapByName.clear(); }
        };
