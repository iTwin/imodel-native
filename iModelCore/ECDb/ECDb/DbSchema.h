...
struct DbIndexManager final : NonCopyableClass
    {
private:
    DbSchema& m_dbSchema;
    mutable std::map<Utf8CP, std::unique_ptr<DbIndex>, CompareIUtf8Ascii> m_indexMap;
    mutable std::vector<DbIndex const*> m_indexes;
    mutable bool m_indexesLoaded = false;

    BentleyStatus LoadIndexes() const;
    DbIndex* CreateIndex(DbIndexId, DbTable&, Utf8StringCR indexName, bool isUnique, std::vector<DbColumn const*> const&, bool addIsNotNullWhereExp, bool isAutoGenerated, ECN::ECClassId, bool applyToSubclassesIfPartial) const;

    ECDbCR GetECDb() const;
public:
    explicit DbIndexManager(DbSchema& dbSchema) : m_dbSchema(dbSchema) {}

    std::vector<DbIndex const*> const& GetIndexes() const;

    void ClearCache() const;

    BentleyStatus InsertIndex(DbIndex const&) const;

    DbIndex* CreateIndex(DbTable&, Nullable<Utf8String> const& indexName, bool isUnique, std::vector<DbColumn const*> const&, bool addIsNotNullWhereExp, bool isAutoGenerated, ECN::ECClassId, bool applyToSubclassesIfPartial = true) const;
    };
...
private:
    ...
    mutable DbIndexManager m_indexManager;
...
public:
    explicit DbSchema(ECDbCR ecdb) : m_ecdb(ecdb), m_nameGenerator("ecdb_"), m_indexManager(*this) {}
    ~DbSchema() {}
    ...
    std::vector<DbIndex const*> const& GetIndexes() const { return m_indexManager.GetIndexes(); }
    DbIndex* CreateIndex(DbTable& table, Nullable<Utf8String> const& indexName, bool isUnique, std::vector<DbColumn const*> const& cols, bool addIsNotNullWhereExp, bool isAutoGenerated, ECN::ECClassId classId, bool applyToSubclassesIfPartial = true) { return m_indexManager.CreateIndex(table, indexName, isUnique, cols, addIsNotNullWhereExp, isAutoGenerated, classId, applyToSubclassesIfPartial); }
