...
struct DbSchemaNameGenerator final {
   ...
   public:
    ...
    void Initialize(uint32_t lastId) {
        BeAssert(IsValid());
        m_lastId = lastId;
    }
...
struct DbColumn final {
   public:
    ...
    struct CreateParams final {
       ...
       public:
        ...
        static Utf8String ColumnNameFromAccessString(Utf8StringCR accessString) {
            Utf8String colName(accessString);
   ...
   public:
    ...
    DbColumnId GetId() const {
        BeAssert(m_id.IsValid() && "DbColumn::GetId must not be called if the DbColumn is not persisted yet");
        return m_id;
    }
    void SetId(DbColumnId id) {
        BeAssert(!m_id.IsValid());
        BeAssert(id.IsValid());
        m_id = id;
    }
...
struct ForeignKeyDbConstraint final : DbConstraint {
   ...
   public:
    ...
    DbTable const& GetReferencedTable() const {
        BeAssert(!m_referencedTableColumns.empty());
        return m_referencedTableColumns[0]->GetTable();
    }
...
struct DbTable final {
   ...
   public:
    ...
    DbTableId GetId() const {
        BeAssert(m_id.IsValid() && "Must not call DbTable::GetId on unpersisted DbTable");
        return m_id;
    }
    void SetId(DbTableId id) {
        BeAssert(!m_id.IsValid());
        BeAssert(id.IsValid());
        m_id = id;
    }
    ...
    DbColumn const& GetECClassIdColumn() const {
        BeAssert(m_classIdColumn != nullptr);
