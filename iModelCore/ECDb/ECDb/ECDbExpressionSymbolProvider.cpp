...
ExpressionStatus ECDbExpressionSymbolProvider::GetClassId(EvaluationResult& evalResult, void* contextP, EvaluationResultVector& args) {
    ...
    if (!args[0].IsECValue() || !args[0].GetECValue()->IsString() || !args[1].IsECValue() || !args[1].GetECValue()->IsString()) {
...
ExpressionStatus ECDbExpressionSymbolProvider::GetRelatedInstanceQueryFormatOld(Utf8StringR query, ECEntityClassCP& relatedClass, ECDbExpressionEvaluationContext const& context, ECInstanceListCR instanceData, EvaluationResult const& arg) {
    ...
    static Utf8CP selectQueryFormat =
        "SELECT related.%%s FROM %%s this, %s relationship, %s related "
        "WHERE this." ECDBSYS_PROP_ECInstanceId
        "=? AND "
        "      this." ECDBSYS_PROP_ECInstanceId "=relationship.[%s] AND this." ECDBSYS_PROP_ECClassId
        "=relationship.[%s] AND "
...
static ExpressionStatus CreateRelatedInstanceQueryFrom3Args(Utf8StringR query, EvaluationResultVector const& args) {
    ...
    query = Utf8PrintfString(
        "SELECT related.%%s FROM %%s this, [%s].[%s] relationship, [%s].[%s] related "
        "WHERE this." ECDBSYS_PROP_ECInstanceId
        "=? AND "
        "      this." ECDBSYS_PROP_ECInstanceId "=relationship.[%s] AND this." ECDBSYS_PROP_ECClassId
        "=relationship.[%s] AND "
