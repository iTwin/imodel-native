...
struct ClassMap {
   ...
   public:
    ...
    virtual ~ClassMap() {}
    void Modified() {
        if (m_state == ObjectState::Persisted) m_state = ObjectState::Modified;
    }
    ...
    TClassMap const& GetAs() const {
        BeAssert(dynamic_cast<TClassMap const*>(this) != nullptr);
        return *static_cast<TClassMap const*>(this);
    }
    ...
    bool IsMixin() const {
        ECN::ECEntityClassCP entityClass = m_ecClass.GetEntityClassCP();
        return entityClass != nullptr && entityClass->IsMixin();
    }
    ...
    TablePerHierarchyHelper const* GetTphHelper() const {
        BeAssert(m_tphHelper != nullptr);
        return m_tphHelper.get();
    }
    ...
    ClassMappingStatus Map(SchemaImportContext& importCtx, ClassMappingInfo const& info) {
        ClassMappingContext ctx(importCtx, info);
        return _Map(ctx);
    }
    BentleyStatus Save(SchemaImportContext&, DbMapSaveContext&);
    BentleyStatus Update(SchemaImportContext& ctx);
    void SetTable(DbTable& newTable) {
        m_tables.clear();
        AddTable(newTable);
    }
    void AddTable(DbTable& newTable) {
        BeAssert(std::find(begin(m_tables), end(m_tables), &newTable) == end(m_tables));
