...
BentleyStatus RemapManager::CleanAddedProperty(RemapInfosForClass& remapInfo, AddedProperty& addedProperty, std::set<ECPropertyId>& cleanedPropertyIds) {  // clean that property by name in all of the derived classes
...
BentleyStatus RemapManager::RestoreAndProcessCleanedPropertyMaps(SchemaImportContext& ctx) {
    ...
    for (auto& pair : m_cleanedMappingInfo) {
        ...
        for (auto& cleaned : pair.second) {
            ...
            if (!Enum::Contains(PropertyMap::Type::SingleColumnData, propMapType)) {  // we always expect this to be single column because the access string is fully qualified
...
void RemapManager::SortCircularRemappedColumnInfos(std::vector<std::vector<RemappedColumnInfo*>>& circularUpdates, std::map<Utf8String, RemappedColumnInfo*>& remainingItemsToSort) {
    ...
    while (successInLastRun && !remainingItemsToSort.empty()) {
        ...
        while (true) {
            ...
            if (nextPtr == remainingItemsToSort.end()) {  // failed to find the next element of the circle
                ...
                for (auto& v : circularUpdate) {  // re-add the items we have taken so far
