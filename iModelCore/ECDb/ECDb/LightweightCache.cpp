...
// @bsimethod
//---------------------------------------------------------------------------------------
LightweightCache::LightweightCache(TableSpaceSchemaManager const& manager) : m_schemaManager(manager) {
    Clear();
}
...
LightweightCache::ClassIdsPerTableMap const& LightweightCache::LoadHorizontalPartitions(ECN::ECClassId classId) const {
    ...
    Utf8String sql;
    sql.Sprintf("SELECT ch.ClassId, t.Name FROM [%s]." TABLE_ClassHasTablesCache
                " ct"
                " INNER JOIN [%s]." TABLE_ClassHierarchyCache
                " ch ON ch.ClassId = ct.ClassId"
...
// @bsimethod
//---------------------------------------------------------------------------------------
CachedStatementPtr LightweightCache::GetCachedStatement(Utf8CP sql) const {
    return m_schemaManager.GetECDb().GetImpl().GetCachedSqliteStatement(sql);
}

//---------------------------------------------------------------------------------------
// @bsimethod
//---------------------------------------------------------------------------------------
BeMutex& LightweightCache::GetECDbMutex() const {
    return m_schemaManager.GetECDb().GetImpl().GetMutex();
}

//---------------------------------------------------------------------------------------
// @bsimethod
//---------------------------------------------------------------------------------------
ECDb& LightweightCache::GetECDbR() const {
    return const_cast<ECDb&>(m_schemaManager.GetECDb());
}
...
std::unique_ptr<StorageDescription> StorageDescription::Create(ClassMap const& classMap, LightweightCache const& lwmc) {
    ...
    if (classMap.GetType() == ClassMap::Type::RelationshipEndTable) {
        ...
        for (ForeignKeyPartitionView::Partition const* partition : fkView->GetPartitions(true /*onlyPhysical*/)) {
            ...
            for (auto const& kvpair : lwmc.GetConstraintClassesForRelationshipClass(classId)) {
    ...
    for (DbTable const* table : lwmc.GetVerticalPartitionsForClass(classId)) {
        if (table->GetType() == DbTable::Type::Virtual || table->GetType() == DbTable::Type::Overflow)
            continue;
...
//@bsimethod
//------------------------------------------------------------------------------------------
Partition::Partition(Partition const& rhs) : m_table(rhs.m_table), m_partitionClassIds(rhs.m_partitionClassIds), m_inversedPartitionClassIds(rhs.m_inversedPartitionClassIds), m_hasInversedPartitionClassIds(rhs.m_hasInversedPartitionClassIds) {}
...
//@bsimethod
//------------------------------------------------------------------------------------------
Partition::Partition(Partition&& rhs) : m_table(std::move(rhs.m_table)), m_partitionClassIds(std::move(rhs.m_partitionClassIds)), m_inversedPartitionClassIds(std::move(rhs.m_inversedPartitionClassIds)), m_hasInversedPartitionClassIds(std::move(rhs.m_hasInversedPartitionClassIds)) {
