...
bool InstancesTable::ContainsInstance(ECClassId classId, ECInstanceId instanceId) const {
    ...
    BeAssert(result == BE_SQLITE_ROW || result == BE_SQLITE_DONE);
...
void ValuesTable::CreateTable() {
    ...
    Utf8PrintfString sql(
        "Id INTEGER PRIMARY KEY, ClassId INTEGER NOT NULL, InstanceId INTEGER NOT NULL, AccessString TEXT NOT NULL, OldValue BLOB, NewValue BLOB, "
        "FOREIGN KEY(ClassId,InstanceId) REFERENCES %s ON DELETE CASCADE ON UPDATE NO ACTION",
        m_instancesTable.GetNameNoPrefix().c_str());
...
ECN::ECClassId ChangeExtractor::GetRelEndClassId(ChangeIterator::RowEntry const& rowEntry, RelationshipClassMapCR relationshipClassMap, ECInstanceId relationshipInstanceId, ECN::ECRelationshipEnd relEnd, ECInstanceId relEndInstanceId) const {
    ...
    if (endIsInOneClass) {
        ...
        // should work, but doesn't for link tables
