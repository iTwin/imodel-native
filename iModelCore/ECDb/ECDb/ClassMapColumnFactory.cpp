...
//@bsimethod
//-----------------------------------------------------------------------------------------
bool ColumnMaps::IsColumnInUse(DbColumn const& column) const {
...
void ColumnMaps::Insert(SingleColumnDataPropertyMap const& propertyMap) {
    Insert(propertyMap.GetAccessString(), propertyMap.GetColumn());
}
...
void ClassMapColumnFactory::EvaluateIfPropertyGoesToOverflow(uint32_t columnsRequired, SchemaImportContext& ctx) const {
    ...
    if (requiredRemainingColumns > nSharedColumns) {  // no need to check, we know there won't be enough columns
        m_putCurrentPropertyToOverflow = true;
        return;
    }
...
bool ClassMapColumnFactory::IsColumnInUse(DbColumn const& column) const {
    return GetColumnMaps()->IsColumnInUse(column);
}
//------------------------------------------------------------------------------------------
//@bsimethod
//-----------------------------------------------------------------------------------------
...
bool ClassMapColumnFactory::IsCompatible(DbColumn const& avaliableColumn, DbColumn::Type type, DbColumn::CreateParams const& params) const {
    if (DbColumn::IsCompatible(avaliableColumn.GetType(), type)) {
        if (m_primaryOrJoinedTable->GetType() == DbTable::Type::Existing || (avaliableColumn.GetConstraints().HasNotNullConstraint() == params.AddNotNullConstraint() &&
...
bool ClassMapColumnFactory::IsColumnUsedByAnyDerivedClass(DbColumn const& column, SchemaImportContext& ctx) const {
    ...
    if (!ecClass.HasId()) {  // Not-yet-persisted class
        BeAssert(false);
