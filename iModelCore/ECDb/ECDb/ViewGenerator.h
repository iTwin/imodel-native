   ...
   private:
    ...
    struct Context {
       ...
       public:
        ...
        TContext& GetAs() {
            BeAssert(dynamic_cast<TContext*>(this) != nullptr);
    ...
    struct ECClassViewContext final : Context {
       ...
       public:
        ...
        void AddViewColumnName(Utf8StringCR propAccessString) {
            BeAssert(MustCaptureViewColumnNames());
            if (!MustCaptureViewColumnNames()) return;
            BeAssert(!propAccessString.empty());
            m_viewColumnNameList.push_back(&propAccessString);
        }
...
struct ConstraintECClassIdJoinInfo final {
   ...
   public:
    ...
    ConstraintECClassIdPropertyMap const& GetConstraintECClassIdPropMap() const {
        BeAssert(RequiresJoin());
        return *m_propertyMap;
    }
    DbColumn const& GetPrimaryECInstanceIdColumn() const {
        BeAssert(RequiresJoin());
        return *m_primaryECClassIdCol;
    }
    DbColumn const& GetPrimaryECClassIdColumn() const {
        BeAssert(RequiresJoin());
        return *m_primaryECClassIdCol;
    }
    DbColumn const& GetForignECInstanceIdColumn() const {
        BeAssert(RequiresJoin());
        return *m_foreignECInstanceIdCol;
    }
