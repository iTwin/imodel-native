...
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
InstanceReader& ECDb::GetInstanceReader() const {
    return m_pimpl->GetInstanceReader();
}

//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
InstanceWriter& ECDb::GetInstanceWriter() const {
    return m_pimpl->GetInstanceWriter();
}

//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
InstanceRepository& ECDb::GetInstanceRepository() const {
    return m_pimpl->GetInstanceRepository();
}
//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
...
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
ECDb::SettingsManager const& ECDb::GetECDbSettingsManager() const {
    return m_pimpl->m_settingsManager;
}
...
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
DbResult ECDb::_OnDbAttached(Utf8CP fileName, Utf8CP dbAlias) const {
    return m_pimpl->OnDbAttached(fileName, dbAlias);
}

//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
DbResult ECDb::_OnDbDetached(Utf8CP dbAlias) const {
    return m_pimpl->OnDbDetached(dbAlias);
}
...
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
BentleyStatus ECDb::ResetInstanceIdSequence(BeBriefcaseId briefcaseId, IdSet<ECN::ECClassId> const* ecClassIgnoreList) {
    return m_pimpl->ResetInstanceIdSequence(briefcaseId, ecClassIgnoreList);
}

//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
SchemaManager const& ECDb::Schemas() const {
...
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
ECN::IECSchemaLocater& ECDb::GetSchemaLocater() const {
...
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
ECN::IECClassLocater& ECDb::GetClassLocater() const {
...
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
bool ECDb::IsChangeCacheAttached() const {
    return m_pimpl->IsChangeCacheAttached();
}

//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
bool ECDb::TryGetChangeCacheFileName(BeFileNameR changeCachePath) const {
    return m_pimpl->TryGetChangeCacheFileName(changeCachePath);
}
//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
DbResult ECDb::AttachChangeCache(BeFileNameCR changeCachePath) const {
    return m_pimpl->GetChangeManager().AttachChangeCacheFile(changeCachePath, true);
}

//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
DbResult ECDb::DetachChangeCache() const {
    return m_pimpl->GetChangeManager().DetachChangeCacheFile();
}

//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
DbResult ECDb::CreateChangeCache(ECDbR changeCache, BeFileNameCR changeCachePath) const {
    return m_pimpl->GetChangeManager().CreateChangeCacheFile(changeCache, changeCachePath);
}
...
//---------------+---------------+---------------+---------------+---------------+------
// static
BeFileName ECDb::GetDefaultChangeCachePath(Utf8CP ecdbPath) {
    return ChangeManager::DetermineDefaultCachePath(ecdbPath);
}
...
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
void ECDb::AddECDbCacheClearListener(IECDbCacheClearListener& listener) {
    m_pimpl->AddECDbCacheClearListener(listener);
}

//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
void ECDb::RemoveECDbCacheClearListener(IECDbCacheClearListener& listener) {
    m_pimpl->RemoveECDbCacheClearListener(listener);
}

//--------------------------------------------------------------------------------------
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+------
ProfileVersion const& ECDb::GetECDbProfileVersion() const {
    return m_pimpl->GetProfileManager().GetProfileVersion();
}
