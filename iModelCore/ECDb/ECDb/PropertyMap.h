...
struct PropertyMapContainer final : ISupportsPropertyMapVisitor {
   ...
   public:
    ...
    size_t IndexOf(PropertyMap const& propertyMap) const;
...
struct PropertyMap : RefCountedBase, ISupportsPropertyMapVisitor {
   ...
   protected:
    ...
    PropertyMap(Type type, PropertyMap const& parentPropertyMap, ECN::ECPropertyCR ecProperty, Utf8StringCR accessString)
        : m_type(type), m_classMap(parentPropertyMap.GetClassMap()), m_ecProperty(ecProperty), m_parentPropertyMap(&parentPropertyMap), m_propertyAccessString(accessString) {}
   ...
   public:
    ...
    //! Gets the most root property id for the given property, does not throw assertions for unmapped properties, like the method above
    bool TryGetRootPropertyId(ECN::ECPropertyId& id) const;
...
struct DataPropertyMap : PropertyMap {
   ...
   protected:
    ...
    DataPropertyMap(Type kind, PropertyMap const& parentPropertyMap, ECN::ECPropertyCR ecProperty, bool appendToAccessString)
        : PropertyMap(kind, parentPropertyMap, ecProperty, appendToAccessString ? Utf8PrintfString("%s.%s", parentPropertyMap.GetAccessString().c_str(), ecProperty.GetName().c_str()) : ecProperty.GetName()) {}
...
struct SingleColumnDataPropertyMap : DataPropertyMap {
   private:
    mutable DbColumn const* m_column;
   ...
   public:
    ...
    void Remap(DbColumn const& column) const { m_column = &column; }
};
...
struct StructPropertyMapBuilder final {
   ...
   public:
    ...
    explicit StructPropertyMapBuilder(StructPropertyMap& prop);
    bool IsValid() const { return m_propMap != nullptr; }

    BentleyStatus AddMember(RefCountedPtr<DataPropertyMap> propertyMap);

    bool IsFinished() const { return m_isFinished; }
    StructPropertyMap const& GetPropertyMapUnderConstruction() {
        BeAssert(IsValid());
        return *m_propMap;
    }

    BentleyStatus Finish();
    RefCountedPtr<StructPropertyMap> GetPropertyMap() {
        BeAssert(m_isFinished);
        return m_propMap;
    }
};
