...
m_mapStrategy(mapStrategy), m_isMapToVirtualTable(IClassMap::IsAbstractECClass(ecClass)), m_replaceEmptyTableWithEmptyView(true), m_classHasTimeStamp(NULL),m_useSharedColumnStrategy (false)
    {
...
void ClassMapInfo::InitializeFromSchema ()
    ...
    InitializeFromClassHasTimeStamp();
...
//+---------------+---------------+---------------+---------------+---------------+------
void ClassMapInfo::InitializeFromClassHasTimeStamp()
    {
    auto classHint = ClassHintReader::ReadClassHasTimeStamp(m_ecClass);
    if (classHint == nullptr)
        return;
   
    WString propertyName;
    ECValue v;
    if (classHint->GetValue(v, L"property") == ECOBJECTS_STATUS_Success && !v.IsNull())
        {
        propertyName = v.GetString();
        ECPropertyP dateTimeProperty = m_ecClass.GetPropertyP(propertyName);
        if (NULL == dateTimeProperty)
            {
            BeAssert(false && "ClassHasTimeStamp Property Not Found in ECClass");
            return;
            }
        if (dateTimeProperty->GetTypeName().Equals(L"dateTime"))
            {
            m_classHasTimeStamp = dateTimeProperty;
            }
        else
            {
