...
// @bsimethod
//---------------------------------------------------------------------------------------
BentleyStatus ComputeHashPropertyMapVisitor::_Visit(SingleColumnDataPropertyMap const& propertyMap) const {
    m_hashBuilder.UpdateString(propertyMap.GetAccessString());
    m_hashBuilder.UpdateString(propertyMap.GetTable().GetName());
    m_hashBuilder.UpdateString(propertyMap.GetColumn().GetName());
    return SUCCESS;
}
//---------------------------------------------------------------------------------------
// @bsimethod
//---------------------------------------------------------------------------------------
BentleyStatus ComputeHashPropertyMapVisitor::_Visit(SystemPropertyMap const& propertyMap) const {
    for (SystemPropertyMap::PerTableIdPropertyMap const* childMap : propertyMap.GetDataPropertyMaps()) {
        m_hashBuilder.UpdateString(childMap->GetAccessString());
        m_hashBuilder.UpdateString(childMap->GetTable().GetName());
        m_hashBuilder.UpdateString(childMap->GetColumn().GetName());
    }
    return SUCCESS;
}
//---------------------------------------------------------------------------------------
// @bsimethod
//---------------------------------------------------------------------------------------
BentleyStatus ComputeHashPropertyMapVisitor::_Visit(CompoundDataPropertyMap const& propertyMap) const {
    for (DataPropertyMap const* childPropMap : propertyMap) {
        if (SUCCESS != childPropMap->AcceptVisitor(*this))
            return ERROR;
    }
