...
BentleyStatus ViewGenerator::DropUpdatableViews(ECDbCR ecdb)
    ...
    Statement stmt;
    stmt.Prepare(ecdb,
                 "SELECT ('DROP VIEW IF EXISTS _' || ec_Schema.NamespacePrefix || '_' || ec_Class.Name) "
...
BentleyStatus ViewGenerator::DropECClassViews(ECDbCR ecdb)
    ...
    Statement stmt;
    stmt.Prepare(ecdb,
                 "SELECT ('DROP VIEW IF EXISTS [' || ec_Schema.NamespacePrefix || '.' || ec_Class.Name || '];') FROM ec_Class "
...
BentleyStatus ViewGenerator::GenerateUpdateTriggerSetClause(NativeSqlBuilder& sql, ClassMap const& baseClassMap, ClassMap const& derivedClassMap)
    ...
    baseClassMap.GetPropertyMaps().Traverse(
        ...
        if (basePropertyMap->GetType() == PropertyMap::Type::Navigation)
            {
            if (!static_cast<NavigationPropertyMap const*>(basePropertyMap)->IsSupportedInECSql())
                return;
            }
...
BentleyStatus ViewGenerator::AppendSystemPropMaps(NativeSqlBuilder& viewSql, RelationshipClassMapCR relationMap, DbTable const& contextTable)
    ...
    //SourceECClassId--------------------------------------
    RelConstraintECClassIdPropertyMap const* classIdPropMap = relationMap.GetSourceECClassIdPropMap();
