...
// @bsimethod
//---------------------------------------------------------------------------------------
ECDb const& CachedConnection::GetPrimaryDb() const {
    return m_cache.GetPrimaryDb();
}
...
void QueryHelper::ReadBlob(ECDbCR conn, RunnableRequestBase& runnableRequest) {
    ...
    if (conn.OpenBlobIO(blobIo, *classCP, request.GetAccessString().c_str(), request.GetInstanceId(), false) != SUCCESS) {
        setError(QueryResponse::Status::Error_BlobIO_OpenFailed, SqlPrintfString("BlobIO: unable to open blob for classname '%s' , accessString '%s' for instanceId '%s'",
                                                                                 request.GetClassName().c_str(), request.GetAccessString().c_str(), request.GetInstanceId().ToHexStr().c_str())
                                                                     .GetUtf8CP());
        return;
    }
...
ConcurrentQueryMgr::ConcurrentQueryMgr(ECDbCR ecdb) {
    m_impl = new Impl(ecdb);
}
ConcurrentQueryMgr::~ConcurrentQueryMgr() {
    delete m_impl;
}
QueryResponse::Future ConcurrentQueryMgr::Enqueue(QueryRequest::Ptr request) {
    return m_impl->Enqueue(std::move(request));
}
void ConcurrentQueryMgr::Enqueue(QueryRequest::Ptr request, OnCompletion onCompletion) {
    m_impl->Enqueue(std::move(request), onCompletion);
}
//---------------------------------------------------------------------------------------
// @bsimethod
//---------------------------------------------------------------------------------------
    ...
    for (auto& kp : m_params) {
        ...
        switch (param.GetType()) {
            case ECSqlParam::Type::Boolean:
                st = stmt.BindBoolean(index, param.GetValueBool());
                break;
            case ECSqlParam::Type::Double:
                st = stmt.BindDouble(index, param.GetValueDouble());
                break;
            case ECSqlParam::Type::Id:
                st = stmt.BindId(index, param.GetValueId());
                break;
            ...
            case ECSqlParam::Type::Integer:
                st = stmt.BindInt(index, param.GetValueInt());
                break;
            case ECSqlParam::Type::Long:
                st = stmt.BindInt64(index, param.GetValueLong());
                break;
            case ECSqlParam::Type::Null:
                st = stmt.BindNull(index);
                break;
            case ECSqlParam::Type::Point2d:
                st = stmt.BindPoint2d(index, param.GetValuePoint2d());
                break;
            case ECSqlParam::Type::Point3d:
                st = stmt.BindPoint3d(index, param.GetValuePoint3d());
                break;
            case ECSqlParam::Type::String:
                st = stmt.BindText(index, param.GetValueString().c_str(), IECSqlBinder::MakeCopy::Yes);
                break;
            case ECSqlParam::Type::Blob: {
                auto buffer = param.GetValueBlob();
                st = stmt.BindBlob(index, &buffer[0], (int)buffer.size(), IECSqlBinder::MakeCopy::Yes);
                break;
            }
...
// @bsimethod
//---------------------------------------------------------------------------------------
ConcurrentQueryMgr::Config::Config() : m_quota(DEFAULT_QUOTA_MAX_TIME, DEFAULT_QUOTA_MAX_MEM),
                                       ...
                                       m_statementCacheSizePerWorker(DEFAULT_STATEMENT_CACHE_SIZE_PER_WORKER),
                                       m_memoryMapFileSize(0) {
}
