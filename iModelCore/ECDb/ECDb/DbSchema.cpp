...
BentleyStatus DbSchema::SynchronizeExistingTables() {
    ...
    for (Utf8StringCR tableName : tableNames) {
        ...
        for (auto const& kvPair : newColumnList) {
...
BentleyStatus DbSchema::InsertTable(DbTable const& table) const {
    ...
    CachedStatementPtr stmt = GetCachedStatement("INSERT INTO main." TABLE_Table "(ParentTableId,Name,Type,ExclusiveRootClassId,Id) VALUES(?,?,?,?,?)");
    if (stmt == nullptr)
        return ERROR;
...
BentleyStatus DbSchema::InsertColumn(DbColumn const& column, int columnOrdinal, int primaryKeyOrdinal) const {
    CachedStatementPtr stmt = GetCachedStatement("INSERT INTO main." TABLE_Column "(TableId,Name,Type,IsVirtual,Ordinal,NotNullConstraint,UniqueConstraint,CheckConstraint,DefaultConstraint,CollationConstraint,OrdinalInPrimaryKey,ColumnKind,Id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
    if (stmt == nullptr)
        return ERROR;
...
// @bsimethod
//---------------------------------------------------------------------------------------
CachedStatementPtr DbSchema::GetCachedStatement(Utf8CP sql) const {
...
bool DbTable::RemoveIndexDef(Utf8StringCR indexName) {
    auto&& indexIt = std::find_if(m_indexes.begin(), m_indexes.end(), [&](std::unique_ptr<DbIndex> const& index) { return index->GetName().Equals(indexName); });
