...
BentleyStatus DbSchema::LoadIndexes() const
    ...
    while (stmt->Step() == BE_SQLITE_ROW)
        ...
        bool isUnique = DbSchemaPersistenceManager::IsTrue(stmt->GetValueInt(3));
        bool addNotNullWhereExp = DbSchemaPersistenceManager::IsTrue(stmt->GetValueInt(4));
        bool isAutoGenerated = DbSchemaPersistenceManager::IsTrue(stmt->GetValueInt(5));
        ECClassId classId = !stmt->IsColumnNull(6) ? stmt->GetValueId<ECClassId>(6) : ECClassId();
        bool appliesToSubclassesIfPartial = DbSchemaPersistenceManager::IsTrue(stmt->GetValueInt(7));
...
std::map<Utf8String, DbTableId, CompareIUtf8Ascii> DbSchema::GetExistingTableMap() const
    ...
    CachedStatementPtr stmt = m_ecdb.GetCachedStatement("SELECT Name, Id FROM ec_Table WHERE Type=" SQLVAL_INT_DbTable_Type_Existing);
    if (stmt == nullptr)
        {
        BeAssert(false);
        return persistedTableMap;
...
BentleyStatus DbSchema::InsertTable(DbTable const& table) const
    ...
    stmt->BindInt(4, DbSchemaPersistenceManager::BoolToSqlInt(table.GetPersistenceType() == PersistenceType::Virtual));
...
BentleyStatus DbSchema::UpdateTable(DbTable const& table) const
    ...
    stmt->BindInt(3, DbSchemaPersistenceManager::BoolToSqlInt(table.GetPersistenceType() == PersistenceType::Virtual));
    if (auto primaryTable = table.GetParentOfJoinedTable())
...
BentleyStatus DbSchema::UpdateColumn(DbColumn const& column, int columnOrdinal, int primaryKeyOrdinal) const
    ...
    stmt->BindInt(3, DbSchemaPersistenceManager::BoolToSqlInt(column.GetPersistenceType() == PersistenceType::Virtual));
    stmt->BindInt64(4, columnOrdinal);
    stmt->BindInt(5, DbSchemaPersistenceManager::BoolToSqlInt(column.GetConstraints().HasNotNullConstraint()));
    stmt->BindInt(6, DbSchemaPersistenceManager::BoolToSqlInt(column.GetConstraints().HasUniqueConstraint()));
...
BentleyStatus DbSchema::InsertColumn(DbColumn const& column, int columnOrdinal, int primaryKeyOrdinal) const
    ...
    stmt->BindInt(5, DbSchemaPersistenceManager::BoolToSqlInt(column.GetPersistenceType() == PersistenceType::Virtual));
    stmt->BindInt64(6, columnOrdinal);
    stmt->BindInt(7, DbSchemaPersistenceManager::BoolToSqlInt(column.GetConstraints().HasNotNullConstraint()));
    stmt->BindInt(8, DbSchemaPersistenceManager::BoolToSqlInt(column.GetConstraints().HasUniqueConstraint()));
...
BentleyStatus DbSchema::InsertIndex(DbIndex const& index) const
    ...
    stmt->BindInt(4, DbSchemaPersistenceManager::BoolToSqlInt(index.GetIsUnique()));
    stmt->BindInt(5, DbSchemaPersistenceManager::BoolToSqlInt(index.IsAddColumnsAreNotNullWhereExp()));

    stmt->BindInt(6, DbSchemaPersistenceManager::BoolToSqlInt(index.IsAutoGenerated()));
    if (index.HasClassId())
        stmt->BindId(7, index.GetClassId());

    stmt->BindInt(8, DbSchemaPersistenceManager::BoolToSqlInt(index.AppliesToSubclassesIfPartial()));
...
BentleyStatus DbSchema::LoadColumns(DbTable& table) const
    ...
    while (BE_SQLITE_ROW == stmt->Step())
        ...
        const PersistenceType persistenceType = DbSchemaPersistenceManager::IsTrue(stmt->GetValueInt(3)) ? PersistenceType::Virtual : PersistenceType::Persisted;

        const bool hasNotNullConstraint = DbSchemaPersistenceManager::IsTrue(stmt->GetValueInt(notNullColIx));
        const bool hasUniqueConstraint = DbSchemaPersistenceManager::IsTrue(stmt->GetValueInt(uniqueColIx));
...
BentleyStatus DbSchema::LoadTable(Utf8StringCR name, DbTable*& tableP) const
    ...
    PersistenceType persistenceType = DbSchemaPersistenceManager::IsTrue(stmt->GetValueInt(2)) ? PersistenceType::Virtual : PersistenceType::Persisted;
