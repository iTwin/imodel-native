...
BentleyStatus DbSchemaPersistenceManager::CreateOrUpdateIndexes(ECDbCR ecdb, DbSchema const& dbSchema)
    ...
    Statement indexDdlStmt;
    if (BE_SQLITE_OK != indexDdlStmt.Prepare(ecdb, "SELECT sql FROM sqlite_master WHERE name=? AND type='index'"))
        {
        BeAssert(false && "Index definition is not valid");
        return ERROR;
        }

    std::vector<DbIndex const*> indexesToDrop;
    bmap<DbIndex const*, Utf8String> indexesToCreate;
    ...
    for (std::unique_ptr<DbIndex> const& indexPtr : dbSchema.GetIndexes())
        ...
        //detect duplicate index definitions, and ignore all but one
        ...
        if (BE_SQLITE_OK != indexDdlStmt.BindText(1, index.GetName().c_str(), Statement::MakeCopy::No))
            {
            BeAssert(false);
            return ERROR;
            }

        bool ddlHasChanged = true;
        if (BE_SQLITE_ROW == indexDdlStmt.Step())
            {
            Utf8CP previousDdl = indexDdlStmt.GetValueText(0);
            ddlHasChanged = !ddl.EqualsIAscii(previousDdl);

            if (ddlHasChanged && LOG.isSeverityEnabled(NativeLogging::SEVERITY::LOG_DEBUG))
                LOG.debugv("Index: %s - Old DDL: %s - New DDL: %s", index.GetName().c_str(), previousDdl, ddl.c_str());
            }

        indexDdlStmt.Reset();
        indexDdlStmt.ClearBindings();

        if (ddlHasChanged)
            {
            //Recreate the index because it has changed
            indexesToDrop.push_back(&index);
            indexesToCreate[&index].assign(ddl);
            }
        }

    indexDdlStmt.Finalize();

    StopWatch dropIndexesTimer(true);
    for (DbIndex const* index : indexesToDrop)
        {
        Utf8String dropIndexSql;
        dropIndexSql.Sprintf("DROP INDEX IF EXISTS [%s]", index->GetName().c_str());
        if (BE_SQLITE_OK != ecdb.ExecuteSql(dropIndexSql.c_str()))
            {
            BeAssert(false && "Dropping index failed");
            return ERROR;
            }
        }
    dropIndexesTimer.Stop();

    StopWatch createIndexesTimer(true);
    for (bpair<DbIndex const*, Utf8String> const& kvPair : indexesToCreate)
        {
        Utf8CP ddl = kvPair.second.c_str();
        if (BE_SQLITE_OK != ecdb.ExecuteSql(ddl))
            {
            DbIndex const& index = *kvPair.first;
    ...
    createIndexesTimer.Stop();
    timer.Stop();
    if (LOG.isSeverityEnabled(NativeLogging::SEVERITY::LOG_INFO))
        {
        LOG.infov("Creating and recreating indexes on EC tables took %.4f ms. Details: Dropped %" PRIu64 " indices in %.4f ms. "
                  "Created %" PRIu64 " indices in %.4f ms.",
                  timer.GetElapsedSeconds() * 1000.0, (uint64_t) indexesToDrop.size(), dropIndexesTimer.GetElapsedSeconds() * 1000.0,
                  (uint64_t) indexesToCreate.size(), createIndexesTimer.GetElapsedSeconds() * 1000.0);
        }
