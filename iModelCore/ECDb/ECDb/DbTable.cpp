...
void DbTable::GeneratePrimaryKeyColumns (Utf8CP primaryKeyColumnName, bool mapToSecondaryTable)
    ...
    if (!mapToSecondaryTable)
        ...
        m_systemKeys.push_back (primaryKeyColumn);
        }
    else
        {
        ...
        auto structIndex = DbIndex::Create (nullptr, *this, true);
        ...
        primaryKeyConstraint.AddColumn (*primaryKeyColumn);
        structIndex->AddColumn (*primaryKeyColumn);
        structIndex->AddColumn (*parentECIdColumn);
        structIndex->AddColumn (*propertyIdColumn);
        structIndex->AddColumn (*arrayIndexColumn);
        AddIndex (*structIndex);

        m_systemKeys.push_back (primaryKeyColumn);
        m_systemKeys.push_back (parentECIdColumn);
        m_systemKeys.push_back (propertyIdColumn);
        m_systemKeys.push_back (arrayIndexColumn);
...
void DbTable::GetCreateTableSQL (Utf8StringR sql)
    ...
    auto const& systemKeys = GetSystemKeys();
    for (auto& primaryKeyColumn : systemKeys)
        if (primaryKeyColumn != nullptr)
            AppendColumnDDL (sql, *primaryKeyColumn, primaryKeyColumn == systemKeys.front ());

    auto classIdColumn = GetClassIdColumnPtr ();
    if (classIdColumn != nullptr)
        AppendColumnDDL (sql, *classIdColumn);

    std::vector<DbColumnCP> prioritySortedList;

    GetColumns ().GetPrioritySortedList (prioritySortedList);
    for (auto column : prioritySortedList)
        {
...
DbResult DbTable::CreateEmptyView (BeSQLiteDbR db) const
    ...
    for (auto column : GetColumns ())
        ...
        for (auto& primaryKeyColumn : systemKeys)
            {
            if (primaryKeyColumn.get() == column)
                {
                skipColumn = true;
                break; // skip columns that we've already added
                }
...
void DbTable::AddClassIdColumn (DbColumnPtr classIdColumn)
    ...
    for (auto& afterSetClassIdHook : m_afterSetClassIdHooks)
        afterSetClassIdHook (classIdColumn);
    }
...
void DbTable::DbColumnCollection::Add (DbColumnPtr& newColumn)
    ...
    if (newColumn->GetPriority () == 0)
        newColumn->SetPriority (static_cast<int>(Size ()));
    }
...
//+---------------+---------------+---------------+---------------+---------------+------
void DbTable::DbColumnCollection::GetPrioritySortedList (std::vector<DbColumnCP>& prioritySortedList) const
    {
    prioritySortedList.clear ();
    prioritySortedList.reserve (Size());
    for (auto const key : *this)
        prioritySortedList.push_back (key);

    std::sort (prioritySortedList.begin (), prioritySortedList.end (), [] (DbColumnCP a, DbColumnCP b) { return a->GetPriority () < b->GetPriority (); });
    }
//---------------------------------------------------------------------------------------
// @bsimethod                                                 Krischan.Eberle     05/2014
//+---------------+---------------+---------------+---------------+---------------+------
...
* @bsimethod                                                    affan.khan      08/2012
+---------------+---------------+---------------+---------------+---------------+------*/
void DbTableConstraint::GetColumns(bvector<DbColumnCP>& columns)const
    {
    columns.clear();
...
* @bsimethod                                                    affan.khan      08/2012
+---------------+---------------+---------------+---------------+---------------+------*/
bool DbTableConstraint::AddColumn(DbColumnCR column) 
    {
    m_columns.push_back (&const_cast<DbColumnR>(column));
    return true;
    }
...
+---------------+---------------+---------------+---------------+---------------+------*/
bool DbTableConstraint::RemoveColumn(DbColumnCR column)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
DbForignKeyConstraint::DbForignKeyConstraint () : 
m_onDeleteAction (DbForignKeyConstraint::ActionType::NotSpecified), m_onUpdateAction (DbForignKeyConstraint::ActionType::NotSpecified), m_onTargetTable (nullptr), m_matchType (DbForignKeyConstraint::MatchType::NotSpecified)
    {
    }
...
* @bsimethod                                                    affan.khan      09/2014
+---------------+---------------+---------------+---------------+---------------+------*/
bool DbForignKeyConstraint::_BuildSqlClause (Utf8StringR sql) 
    {
    DbColumnPtrList columns;
    GetColumns (columns);

    BeAssert (columns.size () == m_targetColumns.size () && "Source and Target columns must be of same number");
    BeAssert (columns.size () > 0 && "Source and Target columns must have atleast one column");
    BeAssert (m_onTargetTable != nullptr && "Target table must be set");

    if (columns.size () != m_targetColumns.size () || columns.empty() || m_targetColumns.empty() || m_onTargetTable == nullptr)
        {
        return false;
        }
    ...
    for (auto itor = columns.begin (); itor != columns.end (); ++itor)
        {
        sql.append ("[");
        sql.append ((*itor)->GetName ());
        sql.append ("]");
        if (itor != (columns.end () - 1))
            sql.append (", ");
        }

    sql.append (") ");
    sql.append ("REFERENCES ");
    sql.append (m_onTargetTable->GetName());
    sql.append (" (");
    for (auto itor = m_targetColumns.begin (); itor != m_targetColumns.end (); ++itor)
        {
        sql.append ("[");
        sql.append ((*itor)->GetName ());
        sql.append ("]");
        if (itor != (m_targetColumns.end () - 1))
            sql.append (", ");
        }

    sql.append (")");

    if (m_matchType != MatchType::NotSpecified)
        {
        sql.append (" MATCH ");
        sql.append (ToSQL (m_matchType));
        }

    if (m_onDeleteAction != ActionType::NotSpecified)
        {
        sql.append (" ON DELETE ");
        sql.append (ToSQL (m_onDeleteAction));
        }

    if (m_onUpdateAction != ActionType::NotSpecified)
        {
        sql.append (" ON UPDATE ");
        sql.append (ToSQL (m_onUpdateAction));
        }
...
* @bsimethod                                                    affan.khan      09/2014
+---------------+---------------+---------------+---------------+---------------+------*/
Utf8CP DbForignKeyConstraint::ToSQL (DbForignKeyConstraint::ActionType actionType)
    {
    switch (actionType)
        {
...
+---------------+---------------+---------------+---------------+---------------+------*/
//static
Utf8CP DbForignKeyConstraint::ToSQL (DbForignKeyConstraint::MatchType matchType)
    {
    switch (matchType)
        {
        case MatchType::Full:
...
+---------------+---------------+---------------+---------------+---------------+------*/
//static
DbForignKeyConstraint::ActionType DbForignKeyConstraint::ParseActionType (WCharCP actionType)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
//static
DbForignKeyConstraint::MatchType DbForignKeyConstraint::ParseMatchType (WCharCP matchType)
    {
...
* @bsimethod                                                    affan.khan      09/2014
+---------------+---------------+---------------+---------------+---------------+------*/
void DbForignKeyConstraint::AddTargetColumn (DbColumnCR column)
    {
    m_targetColumns.push_back (const_cast<DbColumnP>(&column));
    }
...
* @bsimethod                                                    affan.khan      09/2014
+---------------+---------------+---------------+---------------+---------------+------*/
DbForignKeyConstraintPtr DbForignKeyConstraint::Create ()
    {
    return new DbForignKeyConstraint ();
    }
