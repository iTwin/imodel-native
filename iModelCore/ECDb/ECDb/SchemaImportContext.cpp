...
struct IndexCompareInfo
    {
    ECDbSqlIndex const* m_index;
    Utf8String m_finalWhereClause;

    IndexCompareInfo() : m_index(nullptr) {}
    IndexCompareInfo(ECDbSqlIndex const& index, Utf8CP finalWhereClause) : m_index(&index), m_finalWhereClause(finalWhereClause) {}

    bool operator<(IndexCompareInfo const& rhs) const
        {
        if (m_index == nullptr || rhs.m_index == nullptr)
            {
        ...
        int diff = m_index->GetTable().GetName().CompareToI(rhs.m_index->GetTable().GetName());
        if (diff != 0)
            return diff < 0;

        if (m_index->GetIsUnique() != rhs.m_index->GetIsUnique())
            return !m_index->GetIsUnique(); //false is considered less than true

        const size_t lhsColCount = m_index->GetColumns().size();
        diff = (int) (lhsColCount - rhs.m_index->GetColumns().size());
        if (diff != 0)
            return diff < 0;

        for (size_t i = 0; i < lhsColCount; i++)
            {
            diff = m_index->GetColumns()[i]->GetName().CompareToI(rhs.m_index->GetColumns()[i]->GetName());
            if (diff != 0)
                return diff < 0;
            }

        return m_finalWhereClause.CompareToI(rhs.m_finalWhereClause) < 0;
        }
    };
...
BentleyStatus SchemaImportECDbMapDb::CreateOrUpdateIndicesInDb(ECDbR ecdb) const
    {
    bset<IndexCompareInfo> comparableIndexDefs;
    for (std::unique_ptr<ECDbSqlIndex>& indexPtr : m_indexes)
        ...
        IndexCompareInfo info(index, whereClause.ToString());
        auto it = comparableIndexDefs.find(info);
        if (it != comparableIndexDefs.end())
            ...
            Utf8CP existingIndexName = it->m_index->GetName().c_str();
        ...
        comparableIndexDefs.insert(info);

        NativeSqlBuilder ddl;
        if (SUCCESS != BuildCreateIndexDdl(ddl, ecdb, index, whereClause))
            return ERROR;
...
// @bsimethod                                                Krischan.Eberle 10/2015
//---------------------------------------------------------------------------------------
BentleyStatus SchemaImportECDbMapDb::BuildCreateIndexDdl(NativeSqlBuilder& ddl, ECDbCR ecdb, ECDbSqlIndex const& index, NativeSqlBuilder const& whereClause) const
    {
    ddl.Append("CREATE ");
