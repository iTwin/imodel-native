...
//---------------------------------------------------------------------------------------
ECDbSqlIndex* SchemaImportECDbMapDb::CreateIndex(ECDbSqlTable& table, Utf8CP indexName, bool isUnique, std::vector<ECDbSqlColumn const*> const& columns, Utf8CP primaryWhereClause, bool isAutoGenerated, ECN::ECClassId classId, bool applyToSubclassesIfPartial)
    {
    if (columns.empty())
        {
    ...
    return CacheIndex(m_coreMapDb.GetManagerR().GetIdGenerator().NextIndexId(),
...
// @bsimethod                                                    Affan.Khan        09/2014
//---------------------------------------------------------------------------------------
ECDbSqlIndex* SchemaImportECDbMapDb::CreateIndex(ECDbSqlTable& table, Utf8CP indexName, bool isUnique, std::vector<Utf8CP> const& columnNames, Utf8CP primaryWhereClause, bool isAutoGenerated, ECN::ECClassId classId, bool applyToSubclassesIfPartial)
    {
    ...
    return CreateIndex(table, indexName, isUnique, columns, primaryWhereClause, isAutoGenerated, classId, applyToSubclassesIfPartial);
    }

//---------------------------------------------------------------------------------------
// @bsimethod                                                    Affan.Khan        09/2014
//---------------------------------------------------------------------------------------
ECDbSqlIndex* SchemaImportECDbMapDb::CacheIndex(ECDbIndexId id, ECDbSqlTable& table, Utf8CP indexName, bool isUnique, std::vector<ECDbSqlColumn const*> const& columns, Utf8CP primaryWhereClause, bool isAutoGenerated, ECN::ECClassId classId, bool applyToSubclassesIfPartial) const
    {
    if (columns.empty())
        return nullptr;
...
BentleyStatus SchemaImportECDbMapDb::ReadIndexInfosFromDb(ECDbCR ecdb) const
    ...
    while (stmt->Step() == BE_SQLITE_ROW)
        ...
        if (nullptr == CacheIndex(id, *table, name, isUnique, columns, primaryWhereClause, isAutoGenerated, classId, appliesToSubclassesIfPartial))
            return ERROR;
        }
