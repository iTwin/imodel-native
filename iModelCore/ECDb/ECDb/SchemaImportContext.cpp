...
BentleyStatus SchemaImportECDbMapDb::ReadIndexInfosFromDb(ECDbCR ecdb) const
    ...
    ecdb.GetCachedStatement(stmt, "SELECT I.Id, T.Name, I.Name, I.IsUnique, I.PrimaryWhereClause, I.IsAutoGenerated, I.ClassId, I.AppliesToSubclassesIfPartial FROM " EC_INDEX_TableName " I INNER JOIN ec_Table T ON T.Id = I.TableId");
    if (stmt == nullptr)
        return ERROR;
...
BentleyStatus SchemaImportECDbMapDb::InsertIndexInfoIntoDb(ECDbCR ecdb, ECDbSqlIndex const& index) const
    {
    CachedStatementPtr stmt = nullptr;
    ecdb.GetCachedStatement(stmt, "INSERT INTO " EC_INDEX_TableName "(Id,TableId,Name,IsUnique,PrimaryWhereClause,IsAutoGenerated,ClassId,AppliesToSubclassesIfPartial) VALUES(?,?,?,?,?,?,?,?)");
    if (stmt == nullptr)
        return ERROR;
