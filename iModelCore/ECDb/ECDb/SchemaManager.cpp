...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
bvector<ECSchemaCP> SchemaManager::GetSchemas(bool loadSchemaEntities, Utf8CP tableSpace) const {
    return m_dispatcher->GetSchemas(loadSchemaEntities, tableSpace);
}

/*---------------------------------------------------------------------------------------
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
bool SchemaManager::ContainsSchema(Utf8StringCR schemaNameOrAlias, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->ContainsSchema(schemaNameOrAlias, mode, tableSpace);
}

/*---------------------------------------------------------------------------------------
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ECSchemaCP SchemaManager::GetSchema(Utf8StringCR schemaNameOrAlias, bool loadSchemaEntities, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetSchema(schemaNameOrAlias, loadSchemaEntities, mode, tableSpace);
}

/*---------------------------------------------------------------------------------------
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ECClassCP SchemaManager::GetClass(Utf8StringCR schemaNameOrAlias, Utf8StringCR className, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetClass(schemaNameOrAlias, className, mode, tableSpace);
}

/*---------------------------------------------------------------------------------------
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ClassMapStrategy SchemaManager::GetClassMapStrategy(Utf8StringCR schemaNameOrAlias, Utf8StringCR className, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetClassMapStrategy(schemaNameOrAlias, className, mode, tableSpace);
}

/*---------------------------------------------------------------------------------------
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ECClassCP SchemaManager::GetClass(ECClassId ecClassId, Utf8CP tableSpace) const {
    return m_dispatcher->GetClass(ecClassId, tableSpace);
}

/*---------------------------------------------------------------------------------------
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ECN::ECClassCP SchemaManager::FindClass(Utf8StringCR className, Utf8CP tableSpace) const {
    return m_dispatcher->FindClass(className, tableSpace);
}
/*---------------------------------------------------------------------------------------
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ECClassId SchemaManager::GetClassId(Utf8StringCR schemaNameOrAlias, Utf8StringCR className, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetClassId(schemaNameOrAlias, className, mode, tableSpace);
}

/*---------------------------------------------------------------------------------------
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
bool SchemaManager::OwnsSchema(ECN::ECSchemaCR schema) const {
    return m_dispatcher->OwnsSchema(schema);
}
...
// @bsimethod
//---------------------------------------------------------------------------------------
ECDerivedClassesList const* SchemaManager::GetDerivedClassesInternal(ECClassCR ecClass, Utf8CP tableSpace) const {
    return m_dispatcher->GetDerivedClasses(ecClass, tableSpace);
}

//---------------------------------------------------------------------------------------
// @bsimethod
//---------------------------------------------------------------------------------------
Nullable<ECN::ECDerivedClassesList> SchemaManager::GetAllDerivedClassesInternal(ECClassCR ecClass, Utf8CP tableSpace) const {
    return m_dispatcher->GetAllDerivedClasses(ecClass, tableSpace);
}

//---------------------------------------------------------------------------------------
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
ECEnumerationCP SchemaManager::GetEnumeration(Utf8StringCR schemaNameOrAlias, Utf8StringCR enumName, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetEnumeration(schemaNameOrAlias, enumName, mode, tableSpace);
}

//---------------------------------------------------------------------------------------
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
KindOfQuantityCP SchemaManager::GetKindOfQuantity(Utf8StringCR schemaNameOrAlias, Utf8StringCR koqName, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetKindOfQuantity(schemaNameOrAlias, koqName, mode, tableSpace);
}

//---------------------------------------------------------------------------------------
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
ECUnitCP SchemaManager::GetUnit(Utf8StringCR schemaNameOrAlias, Utf8StringCR unitName, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetUnit(schemaNameOrAlias, unitName, mode, tableSpace);
}

//---------------------------------------------------------------------------------------
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
ECFormatCP SchemaManager::GetFormat(Utf8StringCR schemaNameOrAlias, Utf8StringCR formatName, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetFormat(schemaNameOrAlias, formatName, mode, tableSpace);
}

//---------------------------------------------------------------------------------------
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
UnitSystemCP SchemaManager::GetUnitSystem(Utf8StringCR schemaNameOrAlias, Utf8StringCR unitSystemName, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetUnitSystem(schemaNameOrAlias, unitSystemName, mode, tableSpace);
}

//---------------------------------------------------------------------------------------
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
PhenomenonCP SchemaManager::GetPhenomenon(Utf8StringCR schemaNameOrAlias, Utf8StringCR phenName, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetPhenomenon(schemaNameOrAlias, phenName, mode, tableSpace);
}

//---------------------------------------------------------------------------------------
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
PropertyCategoryCP SchemaManager::GetPropertyCategory(Utf8StringCR schemaNameOrAlias, Utf8StringCR catName, SchemaLookupMode mode, Utf8CP tableSpace) const {
    return m_dispatcher->GetPropertyCategory(schemaNameOrAlias, catName, mode, tableSpace);
}
...
void SchemaManager::ClearCache() const {
    return m_dispatcher->ClearCache();
}
...
ECSchemaPtr SchemaManager::_LocateSchema(SchemaKeyR key, SchemaMatchType matchType, ECSchemaReadContextR schemaContext) {
    return m_dispatcher->LocateSchema(key, matchType, schemaContext, nullptr);
}
...
BentleyStatus SchemaManager::CreateClassViewsInDb(bvector<ECN::ECClassId> const& ecclassids) const {
    return Main().CreateClassViews(ecclassids);
}

//---------------------------------------------------------------------------------------
// @bsimethod
//---------------------------------------------------------------------------------------
SchemaChangeEvent& SchemaManager::OnBeforeSchemaChanges() const {
    return Main().OnBeforeSchemaChanges();
}

//---------------------------------------------------------------------------------------
// @bsimethod
//---------------------------------------------------------------------------------------
SchemaChangeEvent& SchemaManager::OnAfterSchemaChanges() const {
    return Main().OnAfterSchemaChanges();
};
...
BentleyStatus SchemaManager::RepopulateCacheTables() const {
    return Main().RepopulateCacheTables();
}

//---------------------------------------------------------------------------------------
// @bsimethod
//---------------------------------------------------------------------------------------
BentleyStatus SchemaManager::UpgradeECInstances() const {
    return Main().UpgradeECInstances() == BE_SQLITE_OK ? SUCCESS : ERROR;
}
...
MainSchemaManager const& SchemaManager::Main() const {
    return GetDispatcher().Main();
}
