...
#define EXTENDEDTYPENAME_ClassId "ClassId"
#define EXTENDEDTYPENAME_TargetId "TargetId"
#define EXTENDEDTYPENAME_TargetClassId "TargetClassId"
#define EXTENDEDTYPENAME_SourceId "SourceId"
#define EXTENDEDTYPENAME_SourceClassId "SourceClassId"
...
struct ECSqlTypeInfo final {
   ...
   public:
    ...
    ECSqlTypeInfo(ECSqlTypeInfo const& rhs)
        : m_kind(rhs.m_kind), m_primitiveType(rhs.m_primitiveType), m_dateTimeInfo(rhs.m_dateTimeInfo), m_enumType(rhs.m_enumType), m_extendedTypeName(rhs.m_extendedTypeName), m_structType(rhs.m_structType), m_minOccurs(rhs.m_minOccurs), m_maxOccurs(rhs.m_maxOccurs), m_propertyMap(rhs.m_propertyMap) {}
    ECSqlTypeInfo& operator=(ECSqlTypeInfo const&);

    ECSqlTypeInfo(ECSqlTypeInfo&& rhs)
        : m_kind(std::move(rhs.m_kind)), m_primitiveType(std::move(rhs.m_primitiveType)), m_dateTimeInfo(std::move(rhs.m_dateTimeInfo)), m_enumType(std::move(rhs.m_enumType)), m_extendedTypeName(std::move(rhs.m_extendedTypeName)), m_structType(std::move(rhs.m_structType)), m_minOccurs(std::move(rhs.m_minOccurs)), m_maxOccurs(std::move(rhs.m_maxOccurs)), m_propertyMap(std::move(rhs.m_propertyMap)) {}
    ...
    DateTime::Info const& GetDateTimeInfo() const {
        BeAssert(GetPrimitiveType() == ECN::PRIMITIVETYPE_DateTime);
        return m_dateTimeInfo.Value();
    }
    ECN::ECEnumerationCP GetEnumerationType() const {
        BeAssert(IsPrimitive() || m_kind == Kind::PrimitiveArray);
        return m_enumType;
    }
    bool HasExtendedType() const { return !m_extendedTypeName.empty(); }
    Utf8StringCR GetExtendedTypeName() const {
        BeAssert(IsPrimitive() || m_kind == Kind::PrimitiveArray);
        return m_extendedTypeName;
    }

    ECN::ECStructClassCR GetStructType() const {
        BeAssert(IsStruct() || m_kind == Kind::StructArray);
        return *m_structType;
    }

    Nullable<uint32_t> GetArrayMinOccurs() const {
        BeAssert(IsArray());
        return m_minOccurs;
    }
    Nullable<uint32_t> GetArrayMaxOccurs() const {
        BeAssert(IsArray());
        return m_maxOccurs;
    }
