...
struct CompoundECSqlPreparedStatement : IECSqlPreparedStatement {
    ...
    struct ProxyECInstanceIdECSqlBinder final : IProxyECSqlBinder {
       private:
        ...
        ECSqlStatus _BindNull() override {
            m_boundValueIsNull = true;
            return GetBinder().BindNull();
        }
        ECSqlStatus _BindInt64(int64_t value) override {
            m_boundValueIsNull = false;
            return GetBinder().BindInt64(value);
        }
        ECSqlStatus _BindText(Utf8CP value, IECSqlBinder::MakeCopy makeCopy, int byteCount) override {
            m_boundValueIsNull = false;
            return GetBinder().BindText(value, makeCopy, byteCount);
        }
        ...
        void _AddBinder(IECSqlBinder& binder) override {
   ...
   public:
    ...
    SingleContextTableECSqlPreparedStatement& GetPrimaryTableECSqlStatement() {
        BeAssert(!m_statements.empty());
    ...
    struct PrepareInfo final {
       ...
       public:
        PrepareInfo(ECSqlPrepareContext& ctx, InsertStatementExp const& exp)
            : m_ctx(ctx), m_exp(exp), m_classNameExp(*exp.GetClassNameExp()), m_propNameListExp(*exp.GetPropertyNameListExp()), m_valuesListExp(*exp.GetValuesExp()), m_ecsqlRenderContext(Exp::ECSqlRenderContext::Mode::GenerateNameForUnnamedParameter) {}
    ...
    struct PrepareInfo final {
       ...
       public:
        PrepareInfo(ECSqlPrepareContext& ctx, UpdateStatementExp const& exp)
            : m_ctx(ctx), m_exp(exp), m_classNameExp(*exp.GetClassNameExp()), m_assignmentListExp(*exp.GetAssignmentListExp()), m_whereExp(exp.GetWhereClauseExp()), m_optionsExp(exp.GetOptionsClauseExp()), m_ecsqlRenderContext(Exp::ECSqlRenderContext::Mode::GenerateNameForUnnamedParameter) {}
        ...
        DbTable const* GetSingleTableInvolvedInAssignmentClause() const {
            BeAssert(IsSingleTableInvolvedInAssignmentClause());
            return m_assignmentExpsByTable.begin()->first;
        }
    };
