...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
DbResult JsonUpdater::Update(ECInstanceId instanceId, BeJsConst json) const
    {
    if (!m_isValid)
    ...
    if (!json.isObject() || json.size() == 0)
        {
        LOG.errorv("JsonUpdater failure. The JSON to update must be a non-empty object, but it was: %s", json.Stringify().c_str());
        return BE_SQLITE_ERROR;
        }

    m_statement.ClearBindings();

    BeAssert(json.isObject() || json.isNull());
    bool failed = json.ForEachProperty([&](Utf8CP memberName, BeJsConst memberJson) {
        ...
        if (ECSqlStatus::Success != JsonECSqlBinder::BindValue(m_statement.GetBinder((int) bindingInfo.GetParameterIndex()), memberJson, bindingInfo.GetProperty(), m_ecdb.GetClassLocater()))
            ...
            LOG.errorv("JsonUpdater failure. Could not bind JSON member '%s' to parameter %" PRIu32 " for ECJSON %s. Underlying ECSQL: %s",
                       memberName, bindingInfo.GetParameterIndex(), json.Stringify().c_str(), m_statement.GetECSql());
            return true;
            }
        return false;
    });
