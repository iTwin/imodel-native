...
ECSqlStatus ECSqlStatement::IsNavigationPropertyValid(Utf8StringCR propertyName, const ECN::ECClassId& relationshipECClassId) const
    {
    if (relationshipECClassId == ECN::ECClassId())
        return ECSqlStatus::Success;

    // Validation for navigation properties should only be done for insert/update statement when the PRAGMA validate_ecsql_writes is enabled
    ...
    if (stmt.Prepare(*GetECDb(), query.GetUtf8CP(), nullptr, false) != ECSqlStatus::Success)
        {
        LOG.errorv("Failed to prepare ECSql statement to validate navigation property '%s' with relationship class Id '%s'.", propertyName.c_str(), relationshipECClassId.ToString().c_str());
        return ECSqlStatus::InvalidECSql;
        }

    // There might be navigation properties with the same name in different classes, so we need to check if the relationship class matches
    // the provided relationshipECClassId.
    while (stmt.Step() == BE_SQLITE_ROW)
        ...
        if (relClass == nullptr)
            {
            LOG.errorv("Trying to bind a navigation property '%s' with relationship class Id '%s', but the class does not exist in the schema.",
                propertyName.c_str(), relationshipECClassId.ToString().c_str());
            return ECSqlStatus::InvalidECSql;
            }

        // Found a matching relationship class for given navigation property
        if (relClass->GetId() == relationshipECClassId)
            return ECSqlStatus::Success;

        // If the relationship class is not the same, check if it is a derived class of the provided relationshipECClassId
        const auto derivedClasses = GetECDb()->Schemas().GetAllDerivedClassesInternal(*relClass);
        if (derivedClasses.IsValid() 
            && std::any_of(derivedClasses.Value().begin(), derivedClasses.Value().end(), [&relationshipECClassId](const auto& checkClass) { return checkClass->GetId() == relationshipECClassId; }))
            return ECSqlStatus::Success;
        }

    // If we reach here, it means the relationship class ID supplied does not match any valid navigation property
    LOG.errorv("The ECSql statement contains a relationship class Id '%s' which does not correspond to any valid navigation property.", relationshipECClassId.ToString().c_str());
    return ECSqlStatus::InvalidECSql;
    }
