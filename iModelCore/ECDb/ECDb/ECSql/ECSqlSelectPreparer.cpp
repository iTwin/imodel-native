...
ECSqlStatus ECSqlSelectPreparer::PrepareDerivedPropertyExp(NativeSqlBuilder::List& nativeSqlSnippets, ECSqlPrepareContext& ctx, DerivedPropertyExp const& exp, size_t referenceSqliteSnippetCount)
    {
    ComputedExp const* innerExp = exp.GetExpression<ComputedExp>();
    ValueExp const* innerValueExp = exp.GetExpression<ValueExp>();
    BooleanExp const* innerBooleanExp = exp.GetExpression<BooleanExp>();
    ...
    else if (!alias.empty() && exp.GetExpression<ComputedExp>()->GetType() != Exp::Type::NavValueCreationFunc)
        {
    ...
    if (ctx.GetCurrentScope().IsRootScope() && ctx.GetCreateField())
        ...
        if (exp.GetExpression<ComputedExp>()->GetType() == Exp::Type::NavValueCreationFunc ||
            (exp.GetExpression<ComputedExp>()->GetType() == Exp::Type::PropertyName && exp.GetExpression<ComputedExp>()->GetAs<PropertyNameExp>().IsPropertyRef() && exp.GetExpression<ComputedExp>()->GetAs<PropertyNameExp>().GetPropertyRef()->LinkedTo().GetExpression<ComputedExp>()->GetType() == Exp::Type::NavValueCreationFunc))
...
BentleyStatus ECSqlSelectPreparer::ValidateSelectClauseItems(ECSqlPrepareContext& ctx, SelectClauseExp const& lhs, SelectClauseExp const& rhs)
    ...
    for (size_t i = 0; i < count; i++)
        ...
        ECSqlTypeInfo const& lhsTypeInfo = lhsDerivedPropExp->GetExpression<ComputedExp>()->GetTypeInfo();
        ECSqlTypeInfo const& rhsTypeInfo = rhsDerivedPropExp->GetExpression<ComputedExp>()->GetTypeInfo();
