...
ECSqlStatus ECSqlSelectPreparer::PrepareDerivedPropertyExp(NativeSqlBuilder::List& nativeSqlSnippets, ECSqlPrepareContext& ctx, DerivedPropertyExp const& exp, size_t referenceSqliteSnippetCount)
    ...
    // else if (!exp.GetSubqueryAlias().empty()) {
    //     resultSet.push_back(exp.GetSubqueryAlias());
    // }
    else
        {
        for (NativeSqlBuilder& snippet : nativeSqlSnippets)
    ...
    if (ctx.GetCurrentScope().IsRootScope() && ctx.GetCreateField())
        ...
        if (
            innerExp->GetType() == Exp::Type::SqlColumnName ||
            innerExp->GetType() == Exp::Type::PropertyName && 
            innerExp->GetAs<PropertyNameExp>().IsPropertyRef() && 
            innerExp->GetAs<PropertyNameExp>().GetPropertyRef()->LinkedTo().GetExpression()->GetType() == Exp::Type::SqlColumnName
            )
            return ECSqlStatus::Success;
