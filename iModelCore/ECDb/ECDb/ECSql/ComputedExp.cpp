    ...
    if (mode == Exp::FinalizeParseMode::AfterFinalizingChildren)
        ...
        if (rhs->GetTypeInfo().GetKind() == ECSqlTypeInfo::Kind::Varies)
            ...
            if (expWithVaryingTypeInfo != nullptr)
                {
                ctx.GetECDbImpl().ReportIssue(ECDb::IssueSeverity::Error, "Only one operand of the expression '%s' can be an expression list.", ToECSql().c_str());
...
Exp::FinalizeParseStatus BinaryBooleanExp::CanCompareTypes (ECSqlParseContext& ctx, ComputedExp const& lhs, ComputedExp const& rhs) const
    ...
    if (!lhsIsParameter && !rhsIsParameter && !lhsTypeInfo.Matches(rhsTypeInfo, &canCompareErrorMessage))
        ...
        if (canCompareErrorMessage.empty ())
            ctx.GetECDbImpl().ReportIssue(ECDb::IssueSeverity::Error, "Type mismatch in expression '%s'.", ToECSql ().c_str ());
        else
            ctx.GetECDbImpl().ReportIssue(ECDb::IssueSeverity::Error, "Type mismatch in expression '%s': %s", ToECSql ().c_str (), canCompareErrorMessage.c_str ());
    ...
    if (m_op == BooleanSqlOperator::Like || m_op == BooleanSqlOperator::NotLike)
        ...
        if ((!lhsIsParameter && (!lhsTypeInfo.IsPrimitive() || lhsTypeInfo.GetPrimitiveType() != PRIMITIVETYPE_String)) ||
            ...
            {
            ctx.GetECDbImpl().ReportIssue(ECDb::IssueSeverity::Error, "Type mismatch in expression '%s'. LIKE operator only supported with string operands.", ToECSql().c_str());
            return FinalizeParseStatus::Error;
            }
        }
    ...
    if (lhsTypeInfo.IsPoint () || rhsTypeInfo.IsPoint ())
        ...
        switch (m_op)
            ...
            default:
                ctx.GetECDbImpl().ReportIssue(ECDb::IssueSeverity::Error, "Type mismatch in expression '%s'. Operator not supported with point operands.", ToECSql ().c_str ());
                return FinalizeParseStatus::Error;
            }
        }
    ...
    if (lhsTypeInfo.IsGeometry () || rhsTypeInfo.IsGeometry ())
        ...
        if ((m_op != BooleanSqlOperator::Is && m_op != BooleanSqlOperator::IsNot) ||
            ...
            ctx.GetECDbImpl().ReportIssue(ECDb::IssueSeverity::Error, "Type mismatch in expression '%s'. For geometry properties only IS NULL and IS NOT NULL is supported.", ToECSql ().c_str ());
            return FinalizeParseStatus::Error;
            }
        }
    ...
    if (lhsTypeKind == ECSqlTypeInfo::Kind::PrimitiveArray ||
        ...
        if ((m_op != BooleanSqlOperator::Is && m_op != BooleanSqlOperator::IsNot) ||
            ...
            {
            //for prim arrays only is null and arrays not supported in where expressions for now
            ctx.GetECDbImpl().ReportIssue(ECDb::IssueSeverity::Error, "Type mismatch in expression '%s'. For primitive arrays only IS NULL and IS NOT NULL is supported.", ToECSql ().c_str ());
            return FinalizeParseStatus::Error;
            }
        }
    ...
    if (lhsTypeKind == ECSqlTypeInfo::Kind::Struct || lhsTypeKind == ECSqlTypeInfo::Kind::StructArray ||
        ...
        ctx.GetECDbImpl().ReportIssue(ECDb::IssueSeverity::Error, "Type mismatch in expression '%s'. Operator not supported with structs and struct arrays.", ToECSql ().c_str ());
        return FinalizeParseStatus::Error;
        }
...
Exp::FinalizeParseStatus UnaryPredicateExp::_FinalizeParsing(ECSqlParseContext& ctx, FinalizeParseMode mode)
    ...
    if (valueExp->IsParameterExp () || (!valueExpTypeInfo.IsBoolean() && !valueExpTypeInfo.IsExactNumeric()))
        {
        ctx.GetECDbImpl().ReportIssue(ECDb::IssueSeverity::Error, "Type mismatch in expression '%s'. Unary predicates can only have expressions of boolean or integral type and cannot be parametrized.", ToECSql().c_str());
        return FinalizeParseStatus::Error;
        }
