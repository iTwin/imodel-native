...
ECSqlStatus PrimitiveMappedToSingleColumnECSqlField::_Init (ECSqlStatusContext& statusContext)
    ...
    if (m_ecsqlColumnInfo.GetDataType ().GetPrimitiveType () == PRIMITIVETYPE_DateTime)
        ...
        if (StandardCustomAttributeHelper::GetDateTimeInfo (dateTimeInfo, *property) != ECOBJECTS_STATUS_Success)
            return SetError (ECSqlStatus::UserError, "Could not read DateTimeInfo custom attribute from the corresponding ECProperty.");
...
DPoint2d PrimitiveMappedToSingleColumnECSqlField::_GetPoint2D () const
    {
    SetError (ECSqlStatus::UserError, "GetPoint2D cannot be called for columns which are not of the Point2D type.");
...
DPoint3d PrimitiveMappedToSingleColumnECSqlField::_GetPoint3D () const
    {
    SetError (ECSqlStatus::UserError, "GetPoint3D cannot be called for columns which are not of the Point3D type.");
...
IECSqlArrayValue const& PrimitiveMappedToSingleColumnECSqlField::_GetArray () const
    {
    SetError (ECSqlStatus::UserError, "GetArray cannot be called for primitive columns.");
...
IECSqlStructValue const& PrimitiveMappedToSingleColumnECSqlField::_GetStruct () const
    {
    SetError (ECSqlStatus::UserError, "GetStruct cannot be called for primitive columns.");
...
bool PrimitiveMappedToSingleColumnECSqlField::CanGetValue (PrimitiveType testType, PrimitiveType requestedType, Utf8CP typeName, Utf8CP getValueMethodName) const
    ...
    if (fieldDataType == testType && requestedType != testType)
        ...
        SetError (ECSqlStatus::UserError, error.c_str ());
        return false;
        }
