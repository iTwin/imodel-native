...
class OOO_DLLPUBLIC_DBTOOLS OSQLParseNode {
   ...
   public:
    ...
    sal_Bool isRule() const {
        return (m_eNodeType == SQL_NODE_RULE) || (m_eNodeType == SQL_NODE_LISTRULE) || (m_eNodeType == SQL_NODE_COMMALISTRULE || m_eNodeType == SQL_NODE_DOTLISTRULE);
    }
    ...
    void setTokenValue(const Utf8String& rString) {
        if (isToken()) m_aNodeValue = rString;
    }
