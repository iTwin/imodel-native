...
SQLParseNodeParameter::SQLParseNodeParameter(const RefCountedPtr<XConnection>& _rxConnection,
    ...
    : rLocale(_rLocale), aMetaData(_rxConnection), pParser(NULL), pSubQueryHistory(new QueryNameSet), xFormatter(_xFormatter), xField(_xField), m_rContext(_pContext ? (const IParseContext&)(*_pContext) : (const IParseContext&)OSQLParser::s_aDefaultContext), cDecSep(_cDecSep), bQuote(_bQuote), bInternational(_bIntl), bPredicate(_bPredicate), bParseToSDBCLevel(_bParseToSDBC) {}
...
OSQLParseNode::OSQLParseNode(const sal_Char* pNewValue,
    ...
    : m_pParent(NULL), m_aNodeValue(pNewValue), m_eNodeType(eNewNodeType), m_nNodeID(nNewNodeID) {
...
OSQLParseNode::OSQLParseNode(Utf8String const& _rNewValue,
                             SQLNodeType eNewNodeType,
                             sal_uInt32 nNewNodeID)
    : m_pParent(NULL), m_aNodeValue(_rNewValue), m_eNodeType(eNewNodeType), m_nNodeID(nNewNodeID) {
    RTL_LOGFILE_CONTEXT_AUTHOR(aLogger, "parse", "Ocke.Janssen@sun.com", "OSQLParseNode::OSQLParseNode");
#if !NDEBUG
    m_ruleId = getKnownRuleID();
#endif
            ...
            if (rString.size()) {
                switch (rString[rString.size() - 1]) {
                    ...
                    case '.':
                        break;
                    default:
                        if (!rParam.aMetaData.getCatalogSeparator().size() || rString[rString.size() - 1] != Utf8StringHelper::toChar(rParam.aMetaData.getCatalogSeparator()))
                            rString.append(" ");
                        break;
                }
            }
