...
struct PropertyPath final {
   ...
   public:
    ...
    void Insert(size_t position, Utf8StringCR propertyName, int arrayIndex = Location::NOT_ARRAY) { m_path.insert((m_path.begin() + position), Location(propertyName, arrayIndex)); }
    Location const& First() const {
        BeAssert(!m_path.empty());
        return m_path[0];
    }
    Location const& Last() const {
        BeAssert(!m_path.empty());
    ...
    PropertyPath Take(size_t k) const;
    void SetPropertyDef(size_t i, ECN::ECPropertyCR property) { m_path[i].SetProperty(property); }
    void SetClassMap(ClassMap const& classMap) {
        if (&classMap == m_classMap)
            return;
...
struct Exp {
   public:
    ...
    struct Collection final {
       ...
       public:
        ...
        struct const_iterator final {
            ...
            using value_type = TExp;
            using difference_type = std::ptrdiff_t;
            using pointer = TExp*;
            using reference = TExp&;
    ...
    enum class FinalizeParseStatus { Completed,
                                     NotCompleted,
                                     Error };
    ...
    struct ECSqlRenderContext final {
       ...
       public:
        ...
        bmap<int, ParameterNameInfo> const& GetParameterIndexNameMap() const {
            BeAssert(m_mode == Mode::GenerateNameForUnnamedParameter);
            return m_parameterIndexNameMap;
        }
        ECSqlRenderContext& AppendToECSql(Utf8StringCR str) {
            m_ecsqlBuilder.append(str);
            return *this;
        }
        ECSqlRenderContext& AppendToECSql(Utf8CP str) {
            m_ecsqlBuilder.append(str);
            return *this;
        }
        ECSqlRenderContext& AppendToECSql(Exp const& exp) {
            exp.ToECSql(*this);
            return *this;
        }
   ...
   protected:
    enum class FinalizeParseMode { BeforeFinalizingChildren,
                                   AfterFinalizingChildren };
