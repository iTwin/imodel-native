...
ECSqlStatus ECSqlExpPreparer::PrepareRelationshipJoinExp(ECSqlPrepareContext& ctx, UsingRelationshipJoinExp const& exp)
    ...
    // if (exp.GetResolvedFromEndPoint().IsViewClass()) {
    //     r = PrepareClassRefExp(sql, ctx, exp.GetResolvedFromEndPoint().GetClassNameRef()->GetParent()->GetAs<SubqueryRefExp>());
    // } else
    ...
    // if (SUCCESS != ViewGenerator::GenerateSelectFromViewSql(
    //     relationshipView,
    //     ctx,
    //     relationshipClassNameExp.GetInfo().GetMap(),
    //     relationshipClassNameExp.GetPolymorphicInfo(),
    //     relationshipClassNameExp.DisqualifyPrimaryJoin())) {
    ...
    // sql.Append(" INNER JOIN ");
    // if (exp.GetResolvedToEndPoint().IsViewClass()) {
    //     r = PrepareClassRefExp(sql, ctx, exp.GetResolvedToEndPoint().GetClassNameRef()->GetParent()->GetAs<SubqueryRefExp>());
    // } else
...
//+---------------+---------------+---------------+---------------+---------------+------
//static
ECSqlStatus ECSqlExpPreparer::PrepareNavValueCreationFuncExp(NativeSqlBuilder::List& nativeSqlSnippets, ECSqlPrepareContext& ctx, NavValueCreationFuncExp const& exp, bool shouldUseOtherSyntax)
    {
    NativeSqlBuilder builder;
    ...
    if (shouldUseOtherSyntax)
        ...
        builder.Append("[").Append(navName).Append("Id]");
        builder.AppendComma().AppendSpace();
        builder.Append("[").Append(navName).Append("RelECClassId]");
        nativeSqlSnippets.push_back(builder);
        return ECSqlStatus::Success;
        }
    ...
    builder.Append(idNativeSql.at(0).GetSql()).AppendSpace().Append("[").Append(navName).Append("_0]");
    builder.AppendComma().AppendSpace();
    builder.Append(relECClassIdNativeSql.at(0).GetSql()).AppendSpace().Append("[").Append(navName).Append("_1]");
    builder.AppendSpace();
