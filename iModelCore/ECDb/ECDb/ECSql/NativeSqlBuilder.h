...
struct NativeSqlBuilder final {
   ...
   public:
    ...
    NativeSqlBuilder& Append(Utf8StringCR str) {
        m_nativeSql.append(str);
        return *this;
    }
    NativeSqlBuilder& Append(Utf8CP str) {
        m_nativeSql.append(str);
        return *this;
    }
    ...
    NativeSqlBuilder& AppendIf(bool condition, Utf8CP stringLiteral) {
        if (condition) Append(stringLiteral);
        return *this;
    }
    NativeSqlBuilder& AppendIIf(bool condition, Utf8CP trueStr, Utf8CP falseStr) { return condition ? Append(trueStr) : Append(falseStr); }
    NativeSqlBuilder& Replace(Utf8CP strToFind, Utf8CP replacement) {
        m_nativeSql.ReplaceAll(strToFind, replacement);
        return *this;
    }
    void Clear() {
        m_nativeSql.clear();
