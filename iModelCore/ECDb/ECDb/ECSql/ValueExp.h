...
struct CastExp final : ValueExp {
   ...
   public:
    ...
    Utf8StringCR GetCastTargetTypeName() const {
        BeAssert(!m_castTargetSchemaName.empty());
        return m_castTargetTypeName;
    }
    Utf8StringCR GetCastTargetPrimitiveType() const {
        BeAssert(m_castTargetSchemaName.empty());
        return m_castTargetTypeName;
    }
    ...
    bool NeedsCasting() const {
        BeAssert(IsComplete());
        return GetCastOperand()->GetTypeInfo() != GetTypeInfo();
    }
};
