...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+--------
PrimitiveECSqlField::PrimitiveECSqlField(IECSqlFieldReader& reader, ECSqlColumnInfo const& ecsqlColumnInfo, int columnIndex)
    : ECSqlField(reader, ecsqlColumnInfo, false, false), m_sqliteColumnIndex(columnIndex)
    {
...
void const* PrimitiveECSqlField::_GetBlob(int* blobSize) const
    {
    if (blobSize != nullptr)
        *blobSize = GetReader().GetColumnBytes(m_sqliteColumnIndex);

    return GetReader().GetValueBlob(m_sqliteColumnIndex);
    }
...
double PrimitiveECSqlField::_GetDateTimeJulianDays(DateTime::Info& metadata) const
    {
    const double jd = GetReader().GetValueDouble(m_sqliteColumnIndex);
