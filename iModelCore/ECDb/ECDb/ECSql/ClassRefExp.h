...
public:
    ClassNameExp(Utf8StringCR className, Utf8StringCR schemaAlias, Utf8CP tableSpace, std::shared_ptr<Info> info, bool isPolymorphic = true, std::unique_ptr<MemberFunctionCallExp> memberFuntionCall = nullptr, bool disqualifyPrimaryJoin = false);
...
public:
    explicit TableValuedFunctionExp (Utf8StringCR schemaName, std::unique_ptr<MemberFunctionCallExp> func, bool isPolymorphic);
public:
