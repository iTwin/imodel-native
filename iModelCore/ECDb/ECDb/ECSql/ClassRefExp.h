...
struct RangeClassInfo final {
   ...
   public:
    ...
    bool IsInherited() const {
        BeAssert(IsValid());
        return m_scope == Scope::Inherited;
    }
    RangeClassRefExp const& GetExp() const {
        BeAssert(IsValid());
        return *m_exp;
    }
};
...
struct PropertyMatchResult {
   ...
   public:
    PropertyMatchResult() : m_derivedProperty(nullptr), m_propertyMap(nullptr), m_confidence(0), m_columnInCTE(false), m_property(nullptr) {}
...
struct RangeClassRefExp : ClassRefExp {
   ...
   public:
    ...
    void SetAlias(Utf8StringCR alias) {
        m_alias = alias;
        _OnAliasChanged();
    }
};
