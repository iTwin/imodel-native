...
void FunctionCallExp::DetermineReturnType(ECDbCR ecdb) {
    ...
    const bool isCustomFunction = ecdb.GetImpl().TryGetSqlFunction(func, m_functionName.c_str(), (int)GetChildrenCount()) || !func && ecdb.GetImpl().TryGetSqlFunction(func, m_functionName.c_str(), -1);
    if (isCustomFunction) {
        switch (func->GetReturnType()) {
            case DbValueType::BlobVal:
                SetTypeInfo(ECSqlTypeInfo::CreatePrimitive(ECN::PRIMITIVETYPE_Binary));
                return;
...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
LiteralValueExp::LiteralValueExp(Utf8CP value, ECSqlTypeInfo const& typeInfo) : ValueExp(Type::LiteralValue, true), m_rawValue(value) {
    SetTypeInfo(typeInfo);
}
