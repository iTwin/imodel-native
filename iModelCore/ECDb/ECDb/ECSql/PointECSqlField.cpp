...
bool PointECSqlField::_IsNull() const
    ...
    const auto coordXValue = reader.GetValueDouble(m_xColumnIndex);
    const auto coordYValue = reader.GetValueDouble(m_yColumnIndex);
    const auto coordZValue = reader.GetValueDouble(m_zColumnIndex);
    return (reader.IsColumnNull(m_xColumnIndex) || std::isinf(coordXValue) || std::isnan(coordXValue) || reader.IsColumnNull(m_yColumnIndex) || std::isinf(coordYValue) || std::isnan(coordYValue)
        || (IsPoint3d() && (reader.IsColumnNull(m_zColumnIndex) || std::isinf(coordZValue) || std::isnan(coordZValue))));
    }
...
DPoint2d PointECSqlField::_GetPoint2d() const
    ...
    return DPoint2d::From(GetReader().GetValueDouble(m_xColumnIndex),
                          GetReader().GetValueDouble(m_yColumnIndex));
    }
...
DPoint3d PointECSqlField::_GetPoint3d() const
    ...
    return DPoint3d::From(GetReader().GetValueDouble(m_xColumnIndex),
                          GetReader().GetValueDouble(m_yColumnIndex),
                          GetReader().GetValueDouble(m_zColumnIndex));
    }
