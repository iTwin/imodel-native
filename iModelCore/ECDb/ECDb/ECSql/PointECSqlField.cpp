...
bool PointECSqlField::_IsNull() const {
    ...
    return (stmt.IsColumnNull(m_xColumnIndex) || std::isinf(coordXValue) || std::isnan(coordXValue) || stmt.IsColumnNull(m_yColumnIndex) || std::isinf(coordYValue) || std::isnan(coordYValue) || (IsPoint3d() && (stmt.IsColumnNull(m_zColumnIndex) || std::isinf(coordZValue) || std::isnan(coordZValue))));
}
