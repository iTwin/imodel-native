...
private:
    ...
    bool _IsNull() const override { return GetReader().IsColumnNull(m_sqliteColumnIndex); }

    void const* _GetBlob(int* blobSize) const override;
    bool _GetBoolean() const override { return GetReader().GetValueBoolean(m_sqliteColumnIndex); }
    ...
    double _GetDouble() const override { return GetReader().GetValueDouble(m_sqliteColumnIndex); }
    int _GetInt() const override { return GetReader().GetValueInt(m_sqliteColumnIndex); }
    int64_t _GetInt64() const override { return GetReader().GetValueInt64(m_sqliteColumnIndex); }
    Utf8CP _GetText() const override { return GetReader().GetValueText(m_sqliteColumnIndex); }
...
public:
    PrimitiveECSqlField(IECSqlFieldReader&, ECSqlColumnInfo const&, int ecsqlColumnIndex);
    ~PrimitiveECSqlField() {}
    };
