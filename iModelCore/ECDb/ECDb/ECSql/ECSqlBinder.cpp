...
// @bsimethod
//---------------------------------------------------------------------------------------
Statement& ECSqlBinder::GetSqliteStatement() const {
    return m_preparedStatement.GetSqliteStatement();
}
...
ECDbCR ECSqlBinder::GetECDb() const {
    return m_preparedStatement.GetECDb();
}
...
std::unique_ptr<IdECSqlBinder> ECSqlBinderFactory::CreateIdBinderForQuery(ECSqlPrepareContext& ctx, ECSqlTypeInfo const& typeInfo, ECSqlBinder::SqlParamNameGenerator& paramNameGen) {
    ...
    BeAssert(ctx.GetCurrentScope().GetECSqlType() == ECSqlType::Select || ctx.GetCurrentScope().GetECSqlType() == ECSqlType::Delete || (ctx.GetCurrentScope().GetECSqlType() == ECSqlType::Update && ctx.GetCurrentScope().GetExp().GetType() != Exp::Type::AssignmentList) && "ECSqlType is not supported and should have been caught before");
