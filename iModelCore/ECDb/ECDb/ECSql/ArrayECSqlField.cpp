...
ECSqlStatus ArrayECSqlField::_OnAfterStep()
    ...
    if (GetReader().IsColumnNull(m_sqliteColumnIndex))
        m_json.SetArray();
    else
        {
        Utf8CP jsonStr = GetReader().GetValueText(m_sqliteColumnIndex);
    ...
    m_value = std::make_unique<JsonECSqlValue>(GetReader().GetECDb(), m_json, GetColumnInfo());
