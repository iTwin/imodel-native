...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
DbResult JsonInserter::Insert(ECInstanceKey& key, BeJsConst json) const
    {
    if (!m_isValid)
    ...
    if (!json.isObject() && !json.isNull())
        {
        LOG.errorv("JsonInserter failure. The JSON to insert must be an object or null, but was: %s", json.Stringify().c_str());
        return BE_SQLITE_ERROR;
        }
    ...
    if (!json.isNull())
        ...
        bool failed = json.ForEachProperty([&](Utf8CP memberName, BeJsConst memberJson) {
            ...
            if (bindingMapLookupIt == m_bindingMap.end())
                ...
                if (strcmp(ECJsonSystemNames::ClassName(), memberName) == 0)
                    ...
                    if (memberJson.isNull() || !memberJson.isString() || !m_jsonClassName.EqualsIAscii(memberJson.asCString()))
                        ...
                        LOG.errorv("JsonInserter failure. The '%s' member of the JSON to insert has an invalid value: %s",
                                   ECJsonSystemNames::ClassName(), memberJson.Stringify().c_str());
                        return true;
                        }
                ...
                if (strcmp(ECJsonSystemNames::SourceClassName(), memberName) == 0 ||
                    ...
                    return false; //those two are ignored as ECSQL doesn't validate them anyways
            ...
            if (ECSqlStatus::Success != JsonECSqlBinder::BindValue(m_statement.GetBinder(bindingInfo.GetParameterIndex()), memberJson, *bindingInfo.GetProperty(), m_ecdb.GetClassLocater()))
                ...
                LOG.errorv("JsonInserter failure. Could not bind JSON member '%s' to parameter %d for EC JSON %s,",
                           memberName, bindingInfo.GetParameterIndex(), json.Stringify().c_str());
                return true;
                }
            return false;
            });
...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
DbResult JsonInserter::Insert(BeJsValue json) const
    {
    ECInstanceKey key;
