...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
ECSqlParseContext::ClassViewPrepareStack::~ClassViewPrepareStack() {
    BeAssert(!m_ctx.m_viewPrepareStack.empty());
    m_ctx.m_viewPrepareStack.pop_back();
}
...
BentleyStatus ECSqlParser::ParseOuterJoinType(ECSqlJoinType& joinType, OSQLParseNode const* parseNode) const {
    ...
    switch (n->getTokenID()) {
        case SQL_TOKEN_LEFT:
            joinType = ECSqlJoinType::LeftOuterJoin;
            break;
        case SQL_TOKEN_RIGHT:
            joinType = ECSqlJoinType::RightOuterJoin;
            break;
        case SQL_TOKEN_FULL:
            joinType = ECSqlJoinType::FullOuterJoin;
            break;
        default:
...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+--------
void ECSqlParseContext::PushArg(std::unique_ptr<ParseArg> arg) {
    m_finalizeParseArgs.push_back(std::move(arg));
}
...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+--------
void ECSqlParseContext::PopArg() {
    BeAssert(!m_finalizeParseArgs.empty());
