...
struct NavValueCreationFuncExp final : ValueCreationFuncExp {
   private:
    ...
    bool _TryDetermineParameterExpType(ECSqlParseContext& ctx, ParameterExp& parameterExp) const override {
        parameterExp.SetTypeInfo(ECSqlTypeInfo::CreatePrimitive(ECN::PRIMITIVETYPE_Long));
        return true;
    }
