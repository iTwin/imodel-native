...
struct PropertyNameExp final : ValueExp {
   ...
   public:
    ...
    bool IsPropertyFromCommonTableBlockWithColumns() const;
    ECSqlSystemPropertyInfo const& GetSystemPropertyInfo() const {
        BeAssert(m_sysPropInfo != nullptr);
        return *m_sysPropInfo;
    }
...
struct ExtractInstanceValueExp final : InstanceValueExp {
   ...
   public:
    ExtractInstanceValueExp(
        PropertyPath instancePath) : InstanceValueExp(Type::ExtractInstance, instancePath) {
        SetTypeInfo(ECSqlTypeInfo::CreatePrimitive(ECN::PRIMITIVETYPE_String));
    }
