...
// @bsiclass
//+===============+===============+===============+===============+===============+======
struct CompoundDataPropertyMapDiff {
   private:
    CompoundDataPropertyMap const* m_propertyMap;
    std::vector<SingleColumnDataPropertyMap const*> m_baseline;
    std::vector<SingleColumnDataPropertyMap const*> m_newProps;

   public:
    CompoundDataPropertyMapDiff() : m_propertyMap(nullptr) {}
    void Init(CompoundDataPropertyMap const& propertyMap);
    void AfterUpdate();
    CompoundDataPropertyMap const& GetPropertyMap() const { return *m_propertyMap; }
    std::vector<SingleColumnDataPropertyMap const*> const& GetNewProps() const;
    std::vector<SingleColumnDataPropertyMap const*> const& GetBaselineProps() const;
    bool IsOverflowed() const;
    DbTable const* GetBaselineTable() const;
    DbTable const* GetNewPropTable() const;
    uint32_t MaxColumnRequired() const { return (uint32_t)(m_baseline.size() + m_newProps.size()); }
};
//======================================================================================
// @bsiclass
//======================================================================================
struct DbMappingManager final {
    ...
    struct Classes final {
       ...
       private:
        struct Context final {
            ...
            ClassMap& m_classMap;
            bool m_nestedPropertyFailedToLoad = false;
            bool m_updateStructProperty = false;
       ...
       public:
        static PropertyMap* MapProperty(SchemaImportContext& importCtx, ClassMap& classMap, ECN::ECPropertyCR prop) {
            Context ctx(importCtx, classMap);
            return ProcessProperty(ctx, prop);
        }
        ...
        static PropertyMap* LoadPropertyMap(DbClassMapLoadContext const& loadCtx, ClassMap& classMap, ECN::ECPropertyCR prop) {
            Context ctx(loadCtx, classMap);
            return ProcessProperty(ctx, prop);
        }
    ...
    struct FkRelationships final {
       ...
       public:
        ...
        static BentleyStatus TryDetermineFkEnd(ECN::ECRelationshipEnd&, ECN::ECRelationshipClassCR, IssueDataSource const&);
    };
