   ...
   private:
    ...
    struct LegacyUnitsHelper final {
       ...
       public:
        ...
        ECN::ECSchemaCP GetUnitsSchema() const {
            Initialize();
            return m_unitsSchema.get();
        }
        ECN::ECSchemaCP GetFormatsSchema() const {
            Initialize();
            return m_formatsSchema.get();
        }
        ...
        void ClearCache() const {
            ...
            m_formatsSchema = nullptr;
            m_unitsSchema = nullptr;
        }
    };
    ...
    struct ReaderCache final {
       ...
       public:
        ...
        ECN::ECEnumerationCP Find(ECN::ECEnumerationId id) const {
            auto it = m_enumCache.find(id);
            return it != m_enumCache.end() ? it->second : nullptr;
        }
        ECN::KindOfQuantityCP Find(ECN::KindOfQuantityId id) const {
            auto it = m_koqCache.find(id);
            return it != m_koqCache.end() ? it->second : nullptr;
        }
        ECN::PropertyCategoryCP Find(ECN::PropertyCategoryId id) const {
            auto it = m_propCategoryCache.find(id);
            return it != m_propCategoryCache.end() ? it->second : nullptr;
        }
        ECN::UnitSystemCP Find(ECN::UnitSystemId id) const {
            auto it = m_unitSystemCache.find(id);
            return it != m_unitSystemCache.end() ? it->second : nullptr;
        }
        ECN::PhenomenonCP Find(ECN::PhenomenonId id) const {
            auto it = m_phenomenonCache.find(id);
            return it != m_phenomenonCache.end() ? it->second : nullptr;
        }
        ECN::ECUnitCP Find(ECN::UnitId id) const {
            auto it = m_unitCache.find(id);
            return it != m_unitCache.end() ? it->second : nullptr;
        }
        ECN::ECFormatCP Find(ECN::FormatId id) const {
            auto it = m_formatCache.find(id);
            return it != m_formatCache.end() ? it->second : nullptr;
        }
    ...
    ReaderCache m_cache;
    mutable ECN::ECSchemaId m_systemSchemaId;

    ECN::ECSchemaId GetSystemSchemaId() const {
        if (!m_systemSchemaId.IsValid()) {
            Statement stmt;
            if (BE_SQLITE_OK == stmt.Prepare(GetECDb(), "SELECT Id FROM ec_Schema WHERE Name='" ECSCHEMA_ECDbSystem "'")) {
                if (BE_SQLITE_ROW == stmt.Step()) {
                    m_systemSchemaId = stmt.GetValueId<ECN::ECSchemaId>(0);
                }
            }
        }
        return m_systemSchemaId;
    }
    // not copyable
