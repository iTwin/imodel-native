...
struct ToSqlPropertyMapVisitor final : IPropertyMapVisitor {
    ...
    struct Result {
       ...
       public:
        ...
        SingleColumnDataPropertyMap const& GetPropertyMap() const {
...
struct SavePropertyMapVisitor final : IPropertyMapVisitor {
   ...
   public:
    explicit SavePropertyMapVisitor(DbClassMapSaveContext& ctx, Action action = Action::InsertNew, DbTable const* tableFilter = nullptr) : IPropertyMapVisitor(), m_context(ctx), m_tableFilter(tableFilter), m_action(action) {}
...
// @bsiclass
//+===============+===============+===============+===============+===============+======
struct ComputeHashPropertyMapVisitor final : IPropertyMapVisitor {
   private:
    BentleyStatus _Visit(SingleColumnDataPropertyMap const&) const override;
    BentleyStatus _Visit(SystemPropertyMap const&) const override;
    BentleyStatus _Visit(CompoundDataPropertyMap const&) const override;
    mutable FNV1HashBuilder m_hashBuilder;

   public:
    ComputeHashPropertyMapVisitor() : IPropertyMapVisitor() {}
    ~ComputeHashPropertyMapVisitor() {}
    uint64_t GetHashCode() const { return m_hashBuilder.GetHashCode(); }
};
