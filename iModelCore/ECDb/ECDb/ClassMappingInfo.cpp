...
BentleyStatus RelationshipMappingInfo::_InitializeFromSchema()
    ...
    if ((hasForeignKeyConstraintCA || useECInstanceIdAsFk) && RequiresLinkTable())
        {
        Issues().Report(ECDbIssueSeverity::Error, "Failed to map ECRelationshipClass %s. It has the 'ForeignKeyConstraint' or 'UsePrimaryKeyAsForeignKey' custom attribute, but implies a link table mapping because of its cardinality or because it defines ECProperties.",
                        m_ecClass.GetFullName());
        return ERROR;
        }
