...
struct CachedQueryAdaptor final : std::enable_shared_from_this<CachedQueryAdaptor> {
   ...
   public:
    ...
    rapidjson::Document& ClearAndGetCachedJsonDocument() {
        m_cachedJsonDoc.Clear();
        m_allocator.Clear();
        return m_cachedJsonDoc;
    }
    std::string& ClearAndGetCachedString() {
...
struct QueryAdaptorCache final {
   ...
   public:
    ...
    void SetMaxCacheSize(uint32_t n) {
        if (n < QueryAdaptorCache::kDefaultCacheSize) return;
        m_maxEntries = n;
    }
...
struct QueryRetryHandler final : BeSQLite::BusyRetry {
   ...
   private:
    ...
    QueryRetryHandler(std::chrono::milliseconds maxTime, std::chrono::milliseconds sleepTime) : m_sleepTime(sleepTime), m_maxTime(maxTime) {}
...
struct RunnableRequestBase {
   ...
   public:
    RunnableRequestBase(RunnableRequestQueue& queue, QueryRequest::Ptr request, QueryQuota quota, uint32_t id)
        : m_queue(queue), m_request(std::move(request)), m_id(id), m_isCompleted(false), m_dequeuedOn(0s), m_isDequeued(false), m_interrupted(false), m_quota(quota), m_submittedOn(std::chrono::steady_clock::now()), m_cancelled(false), m_executorId(0), m_connId(0), m_prepareTime(0) {}
    ...
    void OnDequeued() {
        m_isDequeued = true;
        m_dequeuedOn = std::chrono::steady_clock::now();
    }
    uint32_t GetExecutorId() const { return m_executorId; }
    uint32_t GetConnectionId() const { return m_connId; }
    void SetExecutorContext(uint32_t executorId, uint32_t connId) {
