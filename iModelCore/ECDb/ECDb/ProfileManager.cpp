...
DbResult ProfileManager::UpgradeProfile(Db::OpenParams const& params) const {
    ...
    const auto importOpts = params.m_schemaLockHeld ? SchemaManager::SchemaImportOptions::AllowDataTransformDuringSchemaUpgrade : SchemaManager::SchemaImportOptions::None;
...
DbResult ProfileManager::RunUpgraders(ProfileVersion const& versionBeforeUpgrade) const {
    ...
    if (versionBeforeUpgrade < ProfileVersion(4, 0, 0, 4))
        upgraders.push_back(std::make_unique<ProfileUpgrader_4004>());
...
DbResult ProfileManager::CreateProfileTables() const {
    // ec_Schema
    DbResult stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_Schema
    ...
    // ec_SchemaReference
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_SchemaReference
                                       "(Id INTEGER PRIMARY KEY,"
                                       "SchemaId INTEGER REFERENCES " TABLE_Schema
                                       "(Id) ON DELETE CASCADE,"
                                       "ReferencedSchemaId INTEGER REFERENCES " TABLE_Schema "(Id) ON DELETE CASCADE)");
    if (BE_SQLITE_OK != stat)
        return stat;

    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE UNIQUE INDEX uix_ec_SchemaReference_SchemaId_ReferencedSchemaId ON " TABLE_SchemaReference
                                       "(SchemaId,ReferencedSchemaId);"
                                       "CREATE INDEX ix_ec_SchemaReference_ReferencedSchemaId ON " TABLE_SchemaReference "(ReferencedSchemaId)");
    if (BE_SQLITE_OK != stat)
        return stat;

    // ec_Class
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_Class
                                       "(Id INTEGER PRIMARY KEY,"
                                       "SchemaId INTEGER NOT NULL REFERENCES " TABLE_Schema
                                       "(Id) ON DELETE CASCADE,"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE INDEX ix_ec_Class_SchemaId_Name ON " TABLE_Class
                                       "(SchemaId,Name);"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_ClassHasBaseClasses
                                       "(Id INTEGER PRIMARY KEY,"
                                       "ClassId INTEGER NOT NULL REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
                                       "BaseClassId INTEGER NOT NULL REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
                                       "Ordinal INTEGER NOT NULL)");
    if (BE_SQLITE_OK != stat)
        return stat;

    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE UNIQUE INDEX uix_ec_ClassHasBaseClasses_ClassId_BaseClassId ON " TABLE_ClassHasBaseClasses
                                       "(ClassId,BaseClassId);"
                                       "CREATE INDEX ix_ec_ClassHasBaseClasses_BaseClassId ON " TABLE_ClassHasBaseClasses
                                       "(BaseClassId);"
    ...
    // ec_Enumeration
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_Enumeration
                                       "(Id INTEGER PRIMARY KEY,"
                                       "SchemaId INTEGER NOT NULL REFERENCES " TABLE_Schema
                                       "(Id) ON DELETE CASCADE,"
                                       "Name TEXT NOT NULL COLLATE NOCASE,"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE INDEX ix_ec_Enumeration_SchemaId ON " TABLE_Enumeration
                                       "(SchemaId);"
    ...
    // ec_KindOfQuantity
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_KindOfQuantity
                                       "(Id INTEGER PRIMARY KEY,"
                                       "SchemaId INTEGER NOT NULL REFERENCES " TABLE_Schema
                                       "(Id) ON DELETE CASCADE,"
                                       "Name TEXT NOT NULL COLLATE NOCASE,"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE INDEX ix_ec_KindOfQuantity_SchemaId ON " TABLE_KindOfQuantity
                                       "(SchemaId);"
    ...
    // ec_Phenomenon
    stat = m_ecdb.GetImpl().ExecuteDDL(TABLEDDL_Phenomenon);
    if (BE_SQLITE_OK != stat)
        return stat;

    // ec_Unit
    stat = m_ecdb.GetImpl().ExecuteDDL(TABLEDDL_Unit);
    if (BE_SQLITE_OK != stat)
        return stat;

    // ec_Format
    stat = m_ecdb.GetImpl().ExecuteDDL(TABLEDDL_Format);
    if (BE_SQLITE_OK != stat)
        return stat;

    // ec_FormatCompositeUnit
    stat = m_ecdb.GetImpl().ExecuteDDL(TABLEDDL_FormatCompositeUnit);
    if (BE_SQLITE_OK != stat)
        return stat;

    // ec_PropertyCategory
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_PropertyCategory
                                       "(Id INTEGER PRIMARY KEY,"
                                       "SchemaId INTEGER NOT NULL REFERENCES " TABLE_Schema
                                       "(Id) ON DELETE CASCADE,"
                                       "Name TEXT NOT NULL COLLATE NOCASE,"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE INDEX ix_ec_PropertyCategory_SchemaId ON " TABLE_PropertyCategory
                                       "(SchemaId);"
    ...
    // ec_Property
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_Property
                                       "(Id INTEGER PRIMARY KEY,"
                                       "ClassId INTEGER NOT NULL REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
                                       "Name TEXT NOT NULL COLLATE NOCASE,"
                                       ...
                                       "EnumerationId INTEGER REFERENCES " TABLE_Enumeration
                                       "(Id) ON DELETE CASCADE,"
                                       "StructClassId INTEGER REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
                                       "ExtendedTypeName TEXT,"
                                       "KindOfQuantityId INTEGER REFERENCES " TABLE_KindOfQuantity
                                       "(Id) ON DELETE CASCADE,"
                                       "CategoryId INTEGER REFERENCES " TABLE_PropertyCategory
                                       "(Id) ON DELETE CASCADE,"
                                       "ArrayMinOccurs INTEGER,"
                                       "ArrayMaxOccurs INTEGER,"
                                       "NavigationRelationshipClassId INTEGER REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE UNIQUE INDEX uix_ec_Property_ClassId_Name ON " TABLE_Property
                                       "(ClassId,Name);"
                                       "CREATE UNIQUE INDEX uix_ec_Property_ClassId_Ordinal ON " TABLE_Property
                                       "(ClassId,Ordinal);"
                                       "CREATE INDEX ix_ec_Property_Name ON " TABLE_Property
                                       "(Name);"
                                       "CREATE INDEX ix_ec_Property_EnumerationId ON " TABLE_Property
                                       "(EnumerationId);"
                                       "CREATE INDEX ix_ec_Property_StructClassId ON " TABLE_Property
                                       "(StructClassId);"
                                       "CREATE INDEX ix_ec_Property_KindOfQuantityId ON " TABLE_Property
                                       "(KindOfQuantityId);"
                                       "CREATE INDEX ix_ec_Property_CategoryId ON " TABLE_Property
                                       "(CategoryId);"
    ...
    // ec_PropertyPath
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE Table " TABLE_PropertyPath
                                       "(Id INTEGER PRIMARY KEY,"
                                       "RootPropertyId INTEGER NOT NULL REFERENCES " TABLE_Property
                                       "(Id) ON DELETE CASCADE,"
                                       "AccessString TEXT NOT NULL COLLATE NOCASE)");
    if (BE_SQLITE_OK != stat)
        return stat;

    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE UNIQUE INDEX uix_ec_PropertyPath_RootPropertyId_AccessString ON " TABLE_PropertyPath "(RootPropertyId,AccessString)");
    if (BE_SQLITE_OK != stat)
        return stat;

    // ec_RelationshipConstraint
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_RelationshipConstraint
                                       "(Id INTEGER PRIMARY KEY,"
                                       "RelationshipClassId INTEGER NOT NULL REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE UNIQUE INDEX uix_ec_RelationshipConstraint_RelationshipClassId_RelationshipEnd ON " TABLE_RelationshipConstraint
                                       "(RelationshipClassId,RelationshipEnd);"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_RelationshipConstraintClass
                                       "(Id INTEGER PRIMARY KEY,"
                                       "ConstraintId INTEGER NOT NULL REFERENCES " TABLE_RelationshipConstraint
                                       "(Id) ON DELETE CASCADE,"
                                       "ClassId INTEGER NOT NULL REFERENCES " TABLE_Class "(Id) ON DELETE CASCADE)");
    if (BE_SQLITE_OK != stat)
        return stat;

    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE UNIQUE INDEX uix_ec_RelationshipConstraintClass_ConstraintId_ClassId ON " TABLE_RelationshipConstraintClass
                                       "(ConstraintId,ClassId);"
                                       "CREATE INDEX ix_ec_RelationshipConstraintClass_ClassId ON " TABLE_RelationshipConstraintClass "(ClassId)");
    if (BE_SQLITE_OK != stat)
        return stat;

    // ec_CustomAttribute
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_CustomAttribute
                                       "(Id INTEGER PRIMARY KEY,"
                                       "ClassId INTEGER NOT NULL REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE UNIQUE INDEX uix_ec_CustomAttribute_ContainerId_ContainerType_Ordinal ON " TABLE_CustomAttribute
                                       "(ContainerId,ContainerType,Ordinal);"
                                       "CREATE UNIQUE INDEX uix_ec_CustomAttribute_ContainerId_ContainerType_ClassId ON " TABLE_CustomAttribute
                                       "(ContainerId,ContainerType,ClassId);"
                                       "CREATE INDEX ix_ec_CustomAttribute_ClassId ON " TABLE_CustomAttribute "(ClassId)");
    if (BE_SQLITE_OK != stat)
        return stat;

    // ec_ClassMap
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE Table " TABLE_ClassMap
                                       "(ClassId INTEGER PRIMARY KEY REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
    ...
    // ec_PropertyMap
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE Table " TABLE_PropertyMap
                                       "(Id INTEGER PRIMARY KEY,"
                                       "ClassId INTEGER NOT NULL REFERENCES " TABLE_ClassMap
                                       "(ClassId) ON DELETE CASCADE,"
                                       "PropertyPathId INTEGER NOT NULL REFERENCES " TABLE_PropertyPath
                                       "(Id) ON DELETE CASCADE,"
                                       "ColumnId INTEGER NOT NULL REFERENCES " TABLE_Column "(Id) ON DELETE CASCADE)");
    if (BE_SQLITE_OK != stat)
        return stat;

    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE UNIQUE INDEX uix_ec_PropertyMap_ClassId_PropertyPathId_ColumnId ON " TABLE_PropertyMap
                                       "(ClassId,PropertyPathId,ColumnId);"
                                       "CREATE INDEX ix_ec_PropertyMap_PropertyPathId ON " TABLE_PropertyMap
                                       "(PropertyPathId);"
    ...
    // ec_Table
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_Table
                                       "(Id INTEGER PRIMARY KEY,"
                                       "ParentTableId INTEGER REFERENCES " TABLE_Table
                                       "(Id) ON DELETE CASCADE,"
                                       "Name TEXT UNIQUE NOT NULL COLLATE NOCASE,"  // ECDb requires table names to be unique even across other db schema names
                                       "Type INTEGER NOT NULL,"
                                       "ExclusiveRootClassId INTEGER REFERENCES " TABLE_Class
                                       "(Id) ON DELETE SET NULL,"
                                       "UpdatableViewName TEXT)");  // UpdatableViewName is not used anymore -> WIP_NEXTGEN_DELETE
    if (BE_SQLITE_OK != stat)
        return stat;

    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE INDEX ix_ec_Table_ParentTableId ON " TABLE_Table
                                       "(ParentTableId);"
    ...
    // ec_Column
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_Column
                                       "(Id INTEGER PRIMARY KEY,"
                                       "TableId INTEGER NOT NULL REFERENCES " TABLE_Table
                                       "(Id) ON DELETE CASCADE,"
                                       "Name TEXT NOT NULL COLLATE NOCASE,"
                                       "Type INTEGER NOT NULL,"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE UNIQUE INDEX uix_ec_Column_TableId_Name ON " TABLE_Column
                                       "(TableId,Name);"
    ...
    // ec_Index
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_Index
                                       "(Id INTEGER PRIMARY KEY,"
                                       "Name TEXT UNIQUE NOT NULL COLLATE NOCASE,"
                                       "TableId INTEGER NOT NULL REFERENCES " TABLE_Table
                                       "(Id) ON DELETE CASCADE,"
                                       ...
                                       ")),"
                                       "ClassId INTEGER REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
                                       "AppliesToSubclassesIfPartial BOOLEAN NOT NULL CHECK (AppliesToSubclassesIfPartial IN (" SQLVAL_False "," SQLVAL_True ")))");
    if (BE_SQLITE_OK != stat)
        return stat;

    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE INDEX ix_ec_Index_TableId ON " TABLE_Index
                                       "(TableId);"
    ...
    // ec_IndexColumn
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_IndexColumn
                                       "(Id INTEGER PRIMARY KEY,"
                                       "IndexId INTEGER NOT NULL REFERENCES " TABLE_Index
                                       " (Id) ON DELETE CASCADE,"
                                       "ColumnId INTEGER NOT NULL REFERENCES " TABLE_Column
                                       " (Id) ON DELETE CASCADE,"
                                       "Ordinal INTEGER NOT NULL)");
    if (BE_SQLITE_OK != stat)
        return stat;

    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE UNIQUE INDEX uix_ec_IndexColumn_IndexId_ColumnId_Ordinal ON " TABLE_IndexColumn
                                       "(IndexId,ColumnId,Ordinal);"
                                       "CREATE INDEX ix_ec_IndexColumn_IndexId_Ordinal ON " TABLE_IndexColumn
                                       "(IndexId,Ordinal);"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_ClassHasTablesCache
                                       "("
                                       "Id INTEGER PRIMARY KEY,"
                                       "ClassId INTEGER NOT NULL REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE INDEX ix_ec_cache_ClassHasTables_ClassId_TableId ON " TABLE_ClassHasTablesCache
                                       "(ClassId);"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE TABLE " TABLE_ClassHierarchyCache
                                       "("
                                       "Id INTEGER PRIMARY KEY,"
                                       "ClassId INTEGER NOT NULL REFERENCES " TABLE_Class
                                       "(Id) ON DELETE CASCADE,"
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL("CREATE INDEX ix_ec_cache_ClassHierarchy_ClassId ON " TABLE_ClassHierarchyCache
                                       "(ClassId);"
...
// https://tinyurl.com/bdzxrxe8
    ...
    stat = m_ecdb.GetImpl().ExecuteDDL(R"sql(
