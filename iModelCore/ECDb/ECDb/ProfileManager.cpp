...
DbResult ProfileManager::CreateProfileTables() const
    {
    //ec_Schema
    DbResult stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_Schema
    ...
    //ec_SchemaReference
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_SchemaReference
                           "(Id INTEGER PRIMARY KEY,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE UNIQUE INDEX uix_ec_SchemaReference_SchemaId_ReferencedSchemaId ON " TABLE_SchemaReference "(SchemaId,ReferencedSchemaId);"
                           "CREATE INDEX ix_ec_SchemaReference_ReferencedSchemaId ON " TABLE_SchemaReference "(ReferencedSchemaId)");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_Class
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_Class
                           "(Id INTEGER PRIMARY KEY,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE INDEX ix_ec_Class_SchemaId_Name ON " TABLE_Class "(SchemaId,Name);"
                           "CREATE INDEX ix_ec_Class_Name ON " TABLE_Class "(Name);");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_ClassHasBaseClasses
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_ClassHasBaseClasses
                           "(Id INTEGER PRIMARY KEY,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE UNIQUE INDEX uix_ec_ClassHasBaseClasses_ClassId_BaseClassId ON " TABLE_ClassHasBaseClasses "(ClassId,BaseClassId);"
    ...
    //ec_Enumeration
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_Enumeration
                           "(Id INTEGER PRIMARY KEY,"
                           "SchemaId INTEGER NOT NULL REFERENCES " TABLE_Schema "(Id) ON DELETE CASCADE,"
                           "Name TEXT NOT NULL COLLATE NOCASE,"
                           "DisplayLabel TEXT,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE INDEX ix_ec_Enumeration_SchemaId ON " TABLE_Enumeration "(SchemaId);"
                           "CREATE INDEX ix_ec_Enumeration_Name ON " TABLE_Enumeration "(Name);");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_KindOfQuantity
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_KindOfQuantity
                           "(Id INTEGER PRIMARY KEY,"
                           "SchemaId INTEGER NOT NULL REFERENCES " TABLE_Schema "(Id) ON DELETE CASCADE,"
                           "Name TEXT NOT NULL COLLATE NOCASE,"
                           "DisplayLabel TEXT,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE INDEX ix_ec_KindOfQuantity_SchemaId ON " TABLE_KindOfQuantity "(SchemaId);"
                           "CREATE INDEX ix_ec_KindOfQuantity_Name ON " TABLE_KindOfQuantity "(Name);");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_PropertyCategory
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_PropertyCategory
                           "(Id INTEGER PRIMARY KEY,"
                           "SchemaId INTEGER NOT NULL REFERENCES " TABLE_Schema "(Id) ON DELETE CASCADE,"
                           "Name TEXT NOT NULL COLLATE NOCASE,"
                           "DisplayLabel TEXT,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE INDEX ix_ec_PropertyCategory_SchemaId ON " TABLE_PropertyCategory "(SchemaId);"
                           "CREATE INDEX ix_ec_PropertyCategory_Name ON " TABLE_PropertyCategory "(Name);");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_Property
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_Property
                           "(Id INTEGER PRIMARY KEY,"
                           "ClassId INTEGER NOT NULL REFERENCES " TABLE_Class "(Id) ON DELETE CASCADE,"
                           "Name TEXT NOT NULL COLLATE NOCASE,"
                           "DisplayLabel TEXT,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE UNIQUE INDEX uix_ec_Property_ClassId_Name ON " TABLE_Property "(ClassId,Name);"
    ...
    //ec_PropertyPath
    stat = m_ecdb.ExecuteSql("CREATE Table " TABLE_PropertyPath
                           "(Id INTEGER PRIMARY KEY,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE UNIQUE INDEX uix_ec_PropertyPath_RootPropertyId_AccessString ON " TABLE_PropertyPath "(RootPropertyId,AccessString)");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_RelationshipConstraint
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_RelationshipConstraint
                           "(Id INTEGER PRIMARY KEY,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE UNIQUE INDEX uix_ec_RelationshipConstraint_RelationshipClassId_RelationshipEnd ON " TABLE_RelationshipConstraint "(RelationshipClassId,RelationshipEnd);"
    ...
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_RelationshipConstraintClass
                           "(Id INTEGER PRIMARY KEY,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE UNIQUE INDEX uix_ec_RelationshipConstraintClass_ConstraintId_ClassId ON " TABLE_RelationshipConstraintClass "(ConstraintId,ClassId);"
                           "CREATE INDEX ix_ec_RelationshipConstraintClass_ClassId ON " TABLE_RelationshipConstraintClass "(ClassId)");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_CustomAttribute
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_CustomAttribute
                           "(Id INTEGER PRIMARY KEY,"
                           "ClassId INTEGER NOT NULL REFERENCES " TABLE_Class "(Id) ON DELETE CASCADE,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE UNIQUE INDEX uix_ec_CustomAttribute_ContainerId_ContainerType_Ordinal ON " TABLE_CustomAttribute "(ContainerId,ContainerType,Ordinal);"
    ...
    //ec_ClassMap
    stat = m_ecdb.ExecuteSql("CREATE Table " TABLE_ClassMap
    ...
    //ec_PropertyMap
    stat = m_ecdb.ExecuteSql("CREATE Table " TABLE_PropertyMap
                           "(Id INTEGER PRIMARY KEY,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE UNIQUE INDEX uix_ec_PropertyMap_ClassId_PropertyPathId_ColumnId ON " TABLE_PropertyMap "(ClassId,PropertyPathId,ColumnId);"
    ...
    //ec_Table
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_Table
                           "(Id INTEGER PRIMARY KEY,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE INDEX ix_ec_Table_ParentTableId ON " TABLE_Table "(ParentTableId);"
                           "CREATE INDEX ix_ec_Table_ExclusiveRootClassId ON " TABLE_Table "(ExclusiveRootClassId);");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_Column
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_Column
                           "(Id INTEGER PRIMARY KEY,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE UNIQUE INDEX uix_ec_Column_TableId_Name ON " TABLE_Column "(TableId,Name);"
                           "CREATE UNIQUE INDEX uix_ec_Column_TableId_Ordinal ON " TABLE_Column "(TableId,Ordinal)");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_Index
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_Index
                           "(Id INTEGER PRIMARY KEY,"
                           "Name TEXT UNIQUE NOT NULL COLLATE NOCASE,"
                           "TableId INTEGER NOT NULL REFERENCES " TABLE_Table "(Id) ON DELETE CASCADE,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE INDEX ix_ec_Index_TableId ON " TABLE_Index "(TableId);"
                           "CREATE INDEX ix_ec_Index_ClassId ON " TABLE_Index "(ClassId)");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_IndexColumn
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_IndexColumn
                           "(Id INTEGER PRIMARY KEY,"
    ...
    stat = m_ecdb.ExecuteSql("CREATE UNIQUE INDEX uix_ec_IndexColumn_IndexId_ColumnId_Ordinal ON " TABLE_IndexColumn "(IndexId,ColumnId,Ordinal);"
    ...
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_ClassHasTablesCache "("
                           "Id INTEGER PRIMARY KEY,"
                           "ClassId INTEGER NOT NULL REFERENCES " TABLE_Class "(Id) ON DELETE CASCADE,"
                           "TableId INTEGER NOT NULL REFERENCES " TABLE_Table "(Id) ON DELETE CASCADE)");
    if (BE_SQLITE_OK != stat)
    ...
    stat = m_ecdb.ExecuteSql("CREATE INDEX ix_ec_cache_ClassHasTables_ClassId_TableId ON " TABLE_ClassHasTablesCache "(ClassId);"
                           "CREATE INDEX ix_ec_cache_ClassHasTables_TableId ON " TABLE_ClassHasTablesCache "(TableId);");
    if (BE_SQLITE_OK != stat)
        return stat;

    //ec_cache_ClassHierarchy
    stat = m_ecdb.ExecuteSql("CREATE TABLE " TABLE_ClassHierarchyCache "("
                           "Id INTEGER PRIMARY KEY,"
                           "ClassId INTEGER NOT NULL REFERENCES " TABLE_Class "(Id) ON DELETE CASCADE,"
                           "BaseClassId INTEGER NOT NULL REFERENCES " TABLE_Class "(Id) ON DELETE CASCADE)");
    if (BE_SQLITE_OK != stat)
    ...
    return m_ecdb.ExecuteSql("CREATE INDEX ix_ec_cache_ClassHierarchy_ClassId ON " TABLE_ClassHierarchyCache "(ClassId);"
