...
protected:
    SystemPropertyMap (ECN::ECPropertyCR property, std::vector<DbColumn const*> columns, ECSqlSystemProperty kind);
...
public:
    ...
    static std::vector<DbColumn const*> ToVector(DbColumn const* column);
    };
...
public:
    ...
    void AppendSelectClauseSqlSnippetForView (NativeSqlBuilder& viewSql, DbTable const& table) const;
    };
