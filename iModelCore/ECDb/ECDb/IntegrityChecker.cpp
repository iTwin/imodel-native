...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
DbResult IntegrityChecker::CheckDataTableColumnExists(std::function<bool(std::string,std::string)> callback) {
	auto sql = R"sql(
		SELECT
			[t].[Name] [tbl],
			[c].[Name] [col]
...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
DbResult IntegrityChecker::CheckSoftNavForeignKeyConstraint(std::function<bool(ECInstanceId, Utf8CP, Utf8CP, ECInstanceId, Utf8CP)> callback) {
	...
	for (auto & navProp : navProps) {
        ...
        for(auto& prop : props) {
            ...
            std::string query = SqlPrintfString("SELECT s.ECInstanceId, s.%s.Id FROM %s s LEFT JOIN %s t ON s.%s.Id=t.ECInstanceId AND t.ECInstanceId IS NULL",
...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
DbResult IntegrityChecker::CheckClassIdForNavProperties(std::function<bool(ECInstanceId, Utf8CP, Utf8CP, ECInstanceId, ECN::ECClassId)> callback) {
	std::map<ECN::ECClassId, std::vector<std::string>> navProps;
	auto rc = GetNavigationProperties(navProps);
	if (BE_SQLITE_OK != rc) {
		return rc;
...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
DbResult IntegrityChecker::CheckSoftLinkTableForeignKeyConstraint(std::function<bool(ECInstanceId, Utf8CP, Utf8CP, ECInstanceId, Utf8CP)> callback) {
...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
DbResult IntegrityChecker::CheckClassIdForLinkTableRelationships(std::function<bool(ECInstanceId, Utf8CP, Utf8CP, ECInstanceId, ECN::ECClassId)> callback) {
	std::vector<ECClassId> rootRels;
	auto rc = GetRootLinkTableRelationships(rootRels);
	if (BE_SQLITE_OK != rc) {
		return rc;
...
// @bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
DbResult IntegrityChecker::CheckClassIdForEntityClasses(std::function<bool(Utf8CP, ECInstanceId, ECN::ECClassId)> callback) {
