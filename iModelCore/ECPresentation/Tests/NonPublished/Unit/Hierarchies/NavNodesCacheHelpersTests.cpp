...
TEST_F(NodesCacheHelpersTests, HierarchyLevelExists_ReturnsCorrectValues)
    ...
    auto hlInfo = CacheHierarchyLevel(m_connection->GetId(), TEST_RULESET_ID, BeGuid(), BeGuid());
...
TEST_F(NodesCacheHelpersTests, RulesetExists_ReturnsCorrectValues)
    {
    // returns true for existing data source
    EXPECT_TRUE(NodesCacheHelpers::RulesetExists(m_cache->GetDb(), TEST_RULESET_ID));
    // return false for non-existing data source
...
TEST_F(NodesCacheHelpersTests, GetChildHierarchyLevelIds_ReturnsCorrectValues)
    {
    EXPECT_TRUE(NodesCacheHelpers::GetChildHierarchyLevelIds(m_cache->GetDb(), BeGuid(), nullptr).empty());

    // cache root hierarchy level
    auto hlInfo = CacheHierarchyLevel(m_connection->GetId(), TEST_RULESET_ID, BeGuid(), BeGuid());
    auto hierarchyLevels = NodesCacheHelpers::GetChildHierarchyLevelIds(m_cache->GetDb(), BeGuid(), nullptr);
    ...
    hierarchyLevels = NodesCacheHelpers::GetChildHierarchyLevelIds(m_cache->GetDb(), BeGuid(), "ruleset_id2");
    ASSERT_EQ(hierarchyLevels.size(), 1);
    ...
    auto childHlInfo = CacheHierarchyLevel(m_connection->GetId(), TEST_RULESET_ID, nodeId, nodeId);
    hierarchyLevels = NodesCacheHelpers::GetChildHierarchyLevelIds(m_cache->GetDb(), nodeId, nullptr);
    ASSERT_EQ(hierarchyLevels.size(), 1);
    EXPECT_EQ(hierarchyLevels[0], childHlInfo.GetId());

    // get all root hierarchy levels
    hierarchyLevels = NodesCacheHelpers::GetChildHierarchyLevelIds(m_cache->GetDb(), BeGuid(), nullptr);
