...
TEST_F (HierarchyUpdateTests, RemovesECInstanceNodeAfterECInstanceDelete)
    ...
    // expect 1 node
    nodes = RulesEngineTestHelpers::GetValidatedNodes([&](){ return m_manager->GetRootNodes(m_db, PageOptions(), options.GetJson()).get(); });
...
TEST_F (HierarchyUpdateTests, BaseClassGroupIsUpdatedWhenAdditionalInstancesAreInserted)
    ...
    ASSERT_EQ(1, m_manager->GetChildrenCount(m_db, *rootNodes[0], options.GetJson()).get());
...
TEST_F (HierarchyUpdateTests, PropertyGroupIsCreatedWhenInstanceValuesChange)
    ...
    DataContainer<NavNodeCPtr> childNodes2 = RulesEngineTestHelpers::GetValidatedNodes([&](){ return m_manager->GetChildren(m_db, *rootNodes[1], PageOptions(), options.GetJson()).get(); });
...
TEST_F (HierarchyUpdateTests, HidesDisplayLabelGroupingNodeWhenSiblingIsRemovedFromCustomNode)
    ...
    DataContainer<NavNodeCPtr> widgetChildren = RulesEngineTestHelpers::GetValidatedNodes([&](){ return m_manager->GetChildren(m_db, *childNodes[1], PageOptions(), options.GetJson()).get(); });
...
TEST_F (HierarchyUpdateTests, HidesDisplayLabelGroupingNodeWhenSiblingIsRemovedFromGroupingNode)
    ...
    DataContainer<NavNodeCPtr> widgetChildren = RulesEngineTestHelpers::GetValidatedNodes([&](){ return m_manager->GetChildren(m_db, *childNodes[0], PageOptions(), options.GetJson()).get(); });
    ASSERT_EQ(2, widgetChildren.GetSize());
    EXPECT_STREQ(NAVNODE_TYPE_ECInstanceNode, widgetChildren[0]->GetType().c_str());
    EXPECT_STREQ(NAVNODE_TYPE_ECInstanceNode, widgetChildren[1]->GetType().c_str());
...
TEST_F (HierarchyUpdateTests, UpdatesParentsHasChildrenFlagWhenChildNodeIsInserted_WhenParentIsNotRoot)
    ...
    grandchildNodes = RulesEngineTestHelpers::GetValidatedNodes([&](){ return m_manager->GetChildren(m_db, *childNodes[0], PageOptions(), options.GetJson()).get(); });
...
TEST_F(HierarchyUpdateTests, UserSettingsTrackedWhenCustomizingChildNodesDuringHierarchyUpdate)
    ...
    DataContainer<NavNodeCPtr> childNodes = RulesEngineTestHelpers::GetValidatedNodes([&](){ return m_manager->GetChildren(m_db, *rootNode, PageOptions(), options.GetJson()).get(); });
    ASSERT_EQ(1, childNodes.GetSize());
