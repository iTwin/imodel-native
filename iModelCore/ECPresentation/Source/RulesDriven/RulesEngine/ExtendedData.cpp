...
+---------------+---------------+---------------+---------------+---------------+------*/
bvector<NavNodeExtendedData::RelatedInstanceKey> NavNodeExtendedData::GetRelatedInstanceKeys() const
    {
    bvector<RelatedInstanceKey> keys;
    if (!GetJson().HasMember(NAVNODE_EXTENDEDDATA_RelatedInstanceKeys))
        return keys;

    RapidJsonValueCR json = GetJson()[NAVNODE_EXTENDEDDATA_RelatedInstanceKeys];
    if (!json.IsArray())
        return keys;
    ...
    for (Json::ArrayIndex i = 0; i < json.Size(); i++)
        {
        RapidJsonValueCR jsonKey = json[i];
        Utf8CP alias = jsonKey[NAVNODE_EXTENDEDDATA_RelatedInstanceKeys_Alias].GetString();
        ECClassId classId(jsonKey[NAVNODE_EXTENDEDDATA_InstanceKey_ECClassId].GetUint64());
        ECInstanceId instanceId(jsonKey[NAVNODE_EXTENDEDDATA_InstanceKey_ECInstanceId].GetUint64());
...
* @bsimethod                                    Grigas.Petraitis                11/2016
+---------------+---------------+---------------+---------------+---------------+------*/
void NavNodeExtendedData::SetRelatedInstanceKeys(rapidjson::Value&& json)
    {
    BeAssert(json.IsArray());
    AddMember(NAVNODE_EXTENDEDDATA_RelatedInstanceKeys, std::move(json));
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                    Grigas.Petraitis                11/2016
+---------------+---------------+---------------+---------------+---------------+------*/
void NavNodeExtendedData::SetRelatedInstanceKeys(bvector<RelatedInstanceKey> const& keys)
    {
    rapidjson::Value json(rapidjson::kArrayType);
    for (RelatedInstanceKey const& key : keys)
        {
        rapidjson::Value jsonKey(rapidjson::kObjectType);
        jsonKey.AddMember(NAVNODE_EXTENDEDDATA_RelatedInstanceKeys_Alias, rapidjson::Value(key.GetAlias(), GetAllocator()), GetAllocator());
        jsonKey.AddMember(NAVNODE_EXTENDEDDATA_InstanceKey_ECClassId, key.GetInstanceKey().GetClassId().GetValue(), GetAllocator());
        jsonKey.AddMember(NAVNODE_EXTENDEDDATA_InstanceKey_ECInstanceId, key.GetInstanceKey().GetInstanceId().GetValue(), GetAllocator());
        json.PushBack(std::move(jsonKey), GetAllocator());
        }
    AddMember(NAVNODE_EXTENDEDDATA_RelatedInstanceKeys, std::move(json));
    }
...
* @bsimethod                                    Grigas.Petraitis                11/2016
+---------------+---------------+---------------+---------------+---------------+------*/
void NavNodeExtendedData::SetRelatedInstanceKey(RelatedInstanceKey const& key)
    {
