...
ContentSetItemPtr ContentItemBuilder::BuildItem()
    {
    static Utf8PrintfString const s_variesStr(CONTENTRECORD_MERGED_VALUE_FORMAT, CommonStrings::PRESENTATION_LABEL_VARIES);
...
void MergingContentItemBuilder::_SetLabel(LabelDefinitionCR definition)
    ...
    if (GetLabel().IsValid() && (*GetLabel()) != definition)
        {
        ContentItemBuilder::_SetLabel(*LabelDefinition::Create(CommonStrings::PRESENTATION_LABEL_MULTIPLEINSTANCES));
        }
    else
