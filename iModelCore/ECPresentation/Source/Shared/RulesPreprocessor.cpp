...
static bool MeetsSchemaRequirements(ECDbCR ecdb, RequiredSchemaSpecificationsList const& requirements, Utf8StringCR containerIdentifier)
    ...
    for (auto schemaRequirement : requirements)
        ...
        if (!schema)
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::Rules, LOG_TRACE, LOG_INFO, Utf8PrintfString("Omitting %s - schema requirement is not met. "
...
PresentationRuleSetPtr RulesPreprocessor::GetPresentationRuleSet(IRulesetLocaterManager const& locaters, IConnectionCR connection, Utf8CP rulesetId)
    ...
    DIAGNOSTICS_DEV_LOG(DiagnosticsCategory::Default, LOG_TRACE, Utf8PrintfString("Located %" PRIu64 " rulesets with id '%s'", (uint64_t)rulesets.size(), rulesetId));
    ...
    for (PresentationRuleSetPtr& ruleset : rulesets)
        ...
        if (ruleset->GetRulesetSchemaVersion().GetMajor() > PresentationRuleSet::GetCurrentRulesetSchemaVersion().GetMajor())
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::Rules, LOG_TRACE, LOG_INFO, Utf8PrintfString("Skipping %s - ruleset schema major version (%s) "
    ...
    if (primary.IsNull())
        {
        DIAGNOSTICS_DEV_LOG(DiagnosticsCategory::Default, LOG_TRACE, "Primary ruleset not found");
        return nullptr;
        }
    ...
    DIAGNOSTICS_DEV_LOG(DiagnosticsCategory::Default, LOG_TRACE, Utf8PrintfString("Identified %" PRIu64 " supplemental rulesets.", (uint64_t)supplementalRuleSetsList.size()));
...
bool RulesPreprocessor::VerifyCondition(Utf8CP condition, ECExpressionsCache& expressionsCache, OptimizedExpressionsParameters const* optimizedParams, std::function<ExpressionContextPtr()> contextPreparer)
    ...
    if (ExpressionStatus::Success != node->GetValue(valueResult, *context))
        {
        DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("Failed to evaluate ECExpression: %s", condition));
        return false;
        }
...
ContentRuleInputKeysContainer RulesPreprocessor::_GetContentSpecifications(ContentRuleParametersCR params)
    ...
    for (NavNodeKeyCPtr const& inputNodeKey : contentNodeKeys)
        ...
        // Extending lifetime of preferredDisplayType
