    ...
    * @bsimethod
    ...
    static ExpressionStatus CreateSet(EvaluationResult& evalResult, void*, EvaluationResultVector& args)
        ...
        for (EvaluationResultCR arg : args)
            ...
            if (!arg.IsECValue())
                {
                DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("All arguments in `Set` function should be primitive, got: %d. Skipping", (int)arg.GetValueType()));
                continue;
                }
...
private:
    ...
    * @bsimethod
    ...
    BentleyStatus ParseRelatedClassInfo(ArgumentTreeNodeCR args, RelatedClassInfo& info)
        ...
        if (info.Direction.EqualsI("Forward"))
            ...
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("Invalid relationship direction. Expected 'Forward' or 'Backward', got: '%s'", info.Direction.c_str()));
            return ERROR;
            }
        ...
        if (ECObjectsStatus::Success != ECClass::ParseClassName(info.RelationshipNames.SchemaName, info.RelationshipNames.ClassName, relationshipSchemaAndClassName))
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("Failed to parse relationship name: '%s'", relationshipSchemaAndClassName.c_str()));
            return ERROR;
            }
        ...
        if (ECObjectsStatus::Success != ECClass::ParseClassName(info.RelatedClassNames.SchemaName, info.RelatedClassNames.ClassName, relatedSchemaAndClassName))
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("Failed to parse related class name: '%s'", relatedSchemaAndClassName.c_str()));
            return ERROR;
            }
    ...
    * @bsimethod
    ...
    BentleyStatus ParseJoinClassInfo(ArgumentTreeNodeCR args, JoinClassInfo& info)
        ...
        if (nullptr == lambda)
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, "Expecting second argument to be a lambda");
            return ERROR;
            }
    ...
    * @bsimethod
    ...
    bool HandleHasRelatedInstanceSpecialCase(CallNodeCR node)
        ...
        DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("Invalid number of arguments. Expecting 2 or 3, got: %" PRIu64, args->GetArgumentCount()));
        return false;
        }
    ...
    * @bsimethod
    ...
    bool HandleGetRelatedValueSpecialCase(CallNodeCR node)
        ...
        DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("Invalid number of arguments. Expecting 3 or 4, got: %" PRIu64, args->GetArgumentCount()));
        return false;
        }
    ...
    * @bsimethod
    ...
    bool HandleGetVariableIntValuesSpecialCase(CallNodeCR node)
        ...
        if (nullptr == args)
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, "Missing arguments");
            return false;
            }
        if (1 != args->GetArgumentCount())
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("Invalid number of arguments. Expecting 1, got: %" PRIu64, args->GetArgumentCount()));
            return false;
            }
    ...
    * @bsimethod
    ...
    bool HandleAnyMatchSpecialCase(CallNodeCR node)
        ...
        if (m_currentValueListMethodNode.IsNull() && m_currentValueListIdentNode.IsNull())
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, "`AnyMatch` may only be used on value lists");
            return false;
            }
    ...
    * @bsimethod
    ...
    bool HandleCompareDateTimesSpecialCase(CallNodeCR node)
        ...
        if (2 != args->GetArgumentCount())
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("Invalid number of arguments. Expecting 2, got: %" PRIu64, args->GetArgumentCount()));
            return false;
            }
    ...
    * @bsimethod
    ...
    bool HandleGetFormattedValueSpecialCase(CallNodeCR node)
        ...
        if (1 != args->GetArgumentCount() && 2 != args->GetArgumentCount())
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("Invalid number of arguments. Expecting 1 or 2, got: %" PRIu64, args->GetArgumentCount()));
            return false;
            }
        ...
        if (nullptr == listNode || 2 != listNode->GetNumberOfOperators())
            {
            DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, Utf8PrintfString("First argument should be property accessor, e.g. 'this.MyProperty'. Got: '%s'", args->GetArgument(0)->ToExpressionString().c_str()));
            return false;
            }
    ...
    * @bsimethod
    ...
    void HandleLambdaNode(LambdaNodeCR node)
        ...
        DIAGNOSTICS_LOG(DiagnosticsCategory::ECExpressions, LOG_TRACE, LOG_ERROR, "Only an EQUALS operation or function calls are allowed in lambda expression");
        }
