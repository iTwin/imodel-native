...
bool NavNodeCustomizer::ApplyLabelAndDescriptionOverride()
    ...
    if (nullptr != labelOverride)
        ...
        if (customizeLabel && !labelOverride->GetLabel().empty()
            && ECExpressionEvaluationResult::EVALUATION_Success == ECExpressionsHelper(m_context.GetECExpressionsCache()).EvaluateECExpression(value, labelOverride->GetLabel(), GetNodeExpressionContext())
        ...
        if (!labelOverride->GetDescription().empty()
            && ECExpressionEvaluationResult::EVALUATION_Success == ECExpressionsHelper(m_context.GetECExpressionsCache()).EvaluateECExpression(value, labelOverride->GetDescription(), GetNodeExpressionContext())
            && value.CanConvertToPrimitiveType(PRIMITIVETYPE_String) && value.ConvertPrimitiveToString(valueStr))
            {
...
bool NavNodeCustomizer::ApplyStyleOverride()
    ...
    if (nullptr != styleOverride)
        ...
        if (!styleOverride->GetForeColor().empty()
            && ECExpressionEvaluationResult::EVALUATION_Success == ECExpressionsHelper(m_context.GetECExpressionsCache()).EvaluateECExpression(value, styleOverride->GetForeColor(), GetNodeExpressionContext())
            && value.CanConvertToPrimitiveType(PRIMITIVETYPE_String) && value.ConvertPrimitiveToString(valueStr))
            {
        ...
        if (!styleOverride->GetBackColor().empty()
            && ECExpressionEvaluationResult::EVALUATION_Success == ECExpressionsHelper(m_context.GetECExpressionsCache()).EvaluateECExpression(value, styleOverride->GetBackColor(), GetNodeExpressionContext())
            && value.CanConvertToPrimitiveType(PRIMITIVETYPE_String) && value.ConvertPrimitiveToString(valueStr))
            {
        ...
        if (!styleOverride->GetFontStyle().empty()
            && ECExpressionEvaluationResult::EVALUATION_Success == ECExpressionsHelper(m_context.GetECExpressionsCache()).EvaluateECExpression(value, styleOverride->GetFontStyle(), GetNodeExpressionContext())
            && value.CanConvertToPrimitiveType(PRIMITIVETYPE_String) && value.ConvertPrimitiveToString(valueStr))
            {
...
bool NavNodeCustomizer::ApplyImageIdOverride()
    ...
    if (nullptr != imageIdOverride)
        ...
        ECValue value;
        Utf8String valueStr;
        if (ECExpressionEvaluationResult::EVALUATION_Success == ECExpressionsHelper(m_context.GetECExpressionsCache()).EvaluateECExpression(value, imageIdOverride->GetImageId(), GetNodeExpressionContext())
            && value.CanConvertToPrimitiveType(PRIMITIVETYPE_String) && value.ConvertPrimitiveToString(valueStr))
            {
...
bool NavNodeCustomizer::ApplyCheckboxRules()
    ...
    if (!rule->GetPropertyName().empty() && nullptr != m_node.GetKey()->AsECInstanceNodeKey())
        ...
        isReadOnly = (!rule->GetIsEnabled().empty()
            && ECExpressionEvaluationResult::EVALUATION_Success == ECExpressionsHelper(m_context.GetECExpressionsCache()).EvaluateECExpression(value, rule->GetIsEnabled(), GetNodeExpressionContext())
            && value.IsBoolean()
            && !value.GetBoolean());
        }
...
bool NavNodeCustomizer::ApplyExtendedDataRules()
    ...
    for (ExtendedDataRuleCP rule : rules)
        ...
        for (auto const& entry : rule->GetItemsMap())
            ...
            ECValue value;
            if (usedKeys.find(key) == usedKeys.end() && ECExpressionEvaluationResult::EVALUATION_Success == ECExpressionsHelper(m_context.GetECExpressionsCache()).EvaluateECExpression(value, valueExpr, GetNodeExpressionContext()))
                {
...
NodeArtifacts CustomizationHelper::EvaluateArtifacts(NavNodesProviderContextCR context, NavNodeCR node)
    ...
    for (NodeArtifactsRuleCP rule : rules)
        ...
        for (auto entry : rule->GetItemsMap())
            ...
            Utf8StringCR valueExpr = entry.second;
            ECValue value;
            if (ECExpressionEvaluationResult::EVALUATION_Success == ECExpressionsHelper(context.GetECExpressionsCache()).EvaluateECExpression(value, valueExpr, *evaluationContext))
