...
public:
    ...
    void _ComputeValue(BeSQLite::DbFunction::Context& ctx, int nArgs, BeSQLite::DbValue* args) override
        ...
        if (GetCache().end() == iter)
            ...
            if (!labelDefinition->IsDefinitionValid())
                {
                labelDefinition->SetStringValue(CommonStrings::LABEL_NOTSPECIFIED);
                }
...
public:
    ...
    void _ComputeValue(BeSQLite::DbFunction::Context& ctx, int nArgs, BeSQLite::DbValue* args) override
        ...
        if (GetCache().end() == iter)
            ...
            if (!label.IsValid())
                label = LabelDefinition::Create(CommonStrings::LABEL_NOTSPECIFIED);
        ...
        if (GetCache().end() == iter)
            ...
            if (!labelDefinition->IsDefinitionValid() && nullptr != GetContext().GetQueryExtendedData() && NavigationQueryExtendedData(*GetContext().GetQueryExtendedData()).HasRangesData())
                ...
                if (-1 == rangeIndex)
                    ...
                    labelDefinition->SetStringValue(CommonStrings::LABEL_OTHER);
                    }
                else
                    {
...
public:
    ...
    void _ComputeValue(BeSQLite::DbFunction::Context& ctx, int nArgs, BeSQLite::DbValue* args) override
        ...
        if (CommonStrings::LABEL_NOTSPECIFIED == iter->first || CommonStrings::LABEL_OTHER == iter->first)
            ctx.SetResultNull();
        else
            ctx.SetResultText(iter->second->c_str(), (int)iter->second->size(), BeSQLite::DbFunction::Context::CopyData::No);
        }
