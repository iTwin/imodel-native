...
// @bsimethod
//---------------------------------------------------------------------------------------
static DRange2d computeRangeForText(DRange2dP justificationRange, Utf8CP chars, size_t numChars, AnnotationTextStyleCR effectiveStyle, AnnotationTextRunSubSuperScript subSuperScript, DgnDbR db)
    {
    ...
    auto& font = db.Fonts().FindFont(effectiveStyle.GetFontId());
    ...
    layoutContext.m_isItalic = (effectiveStyle.IsItalic() && FontType::TrueType == font.GetType());
...
static DRange2d computeRangeForTextRun(DRange2dP justificationRange, AnnotationTextRunCR run, size_t charOffset, size_t numChars)
    {
    return computeRangeForText(justificationRange, run.GetContent().c_str() + charOffset, numChars, *run.CreateEffectiveStyle(), run.GetSubSuperScript(), run.GetDbR());
    }
...
static DRange2d computeRangeForFractionRun(DRange2dP numeratorRange, DRange2dP denominatorRange, AnnotationFractionRunCR run)
    ...
    *numeratorRange = computeRangeForText(NULL, run.GetNumeratorContent().c_str(), run.GetNumeratorContent().size(), *effectiveStyle, AnnotationTextRunSubSuperScript::Neither, run.GetDbR());
    scaleRange(*numeratorRange, effectiveStyle->GetStackedFractionScale());

    *denominatorRange = computeRangeForText(NULL, run.GetDenominatorContent().c_str(), run.GetDenominatorContent().size(), *effectiveStyle, AnnotationTextRunSubSuperScript::Neither, run.GetDbR());
