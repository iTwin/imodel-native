...
ClipVectorPtr ViewAttachment::GetClip() const
    {
    return m_jsonProps.isMember(json_clip()) ? ClipVector::FromJson(m_jsonProps[json_clip()]) : nullptr;
    }
...
void ViewAttachment::SetClip(ClipVectorCR clipVector)
    {
    clipVector.ToJson(m_jsonProps[json_clip()]);
    }
...
void ViewAttachment::ClearClip()
    {
    m_jsonProps.removeMember(json_clip());
    }
