...
ECObjectsStatus ElementECInstanceAdapter::_InsertArrayElements(uint32_t propertyIndex, uint32_t index, uint32_t size)
    ...
    ElementAutoHandledPropertiesECInstanceAdapterPtr autoHandledProps = ElementAutoHandledPropertiesECInstanceAdapter::Create(m_element, true);
    if (autoHandledProps.IsNull())
...
void ElementAutoHandledPropertiesECInstanceAdapter::Init(bool loadProperties, size_t initialAllocation)
    ...
    if ((0 != initialAllocation) && (0 == m_element.m_ecPropertyDataSize))
        {
        if (initialAllocation >= CalculateInitialAllocation(_GetClassLayout()))
            AllocateBuffer(initialAllocation); // only pre-allocate if it is larger than minimum required
        }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ElementAutoHandledPropertiesECInstanceAdapterPtr ElementAutoHandledPropertiesECInstanceAdapter::Create(DgnElement const& el, bool loadProperties, size_t initialAllocation)
    {
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ElementAutoHandledPropertiesECInstanceAdapterPtr ElementAutoHandledPropertiesECInstanceAdapter::Create(DgnElement const& el, ECClassCR ecls, ECN::ClassLayoutCR layout, bool loadProperties, size_t initialAllocation)
    {
    if (nullptr == el.GetElementClass())
        {
        BeAssert(false); // not sure how this ever happens, but the crash reports suggest that it does
    ...
    return new ElementAutoHandledPropertiesECInstanceAdapter(el, ecls, layout, loadProperties, initialAllocation);
    }
...
ECObjectsStatus ElementAutoHandledPropertiesECInstanceAdapter::_GetStructArrayValueFromMemory(ECN::ECValueR v, ECN::PropertyLayoutCR propertyLayout, uint32_t index) const
    ...
    if (nullptr == entry)
        {
        BeAssert(false && "struct array entry was null");
        return ECObjectsStatus::Error;
        }
...
void DgnElements::ClearECCaches()
    ...
    ClearCache(); // empty element cache too
    m_stmts.Empty();
    m_classInfos.clear();
...
DgnDbStatus ElementECPropertyAccessor::SetAutoHandledPropertyValue(ECValueCR value, PropertyArrayIndex const& arrayIndex)
    ...
    ElementAutoHandledPropertiesECInstanceAdapterPtr ecdbuffer = ElementAutoHandledPropertiesECInstanceAdapter::Create(m_element, *m_eclass, *m_layout, true);
    if (ecdbuffer.IsNull() || !ecdbuffer->IsValid())
        {
        BeAssert(false);
...
DgnDbStatus DgnElement::_InsertPropertyArrayItems(uint32_t propertyIndex, uint32_t index, uint32_t size)
    {
    // Only auto-handled properties can be arrays
    ElementAutoHandledPropertiesECInstanceAdapterPtr ecPropAccess = ElementAutoHandledPropertiesECInstanceAdapter::Create(*this, true);
    if (ecPropAccess.IsNull())
        return DgnDbStatus::WrongClass;
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
void DgnElement::UnloadAutoHandledProperties() const
    {
    if (nullptr == m_ecPropertyData)
        {
        BeAssert(0 == m_ecPropertyDataSize);
        BeAssert(nullptr == m_structInstances);
        BeAssert(m_flags.m_propState < PropState::InBuffer);
        return;
        }

    ElementAutoHandledPropertiesECInstanceAdapterPtr props = ElementAutoHandledPropertiesECInstanceAdapter::Create(*this, false, 0);
    if (props.IsValid())
        props->FreeAll();

    m_flags.m_propState = PropState::Unknown;
    BeAssert(nullptr == m_ecPropertyData);
    BeAssert(0 == m_ecPropertyDataSize);
    BeAssert(nullptr == m_structInstances);
    }
