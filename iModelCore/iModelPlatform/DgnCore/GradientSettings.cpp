...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus GradientSymb::FromJson(BeJsConst json) {
    ...
    m_angle = BeJsGeomUtils::ToAngle(json["angle"]).Radians();
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
void GradientSymb::ToJson(BeJsValue value) const {
    ...
    if (0.0 != GetAngle())
        BeJsGeomUtils::FromAngle(value["angle"], Angle::FromRadians(GetAngle()));
    ...
    for (uint32_t i = 0; SUCCESS == GetKey(color, keyValue, i); i++) {
        auto keyJson = keys.appendValue();
        keyJson["value"] = keyValue;
        keyJson["color"] = color.GetValue();
    }

    if (Mode::Thematic == GetMode() && m_thematicSettings.IsValid())
        m_thematicSettings->ToJson(value["thematicSettings"]);
}

/*---------------------------------------------------------------------------------**//**
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
void ThematicGradientSettings::ToJson(BeJsValue value) const {
...
/*---------------------------------------------------------------------------------**//**
    * @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
void ThematicGradientSettings::FromJson(BeJsConst value) {
