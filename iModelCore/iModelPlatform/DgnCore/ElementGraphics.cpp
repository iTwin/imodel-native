...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
bool WireframeGeomUtil::CollectLateralRulePoints(CurveVectorCR curves, bvector<DPoint3d>& pts, uint32_t divisor, uint32_t closedDivisor, CheckStop* stopTester)
    {
    switch (curves.HasSingleCurvePrimitive())
        ...
        case ICurvePrimitive::CURVE_PRIMITIVE_TYPE_Arc:
            ...
            for (uint32_t iRule = 0; iRule < divisor; ++iRule)
                {
...
void ViewContext::_DrawStyledCurveVector(Render::GraphicBuilderR graphic, CurveVectorCR curve, Render::GeometryParamsR params, bool doCook)
    ...
    if (nullptr != lsInfo && CurveVector::BOUNDARY_TYPE_None != curve.GetBoundaryType())
        ...
        if (nullptr != currLStyle)
            ...
            auto filled = FillDisplay::Never != params.GetFillDisplay();
            if (filled)
                {
            ...
            if (Is3dView())
                ...
                graphic.AddCurveVector2d(curve, filled, params.GetNetDisplayPriority());
