...
public:
    ...
    BentleyStatus DoReadSupportsTransparency(bool& supports);
};
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus PngData::DoReadSupportsTransparency(bool& supports)
    {
    PngReader reader(*this);
    if (!reader.IsValid())
        return ERROR;

    if (setjmp(png_jmpbuf(reader.m_png)))
        return ERROR;

    reader.ReadInfo();
    supports = reader.SupportsTransparency();
    return SUCCESS;
    }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
bool ImageSource::SupportsTransparency() const
    {
    if (Format::Png != m_format)
        return false;

    PngData pngData(m_stream.GetData(), m_stream.GetSize());
    bool supports = false;
    pngData.DoReadSupportsTransparency(supports);
    return supports;
    }
