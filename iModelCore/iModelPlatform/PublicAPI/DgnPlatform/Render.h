...
private:
    ...
    uint32_t m_wiremesh: 1; //!< Display surfaces with overlaid wiremesh
...
public:
    ...
    bool GetWiremeshDisplay() const { return m_wiremesh; }
    void SetWiremeshDisplay(bool val) { m_wiremesh = val; }
    ...
    DGNPLATFORM_EXPORT void FromJson(BeJsConst);
};
...
public:
    ...
    void SetWiremeshDisplay(bool val) { m_values.SetWiremeshDisplay(val); SetPresent(kWiremesh); }
    ...
    DGNPLATFORM_EXPORT void ToJson(BeJsValue) const;
    DGNPLATFORM_EXPORT static ViewFlagsOverrides FromJson(BeJsConst);
};
...
struct Image
{
    enum class Format {Rgba=0, Rgb=2, Alpha=5};
...
public:
    ...
    //! Scale an existing Image and return the scaled one.
    //! @param[in] image the existing image
    //! @param[in] outWidth the output width of the image in pixels
    //! @param[in] outHeight the output height of the image in pixels
    DGNPLATFORM_EXPORT static Image Scale(ImageCR image, uint32_t outWidth, uint32_t outHeight);
...
public:
    ...
    //! Return whether this ImageSource is encoded in such a way that supports transparency.
    //! @note Support for transparency indicates transparent colors *may* be present - it does not imply that they *must* be present.
    DGNPLATFORM_EXPORT bool SupportsTransparency() const;
};
...
//=======================================================================================
// Categorizes the transparency of a texture image.
// NOTE: Must match TextureTransparency in @itwin/core-common.
// @bsistruct
//=======================================================================================
enum class TextureTransparency
{
    // All pixels are opaque - that is, have alpha greater than (255 - DisplayParams::GetMinTransparency()).
    Opaque,
    // All pixels are translucent - that is, have alpha less than or equal to (255 - DisplayParams::GetMinTransparency()).
    Translucent,
    // The image contains a mix of opaque and translucent pixels.
    Mixed,
};
...
public:
    ...
    bool IsCachable() const { return !m_isGlyph && !m_isTileSection; }
    ...
    virtual TextureTransparency GetTransparency() const = 0;
};
...
struct TextureMapping
{
    ...
    struct Trans2x3
    {
        ...
        DGNPLATFORM_EXPORT bool AlmostEqual(Trans2x3 const& rhs) const;
...
struct Material : RefCounted<NonCopyableClass>
{
    ...
    struct CreateParams
    {
        ...
        double m_ambient = .3;
        bool   m_shadows = true;
        RenderMaterialId m_baseId = RenderMaterialId();  // The base material if this material overrides a persistent base.
...
public:
    ...
    RenderMaterialId GetId() const {return m_params.m_key.IsPersistent() ? m_params.m_key.GetId() : m_params.m_baseId; }
...
public:
    ...
    DGNPLATFORM_EXPORT void ToJson(BeJsValue) const;
    DGNPLATFORM_EXPORT BentleyStatus FromJson(BeJsConst);
...
public:
    ...
    virtual BRepHandlePtr GetBRepHandle(GeometryStreamEntryIdCR, IBRepDataSupplierR) { return nullptr; }
    virtual void AddBRepHandle(BRepHandleR) { BeAssert(false); }
};
...
struct QPoint2d
{
    ...
    struct Params
    {
        ...
        static Params FromNormalizedRange()
            {
            return Params(DRange2d::From(DPoint2d::From(-1.0,-1.0), DPoint2d::From(1.0,1.0)));
            }
...
struct  MeshEdge
    ...
    DGNPLATFORM_EXPORT bool operator<(MeshEdge const& rhs) const;
    };
...
struct HiddenLineParams
{
    ...
    DGNPLATFORM_EXPORT void ToJson(BeJsValue) const;
    DGNPLATFORM_EXPORT static HiddenLineParams FromJson(BeJsConst);
};
...
struct AmbientOcclusionParams
{
    ...
    DGNPLATFORM_EXPORT void ToJson(BeJsValue) const;
    DGNPLATFORM_EXPORT static AmbientOcclusionParams FromJson(BeJsConst);
};
