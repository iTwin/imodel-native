...
struct EXPORT_VTABLE_ATTRIBUTE DgnModel : RefCountedBase
{
    ...
    struct CreateParams
    {
        ...
        DGNPLATFORM_EXPORT CreateParams(DgnDbR db, BeJsConst val);
...
private:
    ...
    BeJsValue GetUserPropsR() {return m_jsonProperties[json_UserProps()];}
...
protected:
    ...
    //! Convert this DgnModel to a BeJsValue.
    ...
    //! Initialize this DgnModel from a BeJsValue.
    ...
    //! Called when a DgnElement in this DgnModel is about to be moved to a different DgnModel.
    //! @param[in] element The element that is about to be moved.
    //! @param[in] targetModel The model that the element is being moved to.
    ...
    //! @note If you override this method, you @em must call the T_Super implementation.
    //! DgnModels maintain an id->element lookup table, and possibly a DgnRangeTree. The DgnModel implementation of this method maintains them.
    virtual void _OnInsertedElement(DgnElementCR element) {CallJsElementPostHandler(element.m_elementId, "onInsertedElement");}
    ...
    //! @note If you override this method, you @em must call the T_Super implementation.
    //! DgnModels maintain an id->element lookup table, and possibly a DgnRangeTree. The DgnModel implementation of this method maintains them.
    virtual void _OnUpdatedElement(DgnElementCR modified, DgnElementCR original) {CallJsElementPostHandler(modified.m_elementId, "onUpdatedElement");}
    ...
    //! @note If you override this method, you @em must call the T_Super implementation.
    //! DgnModels maintain an id->element lookup table, and possibly a DgnRangeTree. The DgnModel implementation of this method maintains them.
    virtual void _OnDeletedElement(DgnElementId id) {CallJsElementPostHandler(id, "onDeletedElement");}

    //! Called after a DgnElement in this DgnModel has been moved to a different DgnModel
    ...
    //! The subclass should import elements from the source model into this model.
    ...
    //! The subclass should import ECRelationships from the source model into this model.
...
public:
    ...
    GraphicalModel3dCP ToGraphicalModel3d() const {return _ToGraphicalModel3d();} //!< more efficient substitute for dynamic_cast<GraphicalModel3dCP>(model)
   ...
   // void AddAppData(AppData::Key const& key, AppData* appData) { BeMutexHolder lock(m_mutex); return AddAppDataInternal(key, appData); }
    ...
    //! Find or create the AppData for the specified key and - while retaining the mutex - invoke the specified function accepting a reference to the AppData.
    template<typename CreateAppData, typename Func> auto WithAppData(AppData::Key const& key, CreateAppData createAppData, Func func)
        {
        BeMutexHolder lock(m_mutex);
        auto data = ObtainAppData(key, createAppData);
        BeAssert(data.IsValid());
        if (data.IsNull())
          throw std::runtime_error("Function to create AppData must return non-null");
    ...
    BeJsConst GetJsonProperties(Utf8CP nameSpace) const {return m_jsonProperties[nameSpace];}
    ...
    void FromJson(BeJsConst val) {return _FromJson(val);}
...
protected:
    ...
    DGNPLATFORM_EXPORT virtual void _ToJson(BeJsValue out, BeJsConst opts) const override;

    explicit GeometricModel(CreateParams const& params) : T_Super(params), m_rangeIndex(nullptr), m_displayInfo(params.m_displayInfo) {}
    virtual AxisAlignedBox3d _GetElementRange(DgnElementId id) const = 0;
...
public:
    ...
    void AddElementRange(DgnElementId id, AxisAlignedBox3d box);
    void UpdateElementRange(DgnElementId id, AxisAlignedBox3d box);
    DGNPLATFORM_EXPORT void RemoveFromRangeIndex(DgnElementId);
    AxisAlignedBox3d GetElementRange(DgnElementId id) { return _GetElementRange(id); }
...
protected:
    ...
    DGNPLATFORM_EXPORT AxisAlignedBox3d _GetElementRange(DgnElementId id) const override;
...
{
    DGNMODEL_DECLARE_MEMBERS(BIS_CLASS_GraphicalModel3d, GeometricModel3d);
    friend struct dgn_ModelHandler::Graphical3d;

protected:
    GraphicalModel3dCP _ToGraphicalModel3d() const override final {return this;}
...
//! A DictionaryModel can contain @em only DefinitionElements or other *information* elements.
...
public:
    ...
    //! Create a SectionDrawingModel that breaks down the specified SectionDrawing element
    DGNPLATFORM_EXPORT static SectionDrawingModelPtr Create(SectionDrawingCR drawing);
};
...
namespace dgn_ModelHandler
{
    ...
    //! The ModelHandler for GraphicalModel3d
    struct EXPORT_VTABLE_ATTRIBUTE Graphical3d : Geometric3d
    {
        MODELHANDLER_DECLARE_MEMBERS(BIS_CLASS_GraphicalModel3d, GraphicalModel3d, Graphical3d, Geometric3d, DGNPLATFORM_EXPORT)
    };
