...
protected:
    ...
    DgnDbStatus _OnInsert(std::optional<EditOptions> options) override {auto status = CheckValid(); return DgnDbStatus::Success == status ? T_Super::_OnInsert(options) : status;}
    DgnDbStatus _OnUpdate(DgnElementCR original, std::optional<EditOptions> options) override {auto status = CheckValid(); return DgnDbStatus::Success == status ? T_Super::_OnUpdate(original, options) : status;}
    ...
    DgnDbStatus _OnChildUpdate(DgnElementCR original, DgnElementCR updated, std::optional<EditOptions> options) const override {return DgnDbStatus::InvalidParent;}
...
public:
    ...
    Utf8CP GetDetails() const { return m_jsonProps[json_details()].asCString(nullptr); }
    void SetDetails(Utf8CP details) {
        if (details)
            m_jsonProps[json_details()] = details;
        else
            m_jsonProps.removeMember(json_details());
    }

    int32_t GetDisplayPriority() const { return GetJsonProperties(json_displayPriority()).asInt(0); }
    void SetDisplayPriority(int32_t priority) { GetJsonPropertiesR(json_displayPriority()) = priority; }

    double GetScale() const { return GetJsonProperties(json_scale()).asDouble(0.0); }
    void SetScale(double scale) { GetJsonPropertiesR(json_scale()) = scale; }
