...
protected:
    ...
    DGNPLATFORM_EXPORT IFacetOptionsPtr GetScaledFacetOptions() const;
...
public:
    ...
    DGNPLATFORM_EXPORT void ProcessCurveVector(CurveVectorCR, bool filled);
    DGNPLATFORM_EXPORT void ProcessSolidPrimitive(ISolidPrimitiveCR);
    DGNPLATFORM_EXPORT void ProcessSurface(MSBsplineSurfaceCR);
    DGNPLATFORM_EXPORT void ProcessPolyface(PolyfaceQueryCR, bool filled);
    DGNPLATFORM_EXPORT void ProcessPolyfaceAsCurves(PolyfaceQueryCR);
    DGNPLATFORM_EXPORT void ProcessBody(IBRepEntityCR);
    DGNPLATFORM_EXPORT void ProcessBodyAsPolyface(IBRepEntityCR);
    DGNPLATFORM_EXPORT void ProcessText(TextStringCR);
    DGNPLATFORM_EXPORT void ProcessImage(ImageGraphicCR);
...
{
public:
    //! Specify how to process a geometric primitive that is not handled by it's specific _Process call.
    //! When returning multiple values, priority is given to the lowest value.
    enum class UnhandledPreference
    {
        ...
        Curve   = 1 << 3, //!< Process ISolidPrimitive, MSBsplineSurface, and PolyfaceQuery as CurveVector (edges), drop TextString gylphs to CurveVector.
