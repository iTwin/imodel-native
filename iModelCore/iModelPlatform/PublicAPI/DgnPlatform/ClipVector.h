...
struct ClipVector : RefCounted<T_ClipPrimitiveVector>
{
    ...
    DGNPLATFORM_EXPORT static ClipVectorPtr FromJson(BeJsConst);

    // Deserialize a ClipVector from the output of @bentley/geometry-core ClipVector.toCompactString().
    // Returns nullptr if input is invalid.
    DGNPLATFORM_EXPORT static ClipVectorPtr FromCompactString(Utf8StringCR);
};
