...
public:
    ...
    DGNPLATFORM_EXPORT void AdjustOrigin(CurveVectorCR boundary);
    DGNPLATFORM_EXPORT void GetSymbolOrientationAndScale(RotMatrixR orientation, double& scale) const;
    // Returns the number of tiles that would be produced - zero if the tile info can't be produced.
    DGNPLATFORM_EXPORT double ComputeSymbolTileInfo(SymbolTileInfo& info, CurveVectorCR boundary, DgnGeometryPartCR symbol, DPoint3dCR origin, RotMatrixCR rot, double scale) const;
    // Returns true if the GeometryParams were modified
    DGNPLATFORM_EXPORT static bool Cook(ViewContextR context, Render::GraphicBuilderR graphic, Render::GeometryParamsR params);
}; // PatternParams
