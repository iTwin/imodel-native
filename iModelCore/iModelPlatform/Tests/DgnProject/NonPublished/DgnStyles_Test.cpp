...
TEST_F(DgnLineStyleTest, UpdateLineStyle)
    ...
    DgnStyleId duplicateStyleId;
    EXPECT_TRUE(SUCCESS != styleTable.Insert(duplicateStyleId, STYLE_NAME, componentId, 53, 0.0)) << "Insert duplicate should fail";
    EXPECT_TRUE(DbResult::BE_SQLITE_OK == project->SaveChanges());
    EXPECT_FALSE(duplicateStyleId.IsValid()) << "Duplicate style id should not be set on failure";

    EXPECT_EQ(SUCCESS, styleTable.Update(newStyleId, STYLE_NAME, componentId, 53, 0.0001)) << "Update of unit def should succeed";
    EXPECT_TRUE(DbResult::BE_SQLITE_OK == project->SaveChanges());

    auto lsEl = LineStyleElement::Get(*project, newStyleId);
    ASSERT_TRUE(lsEl.IsValid());
