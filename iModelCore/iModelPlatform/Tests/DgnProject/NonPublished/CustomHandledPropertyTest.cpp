...
TEST_F(GetSetCustomHandledProprty, ElementProperties)
    ...
    if (true)
    {
        ...
        ASSERT_EQ(DgnDbStatus::Success, el.SetPropertyValue(jsindex, ECN::ECValue(json.Stringify().c_str())));
        ASSERT_EQ(DgnDbStatus::Success, el.GetPropertyValue(checkValue1, jsindex));
        ASSERT_TRUE(checkValue1.Equals(ECN::ECValue(json.Stringify().c_str())));
        checkValue1.Clear();
...
TEST_F(GetSetCustomHandledProprty, ElementProperties3d)
    ...
    checkValue1.Clear();
    // Update the element
    DgnDbStatus stat =  eledit->Update();
    ASSERT_EQ(DgnDbStatus::Success, stat);
    m_db->SaveChanges();
    }
    m_db->CloseDb();
...
TEST_F(GetSetCustomHandledProprty, CategoryProperties)
    ...
    BeJsDocument appearJson;
    subappearence.ToJson(appearJson);
    auto appval = appearJson.Stringify();
    ASSERT_EQ(DgnDbStatus::Success, editsubcategory->SetPropertyValue(scpropindex, ECN::ECValue(appval.c_str())));
    ASSERT_EQ(DgnDbStatus::Success, editsubcategory->GetPropertyValue(checkValue, scpropindex));
    ASSERT_TRUE(checkValue.Equals(ECN::ECValue(appval.c_str())));
    DgnDbStatus  stat = editcategory->Update();
    ASSERT_EQ(stat,DgnDbStatus::Success);
    stat = editsubcategory->Update();
    ASSERT_EQ(stat, DgnDbStatus::Success);
    m_db->SaveChanges();
    }
    m_db->CloseDb();
    ...
    BeJsDocument appearJson;
    subappearence.ToJson(appearJson);
    ASSERT_TRUE(checkValue.Equals(ECN::ECValue(appearJson.Stringify().c_str())));
    }
...
TEST_F(GetSetCustomHandledProprty, Annotation)
    ...
    ASSERT_EQ(DgnDbStatus::Success, textStyle->Update());
    ASSERT_EQ(DgnDbStatus::Success, FrameStyle->Update());
    ASSERT_EQ(DgnDbStatus::Success, LeaderStyle->Update());
    ASSERT_EQ(DgnDbStatus::Success, Textannoseed->Update());
    }
    m_db->SaveChanges();
    m_db->CloseDb();
    m_db = nullptr;
...
TEST_F(GetSetCustomHandledProprty, GeometryPart)
    ...
    checkValue.Clear();
    ASSERT_EQ(DgnDbStatus::Success, geoele->Update());
    }
    m_db->SaveChanges();
    m_db->CloseDb();
    m_db = nullptr;
...
TEST_F(GetSetCustomHandledProprty, Viewdefinition2d)
    ...
    checkValue.Clear();
    ASSERT_EQ(DgnDbStatus::Success, view->Update());
    }
    m_db->SaveChanges();
    m_db->CloseDb();
    m_db = nullptr;
