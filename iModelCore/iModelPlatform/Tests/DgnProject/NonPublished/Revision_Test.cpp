...
TEST_F(RevisionTestFixture, DeleteClassConstraintViolationInCacheTable)
    ...
    expectToNotThrow([&]() { EXPECT_EQ(m_db->Txns().MergeChangeset(*updatedRevision), ChangesetStatus::Success); }, "Detected 1 foreign key conflicts in ChangeSet. Aborting merge.");

    m_db->Txns().ReverseChangeset(*updatedRevision);
