...
TEST_F(ElementAspectTests, UniqueAspect_CRUD)
    ...
    if (true)
        ...
        ASSERT_EQ(DgnDbStatus::Success, m_db->Elements().Update(*tempEl));
        }
    ...
    el = m_db->Elements().Get<TestElement>(el->GetElementId()); // re-read the element to verify aspect was deleted.
    ASSERT_TRUE( el.IsValid() );
...
TEST_F(ElementAspectTests, GenericAspect_CRUD)
    ...
    if (true)
        ...
        DgnDbStatus stat;
        {
            auto createdAspectPtr = DgnElement::GenericMultiAspect::AddAspect(*tempEl, *maspect1, &stat);
    ...
    if (true)
        ...
        DgnDbStatus stat;
        {
            auto createdAspectPtr = DgnElement::GenericMultiAspect::AddAspect(*editEl, *maspect1, &stat);
            EXPECT_TRUE(createdAspectPtr.IsValid());
        }
        ASSERT_EQ(DgnDbStatus::Success, stat);
        ASSERT_EQ(DgnDbStatus::Success, editEl->Update());
    ...
    //    DgnDbStatus stat;
    //    {
    //        auto createdAspectPtr = DgnElement::GenericMultiAspect::AddAspect(*editEl, *maspectWithHandler1, &stat);
    //        EXPECT_TRUE(createdAspectPtr.IsNull());
    //    }
...
TEST_F(ElementAspectTests, PolymorphicUniqueAspects)
    ...
    if (true)
        ...
        DgnDbStatus stat;
        {
            auto createdAspectPtr = DgnElement::GenericUniqueAspect::SetAspect(*tempEl, *d1, baseClass, &stat);
            EXPECT_TRUE(createdAspectPtr.IsValid());
        }
        ASSERT_EQ(DgnDbStatus::Success, stat);
        ...
        tempEl = tempEl->UpdateAndGet()->MakeCopy<TestElement>();  // clears the aspect cache
    ...
    if (true)
        ...
        DgnDbStatus stat;
        {
            auto createdAspectPtr = DgnElement::GenericUniqueAspect::SetAspect(*tempEl, *d2, baseClass, &stat);
            EXPECT_TRUE(createdAspectPtr.IsValid());
        }
        ASSERT_EQ(DgnDbStatus::Success, stat);
        ...
        tempEl->Update();
        tempEl = m_db->Elements().Get<TestElement>(tempEl->GetElementId())->MakeCopy<TestElement>();
...
TEST_F(ElementAspectTests, GenericUniqueAspect_CRUD)
    ...
    if (true)
    {
        ...
        DgnDbStatus stat;
        {
            auto createdAspectPtr = DgnElement::GenericUniqueAspect::SetAspect(*tempEl, *uaspect, nullptr, &stat);
            EXPECT_TRUE(createdAspectPtr.IsValid());
        }
        ASSERT_EQ(DgnDbStatus::Success, stat);
        // Check that we can find the aspect that we just added to the non-persistent element
    ...
    if (true)
    {
        ...
        DgnDbStatus stat;
        {
            auto createdAspectPtr = DgnElement::GenericUniqueAspect::SetAspect(*editEl, *foundProps, nullptr, &stat);
            EXPECT_TRUE(createdAspectPtr.IsValid());
        }
        ASSERT_EQ(DgnDbStatus::Success, stat);
