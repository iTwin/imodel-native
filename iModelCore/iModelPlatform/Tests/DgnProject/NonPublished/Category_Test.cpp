...
TEST_F(CategoryTests, SubCategoryInvariants)
    ...
    ASSERT_EQ(DgnDbStatus::DuplicateCode, defaultSubCat1Edit->Update());
    db.SaveChanges();

    code = DgnSubCategory::CreateCode(db, cat2Id, "Cat1"); // Same category Code doens't effect anything.
    EXPECT_EQ(DgnDbStatus::Success, defaultSubCat1Edit->SetCode(code));
    DgnDbStatus status = defaultSubCat1Edit->Update();
    ...
    EXPECT_EQ(DgnDbStatus::Success, defaultSubCat1Edit->SetCode(code));
    status = defaultSubCat1Edit->Update();
    ASSERT_EQ(DgnDbStatus::Success, status);
    db.SaveChanges();

    // Cannot delete sub-category except via "purge"
    EXPECT_NE(DgnDbStatus::Success, defaultSubCat1->Delete());
    ...
    cpSubcat2B = db.Elements().Get<DgnSubCategory>(pSubcat2B->GetElementId());
    ...
    for (auto const& invalidChar : invalidChars)
        ...
        pSubcat2B->UpdateAndGet(&status);
        if (isspace(invalidChar)) // DgnCodeValue ignores leading and trailing whitespace
            {
            ASSERT_EQ(DgnDbStatus::Success, status);
            ASSERT_STREQ("SubCat", code.GetValueUtf8CP());
            }
        else
            {
...
TEST_F(CategoryTests, SubCategoryFill)
    ...
    DgnDbStatus updateStatus = cat1->Update();
...
TEST_F(CategoryTests, UpdateSubCategory_VerifyPresistence)
    ...
    DgnDbStatus updateStatus = subCatd->Update();
    EXPECT_TRUE(DgnDbStatus::Success == updateStatus);
