...
TEST_F(SqlFunctionsTest, placement_areaxy)
    ...
    obstacle2At90->SetSomeProperty("A");
...
TEST_F(SqlFunctionsTest, DGN_point_min_distance_to_bbox)
    ...
    DgnElementId r1, o1, o1a, o2, o2a;
        ...
        obstacle1->SetTestUniqueAspect("SomeKindOfObstacle");

        ASSERT_STREQ(obstacle1->GetTestUniqueAspect().c_str(), "SomeKindOfObstacle");
        ...
        obstacle1a->SetTestUniqueAspect("SomeOtherKindOfObstacle");

        ASSERT_STREQ(obstacle1a->GetTestUniqueAspect().c_str(), "SomeOtherKindOfObstacle");
        ...
        obstacle2->SetTestUniqueAspect("SomeKindOfObstacle");
        ...
        obstacle2a->SetTestUniqueAspect("SomeOtherKindOfObstacle");
    ...
    if (true)
        ...
        auto obstacle1 = m_db->Elements().Get<ObstacleElement>(o1);
        ASSERT_TRUE(obstacle1.IsValid());
        ASSERT_STREQ(obstacle1->GetTestUniqueAspect().c_str(), "SomeKindOfObstacle");
        ASSERT_STREQ(obstacle1->GetTestUniqueAspect().c_str(), "SomeKindOfObstacle");
        }
    }
...
TEST_F(SqlFunctionsTest, spatialQueryECSql)
    ...
    auto scanRt = qplan.find("SCAN main." DGN_VTABLE_SpatialIndex " VIRTUAL TABLE INDEX");
    ...
    if (true)
        ...
        ASSERT_EQ(m_db->Elements().Update(*robot1CC), DgnDbStatus::Success);

        stmt.Reset();
        stmt.ClearBindings();
        AxisAlignedBox3d r1aabb = robot1CC->GetPlacement().CalculateRange();      // bind to new range
        stmt.BindBlob(stmt.GetParameterIndex("bbox"), &r1aabb, sizeof(r1aabb), BeSQLite::EC::IECSqlBinder::MakeCopy::No);
        stmt.BindText(stmt.GetParameterIndex("propertyValue"), "SomeKindOfObstacle", BeSQLite::EC::IECSqlBinder::MakeCopy::No);
        ...
        stmt.Reset();
        stmt.ClearBindings();
        r1aabb = robot1CC->GetPlacement().CalculateRange();      // bind to new range
        stmt.BindBlob(stmt.GetParameterIndex("bbox"), &r1aabb, sizeof(r1aabb), BeSQLite::EC::IECSqlBinder::MakeCopy::No);
