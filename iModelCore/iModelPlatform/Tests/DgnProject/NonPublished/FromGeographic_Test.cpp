...
TEST(FromGeographicTests, NullIsland)
    ...
    GeoPoint nullIslandCarto(0, 0, 0);
    auto ecef = EcefLocation::FromGeographic(nullIslandCarto, 0);
    EXPECT_TRUE(ecef.m_isValid);
    EXPECT_NEAR(ecef.m_origin.Distance(DPoint3d(6378137.0, 0.0, 0.0)), 0.0, tolerance);
...
TEST(FromGeographicTests, WashingtonDC)
    {
    const double tolerance = 0.001;
    GeoPoint washingtonCarto(Angle::FromDegrees(-77.03652613179416).Radians(), Angle::FromDegrees(38.897712364153854).Radians(), 0); 
    auto ecef = EcefLocation::FromGeographic(washingtonCarto, 0);
    EXPECT_TRUE(ecef.m_isValid);
    EXPECT_NEAR(ecef.m_origin.Distance(DPoint3d(1115023.7963129042, -4843784.750164342, 3983485.836532083)), 0.0, tolerance);
