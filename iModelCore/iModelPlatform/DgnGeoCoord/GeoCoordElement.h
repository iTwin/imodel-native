...
typedef struct GeoCoordType66
    ...
    uint32_t    deprec_uorpersub;       // uors per sub unit (unsigned). This isn't necessarily an integer. Deprecated.
    ...
    uint16_t    gridFileFormat;         // Added feb 2021. Value of 0 indicates not set (format none)
    uint16_t    gridFileDirection;      // Added feb 2021. Related to previous
    char        gridFileName[76];       // Added feb 2021. Related to previous. Default indicates no filename
    char        filler[74];             // So that subsequent growth will not alter the stored element size
    } GeoCoordType66;
...
typedef struct Proj_lmmin
    ...
    double       org_lng; // Origin longitude of the coordinate system, i.e. where X is zero before false origin is applied.  Referred to
