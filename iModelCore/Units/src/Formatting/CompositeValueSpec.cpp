...
// @bsimethod
//--------------------------------------------------------------------------------------
bool CompositeValueSpec::ToJson(BeJsValue out, bool verbose, bool excludeUnits) const
    {
    if (IsProblem())
        return false;
...
bool CompositeValueSpec::FromJson(CompositeValueSpecR out, BeJsConst jval, BEU::IUnitsContextCP context)
    ...
    if (jval.isMember(json_units()))
        {
        auto unitsJson = jval[json_units()];
        bool parseError = false;
        unitsJson.ForEachArrayMember([&](BeJsConst::ArrayIndex, BeJsConst elem) {
            UnitProxy upp;
            if (!upp.FromJson(elem, context))
...
// @bsimethod
//----------------------------------------------------------------------------------------
bool CompositeValueSpec::UnitProxy::ToJson(BeJsValue jUP, bool verbose) const
    {
    if (nullptr == m_unit)
        return false;
    auto val = jUP.appendValue();
