...
* @bsimethod                                                    Ray.Bentley     11/2016
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus TileReader::ReadMeshVertexAttributes (bvector<double>& values, Json::Value const& primitiveValue, size_t nComponents, char const* attributeName)
    {
    Json::Value         accessor, bufferView;
...
* @bsimethod                                                    Ray.Bentley     11/2016
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus   TileReader::ReadPositions (bvector<DPoint3d>& points, Json::Value const& primitiveValue)
    {
    bvector <double>  values;

    if (SUCCESS != ReadMeshVertexAttributes (values, primitiveValue, 3, "POSITION"))
        return ERROR;

    for (size_t i=0, nPoints = values.size()/3; i<nPoints; i++)
        points.push_back(DPoint3d::From(values[i*3], values[i*3 + 1], values[i*3 + 2]));
...
TileMeshPtr  TileReader::ReadMeshPrimitive (Json::Value const& primitiveValue)
    ...
    bvector<DPoint3d>   points, normals;
    bvector<DPoint2d>   params;

    if (SUCCESS != ReadIndices (*mesh, primitiveValue) ||
        SUCCESS != ReadPositions (points, primitiveValue))
        return nullptr;
