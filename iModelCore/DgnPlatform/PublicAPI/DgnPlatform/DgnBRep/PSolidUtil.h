...
//DGNPLATFORM_EXPORT static BentleyStatus GetMaterialAttrib (char** materialString, PK_ENTITY_t entityTag);
//DGNPLATFORM_EXPORT static BentleyStatus SetMaterialAttrib (PK_ENTITY_t entityTag, char const* materialString);
//DGNPLATFORM_EXPORT static BentleyStatus DeleteMaterialAttrib (PK_ENTITY_t entityTag);
...
//DGNPLATFORM_EXPORT static BentleyStatus BodyFromCone (PK_BODY_t& bodyTag, RotMatrixCR rMatrix, BentleyApi::DPoint3dCR topCenter, BentleyApi::DPoint3dCR bottomCenter, double topRadius, double bottomRadius, bool capped, uint32_t nodeId);
...
//DGNPLATFORM_EXPORT static BentleyStatus BodyFromSweep (PK_BODY_t& bodyTag, PK_BODY_t profileTag, PK_BODY_t pathTag, PK_VERTEX_t pathVertex, bool alignParallel, bool selfRepair, BentleyApi::DVec3dCP lockDirection, double const* twistAngle, double const* scale, BentleyApi::DPoint3dCP scalePoint);
...
//! @note To be used by host in query operations. Do not pass this to any kernel api that will modify the entity, use GetEntityTagForModify instead.
...
//! @note To be used by host in operations where the entity is consumed or modified such as being a tool body in a union.
//! @return The tag value.
//DGNPLATFORM_EXPORT static PK_ENTITY_t GetEntityTagForModify(IBRepEntityR entity);

//! Extract and take ownership of solid entity associated with this instance. Entity tag should be set to the null tag and the caller is responsible for freeing it.
//! @note To be used by host in operations where the entity is consumed or modified such as being a tool body in a union.
//! @remarks This method will the null tag if the entity isn't "owned" by this instance (Host may then choose to copy the entity, see GetEntityTagForModify).
...
DGNPLATFORM_EXPORT static bool IsSmoothEdge (PK_ENTITY_t edge);
 
//DGNPLATFORM_EXPORT static BentleyStatus GetPlanarFaceData (DPoint3dP point, BentleyApi::DVec3dP normal, PK_FACE_t entityTag);
DGNPLATFORM_EXPORT static BentleyStatus CoverWires (PK_BODY_t bodyTag);
DGNPLATFORM_EXPORT static BentleyStatus ImprintSegment (PK_BODY_t bodyTag, PK_EDGE_t* edgeTag, BentleyApi::DPoint3dCP segment);
DGNPLATFORM_EXPORT static BentleyStatus MakeEllipseCurve (PK_CURVE_t* curveTag, double* startParam, double* endParam, BentleyApi::DPoint3dP center, RotMatrixP rMatrix, double x1, double x2, double startAngle, double sweepAngle);
...
//DGNPLATFORM_EXPORT static void DeleteEntity (PK_ENTITY_t entity);
