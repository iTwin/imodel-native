...
typedef struct dwgoleheader
    ...
    int32_t             inModelSpace;           // (+102 bytes) ?? 1 = model space, 0= paperspace
    int32_t             _f2_const_0x100;        // (+106 bytes) ?? 4 bytes unknown, const 0x100
    int32_t             objNumber;              // (+110 bytes) Zero based Object number (Ustn's object are 1 based)
    int32_t             activateable;           // (+114 bytes) 1 = application known (
    int16_t             insertedOrClipped;      // (+118 bytes) 0 = copied from clipboard, 1 = inserted
    int32_t             dwAspect;               // (+120 bytes) OLE aspect enumeration
    uint32_t            dwOleDataSize;          // (+124 bytes) Length in bytes of following binary stream
    } DwgOleHeader;
#pragma pack()
#endif
