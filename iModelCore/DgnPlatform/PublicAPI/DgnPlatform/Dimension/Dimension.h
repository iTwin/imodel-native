...
public:
                    explicit                        LinearDimension(CreateParams const& params);
DGNPLATFORM_EXPORT  static LinearDimensionPtr       Create(CreateParams const& params);

                    static DgnClassId               QueryClassId(DgnDbR db) { return DgnClassId(db.Schemas().GetECClassId(DGN_ECSCHEMA_NAME, DGN_CLASSNAME_LinearDimension)); }

                    static LinearDimensionCPtr      Get(DgnDbR db, DgnElementId id) { return db.Elements().Get<LinearDimension>(id); }
                    static LinearDimensionPtr       GetForEdit(DgnDbR db, DgnElementId id) { return db.Elements().GetForEdit<LinearDimension>(id); }
                    LinearDimensionCPtr             Insert() { return GetDgnDb().Elements().Insert<LinearDimension>(*this); }
                    LinearDimensionCPtr             Update() { return GetDgnDb().Elements().Update<LinearDimension>(*this); }
...
#if defined (MOVED_TO_ELEMENT)
DGNPLATFORM_EXPORT  static LinearDimensionPtr       Create(CreateParams const& params, DgnElementId dimStyleId, DPoint3dCR endPoint);
#endif
};
...
{
    DGNELEMENT_DECLARE_MEMBERS(DGN_CLASSNAME_LinearDimension3d, GraphicalElement2d);
};
#endif
...
#if defined (NOT_YET)
    //=======================================================================================
    //! The ElementHandler for LinearDimension
    //=======================================================================================
    struct LinearDimensionHandler : Annotation3d
    {
        ELEMENTHANDLER_DECLARE_MEMBERS(DGN_CLASSNAME_LinearDimension, LinearDimension, LinearDimensionHandler, Annotation2d, DGNPLATFORM_EXPORT);
        virtual void _GetClassParams(ECSqlClassParams& params) override;
    };
#endif
}
