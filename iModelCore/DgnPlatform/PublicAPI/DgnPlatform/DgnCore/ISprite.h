...
/*=================================================================================**//**
 ...
 change both position and which Sprite Definition is shown at that position over time.
  ...
  Sprite definitions are generally created as Windows Icon resources, which are in turn Windows Bitmaps. Sprites
 can be of varying sizes and color depths and can have both opaque and transparent pixels. Icon Resources have a "hotspot"
 associated with them. The hotspot is the pixel that is the the "origin" of the sprite.

 At a given time there can be many Sprites Locations shown in a Viewport. However, since the entire scene is
 refreshed from the backingStore for every frame (see discussion of Scene Creation in the detailed description of \ref Viewports), and
 ...
 @note It is also possible to draw an ISprite onto a Viewport directly (via calls to IViewDraw::DrawSprite)
...
/*=================================================================================**//**
 ...
 Generally loaded once at startup via IViewManager::LoadSpriteFromRsrc and saved for the entire
 session. However, applications can implement this interface on other objects that are able to supply an icon resource.
 @bsiinterface
+===============+===============+===============+===============+===============+======*/
...
//! @param[in]      viewport        The Viewport onto which the Sprite Definition is drawn
