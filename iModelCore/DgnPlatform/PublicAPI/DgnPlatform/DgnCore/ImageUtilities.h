...
struct ImageUtilities
{
    ...
    struct RgbImageInfo
        {
        UInt32  height, width;  //!< height and width of image in pixels
    ...
    DGNPLATFORM_EXPORT static BentleyStatus WriteImageToPngFormat (bvector<UInt8>& pngData, bvector<UInt8>& imageData, RgbImageInfo const& info);
    ...
    //! they are set to 0xff.
    //! @remarks The PNG data is written in compressed format with 8-bit color.
    DGNPLATFORM_EXPORT static BentleyStatus WriteImageToPngFile (BeFile& pngFile, bvector<UInt8>& imageData, RgbImageInfo const& info);
