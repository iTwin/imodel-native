...
public:
    ...
    public:
        ...
        bool                m_isHandlerCreated;

        DGNPLATFORM_EXPORT CreateParams(DgnDbR, JsonValueCR);
        CreateParams(DgnDbR db, DgnModelId modelId, DgnClassId classId, DgnCodeCR code=DgnCode(), Utf8CP label=nullptr, DgnElementId parentId=DgnElementId(), DgnClassId parentRelClassId=DgnClassId(), BeSQLite::BeGuidCR federationGuid=BeSQLite::BeGuid(), bool isHandlerCreated = false)
            : m_dgndb(db), m_modelId(modelId), m_classId(classId), m_code(code), m_parentId(parentId), m_parentRelClassId(parentRelClassId), m_federationGuid(federationGuid), m_isHandlerCreated(isHandlerCreated) {SetUserLabel(label);}
