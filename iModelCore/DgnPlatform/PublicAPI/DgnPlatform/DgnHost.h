...
#define ON_HOST_TERMINATE(obj, isProgramExit) {if (obj) {obj->OnHostTermination(isProgramExit); obj = nullptr;}}
...
struct DgnHost : NonCopyableClass
{
    ...
    template<typename T> struct HostValue : IHostObject
    {
    private:
        T   m_value;
    public:
...
public:
    ...
    //! the value will be 0.
    template<typename INT_TYPE> INT_TYPE GetHostIntVariable(Key& key) {return (INT_TYPE)(intptr_t)GetHostVariable(key);}
    ...
    template <typename INT_TYPE> void SetHostIntVariable(Key& key, INT_TYPE val) {SetHostVariable(key, (void*)(intptr_t)val);}
