...
void PickOutput::_AddHit(DPoint4dCR hitPtScreen, DPoint3dCP hitPtLocal, HitPriority priority)
    ...
    if (nullptr == (source = m_context->GetCurrentElement()) && nullptr == m_context->GetElemTopology())
        return;
    ...
    RefCountedPtr<HitDetail> thisHit = new HitDetail(*m_context->GetViewport(), source, m_pickPointWorld, m_options.GetHitSource(), m_currGeomDetail);
