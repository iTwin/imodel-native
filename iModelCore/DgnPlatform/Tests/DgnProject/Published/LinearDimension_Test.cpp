...
//    ASSERT_TRUE(m_categoryId.IsValid());
    ...
    // Create a 3d model
/*
    DgnModelPtr model = new SpatialModel(SpatialModel::CreateParams(GetDgnDb(), DgnClassId(GetDgnDb().Schemas().GetECClassId(DGN_ECSCHEMA_NAME, DGN_CLASSNAME_SpatialModel)), DgnModel::CreateModelCode(m_modelName)));
    ASSERT_TRUE(DgnDbStatus::Success == model->Insert());
