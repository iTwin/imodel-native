...
struct DgnModelTests : public DgnDbTestFixture
    ...
    void CreateModel(Utf8StringCR modelCode)
        {
        DgnModelPtr m_model = m_defaultModelP->Clone(DgnModel::CreateModelCode(modelCode));
        m_model->SetLabel("Test_splines");
        m_model->Insert();
...
TEST_F(DgnModelTests, GetGraphicElements)
    ...
    CreateModel("Splines");
    //Insert 3d Element
    InsertElement(*m_db, m_db->Models().QueryModelId(DgnModel::CreateModelCode("Splines")), true, true);
...
TEST_F(DgnModelTests, GetRange)
    ...
    //Insert new model
    CreateModel("RangeTest");
    //Insert 3d Element
    InsertElement(*m_db, m_db->Models().QueryModelId(DgnModel::CreateModelCode("RangeTest")), true, true);
...
TEST_F(DgnModelTests, WorkWithDgnModelTable)
    ...
    TestModelProperties models[2], testModel;
    models[0].SetTestModelProperties(L"DefaultModel", L"");
    models[1].SetTestModelProperties(L"Splines", L"Test_splines");
...
TEST_F(DgnModelTests, ImportDictionaryModel)
    ...
    DgnPlatformTestDomain::ImportDummySchema(*dbcopy);
    DgnPlatformTestDomain::ImportSchema(*dbcopy);
