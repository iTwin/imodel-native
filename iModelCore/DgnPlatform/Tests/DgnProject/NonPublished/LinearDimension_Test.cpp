...
void SetUp () override
    ...
    // Create a 2d model
    DgnModelPtr model = new GeometricModel2d(GeometricModel2d::CreateParams(GetDgnDb(), DgnClassId(GetDgnDb().Schemas().GetClassId(BIS_ECSCHEMA_NAME, BIS_CLASS_GeometricModel2d)), DgnModel::CreateModelCode(m_modelName)));
...
//    ASSERT_TRUE(m_categoryId.IsValid());
    ...
    // Create a 3d model
/*
    DgnModelPtr model = new SpatialModel(SpatialModel::CreateParams(GetDgnDb(), DgnClassId(GetDgnDb().Schemas().GetClassId(BIS_ECSCHEMA_NAME, BIS_CLASS_SpatialModel)), DgnModel::CreateModelCode(m_modelName)));
    ASSERT_TRUE(DgnDbStatus::Success == model->Insert());
