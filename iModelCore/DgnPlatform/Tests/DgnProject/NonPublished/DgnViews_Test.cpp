...
struct DgnViewElemTest : DgnViewsTest
{
    void SetupProject()
        ...
        for (auto const& entry : project->Views().MakeIterator())
            {
            DrawingViewDefinition viewDef(DrawingViewDefinition::CreateParams(*project, DrawingViewDefinition::CreateCode(entry.GetName()),
                        DrawingViewDefinition::QueryClassId(*project),
                        DrawingViewDefinition::Data(entry.GetBaseModelId(), entry.GetDgnViewSource(), entry.GetDescription())));
            viewDef.Insert();
            }
...
TEST_F(DgnViewElemTest, WorkWithViewTable)
    ...
    for (auto const& entry : iter)
        {
        WString entryNameW (entry.GetName(), true);
        testView.SetTestViewProperties (entryNameW.c_str(), DgnViewType::Drawing);
        testView.IsEqual (fileViews[i]);
        i++;
        }
    }
...
TEST_F(DgnViewElemTest, CRUD)
    ...
    // Create a new view
    CameraViewDefinition tempView(CameraViewDefinition::CreateParams(*project,
                                  CameraViewDefinition::CreateCode("TestView"),
                                  CameraViewDefinition::QueryClassId(*project),
                                  CameraViewDefinition::Data(DgnModelId((uint64_t)2), DgnViewSource::User, "Test Description")));
    DrawingViewDefinition tempView2(DrawingViewDefinition::CreateParams(*project,
                                    DrawingViewDefinition::CreateCode("TestDrawingView"),
                                    DrawingViewDefinition::QueryClassId(*project),
