...
TEST_F (DomainTests, CheckDomainProperties)
    ...
    BentleyStatus status = Domain_DgnSchema::GetDomain ().ImportSchema (*m_db, DgnPlatform_schemaFile);
    ...
    BentleyStatus status1 = Domain_TestSchema::GetDomain ().ImportSchema (*m_db, Test_schemaFile);
    ASSERT_TRUE (BentleyStatus::SUCCESS == status1);
...
Setup_Project (L"3dMetricGeneral.idgndb", L"DomainTests.idgndb", Db::OpenMode::ReadWrite);
  ...
  BentleyStatus stat = Domain_TestSchema::GetDomain ().ImportSchema (*m_db, Test_schemaFile1);
  EXPECT_FALSE (BentleyStatus::SUCCESS == stat);
}
...
TEST_F (DomainTests, MultipleDomainRegister)
    ...
    BentleyStatus status = Domain_TestSchema::GetDomain ().ImportSchema (*m_db, Test_schemaFile);
    ASSERT_TRUE (BentleyStatus::SUCCESS == status);
    ...
    Utf8CP name=domain->GetDomainName ();
    EXPECT_EQ ("TestSchema", (Utf8String)name);

    //These tests shows that the domain registered second is not in the DB.
    Utf8CP desc=domain->GetDomainDescription ();
    EXPECT_NE ("Test Schema Domain 2", (Utf8String)desc);
...
TEST_F (DomainTests, MultipleSchemasImport)
    ...
    BentleyStatus status = Domain_DgnSchema::GetDomain ().ImportSchema (*m_db, DgnPlatform_schemaFile);
    EXPECT_TRUE (BentleyStatus::SUCCESS == status);

    //Multiple imports of the same schema. Does'nt gives any error.
    BentleyStatus Status1 = Domain_DgnSchema::GetDomain ().ImportSchema (*m_db, DgnPlatform_schemaFile);
    EXPECT_TRUE (BentleyStatus::SUCCESS == Status1);
    ...
    BentleyStatus Status = Domain_DgnSchema::GetDomain ().ImportSchema (*m_db, Test_schemaFile);
    EXPECT_FALSE (BentleyStatus::SUCCESS == Status);
