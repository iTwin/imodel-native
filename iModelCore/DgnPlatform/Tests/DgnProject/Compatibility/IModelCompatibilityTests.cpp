        ...
        switch (originalFileAge)
            {
                ...
                EXPECT_EQ(SchemaVersion(5, 0, 0), notUpgradedDb.GetSchemaVersion("ECDbSystem")) << "Schema has no enums, so was not upgraded to EC32, so no original ECXML persisted yet | " << notUpgradedDb.GetDescription();
                ...
                //ECDb built-in schema versions
                EXPECT_EQ(SchemaVersion(2, 0, 0), upgradedDb.GetSchemaVersion("ECDbFileInfo")) << upgradedDb.GetDescription();
                ...
                EXPECT_EQ(SchemaVersion(4, 0, 0), upgradedDb.GetSchemaVersion("ECDbMeta")) << notUpgradedDb.GetDescription();
                EXPECT_EQ(JsonValue(R"js({"classcount":24, "enumcount": 8})js"), upgradedDb.GetSchemaItemCounts("ECDbMeta")) << upgradedDb.GetDescription();
                EXPECT_EQ(SchemaVersion(5, 0, 0), upgradedDb.GetSchemaVersion("ECDbSystem")) << "Schema has no enums, so was not upgraded to EC32, so no original ECXML persisted yet | " << upgradedDb.GetDescription();
