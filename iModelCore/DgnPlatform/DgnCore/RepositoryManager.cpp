...
#define STMT_SelectElementInModel " SELECT Id FROM " BIS_TABLE(DGN_CLASSNAME_Element) " WHERE ModelId=?"
...
void IBriefcaseManager::RemoveElements(LockRequestR request, DgnModelId modelId) const
    {
    Statement stmt(m_db, "SELECT Id FROM " BIS_TABLE(DGN_CLASSNAME_Element) " WHERE ModelId=?");
