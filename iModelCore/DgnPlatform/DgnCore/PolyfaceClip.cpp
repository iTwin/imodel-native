/*--------------------------------------------------------------------------------------+
|
|     $Source: DgnCore/PolyfaceClip.cpp $
|
...
* @bsimethod                                                    RayBentley      05/2012
+---------------+---------------+---------------+---------------+---------------+------*/
double   ClipUtil::ComputeFrustumOverlap (ViewportR viewport, DPoint3dCP testFrustumRootPoints) 
    {
...
static bool    computeRayPlanesIntersection (double& tNear, double& tFar, DVec3dCR rayDirection, DPoint3dCR rayOrigin, T_ClipPlaneVector& planes)
    ...
    for (T_ClipPlaneVector::iterator curr = planes.begin(), end = planes.end(); curr != end; curr++)
        {
        double          vD = (*curr)->m_normal.dotProduct (&rayDirection),
                        vN = (*curr)->m_normal.dotProduct (&rayOrigin) - (*curr)->m_distance;
...
static bool intersectPlaneEdgesWithPlanes (DRange3dR intersectRange, T_ClipPlaneVector& planes)
    ...
    for (T_ClipPlaneVector::iterator curr = planes.begin(), end = planes.end(), other; curr != end; ++curr)
        ...
        for (other = curr, ++other; other != end; other++)
            ...
            origin.sumOf (NULL, &(*curr)->m_normal, (*curr)->m_distance);
            otherOrigin.sumOf (NULL, &(*other)->m_normal, (*other)->m_distance);

            if (bsiGeom_planePlaneIntersection (&intersectionOrigin, &intersectionNormal, &origin, &(*curr)->m_normal, &otherOrigin, &(*other)->m_normal))
                {
                double      tNear, tFar;
    ...
    if (!intersectFound)
        ...
        for (T_ClipPlaneVector::iterator curr = planes.begin(), end = planes.end(), other; curr != end; ++curr)
            ...
            for (other = curr, ++other; other != end; other++)
                {
                if ((*curr)->m_normal.dotProduct(&(*other)->m_normal) < 0.0 &&
                    (*curr)->m_distance > -(*other)->m_distance)
                    return false;
                }
            }                                     
        return true;
