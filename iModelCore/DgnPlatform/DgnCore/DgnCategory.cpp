...
Json::Value DgnSubCategory::Appearance::ToJson() const
    ...
    if (m_style.IsValid())      val[json_style()]  = m_style.ToHexStr();
...
void DgnSubCategory::Override::ToJson(JsonValueR outValue) const
    ...
    if (m_flags.m_style)        outValue[Appearance::json_style()] = m_value.GetStyle().ToHexStr();
    if (m_flags.m_material)     outValue[Appearance::json_material()] = m_value.GetRenderMaterial().ToHexStr();
