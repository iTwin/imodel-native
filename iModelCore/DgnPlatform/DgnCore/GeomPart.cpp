...
DgnDbStatus DgnGeometryPart::WriteGeometryStream()
    ...
    DgnDbR db = GetDgnDb();
    return GeometryStream::WriteGeometryStream(db.Elements().GetSnappyTo(), db, GetElementId(), BIS_TABLE(DGN_CLASSNAME_DefinitionElement), "sc01"); // NOTE: takes advantage of knowing how the EC --> SQLite mapping will turn out!
    }
