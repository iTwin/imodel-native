...
MeshEdges::MeshEdges(MeshCR mesh)
    ...
    struct  PointMap : bmap<QPoint3d, uint32_t, PointComparator> 
        ...
        uint32_t GetIndex(QPoint3dCR point)
            {
    ...
    bvector<DPoint3d>           triangleNormals(triangles.size());
    ...
    for (uint32_t triangleIndex = 0; triangleIndex < triangles.size(); triangleIndex++)
        ...
        for (size_t j=0; j<3; j++)
            {
            MeshEdge        meshEdge(triangle[j], triangle[(j+1)%3]);
        ...
        triangleNormals[triangleIndex] = mesh.GetTriangleNormal(triangle);
        }
    ...
    for (auto& edge : edgeMap)
        ...
        if (edge.second.m_visible)    
            ...
            if (2 == edge.second.m_faceCount)
                {
                DPoint3d const&  normal0 = triangleNormals[edge.second.m_faceIndices[0]];
                DPoint3d const&  normal1 = triangleNormals[edge.second.m_faceIndices[1]];

                if (!normal0.IsParallelTo(normal1))
                    {
                    // Potential silhouettes.
                    m_silhouette.push_back(edge.second.m_edge);
                    m_silhouetteNormals0.Add(normal0);
                    m_silhouetteNormals1.Add(normal1);
                    }
                }
            }
        }
