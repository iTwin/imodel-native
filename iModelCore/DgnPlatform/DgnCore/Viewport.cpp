...
static  UInt32  s_rasterLinePatterns[8] =
    {
    0xffffffff,     // 0
    0x80808080,     // 1
    0xf8f8f8f8,     // 2
...
Viewport::Viewport ()
    {
    m_viewNumber        = -1;
    m_tempClipElementRef = NULL;
    m_tempClipMaskElementRef = NULL;
    m_rangeStamp        = 0;
    ...
    m_backgroundColor.m_int = 0;
    m_output             = NULL;
    m_bgTextureProvider  = NULL;
    }
...
+---------------+---------------+---------------+---------------+---------------+------*/
double Viewport::GetViewNoClipZMargin ()
    {
...
* @bsimethod                                    Sam.Wilson                      12/2008
+---------------+---------------+---------------+---------------+---------------+------*/
double Viewport::GetMinViewDelta ()
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::InitViewSettings (bool useBgTexture, CookedDisplayStyleP displayOverrides)
    {
    BeAssert (m_output);

    m_output->SetViewAttributes (*GetViewFlags(), m_backgroundColor.m_rgb, useBgTexture, _WantAntiAliasLines(), _WantAntiAliasText(), displayOverrides);
    m_qvParamsSet = true;
    }
...
* @bsimethod                                                    KeithBentley    04/02
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::SetDisplayFlagFill (bool newValue)
    {
...
* @bsimethod                                                    KeithBentley    04/02
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::SetDisplayFlagPatterns (bool newValue)
    {
...
* @bsimethod                                                    KeithBentley    04/02
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::_GetViewCorners (DPoint3dR llb, DPoint3dR urf) const
    {
...
* @bsimethod                                                    KeithBentley    11/02
+---------------+---------------+---------------+---------------+---------------+------*/
BSIRect Viewport::GetViewRect () const
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::ViewToNpc (DPoint3dP npcVec, DPoint3dCP screenVec, int nPts) const
    {
    DPoint3d        llb, urf;
...
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::NpcToView (DPoint3dP screenVec, DPoint3dCP npcVec, int nPts) const
    {
    DPoint3d        llb, urf;
    _GetViewCorners (llb, urf);
...
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::NpcToWorld (DPoint3dP rootPts, DPoint3dCP npcPts, int nPts) const
    {
...
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::WorldToNpc (DPoint3dP npcPts, DPoint3dCP rootPts, int nPts) const
    {
...
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::ViewToWorld (DPoint3dP rootPts, DPoint4dCP screenPts, int nPts) const
    {
    DPoint4d  tPt;
...
* @bsimethod                                                    KeithBentley    11/02
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::ViewToScreen (DPoint3dP screenPts, DPoint3dCP viewPts, int nPts) const
    {
...
* @bsimethod                                                    KeithBentley    11/02
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::ScreenToView (DPoint3dP viewPts, DPoint3dCP screenPts, int nPts) const
    {
...
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::WorldToView (DPoint4dP screenPts, DPoint3dCP rootPts, int nPts) const
    {
...
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::WorldToView (DPoint3dP viewPts, DPoint3dCP rootPts, int nPts) const
    {
...
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::WorldToView2d (DPoint2dP viewPts, DPoint3dCP rootPts, int nPts) const
    {
    DPoint3d    dPt;
...
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::ViewToWorld (DPoint3dP rootPts, DPoint3dCP viewPts, int nPts) const
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::_AdjustAspectRatio (ViewControllerR viewController, bool expandView)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
StatusInt Viewport::RootToNpcFromViewDef (DMap4dR rootToNpc, double* compressionFraction, CameraInfo const* camera,
...
+---------------+---------------+---------------+---------------+---------------+------*/
StatusInt Viewport::_ConnectToOutput ()
    {
    if (m_qvDCAssigned)
        return SUCCESS;
...
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::CalcNpcToView (DMap4dR npcToView)
    {
...
* @bsimethod                                                    Andrew.Edge     08/04
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::_SetFrustumFromRootCorners (DPoint3dCP rootBox, double compressionFraction)
    {
    DPoint3d frustum[4];
...
* @bsimethod                                                    KeithBentley    04/02
+---------------+---------------+---------------+---------------+---------------+------*/
StatusInt Viewport::_SetupFromViewController ()
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::_ResynchColorMap()
    {
    if (NULL == m_output)
        return;

    DgnColorMapCP colorMap = m_viewController->GetDgnProject().Colors().GetDgnColorMap();
    if (!colorMap)
        {
        BeAssert (0);
        return;
    ...
    _GetWindowBgColor (m_backgroundColor.m_rgb);

    m_output->DefineColorMap (colorMap);
    m_output->ModifyColorMapEntry (DgnColorMap::INDEX_Background, GetBackgroundColor ());
    }
...
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::FixFrustumOrder (Frustum& frustum)
    {
...
* @bsimethod                                                    KeithBentley    11/02
+---------------+---------------+---------------+---------------+---------------+------*/
StatusInt Viewport::SetupFromFrustum (Frustum const& inFrustum)
    {
    ViewControllerP   viewController = m_viewController.get();
...
+---------------+---------------+---------------+---------------+---------------+------*/
StatusInt Viewport::ChangeArea (DPoint3dCP pts)
    {
    ViewControllerP viewController = m_viewController.get();
    if (NULL == viewController)
        return  ERROR;
...
+---------------+---------------+---------------+---------------+---------------+------*/
Frustum Viewport::GetFrustum(DgnCoordSystem sys, bool expandedBox) const
    {
    Frustum  box;
...
+---------------+---------------+---------------+---------------+---------------+------*/
DPoint3d Viewport::DetermineDefaultRotatePoint()
    {
    double low, high;
...
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
StatusInt Viewport::Scroll (Point2dCP screenDist) // => distance to scroll in pixels
    {
    ViewControllerP   viewController = m_viewController.get();
    if (NULL == viewController)
        return ERROR;
...
+---------------+---------------+---------------+---------------+---------------+------*/
DPoint3d Viewport::GetCameraTarget () const
    {
    DVec3d viewZ;
...
+---------------+---------------+---------------+---------------+---------------+------*/
double Viewport::GetFocusPlaneNpc ()
    {
...
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
StatusInt Viewport::Zoom (DPoint3dCP newCenterRoot, double factor)
    {
    ViewControllerP   viewController = m_viewController.get();
    if (NULL == viewController)
        return ERROR;
...
/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    Brien.Bastings  08/08
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::CheckCacheRangeExpanded ()
    {
#ifdef DGNV10FORMAT_CHANGES_WIP
    DgnModelP   rootCache = GetRootModel();

    // if the range of the cache changed, we have to recalculate the root to screen transform
    if (rootCache && m_rangeStamp != rootCache->GetRangeStamp() && m_output && !m_output->IsDrawActive())
        _SetupFromViewController ();
#endif
    }
...
+---------------+---------------+---------------+---------------+---------------+------*/
ViewportStatus Viewport::_Activate (QvPaintOptions const& opts)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
int Viewport::GetDefaultIndexedLineWidth (int index)
    {
...
* @bsimethod                                                    KeithBentley    04/02
+---------------+---------------+---------------+---------------+---------------+------*/
int Viewport::_GetIndexedLineWidth (int index) const
    {
    return Viewport::GetDefaultIndexedLineWidth (index);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    BrienBastings   11/09
+---------------+---------------+---------------+---------------+---------------+------*/
UInt32 Viewport::GetDefaultIndexedLinePattern (int index)
    {
    if (index < 0 || index > 7)
        index = 0;
...
* @bsimethod                                                    KeithBentley    04/02
+---------------+---------------+---------------+---------------+---------------+------*/
UInt32 Viewport::_GetIndexedLinePattern (int index) const
    {
    return Viewport::GetDefaultIndexedLinePattern (index);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    KeithBentley    04/02
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::SetSymbologyRgb (UInt32 lineColorTBGR, UInt32 fillColorTBGR, int lineWidth, int lineCodeIndex)
    {
    m_output->SetSymbology (lineColorTBGR, fillColorTBGR, lineWidth, _GetIndexedLinePattern (lineCodeIndex));
    }
...
+---------------+---------------+---------------+---------------+---------------+------*/
static double colorVisibilityCheck (IntColorDef& fg, IntColorDef& bg)
    {
    // Compute luminosity...
    double      red   = abs (fg.m_rgb.red   - bg.m_rgb.red);
    double      green = abs (fg.m_rgb.green - bg.m_rgb.green);
    double      blue  = abs (fg.m_rgb.blue  - bg.m_rgb.blue);
...
* @bsimethod                                                    BrienBastings   08/01
+---------------+---------------+---------------+---------------+---------------+------*/
static void adjustHSVColor (HsvColorDef* fgHsvP, bool darkenColor, int delta)
    {
    if (darkenColor)
        {
        int     weightedDelta = delta*HSV_VALUE_WEIGHT;

        if (fgHsvP->value >= weightedDelta)
            {
            fgHsvP->value -= weightedDelta;
            }
        else
            {
            weightedDelta -= fgHsvP->value;

            fgHsvP->value = 0;
            fgHsvP->saturation = fgHsvP->saturation + weightedDelta < 100 ? fgHsvP->saturation + weightedDelta : 100;
            }
        }
    else
        {
        ...
        if (fgHsvP->saturation >= weightedDelta)
            {
            fgHsvP->saturation -= weightedDelta;
            }
        else
            {
            weightedDelta -= fgHsvP->saturation;

            fgHsvP->saturation = 0;
            fgHsvP->value = fgHsvP->value + weightedDelta < 100 ? fgHsvP->value + weightedDelta : 100;
            }
        }
    }
...
+---------------+---------------+---------------+---------------+---------------+------*/
static void adjustColorForContrast (IntColorDef& fg, IntColorDef& bg, IntColorDef& vw)
    {
    ...
    ColorUtil::ConvertRgbToHsv (&fgHSV, &fg.m_rgb);
    ...
    IntColorDef bright, darker;

    bright = darker = fg; // NOTE: Initialize to original color to preserve transparency
    ColorUtil::ConvertHsvToRgb (&darker.m_rgb, &darkerHSV);
    ColorUtil::ConvertHsvToRgb (&bright.m_rgb, &brightHSV);

    if (bright.m_int == bg.m_int) // Couldn't adjust brighter...
        {
        fg.m_int = darker.m_int;

        return;
        }

    if (darker.m_int == bg.m_int) // Couldn't adjust darker...
        {
        fg.m_int = bright.m_int;

        return;
        }

    // NOTE: Best choice is the one most visible against the background color...
    fg.m_int = (colorVisibilityCheck (bright, vw) >= colorVisibilityCheck (darker, vw)) ? bright.m_int : darker.m_int;
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    KeithBentley    12/01
+---------------+---------------+---------------+---------------+---------------+------*/
UInt32 Viewport::AdjustColorForContrast (UInt32 thisColor, UInt32 againstColor) const
    {
    IntColorDef fg, bg, vw;

    fg.m_int = thisColor;
    bg.m_int = againstColor;
    vw.m_int = GetBackgroundColor ();
...
* @bsimethod                                                    KeithBentley    11/02
+---------------+---------------+---------------+---------------+---------------+------*/
UInt32 Viewport::MakeTrgbColor (int red, int green, int blue, int transparency) 
    {
    return  MakeColorTransparency ((((blue & 0xff) << 16) | ((green & 0xff) << 8) | (red & 0xff)), transparency);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    KeithBentley    11/02
+---------------+---------------+---------------+---------------+---------------+------*/
UInt32 Viewport::MakeTransparentIfOpaque (UInt32 color, int transparency) 
    {
...
* @bsimethod                                                    KeithBentley    11/02
+---------------+---------------+---------------+---------------+---------------+------*/
UInt32 Viewport::MakeColorTransparency (UInt32 color, int transparency) 
    {
    LIMIT_RANGE (0, 255, transparency);

    return  (transparency << 24) | (color & 0xffffff);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    RayBentley      08/2011
+---------------+---------------+---------------+---------------+---------------+------*/
UInt32 Viewport::GetSolidFillEdgeColor (UInt32 rawElementColor, DgnModelP modelRef)
    {
    if (255 == rawElementColor)      // Special case for background color.
        return GetSolidFillEdgeColor (GetBackgroundColor());

    IntColorDef colorDef;

    colorDef.m_int = 0;
    modelRef->GetDgnProject().Colors().Extract(&colorDef, NULL, NULL, NULL, NULL, rawElementColor);
    return GetSolidFillEdgeColor (colorDef.m_int);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    RayBentley      08/2011
+---------------+---------------+---------------+---------------+---------------+------*/
UInt32 Viewport::GetSolidFillEdgeColor (UInt32 inColor)
    {
    // Logic derived from QVCamera::calcContrast...
    IntColorDef     backgroundColorDef, inColorDef, outColorDef;

    backgroundColorDef.m_int = GetBackgroundColor ();
    inColorDef.m_int = inColor;

    double  elemColor[3], bgRGBA[3];

    bgRGBA[0] = backgroundColorDef.m_rgb.red   / 255.0;
    bgRGBA[1] = backgroundColorDef.m_rgb.green / 255.0;
    bgRGBA[2] = backgroundColorDef.m_rgb.blue  / 255.0;

    elemColor[0] = inColorDef.m_rgb.red   / 255.0;
    elemColor[1] = inColorDef.m_rgb.green / 255.0;
    elemColor[2] = inColorDef.m_rgb.blue  / 255.0;
    ...
    outColorDef.m_rgb.red = outColorDef.m_rgb.green = outColorDef.m_rgb.blue = (unsigned char) (255.0 * s);
    return outColorDef.m_int;
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    BrienBastings   02/03
+---------------+---------------+---------------+---------------+---------------+------*/
double Viewport::GetPixelSizeAtPoint (DPoint3dCP rootPtP, DgnCoordSystem coordSys) const // can be NULL - if so, use center of view
    {
    DPoint3d    rootTestPt;
...
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::OutputFrustumErrorMessage (StatusInt errorStatus)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
ViewFrustumStatus Viewport::ValidateWindowSize (DPoint3dR delta, bool messageNeeded)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
Int32 Viewport::GetMaxDisplayPriority() {return MAX_HW_DISPLAYPRIORITY;}
Int32 Viewport::GetDisplayPriorityFrontPlane() {return GetMaxDisplayPriority() - RESERVED_DISPLAYPRIORITY;}                              

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    RayBentley  10/06
+---------------+---------------+---------------+---------------+---------------+------*/
bool Viewport::UseClipVolume (DgnModelCP modelRef) const
    {
    if (!IsActive())
        return false;
...
* @bsimethod                                                    RayBentley  10/06
+---------------+---------------+---------------+---------------+---------------+------*/
StatusInt Viewport::GetDynamicViewClip (EditElementHandleR elHandle, DgnModelP modelRef) const
    {
    return ERROR;
#ifdef DGN_IMPORTER_REORG_WIP
    DynamicViewSettingsCP    settings;

    if (!UseClipVolume (modelRef) || (NULL == (settings = GetDynamicViewSettings ())) || !settings->NonStandard ())
        return ERROR;

    return settings->GetClipBoundElemHandle (elHandle, modelRef);
#endif
    }

#if defined (NEEDS_WORK_DGNITEM)
/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    Jeff.Marker     12/07
+---------------+---------------+---------------+---------------+---------------+------*/
ShadowHealState::ShadowHealState ()
    {
    ResetDisplayStyleItemState ();
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    Jeff.Marker     12/07
+---------------+---------------+---------------+---------------+---------------+------*/
void    ShadowHealState::ResetDisplayStyleItemState ()
    {
    m_hasConfiguredDisplayStyleItem = false;
    m_shadowHealStartTick           = 0;
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    Ray.Bentley     03/08
+---------------+---------------+---------------+---------------+---------------+------*/
void ShadowHealState::ClearDirty ()
    {
    m_dirtyRect.Init (LMAXI4, LMAXI4, LMINI4, LMINI4);
    }
#endif
...
+---------------+---------------+---------------+---------------+---------------+------*/
UInt32 Viewport::GetContrastToBackgroundColor () const
    {
    // should we use black or white
    bool    invert  = ((m_backgroundColor.m_rgb.red + m_backgroundColor.m_rgb.green + m_backgroundColor.m_rgb.blue) > (255*3)/2);
    return  invert ? 0  : 0xffffff;
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                    Keith.Bentley                   03/07
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::_GetViewName (WStringR name) const
    {
    name = L"";
    if (!Is3dView())
        return;
...
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::_SynchWithViewController (bool saveInUndo)
    {
    _SetupFromViewController();
...
//---------------------------------------------------------------------------------------
void Viewport::SetToolGraphicsHandler(ToolGraphicsHandler* handler)
    {
...
// @bsimethod                                                   John.Gooding    05/2013
//---------------------------------------------------------------------------------------
void Viewport::DrawToolGraphics(ViewContextR context, bool isPreupdate)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
double Viewport::GetGridScaleFactor ()
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::GetGridRoundingDistance (DPoint2dR roundingDistance)
    {
#ifdef DGNV10FORMAT_CHANGES_WIP
...
* @bsimethod                                    Barry.Bentley                   11/07
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::GridFix (DPoint3dR pointRoot, RotMatrixCR rMatrixRoot, DPoint3dCR originRoot, DPoint2dCR roundingDistanceRoot, bool isoGrid)
    {
    DVec3d planeNormal;
...
* @bsimethod                                    Barry.Bentley                   11/07
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::PointToStandardGrid (DPoint3dR point, DPoint3dR gridOrigin, RotMatrixR rMatrix)
    {
#ifdef DGNV10FORMAT_CHANGES_WIP
...
+---------------+---------------+---------------+---------------+---------------+------*/
UInt32 Viewport::_GetMenuColor (int index) const
    {
    // Menu colors defined as BGR values...
    static UInt32 s_menuColors[] = 
        {
        0x000000,   // black    = 0 
        0xFF0000,   // blue     = 1 
        0x00FF00,   // green    = 2 
    ...
    return s_menuColors[index];
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                    Keith.Bentley                   06/05
+---------------+---------------+---------------+---------------+---------------+------*/
RgbColorDef ViewController::ResolveBGColor() const
    {
    IntColorDef bgColor;
    ...
    if (GetViewFlags().overrideBackground)
        {
        bgColor.m_rgb = GetBackgroundColor();
        }
    else
        {
        bgColor = m_project.Colors().GetDgnColorMap()->GetColor(DgnColorMap::INDEX_Background);
        }

    // If background color resolved to be black, and user wants inverted, we set background color to white
    if (0 == bgColor.m_int && T_HOST.GetGraphicsAdmin()._WantInvertBlackBackground())
        bgColor.m_rgb.red = bgColor.m_rgb.green = bgColor.m_rgb.blue = 255;

    return bgColor.m_rgb;
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                    Keith.Bentley                   06/05
+---------------+---------------+---------------+---------------+---------------+------*/
bool Viewport::_GetWindowBgColor (RgbColorDef& bgColor) const
    {
    if (!m_viewController.IsValid ())
        return false;

    bgColor = m_viewController->ResolveBGColor();
...
+---------------+---------------+---------------+---------------+---------------+------*/
void Viewport::ScheduleProgressiveDisplay(IProgressiveDisplay& pd)
    {
