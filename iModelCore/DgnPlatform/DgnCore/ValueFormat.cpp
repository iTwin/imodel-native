/*--------------------------------------------------------------------------------------+
|
|     $Source: DgnCore/ValueFormat.cpp $
|
...
* @bsimethod                                                    JoshSchifter    12/07
+---------------+---------------+---------------+---------------+---------------+------*/
UInt16 AngleFormatter::GetLegacyFormat () const
    {
...
* @bsimethod                                                    JoshSchifter    12/07
+---------------+---------------+---------------+---------------+---------------+------*/
UInt16 AngleFormatter::GetLegacyPrecision () const
    {
    /* Used to call old format asyncs */

    return static_cast <UInt16> (m_precision);
    }
...
WString     AngleFormatter::ToStringFromRadians (double radians) const
    {
    double degrees = bsiTrig_radiansToDegrees (radians);
...
+---------------+---------------+---------------+---------------+---------------+------*/
void DirectionFormatter::InitFromLegacyTCBFormat (DirFormat const &dirFormat, UInt16 const &angleFmt, UInt16 const &precision)
    {
    SetDirectionMode ((DirectionMode) dirFormat.mode);
    SetClockwise (dirFormat.flags.clockwise);
    SetBaseDirection (dirFormat.baseDir);

    m_angleFormatter->SetAngleModeFromLegacy (static_cast<AngleFormatVals>(angleFmt));
    m_angleFormatter->SetAnglePrecisionFromLegacy (precision ? precision - 1 : 4);
    }
...
void DirectionFormatter::DirFormatFromLegacyAngleMode (DirFormat& dirFormat, int tentsubmode)
    ...
    dirFormat.mode             = static_cast<UInt16>(formatter.GetDirectionMode());
...
* @bsimethod                                                    JoshSchifter    01/08
+---------------+---------------+---------------+---------------+---------------+------*/
void DirectionFormatter::ToLegacyTCBFormat (DirFormat& dirFormat, UInt16& angleFmt, UInt16& precision) const
    {
    memset (&dirFormat, 0, sizeof(DirFormat));

    dirFormat.mode              = static_cast <UInt16> (GetDirectionMode ());
    dirFormat.flags.clockwise   = GetClockwise ();
    dirFormat.baseDir           = GetBaseDirection ();

    UInt16 numPlaces = m_angleFormatter->GetLegacyPrecision ();

    angleFmt                    = m_angleFormatter->GetLegacyFormat ();
    precision                   = (4 == numPlaces) ? 0 : numPlaces + 1;
    }
...
* @bsimethod                                                    Don.Fu          11/04
+---------------+---------------+---------------+---------------+---------------+------*/
void            DistanceFormatter::InitializeForDwg (ModelInfoCR modelInfo)
    {
...
* @bsimethod                                                    JoshSchifter    02/12
+---------------+---------------+---------------+---------------+---------------+------*/
/*ctor*/        DistanceFormatter::DistanceFormatter(DistanceFormatterCR source)
    {
...
* @bsimethod                                                    JoshSchifter    03/12
+---------------+---------------+---------------+---------------+---------------+------*/
DistanceFormatterPtr    DistanceFormatter::Create (ViewportR viewport)
    {
...
/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    Barry.Bentley   06/04
+---------------+---------------+---------------+---------------+---------------+------*/
static Int64   TruncateDoubleToInt64
(
double  double_num
)
    {
    if (double_num > IMAXI8)
        return IMAXI8;
    else if (double_num < IMINI8)
        return IMINI8;

    // we know it is in range.
    return ((Int64) double_num);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    John.Gooding    05/2005
+---------------+---------------+---------------+---------------+---------------+------*/
static void UorsToMasterSubPositional
(
Int64           *muP,                   /* <= integer master units */
Int64           *suP,                   /* <= integer sub units */
double          *puP,                   /* <= integer pos units */
double          *masterUnitsP,          /* <= fraction version */
double          *subUnitsP,             /* <= fraction version */
bool            *negFlagP,              /* <= true if negative */
double          inUors,                 /* => number to convert */
double          minimumResolution,      /* => minimum display resolution (uors) */
double          subPerMast,             //  Maybe just pass UnitInfo for both sub and master.
double          posPerSub
)
    {
    double  posUnits;

    /* always work in positive uors */
    if (inUors < 0)
        {
        *negFlagP = true;
        inUors    = -inUors;
    ...
    inUors += minimumResolution * ROUNDOFF;

    posUnits      = inUors;

    *masterUnitsP = inUors / (subPerMast * posPerSub);
    *muP          = TruncateDoubleToInt64 (*masterUnitsP);
    posUnits     -= (*muP * (subPerMast * posPerSub));

    *subUnitsP    =  posUnits / posPerSub;
    *suP          = TruncateDoubleToInt64 (*subUnitsP);
    posUnits     -= (*suP * posPerSub);
...
static double getMinimumResolutionForUnitInfo (DgnUnitFormat unitFormat, byte precisionByte, PrecisionType precisionType, UnitDefinitionCR masterUnits, UnitDefinitionCR subUnits)
    ...
    switch (unitFormat)
        ...
        case DgnUnitFormat::SU:        /* Sub Units */
            {
            minimumResolution *= subUnits.ToMillimeters();
            break;
            }

        case DgnUnitFormat::MU:        /* Master Units */
            {
            minimumResolution *= masterUnits.ToMillimeters();
            break;
            }

        case DgnUnitFormat::MUSUPU:    /* Working Units */
            /* just use minimum resolution as already computed */
            break;
        }
...
/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                   Joe.Zbuchalski    03/2010
+---------------+---------------+---------------+---------------+---------------+------*/
void    DistanceFormatter::GetUorsToMasterSubPositional
(
double          inUors,                 /* => number to convert */
Int64           *muP,                   /* <= integer master units */
Int64           *suP,                   /* <= integer sub units */
...
bool            *negFlagP               /* <= true if negative */
    ...
    return UorsToMasterSubPositional (muP, suP, puP, masterUnitsP, subUnitsP, negFlagP, 
                    inUors, minimumResolution, GetSubPerMaster(), GetMillimetersPerSub());
    }
...
* @bsimethod                                                    JoeZbuchalski   12/09
+---------------+---------------+---------------+---------------+---------------+------*/
double      DistanceFormatter::GetMillimetersPerSub () const
    {
    return m_subUnit.ToMillimeters();
    }
...
* @bsimethod                                                    Josh.Schifter   01/12
+---------------+---------------+---------------+---------------+---------------+------*/
WString DistanceFormatter::ToStringForDWG (double uors) const
    {
    PrecisionType   dwgPrecisionType = m_precisionType;
    byte            dwgPrecisionByte = m_precisionByte;

    if (PrecisionType::Fractional == dwgPrecisionType)
        {
        if (0 == dwgPrecisionByte)
            dwgPrecisionType = PrecisionType::Decimal;
        else
            dwgPrecisionByte = 1 << dwgPrecisionByte;
        }

    double          masterUnits, subUnits, positionalUnits;
    bool            isNegative;
    Int64           iMasterUnits, iSubUnits;
    double          subPerMaster;
    double          posPerSub;
    bool            bEngineeringOrArchitectural = DwgUnitFormat::Engineering == m_dwgUnitFormat || DwgUnitFormat::Architectural == m_dwgUnitFormat;

    double localSub (m_storageUnit);
    double  localMaster =(m_storageUnit);
    DgnUnitFormat   dgnUnitformat = DgnUnitFormat::MU;

    // In this case, AutoCad logic makes the assumption that master units are inches and the user really wants feet-inches.
    // Since the AutoCad unit concept is loose, they apply this even if the master units are NOT inches.  So the resulting
    // distance is formatted as 12MU-MU (where 12MU is a unit that is 12 times larger than MU) instead of the usual MU-SU.
    if (bEngineeringOrArchitectural)
        {
        dgnUnitformat = DgnUnitFormat::MUSU;
        localMaster.Scale (1.0/12.0);
        }

    //  First calculate subPerMaster; then calculate posPerSub
    localMaster.ConvertDistanceFrom (subPerMaster, 1.0, localSub);

    // Since localSub is m_storageUnit, posPerSub is m_uorPerStorage
    posPerSub = 1.0;

    if (m_scaleFactor != 0 && m_scaleFactor != 1)
         uors *= m_scaleFactor;

    double minimumResolution = getMinimumResolutionForUnitInfo (dgnUnitformat, dwgPrecisionByte, dwgPrecisionType, localMaster, localSub);

    UorsToMasterSubPositional (&iMasterUnits, &iSubUnits, &positionalUnits, &masterUnits, &subUnits, &isNegative, uors, minimumResolution, subPerMaster, posPerSub);

    WString outputString;

    if (bEngineeringOrArchitectural)
        {
        WString masterUnitString;
        WString subUnitString;
        WString masterUnitsLabel (L"\'");
        WString subUnitsLabel (L"\"");
        WString dash (L"-");
        ...
        if (m_suppressZeroSubUnits && 0 == subUnits)
            {
            subUnitString.clear();
            subUnitsLabel.clear();
            dash.clear();
            ...
            {
            GetPrecisionString (subUnitString, subUnits, dwgPrecisionType, dwgPrecisionByte, m_leadingZero, m_trailingZeros);
            ReplaceDecimalSeparator (subUnitString, m_decimalSeparator);
            }

        outputString.Sprintf (L"%ls%ls%ls%ls%ls", masterUnitString.c_str(), masterUnitsLabel.c_str(), dash.c_str(), subUnitString.c_str(), subUnitsLabel.c_str());
        }
    else
        {
        if (PrecisionType::Scientific == m_precisionType || (masterUnits > RMAXUI4) || (masterUnits < RMINI4))
            {
            byte precision = dwgPrecisionByte;

            if (PrecisionType::Fractional == dwgPrecisionType)
                precision = DoubleFormatter::GetByteFromPrecision (PrecisionFormat::Scientific4Places);

            GetScientificString (outputString, masterUnits, precision);
            ReplaceDecimalSeparator (outputString, m_decimalSeparator);
            }
        else
            {
            GetPrecisionString (outputString, masterUnits, dwgPrecisionType, dwgPrecisionByte, m_leadingZero, m_trailingZeros);
            ReplaceDecimalSeparator (outputString, m_decimalSeparator);
            InsertThousandsSeparator (outputString, m_thousandsSeparator);
            }
        }

    if (isNegative)
        outputString.insert (0, L"-");
...
* @bsimethod                                                    JoeZbuchalski   12/09
+---------------+---------------+---------------+---------------+---------------+------*/
WString         DistanceFormatter::ToString (double uors) const
    {
#if defined (NEEDS_WORK_DGNITEM)
    if (m_useDWGFormattingLogic)
        return ToStringForDWG (uors);
#endif
    ...
    posPerSub = GetMillimetersPerSub();

    if (m_scaleFactor != 0 && m_scaleFactor != 1)
         uors *= m_scaleFactor;

    double minimumResolution = GetMinimumResolution ();
    UorsToMasterSubPositional (&iMasterUnits, &iSubUnits, &positionalUnits, &masterUnits, &subUnits, 
                    &isNegative, uors, minimumResolution, subPerMaster, posPerSub);
    ...
    switch (m_unitFormat)
        ...
        case DgnUnitFormat::MUSUPU:        /* Working Units */
            {
            GetPrecisionString (posUnitString, positionalUnits, m_precisionType, m_precisionByte, m_leadingZero, m_trailingZeros);
            ReplaceDecimalSeparator (posUnitString, m_decimalSeparator);
            if (unitFlag || !m_isDgnCoordReadOutCapable)
                outString.Sprintf (L"%lld%ls%lc%lld:%ls%ls",iMasterUnits, masterUnitsLabel.c_str(), connector, iSubUnits, posUnitString.c_str(), subUnitsLabel.c_str());
            else
                outString.Sprintf (L"%lld:%lld:%ls", iMasterUnits, iSubUnits, posUnitString.c_str());
            break;
            }
        }
...
* @bsimethod                                                    JoshSchifter    02/12
+---------------+---------------+---------------+---------------+---------------+------*/
void            DistanceFormatter::ToLegacyTCBFormat (Autodim1& ad1) const
    {
    // Pre-Vacouver, the highest fractional precision was 1/64
    static const byte MAX_LEGACY_PRECISION_Fractional = (1<<6) - 1;

    // Pre-Vacouver, the highest decimal precision was 0.123456
    static const byte MAX_LEGACY_PRECISION_Decimal = 6;

    switch (GetUnitFormat())
        {
        default:
        case DgnUnitFormat::MU:        ad1.format.ref_mastersub = 1;   break;
        case DgnUnitFormat::MUSU:      ad1.format.ref_mastersub = 0;   break;
        case DgnUnitFormat::MUSUPU:    ad1.format.ref_mastersub = 2;   break;
        }

    byte                    precisionByte = DoubleFormatter::GetByteFromPrecision (GetPrecision());
    PrecisionType           precisionType = DoubleFormatter::GetTypeFromPrecision (GetPrecision());
    LegacyDecFracAccuracy   decOrFrac;

    memset (&decOrFrac, 0, sizeof decOrFrac);

    if (PrecisionType::Fractional == precisionType)
        {
        ad1.format.ref_decfract = 1;

        if (MAX_LEGACY_PRECISION_Fractional < precisionByte)
            precisionByte = MAX_LEGACY_PRECISION_Fractional;

        int denominator = precisionByte + 1;

        /* convert it to the sequential precision number, i.e. 1/1 = 0, 1/2 = 1, 1/4 = 2, 1/8 = 3 ... 1/256 = 8 */
        int iFract;
        for (iFract = 1; denominator > 2; denominator /= 2, iFract++);

        decOrFrac.adres2.u.fract = (1 << (iFract-1)) - 1;

        ad1.format.accuracyFlags = precisionByte > 0 ? LEGACY_ACCURACYFLAGS_Default : LEGACY_ACCURACYFLAGS_FractionalZero;
        }
    else if (PrecisionType::Scientific == precisionType)
        {
        ad1.format.ref_decfract = 0;
        ad1.format.adres2 = precisionByte;

        ad1.format.accuracyFlags = LEGACY_ACCURACYFLAGS_Scientific;
        ...
        if (MAX_LEGACY_PRECISION_Decimal < precisionByte)
            precisionByte = MAX_LEGACY_PRECISION_Decimal;

        if (4 == precisionByte)
            decOrFrac.adres2.u.dec = 0;
        else
            decOrFrac.adres2.u.dec = precisionByte + 1;

        ad1.format.accuracyFlags = LEGACY_ACCURACYFLAGS_Default;
        }

    ad1.format.adres2 = decOrFrac.adres2.b;
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    JoshSchifter    03/12
+---------------+---------------+---------------+---------------+---------------+------*/
void            DistanceFormatter::ReadLegacyTCBFormat (DgnUnitFormat& unitFormat, PrecisionFormat& precisionFormat, Autodim1 const& ad1)
    {
    /* just be sure unit format to be correct */
    switch (ad1.format.ref_mastersub)
        {
        default:
        case 1:     unitFormat = DgnUnitFormat::MU;        break;
        case 0:     unitFormat = DgnUnitFormat::MUSU;      break;
        case 2:     unitFormat = DgnUnitFormat::MUSUPU;    break;
        }
    ...
    /* get accuracy mode */
    if (LEGACY_ACCURACYFLAGS_Scientific == ad1.format.accuracyFlags)
        precisionType = PrecisionType::Scientific;
    else if (ad1.format.ref_decfract)
        precisionType = PrecisionType::Fractional;
    else
        precisionType = PrecisionType::Decimal;

    /* get accuracy based on accuracy mode */
    LegacyDecFracAccuracy     decOrFrac;

    memset (&decOrFrac, 0, sizeof decOrFrac);

    decOrFrac.adres2.b = (UChar)(ad1.format.adres2);

    if (PrecisionType::Fractional == precisionType)
        {
        /* fractional accuracy */
        if (LEGACY_ACCURACYFLAGS_FractionalZero == ad1.format.accuracyFlags)
            precisionByte = 0;
        else
            precisionByte = static_cast <byte> ((decOrFrac.adres2.u.fract << 1) + 1);
        }
    else if (LEGACY_ACCURACYFLAGS_Scientific == ad1.format.accuracyFlags)
        {
        precisionByte = ad1.format.adres2;
        }
    else
        {
        if (0 == decOrFrac.adres2.u.dec)
            precisionByte = 4;
        else
            precisionByte = static_cast <byte> (decOrFrac.adres2.u.dec - 1);
        }

    precisionFormat = DoubleFormatter::ToPrecisionEnum (precisionType, precisionByte);
    }
...
* @bsimethod                                                    JoshSchifter    03/12
+---------------+---------------+---------------+---------------+---------------+------*/
PointFormatterPtr   PointFormatter::Create (ViewportR viewport)
    {
    DgnModelP       targetModel = viewport.GetViewController ().GetTargetModel();
#ifdef WIP_V10_MODEL_ACS
...
* @bsimethod                                                    JoshSchifter    03/12
+---------------+---------------+---------------+---------------+---------------+------*/
/* ctor */      AreaOrVolumeFormatterBase::AreaOrVolumeFormatterBase ()   { Init(); }
...
* @bsimethod                                                    Don.Fu          11/04
+---------------+---------------+---------------+---------------+---------------+------*/
void            AreaOrVolumeFormatterBase::InitializeForDwg (ModelInfoCR modelInfo)
    {
    DgnUnitFormat       dgnUnitFormat   = modelInfo.GetLinearUnitMode();
    PrecisionFormat     precision       = modelInfo.GetLinearPrecision();
    UnitDefinitionCR    masterUnit      = modelInfo.GetMasterUnit();
...
* @bsimethod                                                    JoshSchifter    03/12
+---------------+---------------+---------------+---------------+---------------+------*/
AreaFormatterPtr    AreaFormatter::Create (ViewportR viewport)
    {
    DgnModelP    targetModel = viewport.GetViewController ().GetTargetModel();
...
* @bsimethod                                                    JoshSchifter    03/12
+---------------+---------------+---------------+---------------+---------------+------*/
WString         AreaFormatter::ToStringForDWG (double uors) const
    {
    bool            bEngineeringOrArchitectural = DwgUnitFormat::Engineering == m_dwgUnitFormat || DwgUnitFormat::Architectural == m_dwgUnitFormat;

    UnitDefinition  localMaster (m_storageUnit);
    DgnUnitFormat   dgnUnitformat = DgnUnitFormat::MU;

    // In this case, AutoCad logic makes the assumption that master units are inches and the user really wants feet.
    // Since the AutoCad unit concept is loose, they apply this even if the master units are NOT inches.  So the resulting
    // distance is formatted as 12MU (where 12MU is a unit that is 12 times larger than MU) instead of the usual MU.
    if (bEngineeringOrArchitectural)
        {
        dgnUnitformat = DgnUnitFormat::MUSU;
        localMaster.Scale (1.0/12.0);
    ...
    uors /= m_scaleFactor * m_scaleFactor;

    UnitDefinition  sqMasterUnit  = localMaster;
    UnitDefinition  sqStorageUnit = m_storageUnit;

    sqMasterUnit.Square();
    sqStorageUnit.Square();

    double sqStoragePerMaster;
    sqStorageUnit.GetConversionFactorFrom (sqStoragePerMaster, sqMasterUnit);

    double  masterUnits = uors / sqStoragePerMaster;

    WString outString = T_Super::ToStringBasic(masterUnits);

    if (m_showUnitLabel)
        appendUnitLabel (outString, sqMasterUnit.GetLabelCP(), m_labelDecoratorAsSuffix ? DgnCoreL10N::UNIT_LABEL_SUFFIX_Area : DgnCoreL10N::UNIT_LABEL_PREFIX_Area, m_labelDecoratorAsSuffix);
...
* @bsimethod                                                    JoshSchifter    03/12
+---------------+---------------+---------------+---------------+---------------+------*/
WString         AreaFormatter::ToString (double millimeters) const
    {
#if defined (NEEDS_WORK_DGNITEM)
    if (m_useDWGFormattingLogic)
        return ToStringForDWG (uors);
    ...
    millimeters /= m_scaleFactor * m_scaleFactor;

    double mastMillis = m_masterUnit.ToMillimeters();
    double area = millimeters / (mastMillis*mastMillis);
...
* @bsimethod                                                    JoshSchifter    03/12
+---------------+---------------+---------------+---------------+---------------+------*/
VolumeFormatterPtr    VolumeFormatter::Create (ViewportR viewport)
    {
    DgnModelP    targetModel = viewport.GetViewController ().GetTargetModel();
...
* @bsimethod                                                    JoshSchifter    03/12
+---------------+---------------+---------------+---------------+---------------+------*/
WString         VolumeFormatter::ToString (double uors) const
    {
    uors /= m_scaleFactor * m_scaleFactor * m_scaleFactor;

    double mastMillis = m_masterUnit.ToMillimeters();
    double cube= uors / (mastMillis*mastMillis*mastMillis);
...
UInt8   DateTimeFormatter::GetFractionalSecondPrecision() const         { return m_fractionalPrecision; }
...
void DateTimeFormatter::SetFractionalSecondPrecision (UInt8 prec)   { m_fractionalPrecision = prec; }
...
static WString getDayName (int d, bool useShort)
    {
    BeAssert (7 > d);
    UInt32 base = useShort ? DgnCoreL10N::DATETIME_DAY_SHORT_0 : DgnCoreL10N::DATETIME_DAY_0;
...
static WString getMonthName (int m, bool useShort)
    {
    BeAssert (12 > m);
    UInt32 base = useShort ? DgnCoreL10N::DATETIME_MONTH_SHORT_0 : DgnCoreL10N::DATETIME_MONTH_0;
...
static WString getAmPmName (bool pm, bool useShort)
    {
    UInt32 base = useShort ? DgnCoreL10N::DATETIME_AM_SHORT : DgnCoreL10N::DATETIME_AM;
...
static void formatUtc(WStringR str, DateTimeCR local, DateTimeFormatterCR formatter, DateTimeFormatPart part)
    ...
    UInt64 offsetHnsUnsigned = (isOffsetNegative ? (UInt64)-offsetHns : (UInt64)offsetHns);
    ...
    const UInt64 HECTONANOSECS_IN_DAY = (86400ULL * 10000000ULL); // seconds in a day * HNS in a second
...
WString DateTimeFormatter::ToString (DateTimeCR dtIn) const
    ...
    for (DateTimeFormatPart part: partList)
        ...
        case DATETIME_PART_FractionalSeconds:
            ...
            UInt64 ms = dt.GetMillisecond();
            double dms = static_cast<double>(ms);
            tmp.clear();
            switch (m_fractionalPrecision)
                {
            case 0:     break;
            case 1:     tmp.Sprintf (L"%d", (Int32)(dms / 100.0 + 0.5)); break;
            case 2:     tmp.Sprintf (L"%02d", (Int32)(dms / 10.0 + 0.5)); break;
