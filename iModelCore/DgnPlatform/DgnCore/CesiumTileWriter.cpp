...
using WriteStatus = Dgn::Tile::IO::WriteStatus;
using Gltf = Dgn::Tile::IO::Gltf;
using B3dm = Dgn::Tile::IO::B3dm;
using PointCloud = Dgn::Tile::IO::PointCloud;
using Format = Dgn::Tile::IO::Format;
using FutureWriteStatus = folly::Future<WriteStatus>;
using TaskQueue = BeFolly::LimitingTaskQueue<BentleyStatus>;
using TaskQueueSPtr = std::shared_ptr<TaskQueue>;
...
/*---------------------------------------------------------------------------------**//**
* Convert json to string and pad to 4-byte boundary.
...
public:
    BatchTableBuilder(FeatureTableCR, DgnDbR, bool is3d);
...
{ 
    ...
    MeshMaterial (Utf8StringCR suffix, MaterialCP material, TextureCP texture) : m_name("Material_" + suffix), m_material(material), m_texture(texture) { }

    bool IsTextured() const { return m_texture.IsValid(); }
    bool IgnoresLighting() const { return false; }
    Utf8StringCR GetName() const { return m_name; }
    TextureCPtr GetTexture() const { return m_texture; }
};
...
struct CesiumTileWriter : Dgn::Tile::IO::Writer
{
    ...
    CesiumTileWriter(StreamBufferR streamBuffer, GeometricModelR model) : Dgn::Tile::IO::Writer(streamBuffer, model) { }
};

//=======================================================================================
// @bsistruct                                                   Ray.Bentley 04/2017
//=======================================================================================
struct CesiumTileOutput : Cesium::Output
{
private:
    StreamBuffer m_streamBuffer;
    CesiumTileWriter m_writer;
    Json::Value m_primitives;
    DRange3d m_range;
    FeatureTable m_featureTable;
    bvector<PointCloudData> m_pointClouds;

    CesiumTileOutput(GeometricModelR model) : m_writer(m_streamBuffer, model), m_range(DRange3d::NullRange()), m_featureTable(model.GetModelId(), 0xffff) { }

    WriteStatus WriteMeshTile(Cesium::PublishedTileR);
    WriteStatus WritePointCloudTile(Cesium::PublishedTileR);
public:
    static RefCountedPtr<CesiumTileOutput> Create(GeometricModelR model) { return new CesiumTileOutput(model); }

    void _AddBatch(DRange3dCR range, Render::FeatureTable&& features) override
        {
        // m_featureTable = std::move(features);
        m_range = range;
        }

    void _AddTriMesh(TriMeshArgsCR args) override;
    void _AddPointCloud(PointCloudArgsCR args) override
        {
        m_range.Extend(args.m_qParams.GetRange());
        m_pointClouds.push_back(args);
        }

    WriteStatus WriteTile(Cesium::PublishedTileR);
};
...
struct Context
{
    Cesium::TilePtr             m_inputTile;
    Cesium::PublishedTilePtr    m_outputTile;
    Cesium::ICesiumPublisher*   m_publisher;
    double                      m_leafTolerance;
    GeometricModelP             m_model;
    ClipVectorPtr               m_clip;
    TaskQueueSPtr               m_requestTileQueue;

    Context(Cesium::TileP inputTile, Cesium::PublishedTileP outputTile, ClipVectorCP clip, Cesium::ICesiumPublisher* publisher, double leafTolerance, GeometricModelP model, TaskQueueSPtr requestTileQueue) 
        : m_inputTile(inputTile), m_outputTile(outputTile), m_publisher(publisher), m_leafTolerance(leafTolerance), m_model(model), m_requestTileQueue(requestTileQueue)
        {
        if (nullptr != clip)
            m_clip = clip->Clone(nullptr);
        }

    Context(Cesium::TileP inputTile, Cesium::PublishedTileP outputTile, Context const& inContext) : m_inputTile(inputTile), m_outputTile(outputTile),
        m_clip(inContext.m_clip), m_publisher(inContext.m_publisher), m_leafTolerance(inContext.m_leafTolerance), m_model(inContext.m_model), m_requestTileQueue(inContext.m_requestTileQueue) { }
};

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                    Mathieu.Marchand                10/2017
+---------------+---------------+---------------+---------------+---------------+------*/
static folly::Future<BentleyStatus> requestTile(Context context, Cesium::OutputR output)
    {
    if (context.m_inputTile->IsNotLoaded())
        ...
        return context.m_requestTileQueue->Push([=]()
            {
            return context.m_inputTile->GetRoot()._RequestTile(*context.m_inputTile, *loadState, *pOutput);
            });
        }
...
static FutureWriteStatus generateParentTile(Context context)
    {
    auto output = CesiumTileOutput::Create(*context.m_model);
    return requestTile(context, *output).then([=](BentleyStatus status)
        {
        if (SUCCESS != status)
            return folly::makeFuture(WriteStatus::UnableToLoadTile);
        
        WriteStatus writeStatus = output->WriteTile(*context.m_outputTile);
...
static FutureWriteStatus generateChildTiles (WriteStatus parentStatus, Context context)
    ...
    auto children = context.m_inputTile->_GetChildren(true);
    if (nullptr == children)
    ...
    for (auto& child : *children)
        {
        Cesium::PublishedTilePtr publishedTile = new Cesium::PublishedTile(*child, context.m_publisher->_GetOutputDirectory(*context.m_model));
...
FutureWriteStatus writeCesiumTileset(Cesium::ICesiumPublisher* publisher, GeometricModelP model, double leafTolerance)
    ...
    Cesium::PublishedTilePtr publishedRoot = new Cesium::PublishedTile(*tileRoot->GetRootTile(), publisher->_GetOutputDirectory(*model));
    ...
    .then([=](WriteStatus status)
        ...
        auto inputTile = tileRoot->GetRootTile();
        auto requestTileQueue = std::make_shared<TaskQueue>(BeFolly::ThreadPool::GetIoPool(), 20);
                                                                                                                
        Context context(inputTile.get(), publishedRoot.get(), nullptr, publisher, leafTolerance, model, requestTileQueue);
...
protected:
    ...
    void WriteModelMetadataTree (DRange3dR range, Json::Value& root, Cesium::PublishedTileCR tile, GeometricModelCR model);
    WriteStatus WriteTileset (GeometricModelCR model, TransformCR tileToDb, Cesium::PublishedTileCR rootTile);
    ...
    WriteStatus _EndProcessModel(GeometricModelCR model, TransformCR tileToDb, Cesium::PublishedTileCR rootTile, WriteStatus status) override
        {
        if (WriteStatus::Success != status)
            {
...
* @bsimethod                                                    Paul.Connelly   09/18
+---------------+---------------+---------------+---------------+---------------+------*/
static bool doesTextureRepeat(TextureCR texture)
    {
    return false; // Add support if at some point we find a use case...
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    Ray.Bentley     11/2016
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus writeJsonToFile(WCharCP fileName, Json::Value const& value)
    {
    BeFile outputFile;
...
+---------------+---------------+---------------+---------------+---------------+------*/
WString getRootNameForModel(DgnModelId modelId, bool asClassifier)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
DgnCategoryId BatchTableBuilder::QueryCategoryId(DgnSubCategoryId subCategoryId)
    {
...
* @bsimethod                                                    Ray.Bentley     09/2017
+---------------+---------------+---------------+---------------+---------------+------*/
DgnElementId BatchTableBuilder::QueryAssemblyId(DgnElementId childId) 
    {
...
* @bsimethod                                                    Ray.Bentley     08/2017
+---------------+---------------+---------------+---------------+---------------+------*/
BatchTableBuilder::BatchTableBuilder(FeatureTableCR attrs, DgnDbR db, bool is3d) : m_json(Json::objectValue), m_db(db), m_is3d(is3d)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
void CesiumTileWriter::AddShader(Json::Value& shaders, Utf8CP name, int32_t type, Utf8CP buffer)
    {
...
* @bsimethod                                                    Paul.Connelly   08/16
+---------------+---------------+---------------+---------------+---------------+------*/
void CesiumTileWriter::AddTechniqueParameter(Json::Value& technique, Utf8CP name, Gltf::DataType type, Utf8CP semantic)
    {
...
* @bsimethod                                                    Paul.Connelly   08/16
+---------------+---------------+---------------+---------------+---------------+------*/
void CesiumTileWriter::AppendProgramAttribute(Json::Value& program, Utf8CP attrName)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
Utf8String CesiumTileWriter::AddMeshShaderTechnique(Utf8StringCR idStr, Utf8StringCR vertexShaderString, Utf8StringCR fragmentShaderString, bool textured)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
void CesiumTileWriter::AddTextureSampler(Utf8StringCR sampler, TextureCR texture)
    {
...
Utf8String CesiumTileWriter::AddTextureImage (TextureCR texture, Utf8StringCR idStr)
    ...
    Utf8String  samplerName = doesTextureRepeat(texture) ? "sampler_0" : "sampler_2";
...
* @bsimethod                                                    Ray.Bentley     07/2018
+---------------+---------------+---------------+---------------+---------------+------*/
Json::Value CesiumTileWriter::CreateMaterialJson(MeshMaterial const& material, Utf8StringCR idStr)
    { 
...
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus CesiumTileWriter::CreateTriMesh(Json::Value& primitiveJson, TriMeshArgs const& meshArgs, MeshMaterial const& meshMaterial, Utf8StringCR idStr)
    {
...
* @bsimethod                                                    Ray.Bentley     06/2017
+---------------+---------------+---------------+---------------+---------------+------*/
void CesiumTileWriter::AddTriMesh(Json::Value& primitivesNode, TriMeshArgsCR meshArgs, ColorTableCR colorTable, MeshMaterial const& meshMaterial, size_t& index)
    {
...
* @bsimethod                                                    Ray.Bentley     06/2017
+---------------+---------------+---------------+---------------+---------------+------*/
void CesiumTileWriter::BeginBatchedModel(uint32_t& startPosition, uint32_t& lengthDataPosition, Render::FeatureTableCR featureTable)
    {
...
* @bsimethod                                                    Ray.Bentley     06/2017
+---------------+---------------+---------------+---------------+---------------+------*/
void CesiumTileWriter::EndBatchedModel(uint32_t startPosition, uint32_t lengthDataPosition)
    {
    PadToBoundary ();
...
* @bsimethod                                                    Ray.Bentley     08/2017
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus CesiumTileWriter::WriteBatchedModel(Render::FeatureTableCR featureTable)
    {
...
* @bsimethod                                                    Ray.Bentley     08/2017
+---------------+---------------+---------------+---------------+---------------+------*/
void CesiumTileOutput::_AddTriMesh(TriMeshArgsCR args)
    {
...
* @bsimethod                                                    Ray.Bentley     08/2017
+---------------+---------------+---------------+---------------+---------------+------*/
WriteStatus CesiumTileOutput::WriteTile(Cesium::PublishedTileR outputTile)
    {
    if (m_range.IsNull())
...
* @bsimethod                                                    Ray.Bentley     08/2017
+---------------+---------------+---------------+---------------+---------------+------*/
WriteStatus CesiumTileOutput::WriteMeshTile(Cesium::PublishedTileR outputTile)
    {
...
* @bsimethod                                                    Ray.Bentley     08/2017
+---------------+---------------+---------------+---------------+---------------+------*/
WriteStatus CesiumTileOutput::WritePointCloudTile(Cesium::PublishedTileR outputTile)
    {
...
+---------------+---------------+---------------+---------------+---------------+------*/
void CesiumTilesetPublisher::WriteBoundingVolume(Json::Value& val, DRange3dCR range)
    {
...
void CesiumTilesetPublisher::WriteModelMetadataTree(DRange3dR range, Json::Value& root, Cesium::PublishedTileCR tile, GeometricModelCR model)
    ...
    WString         rootName = getRootNameForModel(model.GetModelId(), false);
...
+---------------+---------------+---------------+---------------+---------------+------*/
Json::Value CesiumTilesetPublisher::TransformToJson(TransformCR tf)
    {
...
* @bsimethod                                                    Ray.Bentley     07/2018
+---------------+---------------+---------------+---------------+---------------+------*/
WriteStatus CesiumTilesetPublisher::WriteTileset(GeometricModelCR model, TransformCR tileToDb, Cesium::PublishedTileCR rootTile)
    {
    ...
    return SUCCESS == writeJsonToFile (m_outputFileName.c_str(), val) ? WriteStatus::Success : WriteStatus::UnableToWriteFile;
    }
...
* @bsimethod                                                    Ray.Bentley     08/2017 
+---------------+---------------+---------------+---------------+---------------+------*/
PublishedTile::PublishedTile(TileCR inputTile, BeFileNameCR outputDirectory) : m_publishedRange(DRange3d::NullRange()), m_outputDirectory(outputDirectory)
    {
    m_name = inputTile._GetName();
    m_name.ReplaceAll(":", "_");
    m_name.ReplaceAll(".", "_");
    m_name.ReplaceAll("/", "_");
...
WriteStatus ICesiumPublisher::PublishCesiumTileset(ICesiumPublisher& publisher, GeometricModelCR model, TransformCR transformFromDgn, double leafTolerance)
    {
    return writeCesiumTileset(&publisher, const_cast<GeometricModelP>(&model), leafTolerance).get();
    }
