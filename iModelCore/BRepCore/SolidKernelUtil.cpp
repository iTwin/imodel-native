...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
PolyfaceHeaderPtr BRepUtil::FacetEntity(IBRepEntityCR entity, IFacetOptionsCR facetOptions)
    {
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
bool BRepUtil::FacetEntity(IBRepEntityCR entity, bvector<PolyfaceHeaderPtr>& polyfaces, bvector<FaceAttachment>& params, IFacetOptionsCR facetOptions)
    {
...
size_t BRepUtil::GetBodyFaces(bvector<ISubEntityPtr>* subEntities, IBRepEntityCR in)
    ...
    if (subEntities)
        {
        _Analysis_assume_(faceTags != nullptr);
...
size_t BRepUtil::GetBodyEdges(bvector<ISubEntityPtr>* subEntities, IBRepEntityCR in)
    ...
    if (subEntities)
        {
        _Analysis_assume_(edgeTags != nullptr);
...
size_t BRepUtil::GetBodyVertices(bvector<ISubEntityPtr>* subEntities, IBRepEntityCR in)
    ...
    if (subEntities)
        {
        _Analysis_assume_(vertexTags != nullptr);
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
uint32_t BRepUtil::GetSubEntityId(ISubEntityCR subEntity)
    {
    return PSolidSubEntity::GetSubEntityTag(subEntity);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ISubEntityPtr BRepUtil::CreateSubEntityFromId(uint32_t id, TransformCR entityTransform)
    {
    PK_LOGICAL_t isEntity = PK_LOGICAL_false;
    if (PK_ERROR_no_errors != PK_ENTITY_is(id, &isEntity) || !isEntity)
        return nullptr;

    return PSolidSubEntity::CreateSubEntity(id, entityTransform);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ISubEntityPtr BRepUtil::CreateSubEntityFromId(uint32_t id, IBRepEntityCR parent)
    {
    PK_LOGICAL_t isEntity = PK_LOGICAL_false;
    if (PK_ERROR_no_errors != PK_ENTITY_is(id, &isEntity) || !isEntity)
        return nullptr;

    return PSolidSubEntity::CreateSubEntity(id, parent);
    }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus BRepUtil::GetPlanarFaceData(ISubEntityCR subEntity, DPoint3dR point, DVec3dR normal)
    {
    PK_FACE_t entityTag = PSolidSubEntity::GetSubEntityTag(subEntity);

    if (PK_ENTITY_null == entityTag || SUCCESS != PSolidUtil::GetPlanarFaceData(&point, &normal, entityTag))
        return ERROR;
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
bool BRepUtil::IsRedundantEdge(ISubEntityCR subEntity)
    {
    // NOTE: Smooth test excludes laminar edges...
    if (!BRepUtil::IsSmoothEdge(subEntity))
        return false;

    bvector<ISubEntityPtr> faces;
    if (SUCCESS != BRepUtil::GetEdgeFaces(faces, subEntity) || 2 != faces.size())
        return false;

    PK_SURF_t surfTag0 = PK_ENTITY_null;
    if (SUCCESS != PK_FACE_ask_surf(PSolidSubEntity::GetSubEntityTag(*faces[0]), &surfTag0))
        return false;

    PK_SURF_t surfTag1 = PK_ENTITY_null;
    if (SUCCESS != PK_FACE_ask_surf(PSolidSubEntity::GetSubEntityTag(*faces[1]), &surfTag1))
        return false;

    return (surfTag0 == surfTag1);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
bool BRepUtil::IsSmoothVertex(ISubEntityCR subEntity)
    {
    bvector<ISubEntityPtr> subEntities;

    if (SUCCESS != BRepUtil::GetVertexEdges(subEntities, subEntity))
        return false;

    for (ISubEntityPtr& edge : subEntities)
        {
        if (BRepUtil::IsSmoothEdge(*edge))
            return true;
        }
...
bool BRepUtil::Locate(IBRepEntityCR entity, DRay3dCR boresite, bvector<ISubEntityPtr>& intersectEntities, size_t maxFace, size_t maxEdge, size_t maxVertex, double maxEdgeDistance, double maxVertexDistance)
    ...
    for (size_t iHit = 0; iHit < tmpIntersectEntityTags.size(); ++iHit)
        ...
        switch (subEntity->GetSubEntityType())
            {
            case ISubEntity::SubEntityType::Face:
                subEntity->SetFaceLocation(tmpIntersectPts.at(iHit), tmpIntersectParams.at(iHit));
                break;

            case ISubEntity::SubEntityType::Edge:
                subEntity->SetEdgeLocation(tmpIntersectPts.at(iHit), tmpIntersectParams.at(iHit).x);
                break;
            }
...
ISubEntityPtr BRepUtil::ClosestSubEntity(IBRepEntityCR entity, DPoint3dCR testPt)
    ...
    if (subEntity.IsValid())
        ...
        switch (subEntity->GetSubEntityType())
            {
            case ISubEntity::SubEntityType::Face:
                subEntity->SetFaceLocation(point, param);
                break;

            case ISubEntity::SubEntityType::Edge:
                subEntity->SetEdgeLocation(point, param.x);
                break;
            }
        }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
ISubEntityPtr BRepUtil::ClosestFace(IBRepEntityCR entity, DPoint3dCR testPtIn, DVec3dCP preferredDir)
    {
    if (IBRepEntity::EntityType::Wire == entity.GetEntityType())
        return nullptr;

    ISubEntityPtr closeEntity = BRepUtil::ClosestSubEntity(entity, testPtIn);
    ...
    if (haveInvalidAttachment)
        ...
        if (SUCCESS == PSolidTopo::GetBodyFaces(allFaces, PSolidUtil::GetEntityTag(target)))
            ...
            for (PK_FACE_t faceTag : allFaces)
                {
                int32_t faceAttachmentIndex = 0;

                if (SUCCESS == PSolidAttrib::GetFaceMaterialIndexAttribute(faceAttachmentIndex, faceTag) && (faceAttachmentIndex <= 0 || faceAttachmentIndex >= faceAttachmentsVec.size()))
...
// @bsimethod
//---------------------------------------------------------------------------------------
FaceAttachment BRepUtil::GetFaceMaterialAttachmentForSubEntity(ISubEntityCR subEntity, IFaceMaterialAttachmentsCR attachments)
    {
    T_FaceAttachmentsVec const& faceAttachmentsVec = attachments._GetFaceAttachmentsVec();
    int32_t attachmentIndex = 0;

    PK_ENTITY_t faceTag = PK_ENTITY_null;

    switch (subEntity.GetSubEntityType())
        {
        case ISubEntity::SubEntityType::Vertex:
            {
            faceTag = PSolidUtil::GetPreferredFaceAttachmentFaceForVertex(PSolidSubEntity::GetSubEntityTag(subEntity));
            break;
            }

        case ISubEntity::SubEntityType::Edge:
            {
            faceTag = PSolidUtil::GetPreferredFaceAttachmentFaceForEdge(PSolidSubEntity::GetSubEntityTag(subEntity));
            break;
            }

        case ISubEntity::SubEntityType::Face:
            {
            faceTag = PSolidSubEntity::GetSubEntityTag(subEntity);
            break;
            }
        }
    ...
    return faceAttachmentsVec.at(attachmentIndex);
    }
...
struct BRepFaceHatchProcessor : IParasolidWireOutput
    ...
    bvector<ICurvePrimitivePtr>&    m_curves;
    ...
    * @bsimethod
    ...
    BentleyStatus _ProcessGoOutput(ICurvePrimitiveCR hatchCurve, PK_ENTITY_t entityTag)
        {
        ICurvePrimitivePtr curve = hatchCurve.Clone();
    ...
    BRepFaceHatchProcessor(bvector<ICurvePrimitivePtr>& curves, TransformCR entityTransform) : m_curves(curves), m_entityTransform(entityTransform) {}
...
// @bsimethod
//---------------------------------------------------------------------------------------
BentleyStatus BRepUtil::GetFaceHatching(bvector<ICurvePrimitivePtr>& curves, ISubEntityCR subEntity, uint32_t divisor, double tolerance)
    {
    if (ISubEntity::SubEntityType::Face != subEntity.GetSubEntityType())
        return ERROR;

    IBRepEntityCPtr parentEntity = subEntity.GetParentGeometry();

    if (!parentEntity.IsValid())
        return ERROR;

    size_t oldSize = curves.size();
    BRepFaceHatchProcessor output(curves, parentEntity->GetEntityTransform());

    PSolidGoOutput::ProcessFaceHatching(output, divisor, PSolidSubEntity::GetSubEntityTag(subEntity), tolerance);

    return (curves.size() > oldSize ? SUCCESS : ERROR);
    }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
IGeometryPtr BRepUtil::Create::SimplifyBody(IBRepEntityCR entity)
    {
    IGeometryPtr simplified = PSolidSimplify::Simplify(PSolidUtil::GetEntityTag(entity), false);

    if (simplified.IsValid())
        simplified->TryTransformInPlace(entity.GetEntityTransform());
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus BRepUtil::Create::BodyFromFace(IBRepEntityPtr& out, ISubEntityCR face)
    {
    if (ISubEntity::SubEntityType::Face != face.GetSubEntityType())
        return ERROR;
    ...
    if (SUCCESS != PK_FACE_make_sheet_body(1, &faceTag, &sheetTag))
        return ERROR;

    Transform   entityTransform = PSolidSubEntity::GetSubEntityTransform(face);

    out = PSolidUtil::CreateNewEntity(sheetTag, entityTransform);
...
BentleyStatus BRepUtil::Modify::BooleanCut(IBRepEntityPtr& target, IBRepEntityCR planarTool, CutDirectionMode directionMode, CutDepthMode depthMode, double depth, bool inside)
    {
    // NOTE: The PP version of this method specifies the profile as a CurveVector instead of a sheet body.
    ...
    //       The PP result can be achieved using BRepUtil::Create::SweptBodyFromOpenCurveVector and a solid/surface boolean subtract.
    if (!target.IsValid())
        return ERROR;
...
BentleyStatus BRepUtil::Modify::SewBodies(bvector<IBRepEntityPtr>& sewnEntities, bvector<IBRepEntityPtr>& unsewnEntities, bvector<IBRepEntityPtr>& toolEntities, double gapWidthBound, size_t nIterations)
    ...
    if (SUCCESS == status)
        ...
        for (IBRepEntityPtr& toolEntity : toolEntities)
            ...
            if (PK_ENTITY_null != toolEntityTag)
                {
                bvector<IBRepEntityPtr>::iterator itSewn = std::find_if(sewnEntities.begin(), sewnEntities.end(), [&](IBRepEntityPtr const& result) { return PSolidUtil::GetEntityTagForModify(*result) == toolEntityTag; });
                bvector<IBRepEntityPtr>::iterator itUnsewn = std::find_if(unsewnEntities.begin(), unsewnEntities.end(), [&](IBRepEntityPtr const& result) { return PSolidUtil::GetEntityTagForModify(*result) == toolEntityTag; });

                if (itSewn != sewnEntities.end() || itUnsewn != unsewnEntities.end())
                    PSolidUtil::ExtractEntityTag(*toolEntity); // Don't free body that is being returned in sewm/unsewn...
                }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus BRepUtil::Modify::ConvertSolidToSheet(IBRepEntityR targetEntity)
    {
    if (IBRepEntity::EntityType::Solid != targetEntity.GetEntityType())
        return ERROR;
    ...
    if (BRepUtil::IsDisjointBody(targetEntity))
        {
        bvector<IBRepEntityPtr> bodies;

        if (SUCCESS != BRepUtil::Modify::DisjoinBody(bodies, targetEntity))
            {
            PK_MARK_goto(markTag);
            PK_MARK_delete(markTag);
        ...
        // NOTE: bodies doesn't contain original; either it's unchanged or it now refers to the first region...
        if (SUCCESS != PSolidUtil::ConvertSolidBodyToSheet(targetEntityTag))
            {
            PK_MARK_goto(markTag);
            PK_MARK_delete(markTag);
        ...
        for (IBRepEntityPtr thisEntity : bodies)
            {
            PK_ENTITY_t thisEntityTag = PSolidUtil::GetEntityTagForModify(*thisEntity);

            if (SUCCESS != PSolidUtil::ConvertSolidBodyToSheet(thisEntityTag))
                {
                PK_MARK_goto(markTag);
                PK_MARK_delete(markTag);
        ...
        for (IBRepEntityPtr thisEntity : bodies)
            {
            PK_ENTITY_t thisEntityTag = PSolidUtil::ExtractEntityTag(*thisEntity); // Don't free entity that will be combined...

            if (SUCCESS != PSolidUtil::CombineBody(targetEntityTag, thisEntityTag))
                {
                PK_MARK_goto(markTag);
                PK_MARK_delete(markTag);
    ...
    BentleyStatus status = PSolidUtil::ConvertSolidBodyToSheet(targetEntityTag);
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus BRepUtil::Modify::OffsetBody(IBRepEntityR targetEntity, double distance, StepFacesOption addStep)
    {
    PK_ENTITY_t targetEntityTag = PSolidUtil::GetEntityTagForModify(targetEntity);
    ...
    invTargetTransform.InverseOf(targetEntity.GetEntityTransform());
    invTargetTransform.ScaleDoubleArrayByXColumnMagnitude(&offset, 1);

    PK_BODY_offset_o_t options;

    PK_BODY_offset_o_m(options);
    options.grow = PK_FACE_grow_auto_c;
    options.allow_disjoint = PK_LOGICAL_true;
    ...
    BentleyStatus   status = (SUCCESS == PK_BODY_offset_2(targetEntityTag, offset, 1.0e-6, &options, &tracking, &results) && PK_local_status_ok_c == results.status) ? SUCCESS : ERROR;
...
BentleyStatus BRepUtil::Modify::TransformEdges(IBRepEntityR targetEntity, bvector<ISubEntityPtr>& edges, bvector<Transform> const& transforms, StepFacesOption addStep)
    ...
    for (size_t iEdge = 0; iEdge < edges.size(); ++iEdge)
        ...
        bvector<PK_CURVE_t>::iterator itCurve = std::find(curves.begin(), curves.end(), curveTag);
        bool isSameCurve = (itCurve != curves.end());
        curves.push_back(curveTag);
        ...
        for (ISubEntityPtr facePtr : edgeFaces)
            {
            bvector<ISubEntityPtr>::iterator itFace = std::find_if(faces.begin(), faces.end(), [&](ISubEntityPtr const& face) { return facePtr->IsEqual(*face); });

            if (itFace != faces.end())
                {
                if (!isSameCurve)
                    faceTransforms[itFace - faces.begin()] = edgeTransform; // If more than 1 edge from same face is selected...just apply input transform to face...
                continue;
                }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus BRepUtil::RestoreEntityFromMemory(IBRepEntityPtr& out, uint8_t const* buffer, size_t bufferSize, TransformCR transform)
    {
    return PSolidUtil::RestoreEntityFromMemory(out, buffer, bufferSize, transform);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus BRepUtil::SaveEntityToMemory(uint8_t** buffer, size_t& bufferSize, IBRepEntityCR in)
    {
    return PSolidUtil::SaveEntityToMemory(buffer, bufferSize, in);
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
bool BRepUtil::HasConsistentTopologyAndGeometry(IBRepEntityCR entity)
    {
    int                 nFaults = 0;
    PK_check_fault_t*   faults = nullptr; // Should not be used with max_faults = 0
    ...
    PK_ERROR_code_t failure = PK_BODY_check(PSolidUtil::GetEntityTag(entity), &options, &nFaults, &faults);

    return (PK_ERROR_no_errors == failure);
    }