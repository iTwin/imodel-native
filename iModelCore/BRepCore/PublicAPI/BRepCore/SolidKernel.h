...
//! @note: The entity transform must be saved separately.
BREPCORE_EXPORT static BentleyStatus SaveEntityToMemory(uint8_t** ppBuffer, size_t& bufferSize, IBRepEntityCR);

//! Transmit a part to a logical block of data (in binary format) using the solid kernel.
BREPCORE_EXPORT static BentleyStatus RestoreEntityFromMemory(IBRepEntityPtr&, uint8_t const* pBuffer, size_t bufferSize, TransformCR);

//! Return true if the supplied entity passes curve/edge and surface/face consistency checks.
BREPCORE_EXPORT static bool HasConsistentTopologyAndGeometry(IBRepEntityCR);
...
//! Return a non-persistent identifier for this sub-entity that remains valid for the lifespan of the parent entity.
BREPCORE_EXPORT static uint32_t GetSubEntityId(ISubEntityCR);

//! Create a new ISubEntityPtr from its non-persistent identifier and parent entity transform.
BREPCORE_EXPORT static ISubEntityPtr CreateSubEntityFromId(uint32_t id, TransformCR entityTransform);

//! Create a new ISubEntityPtr from its non-persistent identifier and parent entity.
BREPCORE_EXPORT static ISubEntityPtr CreateSubEntityFromId(uint32_t id, IBRepEntityCR parent);
...
//! Evaluate point and normal at uv of center of the surface of the given face sub-entity.
//! @param[in] subEntity The face sub-entity to query.
//! @param[out] point The coordinates of the point on the surface at the center uv parameter.
//! @param[out] normal The normalized surface normal at the center uv parameter.
//! @return SUCCESS if the parameter could be evaluated and face was planar.
BREPCORE_EXPORT static BentleyStatus GetPlanarFaceData(ISubEntityCR subEntity, DPoint3dR point, DVec3dR normal);
...
//! Return whether the supplied entity is a redundant edge where the containing faces share a surface.
BREPCORE_EXPORT static bool IsRedundantEdge(ISubEntityCR);
...
BREPCORE_EXPORT static bool IsSmoothVertex(ISubEntityCR);
...
BREPCORE_EXPORT static PolyfaceHeaderPtr FacetEntity(IBRepEntityCR, IFacetOptionsCR);
...
//! Return the face attachment information for the supplied face, or the preferred face when an edge or vertex is specified.
//! @return Face attachment information most appropriate to the supplied face, edge, or vertex.
BREPCORE_EXPORT static FaceAttachment GetFaceMaterialAttachmentForSubEntity(ISubEntityCR subEntity, IFaceMaterialAttachmentsCR attachments);

//! Query the uv hatching curve geometry for the input face sub-entity.
//! @param[out] curves A vector to hold the hatch curves.
//! @param[in] subEntity The face sub-entity to query.
...
//! @return SUCCESS if input entity was the correct type and output vector was populated.
BREPCORE_EXPORT static BentleyStatus GetFaceHatching(bvector<ICurvePrimitivePtr>& curves, ISubEntityCR subEntity, uint32_t divisor, double tolerance = 0.0);
...
struct Create
    ...
    //! Represent a body as a CurveVector or ISolidPrimitive.
    //! @param[in] entity The body to try to simplify.
    //! @return nullptr if supplied entity could not be simplified.
    BREPCORE_EXPORT static IGeometryPtr SimplifyBody(IBRepEntityCR entity);
    ...
    //! Create a new sheet body from the supplied fadce.
    //! @param[out] out The new body.
    //! @param[in] face The face to create a body from.
    //! @return SUCCESS if body was created.
    BREPCORE_EXPORT static BentleyStatus BodyFromFace(IBRepEntityPtr& out, ISubEntityCR face);
...
struct Modify
    ...
    //! Convert the supplied solid body to a sheet body.
    //! @param[in,out] target The target solid body to change to a sheet.
    //! @return SUCCESS if change to surface could be completed.
    BREPCORE_EXPORT static BentleyStatus ConvertSolidToSheet(IBRepEntityR target);
    ...
    //! Modify the target solid or sheet body by offsetting all faces.
    //! @param[in,out] target The target body to modify.
    //! @param[in] distance The offset for each face.
    //! @param[in] addStep The option for how to handle the creation of step faces.
    //! @return SUCCESS if body could be offset.
    BREPCORE_EXPORT static BentleyStatus OffsetBody(IBRepEntityR target, double distance, StepFacesOption addStep = StepFacesOption::AddNone);
