...
BREPCORE_EXPORT static void ProcessFaceHatching(IParasolidWireOutput& output, int divisor, PK_FACE_t entityTag, double tolerance = 0.0);
...
BREPCORE_EXPORT static void SetLocation(ISubEntityR subEntity, DPoint3dCR point, DPoint2dCR param); //!< @private param x/y is face u/v, param x is edge u, na for vertex...
BREPCORE_EXPORT static void SetDisplayTangentEdges(ISubEntityR subEntity, bool display); //!< @private LocateSubEntityTool use. Include tangent edges in graphic...
BREPCORE_EXPORT static void UpdateCache(ISubEntityR subEntity, ISubEntityCR donorEntity); //!< @private LocateSubEntityTool use. Use existing geometry/graphic...
...
BREPCORE_EXPORT static IBRepEntityPtr InstanceEntity(IBRepEntityCR); //!< Create non-owning instance of an existing entity...
...
BREPCORE_EXPORT static PolyfaceHeaderPtr FacetEntity(IBRepEntityCR entity, IFacetOptionsR);
