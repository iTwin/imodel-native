...
BentleyStatus   PSolidGeom::BodyToCurveVectors (bvector<CurveVectorPtr>& curves, IBRepEntityCR entity, EdgeToCurveIdMap const* idMap)
    ...
    if (SUCCESS == PK_BODY_ask_regions (PSolidUtil::GetEntityTag (entity), &nRegions, &regions))
        {
        for (int iRegion = nRegions > 1 ? 1 : 0; iRegion < nRegions; iRegion++)
            {
            PK_SHELL_t*  shells = NULL;
            int          nShells = 0;

            if (SUCCESS == PK_REGION_ask_shells (regions[iRegion], &nShells, &shells))
                {
                for (int iShell=0; iShell<nShells; iShell++)
                    ...
                    if (SUCCESS == PK_SHELL_ask_wireframe_edges (shells[iShell], &nEdges, &edges) && nEdges > 0)
                        {
                        ...
                        for (int iEdge=0; iEdge<nEdges; iEdge++)
                            addEdgeToCurveVector (*curveVector, edges[iEdge]);
...
static BentleyStatus bodyFromCurveVector (PK_BODY_t& bodyTag, PK_VERTEX_t* startVertexP, CurveVectorCR curves, TransformCR uorToBodyTransform, bool coverClosed, EdgeToCurveIdMap* idMap)
    ...
    if (curves.IsUnionRegion ())
        ...
        else if (PK_ENTITY_null == bodyTag)
            {
            status = ERROR; // Hole loops consumed solid loop...treat as error...
            }
        }
    else
        {
        ...
        for (size_t iCurve = curves.size (); iCurve > 0; --iCurve)
            ...
            switch (curve->GetCurvePrimitiveType ())
                ...
                case ICurvePrimitive::CURVE_PRIMITIVE_TYPE_LineString:
                    ...
                    for (size_t j = points.size (); j > 1 && SUCCESS == status; --j)
                        {
                        DSegment3d  segment = DSegment3d::From (points[j-2], points[j-1]);
                ...
                case ICurvePrimitive::CURVE_PRIMITIVE_TYPE_Spiral:
                    ...
                    MSBsplineCurve    newCurve;

                    newCurve.CopyTransformed (*bcurve, uorToBodyTransform);

                    PK_CURVE_t      curveTag = 0;

                    if (SUCCESS == (status = PSolidGeom::CreateCurveFromMSBsplineCurve (&curveTag, newCurve)))
                        status = pki_scribe_curve_on_body (&edgeTag, bodyTag, curveTag, NULL, NULL);

                    if (SUCCESS != status)
                        {
                        PSolidUtil::NormalizeBsplineCurve (newCurve);
...
BentleyStatus PSolidGeom::BodyFromSolidPrimitive (IBRepEntityPtr& entityOut, ISolidPrimitiveCR primitive, uint32_t nodeId)
    ...
    switch (primitive.GetSolidPrimitiveType ())
        ...
        case SolidPrimitiveType_DgnRuledSweep:
                    ...
                    for (SolidLocationDetail::FaceIndices const& thisFace: faceIndices)
                        ...
                        switch (faceGeom->GetGeometryType ())
                            ...
                            case IGeometry::GeometryType::CurveVector:
                                {
                                CurveVectorPtr thisCurve = faceGeom->GetAsCurveVector();
                                PK_BODY_t thisSurfTag;

                                if (SUCCESS != PSolidGeom::BodyFromCurveVector (thisSurfTag, nullptr, *thisCurve, dgnToSolid, true))
                                    break;
                            ...
                            case IGeometry::GeometryType::BsplineSurface:
                                {
                                MSBsplineSurfacePtr thisSurface = faceGeom->GetAsMSBsplineSurface();

                                if (SUCCESS != thisSurface->TransformSurface (&dgnToSolid))
                                    break;

                                PK_BODY_t thisSurfTag;

                                if (SUCCESS != PSolidGeom::BodyFromMSBsplineSurface (thisSurfTag, *thisSurface))
                                    break;
            ...
            std::vector<PK_BODY_t>   profileBodies(nProfiles);
            std::vector<PK_VERTEX_t> startVertices(nProfiles);

            BeAssert(profileBodies[0] == 0 && profileBodies[nProfiles-1] == 0);
            ...
            PK_ENTITY_delete ((int) nProfiles, profileBodies.data());
...
BentleyStatus PSolidGeom::BodyFromLoft(IBRepEntityPtr& out, CurveVectorPtr* profiles, size_t nProfiles, CurveVectorPtr* guides, size_t nGuides, bool periodic, uint32_t nodeId)
    ...
    if (SUCCESS == status)
        ...
        makeProfilesWithUniformVertexCount(profileBodies.data(), nProfiles);

        if (SUCCESS == (status = PSolidGeom::BodyFromLoft (bodyTag, profileBodies.data(), startVertices.data(), nProfiles, guideBodies, nGuides, periodic)))
            {
            if (nodeId)
                PSolidTopoId::AddNodeIdAttributes (bodyTag, nodeId, true);
