...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
BentleyStatus PSolidUtil::CombineBody(PK_BODY_t targetBodyTag, PK_BODY_t toolBodyTag)
    {
    int          nRegions = 0;
    PK_REGION_t* regions = nullptr;

    if (SUCCESS != PK_BODY_ask_regions(targetBodyTag, &nRegions, &regions) || 0 == nRegions)
        return ERROR;

    PK_ERROR_code_t status = PK_REGION_combine_bodies(regions[0], toolBodyTag);
