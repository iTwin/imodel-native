...
BentleyStatus   PSolidGeom::CreateCurveFromMSBsplineCurve2d (PK_CURVE_t* curveP, MSBsplineCurveCR msbCurve)
    ...
    if (SUCCESS == PK_CURVE_ask_interval (curveTag, &range) &&
        ...
        _Analysis_assume_(pNewVertex != nullptr);
...
static void     setHoleOrigin (MSBsplineSurfaceR surface)
    ...
    if (nullptr == minBoundP)
        {
        BeAssert(false && "NEEDS WORK STATIC ANALYSIS - no point closer than 1.0E8");
        return;
        }
...
(
std::vector<MSBsplineCurve>& curves, /* <=> populated with curves */
...
double          tolerance   /* => tolerance used in the least square fitting */
    ...
    int         i, j, tmp, numPts, numCorners;

    /* pCorners used to query which points are corners so to break into seperate curves */
    if (num < 2)
        return  ERROR;

    std::vector<int> pCorners(num);
    std::vector<DPoint3d> pPoints(num);

    /* Step 1: Find all sharp corner points */
    findAllCorners (pCorners.data(), &numCorners, pPts, num, angle);
    BeAssert(numCorners <= (int)pCorners.size());
    ...
    for (i = 0; i < numCorners; i++)
        ...
        if (i != numCorners - 1)
            {
            BeAssert(numPts <= (int)pPoints.size());
            BeAssert(pCorners[i]+numPts < num);
            ...
            BeAssert(tmp <= (int)pPoints.size());
            BeAssert(pCorners[i]+tmp < num);
            ...
            BeAssert(pCorners[0]+tmp <= (int)pPoints.size());
            BeAssert(pCorners[0] < num-1);
        ...
        MSBsplineCurve curve;
        if (SUCCESS == createFittingCurve (&curve, pPoints.data(), numPts, tolerance, smoothEnd)) // NEEDS WORK: did we mean to set status here?
            {
...
double          tolerance
    ...
    if (NULL != pBoundary->pFirst)
        ...
        uvCurvesPP[index] = new PK_ENTITY_t[numCurves + 1];
        ...
        if (num <= 1)
            ...
            {
            std::vector<MSBsplineCurve> curves;
            if (SUCCESS == (status = fitBoundaryToG1Curves (curves, pPts, num, angle, tolerance)))
                {
                ...
                for (int i=0; i < numCurves; i++)
                    {
                    PSolidGeom::CreateCurveFromMSBsplineCurve2d (&uvCurvesPP[index][i], curves[i]);
            ...
            uvCurvesPP[index] = new PK_ENTITY_t[num];
...
double          fitTol          // => used to fit in UV space
    ...
    for (int i = 0, numLoopsInCurrentBody = 0; i < num; i++)
        ...
        PK_ENTITY_t**   uvCurvesPP = new PK_ENTITY_t *[numLoopsInCurrentBody];
...
//      BeAssert (false && "Unable to extract boundaries for surface");
    ...
    // The normalization step should not be necessary - but our evaluate function doesn't handle max != 1.0 and I'm afraid to fix it right now...
    DPoint2d        min, max;
