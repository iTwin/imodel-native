...
static void resolveDuplicateFaceIds (int numFace, PK_FACE_t* pFaceTagArray, uint32_t lastNodeId)
    {
    std::vector<PK_FACE_t> pSameNodeIdArray(numFace);
    std::vector<PK_FACE_t> pSameFaceIdArray(numFace);
    std::vector<uint32_t>  pEntityIdArray(numFace);
    ...
    for (int outerIndex = 0; outerIndex < numFace; outerIndex++)
        ...
        if (facesOfFaceId > 1)
            ...
            if (lastNodeId) // Yuck...horribly complicated/inefficient sort that only matters for feature solids...
                std::sort (pSameFaceIdArray.data(), pSameFaceIdArray.data() + facesOfFaceId, SurroundingFaceComparer (lastNodeId));

            std::sort (pEntityIdArray.data(), pEntityIdArray.data() + facesOfNodeId);
