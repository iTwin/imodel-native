...
PK_LOGICAL_t    writeIn /* => input whether in read-mode or write-mode ? */
    ...
    if (PK_LOGICAL_true == writeIn)
        ...
        _Analysis_assume_(pNewMark != nullptr);
...
// One structure per open file containing info such as filename.
// The structures are stored in a map with C stream id as key.
// Accessed via the "s_pFrOpenFiles" variable.
struct FrustrumFile
    {
    int             m_guise = 0;
    int             m_format = 0;
...
typedef bmap<int, FrustrumFile> FrustrumFileMap;

static FrustrumFileMap* s_pFrOpenFiles;
...
struct TextStream : ByteStream
{
    operator char*() {return (char*)GetDataP();}
    operator const char*() const {return (const char*)GetData();}
};
static TextStream s_pFrIOBuffer;
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
static void     prependSchemaExtension (BeFileNameR filename, int guise)
    {
    if (FFCSCH != guise)
        return;
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
static void     appendExtension (BeFileNameR filename, wchar_t const* extension)
    {
    WString     ext;
    BeFileName::ParseName (NULL, NULL, NULL, &ext, filename);

    if (ext.empty())
        filename.AppendString(extension);
    }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
static int      deleteFile (const wchar_t* filename)
    {
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
static FrustrumFile* findOpenFile(int strid)
    {
    if (nullptr == s_pFrOpenFiles)
        return nullptr;

    FrustrumFileMap::iterator found = s_pFrOpenFiles->find(strid);

    if (found == s_pFrOpenFiles->end())
        return nullptr;
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
static int closeOpenFile(int strid, int action)
    {
    int failureCode = FR_close_fail;

    if (nullptr == s_pFrOpenFiles)
        return failureCode;

   FrustrumFile* pCurrOpenFile = findOpenFile(strid);

    if (nullptr == pCurrOpenFile)
        return failureCode;

    if (EOF == fclose(pCurrOpenFile->m_stream))
        return failureCode;

    if (FR_READ_WRITE_ACCESS == pCurrOpenFile->m_access || (FR_WRITE_ACCESS == pCurrOpenFile->m_access && FFABOR == action))
        failureCode = deleteFile(pCurrOpenFile->m_name.c_str());
    else
        failureCode = FR_no_errors;

    s_pFrOpenFiles->erase(strid);
...
static void closeAllOpenFiles()
    ...
    std::for_each(s_pFrOpenFiles->begin(), s_pFrOpenFiles->end(), [&](auto& entry) { fclose (entry.second.m_stream); });
    s_pFrOpenFiles->clear();
    }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
static FrustrumFile* registerOpenFile
(
...
int*            pStreamId       // <= output stream id fr file structure
)
    {
    int strid = 0;

    if (nullptr == s_pFrOpenFiles)
        s_pFrOpenFiles = new FrustrumFileMap();

    // NOTE: Expect only a couple entries in map...
    while (s_pFrOpenFiles->end() != s_pFrOpenFiles->find(++strid)) {};

    FrustrumFile ff;

    ff.m_guise  = guiseIn;
    ff.m_format = formatIn;
    ff.m_access = accessIn;
    ff.m_name.SetName(pFilenameIn);
    ff.m_key.SetName(pKeynameIn);
    ff.m_stream = pFileIn;

    *pStreamId = strid;
    (*s_pFrOpenFiles)[strid] = ff;

    return findOpenFile(strid);
    }
...
(
FrustrumFile*   pFileIn,            // => input file pointer
...
int*            pBufferLenOut       // <= no. of chars read
    ...
    {
    if (headerFlagIn || FFTEXT == pFileIn->m_format)
        {
        if (1 == maxBufferLenIn)
            {
            int     value = fgetc (pFileIn->m_stream);

            if (EOF == value)
                {
                return (feof (pFileIn->m_stream) ? FR_end_of_file : FR_read_fail);
                }
            else
                {
        ...
        s_pFrIOBuffer.Resize(required); // check whether current global input-output buffer long enough
        ...
        if (NULL == fgets (s_pFrIOBuffer, maxBufferLenIn + 1, pFileIn->m_stream))
            return (feof (pFileIn->m_stream) ? FR_end_of_file : FR_read_fail);
    ...
    int     chars = (int) fread (pBufferOut, (unsigned) (sizeof (char)), maxBufferLenIn, pFileIn->m_stream);
...
size_t          bufferLenIn         // => input buffer size
    ...
    {
    if (headerFlagIn || pFileIn->m_format == FFTEXT)
        {
        if (1 == bufferLenIn)
            {
            if (EOF == fputc (pBufferIn[0], pFileIn->m_stream))
        ...
        if (EOF == fputs (s_pFrIOBuffer, pFileIn->m_stream))
            return FR_write_fail;

        return FR_no_errors;
        }

    int     written = (int) fwrite (pBufferIn, (unsigned) (sizeof (char)), bufferLenIn, pFileIn->m_stream);
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
static int      skipFileHeader (FrustrumFile* pFileIn)
    {
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
static int      writeFileHeader (FrustrumFile* pFileIn, const char pr2hdr[])
    {
    ...
    BeStringUtilities::Strncpy (buffer, "APPL=iTwin;\n");
    if (FR_no_errors != writeToFile (pFileIn, buffer, 1, strlen (buffer)))
        return FR_write_fail;
    ...
    strcat (buffer, getFileFormatString (pFileIn->m_format));
    strcat (buffer, ";\n");
    if (FR_no_errors != writeToFile (pFileIn, buffer, 1, strlen (buffer)))
        return FR_write_fail;

    BeStringUtilities::Strncpy (buffer, "GUISE=");
    strcat (buffer, getFileGuiseString (pFileIn->m_guise));
    strcat (buffer, ";\n");
    if (FR_no_errors != writeToFile (pFileIn, buffer, 1, strlen (buffer)))
        return FR_write_fail;

    char    fileStr[FR_MAX_HEADER_LINE + 1];

    BeStringUtilities::Strncpy (buffer, "KEY=");
    strcat (buffer, BeStringUtilities::WCharToCurrentLocaleChar (fileStr, pFileIn->m_key.c_str(), sizeof (fileStr)));
    strcat (buffer, ";\n");
    if (FR_no_errors != writeToFile (pFileIn, buffer, 1, strlen (buffer)))
        return FR_write_fail;

    BeStringUtilities::Strncpy (buffer, "FILE=");
    strcat (buffer, BeStringUtilities::WCharToCurrentLocaleChar (fileStr, pFileIn->m_name.c_str(), sizeof (fileStr)));
    strcat (buffer, ";\n");
    if (FR_no_errors != writeToFile (pFileIn, buffer, 1, strlen (buffer)))
        return FR_write_fail;
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
static int      writeXMLFileHeader (FrustrumFile* pFileIn, const char pr2hdr[])
    {
    char        buffer[FR_MAX_HEADER_LINE];
...
int*                strid       // <= output stream-id on which file is opened
    ...
    BeFileName filename((wchar_t const*) name);

#if defined(BENTLEYCONFIG_OS_UNIX)
    WString lower(filename.c_str());
    lower.ToLower();
    filename.SetName(lower);
#endif
...
#elif defined(BENTLEYCONFIG_OS_UNIX)
        fp = fopen(Utf8String(filename.c_str()).c_str(), "r");
#else
        #error Unsupported OS.
#endif
    else
...
#elif defined(BENTLEYCONFIG_OS_UNIX)
        fp = fopen(Utf8String(filename.c_str()).c_str(), "rb");
#else
        #error Unsupported OS.
#endif
    ...
    FrustrumFile* pFr = registerOpenFile(fp, guise, format, FR_READ_ACCESS, filename, (wchar_t const*) name, strid);
...
int*                strid       // <= output stream-id on which file is opened
...
#elif defined(BENTLEYCONFIG_OS_UNIX)
        fp = fopen(Utf8String(filename.c_str()).c_str(), "w");
#else
        #error Unsupported OS.
#endif
    else
...
#elif defined(BENTLEYCONFIG_OS_UNIX)
        fp = fopen(Utf8String(filename.c_str()).c_str(), "wb");
#else
        #error Unsupported OS.
#endif
    ...
    FrustrumFile* pFr = registerOpenFile (fp, guise, format, FR_WRITE_ACCESS, filename, (wchar_t const*) name, strid);
...
int*            pSizeOut,           // <= output actual file size (bytes)
...
int*            pFailureCodeOut     // <= outputs failure code if error, else 0
    ...
    *pStreamIdOut = FR_NULL_STRID;
    ...
    BeFileName filename(L"rollback.001");
    BeFileName keyname(L"rollback");

#if defined(BENTLEYCONFIG_OS_WINDOWS)
    FILE*   fp = _wfopen (filename.c_str(), L"a");
#elif defined(BENTLEYCONFIG_OS_UNIX)
    FILE*   fp = fopen(Utf8String(filename.c_str()).c_str(), "a");
#else
        #error Unsupported OS.
#endif
    ...
    FrustrumFile* pFr = registerOpenFile (fp, *pGuiseIn, fileFormat, FR_READ_WRITE_ACCESS, filename.c_str(), keyname, pStreamIdOut);
...
int*            pFailureCodeOut     // <= outputs failure code if error, else 0
    ...
    BeStringUtilities::CurrentLocaleCharToWChar(fileKeyW, fileKeyIn);

    *pFailureCodeOut = pki_UCOPRD (*pGuiseIn, *pFormatIn, (const PK_UCHAR_t *) fileKeyW.c_str(), (*pSkipHeaderIn == 0 ? PK_LOGICAL_false : PK_LOGICAL_true), pStreamIdOut);
    }
...
int*            pFailureCodeOut         // <= outputs failure code if error, else 0
    ...
    *pFailureCodeOut = pki_UCOPWR (*pGuiseIn, *pFormatIn, (const PK_UCHAR_t *) fileKeyW.c_str(), pr2hdrP, pStreamIdOut);
    }
...
int*            pFailureCodeOut     // <= outputs failure code if error, else 0
    ...
    *numCharOut = 0;
    ...
    FrustrumFile* pCurrOpenFile = findOpenFile(*pStreamIdIn);
    ...
    // check file guise
    if (*pGuiseIn != pCurrOpenFile->m_guise)
        {
        *pFailureCodeOut = FR_unspecified;
        return;
        }
    ...
    // check access
    if (pCurrOpenFile->m_access != FR_READ_ACCESS && pCurrOpenFile->m_access != FR_READ_WRITE_ACCESS)
        {
        *pFailureCodeOut = FR_unspecified;
        return;
        }
...
int*            pFailureCodeOut     // <= outputs failure code if error, else 0
    ...
    *pFailureCodeOut = FR_unspecified;
    ...
    // reset file pointer
    if (fseek (pCurrOpenFile->m_stream, (long) (*pPositionIn), 0) != 0)
        {
        *pFailureCodeOut = FR_unspecified;
        return;
        }
...
int*            pFailureCodeOut     // <= outputs failure code if error, else 0
    ...
    *pFailureCodeOut = FR_unspecified;
    ...
    // note file pointer
    *pPositionOut = ftell (pCurrOpenFile->m_stream);
...
int*            pFailureCodeOut     // <= outputs failure code if error, else 0
    ...
    *pFailureCodeOut = FR_unspecified;
    ...
    // check access
    if (pCurrOpenFile->m_access != FR_WRITE_ACCESS && pCurrOpenFile->m_access != FR_READ_WRITE_ACCESS)
        {
        *pFailureCodeOut = FR_unspecified;
        return;
        }
...
int*            pFailureCodeOut     // <= outputs failure code if error, else 0
    ...
    *pFailureCodeOut = closeOpenFile(*pStreamIdIn, *pActionIn);
    }
...
// @bsimethod
//---------------------------------------------------------------------------------------
static bool setupHatchOptionsForFace(PK_TOPOL_render_line_o_t& options, PK_FACE_t faceTag, uint32_t divisor, double tolerance)
    {
...
// @bsimethod
//---------------------------------------------------------------------------------------
void PSolidGoOutput::ProcessFaceHatching(IParasolidWireOutput& output, uint32_t divisor, PK_FACE_t entityTag, double tolerance)
    {
    if (0 == divisor)
        return;
...
#endif
    PK_MARK_stop(); // pki_start_modeler calls pki_mark_start...
