...
virtual UInt32          _GetExtenderId() const override {return GEOCOORD_ACS_EXTENDERID;}
virtual UInt32          _GetSerializedSize () const override {return 0;};
virtual StatusInt       _Serialize (void *buffer, UInt32 maxSize) const override {return SUCCESS;}
...
* @bsimethod                                                    Barry.Bentley   01/07
+---------------+---------------+---------------+---------------+---------------+------*/
virtual IAuxCoordSysP   _Deserialize (void *persistentData, UInt32 dataSize, DgnModelRefP modelRef) override
    {
    // if we don't have a GCS, can't do anything.
    DgnGCSP     dgnGCS;
    if (NULL == (dgnGCS = DgnGCS::FromModel (modelRef, true)))
...
* @bsimethod                                    Barry.Bentley                   06/10
+---------------+---------------+---------------+---------------+---------------+------*/
virtual UInt32          _GetSerializedSize () const override 
    {
...
* @bsimethod                                    Barry.Bentley                   06/10
+---------------+---------------+---------------+---------------+---------------+------*/
virtual StatusInt       _Serialize (void *buffer, UInt32 maxSize) const override 
    {
...
virtual UInt32          _GetExtenderId() const override {return GEOCOORD_MGRS_EXTENDERID;}
