...
typedef struct GeoCoordType66
    {
    Int32       gcoord66_size;          // Size in Bytes of this structure.  If we find something we do not expect here, we should stop
    Int32       version;                // Version Number
    UInt32      deprec_fPos;            // last known position of type 66 element. Not used, and as of 8.9.4 not set. I don't see how this was ever usable.
    Int16       unused1;                // added to account for abovementioned errors.
    Int16       unused2;                // added to account for abovementioned errors.
    ...
    UInt32      deprec_mastunits;       // master units per design - this is never used and is always 0 as of 8.9.4
    UInt32      deprec_subpermast;      // sub-units per master unit (unsigned)
    UInt32      deprec_uorpersub;       // uors per sub unit (unsigned). This isn't necessarily an integer in MicroStation V8, as of 8.9.4 deprecated.

    UInt32      protect;                // If true then this cannot be changed by end user
    Int32       projType;               // which member of union proj to use
    Int32       family;                 // *** OBSOLETE ***  This is now a derived field.  Don't delete the field or else alignment problems
    Int32       coordsys;               // Actual coordinate system we base on
    ...
    UInt32      to84_via;               // Conversion technique, one of: cs_DTCTYP_MOLO, cs_DTCTYP_BURSA, cs_DTCTYP_MREG, cs_DTCTYP_NAD27, cs_DTCTYP_NAD83
                                        // cs_DTCTYP_WGS84, cs_DTCTYP_WGS72 cs_DTCTYP_HPGN,  cs_DTCTYP_LCLGRF, cs_DTCTYP_GENGRID
    Int32       minor_version;
    Int32       attributes;
    Int32       prj_code;               // CSMap equivalent to projType (different values)
    UInt16      verticalDatum;          // A member of the the VertDatumCode enumerator. Ignored if verticalDatumValid not set correctly.
    UInt16      verticalDatumValid;     // This must have value 0x8117 to verify that that verticalDatum is valid. That's because we can't say for that filler was always 0'ed
