...
private:
    ...
    UInt32              m_pointsReprojected;
    UInt32              m_transformsComputed;
    UInt32              m_detailPointsAdded;
...
StatusInt DoReprojection (bool undoable, bool highFidelity)
    ...
    UInt32                          originalEndOfFile = cache->GetEOF (DgnModelSections::GraphicElements);
    iter.SetModel (cache, DgnModelSections::GraphicElements);
    for (currentCount=0, elemRef = iter.GetFirstElementRef(); (NULL != elemRef) && !iter.HitEOF(); elemRef = iter.GetNextElementRef())
        {
        bool    replaced = false;

        if (elemRef->GetFilePos() >= originalEndOfFile)
            break;
    ...
    originalEndOfFile   = cache->GetEOF (DgnModelSections::ControlElements);
    iter.SetModel (cache, DgnModelSections::ControlElements);
    for (currentCount=0, elemRef = iter.GetFirstElementRef(); (NULL != elemRef) && !iter.HitEOF(); elemRef = iter.GetNextElementRef())
        {
        bool    replaced = false;
...
bool DgnGeoCoordinationAdmin::_CanShareDgnFile (DgnFileR testDgnFile, DgnAttachmentR refP) const
    ...
    if ((NULL == modelName) || (0 == *modelName))
        ...
        {
        if (SUCCESS != dgnFileObj_findModelIDByName (&testDgnFile, &modelID, modelName))
            return true;
        }
...
* @bsimethod                                                    Barry.Bentley   10/06
+---------------+---------------+---------------+---------------+---------------+------*/
DGNGEOCOORD_EXPORTED StatusInt dgnGeoCoord_reprojectToGCS (DgnModelRefP modelRef, DgnGCSP newGCS, bool reportProblems)
    {
    DgnGCSP     sourceGCS;
