...
+===============+===============+===============+===============+===============+======*/
ref class    GeoDatumStatusPanel : SWF::Panel
{
private:
...
+---------------+---------------+---------------+---------------+---------------+------*/
public:
GeoDatumStatusPanel (Datum^ datum, SWF::Button^ okButton)
    {
    m_datum             = datum;
    m_instanceList      = nullptr;
    m_okButton          = okButton;
    m_originalName      = datum->Name;
...
void            SetListeners (ECI::ECInstanceList^ instanceList)
    ...
    for each (ECI::IECInstance^ instance in instanceList)
        {
        instance->ECPropertyValueChanged += gcnew ECI::ECPropertyValueChangedHandler (this, &GeoDatumStatusPanel::PropertyValueChangedHandler);
        }
    }
...
void            PropertyValueChangedHandler (System::Object^ sender, ECI::ECPropertyValueChangedEventArgs^ args)
    ...
    if ( (nullptr != m_originalName) && (0 == String::Compare (m_datum->Name, m_originalName, true)) )
        isOk = true;
    else
        isOk = m_datum->NameUnique (inSystemLibrary);
    ...
    if (isOk)
        ...
        {
        m_lines[0]->Text = GeoCoordinateLocalization::GetLocalizedString (inSystemLibrary ? "DatumInSystemLibrary" : "DatumInUserLibrary");
        m_lines[1]->Text = GeoCoordinateLocalization::GetLocalizedString ("SelectAnotherDatumName");
        }
...
* @bsiclass                                                     Barry.Bentley   12/06
+===============+===============+===============+===============+===============+======*/
ref class    GeoDatumPropertyPane : ECUI::ECPropertyPane
{
Datum^                  m_datum;
GCSECObjectModel^       m_gcsECObjectModel;
GeoDatumStatusPanel^    m_statusPanel;

public:
/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    BarryBentley    08/07
+---------------+---------------+---------------+---------------+---------------+------*/
GeoDatumPropertyPane
(
Datum^                  datum,
GCSECObjectModel^       gcsECObjectModel,
String^                 stateFileName
) : ECUI::ECPropertyPane (nullptr, nullptr, nullptr, L"DatumProps", false, false, true, stateFileName, 1000)
    {
    m_datum             = datum;
...
virtual void         ReloadInstanceLists () override
    ...
    if (nullptr != m_datum)
        {
        instanceList    = m_gcsECObjectModel->GetDatumProperties (m_datum);
...
+---------------+---------------+---------------+---------------+---------------+------*/
property Bentley::GeoCoordinatesNET::Datum^   Datum
    {
    Bentley::GeoCoordinatesNET::Datum^      get () { return m_datum; }
    void        set (Bentley::GeoCoordinatesNET::Datum^ value) { m_datum= value; ReloadInstanceLists(); }
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    BarryBentley    08/07
+---------------+---------------+---------------+---------------+---------------+------*/
property GeoDatumStatusPanel^   StatusPanel
    {
    GeoDatumStatusPanel^    get () {return m_statusPanel;}
    void                    set (GeoDatumStatusPanel^ value) {m_statusPanel = value;}
    }
};
...
* @bsiclass                                                     Barry.Bentley   12/06
+===============+===============+===============+===============+===============+======*/
public ref class    GeoDatumPropertiesPanel : SWF::Panel
{
private:
GeoDatumPropertyPane^   m_propertyPane;
GeoDatumStatusPanel^    m_statusPanel;
...
SWF::Button^        okButton
    ...
    {
    Init (datum, forEditing, okButton);
    }
...
SWF::Button^        okButton
    ...
    m_propertyPane                          = gcnew GeoDatumPropertyPane (nullptr, gcsECObjectModel, "GCSProperty");
    ...
    if (!editData)
        ...
        m_statusPanel               = gcnew GeoDatumStatusPanel (datum, okButton);
...
* @bsiclass                                                     Barry.Bentley   12/06
+===============+===============+===============+===============+===============+======*/
public ref class    EditDatumFormContents
{
private:
...
GeoDatumPropertiesPanel^    m_propertiesPanel;
...
bool                forEditing
    ...
    {
    InitializeComponent (form, datum, forEditing);
    }
...
bool                forEditing
    ...
    m_propertiesPanel           = gcnew GeoDatumPropertiesPanel (datum, forEditing, m_okButton);
...
+===============+===============+===============+===============+===============+======*/
public ref class   DatumTreeNode : SWF::TreeNode
{
internal:
/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    BarryBentley    05/10
+---------------+---------------+---------------+---------------+---------------+------*/
DatumTreeNode (String^ name, SWF::ContextMenu^ contextMenu) : SWF::TreeNode (name)
    {
...
* @bsiclass                                                     Barry.Bentley   05/10
+===============+===============+===============+===============+===============+======*/
public ref class    DatumNode : DatumTreeNode
{
private:
Datum^      m_datum;
bool        m_fromUserLib;
...
+---------------+---------------+---------------+---------------+---------------+------*/
DatumNode (String^ name, SWF::ContextMenu^ contextMenu, Datum^ datum, bool fromUserLib) : DatumTreeNode (name, contextMenu)
    {
...
property Datum^  Datum
    {
    Bentley::GeoCoordinatesNET::Datum^  get () { return m_datum; }
    void    set (Bentley::GeoCoordinatesNET::Datum^ value) { m_datum = value; }
    }
...
property String^    KeyName
    {
    String^ get () { return m_datum->Name; }
    }
};
...
* @bsiclass                                                     Barry.Bentley   05/10
+===============+===============+===============+===============+===============+======*/
public ref class    DatumLibraryNode : DatumTreeNode
{
private:
bool            m_isUserLib;
...
* @bsimethod                                                    Barry.Bentley   05/10
+---------------+---------------+---------------+---------------+---------------+------*/
DatumLibraryNode (String^ name, SWF::ContextMenu^ contextMenu, BGC::LibraryP sourceLibrary) : DatumTreeNode (name, contextMenu)
    {
    ...
    for (uint32_t iDatum=0; iDatum < datumCount; iDatum++)
        ...
        if (BSISUCCESS == sourceLibrary->GetDatumName (iDatum, datumName))
            {
            CSDatumDef* datumDef = sourceLibrary->GetDatum (datumName.c_str());
            if (NULL != datumDef)
                {
                Datum^   geoDatum = gcnew Datum (*datumDef, sourceLibrary);
                if (geoDatum->Valid)
                    Nodes->Add (gcnew DatumNode (geoDatum->Name, contextMenu, geoDatum, m_isUserLib));
                }
            }
        }
    }
...
* @bsimethod                                                    BarryBentley    05/10
+---------------+---------------+---------------+---------------+---------------+------*/
void            DeleteMember (DatumNode^ node, SWF::Form^ hostForm)
    {
    // the node passed in represents the Datum to be deleted.
    // The call to Library->DeleteDatum makes sure that the Datum is not in use by any of the GC's in this library (those are the only ones that can us it).
    BI::ScopedString    datumName (node->KeyName);
    StatusInt   status = m_library->DeleteDatum (datumName.Uni());
    if (BGC::GEOCOORDERR_DatumInUse == status)
        {
        SWF::MessageBox::Show (hostForm, GeoCoordinateLocalization::GetLocalizedString ("DatumInUse"), 
                                GeoCoordinateLocalization::GetLocalizedString ("CannotDeleteDatumTitle"), SWF::MessageBoxButtons::OK);
        return;
        }
...
* @bsiclass                                                     Barry.Bentley   05/10
+===============+===============+===============+===============+===============+======*/
private ref class    DatumEditingPanel : SWF::Panel
{
private:
SWF::Form^                  m_hostForm;
...
public:
static String^        DatumImageKey = "Datum";
...
* @bsimethod                                    Barry.Bentley                   05/10
+---------------+---------------+---------------+---------------+---------------+------*/
void            SetSelectedDatum (Datum^ datum)
    {
    m_propertyPane->Datum = datum;
    }
...
* @bsimethod                                    Barry.Bentley                   05/10
+---------------+---------------+---------------+---------------+---------------+------*/
bool            EditDatum (Datum^ datum)
    {
    datum->CanEdit                      = true;
    SWF::Form^                form      = HostFormProvider->GetEditDatumForm();
    gcnew EditDatumFormContents (form, datum, true);
    ...
    // if the user hits OK, verify that the new Datum has a unique name.
...
void            PopulateLibraryNode ()
    ...
    m_treeView->Nodes->Add (gcnew DatumLibraryNode (gcnew String (m_library->GetGUIName().data()), m_contextMenu, m_library));
    }
...
SWF::TreeViewEventArgs^   eventArgs
    ...
    {
    DatumNode^  datumNode;
    if (nullptr != (datumNode = dynamic_cast<DatumNode^> (eventArgs->Node)))
        {
        m_propertyPane->Datum = datumNode->Datum;
        SetSelectedDatum (datumNode->Datum);
        }
    else
        {
        SetSelectedDatum (nullptr);
        }

    bool libraryReadOnly    = m_library->IsReadOnly();
    m_editButton->Enabled   = (nullptr != datumNode) && !libraryReadOnly;
    m_deleteButton->Enabled = (nullptr != datumNode) && !libraryReadOnly;
    }
...
System::EventArgs^  eventArgs
    ...
    // 1) DatumLibraryNode that can't be editted.
    // 2) DatumNode that can be editted. Datums in User Libraries are editable, Datums in the system library are not.
    ...
    DatumTreeNode^       treeNode;
    if (nullptr == (treeNode = dynamic_cast <DatumTreeNode^> (m_treeView->SelectedNode)))
        return;
...
System::EventArgs^  eventArgs
    ...
    DatumLibraryNode^   libraryNode;
    if ( (nullptr == (libraryNode = dynamic_cast<DatumLibraryNode^>(selectedNode))) &&
         (nullptr == (libraryNode = dynamic_cast<DatumLibraryNode^>(selectedNode->Parent))) )
        return;
...
System::EventArgs^  eventArgs
    ...
    // get node and make sure it's a group (it should be)
    DatumNode^         datumNode;
    if ( (nullptr != m_treeView) && (nullptr != (datumNode = dynamic_cast<DatumNode^>(m_treeView->SelectedNode))) )
        {
        SWF::Clipboard::SetDataObject (gcnew SWF::DataObject (gcnew MemberNodeInfo (datumNode->KeyName)));
        }
    }
...
System::EventArgs^  eventArgs
    ...
    DatumNode^          datumNode;
    if ( (nullptr == (datumNode = dynamic_cast<DatumNode^>(selectedNode))) )
        return;

    SWF::Clipboard::SetDataObject (gcnew SWF::DataObject (gcnew MemberNodeInfo (datumNode->KeyName)));

    // remove from the tree
    DatumLibraryNode^  parentNode;
    if (nullptr != (parentNode = dynamic_cast<DatumLibraryNode^>(datumNode->Parent)))
        parentNode->DeleteMember (datumNode, m_hostForm);
    }
...
System::EventArgs^  eventArgs
    ...
    Datum^   clone = gcnew Datum (datumNode->Datum);

    if (EditDatum (clone))
        {
        // we need to update the node name (if the name changed), update the file, and update the info panel.
        StatusInt status = datumNode->Datum->ReplaceInLibrary (clone);
        if (BGC::GEOCOORDERR_DatumIllegalName == status)
            {
            SWF::MessageBox::Show (m_hostForm, GeoCoordinateLocalization::GetLocalizedString ("IllegalDatumName"),
                            GeoCoordinateLocalization::GetLocalizedString ("CannotReplaceDatumTitle"), SWF::MessageBoxButtons::OK);
            return;
            }
        ...
        else if (BGC::GEOCOORDERR_DatumNoUniqueName == status)
            {
        ...
        datumNode->Datum = clone;
        datumNode->Text  = clone->Name;
        SetSelectedDatum (datumNode->Datum);
...
* @bsimethod                                    Barry.Bentley                   05/10
+---------------+---------------+---------------+---------------+---------------+------*/
Datum^       CreateNewDatum (String^ nameTemplate)
    {
    BI::ScopedString    newNameTemplate (nameTemplate);
    BGC::DatumP         createdDatum;

    if (BSISUCCESS == m_library->CreateNewDatum (createdDatum, newNameTemplate.Uni()))
        {
        Datum^   newDatum = gcnew Datum (createdDatum);
        newDatum->Source        = m_userOrganizationName;
        newDatum->Description   = GeoCoordinateLocalization::GetLocalizedString ("UserDefined");
        return newDatum;
        }
    else
        {
...
System::EventArgs^      eventArgs
    ...
    Datum^      newDatum = CreateNewDatum (newNameTemplate);
    ...
    // Allow the user to initialize the new Datum.
    if (EditDatum (newDatum))
        {
        // should have unique name since we enforce that while editing.
        StatusInt   status;
        if (BSISUCCESS != (status = newDatum->AddToLibrary()))
            {
            if (BGC::GEOCOORDERR_DatumIllegalName == status)
                {
                SWF::MessageBox::Show (m_hostForm, GeoCoordinateLocalization::GetLocalizedString ("IllegalDatumName"), 
                                GeoCoordinateLocalization::GetLocalizedString ("CannotAddDatumTitle"), SWF::MessageBoxButtons::OK);
                return;
                }
            else if (BGC::GEOCOORDERR_LibraryReadonly == status)
                {
            ...
            else if (BGC::GEOCOORDERR_MaxUserLibraryDatums == status)
                {
                SWF::MessageBox::Show (m_hostForm, GeoCoordinateLocalization::GetLocalizedString ("MaxUserDatums"), 
                                GeoCoordinateLocalization::GetLocalizedString ("CannotAddDatumTitle"), SWF::MessageBoxButtons::OK);
                return;
                }
            assert (false);
        ...
        BI::ScopedString    datumName (newDatum->Name);
        CSDatumDef* datumDef;
        if (NULL == (datumDef = m_library->GetDatum (datumName.Uni())))
            {
            assert (false);
            return;
            }

        Datum^      geoDatum = gcnew Datum (*datumDef, m_library);
        m_treeView->Nodes[0]->Nodes->Add (gcnew DatumNode (geoDatum->Name, m_contextMenu, geoDatum, true));
        }
    }
...
void            CreateMenuItems ()
    ...
    m_cutMenuItem->Click += gcnew System::EventHandler (this, &DatumEditingPanel::CutMemberClickEvent);

    m_copyMenuItem = gcnew SWF::MenuItem (GeoCoordinateLocalization::GetLocalizedString ("Copy"));
    m_copyMenuItem->Click += gcnew System::EventHandler (this, &DatumEditingPanel::CopyMemberClickEvent);

    m_pasteMenuItem = gcnew SWF::MenuItem (GeoCoordinateLocalization::GetLocalizedString ("Paste"));
    m_pasteMenuItem->Click += gcnew System::EventHandler (this, &DatumEditingPanel::PasteClickEvent);

    m_deleteFromLibraryMenuItem = gcnew SWF::MenuItem (GeoCoordinateLocalization::GetLocalizedString ("DeleteFromLibrary"));
    m_deleteFromLibraryMenuItem->Click += gcnew System::EventHandler (this, &DatumEditingPanel::DeleteFromLibraryClickEvent);

    m_editMenuItem = gcnew SWF::MenuItem (GeoCoordinateLocalization::GetLocalizedString ("EditProperties"));
    m_editMenuItem->Click += gcnew System::EventHandler (this, &DatumEditingPanel::EditPropertiesClickEvent);
    }
...
+---------------+---------------+---------------+---------------+---------------+------*/
void            DatumEditingPanel::InitializeComponent
(
SWF::ImageList^             imageList
)
    {
    ...
    m_editButton->Click                 += gcnew System::EventHandler (this, &DatumEditingPanel::EditPropertiesClickEvent);
    ...
    m_newButton->Click                  += gcnew System::EventHandler (this, &DatumEditingPanel::NewButtonClicked);
    ...
    m_deleteButton->Click               += gcnew System::EventHandler (this, &DatumEditingPanel::DeleteFromLibraryClickEvent);
    ...
    m_treeView->AfterSelect             += gcnew SWF::TreeViewEventHandler (this, &DatumEditingPanel::NodeSelected);
    m_treeView->NodeMouseDoubleClick    += gcnew SWF::TreeNodeMouseClickEventHandler (this, &DatumEditingPanel::NodeDoubleClicked);
    m_treeView->NodeMouseClick          += gcnew SWF::TreeNodeMouseClickEventHandler (this, &DatumEditingPanel::NodeClicked);

    PopulateLibraryNode ();
    m_treeView->SelectedNode            = m_treeView->Nodes[0];
    m_treeView->Nodes[0]->Expand();

    m_propertyPane                      = gcnew GeoDatumPropertyPane (nullptr, m_ecObjectModel, "GCSLibrary");
    m_propertyPane->LayoutPreset        = BUCWG::GroupLayoutPreset::StrictTopDown;
    m_propertyPane->Dock                = SWF::DockStyle::Fill;
...
* @bsimethod                                                    BarryBentley    03/07
+---------------+---------------+---------------+---------------+---------------+------*/
void            DatumNode::InitializeImageKeys ()
    {
    ImageKey            = DatumEditingPanel::DatumImageKey;
    SelectedImageKey    = DatumEditingPanel::DatumImageKey;
    }

/*---------------------------------------------------------------------------------**//**
* @bsimethod                                                    BarryBentley    03/07
+---------------+---------------+---------------+---------------+---------------+------*/
void    DatumLibraryNode::InitializeImageKeys ()
    {
