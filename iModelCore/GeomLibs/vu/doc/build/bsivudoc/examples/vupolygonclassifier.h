/*--------------------------------------------------------------------------------------+
|
|     $Source: vu/doc/build/bsivudoc/examples/vupolygonclassifier.h $
|
...
static void MarkNonParityExterior
(
VuSetP pGraph
)
    {
...
static void ExpandInteriorFaces
(
VuSetP pGraph,
int noisy
)
...
void SetupPolygon(bool bPushStack, STDVectorDPoint3dR xyzArray)
    {
    if (bPushStack)
...
void SetupForPolygonBinaryOp (STDVectorDPoint3dR xyzA, STDVectorDPoint3dR xyzB)//Igor.Sokolov 28.08.2008: use references - avoid copy data (STDVectorDPoint3d xyzA, STDVectorDPoint3d xyzB)
    {
...
bool InsertOneLoop (STDVectorDPoint3dR xyzIn, int iStart, int &iNext)
    {
    int i0 = iStart;
    int i1 = i0;
...
void InsertLoopsWithDisconnects (STDVectorDPoint3dR xyzIn)
    {
    int iNext = 0;
...
// Perform a planar merge of all edges in the array. (Disconnects as separators)
// On exit, the face array is prepped for looping over all faces with GetFace ()
// If shortEdgeTol is positive, the graph is triangulated, and added edges that are shorter
//  are promoted to be treated as if they were part of the original input.
void ClassifyPlane (DPoint3d *pXYZArrayA, void **pEdgeData, int numXYZA, double shortEdgeTol)
    {
    vu_reinitializeVuSet (mpGraph);
    InsertEdgesWithDisconnects (pXYZArrayA, pEdgeData, numXYZA);
    vu_mergeOrUnionLoops (mpGraph, VUUNION_UNION);
    ...
    if (shortEdgeTol > 0.0)
        {
        double tol2 = shortEdgeTol * shortEdgeTol;
        vu_triangulateMonotoneInteriorFaces (mpGraph, false);
        vu_flipTrianglesToImproveQuadraticAspectRatio (mpGraph);
        VU_SET_LOOP (pCurr, mpGraph)
            {
            if (!vu_getMask (pCurr, VU_BOUNDARY_EDGE))
                {
                if (vu_getXYDistanceSquared (pCurr, vu_fsucc (pCurr)) < tol2)
                    {
                    vu_setMaskAroundEdge (pCurr, VU_BOUNDARY_EDGE);
                    }
                }
            }
        END_VU_SET_LOOP (pCurr, mpGraph)

        vu_freeNonMarkedEdges (mpGraph, VU_BOUNDARY_EDGE);
        vu_regularizeGraph (mpGraph);
        }
    MarkNonParityExterior (mpGraph);
    SetupForLoopOverInteriorFaces ();
...
// On exit, the face array is prepped for looping over all faces with GetFace ()
void ClassifyAIntersectB (STDVectorDPoint3dR xyzA, STDVectorDPoint3dR xyzB)
    {
...
// On exit, the face array is prepped for looping over all faces with GetFace ()
void ClassifyMultiUnion (STDVectorDPoint3dR xyzA)
    {
    int iNext = 0;
    vu_reinitializeVuSet (mpGraph);
...
// On exit, the face array is prepped for looping over all faces with GetFace ()
void ClassifyAMinusB (STDVectorDPoint3dR xyzA, STDVectorDPoint3dR xyzB)
    {
    SetupForPolygonBinaryOp (xyzA, xyzB);
...
// On exit, the face array is prepped for looping over all faces with GetFace ()
void ClassifyAUnionB (STDVectorDPoint3dR xyzA, STDVectorDPoint3dR xyzB)
    {
    SetupForPolygonBinaryOp (xyzA, xyzB);
...
bool GetFace (STDVectorDPoint3dR xyzOut, bvector <void *> * pEdgeDataArray)
    {
    xyzOut.clear ();
    if (pEdgeDataArray)
...
//          }
//  Once the faces are visited, use SetupForLoopOverInteriorFaces to reset for additional visits.
bool GetFace (STDVectorDPoint3dR xyz)
    {
