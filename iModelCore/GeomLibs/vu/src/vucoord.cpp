...
bool    assignAroundCompleteVertexLoop
    ...
    VU_FACE_LOOP (pCurrNode, pLoopStart)
        {
        double aCurr = vu_getIndexedXYZ (pCurrNode, indexA);
...
void VuNode::SetMask (VuMask mask){vu_setMask (this, mask);}
void VuNode::ClearMask(VuMask mask) { vu_clrMask(this, mask); }
void VuNode::ToggleMask(VuMask mask)
    {
    if (HasMask(mask))
        vu_clrMask (this, mask);
    else
...
void VuNode::SetMaskAroundEdge(VuMask mask)
    {
    vu_setMask (this, mask);
    vu_setMask (this->EdgeMate (), mask);
    }

void VuNode::ClearMaskAroundEdge(VuMask mask)
    {
    vu_clrMask(this, mask);
    vu_clrMask(this->EdgeMate(), mask);
    }

void VuNode::SetMaskAroundVertex (VuMask mask)
    {
    VU_VERTEX_LOOP (node, this)
        {
        vu_setMask(node, mask);
        }
    END_VU_VERTEX_LOOP (node, this)
    }

void VuNode::ClearMaskAroundVertex(VuMask mask)
    {
    VU_VERTEX_LOOP(node, this)
        {
        vu_clrMask(node, mask);
        }
    END_VU_VERTEX_LOOP(node, this)
    }

void VuNode::SetMaskAroundFace(VuMask mask)
    {
...
void VuNode::ClearMaskAroundFace(VuMask mask)
    {
    VU_FACE_LOOP(node, this)
        {
        vu_clrMask(node, mask);
...
//==   FindMaskAround_____
VuP VuNode::FindMaskAroundVertex(VuMask mask)
    {
    VU_VERTEX_LOOP(node, this)
        {
...
VuP VuNode::FindMaskAroundFace(VuMask mask)
    {
    VU_FACE_LOOP(node, this)
        {
        if (node->HasMask(mask))
