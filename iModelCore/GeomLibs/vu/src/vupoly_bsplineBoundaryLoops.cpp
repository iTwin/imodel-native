...
/*---------------------------------------------------------------------------------**//**
* Output polygons from a graph in which exterior edges are properly marked.
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
...
/*---------------------------------------------------------------------------------**//**
* Split vertex loops as needed so that no more than one outgoing edge has a given mask. Usual use: call with VU_EXTERIOR_MASK so that multiple
* polygons that share a vertx become disconnected. Returns the number of splits performed.
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
...
bool            isVerticalSeam      /* True for vertical seam (left and right),
    ...
    VU_SET_LOOP (high1P, graphP)
        {
        VuP low1P = vu_partnerOfSeedCandidate (graphP, high1P,lowParam, highParam, isVerticalSeam);
        if (low1P)
            {
...
/*---------------------------------------------------------------------------------**//**
* Collect parent-child polygons from a graph with full parity and and connectivity.
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
