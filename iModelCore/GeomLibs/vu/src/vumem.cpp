...
_VuSet::~_VuSet ()
    ...
    // This assert always returns true -- putting it as assert ensures it (and its possible print) does not appear in NDEBUG build ...
...
VuSetP          sourceGraphP
    ...
    */
    ...
    for (VuP sourceNodeP1 : sourceGraphP->mGraphStack)
        {
        destGraphP->mGraphStack.push_back ((VuP)vu_getInternalDataP (sourceNodeP1));
        }
