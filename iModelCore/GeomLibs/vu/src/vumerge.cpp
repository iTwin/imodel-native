...
**      For instance, presume a surface has a collection of unoriented,
...
/*---------------------------------------------------------------------------------**//**
* Depth first search to spread boundary parity to all faces reachable from seedP without crossing boundary edges.
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
...
/*---------------------------------------------------------------------------------**//**
* Depth first search to spread boundary parity to all faces reachable from seedP, including those across boundary edges (in which case the
* parity flips as the boundary is crossed).
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
...
bool    suppressTrivialFaces
    ...
    {
    // In test PolyfaceConstruction.NearlyColinearTriangulation, the area of what should be an exterior face is computed as
    // 5.6e-38. This causes the face to be classified as interior, which leads to an erroneous triangulation downstream.
