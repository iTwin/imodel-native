/*--------------------------------------------------------------------------------------+
|
|     $Source: geom/build/bsibasegeom/test/test15/mxspline.h $
|
...
public:
    ...
    void InitD1 (double slope) { Init (ECTypes::D1, slope, 0.0);}
    void InitD2 (double m) { Init (ECTypes::D2, 0.0, m);}
    void InitD12 (double slope, double m) { Init (ECTypes::D12, slope, m);}
...
private:
    ...
    bool FitWithArcLengthIterations (double *pX, double *pY, double *pT, int numXY, double relTol)
        {
        bool fitOK = false;
        if (ConvertEndConditions ())
            {
            double *pT = new double [numXY];
            ...
            for (int i = 1; i < numXY; i++)
                ...
                pT[i] = pT[i-1] + sqrt (dx * dx + dy * dy);
                }
            double tMax = pT[numXY-1];
            bool stat = false;

            for (int arcStep = 0; arcStep < mMaxArcSteps; arcStep++)
                {
                if (   !mxSpline.SetupAndSolve (pT, pX, numXY, ec1d[0][0], ec1d[1][0])
                    || !mySpline.SetupAndSolve (pT, pY, numXY, ec1d[0][1], ec1d[1][1]))
                    break;
                //mxSpline.PrintXYM (" XXXXXX ");
                //mySpline.PrintXYM (" YYYYYY ");
                double arcChange = UpdateArcLengths (pT, numXY);
...
public:
    ...
    bool FitWithArcLengthParameterization (double *pX, double *pY, int numXY, double relTol)
        ...
        if (ConvertEndConditions ())
            {
            double *pT = new double [numXY];
            fitOK = FitWithArcLengthIterations (pX, pY, pT, numXY, relTol);
            delete [] pT;
            }
        return fitOK;
        }
