...
double          tolerance           /* => max dist betw coincide pts or closed curve */
    ...
    if (closed == nullptr)
        return ERROR;
    if (inPts == nullptr || numPts < 2)
        return ERROR;

    outPts.resize(numPts + 1); // must allocate an extra slot in case we close it (below)
    if (inParams != nullptr)
        outParams.resize(numPts + 1);

    /* remove coincident data outPts (and corresponding outParams) within tolerance */
    if (remvData)
        {
        mdlBspline_removeCoincidentPoints (outPts.data(), inParams? outParams.data(): nullptr, &newNumPts, inPts, inParams, numPts, tolerance * tolerance, false, true);
        if (newNumPts > outPts.size())
            {
            BeAssert(false && "was not expecting mdlBspline_removeCoincidentoutPts to make the array longer. That probably corrupdated the heap!");
            return ERROR;
            }
        }
    else
        {
        newNumPts = numPts;
        std::copy(inPts, inPts + numPts, outPts.begin()); // NB: don't call outPts.assign here. We must preserve the extra slot at the end.
        if (inParams != nullptr)
            std::copy(inParams, inParams + numPts, outParams.begin());
        }
    ...
    BeAssert((outPts.size() == (numPts + 1)) && "outPts must still have the extra slot");
    BeAssert(((inParams == nullptr) || (outParams.size() == (numPts + 1))) && "outParams must still have the extra slot");
    BeAssert((newNumPts < numPts + 1) && "should have only done reductions up to here");
    ...
    if (*closed)
        ...
        if (endGap > tolerance)
            ...
            if (inParams != nullptr && newNumPts > 1)
    ...
    if (inParams != nullptr && SUCCESS != normalizeKnots(outParams.data(), newNumPts))
        return ERROR;
...
int             numPoints   /* => count last point which is the same as 1st point */
    ...
    BeStringUtilities::Memcpy (leftPtsP, num * sizeof(DPoint3d), dataPtsP, num*sizeof(DPoint3d));
    if (dataWtsP && outWtsP)
        BeStringUtilities::Memcpy (leftWtsP, num * sizeof(double), dataWtsP, num*sizeof(double));
...
bool            naturalTangents     /* => T/F: compute natural/bessel endTangent (zero/NULL endTangent, open spline) */
    ...
    /* solve closed interpolating cubic spline system */
        ...
        */
        ...
        BeStringUtilities::Memmove (outPts + 1, (bsplineParams->numPoles - 1) * sizeof (DPoint3d),
                                    outPts, (bsplineParams->numPoles - 1) * sizeof (DPoint3d));
        outPts[0] = tPole;
        if (outWts && weights)
            {
            double   tWt = weights[bsplineParams->numPoles - 1];
            BeStringUtilities::Memmove (outWts + 1, (bsplineParams->numPoles - 1) * sizeof (double),
                                        outWts, (bsplineParams->numPoles - 1) * sizeof (double));
            outWts[0] = tWt;
            }
        }
...
bool            naturalTangents     /* => T/F: compute natural/bessel endTangent (zero/NULL endTangent, open spline) */
    ...
    if (SUCCESS != (status = mdlBspline_c2CubicInterpolatePrepareFitPoints (points, paramsBuffer, &closedCurve, inPts, inParams, numPts, remvData, tolerance)))
        return status;

    int numPoints = (int)points.size();
    double* params = inParams? paramsBuffer.data(): nullptr;
    ...
    if (SUCCESS != (status = bspcurv_c2CubicInterpolatePolesExt (curve->GetPoleP (), NULL, curve->knots, params, points.data(), endTangents, NULL,
