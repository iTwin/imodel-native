...
void MSBsplineSurface::EvaluateUniformGrid (size_t numUPoint, size_t numVPoint, bvector<double> &uParams, bvector<double> &vParams, bvector<DPoint3d> &gridPoints) const
    {
    double uMax, uMin, vMax, vMin;
    GetKnotRange (uMin, uMax, BSSURF_U);
    GetKnotRange (vMin, vMax, BSSURF_V);
    uParams.clear ();
    vParams.clear ();
    DoubleOps::AppendInterpolated (uParams, uMin, uMax, numUPoint, true);
    DoubleOps::AppendInterpolated (vParams, vMin, vMax, numVPoint, true);
    bvector<DPoint2d> uvParams;
    for (size_t j = 0; j < numVPoint; j++)
        for (size_t i = 0; i < numUPoint; i++)
            {
            DPoint3d xyz;
            bspsurf_evaluateSurfacePoint (&xyz, NULL, NULL, NULL, uParams[i], vParams[j], this);
...
void MSBsplineSurface::EvaluateUniformGrid (size_t numUPoint, size_t numVPoint, bvector<DPoint2d> &uvParams, bvector<DPoint3d> &gridPoints) const
    ...
    GetKnotRange (uMin, uMax, BSSURF_U);
    GetKnotRange (vMin, vMax, BSSURF_V);
    bvector<double> uParams;
    bvector<double> vParams;
    uvParams.clear ();
    DoubleOps::AppendInterpolated (uParams, uMin, uMax, numUPoint, true);
    DoubleOps::AppendInterpolated (vParams, vMin, vMax, numVPoint, true);
    for (size_t j = 0; j < numVPoint; j++)
        for (size_t i = 0; i < numUPoint; i++)
            {
            DPoint3d xyz;
            uvParams.push_back (DPoint2d::From (uParams[i], vParams[j]));
            bspsurf_evaluateSurfacePoint (&xyz, NULL, NULL, NULL, uParams[i], vParams[j], this);
            gridPoints.push_back (xyz);
            }
    }
...
/*---------------------------------------------------------------------------------**//**
* @bsimethod                                    earlin.lutz                     08/2014
+---------------+---------------+---------------+---------------+---------------+------*/
bool    MSBsplineSurface::GetPrincipalExtents (TransformR frame) const
    {
    bvector<DPoint3d> poles;
    GetUnWeightedPoles (poles);
    return DPoint3dOps::PrincipalExtents (poles, frame);
    }
...
* @bsimethod                                    Peter.Yu                        04/2010
+---------------+---------------+---------------+---------------+---------------+------*/
MSBsplineStatus MSBsplineSurface::MakeBeziers (bvector<MSBsplineSurface>& beziers) const
    {
    ...
    int j = 0, num = vParams.numPoles - vParams.order + 1;
    BSurfPatch patch;
    ...
    while (j<num)
        {
        for (size_t i = 0; tmpSurf.GetPatch (patch, i, j); i ++)
            {
            if (!patch.isNullU && !patch.isNullV)
                {
                MSBsplineSurface surf;
                surf.Zero ();
                beziers.push_back (surf);
                cps.clear ();
                cws.clear ();

                if (rational)
                    {
                    for (size_t i = 0; i< patch.xyzw.size (); i++)
                        {
                        point.Init (patch.xyzw[i].x, patch.xyzw[i].y, patch.xyzw[i].z);
                        cps.push_back (point);
                        cws.push_back (patch.xyzw[i].w);
                        }
                    }
                else
                    {
                ...
                beziers.at(beziers.size () - 1).Populate (cps, rational ? &cws : NULL, NULL, uParams.order, uParams.order, false,
