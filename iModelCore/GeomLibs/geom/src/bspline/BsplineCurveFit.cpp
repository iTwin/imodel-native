...
bvector<DPoint3d>   P,              /* => input points */
bvector<double>     up,             /* => input params */
bvector<double>     uq,             /* => input params */
int                     p,              /* => desired degree of the pOut, 3 is recommended */
DVec3d                  *Ts,            /* => start tangent */
DVec3d                  *Te,            /* => end tangent */
...
double              tol             /* => geometric tolerance, this should be in general the chord height tol */
    ...
    while( i < mp )
        {
        while( i < mp - 1 &&  fabs (UP[i] - UP[i+1])< KNOT_TOLERANCE_BASIS )
            i++;
        if (i == mp - 2)  //Kevin.Nyman: Just putting this in place to get the prg build "ok" && fabs (UP[mp-1] - UP[mp-1])< KNOT_TOLERANCE_BASIS
            i++;
        ur = UP[i];
    ...
    if( keepTangent )
        ...
        pIn->FractionToPoint (xyz, Tangent, UP[mp-1]); // Kevin.Nyman: temp fix for PRG, not sure if mp should be 1 less but looking above I'm not sure.
        Te = Tangent;
...
bool                    bMaintainEndTangents
    ...
    if (pIn->params.numPoles < s_poleThreshholdForG1 && isCurveG1Continuous (pIn, s_coarseParallelTolerance))
        status = MSBsplineCurve::ApproximateG1Curve (this, &tmpCurve, order - 1, bMaintainEndTangents, parametrization, tolerance, 10.0 * tolerance, 0.01 * tolerance);
    
    status = MSBsplineCurve::ApproximateNurbsCurve (this, &tmpCurve, order - 1, bMaintainEndTangents, parametrization, tolerance);
...
int             order
    ...
    int i;
    bvector<double> u (numPoints), knots;
    ...
    if (endControl && sTangent != NULL && eTangent != NULL)
        {
        computeKnotVectorEnds (knots, u, order - 1);
        DVec3d Ts, Te;
        ...
        return InterpolationByPointsEndsParamsAndKnots (this, Q, u, knots, order, Ts, Te);
        }
    else
        {
        computeKnotVector (knots, u, order - 1);
        return InterpolationByPointsParamsAndKnots (this, Q, u, knots, order);
        }
    }
