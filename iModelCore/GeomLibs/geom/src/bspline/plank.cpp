...
int  numPoint
    ...
    ScopedArray<double> ffArray(2 * numInternalPoint); double *pff = ffArray.GetData ();
    ScopedArray<double> aaArray(2 * numInternalPoint * 6); double *pAA = aaArray.GetData ();
...
void *pSurfaceData
    ...
    if (numPoint < 1)
        return false;
    ScopedArray<PlankDerivatives> dArray(numPoint);    PlankDerivatives *pD = dArray.GetData();
    int i;
...
double yPeriod
    ...
    double globalFraction;
    ScopedArray<DPoint2d> deltaUVArray (numPoint);  DPoint2d* pDeltaUV = deltaUVArray.GetData ();
