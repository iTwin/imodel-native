...
int             numPoints               /* => number of Akima points */
    ...
    DPoint3d        p[6], m[5], t[2], *poleArray = NULL;
    BsplineParam    tParams;

    if (numPoints > MAX_VERTICES || numPoints < 6)
        return DGNHANDLERS_STATUS_BadElement;

    IndexedScopedArray<DPoint3d> pnts(numPoints);
    ...
    mdlBspline_removeCoincidentPoints (pnts.GetData(), NULL, &numVerts, points, NULL, numPoints, tol2, true, true);
