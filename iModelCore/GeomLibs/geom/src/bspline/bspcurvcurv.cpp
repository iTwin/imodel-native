...
bool            openAll                /* => forces opening */
    ...
    for (size_t i=0; i < numCurves; i++)
        ...
        if (!bezier)
            ...
            int numKnotsThisCurve = cvP->params.numPoles + cvP->params.order; /*OPEN*/
            ...
            if ( i == 0  ||  maxNumKnots < numKnotsThisCurve)
                maxNumKnots = numKnotsThisCurve;
            }
        }
    ...
    if (sameNum && sameKnots && sameMult)
        ...
        do
            ...
            if (sum == numCurves) /* been through all knots of all curves */
                ...
                size_t iDifferent;   // at end, this is the index where knots were different.
                for(iDifferent=0; iDifferent < numCurves-1; iDifferent++)
                    {
                    if (fabs (knots[tags[iDifferent]]-knots[tags[iDifferent+1]]) < knotTolerance)
                        avgKnot += knots[tags[iDifferent+1]];
                    else
                        break;
                    }
                size_t j = iDifferent;
