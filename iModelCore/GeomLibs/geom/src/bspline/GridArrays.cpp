...
static void SplitWeights (bvector<DPoint4d> const &xyzw, bvector<DPoint3d> &xyz, bvector<double> &w)
    ...
    xyz.resize (n);
    w.resize (n);
    for (size_t i = 0, n = xyzw.size(); i < n; i++)
        {
        xyz[i].x = xyzw[i].x;
        xyz[i].y = xyzw[i].y;
        xyz[i].z = xyzw[i].z;
...
bvector<CurveAndSolidLocationDetail> &intersections
    ...
    if (   TryGetXYZWUV01 (i, j, xyz00, w00, uv00a, true)
        ...
        if (quadRange.IntersectsWith (curveRange))
            ...
            for (size_t i = num0; i < intersections.size (); i++)
                {
                DPoint2d uvQuad = intersections[i].m_solidDetail.GetUV ();
                DPoint2d uvPatch = DPoint2d::FromInterpolateBilinear (
                    uv00a, uv10a, uv01a, uv11a, uvQuad.x, uvQuad.y);
                intersections[i].m_solidDetail.SetUV (uvPatch);
                }
            }
        return true;
        }
...
bool TryClosestPointByTriangle (size_t iGrid, size_t jGrid, size_t baseVertexSelect, DPoint3dCR spacePoint, DPoint2dR uv01Out, DPoint3dR xyzOut, DPoint3dR triangleUVBounded, DPoint3dR triangleUVUnbounded)
    ...
    DPoint2d uv[4];
    double   w[4];
    if (   TryGetXYZWUV01 (iGrid, jGrid, xyz[0], w[0], uv[0], true)
        && TryGetXYZWUV01 (iGrid+1, jGrid, xyz[1], w[1], uv[1], true)
        && TryGetXYZWUV01 (iGrid+1, jGrid+1, xyz[2], w[2], uv[2], true)
        && TryGetXYZWUV01 (iGrid, jGrid+1, xyz[3], w[3], uv[3], true)
        )
        {
        ...
        triangleXYZ[0] = xyz[i0];
        triangleXYZ[1] = xyz[i1];
        triangleXYZ[2] = xyz[i2];
