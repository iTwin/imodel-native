...
int             numPoints           /* => number of points (incl redundant endpt if closed) */
    ...
    Now we produce interpolants that, if exported to DWG fit-point based
...
int             dimension
    ...
    std::vector<DPoint3d> tmpPoles(numCols);
    std::for_each(tmpPoles.begin(), tmpPoles.end(), [](DPoint3d& pt) {pt.Zero();}); // DPoint3d does not have a constructor, so xyz are not automatically initialized to 0.0
    std::vector<double> product(numProduct);
    std::vector<double> auxTmpPoles(numCols * dimension);

    /* compute the product matrix and the right hand vector */
    for (j = 0, dP0=auxTmpPoles.data(); j < numCols; j++, dP0+=dimension)
        {
        jtmp = j * num + k;
        for (i = 0, dP1=auxCurvePoles; i < numRows; i++, dP1 += dimension)
            ...
            if ((uLeft[i] <= j) && (j <= uRight[i]))
                ...
                index = i * order;
                ltmp =jtmp - itmp;
                for (prodP=endP=product.data()+ltmp, mP=matrix+index,
    ...
    bspknotrm_systemSolver (gPolesP, auxGPolesP, numCols, order, k, k, product.data(),
                            tmpPoles.data(), auxTmpPoles.data(), dimension);
    ...
    for (i = 0; i < MAX_CHORD_ITER; i++)
        ...
        if (converge || i == MAX_CHORD_ITER - 1)
            ...
            for (int k = 0; k < numParams; k++)
                outparams.push_back (oldParams[k]);
            outparams.push_back (1.0);
...
size_t          numSeg
    ...
    {
    if (SUCCESS == computeEqualChordByNumber (points, outParams, this, numSeg, 0))
        return true;
    if (SUCCESS == computeEqualChordByNumber (points, outParams, this, numSeg, 1))
        return true;
    return false;
    }
...
int             numSeg     /* => number of chords: number of pointsP - 1 */
    ...
    if (numPoint < 1)
        return ERROR;
    ScopedArray <DPoint3d>pointBuffer (numFunc + 2);    DPoint3d   *pPointBuffer     = pointBuffer.GetData ();
    ScopedArray <double>paramBuffer (numFunc + 2);      double   *pParamBuffer     = paramBuffer.GetData ();
    ScopedArray <double>distanceBuffer (numFunc + 1);   double   *pDistanceBuffer     = distanceBuffer.GetData ();

    ScopedArray <double>leftDeriv (numFunc);        double   *pLeftDeriv     = leftDeriv.GetData ();
    ScopedArray <double>rightDeriv (numFunc);       double   *pRightDeriv     = rightDeriv.GetData ();
    ScopedArray <double>diag (numFunc);             double   *pDiag     = diag.GetData ();

    ScopedArray <double>upper (numFunc);             double   *pUpper     = upper.GetData ();
    ScopedArray <double>lower (numFunc);             double   *pLower     =lower.GetData ();
    ScopedArray <double>step (numFunc);             double   *pStep     = step.GetData ();
    ScopedArray <double>funcBuffer (numFunc);             double   *pFuncBuffer     = funcBuffer.GetData ();
