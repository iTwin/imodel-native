...
struct EqualChordsByLengthContext
{
    ...
    EmbeddedDPoint3dArray *mpXYZArray;
    EmbeddedDoubleArray   *mpParamArray;
...
EqualChordsByLengthContext (MSBsplineCurve *curveP, double chordLength)
    ...
    mpXYZArray = jmdlEmbeddedDPoint3dArray_grab ();
    mpParamArray = jmdlEmbeddedDoubleArray_grab ();

    mParamOld = 0.0;
    bspcurv_evaluateCurvePoint (&mXYZOld, NULL, mpCurve, mParamOld);
    jmdlEmbeddedDPoint3dArray_addDPoint3d (mpXYZArray, &mXYZOld);
    jmdlEmbeddedDoubleArray_addDouble (mpParamArray, mParamOld);
    }

~EqualChordsByLengthContext ()
    {
    jmdlEmbeddedDPoint3dArray_drop (mpXYZArray);
    jmdlEmbeddedDoubleArray_drop (mpParamArray);
    }
    ...
    for (bool bCurveAlive = true; bCurveAlive;)
        ...
        for (int i = 0; i < numRoots && !bNextPointFound; i++)
            ...
            if (u > uBase)
                ...
                jmdlEmbeddedDPoint3dArray_addDPoint3d (mpXYZArray, &xyz);
                jmdlEmbeddedDoubleArray_addDouble (mpParamArray, s);
...
(
DPoint3d        **pointsPP,     /* <= points at each chord (or NULL) */
double          **paramsPP,     /* <= parameters at each chord points (or NULL) */
int             *numChordsP,     /* <= number of equal length chords + 1 (size of output arrays) */
double          chordLength,    /* => chord length */
MSBsplineCurve  *curveP         /* => B-spline curve */
)
    {
    StatusInt status = SUCCESS;
    EqualChordsByLengthContext context (curveP, chordLength);

    BCurveSegment segment;
    bool handlerOK = true;
    for (size_t spanIndex = 0; handlerOK && curveP->GetBezier (segment, spanIndex); spanIndex++)
        {
        if (!segment.IsNullU ())
...
//    return SUCCESS;   // Why was this here?  context.ProcessBezier builds mp arrays, need to copy to flat arrays.
//    bspcurv_emitBezierSpans (curveP, &context);

    int numPoints = jmdlEmbeddedDPoint3dArray_getCount (context.mpXYZArray);

    if (numChordsP)
        *numChordsP = numPoints;

    if (numPoints > 0 && pointsPP)
        {
        int numBytes = numPoints * sizeof (DPoint3d);

        if (NULL != (*pointsPP = (DPoint3d*)BSIBaseGeom::Malloc (numBytes)))
            memcpy (*pointsPP, jmdlEmbeddedDPoint3dArray_getPtr (context.mpXYZArray, 0), numBytes);
        else
            status = ERROR;
        }

    if (SUCCESS == status && paramsPP)
        {
        int numBytes = numPoints * sizeof (double);
        if (NULL != (*paramsPP = (double*)BSIBaseGeom::Malloc (numBytes)))
            memcpy (*paramsPP, jmdlEmbeddedDoubleArray_getPtr (context.mpParamArray, 0), numBytes);
        else
            status = ERROR;
        }
    ...
    if (SUCCESS != status)
        {
        if (paramsPP && !*paramsPP)
            {
            BSIBaseGeom::Free (*paramsPP);
            *paramsPP = NULL;
            }
        if (pointsPP && !*pointsPP)
            {
            BSIBaseGeom::Free (*pointsPP);
            *pointsPP = NULL;
            }
        }
...
DPoint3d        **pointsPP,     /* <= points at each chord (or NULL) */
double          **paramsPP,     /* <= parameters at each chord points (or NULL) */
int             *numChords,     /* <= number of equal length chords + 1 (size of output arrays) */
double          chordLength,    /* => chord length */
MSBsplineCurve  *curveP,        /* => B-spline curve */
double          tolerance       /* => curve-curve intersection tolerance */
)
    {
    return bspcurv_computeEqualChordByLengthExt (pointsPP, paramsPP, numChords, chordLength, curveP);
    }
