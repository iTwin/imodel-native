...
InterpolationParam      *pParams
    ...
    if (SUCCESS == (status = bspcurv_allocateInterpolationCurve (pCurve)))
        ...
        BeStringUtilities::Memcpy (pCurve->fitPoints, pParams->numPoints * sizeof(*pPoints), pPoints, pParams->numPoints * sizeof(*pPoints));
        BeStringUtilities::Memcpy (pCurve->knots, pParams->numKnots * sizeof(*pKnots), pKnots, pParams->numKnots * sizeof(*pKnots));
...
bool            naturalTangents     /* => T/F: compute natural/bessel endTangent (zero/NULL endTangent, open spline) */
    ...
    if (SUCCESS != (status = mdlBspline_c2CubicInterpolatePrepareFitPoints (points, _params, &closedCurve, inPts, NULL, numPts, remvData, remvTol)))
       return status;

    numPts = (int)points.size();

    memset (&curve, 0, sizeof (MSBsplineCurve));
    if (SUCCESS == (status = bspcurv_c2CubicInterpolateCurveExt (&curve, points.data(), NULL, numPts, false, remvTol, endTangents, closedCurve,
        ...
        status = bspcurv_constructInterpolationWithKnots (pCurve, points.data(), pStartTan, pEndTan, curve.knots, &params);
...
double                  u          /* => currently only allows u=0.0 */
    ...
    open curve will not export to DWG, which does not allow for custom end
    tangent scaling.
    */
...
MSInterpolationCurve  *outCurve
    ...
    if (outCurve != inCurve)
        ...
        BeStringUtilities::Memcpy (points, inCurve->params.numPoints * sizeof(DPoint3d), inCurve->fitPoints, inCurve->params.numPoints * sizeof(DPoint3d));
        BeStringUtilities::Memcpy (knots, inCurve->params.numKnots * sizeof(double), inCurve->knots, inCurve->params.numKnots * sizeof(double));
        for (i = 0; i < inCurve->params.numPoints; i++)
            {
