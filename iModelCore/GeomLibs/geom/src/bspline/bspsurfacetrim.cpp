...
int baseCount
    ...
    //DPoint3d *pXYZBuffer = jmdlEmbeddedDPoint3dArray_getPtr (pXYZArray, baseCount - 1);
    ptrdiff_t count = pXYZArray->size ();
    ...
    for (i0 = 0; i0 < numTest; i0++)
        {
        xyz0 = pXYZArray->at (baseIndex + i0);
        xyz1 = pXYZArray->at (baseIndex + i0 + 1);
        xyz2 = pXYZArray->at (baseIndex + i0 + 2);
...
double param1                           // => end parameter on curve
    ...
    double curveFactor, surfaceFactor;
    if (curveTol <= 0.0)
    ...
    for (i = 1; i < minPoints; i++)
        ...
        for (;; surfaceError0 = surfaceError, curveError0 = curveError)
            ...
            curveError   = maxChordDeviationInArrayTail (&curvePoints, (int) baseCount);
...
const MSBsplineSurface *surface
    ...
    {
    DPoint3d xyzA = xyzAIn;
    DPoint3d xyzB = xyzBIn;
    xyzA.x = DoubleOps::ClampFraction (xyzAIn.x);
    xyzA.y = DoubleOps::ClampFraction (xyzAIn.y);

    xyzB.x = DoubleOps::ClampFraction (xyzBIn.x);
    xyzB.y = DoubleOps::ClampFraction (xyzBIn.y);

    StatusInt stat = ERROR;
    int baseCount = jmdlEmbeddedDPoint3dArray_getCount (pSurfacePoints);
    DPoint3d curvePoint0, surfacePoint0, curvePoint1, surfacePoint1;
    double baseStep, localStep;
    int i, j, numJ, newNumJ;
    bool    bAddBasePoint;
    ...
    if (bAddBasePoint)
        {
        pCurvePoints->push_back (curvePoint1);
        pSurfacePoints->push_back (surfacePoint1);
        curvePoint0 = curvePoint1;
        surfacePoint0 = surfacePoint1;
        }
    ...
    for (i = 1; i < minPoints; i++)
        {
        baseCount = jmdlEmbeddedDPoint3dArray_getCount (pSurfacePoints);
...
void AnalyzeSegment (DPoint3dCR xyz0, DPoint3dCR xyz1)
    ...
    AppendFractions (xyz0.x, xyz1.x, m_uBreaks);
    AppendFractions (xyz0.y, xyz1.y, m_vBreaks);
    ...
    if (pCurve->params.order == 2)
        ...
        for (int i0 = 0; SUCCESS == status && i0 < numSeg; i0++)
            ...
            for (size_t i = 0; breakContext.GetSubSegment (i, xyz0, xyz1); i++)
                {
                status = appendSegmentStrokes (pCurvePoints,
                        pSurfacePoints,
                        surfaceTol, 3,
                        xyz0, xyz1,
                        pSurface
        ...
        {
        return appendPCurveStrokes (*pCurvePoints, *pSurfacePoints, curveTol, surfaceTol, minPoints,
...
MSBsplineSurface    *pTempSurface
    ...
    int boundaryIndex;
    //int numError = 0;
    for (boundaryIndex = 0; boundaryIndex < pTempSurface->numBounds; boundaryIndex++)
        {
