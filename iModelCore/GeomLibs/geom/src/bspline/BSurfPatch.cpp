...
static bool RefinePatchClosestPoint_interiorOnly (BSurfPatchCR patch, DPoint3dCR spacePoint, double maxDelta01, SolidLocationDetailR detail)
    ...
    if (s_skip)
        return false;
    // newton tolerances... note that the newton will run until multiple convergences, so passing a loose tol
    //   still gets a further refinement.
    static double s_abstol = 1.0e-10;        // an abstol in parameter space.
    static double s_reltol = 1.0e-11;
    static double s_maxDelta = 0.25;
    static double s_edgeAdjustTol = 1.0e-2;     // allow pretty big motion (in uv space) towards edges.
    PatchProjectionFunction F (patch, spacePoint);
    NewtonIterationsRRToRR newton (s_abstol, s_reltol);
    ...
    if (newton.RunApproximateNewton (u, v, F, s_maxDelta, s_maxDelta))
        ...
        if (DoubleOps::IsIn01 (u, v))
            ...
            patch.Evaluate (DPoint2d::From (u1,v1), xyzB);
            dB = spacePoint.Distance (xyzB);
            if (dB < dA && fabs (u1 - u) < s_edgeAdjustTol && fabs (v1 - v) < s_edgeAdjustTol)
                {
                u = u1;
                v = v1;
                accept = true;
...
static bool ClosestPointOnSurface (MSBsplineSurfaceCR surface, DPoint3dCR spacePoint, SolidLocationDetailR detail)
    ...
    for (size_t iPatch = 0; iPatch < numU; iPatch++)
        ...
        for (size_t jPatch = 0; jPatch < numV; jPatch++)
            {
            if (surface.GetPatch (patch, iPatch, jPatch)
              && !patch.isNullU
              && !patch.isNullV
              )
                {
                ...
                if (s_timerSelect == 1)
                    surface.GetPatch (patch, iPatch, jPatch);
                ...
                if (s_timerSelect == 2)
                    grid.EvaluateBezier (0.0, 1.0, numUGrid, 0.0, 1.0, numVGrid, patch, false, 0);
                for (size_t i1 = 0; i1 + 1 < numUGrid; i1++)
                    {
                    for (size_t j1 = 0; j1 + 1 < numVGrid; j1++)
                        {
                        for (size_t t = 0; t <= 3; t += 2)
                            {
                            DPoint3d planeUVW;
                            if (grid.TryClosestPointByTriangle (i1, j1, t, spacePoint, quadParam, quadXYZ, triangleUVW, planeUVW))
                                {
                                if (s_timerSelect == 3)
                                    grid.TryClosestPointByTriangle (i1, j1, t, spacePoint, quadParam, quadXYZ, triangleUVW, planeUVW);
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
DRange3d  MSBsplineSurface::TrimmedSurfaceRange(TransformCP transform, double expansionFraction, bool restrictToPoleRange) const
    {
    ...
    if (nullptr != transform)
        GetPoleRange(poleRange, *transform);
    else
        GetPoleRange(poleRange);

    if (0 != GetNumBounds())
        {
        IFacetOptionsPtr options = IFacetOptions::CreateForSurfaces();
        static int s_minPerBezier = 3;
        options->SetMinPerBezier(s_minPerBezier);
        ...
        DRange3d meshRange;
        if (transform)
            meshRange = DRange3d::From(*transform, polyface.Point());
        else
            meshRange = DRange3d::From(polyface.Point());
        if (expansionFraction > 0.0)
            meshRange.ScaleAboutCenter(meshRange, 1.0 + expansionFraction);

        if (meshRange.IsNull())
            LOG.debug(L"B-spline surface trimmed range is null because its trim boundary has removed the entire surface.");

        return restrictToPoleRange ? DRange3d::FromIntersection(poleRange, meshRange, true) : meshRange;
        }

    DRange3d    rangeByBeziers = FullSurfaceRange( transform);
    if (expansionFraction > 0.0)
        rangeByBeziers.ScaleAboutCenter(rangeByBeziers, 1.0 + expansionFraction); // Pad range to make sure surface is fully inside range...

    return restrictToPoleRange ? DRange3d::FromIntersection(poleRange, rangeByBeziers, true) : rangeByBeziers;
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
DRange3d MSBsplineSurface::FullSurfaceRange (TransformCP transform) const
    {
    Transform worldToLocal = transform ? (*transform) : Transform::FromIdentity ();
    PatchRangeAccumulator localRangeAccumulator(worldToLocal);
    GridProcessor surfaceProcessor(*this);
    surfaceProcessor.Process(localRangeAccumulator);
    return localRangeAccumulator.GetRange();
    }
