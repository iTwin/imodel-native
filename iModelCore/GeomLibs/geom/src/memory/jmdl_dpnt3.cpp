...
|points.                                                                |
| As of July 2009, the EmbeddedDoubleArray is a typedef renaming of     |
| bvector<double>, and the functions here are wrappers around       |
| bvector<double> operations with the "negative index" semantics    |
| of Microstation omdlVArray.                                           |
|                                                                       |
...
int                 n
    ...
    if (pHeader)
        {
        return (SUCCESS == DPoint3dArrayWrapper::setBufferSize (pHeader, n));
        }
    else
        return false;
    }
...
const Transform  *pTransform
    ...
    if (pHeader &&
        ...
        bsiTransform_multiplyDPoint3dArray (pTransform, pPointArray, pPointArray, numPoint);
        }
    }
...
const Transform  *pTransform
    ...
    if (pSource && pDest &&
        ...
        pDestBuffer   = (DPoint3d *)DPoint3dArrayWrapper::getNewBlock
                    (pDest, numPoint);
        bsiTransform_multiplyDPoint3dArray (pTransform, pDestBuffer, pSourceBuffer, numPoint);
        }
    }
...
const RotMatrix *pMatrix
    ...
    if (pHeader &&
        ...
        {
        pPointArray = (DPoint3d *)DPoint3dArrayWrapper::getPtr (pHeader, 0);
        bsiRotMatrix_multiplyDPoint3dArray (pMatrix, pPointArray, pPointArray, numPoint);
        }
    }
