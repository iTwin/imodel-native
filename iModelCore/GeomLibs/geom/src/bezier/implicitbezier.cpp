...
double relTol
    ...
    if (order < 1)
        return 0;
    IndexedScopedArray<DPoint4d> pA1(order);
    IndexedScopedArray<DPoint4d> pA2(order);
    ...
    while (order > 2)
        ...
        bsiBezier_reduceDegreeDirectional ((double*)pA1.GetData(), (double*)pDest, order, 4, false);
        bsiBezier_reduceDegreeDirectional ((double*)pA2.GetData(), (double*)pDest, order, 4, true );
        order1 = order - 1;
...
const DPoint4d  *pB,
    ...
    ScopedArray<DPoint4d> A1_scopedArray (orderA); DPoint4d *pA1 = A1_scopedArray.GetData ();
    ScopedArray<DPoint4d> B1_scopedArray(orderB); DPoint4d *pB1 = B1_scopedArray.GetData();
...
const DConic4d  *pConic,
    ...
    if (nullCount == 1)
        ...
        for (int i = 0; i < numRoots; i++)
            ...
            double theta = Angle::Atan2 (xyw.y, xyw.x);
            if (extendConic || bsiDConic4d_angleInSweep (pConic, theta))
                {
                if (pPointA)
