...
const   DPoint4d    *pPoleArray,
    ...
    // Bug #792827: a small, straight Bezier should return only 1 edge
    if (maxEdgeLength <= 0.0 || (xyz[0].DistanceSquared(xyz[order - 1]) <= maxEdgeLength * maxEdgeLength))
        {
        if (PolylineOps::IsColinear(xyz, order))
            edgeCount = 1;
        }
...
(
    ...
    {
    if (!pWeight)
        bsiBezier_saturateKnotsInInterval((double*)pPoint, 3, pKnot, order, isNullInterval,
                saturateLeft, saturateRight);
    else
        {
        DPoint4d xyzw[MAX_BEZIER_ORDER];
        for (int i = 0; i < order; i++)
            {
            xyzw[i].x = pPoint[i].x;
            xyzw[i].y = pPoint[i].y;
        ...
        bsiBezier_saturateKnotsInInterval((double*)xyzw, 4, pKnot, order, isNullInterval,
            saturateLeft, saturateRight);
        for (int i = 0; i < order; i++)
            {
            pPoint[i].x = xyzw[i].x;
...
bool saturateRight
    ...
    static bool s_saturateAll = false;
    if (s_saturateAll || saturateLeft)
        {
        while (knot[0] < aLeft)
            {
    ...
    if (s_saturateAll || saturateRight)
        {
