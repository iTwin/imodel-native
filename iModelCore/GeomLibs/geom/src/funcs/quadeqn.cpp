...
#include <Geom/capi/quadeqn_capi.h>
#include <Geom/capi/bezroot_capi.h>
BEGIN_BENTLEY_GEOMETRY_NAMESPACE
/*------------------------------------------------------+
...
/*---------------------------------------------------------------------------------**//**
* Carry out the numerically sensitive steps in solving a quadratic. Called internally by quadratic solvers after discriminant is determined to
* be positive.
* This step is separated out so that it can be invoked from both regular quadratic solver (for ax^2 + bx + c=0) and symmetric solver a00*x0^2
* + a01*x0*x1 + a11*x1^2=0.
* (The latter case DEPENDS ON the fact that pXplus corresponds to the ADDED discriminant, pXminus subtracted, even if the are computed in the
* opposite order and with another formula.
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
