...
void AppendPoint(bvector<DPoint3d> &points, DPoint3dCR newPoint)
    {
    if (points.size() == 0
        || !DPoint3dOps::AlmostEqual(points.back(), newPoint))
        {
...
*/
void PolylineOps::SplitAtBowTies
(
    ...
    if (points.size() > 2)
        {
        double fractionI, fractionJ;
        DPoint3d pointI, pointJ;
        size_t iLast = points.size();
        // Main sweep
        primaryChain.push_back(points.front());
        for (size_t i0 = 0; i0 < iLast;)
            {
            size_t jLast = iLast - 1;
            if (DPoint3dOps::AlmostEqual(points[i0], points.back()))
                jLast--;
            size_t i1 = i0 + 2;
            if (i0 + 1 < iLast)
                {
                auto segmentA = DSegment3d::From(points[i0], points[i0 + 1]);
                for (; i1 < jLast; i1++)
                    {
                    auto segmentB = DSegment3d::From(points[i1], points[i1 + 1]);
                    if (DSegment3d::IntersectXY(fractionI, fractionJ, pointI, pointJ, segmentA, segmentB))
                        {
                        if (DoubleOps::IsAlmostIn01(fractionI) && DoubleOps::IsAlmostIn01(fractionJ))
                            {
                            AppendPoint(primaryChain, points[i0]);
                            AppendPoint(primaryChain, pointI);
                            bowTies.push_back(bvector<DPoint3d>());
                            AppendPoint(bowTies.back(), pointI);
                            AppendPoint(bowTies.back(), points[i0 + 1]);
                            for (size_t i = i0 + 1; i <= i1; i++)
                                AppendPoint(bowTies.back(), points[i]);
                            AppendPoint(bowTies.back(), pointJ);
                            i0 = i1;
                            break;
                            }
                        }
            ...
            if (i0 < i1)
                {
                // there were no hits from i0.
                if (i0 + 1 < points.size())
                    AppendPoint(primaryChain, points[i0 + 1]);
                i0++;
                }
            }
        }
...
*/
void PolylineOps::MultiPassSplitAtBowTies
(
bvector<DPoint3d> const &points,
bvector<DPoint3d> &correctedPoints,
bool favorLongestPart
)
    {
    bvector<DPoint3d> workPoints = points;
    bvector<bvector<DPoint3d>> bowties;
    for (int i = 0;; i++)
        {
        PolylineOps::SplitAtBowTies(workPoints, correctedPoints, bowties);
        if (bowties.size () == 0 || i > 5)
            return;
        workPoints = correctedPoints;
        if (favorLongestPart)
            {
            double d0 = PolylineOps::Length (correctedPoints);
            for (auto &lobe : bowties)
                {
                double d1 = PolylineOps::Length (lobe);
                if (d1 > d0)
                    {
                    workPoints.swap (lobe);
                    correctedPoints = workPoints;
                    d0 = d1;
                    }
                }
            }
