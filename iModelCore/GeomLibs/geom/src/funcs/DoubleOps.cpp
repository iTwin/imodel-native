...
double DoubleOps::LargestCoordinate(DPoint3dCR dataA, DPoint3dCR dataB)
    {
    return MaxAbs (dataA.x, dataA.y, dataA.z, dataB.x, dataB.y, dataB.z);
    }
...
void DoubleOps::MovingAverages(bvector<double> &dest, bvector<double> const &source, size_t blockSize, size_t numSkip0, size_t numSkip1)
    ...
    for (size_t i0 = numSkip0; i0 + blockSize + numSkip1 <= n; i0++)
        {
