...
DPoint3dCP pSweepDirection
    ...
    if (!worldToLocal.InverseOf (localToWorld))
        ...
        {
        ScopedArray<DPoint3d> XYZArray(numPoint + 1);    DPoint3d    *pXYZ= XYZArray.GetData();
...
int numB
    ...
    DVec3d zvecB;
     ...
     ScopedArray<DPoint3d > scoped_XYZA_localB((numA + 1) );   DPoint3d  *pXYZA_localB = scoped_XYZA_localB.GetData();
     ScopedArray<DPoint3d > scoped_XYZB_localB((numB + 1) );   DPoint3d  *pXYZB_localB = scoped_XYZB_localB.GetData();
     ScopedArray<DPoint3d > scoped_XYZB_world((numB + 1) );   DPoint3d  *pXYZB_world = scoped_XYZB_world.GetData();
...
int numB
    ...
    DVec3d zvecA, zvecB;
     ...
     ScopedArray<DPoint3d > scoped_XYZA_localA((numA + 1) );   DPoint3d  *pXYZA_localA = scoped_XYZA_localA.GetData();
     ScopedArray<DPoint3d > scoped_XYZB_localB((numB + 1) );   DPoint3d  *pXYZB_localB = scoped_XYZB_localB.GetData();

     ScopedArray<DPoint3d > scoped_XYZA_world((numA + 1) );   DPoint3d  *pXYZA_world = scoped_XYZA_world.GetData();
     ScopedArray<DPoint3d > scoped_XYZB_world((numB + 1) );   DPoint3d  *pXYZB_world = scoped_XYZB_world.GetData();

     ScopedArray<DPoint3d > scoped_XYZA_localB((numA + 1) );   DPoint3d  *pXYZA_localB = scoped_XYZA_localB.GetData();
     ScopedArray<DPoint3d > scoped_XYZB_localA((numB + 1) );   DPoint3d  *pXYZB_localA = scoped_XYZB_localA.GetData();
...
double*     onTolerance
    ...
    int         imin, imax, i, i0, i1, istart;
     ScopedArray<double     > scoped_Altitude(numXYZ );   double      *pAltitude = scoped_Altitude.GetData();
     ScopedArray<DPoint4d   > scoped_Crossing(numXYZ );   DPoint4d    *pCrossing = scoped_Crossing.GetData();
...
int             numXYZB
    ...
    if (numXYZA < 2 || numXYZB < 2)
        return false;
     ScopedArray<double > scoped_ParamA(numXYZA );   double  *pParamA = scoped_ParamA.GetData();
     ScopedArray<double > scoped_ParamB(numXYZB );   double  *pParamB = scoped_ParamB.GetData();
...
int numB
    ...
    *pPointB = pPolygonB[0];
    ...
    // Vertex projection
    ScopedArray<DPoint3d> localArrayA(numA);    DPoint3d    *pLocalA = localArrayA.GetData();
    ScopedArray<DPoint3d> localArrayB(numB);    DPoint3d    *pLocalB = localArrayB.GetData();
...
int numB
    ...
    DVec3d normalA;
    double area = bsiPolygon_polygonNormalAndArea(&normalA, nullptr, pPolygonA, numA);
    if (area <= 0.0)
        normalA = DVec3d::FromZero();   // vertex projection will be NOOP, proceed with closest approach by edges

    double minDistSquared = pointA.DistanceSquared (pointB);
...
double          tolerance
    ...
    if (numXYZ < 2)
        return false;
    ScopedArray<double> altitudeArray (numXYZ);    double    *pAltitude  = altitudeArray.GetData ();
