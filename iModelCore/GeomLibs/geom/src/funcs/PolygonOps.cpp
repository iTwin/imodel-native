...
size_t PolygonOps::SplitToConvexPartsXY(bvector<int>& indices, bvector<DPoint3d>& xyzOut, bvector<DPoint3d> const& xyzIn, double xyTol)
    {
    size_t numConvexFaces = 0;
    if (xyzIn.empty())
        return numConvexFaces;

    indices.clear();
    DPoint3dOps::Copy(&xyzOut, &xyzIn);
    ...
    if (xyTol < 0.0)
        ...
        if (!xyRange.IsNull())
            ...
            xyTol = s_relTol * xyRange.high.Distance(xyRange.low);
            }
        }

    VuSetP  graph = vu_newVuSet(0);
    VuMask  numberedNodeMask = vu_grabMask(graph);
    vu_clearMaskInSet(graph, numberedNodeMask);

    // add nodes with 0-based polygon vertex index as user data int
    VuMask newNodeMask = numberedNodeMask | VU_BOUNDARY_EDGE;
    if (!VuOps::MakeIndexedLoopsFromArrayWithDisconnects(graph, &xyzIn, newNodeMask, newNodeMask, xyTol))
        return numConvexFaces;
    ...
    vu_removeEdgesToExpandConvexInteriorFaces(graph);

    VuArrayP faces = vu_grabArray(graph);
    ...
    for (; vu_arrayRead(faces, &face); )
        ...
        VU_FACE_LOOP(curr, face);
            {
            int oneBasedVertexIndex;
            if (VuP originalNode = vu_findMaskAroundVertex(curr, numberedNodeMask))
                {
                oneBasedVertexIndex = 1 + vu_getUserDataPAsInt(originalNode);
                }
            else    // it's a new vertex generated by self-intersection
                {
                DPoint3d newXYZ;
                vu_getDPoint3d(&newXYZ, curr);
                xyzOut.push_back(newXYZ);
                oneBasedVertexIndex = (int) xyzOut.size();  // 1-based
                vu_setMask(curr, numberedNodeMask);
                vu_setUserDataPAsInt(curr, oneBasedVertexIndex - 1);
                }

            if (!vu_getMask(curr, newNodeMask))
                oneBasedVertexIndex = -oneBasedVertexIndex;

            indices.push_back(oneBasedVertexIndex);
            }
        END_VU_FACE_LOOP(curr, face);

        indices.push_back(0);  // face loop separator
        ++numConvexFaces;
        }

    vu_returnMask(graph, numberedNodeMask);
    vu_returnArray(graph, faces);
    vu_freeVuSet(graph);
    return numConvexFaces;
    }

void PolygonOps::CollectAllLoopsXY
(
bvector<bvector<DPoint3d>>&         outerLoops,
bvector<bvector<DPoint3d>>&         holeLoops,
bvector<bvector<DPoint3d>> const&   inputLoops,
double                              xyTol
)
    {
    outerLoops.clear();
    ...
    VuSetP graph = vu_newVuSet(0);
    vu_setTol(graph, absTol, relTol);
    vu_getTol(graph, &absTol, &relTol);
    for (auto const& loop : inputLoops)
        {
        if (auto node = vu_loopFromDPoint3dArrayXYTol(graph, loop.data(), (int) loop.size(), absTol, relTol))
            {
            node->SetMaskAroundFace(VU_BOUNDARY_EDGE);
            node->EdgeMate()->SetMaskAroundFace(VU_BOUNDARY_EDGE);
            }
        }

    vu_mergeOrUnionLoops(graph, VUUNION_UNION); // may produce null faces between two loops with adjacent edges
    vu_regularizeGraph(graph); // add unmarked bridge edges so everything is reachable via flood
    vu_markAlternatingExteriorBoundaries(graph, true); // mark neg area face EXTERIOR, then flood with parity flip across BOUNDARY to mark parity 0 holes EXTERIOR
    vu_freeEdgesByMaskCount(graph, VU_EXTERIOR_EDGE, false, false, true);  // remove dangling input edges
    vu_freeEdgesByMaskCount(graph, VU_BOUNDARY_EDGE, true, false, false);  // remove bridge edges
    ...
    VU_SET_LOOP(faceSeed, graph)
        ...
        if (!vu_getMask(faceSeed, visitMask))
            ...
            if (vu_faceLoopSize(faceSeed) > 1 && vu_getMask(faceSeed, VU_EXTERIOR_EDGE))
                {
                bvector<DPoint3d> loop;
                VU_REVERSE_FACE_LOOP(node, faceSeed) // record it backwards
                    {
                    DPoint3d xyz;
                    vu_getDPoint3d(&xyz, node);
                    loop.push_back(xyz);
                    }
                END_VU_REVERSE_FACE_LOOP(node, faceSeed)
                ...
                if (area > areaTol)
                    ...
                    outerLoops.push_back(loop); // exterior CW face becomes CCW outer loop
                else
                    ; // ignore tiny components/holes
                }
            }
        }
