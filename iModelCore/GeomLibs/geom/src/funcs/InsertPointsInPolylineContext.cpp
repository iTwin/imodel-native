/*--------------------------------------------------------------------------------------+
|
|     $Source: geom/src/funcs/InsertPointsInPolylineContext.cpp $
|
...
void InsertPointsInPolylineContext::InsertPointsInPolyline (bvector<DPoint3d> const &source, bvector<DPoint3d> &dest, bvector<DPoint3d> const &spacePoints)
    ...
    for (size_t i = 1; i < source.size (); i++)
        ...
        if (m_interiorPoints.size () > 0)
            ...
            for (size_t i = 1; i < m_interiorPoints.size (); i++)
                {
                if (!AlmostEqual (m_interiorPoints[i].xyz, dest.back ()))
                    dest.push_back (m_interiorPoints[i].xyz);
                }
            }
