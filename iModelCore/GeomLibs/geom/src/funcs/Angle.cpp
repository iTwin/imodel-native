...
bool Angle::IsNearZeroAllowPeriodShift(double radians, double radiansTol)
    {
    radians = fabs(radians);
    if (radians <= radiansTol)
        return true;
    if (radians < msGeomConst_2pi - radiansTol)
        return false;
    if (radians <= msGeomConst_2pi + radiansTol)
        return true;
    // radians is larger than 2pi.  Knock it down to [0..2pi).  Recursion will not reach this line again.
    return IsNearZeroAllowPeriodShift(fmod(radians, msGeomConst_2pi), radiansTol);
    }
...
/*--------------------------------------------------------------------------------**//**
* To be called when caller expects positiveDegrees is a decent positive angle in degrees
* but is suspcious (e.g. observed more than 720)
* Return 0.0 for several observed bad behaviors:
*  1) any negative angle.
*  2) std::isnan
*  3) normalization by subtracting a multiple of 360 fails to reduce the angle.
* After passing all those, return the normalization.
+--------------------------------------------------------------------------------------*/
static double ReduceSuspiciousPositiveDegrees(double positiveDegrees)
    {
    if (positiveDegrees < 0.0)
        return 0.0;
    if (isnan(positiveDegrees))
        return 0.0;
    double numWrap = floor(positiveDegrees / 360.0);
    double reducedDegrees = positiveDegrees - numWrap * 360.0;
    if (reducedDegrees < positiveDegrees)
...
static double CosPositive (double positiveDegrees)
    ...
    return CosPositive (ReduceSuspiciousPositiveDegrees (positiveDegrees));
    }
...
static double SinPositive (double positiveDegrees)
    ...
    return SinPositive(ReduceSuspiciousPositiveDegrees(positiveDegrees));
    }
