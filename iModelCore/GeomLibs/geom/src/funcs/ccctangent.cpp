        ...
        for (int k1 = 0; k1 < n1; k1++)
            ...
            for (int k2 = 0; k2 < n2; k2++)
                ...
                bsiGeom_circleTTT_colinear_append_rotated (pCenterArrayOut, pRadiusArrayOut,
                        pTangentAOut, pTangentBOut, pTangentCOut, *pNumOut, maxOut,
...
#define SQUARE(a) ((a) * (a))
    ...
    for (int k1 = 0; k1 < n1; k1++)
        ...
        for (int k2 = 0; k2 < n2; k2++)
            ...
            double qb = 2.0 * (a.x * b.x + a.y * b.y - r0);
            double qc = SQUARE (a.x) + SQUARE (a.y) - SQUARE (r0);
            double rr[2];
...
----------------------------------------------------------------------------------------*/
    ...
    if (normalM.IsParallelTo (normalN)
        ...
        double a = 0.5 * fabs (vectorAB.DotProductXY(normalN));
        ...
        double targetRadius[2] = {radiusC + a, radiusC - a};
        int numPass = 2;
        ...
        if (radiusC == 0.0)
            ...
            if (fabs (dotMA) < s_relTol * dMax)
                ...
                center.SumOf (centerC, normalN, dotNB > 0.0 ? a : -a);
                centerKnown = true;
                }
            else if (fabs (dotNB) < s_relTol * dMax)
                {
                // Tangency is on lineA.  Move towards line B...
                center.SumOf (centerC, normalM, dotMA > 0.0 ? a : -a);
                centerKnown = true;
                }
            if (centerKnown)
                {
