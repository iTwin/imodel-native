...
#include <Geom/capi/bezeval_capi.h>
...
/*---------------------------------------------------------------------------------**//**
* Estimate an integral by 5 point newton cotes rule, and estimate the absolute value integral by 4-part trapezoid rule.
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
static void nc5Quad
...
/*---------------------------------------------------------------------------------**//**
* Note This is an adaptive method and no value is evaluated twice during the calculation
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
...
/*---------------------------------------------------------------------------------**//**
* Invoke the "extend" function which reports addition of one lowest-level interval to the accepted integral.
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
...
/*---------------------------------------------------------------------------------**//**
* enterIntervalFunc (pGrid, pUserData) - test if need to enter the interval. Return true to enter, false to skip quietly. If no function, act
* as if true.
* evaluateFunc (pUserData, &f, x) -- evaluate function at x. No control flow changes possible -- must evaluate. integrateFunc (&f, &fa,
* pXArray, pFArray, n, pUserData) Evaluate integral and absolute integral (for tolerance scaling) over interval. Return true to continue,
* false to abort.
* pFinalNodeFunc (pUserData, fNode, fLeft, fRight) -- called with integral values where a node and its children are being accepted. return
* false to abort. pLeafFunc (pUserData, pXArray, pFArray, n, g) -- process grid at an accepted leaf. These calls are made in strict
...
/*---------------------------------------------------------------------------------**//**
* Integrate from u0 to u1, using polynomial approximations in binary subdivision of the interval x0 to x1.
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
