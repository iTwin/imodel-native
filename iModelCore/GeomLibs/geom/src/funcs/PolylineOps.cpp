...
bool PolylineOps::ClosestPointXY
    ...
    // i is the HEAD of a segment.  In closure case, initial edge is from (n-1) to 0
    for (size_t i = i0; i < n; i++)
        {
        DSegment3d segment;
        segment.point[0] = basePoint;
        ...
        if (distanceB < xyDistance)
            ...
            edgeFraction = paramB;
            edgeIndex    = i == 0 ? n -1 : i - 1;
            globalFraction = (edgeIndex + edgeFraction)*df;
            curvePoint = pointB;
            }
        basePoint = segment.point[1];
        }
    return true;
    }
...
double endFraction
    ...
    if (points.size () == 0)
        ...
        for (size_t iPoint = 1; iPoint < points.size (); iPoint++)
            {
            DSegment3d localSegment = DSegment3d::From (points[iPoint -1], points[iPoint]);
            ...
            if (edgeCount >= 2)
                ...
                for (size_t iEdge = 1; iEdge < edgeCount; iEdge++)
                    {
                    DPoint3d xyz;
                    xyz.SumOf (localSegment.point[0], extent, iEdge * df);
        ...
        for (size_t iPoint = 1; iPoint < newPoints.size (); iPoint++)
            {
            DSegment3d localSegment = DSegment3d::From (newPoints[iPoint -1], newPoints[iPoint]);
            size_t edgeCount = options.SegmentStrokeCount (localSegment);
            if (edgeCount > 2)
                {
                double df = 1.0 / (double) (edgeCount - 1);
                DVec3d extent = DVec3d::FromStartEnd (localSegment.point[0], localSegment.point[1]);
                for (size_t iEdge = 1; iEdge < edgeCount - 1; iEdge++)
                    {
                    DPoint3d xyz;
                    xyz.SumOf (localSegment.point[0], extent, iEdge * df);
                    strokes.push_back (xyz);
...
static bool IsColinearPointToEliminate (DPoint3dCR origin, DPoint3dCR pointA, DPoint3dCR pointB, double tol2, bool eliminateOverdraw, bool xyOnly)
    ...
    if (xyOnly)
        vectorA.z = 0.0;
    double aa = vectorA.MagnitudeSquared();
    if (aa < tol2)
        return true;
    ...
    if (dot >= aa) // pointB is "beyond" pointA ...
        {
        vectorB.GetPerpendicularParts (vectorA, fraction, parallelPart, perpendicularPart);
        return fraction < 1.0 && perpendicularPart.MagnitudeSquared () <= tol2;
        }
...
* @bsimethod
+--------------------------------------------------------------------------------------*/
void PolylineOps::CompressPairedColinearPoints
(
...
bool xyOnly
    ...
    if (n < 2 || n != pointsB.size ())
        return;

    double tol = absTol;
    if (absTol < 0.0)
        tol = DoubleOps::Max (
                DPoint3dOps::Tolerance(pointsA, 0.0, Angle::SmallAngle()),
                DPoint3dOps::Tolerance(pointsB, 0.0, Angle::SmallAngle()));
    double tol2 = tol * tol;
    DPoint3d lastAcceptedPointA = pointsA[0];
    DPoint3d lastAcceptedPointB = pointsB[0];
    size_t numAccepted = 1;
    // !! lastAcceptedPoint is always the last accepted coordinate (lastAcceptedPoint == points[numAccepted - 1])
    // look ahead to points j and k==j+1
    //  (j can get multiple steps ahead.  k is one beyond j)
    for (size_t j = 1; j < n;)
        ...
        if (k < n)
            {
            if (   IsColinearPointToEliminate(lastAcceptedPointA, pointsA[j], pointsA[k], tol2, eliminateOverdraw, xyOnly)
                && IsColinearPointToEliminate(lastAcceptedPointB, pointsB[j], pointsB[k], tol2, eliminateOverdraw, xyOnly)
                )
                {
                // and leave lastAcceptedPoint alone ...
                }
                ...
                {
                lastAcceptedPointA = pointsA[numAccepted] = pointsA[j];
                lastAcceptedPointB = pointsB[numAccepted++] = pointsB[j];
                }
            }
        j = k;
        }
    ...
    if (wrap)
        {
        double d0A = pointsA[n - 1].DistanceSquared(pointsA[0]);
        double d0B = pointsB[n - 1].DistanceSquared(pointsB[0]);
        if (d0A <= tol2 && d0B <= tol2 && numAccepted > 1
            && IsColinearPointToEliminate(lastAcceptedPointA, pointsA[0], pointsA[1], tol2, eliminateOverdraw, xyOnly)
            && IsColinearPointToEliminate(lastAcceptedPointB, pointsB[0], pointsB[1], tol2, eliminateOverdraw, xyOnly))
            {
            // move point[0] back to lastAcceptedPoint ...
            pointsA[0] = lastAcceptedPointA;
            pointsB[0] = lastAcceptedPointB;
            }
        else
            {
            pointsA[numAccepted] = pointsA[n - 1];
            pointsB[numAccepted++] = pointsB[n - 1];
            }
        }
    else
        {
        if (   pointsA[n - 1].DistanceSquared(lastAcceptedPointA) > tol2
            && pointsB[n - 1].DistanceSquared(lastAcceptedPointB) > tol2)
            {
            pointsA[numAccepted] = pointsA[n - 1];
            pointsB[numAccepted++] = pointsB[n - 1];
            }
        }
    pointsA.resize(numAccepted);
    pointsB.resize(numAccepted);
    }
...
bool PolylineOps::IsColinear(DPoint3dCP points, size_t numPoints, double absTol, double relTol)
    {
    if (numPoints < 2)
        return false;

    if (absTol <= 0.0) absTol = DoubleOps::SmallCoordinateRelTol();
    if (relTol <= 0.0) relTol = DoubleOps::SmallCoordinateRelTol();
    double tol = VectorOps<DPoint3d>::Tolerance(points, numPoints, absTol, relTol);
    double tol2 = tol * tol;

    DPoint3d p0 = points[0];
    DVec3d unitBaseLine = DVec3d::FromStartEnd(p0, points[numPoints - 1]);
    double baseLineLength = unitBaseLine.Normalize();

    if (baseLineLength <= tol)
        return false;

    // test interior points for colinearity with bounded baseLine segment
    for (size_t i = 1; i < numPoints - 1; ++i)
        {
        double projectionLength = points[i].DotDifference(p0, unitBaseLine);
        DPoint3d projection = DPoint3d::FromSumOf(p0, unitBaseLine, projectionLength);
        if (projection.DistanceSquared(points[i]) > tol2)
            return false;   // point is too far off baseline segment
        if (projectionLength < 0.0 && -projectionLength > tol)
            return false;   // projection before baseline start
        if (projectionLength > 0.0 && projectionLength - baseLineLength > tol)
            return false;   // projection after baseline end
        }

    return true;
    }

bool PolylineOps::IsColinear(bvector<DPoint3d> const& points, double absTol, double relTol)
    {
    return IsColinear(points.data(), points.size(), absTol, relTol);
    }
...
/// Test if pointD is "in" the circle through pointA, pointB, pointC.
/// "onisIn" affects points near on on the circle.
///
template <typename PointType>
bool InCircleGo(PointType const &pointA, PointType const &pointB, PointType const &pointC, PointType const &pointD, bool onIsIn, double &determinant, double &crossProduct)
    {
    double bx = pointB.x - pointA.x;
    double by = pointB.y - pointA.y;

    double cx = pointC.x - pointA.x;
    double cy = pointC.y - pointA.y;

    double dx = pointD.x - pointA.x;
    double dy = pointD.y - pointA.y;
    RotMatrix matrix;
    ...
    crossProduct = (bx * cy - by * cx);
    static double s_relTol = 1.0e-8;
    double a = determinant * crossProduct;
    double b = s_relTol * fabs(crossProduct * crossProduct * crossProduct);
    if (fabs(a) < fabs(b))
        {
        return onIsIn;
        }
    // simple "negative is in"
    return a < 0.0;
    }
template <typename PointType>
bool InCircle(PointType pointA, PointType pointB, PointType pointC, PointType pointD, bool onIsIn = true)
    {
    double q, c;
    return InCircleGo(pointA, pointB, pointC, pointD, onIsIn, q, c);
    }
...
bool DPoint3dOps::InCircleXY(DPoint3d const &pointA, DPoint3d const &pointB, DPoint3d const &pointC, DPoint3d const &pointD, bool onIsIn, double &determinant, double &refArea)
    {
    return InCircleGo(pointA, pointB, pointC, pointD, onIsIn, determinant, refArea);
    }
...
bool DPoint3dOps::InCircleXY(DPoint3d const &pointA, DPoint3d const &pointB, DPoint3d const &pointC, DPoint3d const &pointD, bool onIsIn)
    {
    double determinant, refArea;
    return InCircleGo(pointA, pointB, pointC, pointD, onIsIn, determinant, refArea);
    }
...
//! @param refArea [out] reference length squared quantity for zero tests.
//! @return true if pointD is inside the circle.
bool DPoint2dOps::InCircle(DPoint2d const &pointA, DPoint2d const &pointB, DPoint2d const &pointC, DPoint2d const &pointD, bool onIsIn, double &determinant, double &refArea)
    {
    return InCircleGo(pointA, pointB, pointC, pointD, onIsIn, determinant, refArea);
    }

//! Short form of InCircleXY.  All parameters match, but this short form does not ahve the determinant and cross product return values.
bool DPoint2dOps::InCircle(DPoint2d const &pointA, DPoint2d const &pointB, DPoint2d const &pointC, DPoint2d const &pointD, bool onIsIn)
    {
    double determinant, refArea;
    return InCircle(pointA, pointB, pointC, pointD, onIsIn, determinant, refArea);
    }
static void LoadResults(
    ...
    {
    if (triangleIndices)
        triangleIndices->push_back (Point3d::From (indexA, indexB, indexC));
    if (trianglePoints)
        trianglePoints->push_back(DTriangle3d (xyA, xyB, xyC));
    }
...
void PolylineOps::GreedyTriangulateBetweenLinestringsWithIndices
(bvector<DPoint2d> &pointA, bvector<int> &indexA,
bvector<DPoint2d> pointB, bvector<int> &indexB,
bvector<Point3d> *triangleIndices,
bvector<DTriangle3d> *trianglePoints
)
    {
    ...
    size_t iA = 0;
    size_t iB = 0;
    size_t nA = pointA.size();
    size_t nB = pointB.size();
    double signedHalo = 0.0;
    while (iA + 1 < nA || iB + 1 < nB)
        {
        int selector = 1;
        if (iA + 1 < nA && iB + 1 < nB)
            {
            auto distance = DPoint2dOps::SignedDistanceToCircleThrough3Points(
                pointA[iA], pointB[iB], pointA[iA + 1], pointB[iB + 1]);
            if (distance.IsValid())
                {
                // all "near" cases resolve to B side ... This is not an iterative flip so there are no termination consequences.
                if (distance.Value () > signedHalo)
                    selector = 1;
                else
                    selector = 2;
                }
        ...
        if (selector == 1)
            {
            LoadResults (pointA[iA], indexA[iA], pointB[iB], indexB[iB], pointA[iA+1], indexA[iA+1],
                triangleIndices, trianglePoints);
            iA++;
            }
        else if (selector == 2)
            {
            LoadResults(pointA[iA], indexA[iA], pointB[iB], indexB[iB], pointB[iB+1], indexB[iB + 1],
                triangleIndices, trianglePoints);
            iB++;
            }
        else
...
bool DPoint2dOps::CircleCenterVector(double ux, double uy, double vx, double vy, double &cx, double &cy)
    {
    DVec3d vectorU, vectorV, vectorA;
    vectorU.Init (ux, uy, ux*ux + uy*uy);
    vectorV.Init (vx, vy, vx*vx + vy*vy);
    vectorA.CrossProduct (vectorU, vectorV);
    if (DoubleOps::SafeDivide (cx, -0.5 *vectorA.x, vectorA.z, 0.0)
        && DoubleOps::SafeDivide(cy, -0.5 * vectorA.y, vectorA.z, 0.0))
        return true;
    cx = cy = 0.0;
    return false;
    }
bool DPoint2dOps::CircleCenterThrough3Points(DPoint2dCR pointA, DPoint2dCR pointB, DPoint2dCR pointC, DPoint2dR center)
    {
    double cx, cy;
    bool ok = CircleCenterVector (pointB.x - pointA.x, pointB.y - pointA.y, pointC.x - pointA.x, pointC.y - pointA.y, cx, cy);
    center.x = pointA.x + cx;
    center.y = pointA.y + cy;
    return ok;
    }

ValidatedDouble DPoint2dOps::SignedDistanceToCircleThrough3Points(DPoint2dCR pointA, DPoint2dCR pointB, DPoint2dCR pointC, DPoint2dR spacePoint)
    {
    DPoint2d center;
    if (!CircleCenterVector(pointB.x - pointA.x, pointB.y - pointA.y, pointC.x - pointA.x, pointC.y - pointA.y,
        center.x, center.y))
        return ValidatedDouble (0.0, false);
    center.x += pointA.x;
    center.y += pointA.y;
    return center.Distance (spacePoint) - center.Distance (pointA);
