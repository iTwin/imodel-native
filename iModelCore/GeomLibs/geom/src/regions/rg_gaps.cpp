            ...
            for (readIndex = 0; getNonNegativeBlock (pBlockIndexArray, &readIndex, &i0, &i1);)
                ...
                if (numVertex > 1)
                    ...
                    if (s_addMode == 1) /* Rectangle */
                        ...
                        int num00;
                        int num1;
                        xyz0.resize ((size_t)num0);
                        xyz1.resize ((size_t)(num0 + 1));

                        num00 = 0;
                        for (i = i0; i < i1;  i++)
                            {
                            jmdlEmbeddedIntArray_getInt (pBlockIndexArray, &iIndex, i);
                            jmdlEmbeddedDPoint3dArray_getDPoint3d (pNodeXYZArray, &xyz0[num00++], iIndex);
                            }

                        bsiDPoint3dArray_convexHullXY (&xyz1[0], &num1, &xyz0[0], num0);
                        if (num1 > 2)
                            {
                            xyz1[num1++] = xyz1[0];
                            }
    ...
    if (vertexSize > 0)
        ...
        */
        MTGARRAY_SET_LOOP (currNodeId, pGraph)
            {
            jmdlRG_getVertexData (pRG, NULL, 0, &currVertex, currNodeId, 0.0);
            ...
            jmdlEmbeddedIntArray_setInt (pVertexToNodeIdListArray, currNodeId, baseVertex);
            jmdlEmbeddedIntArray_setInt (pNodeIdListArray, baseNodeId, currNodeId);
            }
