/*--------------------------------------------------------------------------------------+
|
|     $Source: geom/src/CurvePrimitive/cp_offsetCurve.cpp $
|
...
bool IntegrateLength (double &length, double f0, double f1, RotMatrixCP matrix) const
    ...
    for (size_t i = 0; i < numStroke; i++)
        ...
        double f0 = i * df;
        double f1 = (i+1) * df;
        if (!DoubleOps::AlmostEqual (f0, f1))
            quadrature.AccumulateWeightedSums (integrand, f0, f1, sums, 1);
        }
    length = sums[0];
    return true;
    }
