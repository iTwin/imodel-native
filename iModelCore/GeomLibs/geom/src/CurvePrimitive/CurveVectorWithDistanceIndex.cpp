...
double tolerance
    ...
    PathLocationDetail detailB0 = pathAIntervals.empty () ? pathB.AtStart () : pathBIntervals.back ().DetailB ();
...
static bool ComputeSimpleOvelap (ICurvePrimitiveCR curveA, ICurvePrimitiveCR curveB, DSegment1dR intervalA, DSegment1dR intervalB, double abstol)
    ...
    if (curveA.TryGetArc (arcA) && curveB.TryGetArc (arcB))
        {
        DEllipse3d fullArcA = arcA; fullArcA.MakeFullSweep ();
        DEllipse3d fullArcB = arcB; fullArcB.MakeFullSweep ();
                fullArcB.start = fullArcA.start;    // remove sweep from comparison
        if (fullArcA.IsAlmostEqual(fullArcB, abstol))
            {
            // simple intervals for overlap...
            auto parentIntervalA = DSegment1d (arcA.start, arcA.start + arcA.sweep);
            auto parentIntervalB = DSegment1d(arcB.start, arcB.start + arcB.sweep);
            if (DSegment1d::NonZeroFractionalDirectedOverlap(parentIntervalA, parentIntervalB,
                intervalA, intervalB))
                {
                return intervalA.Length() >= DoubleOps::SmallCoordinateRelTol();
