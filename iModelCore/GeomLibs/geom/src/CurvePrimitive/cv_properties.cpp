    ...
    for (size_t i = 0, n = size (); i < n; i++)
        ...
        switch (curveType)
            ...
            case ICurvePrimitive::CURVE_PRIMITIVE_TYPE_LineString:
                ...
                for (size_t k = 0, nk = points->size (); k < nk; k++)
                    dotRange.Extend (points->at(k).DotDifference (ray.origin, ray.direction));
                break;
                }
...
void _ProcessBsplineCurve(ICurvePrimitiveCR curve, MSBsplineCurveCR bcurve, DSegment1dCP interval) override
    ...
    if (NULL == interval)
        {
        for (size_t iBezier = 0; bcurve.GetBezier (segment, iBezier); iBezier++)
            {
            if (!segment.IsNullU ())
                {
