...
void _ProcessArc(ICurvePrimitiveCR curve, DEllipse3dCR ellipse, DSegment1dCP interval) override
    ...
    for (size_t iInterval = 0; iInterval < num; iInterval++)
        {
        double theta0 = ellipse.start + iInterval * dTheta;
        double theta1 = ellipse.start + (iInterval + 1) * dTheta;
        for (int iGauss = 0; gaussRule.GetEval (iGauss, theta0, theta1, theta, w); iGauss++)
            {
