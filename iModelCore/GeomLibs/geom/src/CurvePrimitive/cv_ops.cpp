...
DPoint3dR farthestPoint
    ...
    {
    double d = basePoint.DistanceSquared (candidatePoint);
    if (d > maxDistance)
        {
        maxDistance = d;
...
(
bvector <CurveLocationDetailPair> const& curveIntersections,
DRay3dR sortRay
)
    {
    if (curveIntersections.empty())
        return false;

    DPoint3d point0 = curveIntersections[0].detailA.point;
    DPoint3d point1 = point0;
    // Find most distant point.
    // Vector to most distant point is at least half the total span.
    double dMax = 0.0;
    for (size_t i = 0; i < curveIntersections.size(); i++)
        {
        UpdateMaxDistance (point0, curveIntersections[i].detailA.point, dMax, point1);
        UpdateMaxDistance (point0, curveIntersections[i].detailB.point, dMax, point1);
        }
...
bvector <CurveLocationDetailPair> &segments
    ...
    if (!ComputeSortRay(curveIntersections, sortRay))
        return false;
    // Shuffle single points at start
    size_t i1 = CollectSinglePoints (curveIntersections);
    ComputeSortCoordinates (curveIntersections, sortRay);
    ...
    if (GetAnyFrenetFrame (localToWorld)
        ...
        if (!zeroX && zeroY && zeroZ)
            ...
            if (worldToLocal.InverseOf (localToWorld)
                ...
                {
                double maxAbs1 = range.LargestCoordinate ();
                double dz1 = range.high.z - range.low.z;
                return DoubleOps::AlmostEqual (maxAbs1, maxAbs1 + dz1);
                }
            }
        }
    return false;
