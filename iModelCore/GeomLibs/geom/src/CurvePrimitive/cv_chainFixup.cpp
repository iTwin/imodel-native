...
bool TryGetVertexIndexAndXYZ(MTGNodeId nodeId, size_t &index, DPoint3dR xyz)
    ...
    if (IsValidNodeId(nodeId))
        {
        int label;
        TryGetLabel(nodeId, m_vertexLabel, label);
        index = (size_t)label;
        xyz = m_xyz[index];
        return true;
        }
...
CurveVectorPtr CurveVector::AssembleChains ()
    ...
    for (size_t i = 0; i < allSeeds.size (); i++)
        ...
        if (!assembler.GetMaskAt (vertexSeedNodeId, visitMask)
            ...
            {
            MTGARRAY_VERTEX_LOOP (chainSeedNodeId, &assembler, vertexSeedNodeId)
                {
            ...
            MTGARRAY_END_VERTEX_LOOP(chainSeedNodeId, &assembler, vertexSeedNodeId)
            }
        }
...
void AppendPoint(ChainAssembler &assembler, MTGNodeId nodeId, bvector<DPoint3d> &points)
    {
    size_t index;
    DPoint3d xyz;
    if (assembler.TryGetVertexIndexAndXYZ(nodeId, index, xyz))
        {
        points.push_back (xyz);
        }
    }
void PolylineOps::AssemblePolylines(bvector<DSegment3d> const &segments, bvector<bvector<DPoint3d>> &linestrings)
    {
    linestrings.clear ();
    DRange3d range = DRange3d::NullRange ();
    for (auto &s : segments)
        {
        range.Extend (s.point[0]);
        range.Extend (s.point[1]);
        }
    static double s_relTol = 1.0e-8;
    static int s_print = 0;
    double absTol = s_relTol * range.low.Distance (range.high);
    ChainAssembler assembler(absTol);
    // Load the primitives with start end.
    // A lot happens (quietly!!!) during each insert ...
    for (size_t i = 0, n = segments.size (); i < n; i++)
        {
        assembler.AddEdge(i, segments[i].point[0], segments[i].point[1]);
        }
        ...
        if (!assembler.GetMaskAt(vertexSeedNodeId, visitMask)
            ...
            MTGARRAY_VERTEX_LOOP(chainSeedNodeId, &assembler, vertexSeedNodeId)
                ...
                if (!assembler.GetMaskAt(chainSeedNodeId, visitMask))
                    {
                    MTGNodeId currNodeId = chainSeedNodeId;
                    linestrings.push_back (bvector<DPoint3d> ());
                    bvector<DPoint3d> &activeLineString = linestrings.back ();
                    AppendPoint (assembler, currNodeId, activeLineString);
                    do
                        {
                        assembler.SetMaskAroundEdge(currNodeId, visitMask);
                        currNodeId = assembler.FSucc(currNodeId);
    ...
    for (size_t i = 0; i < allSeeds.size(); i++)
        ...
        if (!assembler.GetMaskAt(seedNodeId, visitMask))
            ...
            linestrings.push_back(bvector<DPoint3d>());
            bvector<DPoint3d> &activeLineString = linestrings.back();
            AppendPoint(assembler, seedNodeId, activeLineString);
            MTGARRAY_FACE_LOOP(currNodeId, &assembler, seedNodeId)
                {
                assembler.SetMaskAroundEdge(currNodeId, visitMask);
                AppendPoint(assembler, assembler.FSucc (currNodeId), activeLineString);
                }
            MTGARRAY_END_FACE_LOOP(currNodeId, &assembler, seedNodeId)
            }
        }
