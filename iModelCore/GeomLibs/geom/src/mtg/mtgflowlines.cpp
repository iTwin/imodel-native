...
struct MTGPositionDetail
    ...
    MTGPositionDetail EdgeMate (MTGGraphCP graph) const
        ...
        if (m_nodeId == MTG_NULL_NODEID)
            return result;
        result.m_nodeId = jmdlMTGGraph_getEdgeMate (graph, m_nodeId);
...
bool GetNormalAtNode (MTGNodeId nodeId, DVec3dR normal)
    ...
    if (jmdlMTGGraph_getMask (graph, nodeId, MTG_EXTERIOR_MASK))
        return false;
    MTGNodeId nodeIdA = jmdlMTGGraph_getFSucc (graph, nodeId);
    MTGNodeId nodeIdB = jmdlMTGGraph_getFPred (graph, nodeId);
...
MTGPositionDetail MinZAlongEdge (MTGNodeId tailNodeId)
    ...
    MTGNodeId headNodeId = jmdlMTGGraph_getFSucc (m_graph, tailNodeId);
...
MTGPositionDetail MinZAroundFaceOnSteepestDescent (MTGPositionDetailCR seedPos)
    ...
    MTGARRAY_FACE_LOOP (nodeIdA, m_graph, seedNodeId)
        {
        MTGNodeId nodeIdB = jmdlMTGGraph_getFSucc (m_graph, nodeIdA);
        jmdlMTGFacets_getNodeCoordinates (m_facets, &xyzA, nodeIdA);
        jmdlMTGFacets_getNodeCoordinates (m_facets, &xyzB, nodeIdB);
        ...
        if (hA * hB < 0.0)
            ...
            double f = -hA / (hB - hA);
            DPoint3d xyz;
            bsiDPoint3d_interpolate (&xyz, &xyzA, f, &xyzB);
...
void FollowFlowFromNodeAndPoint (MTGPositionDetailCR seedPos, EmbeddedDPoint3dArrayP path)
    {
    path->clear ();
...
bool FaceContainsPoint (MTGNodeId faceSeedNodeId, DPoint3dCR testXYZ, MTGPositionDetailR faceDetail)
    ...
    MTGGraphP graph = jmdlMTGFacets_getGraph (m_facets);
    MTGNodeId nodeIdA = jmdlMTGGraph_getFSucc (graph, faceSeedNodeId);
    MTGNodeId nodeIdB = jmdlMTGGraph_getFSucc (graph, nodeIdA);
    DPoint3d xyzOrigin, xyzA, xyzB;
    ...
    while (nodeIdB != faceSeedNodeId)
        ...
        if (bsiTrig_safeDivide (&lambdaA, areaOriginAQ, areaOriginAB, 0.0)
            && bsiTrig_safeDivide (&lambdaB, areaOriginQB, areaOriginAB, 0.0))
            {
        ...
        nodeIdA = nodeIdB;
        xyzA = xyzB;
        nodeIdB = jmdlMTGGraph_getFSucc (graph, nodeIdB);
        }
...
MTGPositionDetail FindContainingFace (DPoint3dCR seedXYZ)
    ...
    MTGMask visitMask = jmdlMTGGraph_grabMask (m_graph);
    jmdlMTGGraph_clearMaskInSet (m_graph, visitMask);
    MTGARRAY_SET_LOOP (seedNodeId, m_graph)
        {
        if (!jmdlMTGGraph_getMask (m_graph, seedNodeId, visitMask | MTG_EXTERIOR_MASK))
            {
            jmdlMTGGraph_setMaskAroundFace (m_graph, seedNodeId, visitMask);
...
cleanup:
    jmdlMTGGraph_dropMask (m_graph, visitMask);
    return faceDetail;
    }

public:
void CollectFlowPaths (DPoint3dCP strokePoints, int numPoints, EmbeddedDPoint3dArray *pFlowPoints)
    {
    EmbeddedDPoint3dArrayP pathPoints = jmdlEmbeddedDPoint3dArray_grab ();
    jmdlEmbeddedDPoint3dArray_empty (pFlowPoints);
    for (int i = 0; i < numPoints; i++)
        {
        MTGPositionDetail pierceData = FindContainingFace (strokePoints[i]);
        if (pierceData.HasNodeId ())
            {
            FollowFlowFromNodeAndPoint (pierceData, pathPoints);
            int numPathPoints = jmdlEmbeddedDPoint3dArray_getCount (pathPoints);
            if (numPathPoints > 1)
                {
                jmdlEmbeddedDPoint3dArray_addDPoint3dArray (pFlowPoints,
                            jmdlEmbeddedDPoint3dArray_getPtr (pathPoints, 0), numPathPoints);
                jmdlEmbeddedDPoint3dArray_addDisconnect (pFlowPoints);
                }
            }
        }
    jmdlEmbeddedDPoint3dArray_drop (pathPoints);
    }
...
EmbeddedDPoint3dArrayP flowLines
    ...
    {
    jmdlEmbeddedDPoint3dArray_empty (flowLines);
    if (NULL != facets)
        {
        FlowLinesContext context (facets);
        context.CollectFlowPaths (points, numPoints, flowLines);
        context.GrabFacets ();
        }
    }
