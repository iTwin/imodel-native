...
double          relTol
    ...
    {
    ScopedArray<DPoint3d> XYZCC_scopedArray (numXYZ);  DPoint3d* pXYZCC = XYZCC_scopedArray.GetData ();

    MTGNodeId   baseNodeId = MTG_NULL_NODEID, partnerNodeId;

    bsiPolygon_compressDuplicateVertices (pXYZCC, NULL, NULL, &numXYZ, pXYZArray, NULL, numXYZ, absTol, relTol);
...
void MTGFacets::ClearGraphAndArrays()
    {
    vertexArrayHdr.clear();
    normalArrayHdr.clear();
    param1ArrayHdr.clear();
    param2ArrayHdr.clear();
    graphHdr.ClearNodes(true);
    }
/*---------------------------------------------------------------------------------**//**
* 
+---------------+---------------+---------------+---------------+---------------+------*/
