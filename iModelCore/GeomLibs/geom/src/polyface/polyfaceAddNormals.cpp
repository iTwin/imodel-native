...
bvector<SectorData> &offsetVectors
    ...
    MTGMask myVisitMask = graph->GrabMask ();
    graph->ClearMask (myVisitMask);
    ...
    MTGARRAY_SET_LOOP (vertexSeed, graph)
        {
        if (!graph->GetMaskAt (vertexSeed, myVisitMask))
            {
            graph->SetMaskAroundVertex (vertexSeed, myVisitMask);
            sectors.clear ();
            baseSectors.clear ();
...
bvector<SectorData> &offsetVectors
    ...
    MTGARRAY_SET_LOOP (faceSeed, graph)
        {
        if (!graph->GetMaskAt (faceSeed, myVisitMask))
            {
            graph->SetMaskAroundFace (faceSeed, myVisitMask);
    ...
    MTGARRAY_SET_LOOP (nodeA, graph)
        {
        if (!graph->GetMaskAt (nodeA, myVisitMask))
            {
            graph->SetMaskAroundEdge (nodeA, myVisitMask);
            xyz.clear ();
            MTGNodeId nodeB = graph->FSucc (nodeA);
            MTGNodeId nodeC = graph->VSucc (nodeB);
            MTGNodeId nodeD = graph->FSucc (nodeC);
