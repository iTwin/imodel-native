...
size_t MapZeroBasedPointIndex(size_t oldIndex, bvector<size_t> &oldToNew, bvector<DPoint3d> &oldXYZ, bvector<DPoint3d> &newXYZ)
    {
    ...
    if (oldToNew[oldIndex] < newXYZ.size ())
...
+---------------+---------------+---------------+---------------+------*/
PolyfaceHeaderPtr MTGFacetsSubsetToIndexedPolyface
(
MTGFacets*                      pFacets,
MTGMask                     exclusionMask,
MTGMask                     visibleEdgeMask,
bvector<MTGNodeId> const &  nodes
    ...
    bvector<size_t> oldPointIndexToNewPointIndex;
    int vertexLabelOffset = pFacets->vertexLabelOffset;
    size_t facetCount = 0;
    int maxMTGVertexIndex = -1;
    MTGMask skipMask = visitMask | exclusionMask;
