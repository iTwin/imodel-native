...
TaggedPolygonVector* debugPolygons
    ...
    for (size_t polygonIndex = 0; polygonIndex < punchPolygons.size (); polygonIndex++)
        {
        bvector<DPoint3d> const &points = punchPolygons[polygonIndex].GetPointsCR ();
        ...
        if (points.size () > 2)
            ...
            if (direction > 0)
                ...
                DPoint3dOps::ConvexHullXY (points, hullPoints);
                }
                ...
                for (size_t k = 0; k < clipper.m_clipChain.size () && !shards[shard0].empty (); k++)
                    ...
                    while (!shards[shard0].empty ())
                        ...
                        if (debugPolygons != nullptr)
                            ...
                            for (size_t k1 = count1; k1 <shards[shard1].size (); k1++)
                                AddTransformedPolygon (*debugPolygons, shards[shard1][k1], numDebugStep * debugStep, 3);
                            }
