...
bool _AdvanceToNextFace () override
    ...
    uint32_t i1 = i0 + m_numPerFaceInParent;    // one past current face...
    m_numEdgesThisFace = m_numPerFaceInParent;
    if (i1 <= m_parentMesh.GetPointCount ())    // ...therefore this is OK
        {
        ...
        for (size_t j = 0; j < m_numWrap; j++)
            {
            auto jA = j % m_numEdgesThisFace;       // prevent index overflow if m_numWrap > m_numEdgesThisFace
            m_visible.push_back (true);
            m_indexPosition.push_back (i0 + jA);
            }
...
bool _AdvanceToNextFace () override
    ...
    if (m_numEdgesThisFace > 0)
        {
        auto numSoFar = m_numEdgesThisFace;
        for (uint32_t i = 0; i < m_numWrap; i++)
            {
            auto iA = i % numSoFar;     // prevent index overflow if m_numWrap > numSoFar
            indices[m_numEdgesThisFace++] = indices[iA];
            }
        }
...
bool _AdvanceToNextFace () override
    ...
    if (DelimitIndicesInFace (m_nextReadIndex, m_numEdgesThisFace, i0, i2))
        ...
        for (size_t i1 = 0; i1 < m_numWrap; i1++)
            {
            auto i1A = i1 % m_numEdgesThisFace;     // prevent index overflow if m_numWrap > m_numEdgesThisFace
            m_visible.push_back (m_parentMesh.GetPointIndexCP()[m_nextReadIndex + i1A] > 0);
            m_indexPosition.push_back (m_nextReadIndex + i1A);
            }

        if (m_allData)
            {
            // normals and params default to point index if needed
...
* @bsimethod
+--------------------------------------------------------------------------------------*/
void PolyfaceVisitor::SetNumWrap (uint32_t numWrap)
    {
    if (numWrap > MAX_WRAP)
        numWrap = MAX_WRAP;
    m_numWrap = numWrap;
...
* @bsimethod
+--------------------------------------------------------------------------------------*/
PolyfaceVisitor::PolyfaceVisitor(uint32_t numWrap)
    {
    SetNumWrap(numWrap);
    }
