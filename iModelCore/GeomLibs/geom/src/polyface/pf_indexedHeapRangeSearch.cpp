...
void ComputeDiagonalProperties(RangeIndexingContext const &context, bvector<DPoint3d> &data, int numDimensions)
    {
    data.clear();
    if (context.m_ranges.size() == 0)
        return;
    bvector<InclusiveIndexBlock> stack;
    stack.push_back(InclusiveIndexBlock(0, context.m_ranges.size() - 1, 0));
    while (stack.size() > 0)
        {
        InclusiveIndexBlock window = stack.back();
        stack.pop_back();
        DRange3d range;
        if (context.GetRange(window.m_i0, window.m_i1, range))
            {
...
size_t PolyfaceIndexedHeapRangeTree::LoadPolyface (PolyfaceQueryCR polyface, bool sortX, bool sortY, bool sortZ)
    ...
    if (sortX || sortY || sortZ)
        ...
        ComputeDiagonalProperties (rangeBuilderMap, sum0, 2);
#endif
        ApplySortOrder (rangeBuilderMap.m_ranges, m_heapIndexToReadIndex, sortOrder);
#ifndef NDEBUG
        ComputeDiagonalProperties(rangeBuilderMap, sum1, 2);
#endif
        }
    if (n > 0)
