...
void ensureZeroTerminated(bvector<int> &data)
    {
...
bool PolyfaceHeader::ConvertToVariableSizeSignedOneBasedIndexedFaceLoops ()
    ...
    switch (m_meshStyle)
        ...
        case MESH_ELM_STYLE_INDEXED_FACE_LOOPS:
            ...
            stat = true;
            ensureZeroTerminated (m_pointIndex);
            ensureZeroTerminated(m_normalIndex);
            ensureZeroTerminated(m_paramIndex);
            ensureZeroTerminated(m_colorIndex);
            ensureZeroTerminated(m_faceIndex);
            break;
...
bool PolyfaceHeader::Triangulate (size_t maxEdge, bool hideNewEdges, IPolyfaceVisitorFilter *tester)
    ...
    for (visitor->Reset (); visitor->AdvanceToNextFace (); )
        ...
        if (facePoints.size () < 4) // 4th point is closure -- anything less is degenerate, ignored.
            continue;
...
void PolyfaceHeader::CopyFrom (PolyfaceQueryCR source)
    ...
    SetExpectedClosure(source.GetExpectedClosure());
    ...
    auto numericDataP = source.GetNumericTagsCP();
    if (numericDataP)
        SetNumericTags (*numericDataP);
    }
...
//        return false;
    ...
    for (int &index1 : indices) // index1 will be modified in place!!!
        ...
        if (index1 != 0)
            ...
            if (index0 < oldDataToNewData.size ())
                {
                ...
                {
                // The upper loop should have replaced bad indices with zero ... this should never be reached
                errors++;
                }
            }
        }
...
* @bsimethod
+--------------------------------------------------------------------------------------*/
void PolyfaceHeader::Compress(double pointAbsTol, double normalAbsTol, double paramAbsTol)
    {
    if (GetMeshStyle() == MESH_ELM_STYLE_INDEXED_FACE_LOOPS)
        {
        ...
        DPoint3dOps::Cluster (m_point, &xyzPack, oldIndexToNewIndex, pointAbsTol);
        ...
        if (Param().Active () && ParamIndex ().Active ())
            ...
            DPoint2dOps::Cluster (m_param, &paramPack, oldIndexToNewIndex, paramAbsTol);
        ...
        if (Normal().Active () && NormalIndex ().Active ())
            ...
            DVec3dOps::Cluster (m_normal, &packedNormals, oldIndexToNewIndex, normalAbsTol);
