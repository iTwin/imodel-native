...
bool IPolyfaceConstruction::AddTriangulation (bvector <DPoint3d> const &originalInPoints)
    ...
    if (buildSimpleIndices && convexRequired && !bsiGeom_testPolygonConvex(points.data(), numPoints))
    ...
    else if (SUCCESS != vu_triangulateProjectedPolygon (&loopIndex, NULL, &outPoints, &localToWorld, &worldToLocal, points.data(), numPoints, 0.0, true, maxPerFace))
        {
        return false;
        }
...
PolyfaceHeaderPtr PolyfaceHeader::CreateXYTriangulation (bvector <DPoint3d> const &points, double fringeExpansionFactor, bool retainFringeTriangles, bool convexHull)
    ...
    if (convexHull)
        {
        DPoint3dOps::ConvexHullXY (localPointB, hullPoints);
...
bvector<DPoint3d> const points
    ...
    if (s_boundaryMode == 0)
        ...
        DPoint3dOps::ConvexHullXY(localPoints, xyzHull);
            ...
            if (userData1 >= 0)
                ...
                VU_VERTEX_LOOP (outboundEdge, vertexSeed)
                    ...
                    if (indexA < points.size () && indexB < points.size ())
                        ...
                        if (plane.IsValid ())
                            {
                            DPlane3d plane1a = plane.Value ();
                            plane1a.normal = s_sign * plane1a.normal;
                            planes.push_back (ClipPlane (plane1a, false, s_interior));
                            }
...
// ZERO BASED corner indices for sides of a box with x-then-y-then-z indexing
...
static void AddZeroBasedQuadIndices(PolyfaceHeaderPtr &mesh, int zeroBasedIndices[], bool triangulate, bool reverse)
    {
    if (reverse)
        {
        int reversedIndices[4] = {
            zeroBasedIndices[0],
            zeroBasedIndices[3],
            zeroBasedIndices[2],
            zeroBasedIndices[1]};
        AddZeroBasedQuadIndices(mesh, reversedIndices, triangulate, false);
        return;
        }
    if (triangulate)
        {
        mesh->PointIndex().push_back(1 + zeroBasedIndices[0]);
        mesh->PointIndex().push_back(1 + zeroBasedIndices[1]);
        mesh->PointIndex().push_back(-(1 + zeroBasedIndices[2]));
        mesh->PointIndex().push_back(0);
        mesh->PointIndex().push_back(1 + zeroBasedIndices[2]);
        mesh->PointIndex().push_back(1 + zeroBasedIndices[3]);
        mesh->PointIndex().push_back(-(1 + zeroBasedIndices[0]));
        mesh->PointIndex().push_back(0);
        }
    else
...
TransformCP placement
    ...
    if (placement)
        ...
        if (placement->Determinant () < 0.0)
            reverse = true;
        placement->Multiply (corners, 8);
        }
    auto mesh = PolyfaceHeader::CreateVariableSizeIndexed ();
    for (int i = 0; i < 8; i++)
        mesh->Point ().push_back (corners[i]);
    for (uint32_t faceIndex = 0; faceIndex < 6; faceIndex++)
        AddZeroBasedQuadIndices (mesh, s_cornerIndexCCW[faceIndex], triangulate, reverse);
    return mesh;
    }
