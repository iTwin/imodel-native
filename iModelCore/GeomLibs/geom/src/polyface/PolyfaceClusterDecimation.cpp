...
PolyfaceHeaderPtr   PolyfaceQuery::ClusteredVertexDecimate (double tolerance, double minCompressionRatio, bool doNormals) const
    ...
    doNormals = doNormals && (nullptr != normals);
    bool doParams = (nullptr != params);
    bool doAux = auxData.IsValid();
    ...
    bmap<int32_t, DPoint2d>         inputPointIndexToParam;
    ...
    for (PolyfaceVisitorPtr visitor = PolyfaceVisitor::Attach (*this); visitor->AdvanceToNextFace(); )
        ...
        for (size_t i=0, count = visitor->NumEdgesThisFace(); i<count; i++)
            ...
            if (doParams)
                {
                auto    param = params[inputParamIndex];
                auto foundParam = inputPointIndexToParam.find(inputPointIndex);
                if (foundParam == inputPointIndexToParam.end()) {
                    inputPointIndexToParam.Insert(inputPointIndex, param);
                    }
                else
                    {
