...
bool PolyfaceConstruction::AddPolyface_matched (PolyfaceQueryCR source)
    ...
    // mutate the twoSided and closure summaries ...
    if (0 == m_polyfacePtr->GetPointCount ())
        {
        // On first added mesh, copy twoSided and expectedClosure from source ...
        ...
        int sourceClosure = source.GetExpectedClosure();
        int builderClosure = m_polyfacePtr->GetExpectedClosure ();
        m_polyfacePtr->SetExpectedClosure (std::min (builderClosure, sourceClosure));
        }
