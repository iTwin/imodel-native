...
void IPolyfaceConstruction::AddFullDiskTriangles (DEllipse3dCR ellipse, size_t numPerQuadrant)
    ...
    if (numRadius > 1)
        ...
        for (size_t i = 2; i <= numRadius; i++)
            ...
            size_t numChordB1 = ProportionalChordCount (numChord, i, numRadius);
            trigPointB.clear ();

            AppendFullEllipseStrokes (ellipse, numChordB1, nullptr, nullptr, &trigPointB, i * radiusFraction);
            trigPointBXYZ.clear ();
            DPoint3dOps::AppendXY0 (trigPointBXYZ, trigPointB);
    ...
    if (boundaryType == CurveVector::BOUNDARY_TYPE_Open)
        ...
        if (CollectChildAreaNormals (*this, *contours[0], normals, maxNormalIndex))
            ...
            for (size_t i = 0; i < numChildren; i++)
                ...
                stat &= context.AddRuledBetweenCorrespondingChains (oneContourSet, currentLoopReversal);
                }
            if (capped)
...
bvector<double> &curveLengths
    ...
    for (size_t iCurve = 0; iCurve < curves.size (); iCurve++)
        {
        ICurvePrimitivePtr primitive = curves.at(iCurve);
        if (NULL != (child = primitive->GetChildCurveVectorCP ()))
            ...
            if (primitive->TryGetLine (segment))
                ...
                for (size_t i = 1, n = linestring->size (); i < n; i++)
                    {
                    DSegment3d segment1;
                    segment1.point[0] = linestring->at(i-1);
                    segment1.point[1] = linestring->at(i);
                    StrokeByUniformFractions<DSegment3d> (segment1,
                        options.SegmentStrokeCount (segment1),
                    points.back (), tangents.back ());
                    curveLengths.back () += segment1.Length ();
                    }
                }
        ...
        if (NULL != (child = primitive->GetChildCurveVectorCP ()))
            ...
            if (primitive->TryGetLine (segment))
                ...
                for (size_t i = 1; i < n; i++)
                    ...
                    segment1.point[0] = linestring->at(i-1);
                    segment1.point[1] = linestring->at(i);
                    StrokeByUniformFractions<DSegment3d> (primitive.get (), segment1, (i - 1) * df, i * df,
                        options.SegmentStrokeCount (segment1), pointTangentCurve.back ());
                    curveLength.back () += segment1.Length ();
                    }
                }
            else if (primitive->TryGetArc (ellipse))
                {
