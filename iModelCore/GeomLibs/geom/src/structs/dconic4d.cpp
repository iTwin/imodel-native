...
DConic4dP pConic
    ...
    if (!bsiDConic4d_isFullSweep (pConic))
        pConic->sweep = Angle::ReverseComplement(pConic->sweep);
    }
...
double      theta
    ...
    if (nullptr == pX)
        {
        BeAssert(false && "NEEDS WORK STATIC ANALYSIS - This function does nothing other than assign to pX");
        return;
        }
...
int        clipType
    ...
    */
    ...
    ScopedArray<DPoint3d> criticalAngle_scopedArray(2 * nPlane + 4); DPoint3d *pCriticalAngleArray = criticalAngle_scopedArray.GetData ();
    numOut = 0;
    if ( nPlane <= 0)
        {
        if (numOut < maxOut)
