...
DRange3dP     range
    ...
    if (NULL != globalToLocal && NULL != localToGlobal)
...
DPoint3dCR end
    ...
    DVec3d endVector;
    vector01.DifferenceOf (middle, startPoint);
    ...
    if (DRay3d::ClosestApproachUnboundedRayUnboundedRay
        ...
        {
        center = center0;
        vector0.DifferenceOf (startPoint, center0);
...
* @bsimethod
+--------------------------------------------------------------------------------------*/
bool DEllipse3d::IsAlmostEqualFullEllipsePointSet(DEllipse3dCR other, double tolerance) const
    {
    if (DPoint3dOps::AlmostEqual(center, other.center, tolerance))
        {
        Transform ellipseFrame, ellipseInverse;
        if (GetLocalFrame(ellipseFrame, ellipseInverse))
            {
            DVec3d vector0B, vector90B;
            DPoint3d xyz0, xyz90;
            ellipseInverse.MultiplyMatrixOnly (vector0B, other.vector0);
            ellipseInverse.MultiplyMatrixOnly (vector90B, other.vector90);
            // We expect inverted points to be on the unit circle
            //    z == 0
            //    xy parts have magnitude 1
            // to get comparisons by distance, force z=0, normalize the xy parts, and go back to regular coordinates
            vector0B.z = vector90B.z = 0.0;
            auto unit0 = vector0B.ValidatedNormalize ();
            auto unit90 = vector90B.ValidatedNormalize();
            if (   unit0.IsValid ()
                && unit90.IsValid ()
                && vector0B.IsPerpendicularTo(vector90B))
                {
                ellipseFrame.Multiply (xyz0, unit0.Value ());
                ellipseFrame.Multiply (xyz90, unit90.Value ());
                return xyz0.AlmostEqual (other.center + other.vector0, tolerance)
                    && xyz90.AlmostEqual (other.center + other.vector90, tolerance);
                }
            }
        }
    return false;
...
* @bsimethod
+--------------------------------------------------------------------------------------*/
bool DEllipse3d::IsAlmostEqualByFractionMatch(DEllipse3dCR other, double tolerance) const
    {
    // "5 points determines a conic" -- So 5 points (non-symmetric) confirm point set match.
    // Evaluating each at fraction enforces sweep matching.
    if (DPoint3dOps::AlmostEqual(center, other.center, tolerance))
        {
        for (uint32_t i = 0; i < 5; i++)
            {
            double f = i * 0.20;
            DPoint3d xyzA = FractionToPoint (f);
            DPoint3d xyzB = other.FractionToPoint (f);
            if (!xyzA.AlmostEqual(xyzB, tolerance))
                return false;
            }
        return true;
        }
