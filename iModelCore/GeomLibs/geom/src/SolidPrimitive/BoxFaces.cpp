...
// (static method)
DSegment3d BoxFaces::GetEdgeSegment (DPoint3d const corners[8], uint32_t axis, uint32_t edgeIndex)
    {
    if (axis > 2)
        axis = axis % 3;
    if (edgeIndex > 3)
        edgeIndex = edgeIndex % 4;
    return DSegment3d::From (
                corners[s_axisEdgeVertex[axis][edgeIndex][0]],
                corners[s_axisEdgeVertex[axis][edgeIndex][1]]
                );
    }
...
* @bsimethod
+--------------------------------------------------------------------------------------*/
void BoxFaces::Load8Corners(DPoint3d corners[8])
    {
...
* @bsimethod
+--------------------------------------------------------------------------------------*/
void BoxFaces::Get5PointCCWFace(int faceIndex, DPoint3dCP corners, bvector<DPoint3d> &points)
    {
    faceIndex = faceIndex % 6;
    points.clear ();
    for (int k = 0; k < 4; k++)
        points.push_back (corners[s_cornerIndexCCW[faceIndex][k]]);
