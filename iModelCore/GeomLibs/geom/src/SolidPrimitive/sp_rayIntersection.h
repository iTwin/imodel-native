            ...
            for (int i = 0; i < 2; i++)
                ...
                if (fabs (z[i]) < 1.0) // z[i]==1 is a pole -- no cap surface 
                    ...
                    if (DoubleOps::SafeDivide (pierceFraction, z[i] - localRay.origin.z, localRay.direction.z, 0.0))
                        {
                        DPoint3d uvw1 = localRay.FractionParameterToPoint (pierceFraction);
                        if (uvw1.MagnitudeSquaredXY () <= r)
                            {
                            SolidLocationDetail detail (parentId, pierceFraction, ray.FractionParameterToPoint (pierceFraction));
                            detail.SetCapSelector (i);
                            double uFraction, vFraction;
                            double da = 2.0 * r;
                            FractionalizeInCircle (uvw1.x, uvw1.y, r, uFraction, vFraction);
                            DVec3d uVector, vVector;
...
int parentId
    ...
    for (size_t i = 0, n = curveA.size (); i < n; i++)
        ...
        if (!childA && !childB)
            {
            size_t pickDataInitialSize = pickData.size ();
            ...
            for (size_t ii = pickDataInitialSize; ii < pickData.size (); ii++)
                {
                pickData[ii].SetParentId (parentId);
                pickData[ii].SetFaceIndices01 ((ptrdiff_t)pickDataInitialSize, (ptrdiff_t)primitiveCounter);
                }
            primitiveCounter++;
            }
