...
bool _IsValidGeometry (GeometryValidatorPtr &validator) const override { return m_data.IsValidGeometry(validator);}
...
bool DgnTorusPipeDetail::IsValidGeometry(GeometryValidatorPtr &validator) const
    {
    if (!validator.IsValid ())
        return true;
    return validator->IsValidGeometry(m_center)
        && validator->IsValidGeometry(m_vectorX)
        && validator->IsValidGeometry(m_vectorY)
        && validator->IsValidDouble(m_majorRadius)
        && validator->IsValidDouble(m_minorRadius)
        && validator->IsValidDouble(m_sweepAngle)
        // bool is always valid . . . . && validator->IsValidGeometry(capped)
        ;
    }
...
bool DgnConeDetail::IsValidGeometry(GeometryValidatorPtr &validator) const
    {
    if (!validator.IsValid())
        return true;
    return validator->IsValidGeometry (m_centerA)
        && validator->IsValidGeometry (m_centerB)
        && validator->IsValidDouble(m_radiusA)
        && validator->IsValidDouble(m_radiusB)
        // bool is always valid . . . . && validator->IsValidGeometry(capped)
        ;
    }
/*--------------------------------------------------------------------------------**//**
...
bool DgnBoxDetail::IsValidGeometry(GeometryValidatorPtr &validator) const
    {
    if (!validator.IsValid())
        return true;
    return validator->IsValidGeometry(m_baseOrigin)
        && validator->IsValidGeometry(m_topOrigin)
        && validator->IsValidGeometry(m_vectorX)
        && validator->IsValidGeometry(m_vectorY)
        && validator->IsValidDouble(m_baseX)
        && validator->IsValidDouble(m_baseY)
        && validator->IsValidDouble(m_topX)
        && validator->IsValidDouble(m_topY)
        // bool is always valid . . . . && validator->IsValidGeometry(capped)
        ;
    }
...
bool DgnSphereDetail::IsValidGeometry(GeometryValidatorPtr &validator) const
    {
    if (!validator.IsValid())
        return true;
    return validator->IsValidGeometry(m_localToWorld)
        && validator->IsValidDouble(m_startLatitude)
        && validator->IsValidDouble(m_latitudeSweep)
        // bool is always valid . . . . && validator->IsValidGeometry(capped)
        ;
    }
/*--------------------------------------------------------------------------------**//**
...
bool DgnExtrusionDetail::IsValidGeometry(GeometryValidatorPtr &validator) const
    ...
    if (!validator.IsValid())
        return true;
    return validator->IsValidGeometry(m_baseCurve)
        && validator->IsValidGeometry(m_extrusionVector)
        // bool is always valid . . . . && validator->IsValidGeometry(m_capped)
        // int is not validated . . . && validator->IsValidGeometry(m_numVRules)
        ;
    }
...
bool DgnRotationalSweepDetail::IsValidGeometry(GeometryValidatorPtr &validator) const
    {
    if (!validator.IsValid())
        return true;
    return validator->IsValidGeometry(m_baseCurve)
        && validator->IsValidGeometry(m_axisOfRotation.origin)
        && validator->IsValidGeometry(m_axisOfRotation.direction)
        && validator->IsValidDouble(m_sweepAngle)
        // bool is always valid . . . . && validator->IsValidGeometry(m_capped)
        // int is not validated . . . && validator->IsValidGeometry(m_numVRules)
        ;
...
bool DgnRuledSweepDetail::IsValidGeometry(GeometryValidatorPtr &validator) const
    {
    if (!validator.IsValid ())
        return true;
    for (auto &section : m_sectionCurves)
        {
        if (!validator->IsValidGeometry (section))
            return false;
        }
  // bool is always valid . . . . && validator->IsValidGeometry(m_capped)
    return true;
