...
void testTightSurfaceExtents (MSBsplineSurfacePtr &surface)
    ...
    Check::Shift(0, dY, 0);
    Transform transform1 = Transform::FromIdentity ();
    DRange3d worldRange1 = surface->FullSurfaceRange(nullptr);
    SavePointsFromPatches (cloud);
    SaveBox (transform1, worldRange1);
    }
                ...
                for (bool chordLengthTangents : truefalse)
                    ...
                    for (bool naturalTangents : truefalse)
                        ...
                        if (s == SUCCESS)
                            ...
                            DoRoundTrip (IGeometry::Create (cp), true, 0);
                            DoRoundTrip(IGeometry::Create(cp), true, 1);
                            }
                        }
                    }
                }
...
TEST(Spiral,PartialSpiralEvaluation)
    ...
    // lengthC from polygon SHOULD be less than snippet length.  But with the bspline approximation in the way, it is bizarrely not so . . . just test close
    Check::Near (lengthC, snippetLength, "polygon length", toleranceFactor);
    Check::Near (lengthB, snippetLength, " snippet length", toleranceFactor);
    ...
    double distanceAlongTol = 1.0e-5;
    for (PathLocationDetailCR pntDetail : locations)
        ...
        if(!DoubleOps::AlmostEqual(prevDropDist, distanceAlong))
            ...
            Check::LessThanOrEqual (prevDropDist, distanceAlong + distanceAlongTol);
            Check::LessThanOrEqual (distanceAlong, snippetLength + distanceAlongTol);
            }
