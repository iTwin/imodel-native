...
TEST(BsplineCurve,PoleAccess)
    ...
    // Bug #816037: clamp knots to avoid clamping in MSBsplineCurve::Populate, which changes knots & poles, causing below tests to fail
    double leftKnot = knotA[order - 1];
    double rightKnot = knotA[knotA.size() - order];
    for (size_t i = 1; i < order; ++i)
        {
        knotA[order - 1 - i] = leftKnot;
        knotA[knotA.size() - order + i] = rightKnot;
        }
