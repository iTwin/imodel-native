...
TEST(BsplineCurve,KnotSearch)
    ...
    for (double  t = -0.5; t < 5.0; t += 0.5)
        ...
        if ( t > knots[order-1] && t <= knots[numKnots - order])
            {
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
TEST(Bspline,ApproximateNurbsCurve)
    {
    double tol = 1.0e-5;
    for (int order = 2; order < 7; order++)
        {
        for (int numPoles = order, step = 1; numPoles < 4 * order; numPoles += (step++))
            {
            double dtheta = Angle::TwoPi() / numPoles;
            bvector<DPoint3d> poles;
            for (int i = 0; i < numPoles; i++)
                {
                double theta = i * dtheta;
                poles.push_back(DPoint3d::From (cos(theta), 1.3 * sin(theta)));
                }
            MSBsplineCurvePtr bspline = MSBsplineCurve::CreateFromPolesAndOrder (poles, nullptr, nullptr, order, false);
            MSBsplineCurvePtr approx = MSBsplineCurve::CreatePtr();
            MSBsplineStatus status = MSBsplineCurve::ApproximateNurbsCurve(approx.get(), bspline.get(), 3, true, 2, tol);
            if (Check::True(0 == status, "Approximation constructed"))
                {
                SaveAndRestoreCheckTransform shifter(5, 0, 0);
                Check::SaveTransformed(*bspline.get());
                Check::SaveTransformed(*approx.get());

                DPoint3d start, end, startA, endA;
                DVec3d startTan, endTan, startTanA, endTanA;
                bspline->FractionToPoint(start, startTan, 0.0);
                bspline->FractionToPoint(end, endTan, 1.0);
                approx->FractionToPoint(startA, startTanA, 0.0);
                approx->FractionToPoint(endA, endTanA, 1.0);
                if (Check::Near(start, startA, "Interpolate start pt")
                    && Check::Near(end, endA, "Interpolate end pt")
                    && Check::True(fabs(startTan.SmallerUnorientedAngleToXY(startTanA)) <= Angle::SmallAngle(), "Interpolate start tangent")
                    && Check::True(fabs(endTan.SmallerUnorientedAngleToXY(endTanA)) <= Angle::SmallAngle(), "Interpolate end tangent"))
                    {
                    ...
                    for (size_t i = 1; i < numSamples; ++i)
                        ...
                        DPoint3d pt, ptA;
                        double paramA, dist;
                        bspline->FractionToPoint(pt, i * delta);
                        approx->ClosestPointXY(ptA, paramA, dist, pt, nullptr);
                        if (!Check::True(dist <= tol, "Approximation sample is close enough"))
                            Check::SaveTransformed(DSegment3d::From(pt, ptA));
                        }
                    }
                }
            }
        }
    Check::ClearGeometry ("BsplineCurve.ApproximateNurbsCurve");
    }
...
uint32_t numParamTest,  // There is an array of parameters of interest.  Test at most numParamTest of them.
double dz,               // Additional z change for the center.   0 is the hard start case with very dissimilar u and v size.   1 makes it easier.
bool noisy
)
    {
    // TODO: increase z separation and see if that helps? Could be u/v tangent magnitude difference is problem.
    DPoint3d center0 = DPoint3d::From(-1385.2222, -616.8729, -845.9954099);
    DPoint3d center1 = center0; center1.z = -846.0363934 + dz;

    DVec3d vector00 = DVec3d::From(-2.3035634, 2.3035634);
    DVec3d vector900 = vector00; vector900.y *= -1.0;

    DVec3d vector01 = DVec3d::From(-2.2745875, 2.2745875);
    DVec3d vector901 = vector01; vector901.y *= -1.0;

    DEllipse3d base0 = DEllipse3d::FromVectors(center0, vector00, vector900, 0.0, msGeomConst_2pi);
    DEllipse3d base1 = DEllipse3d::FromVectors(center1, vector01, vector901, 0.0, msGeomConst_2pi);

    // double uorScale = 10000;    // test case for ADO #788177 is in master units
    Transform scaleToUors = Transform::FromRowValues(uorScale,0,0,0, 0,uorScale,0,0, 0,0,uorScale,0);
    scaleToUors.Multiply(base0);
    scaleToUors.Multiply(base1);
    bvector<DPoint2d> params;
    params.push_back(DPoint2d::From(0.8, 0.217));                           // unscaled: FAIL param     scaled: FAIL param      (v = 0.21700000000000000)
    params.push_back(DPoint2d::From(0.8, 0.2 + 0.017));                     // unscaled: OK             scaled: FAIL param      (v = 0.21700000000000003)
    params.push_back(DPoint2d::From(0.8, 0.234));                           // unscaled: FAIL param     scaled: OK              (v = 0.23400000000000001)
    params.push_back(DPoint2d::From(0.8, 0.523));                           // unscaled: OK             scaled: OK              (v = 0.52300000000000002)
    params.push_back(DPoint2d::From(0.8, 0.523 + 0.00000000000000022204));  // unscaled: OK             scaled: OK              (v = 0.52300000000000024)

    auto baseArc0 = ICurvePrimitive::CreateArc(base0);
    auto baseArc1 = ICurvePrimitive::CreateArc(base1);
    if (Check::True(baseArc0.IsValid() && baseArc1.IsValid(), "Cone bases valid"))
        {
        auto curve0 = baseArc0->GetMSBsplineCurvePtr();
        auto curve1 = baseArc1->GetMSBsplineCurvePtr();
        if (Check::True(curve0.IsValid() && curve1.IsValid(), "B-spline curve bases valid") &&
            Check::True(curve0->IsClosed() && curve1->IsClosed(), "B-spline curve bases closed") &&
            Check::Size(curve0->GetNumPoles(), curve1->GetNumPoles(), "Cone bases compatible"))
            {
            ...
            if (Check::True(ruledSurface.IsValid(), "B-spline surface valid"))
                ...
                for (uint32_t i = 0; i < numParamTest && i < params.size (); i++)
                    {
                    auto uv = params[i];
                    DPoint3d evaluatedPoint, closestPoint;
                    DPoint2d closestParam;
                    ruledSurface->EvaluatePoint(evaluatedPoint, uv.x, uv.y);
                    //IModelJson::TryGeometryToIModelJsonString(json, *IGeometry::Create(ICurvePrimitive::CreateLine(DSegment3d::From(evaluatedPoint, DPoint3d::From(evaluatedPoint.x, evaluatedPoint.y, evaluatedPoint.z + 20000)))));
                    ruledSurface->ClosestPoint(closestPoint, closestParam, evaluatedPoint);
                    Check::SaveTransformed(DSegment3d::From(evaluatedPoint, closestPoint));
                    if (Check::True(DPoint2dOps::AlmostEqual(uv, closestParam, 1.0e-10), "Param roundtrip"))  // 1.0e-12 is too tight
                        Check::Near(evaluatedPoint, closestPoint, "Point roundtrip");
                    if (noisy)
                        printf (" (uv0 %.16lg %.16lg) (uv0 %.16lg %.16lg)  (duv %.16lg) (dxyz %.16lg)\n",
                                uv.x, uv.y, closestParam.x, closestParam.y,
                                uv.Distance (closestParam),
                                evaluatedPoint.Distance (closestPoint));
                    // Check::Shift(0, 0, 1 + dz);
                    }
                }
            }
        }
    }
TEST(BsplineSurface, ConeProjectionRoundTrip)
    {
    static bool s_noisyProjection = false;
    doConeProjectionTest (1.0, 11, 0.0, s_noisyProjection);
    doConeProjectionTest (1.0, 11, 1.0, s_noisyProjection);
    double uorScale = 10000.0;
