...
void VerifyFacetEdgeLocations (PolyfaceHeaderR mesh, CurveVectorCP curve)
    ...
    if (NULL == curve)
        return;
    for (size_t curveIndex = 0; curveIndex < curve->size (); curveIndex++)
        {
        ICurvePrimitiveCP prim = curve->at (curveIndex).get ();
        if (NULL != prim)
            {
            ...
            if (edgeData.IsValid () && edgeData->size () == points->size ())
                ...
                for (size_t i1 = 0; edgeData->TryGet (i1, data); i1++)
                    {
                    DPoint3d xyz;
                    if (Check::True (mesh.TryEvaluateEdge (data, xyz), "EvaluateEdge"))
                        Check::Near (points->at(i1), xyz, "Edge XYZ");
                    }
                }
            }
        }
...
void TestLargeMesh (int numX, int numY, int numCall, double globalScale, DrapeAction action)
    ...
    DSegment3d segmentA = DSegment3d::From (0.5, 0.5, 100, 10.5, 1.5, 200);
    bvector<DPoint3d> linestring;
    linestring.push_back (segmentA.point[0]);
    linestring.push_back (segmentA.point[1]);
    ...
    if (action == DrapeAction::FastDrape)
        ...
        for (int i = 0; i < numCall; i++)
            {
            context.DoDrapeXY (segmentA, drapeSegments);
            }
        time1 = BeTimeUtilities::QueryMillisecondsCounter ();
        }
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
TEST(Polyface, ValidateMeshConstruction)
    {
    BeFileName dataPath;
    BeTest::GetHost().GetDocumentsRoot(dataPath);
    BeFileName outputPath;
    ...
    dataPath.AppendToPath(L"GeomLibsTestData");
    dataPath.AppendToPath(L"Polyface");
    dataPath.AppendToPath(L"validation");
    bvector<IGeometryPtr> g0;
    bvector<WCharCP> filenames{
        L"Dodecahedron.imjs",
        };
    ...
    for (auto &filename : filenames)
        ...
        if (DGNJSFileToGeometry(fullPath, geometry))
            ...
            for (auto g : geometry)
                {
                Check::True (g->IsValidGeometry (validator));
                }
            }
        }
    }
...
#endif
#ifdef CompileBigSynchroMesh
// Load a largish file that has had param indexing problems in imodel02 display.
TEST(Polyface, BigSynchroMesh)
    {
    BeFileName dataPath;
    BeTest::GetHost().GetDocumentsRoot(dataPath);
    BeFileName outputPath;
    ...
    dataPath.AppendToPath(L"GeomLibsTestData");
    dataPath.AppendToPath(L"Polyface");
    dataPath.AppendToPath(L"MeshParamsInFlatbuffer");
    bvector<IGeometryPtr> g0;
    bvector<WCharCP> filenames{
        L"BigSynchroMesh.imjs",
        };
    bvector<PolyfaceHeaderPtr> allMesh;
    for (auto &filename : filenames)
        {
        ...
        if (DGNJSFileToGeometry(fullPath, geometry))
            ...
            for (auto g0 : geometry)
                {
                auto mesh = g0->GetAsPolyfaceHeader();
                if (mesh.IsValid())
                    {
                    bvector<Byte> buffer;
                    ...
                    if (g1.IsValid ())
                        Check::True (g1->IsSameStructureAndGeometry(*g0));
                    }
                }
            }
        }
