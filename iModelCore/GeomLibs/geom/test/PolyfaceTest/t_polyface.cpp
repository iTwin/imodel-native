...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
TEST(Polyface, CompressDegnerateMesh)
    {
    for (int numPoint : {10, 0})
        {
        auto polyface = PolyfaceHeader::CreateVariableSizeIndexed();
        for (int i = 0; i < numPoint; i++)
            polyface->Point ().push_back (DPoint3d::From (i,0,0));
        polyface->PointIndex().push_back (1000);
        polyface->PointIndex().push_back(1002);
        polyface->PointIndex().push_back(1001);
        polyface->PointIndex().push_back(0);
        polyface->Compress ();
        Check::Size (0, polyface->Point ().size ());
        }
    }
...
void VerifyPolyface (PolyfaceHeader  &meshVectors, PolyfaceQueryR meshQuery, char const* name)
    ...
    if (edgeFailureIndices.size () > 0)
        ...
        {
        size_t numVertex1, numFacet1, numTri1, numQuad1, numImplicitTri1, numVisible1, numInvisible1;
        meshVectors.CollectCounts (numVertex1, numFacet1, numTri1, numQuad1, numImplicitTri1, numVisible1, numInvisible1);
        Check::Size (numFacet0, numFacet1, "Facet Counts");
        }
...
TEST(PolyfaceConstruction, LinearSweep)
    ...
    IPolyfaceConstructionPtr builder =  IPolyfaceConstruction::Create (*options);
    double mySize = 1.0;
    UNUSED_VARIABLE(mySize); //llvm-13 detect this as unused var
...
size_t numRow
    ...
    if (normals != NULL)
        {
        size_t n2 = numPerRow * numRow;
        builder->FindOrAddNormals (normals, n2, index);
        for (size_t j1 = 1; j1 < numRow; j1++)
            {
            size_t j0 = j1 - 1;
            for (size_t i1 = 1; i1 < numPerRow; i1++)
...
* @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
TEST (PolyfaceVisitor, ParamToDetail)
    {
...
void AddQuads (IPolyfaceConstructionPtr &builder, DPoint3d xyz00, DPoint3d xyz11, size_t numX, size_t numY)
    ...
    double area = 0.0;
    UNUSED_VARIABLE(area); //llvm-13 detect this as unused var
...
double dy
    ...
    IPolyfaceConstructionPtr builder = IPolyfaceConstruction::Create(*options);
    Check::Int (0, builder->GetClientMeshPtr ()->GetExpectedClosure());
    int ec1 = mesh1->GetExpectedClosure();
    int ec2 = mesh2->GetExpectedClosure();
    builder->AddPolyface (*mesh1);
    builder->AddPolyface (*mesh2);
    auto mergedMesh = builder->GetClientMeshPtr ();
    int ec3 = mergedMesh->GetExpectedClosure ();
    Check::Int (std::min (ec1, ec2), ec3, "composite expectedClosure");
    auto purgedMesh = mergedMesh->CloneWithIndexedDuplicatesRemoved ();
    int ec4 = purgedMesh->GetExpectedClosure ();
    Check::Int(0, ec4, "expectedClosure after CloneWithIndexedDuplicatesRemoved");
...
void ExercisePunchAndMaximal (PolyfaceHeaderPtr &clipper, PolyfaceHeaderPtr & target)
    ...
    for (auto mergeEdges : trueFalse)
        {
        SaveAndRestoreCheckTransform shifter1 (0,a,0);
        for (auto mergeFaces : trueFalse)
            {
            SaveAndRestoreCheckTransform shifter2 (a,0,0);
 ...
 TEST(Polyface,PiotrSweep)
    ...
    for (auto axes :
        ...
        for (bool clockwise : {false, true})
            {
            SaveAndRestoreCheckTransform shifter2 (0,20,0);
...
+---------------+---------------+---------------+---------------+---------------+------*/
 TEST(Polyface,FilledDisk)
    ...
    for (double maxEdgeLength : {10.0, 5.0, 1.0})
        {
        SaveAndRestoreCheckTransform shifter1 (3.0 * radiusA, 0, 0);
        options->SetMaxEdgeLength (maxEdgeLength);
        for (double radiusB : {5.0, 4.0, 1.0, 8.0, 12.0})
            {
            counter++;
            SaveAndRestoreCheckTransform shifter2 (0.0, 8.0 * radiusB, 0);
            IPolyfaceConstructionPtr builder = IPolyfaceConstruction::Create (*options);
...
TEST(Polyface, FromDRange3d)
    {
    DRange3d range = DRange3d::From (-1,-2,0, 5,4,6);
    double e = 0.2;
    double ez = 0.5;
    double volumeA = range.Volume ();
    for (bool triangulate : { false, true })
        {
        SaveAndRestoreCheckTransform shifter (0,50,0);
        auto meshB = PolyfaceHeader::CreateDRange3dFaces(range, false, nullptr);
        auto volumeB = meshB->SumTetrahedralVolumes(DPoint3d::From (0,0,0));
        Check::Near (volumeA, volumeB, "range volume == mesh volume");
        Check::SaveTransformed(*meshB);
        Check::Shift(20, 0, 0);
        for (double z : { 1.0, 2.0, -1.5 })
            {
            // note: expect all column magnitudes greater than or equal to 1 so volume can only increase.
            auto placement = Transform::FromRowValues(
                1, 0, ez, 1,
                e, 1, 0, 1,
                0, e, z, -1
                );
            double det = placement.Determinant ();
            auto meshC = PolyfaceHeader::CreateDRange3dFaces (range, triangulate, &placement);
            auto volumeC = meshC->SumTetrahedralVolumes(DPoint3d::From(0, 0, 0));
            Check::Near(volumeA * fabs (det), volumeC, "range volume == transformed mesh volume");
     ...
     Check::ClearGeometry("Polyface.FromDRange3d");
     }

TEST(Polyface, TryGetAtReadIndex)
    {
    auto mesh = DodecahedronMesh();
    if (mesh.IsValid())
        {
        Check::True(mesh->Point().Active());
        Check::True(mesh->PointIndex().Active());
        Check::True(mesh->Normal().Active());
        Check::True(mesh->NormalIndex().Active());
        Check::True(mesh->Param().Active());
        Check::True(mesh->ParamIndex().Active());
        Check::True(mesh->IntColor().Active());
        Check::True(mesh->ColorIndex().Active());
        Check::True(mesh->FaceData().Active());
        Check::True(mesh->FaceIndex().Active());
        ...
        for (visitor->Reset(); visitor->AdvanceToNextFace();)
            {
            for (size_t localIndex = 0; localIndex < visitor->NumEdgesThisFace(); ++localIndex)
                {
                auto globalIndex = visitor->GetReadIndex() + localIndex;
                Check::Size(visitor->IndexPosition()[localIndex], globalIndex);
                ...
                Check::True(mesh->TryGetParamAtReadIndex(globalIndex, uv));
                Check::Exact(uv, visitor->Param()[localIndex]);

                Check::True(mesh->TryGetIntColorAtReadIndex(globalIndex, color));
                Check::Size(color, visitor->IntColor()[localIndex]);

                Check::True(mesh->TryGetFacetFaceDataAtReadIndex(globalIndex, facetData, _));
                Check::ExactRange(facetData.m_paramRange, visitor->GetFaceDataCP()[localIndex].m_paramRange);
                Check::ExactRange(facetData.m_paramDistanceRange, visitor->GetFaceDataCP()[localIndex].m_paramDistanceRange);
                }
            }

        // test TryGet input validation
        for (size_t readIndex = 0; readIndex < mesh->GetPointIndexCount(); ++readIndex)
            {
            Check::Bool(0 != mesh->GetPointIndexCP()[readIndex], mesh->TryGetPointAtReadIndex(readIndex, xyz));
            Check::Bool(0 != mesh->GetNormalIndexCP(true)[readIndex], mesh->TryGetNormalAtReadIndex(readIndex, normal));
            Check::Bool(0 != mesh->GetParamIndexCP(true)[readIndex], mesh->TryGetParamAtReadIndex(readIndex, uv));
            Check::Bool(0 != mesh->GetColorIndexCP()[readIndex], mesh->TryGetIntColorAtReadIndex(readIndex, color));
            Check::Bool(0 != mesh->GetFaceIndexCP()[readIndex], mesh->TryGetFacetFaceDataAtReadIndex(readIndex, facetData, _));
            }
        }
    }
...
void testVisitorNumWrap(PolyfaceHeaderCR mesh, uint32_t maxNumWrap)
    ...
    Check::Shift(mesh.PointRange().XLength() + gap, 0, 0);
    ...
    for (uint32_t numWrap = 0; numWrap <= maxNumWrap; ++numWrap)
        ...
        for (visitor->Reset(); visitor->AdvanceToNextFace();)
            ...
            for (uint32_t iHead = 0, iTail = n; iTail < n + visitor->GetNumWrap(); ++iHead, ++iTail)
                {
                iHead = iHead % n;
                if (!visitor->Point().empty())
                    {
                    auto a = visitor->Point();
                    Check::True(iHead < a.size());
                    Check::True(iTail < a.size());
                    Check::Exact(a[iHead], a[iTail]);
                    }
                if (!visitor->Visible().empty())
                    {
                    auto a = visitor->Visible();
                    Check::True(iHead < a.size());
                    Check::True(iTail < a.size());
                    Check::Bool(a[iHead], a[iTail]);
                    }
                if (!visitor->IndexPosition().empty())
                    {
                    auto a = visitor->IndexPosition();
                    Check::True(iHead < a.size());
                    Check::True(iTail < a.size());
                    Check::Size(a[iHead], a[iTail]);
                    }
                if (!visitor->Normal().empty())
                    {
                    auto a = visitor->Normal();
                    Check::True(iHead < a.size());
                    Check::True(iTail < a.size());
                    Check::Exact(a[iHead], a[iTail]);
                    }
                if (!visitor->Param().empty())
                    {
                    auto a = visitor->Param();
                    Check::True(iHead < a.size());
                    Check::True(iTail < a.size());
                    Check::Exact(a[iHead], a[iTail]);
                    }
                if (!visitor->IntColor().empty())
                    {
                ...
                if (auto a = visitor->GetFaceDataCP())
                    {
                    Check::ExactRange(a[iHead].m_paramRange, a[iTail].m_paramRange);
                    Check::ExactRange(a[iHead].m_paramDistanceRange, a[iTail].m_paramDistanceRange);
                    }
                if (!visitor->ClientPointIndex().empty())
                    {
                    auto a = visitor->ClientPointIndex();
                    Check::True(iHead < a.size());
                    Check::True(iTail < a.size());
                    Check::Int(a[iHead], a[iTail]);
                    }
                if (!visitor->ClientNormalIndex().empty())
                    {
                    auto a = visitor->ClientNormalIndex();
                    Check::True(iHead < a.size());
                    Check::True(iTail < a.size());
                    Check::Int(a[iHead], a[iTail]);
                    }
                if (!visitor->ClientParamIndex().empty())
                    {
                    auto a = visitor->ClientParamIndex();
                    Check::True(iHead < a.size());
                    Check::True(iTail < a.size());
                    Check::Int(a[iHead], a[iTail]);
                    }
                if (!visitor->ClientColorIndex().empty())
                    {
                    auto a = visitor->ClientColorIndex();
                    Check::True(iHead < a.size());
                    Check::True(iTail < a.size());
                    Check::Int(a[iHead], a[iTail]);
                    }
                if (!visitor->ClientFaceIndex().empty())
                    {
                    auto a = visitor->ClientFaceIndex();
                    Check::True(iHead < a.size());
                    Check::True(iTail < a.size());
                    Check::Int(a[iHead], a[iTail]);
                    }
...
void testBlockedIntNumWrap(bvector<int> const& a, size_t index0, size_t numTotal, size_t numValues, size_t numWrap)
    {
    Check::Size(numTotal, numValues + numWrap);
    for (size_t i = 0, iHead = index0, iTail = index0 + numValues; i < numWrap; ++i, ++iHead, ++iTail)
        {
        iHead = index0 + ((iHead - index0) % numValues);
        if (!a.empty())
            {
            Check::True(iHead < a.size());
            Check::True(iTail < a.size());
...
void testBlockedBoolNumWrap(bvector<BoolTypeForVector> const& a, size_t index0, size_t numTotal, size_t numValues, size_t numWrap)
    {
    bvector<int> aInt;
    for (auto entry : a)
        aInt.push_back(entry);
    testBlockedIntNumWrap(aInt, index0, numTotal, numValues, numWrap);
    }

TEST(Polyface, NumWrapValidation)
    {
    uint32_t maxNumWrap = 11;

    // test larger face loop sizes
    auto mesh = DodecahedronMesh();
    testVisitorNumWrap(*mesh, maxNumWrap);
    ...
    // test STL style
    auto allOnes = DVec3d::From(DPoint3d::FromOne());
    auto plane = DPoint3dDVec3dDVec3d(DPoint3d::FromZero(), allOnes, DVec3d::FromUnitPerpendicularXY(allOnes).Value());
    mesh = UnitGridPolyface(plane, 4, 5, true, true);
    testVisitorNumWrap(*mesh, maxNumWrap);

    // test tensor product (grid) style
    mesh = UnitGridPolyface(plane, 6, 4, false, false);
    testVisitorNumWrap(*mesh, maxNumWrap);

    bvector<int> values = {0,1,2,3,4,5};
    bvector<int> oneBasedIndices = {4,6,-2,-1,5,2,-3};
    BlockedVectorInt a, b;
    b.Append(values.data(), values.size());
    for (uint32_t numWrap = 0; numWrap <= maxNumWrap; ++numWrap)
        {
        size_t index0 = 0;
        size_t numOut = a.ClearAndAppendBlock(values.data(), values.size(), 0, (uint32_t) values.size(), numWrap);
        testBlockedIntNumWrap(a, index0, numOut, values.size(), numWrap);

        numOut = a.ClearAndAppendBlock(b, 0, (uint32_t) b.size(), numWrap);
        testBlockedIntNumWrap(a, index0, numOut, b.size(), numWrap);

        bvector<int> zeroBasedIndices;
        bvector<BoolTypeForVector> positive;
        numOut = a.ClearAndAppendByOneBasedIndices(zeroBasedIndices, &positive, values, oneBasedIndices, 0, (uint32_t) oneBasedIndices.size(), numWrap);
        testBlockedIntNumWrap(a, index0, numOut, oneBasedIndices.size(), numWrap);
        testBlockedIntNumWrap(zeroBasedIndices, 0, numOut, oneBasedIndices.size(), numWrap);
        testBlockedBoolNumWrap(positive, 0, numOut, oneBasedIndices.size(), numWrap);

        zeroBasedIndices.clear();
        positive.clear();
        numOut = a.ClearAndAppendByOneBasedIndices(zeroBasedIndices, &positive, values.data(), values.size(), oneBasedIndices.data(), oneBasedIndices.size(), 0, (uint32_t) oneBasedIndices.size(), numWrap);
        testBlockedIntNumWrap(a, index0, numOut, oneBasedIndices.size(), numWrap);
        testBlockedIntNumWrap(zeroBasedIndices, 0, numOut, oneBasedIndices.size(), numWrap);
        testBlockedBoolNumWrap(positive, 0, numOut, oneBasedIndices.size(), numWrap);
        ...
        int value0 = 100;
        size_t numValue = 5;
        a.AddSequentialBlock(value0, numValue, numWrap, 0, false /* clearFirst */);
        numOut = a.size() - index0;
        testBlockedIntNumWrap(a, index0, numOut, numValue, numWrap);

        index0 = a.size();
        a.AddSteppedBlock(value0, 3, numValue, numWrap, 0, false /* clearFirst */);
        numOut = a.size() - index0;
        testBlockedIntNumWrap(a, index0, numOut, numValue, numWrap);
        }

    Check::ClearGeometry("Polyface.NumWrapValidation");
    }
...
// ignores aux data!
PolyfaceHeaderPtr clonePolyfaceRandomOrder(PolyfaceHeaderCR mesh)
    {
    size_t numFacets = mesh.GetNumFacet();
    ...
    auto cloneShuffled = mesh.Clone();
    cloneShuffled->PointIndex().clear();

    auto visitor = PolyfaceVisitor::Attach(mesh, false);
    for (auto const& iFacet : facetIndices)
        {
        visitor->Reset();
        for (size_t i = 0; visitor->AdvanceToNextFace() && i < iFacet; ++i);    // yuck, linear search
        cloneShuffled->PointIndex().AddAndTerminate(mesh.GetPointIndexCP() + visitor->GetReadIndex(), visitor->NumEdgesThisFace());
        }

    return cloneShuffled;
    }

PolyfaceHeaderPtr clonePolyfaceWithoutDegenerateFaces(PolyfaceHeaderCR mesh)
    {
    auto cloneFiltered = mesh.Clone();
    cloneFiltered->PointIndex().clear();

    for (auto visitor = PolyfaceVisitor::Attach(mesh, false); visitor->AdvanceToNextFace(); )
        {
        ...
        cloneFiltered->PointIndex().AddAndTerminate(mesh.GetPointIndexCP() + visitor->GetReadIndex(), visitor->NumEdgesThisFace());
        }
...
TEST(Polyface, ConnectedComponentsMaxFaces)
    {
    auto sphere = DgnSphereDetail(DPoint3d::FromZero(), 10);
    auto options = IFacetOptions::CreateForSurfaces(0.0, 0.15);
    auto builder = PolyfaceConstruction::Create(*options);
    if (Check::True(builder->Add(sphere), "mesh builder successfully added sphere"))
        {
        auto originalMesh = builder->GetClientMeshPtr();
        auto numFacets = originalMesh->GetNumFacet();
        auto filteredMesh = clonePolyfaceWithoutDegenerateFaces(*originalMesh); // shouldn't be necessary...
        numFacets = filteredMesh->GetNumFacet();
        auto mesh = clonePolyfaceRandomOrder(*filteredMesh);
        if (Check::True(mesh.IsValid() && mesh->GetNumFacet() == numFacets, "randomized mesh has same number of facets"))
            {
            auto pFacets = jmdlMTGFacets_new();
            if (Check::True(PolyfaceToMTG(pFacets, nullptr, nullptr, *mesh, true, 0, 0, 1), "mesh to MTG successful") &&
                Check::Size(pFacets->GetGraphP()->CountFaceLoops(), numFacets, "mesh and MTG have same number of facets"))
                {
                bvector<bvector<MTGNodeId>> components;
                size_t maxFacets = numFacets / 3;
                pFacets->GetGraphP()->CollectConnectedComponents(components, maxFacets);
                for (auto const& component : components)
                    {
                    auto subMesh = MTGFacetsSubsetToIndexedPolyface(pFacets, MTG_EXTERIOR_MASK, MTG_DIRECTED_EDGE_MASK, component);
                    Check::True(subMesh.IsValid() && subMesh->GetNumFacet() <= maxFacets, "subMesh facet count less than maxFacets");
