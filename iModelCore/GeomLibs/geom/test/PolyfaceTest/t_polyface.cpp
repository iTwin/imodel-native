...
TEST (Polyface, ClipNGonSweeps)
    {
    for (int numSide : {4,7,12})
        {
        SaveAndRestoreCheckTransform shifter (20,0,0);
        for (auto pass : {0,1, 2, 4, -1, -2})
            {
            ...
            if (pass > 0 && pass < 5)
                ...
                bvector<DPoint3d> points {
                    DPoint3d::From (-1.1, yy, z0),
                    DPoint3d::From (-1,yy,z1),
                    DPoint3d::From ( 1, yy, z1),
                    DPoint3d::From ( 1.3,yy,z0)
                    };
                if (pass == -2)
                    {
                    // close the hole
            ...
            // Create the output mesh and bind its points to a PolyfaceCoordinateMap . . . 
            PolyfaceHeaderPtr outputMeshPtr = PolyfaceHeader::CreateVariableSizeIndexed ();
            PolyfaceCoordinateMapPtr map1Ptr = PolyfaceCoordinateMap::Create (*outputMeshPtr);

            bool badCuts;
            PolyfaceCoordinateMap::AddClippedPolyface (builder->GetClientMeshR (), map1Ptr.get (), NULL, badCuts, &clipper, true);
            Check::SaveTransformed (*outputMeshPtr);
            }
        }
...
void testContours (int splineOrder, double z0, double zStep, bool capped)
    ...
    GraphicsPointArray contourA, contourB, contourC;
    contours.push_back (&contourA);
    contours.push_back (&contourB);
    contours.push_back (&contourC);
    contourA.Add (DEllipse3d::From (0,0,z0,   1,0,0,  0,1,0, 0.0, msGeomConst_piOver2));
    contourA.Add (DSegment3d::From (0,1,z0,   0,2,z0));
    contourA.MarkBreak ();

    MSBsplineCurve curve;
    bvector<DPoint3d> poles;
    poles.push_back (DPoint3d::FromXYZ (0,2,z0));
    poles.push_back (DPoint3d::FromXYZ (0,3,z0));
    poles.push_back (DPoint3d::FromXYZ (1,3,z0));
    poles.push_back (DPoint3d::FromXYZ (2,3,z0));
    poles.push_back (DPoint3d::FromXYZ (3,2,z0));
    poles.push_back (DPoint3d::FromXYZ (3,0,z0));
    poles.push_back (DPoint3d::FromXYZ (1,0,z0));
    curve.Populate (poles, NULL, NULL, splineOrder, false, false);

    contourA.AddAsCompleteBsplineCurve (curve);

    Transform zTransform = Transform::From (0,0,zStep);

    contourB = contourA;
    contourB.Multiply (zTransform);

    contourC = contourB;
    contourC.Multiply (zTransform);

    builder->AddRuledBetweenCorespondingCurves (contours, capped);
    ExaminePolyface (builder->GetClientMeshR (), "GPATower");
    curve.ReleaseMem ();
    }
...
TEST (Polyface, RotateGPA)
    ...
    for (int splineOrder = 2; splineOrder < 5; splineOrder++)
        {
        for (double sweep = msGeomConst_2pi; sweep > 0.4 * msGeomConst_pi; sweep *= 0.5)
            {            
            GraphicsPointArray   contourA;
            
            contourA.Add (DEllipse3d::From (0,0,0,   1,0,0,  0,1,0, 0.0, msGeomConst_piOver2));
            contourA.Add (DSegment3d::From (0,1,0,   0,2,0.0));
            contourA.MarkBreak ();

            MSBsplineCurve curve;
            bvector<DPoint3d> poles;
            poles.push_back (DPoint3d::FromXYZ (0,2,0.0));
            poles.push_back (DPoint3d::FromXYZ (0,3,0.0));
            poles.push_back (DPoint3d::FromXYZ (1,3,0.0));
            poles.push_back (DPoint3d::FromXYZ (2,3,0.0));
            poles.push_back (DPoint3d::FromXYZ (3,2,0.0));
            poles.push_back (DPoint3d::FromXYZ (3,0,0.0));
            poles.push_back (DPoint3d::FromXYZ (1,0,0.0));
            curve.Populate (poles, NULL, NULL, splineOrder, false, false);

            contourA.AddAsCompleteBsplineCurve (curve);
            DPoint3d origin = DPoint3d::FromXYZ (-1, 0, 0);
            DVec3d   axis   = DVec3d::From (0,1,0);
            builder->Clear ();
            builder->AddRotationalSweep (contourA, origin, axis, sweep, true);
            ExaminePolyface (builder->GetClientMeshR (), "RotationalSweep");
            SetTransformToNewGridSpot (*builder);
            curve.ReleaseMem ();
            }
        }
...
TEST (Polyface, Contours)
    ...
    testContours (2, z0, zStep, false);
    testContours (3, z0, zStep, false);
    testContours (4, z0, zStep, false);

    testContours (2, z0, zStep, true);
    testContours (3, z0, zStep, true);
    testContours (4, z0, zStep, true);
    Check::Size ((size_t)allocationCounter, (size_t)BSIBaseGeom::GetAllocationDifference ());
    }
...
TEST (Polyface, ClosedContours)
    ...
    contours.push_back (&contourB);
    contours.push_back (&contourC);
    contourA.Add (DEllipse3d::From (0,0,0,   1,0,0,  0,1,0, 0.0, msGeomConst_2pi));

    double f = 0.5;
    Transform zStep = Transform::FromRowValues (
                    f,0,0,0,
                    0,f,0,0,
                    0,0,1,1);
    contourB = contourA;
    contourB.Multiply (zStep);

    contourC = contourB;
    contourC.Multiply (zStep);

    builder->AddRuledBetweenCorespondingCurves (contours, true);
