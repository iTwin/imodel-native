...
MTGNodeId FindNodeWithModalFaceValue(MTGDoubleFaceFacets &facets, bvector<MTGNodeId> const &candidates, int value)
    {
    for (auto node : candidates)
        {
        if (facets.GetFaceValueInNode(node) == value)
            return node;
        }
    return MTG_NULL_NODEID;
    }
TEST(MTG, DoubleFaceComponents)
    {
    MTGDoubleFaceFacets facets(0.0);
    facets.DefineModalFaceLabel (-1, -1);
    // construct abutting squares, with even and odd starts in different modal groups.
    bvector<int> x0Array{ 0, 1, 3, 4, 5, 8, 9, 11, 14};
    double x1Old = DBL_MAX;
    ...
    for (int x0 : x0Array)
        ...
        facets.SetModalFaceValue (x0 & 0x01);       // each facet is in group 0 or group 1
        facets.AddDoubleSidedFace (points);
        if (x0 != x1Old)
            numComponent++;
        x1Old = x1;
        }

    // The common points have not been detected ... expect one component per rectangle .
    bvector<bvector<MTGNodeId>> componentNodes;
    facets.GetGraphP ()->CollectConnectedComponents (componentNodes);

    Check::Size (x0Array.size (), componentNodes.size ());

    // splice them all together ...
    jmdlMTGFacets_stitchFacets(&facets, DoubleOps::SmallMetricDistance (), 0.0);
    auto polyfaceA = PolyfaceHeader::CreateVariableSizeIndexed ();
    AddMTGFacetsToIndexedPolyface(&facets, *polyfaceA);
    Check::SaveTransformed(*polyfaceA);
    Check::Shift (0, 3);

    facets.GetGraphP()->CollectConnectedComponents(componentNodes);
    Check::Size(numComponent, componentNodes.size());
    // A component with 8 faces is one square.
    // A component with more is multiple squares
    for (auto &component : componentNodes)
        {
        size_t numValue = 0;
        if (FindNodeWithModalFaceValue(facets, component, 0) != MTG_NULL_NODEID)
            numValue++;
        if (FindNodeWithModalFaceValue(facets, component, 1) != MTG_NULL_NODEID)
            numValue++;
        if (component.size() == 8) // it is a singleton, should have one modal face value but not the other
            {
            Check::Size (1, numValue, "Singleton face should have one group");
            }
        else
            {
            // abutting faces should represent both
            Check::Size(2, numValue, "Multiple faces should have two groups");
            }
        auto polyface = MTGFacetsSubsetToIndexedPolyface (&facets, MTG_EXTERIOR_MASK, MTG_DIRECTED_EDGE_MASK, component);
        Check::SaveTransformed (*polyface);
        }
    Check::ClearGeometry ("MTG.DoubleFaceComponents");
    }