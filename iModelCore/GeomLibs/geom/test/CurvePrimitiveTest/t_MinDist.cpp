...
TEST(CurveVector, IsSameStructureAndGeometryWithTransform)
    ...
    for (size_t i = 0; i < curveSets.size (); i++)
        ...
        for (size_t j = i; j < curveSets.size (); j++)
            {
            Check::NamedScope scopeJ ("curveJ", j);
    ...
    for (double radiusB : {2.0, 1.0, 3.0})
        ...
        for (double degreesA : manyDegrees)
            ...
            for (double degreesB : manyDegrees)
                {
                SaveAndRestoreCheckTransform shifterB(0, bigStep, 0);
                ...
                for (DPoint2d signs : {
                    ...
                    {
                    SaveAndRestoreCheckTransform shifterC(0, 10, 0);
...
TEST(CurveCurve, ClosestApproach)
    {
    // test case from ADO#1229059 in feet
    Angle startAngle = Angle::FromDegrees(-310.02784398485130);
    Angle endAngle = Angle::FromDegrees(-322.67387688089713);
