...
MTGNodeId &nodeD
    ...
    for (int i = 0; i < 4; i++)
        ...
        if (oldFace == m_faces.end ())
            ...
            MTGNodeId nodeA1 = newFaces[i].m_node;
            MTGNodeId nodeB1 = partner.m_node;
            m_faces.erase (partner);
            // Each face has 3 nodes.
            // Walk side-by-side, moving to FPred or FSucc on respective sides.
            for (int innerCounter = 0; innerCounter < 3; innerCounter++, nodeA1 = FSucc (nodeA1), nodeB1 = FPred (nodeB1))
                {
                SetFaceMates (nodeA1, nodeB1);
                }
            }
        }
    }
