...
static void AddBoxes (bvector<IGeometryPtr> &data, double ax, double ay, double az, bool capped, uint32_t maxPerType = 32)
    {
...
static void AddCones (bvector<IGeometryPtr> &data, double radiusA, double radiusB, bool capped, uint32_t maxAdd = 100)
    {
    bvector<double>signs {1.0, -1.0};
    // ====================
    uint32_t numAdd = 0;
...
static void AddMeshedSolidPrimtives (
    bvector<PolyfaceHeaderPtr> &meshArray,
    bvector<IGeometryPtr> const &solids,
    int maxPerFace,
    ...
    options->SetAngleTolerance(angleTolRadians);
    for (auto &g : solids)
        {
        ...
        if (s.IsValid())
            ...
            builder->AddSolidPrimitive(*s);
            auto m = builder->GetClientMeshPtr();
            if (m.IsValid ())
...
static void AddSimplestSolidPrimitives (bvector<IGeometryPtr> &data, bool capped, uint32_t maxPerType = 20)
    {
    ...
    AddCones (data, 1.0, 1.0, capped, maxPerType);
    AddCones (data, 1.0, 0.5, capped, maxPerType);
    AddCones (data, 0.5, 1.0, capped, maxPerType);

    // ====================
    AddBoxes (data, 1,1,10, capped, maxPerType);
    // ====================
    numThisType = 0;
    for (double sY : signs)
        for (double sX : signs)
...
Angle surfaceSweep)
    {
    auto arc = DEllipse3d::From (cx, cy, 0, r,0,0, 0,0,r,
        arcStart.Radians (), arcStart.Radians () - arcEnd.Radians());
    auto prim = ICurvePrimitive::CreateArc (arc);
    auto section = CurveVector::Create(CurveVector::BOUNDARY_TYPE_Open);
    section->push_back (prim);
    DgnRotationalSweepDetail detail
        (
        section,
        DPoint3d::From(0, 0, 0),
        DVec3d::From(0, 0, 1),
        surfaceSweep.Radians (),
        false
        );
...
/**
* Build a ClipPlaneSet for an L shape with one interior diagonal plane, 2 quads.
* @param ax long dimension to right
* @param ay long dimenison to top
* @param bx interior point x coordinate
* @param by interior point y coordinates
*/
static void BuildLShapedClipper(double ax, double ay, double bx, double by, ClipPlaneSetR clipper, bvector<DPoint3d> &rightLeg, bvector<DPoint3d> &upperLeg)
    {
    DPoint3d interiorPoint = DPoint3d::From(bx, by);
    rightLeg = {
        DPoint3d::From(0,0),
        DPoint3d::From(ax,0),
        DPoint3d::From(ax,by),
