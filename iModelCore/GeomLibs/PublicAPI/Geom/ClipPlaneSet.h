...
struct  ConvexClipPlaneSet : T_ClipPlanes
    ...
    GEOMDLLIMPEXP ConvexClipPlaneSet
        (
        DRange3dCR range,
    ...
    GEOMDLLIMPEXP bool ClipBoundedSegment
        (
        DPoint3dCR pointA,
        DPoint3dCR pointB,
        double &fraction0,
        ...
        ClipPlaneCP planeToSkip = nullptr
        ) const;

    //! Clip an unbounded line to this set of planes.
    GEOMDLLIMPEXP bool ClipUnBoundedSegment
        (
        DPoint3dCR pointA,
        DPoint3dCR pointB,
        double &fraction0,
    ...
    //! </ul>
    GEOMDLLIMPEXP ClipPlaneContainment ClassifyPointContainment
        (
        DPoint3dCP points,
        size_t nPoints,
   ...
   //! multiply [ax,ay,az,aw] * matrix
    ...
    //! (Being careful that if the plane is one of the planes of the set it is NOT used as a coincident clipper)
    ...
    //! Compute crossings of this set's planes with a curve primitive.
...
struct  ClipPlaneSet :  bvector <ConvexClipPlaneSet>
   ...
   //! multiply [ax,ay,az,aw] * matrix
    ...
    GEOMDLLIMPEXP void AppendIntervals
        (
    ...
    //! to a positive ClipPlaneSet and a mask (hole) ClipPlaneSet.
    ...
    //! Clip a polyface to a a positive ClipPlaneSet and a mask (hole) ClipPlaneSet.
    //! @param polyface [in] polyface to test
    //! @param clipSet [in] the positive clip set
    //! @param maskSet [in] the negative (holes) clip set
    ...
    //! Clip a polyface to a positive ClipPlaneSet, producing only facets and linework on the cut planes.
    //! @param polyface [in] polyface to test
    //! @param clipSet [in] the positive clip set
    //! @param cutSections [out] (optional) triangulations of sections that are closed loops
    //! @param lineStrings [out] (optional) raw section line strings
    ...
    //! <ul>
    //! <li> This is appropriate whether or not the polyface is closed.
    //! </ul>
    ...
    //! @return false if the input curve is invalid.
    GEOMDLLIMPEXP bool ClipPath
        (
        CurveVectorCR curve,
        bvector<CurveVectorPtr>* pClippedCurves = nullptr,
        bvector<CurveLocationDetailPair>* pClippedDetails = nullptr
        ) const;

    //! Clip any curve to this ClipPlaneSet.
    //! Note that the returned bool has nothing to do with whether or not the curve has been clipped.
    //! Curves or regions that are entirely inside the clipper return a clone of the input.
    //! Regions are only returned if the input satisfies IsAnyRegionType.
    //! @param curve [in] The CurveVector to be clipped.
    //! @param clippedCurves [out] array of clipped curves. clippedCurves is empty if the input is entirely clipped away;
    //! otherwise it is populated with new curves representing the portions of the curve/region that lie inside the clipper.
    //! @return false if the input curve is invalid.
...
DRange1d InitialDRange1d (double low = 0.0, double high = 1.0) const
    {
    return m_initialState ? DRange1d::From (low, high) : DRange1d::NullRange ();
    }
};
