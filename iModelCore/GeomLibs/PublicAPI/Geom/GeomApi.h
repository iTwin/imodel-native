...
DEFINE_REF_COUNTED_PTR(GeometryValidator);
...
//=======================================================================================
//! Check values within various geometry structures.
//! Base class tests for IsNaN in various methods.
//! But all are marked virtual so other test classes can drop in with geometry-specific fixup.
// @bsiclass
//=======================================================================================
struct  GeometryValidator : public RefCountedBase
    {
    private:
        GeometryValidator();
    public:
        static GEOMDLLIMPEXP GeometryValidatorPtr Create();
        GEOMDLLIMPEXP virtual bool IsValidDouble(double x);
        GEOMDLLIMPEXP virtual bool IsValidDouble(double x, double minValue, double maxValue);
        GEOMDLLIMPEXP virtual bool IsValidMetricGeoCoordinate(double x);
        GEOMDLLIMPEXP virtual bool IsValidCoordinate(double x);

        GEOMDLLIMPEXP virtual bool IsValidGeometry(DSegment3dCR segment);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(DEllipse3dCR arc);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(RotMatrixCR arc);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(TransformCR arc);
        GEOMDLLIMPEXP virtual bool IsValidGeometry (double const *data, uint32_t count);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(double const *data, uint32_t count, double minValue, double maxValue);
        GEOMDLLIMPEXP virtual bool IsValidRadians(double x);
        GEOMDLLIMPEXP virtual bool IsValidSweepRadians(double x);
        GEOMDLLIMPEXP virtual bool IsValidDegrees(double x);
        GEOMDLLIMPEXP virtual bool IsValidSweepDegrees(double x);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(DVec3dCR vector);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(DPoint3dCR point);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(DPoint2dCR point);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(bvector<DPoint3d> const &data);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(DPoint3d const *data, size_t count);
        // These dispatch (after immediate geometry.IsValidGeometry() check) to the respective
        // IsValidGeometry(GeometryValidatorCR) method.
        GEOMDLLIMPEXP virtual bool IsValidGeometry(IGeometryPtr const &geometry);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(ICurvePrimitivePtr const &geometry);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(ISolidPrimitivePtr const &geometry);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(CurveVectorPtr const &geometry);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(MSBsplineSurfacePtr const &geometry);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(MSBsplineCurvePtr const &geometry);
        GEOMDLLIMPEXP virtual bool IsValidGeometry(PolyfaceHeaderPtr const &geometry);
        //!
        //! <ul>
        //! <li> If validator is null, leave the inout alone, return immediately.
        //! <li> test validity of each IGeometryPtr in inout array.
        //! <li> if invalid:
        //!     <ul>
        //!     <li> remove from the inout array.
        //!     <li> if invalidGeometry array is given, save the invalid onces there.
        //!     </ul>
        //! <li> if valid, retain it (possibly at lower index if prior entries were invalid)
        //! </ul>
        //!
        GEOMDLLIMPEXP static void RemoveInvalidGeometry (
        ...
        //! <li> If considered valid, record (if possible) in the validGeometry array.
        //! <li> If considered invalid, record (if possible) in the invalidGeometry array.
        //! <li> If the geometry is valid, return true;
        //! </ul>
        GEOMDLLIMPEXP static bool ValidateAndAppend(
            GeometryValidatorPtr &validator,
            IGeometryPtr &g,
        ...
        //! <li> If considered valid, record (if possible) in the validGeometry array.
        //! <li> If considered invalid, record (if possible) in the invalidGeometry array.
        //! <li> If ALL geometry is valid, return true;
        //! </ul>
        GEOMDLLIMPEXP static bool ValidateAndAppend(
            GeometryValidatorPtr &validator,
...
void Set (T const &value, size_t tagA) { m_value = value; m_tagA = tagA;}
...
GEOMDLLIMPEXP bool IsValidGeometry (GeometryValidatorPtr &validator);
...
#include "capi/fpoint2d_capi.h"
#include "capi/fpoint3d_capi.h"
#include "capi/frange2d_capi.h"
...
#include "capi/polyline2d_capi.h"
#include "capi/polyline3d_capi.h"
#include "capi/polyplane4d_capi.h"
#include "capi/polysolv_capi.h"
#include "capi/proximitydata_capi.h"
...
//=======================================================================================
//! tagged data with arrays of integer, double, point, and vector data.
//! User code can use multiple values within an instance, or multiple instances, as appropriate
//! to their domain.
//=======================================================================================
struct TaggedNumericData
    ...
    struct SubdivisionMethod
        {
        private: SubdivisionMethod() {}       // no instances
        public:
            static const int32_t ChooseBasedOnFacets = 0;
            static const int32_t CatmullClark = 1;
            static const int32_t Loop = 2;
            static const int32_t DooSabin = 3;
        };
    struct SubdivisionControlCode
        {
        private: SubdivisionControlCode() {}       // no instances
        public:
            static const int32_t FixedDepth = -100; // Paired value: (FixedDepth, value)
            static const int32_t AbsoluteTolerance = -101;  // Paired value: (FixedDepth, indexInDoubleData)
            static const int32_t FractionOfRangeBoxTolerance = -102;  // Paired value: (FixedDepth, indexInDoubleData)
            static const int32_t BoundaryAction = -103;     // Paired value: (BoundaryAction, 0 or 1: See PolyfaceHeader::CloneSubdivided)
        };
    int32_t m_tagA;
    int32_t m_tagB;
    bvector<int32_t> m_intData;
    bvector<double> m_doubleData;
    TaggedNumericData(int32_t tagA, int32_t tagB) :
        m_tagA(tagA),
        m_tagB(tagB)
        {
        }
    ...
    // Ask if tagA has a given value .. just for easy testing.
    bool IsTagA (int value) const {return m_tagA == value;}

    // requires match of counts and item-by-item in order given.
    // a nullptr param is considred "same" as an IsZero () param.
    GEOMDLLIMPEXP static bool AreSameStructureAndGeometry
        (
        TaggedNumericData const *dataA,
        TaggedNumericData const *dataB,
    ...
    // Test if the structure has empty arrays and both tags are zero.
    GEOMDLLIMPEXP bool IsZero () const;
    // Append 2 ints to the int array (for easy support of <id,value> pairs)
    GEOMDLLIMPEXP void PushInts (int value0, int value1);
    // Append a double to the double array.   Push <value0, index> on the int array.
    GEOMDLLIMPEXP void PushIndexedDouble(int value0, double doubleValue);
    // Search the int array in (targetTag,value) pairs.
    // On first appearance of targetTag, return the value clamped to minValue, maxValue.
    // If none found, return defaultValue.
    GEOMDLLIMPEXP int32_t TagToInt(int32_t targetTag, int32_t minValue, int32_t maxValue, int32_t defaultValue) const;
    // Search the int array in (targetTag,value) pairs.
    // On first appearance of targetTag, use the value as index into the double array;
    //              return the value clamped to minValue, maxValue.
    // If none found, return defaultValue.
    GEOMDLLIMPEXP double TagToDouble(int32_t targetTag, double minValue, double maxValue, double defaultValue) const;
    };
...
#include <Geom/TransformedDRange3d.h>
#ifndef SmallGeomLib
