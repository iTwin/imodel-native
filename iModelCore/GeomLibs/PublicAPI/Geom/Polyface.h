...
//! Clear this vector.   Append up to n values from source, with first source index i0.   Replicate leading numWrap values at end.
    GEOMDLLIMPEXP uint32_t ClearAndAppendBlock (BlockedVector<T> &source, uint32_t i0, uint32_t n, uint32_t numWrap);
//! Clear this vector.   Append up to n values from source, with first source index i0.   Replicate leading numWrap values at end.
    GEOMDLLIMPEXP uint32_t ClearAndAppendBlock (T const *source, size_t sourceSize, uint32_t i0, uint32_t n, uint32_t numWrap);
...
//! return a (const) pointer to the numeric data tags.
GEOMDLLIMPEXP TaggedNumericData const *GetNumericTagsCP() const;
//! set the numeric data tags.
//! Note that for a PolyfaceQueryCarrier, this sets in the carrier but not the original data.
GEOMDLLIMPEXP void SetNumericTags(TaggedNumericData const & source);
...
//! Try to access int color through a readIndex in the colorIndex array.
GEOMDLLIMPEXP bool                          TryGetIntColorAtReadIndex (size_t readIndex, uint32_t& data) const;
//! Try to access facet face data through a readIndex in the colorIndex array.
...
//! @param polyfaceQuery const reference to the target polyface
GEOMAPI_VIRTUAL StatusInt   _ProcessUnclippedPolyface (PolyfaceQueryCR polyfaceQuery) = 0;
//! @description Called (once) if the target polyface is clipped by the clipper.
//! @param polyfaceHeader the polyface with clipping applied
...
//!  @description Fast clustered vertex decimator - used during tile generation.
GEOMDLLIMPEXP PolyfaceHeaderPtr ClusteredVertexDecimate (double tolerance, double minCompressionRatio = .5, bool processNormals = true) const;
...
//! If the polyface's numeric tags request it, compute and return the alternate polyface (e.g. decimated or subdivided)
//! compute and return it.
GEOMDLLIMPEXP static PolyfaceHeaderPtr  ComputeAlternatePolyface
(
...
private:
    ...
    uint32_t            m_expectedClosure;
    TaggedNumericData  m_numericTags;
public:
...
//! Set data and index pointers to null, count to 0
GEOMDLLIMPEXP void RemoveNormals();
//! Set data and index pointers to null, count to 0
GEOMDLLIMPEXP void RemoveParams();
//! Set data and index pointers to null, count to 0
GEOMDLLIMPEXP void RemoveColors();
...
uint32_t            _GetExpectedClosure()       const override { return m_expectedClosure; }

//! get (a pointer to !!) extra geometry array . . .
GEOMAPI_VIRTUAL TaggedNumericData const *_GetNumericTagsCP() const override {
    return m_numericTags.IsZero () ? nullptr : &m_numericTags;
    }
GEOMAPI_VIRTUAL void _SetNumericTags (TaggedNumericData const & source) override {
    m_numericTags = source;
    }
};
// Facet Factoids:
...
uint32_t                _GetExpectedClosure()    const override;
PolyfaceVectors *       _AsPolyfaceVectorsP() const override; // fast upcast

TaggedNumericData const *_GetNumericTagsCP() const override;
void _SetNumericTags(TaggedNumericData const &source) override;
...
//! Negative tolerances indicate using (tight) defaults
GEOMDLLIMPEXP void Compress(double pointAbsTol, double normalAbsTol = -1.0, double paramAbstol = -1.0);
...
//! <ul>
//! <li> 0 = measure radius to vertices
//! <li> 1 = radius to midedge, orient with x axis to mid-edge (see 101)
//! <li> 2 = radius to midface
//! <li> 100 = radius to vertex, but orient cube and octahedron with square cross section in xy plane.
//! <li> 101 = radius to midedge, but orient cube and octahedron with square cross section in xy plane.
//! <li> 102 = radius to midface, but orient cube and octahedron with square cross section in xy plane.
//! </ul>
//! The orientation is as follows:
//! <ul>
//! <li> For selection 100, 101 and 102 for cube and octahedron, a square cross section is parallel to the xy plane.
//! <li> For all other cases (0,1,2 for all polyhedra, and 101,101,102 for other than cube and octahedron)
//!      <li> A vertex, mid-edge, or centroid point (respective to the measurement) is on the positive x axis
//!      <li> Another edge of an incident face is in the y direction.
//!      <li> Hence when viewed from positive x towards center, there will be a horizontal edge
//!      <li> on a face with a vertex on the x axis.
//! </ul>
//! @param [in] radius radius of sphere
//! @param [in] polyhedronSelect integer selector, as enumerated above.
//! @param [in] radiusSelect control of sphere radius, as enumerated above.
//! @param [in] transform optional transform to place the sphere.
GEOMDLLIMPEXP static PolyfaceHeaderPtr CreateRegularPolyhedron
(
double radius,
int polyhedronSelect,
int radiusSelect = 0,
TransformCP transform = nullptr
);
/*---------------------------------------------------------------------------------**//**
...
* @param [in] transform optional transform to place the sphere.
+---------------+---------------+---------------+---------------+---------------+------*/
GEOMDLLIMPEXP static PolyfaceHeaderPtr CreateRegularPolyhedronWithSubtriangulation
(
    double                  radius,
    int                     polyhedronSelect,
...
//!  @description Apply a subdivsion method to create a denser, smoother mesh.
//! <ul>
//!   <ul>
//!   <li>subdivisionMethod=1 is Catmull-Clark.
//!     <ul>
//!     <li>This can be applied to any mix of triangles, quads, or other convex facets.
//!     <li>Output is entirely quads
//!     <li>Each output quad has vertices near to:
//!         <ul>
//!         <li>A mid-edge point
//!         <li>a vertex
//!         <li>then next mid edge point
//!         <li>the prior centroid
//!         </ul>
//!      </ul>
//!   <li> subdivisionMethod=2 is "Loop"
//!     <ul>
//!     <li>This should only be used when input is entirely triangles
//!     <li>The mesh will be triangulated if there are non-triangles in the input.
//!     <li>Output is entirely triangles
//!     <li>There are output vertices
//!         <ul>
//!         <li>near each input vertex
//!         <li>near each mid-edge
//!         </ul>
//!      </ul>
//!   <li> subdivisionMethod=3 is "DooSabin"
//!     <ul>
//!     <li>This can be applied to any mix of triangles, quads, or other convex facets.
//!     <li>Output has a mixture of vertices-per-facet.
//!     <li>Each output facet can be seen as one of
//!         <ul>
//!         <li>a shrunken version of a corresponding input facet
//!         <li>a quad "along" each input edge
//!         <li>an area "around" each input vertex, with one output facet edge per incoming edge.
//!         </ul>
//!      </ul>
//!      <li> (6/21) DooSabin does not support unchanged boundary -- the boundary always moved inward.
//!   </ul>
//! <li> subdivisionDepth is limited to 4, i.e. each facet splits to at most 256.
//! <li> If subdivisionDepth is 0, the result is just a clone of the input.
//! <li>boundary action affects how boundary edges (of non-closed meshes) are handled.
//!    <ul>
//!    <li> 0 holds them unchanged
//!    <li> 1 allows the subdivision mesh to move inward.
//!    </ul>
//! </ul>
...
GEOMDLLIMPEXP static PolyfaceHeaderPtr CreateDRange3dFaces
(
DRange3dCR range,               //!< [in] points to triangulate
bool triangulate = false,       //!< [in] if true, add a (hidden) diagonal in each face
TransformCP placement = nullptr //!< [in] optional transform to apply
);
...
//! Builder object for polyface meshes, using integer grids for points, params, and normals.
struct LightweightPolyfaceBuilder : RefCountedBase
{
    struct QPoint3d
        ...
        QPoint3d(int64_t x, int64_t y, int64_t z);
...
public:
    //! Construct a builder with specified grid sizes for point, param, and normal data.
    ...
    //! Find or add a point, searching 8 blocks nearest the point.
    GEOMDLLIMPEXP size_t  FindOrAddPointCluster(DPoint3dCR point);
