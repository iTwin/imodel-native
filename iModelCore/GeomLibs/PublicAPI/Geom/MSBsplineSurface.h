...
struct GEOMDLLIMPEXP MSBsplineSurface
    ...
    //! Return the (UNTRIMMED) surface range in its principal axis coordinates.
    ...
    //! Return the (UNTRIMMED) surface range in global coordinates or after an optional tranform.
    DRange3d FullSurfaceRange
    (
    TransformCP optionalTransform = nullptr//! [in] optional transform to apply.
    ) const;

    //! Return the surface range in global coordinates or after an optional tranform.
    //! Expansion logic is:
    //! <ul>
    ...
    (
    TransformCP optionalTransform = nullptr,//! [in] optional transform to apply.
    double expansionFactor = 0.0,    // optional factor to apply to the range.   E.g. 0.1 to expand 10% in all directions
    bool restrictToPoleRange = true        //! [in] option to restrict to trim range (after applying the expansion factor)
    ) const;
...
//! Return the true allocated size of the pole array....
    ...
    //! @remark Caller must free with ClearBeziers.
    MSBsplineStatus MakeBeziers(bvector<bvector<MSBsplineSurface>>& beziers, bvector<bvector<DRange2d>> *knotRanges = nullptr) const;
    //! Free the series of Bezier surfaces created by MakeBeziers.
    static void ClearBeziers(bvector<bvector<MSBsplineSurface>>& beziers);
