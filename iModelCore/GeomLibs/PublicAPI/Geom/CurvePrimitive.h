...
GEOMAPI_VIRTUAL bool _IsValidGeometry (GeometryValidatorPtr &validator) const = 0;
...
//! Dispatch to _IsValidGeometry(validator) virtual
//! <ul>
...
//! Allocate and fill a spiral curve.
//! @param [in] startRadians bearing at start
//! @param [in] startRadius radius at start (or 0 of flat)
//! @param [in] endRadians bearing at end
...
//! @param [in] fractionA start fraction for active portion of curve
//! @param [in] fractionB end fraction for active portion of curve
//! @praam [in] extraData type-specific extra data.
...
//! Same as CreateSpiralBearingRadiusLengthRadius, but no extraData.
GEOMDLLIMPEXP static ICurvePrimitivePtr CreateSpiralBearingRadiusLengthRadius
(int transitionType, double startRadians, double startRadius, double length, double endRadius, TransformCR frame, double fractionA, double fractionB);
...
// (These are very close to true radii from derivative evaluations)
// This function is called by CreatePseudolSpiralWithTrueRadiusLengthRadius for the RailCorp spiral
GEOMDLLIMPEXP static ICurvePrimitivePtr CreateAustralianRailCorpBearingRadiusLengthRadius (
      DPoint3dCR startPoint,
      double startRadians,
      double startRadius,
      double targetLength,
...
/*
* * Set a modal flag which selects implementation of spiral curve primitives.
...
*        * higher accuracy length and distance-to-point evaluations based on the spiral integrations.
* * This setting is retained modally.
* * The prior setting is returned for callers to restore after making a selection for temporary use.
*/
GEOMDLLIMPEXP static int SelectSpiralImplementation (int selector);
