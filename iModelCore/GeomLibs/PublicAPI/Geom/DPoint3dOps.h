...
//! Test whether pointD is inside the circle through pointA, pointB, pointC
//! @param [in] pointA first pass-through point of circle
//! @param [in] pointB second pass-through point of circle
//! @param [in] pointC second pass-through point of circle
//! @param onIsIn [in] if true, treat a small fuzz area beyond the cirdle as in.   If false, no fuzz area.
//! @param determinant [out] determinant scaled by (signed) area.  The units of this are length to the fourth power.
//! @param refArea [out] reference length squared quantity for zero tests.
//! @return true if pointD is inside the circle.
static bool GEOMDLLIMPEXP InCircle(DPoint2d const &pointA, DPoint2d const &pointB, DPoint2d const &pointC, DPoint2d const &pointD, bool onIsIn, double &determinant, double &refArea);
//! Short form of InCircle.  All parameters match, but this short form does not ahve the determinant and cross product return values.
static bool GEOMDLLIMPEXP InCircle(DPoint2d const &pointA, DPoint2d const &pointB, DPoint2d const &pointC, DPoint2d const &pointD, bool onIsIn = true);
//! Compute the vector from the origin to the center of a circle passing through the origin and two points given as vectors from the origin.
//! If the vectors are parallel, return false with cx=cy=0;
static bool GEOMDLLIMPEXP CircleCenterVector (double ux, double uy, double vx, double vy, double &cx, double &cy);
//! Compute the center of a circle through 3 points.
//! If the points are colinear, return false with pointA as center.
static bool GEOMDLLIMPEXP CircleCenterThrough3Points(DPoint2dCR pointA, DPoint2dCR pointB, DPoint2dCR pointC, DPoint2dR center);
//! Return the signed distance from spacePoint to the circle through pointA, pointB, pointC, pointD.
//! <ul>
//! <li>This is zero if spacePoint is on the circle.
//! <li>This is negative if spacePoint is inside
//! <li>This is positive if spacePoint is outside.
//! </ul>
//! Returns invalid distance if the three points are colinear.
static GEOMDLLIMPEXP ValidatedDouble SignedDistanceToCircleThrough3Points(DPoint2dCR pointA, DPoint2dCR pointB, DPoint2dCR pointC, DPoint2dR spacePoint);
};
...
static GEOMDLLIMPEXP double LargestCoordinate (DPoint3dCR dataA, DPoint3dCR dataB);
...
* <ul>
* <li> The hull starts at the point most distant from the centroid
* <li> Hull points proceed counterclocwise.
* <li> The hull optionally repeats its first point at the back for closure.
* <li> In the degenerate case of (a) there is only a single point and (b) includeClosurePoint is true, the single point is NOT repeated.
* </ul>
*/
static GEOMDLLIMPEXP void ConvexHullXY(bvector<DPoint3d> const &points, bvector<DPoint3d> &hullPoints, bool includeClosurePoint = false);
//! Test whether pointD is inside the circle through pointA, pointB, pointC (using only XY coordinates)
//! @param [in] pointA first pass-through point of circle
//! @param [in] pointB second pass-through point of circle
//! @param [in] pointC second pass-through point of circle
//! @param onIsIn [in] if true, treat a small fuzz area beyond the cirdle as in.   If false, no fuzz area.
...
//! @param refArea [out] reference length squared quantity for zero tests.
//! @return true if pointD is inside the circle.
static bool GEOMDLLIMPEXP InCircleXY(DPoint3d const &pointA, DPoint3d const &pointB, DPoint3d const &pointC, DPoint3d const &pointD, bool onIsIn, double &determinant, double &refArea);
...
//! (See the longer arg list for notes)
...
//!   <li> edgeIndex and edgeIndex + 1 are always good indices.
//!   </ul>
//! <li> For the closed case
//!  <ul>
//!  <li> numEdge remains (xyz.size() - 1)
//!  <li> The index of the edge base is still edgeIndex, but can be one larger, i.e. up to xyz.size () - 1
//!  <li> the index of the edge head is ((edgeIndex + 1) % xyz.size ())
//!  <li> fraction 1.0 is still the end of the REAL polyline, not the wrapped point.
//!  </ul>
//! <li> That is, the "closure" is treated as a special case but all fraction properties within the polyline itself are not altered.
//! <ul>
public: static GEOMDLLIMPEXP bool ClosestPointXY
        (
        bvector<DPoint3d> const &xyz,  //!< [in] points of the polyline
        bool addClosurePoint,           //!< If true, consider a closure edge in the search
        DPoint3dCR spacePoint,          //!< [in] point to project to the polyline
        DMatrix4dCP worldToLocal,       //!< [in] optional transform to the xy plane for the search
        double &globalFraction,         //!< [out] global fraction. See note above regarding fraction
        DPoint3dR curvePoint,           //!< [out] point on the polyline
        size_t &edgeIndex,              //!< [out] index of point at base of the edge.
        size_t &numEdge,                //!< [out] number of REAL edges.  This is ALWAYS (xyz.size () - 1.
        double &edgeFraction,           //!< [out] fraction along edge
        double &xyDistance,             //!< [out] projected distance
        bool extend0,                   //!< [out] true to extend first segment backwards. Ignored if addClosurePoint is true
        bool extend1                    //!< [out] true to extend last segment forwards.  Ignored if accClosurePoint is true.
        );
...
//! <li> Otherwise find indices i where pointA[i] and pointsB[i] are each (within their own array) colinear
//!          with surrounding points.
//! <li> Eliminate these points.
//! <li> Note that this is analogous to CompressColinearPoints, but requires simultaneous
//!     conditions on pointA[i] and pointB[i], hence maintaining total count match and partners.
//! </ul>
...
/**
* A crude algorithm to excise "bowties":
*  * Search forward from each for self intersections.
*  * Treat the intersection as a cut point.
*  * Return the main chain inprimaryChain.
*  * Push each bowTie onto bowTies.
* @param points [in] a polyline which may have "bowties" -- undesired small cutbacks (aka lobes)
* @param primaryChain [out] a polyline with the bowties excised.
* @param bowTies [out] an array containing all the bowties
*/
static GEOMDLLIMPEXP void SplitAtBowTies
(
bvector<DPoint3d> const &points,
bvector<DPoint3d> &primaryChain,
bvector<bvector<DPoint3d>> &bowTies
);
/**
* Execute PolylineOps::SplitAtBowTies one or more times to trim bowties.
* <ul>
*  <li>This narrows down to a (heurestically) best offset choice
*  <li>If favorLongestPart is false, only the primaryChain from each call is accepted.
*     This may cause closed or nearly-closed paths to disappear.
* </ul>
* @param points [in] a polyline which may have "bowties" -- undesired small cutbacks (aka bowties)
* @param correctedPoints [out] a polyline with the bowties excised.
* @param favorLongestPart [in] If true, the longest path is chosen from each step.
*/
static GEOMDLLIMPEXP void MultiPassSplitAtBowTies
(
bvector<DPoint3d> const &points,
bvector<DPoint3d> &correctedPoints,
bool favorLongestPart = true
);
...
//! Find common points at ends of segments and assemble to linestrings.
static GEOMDLLIMPEXP void AssemblePolylines(bvector<DSegment3d> const &segments, bvector<bvector<DPoint3d>> &linestrings);

//! Greedy triangulation between 2D linestrings.
//! <ul>
//! <li>Designed for case where each linestring is simply a sequence of points along a single straight line.
//! <li>But will probably work well for cases with not-to-much wandering.
//! </ul>
//! @param pointA [in] first set of points
//! @param indexA [in] integer indices fir pointA
//! @param pointB [in] second set of points
//! @param indexB [in] integer indices for pointB
//! @param triangleIndices [out] optional array where x,y,z are indices taken from indexA and indexB for each triangle.
//! @param trianglePoints [out] optional array of triangle coordinates (at z = 0)
static GEOMDLLIMPEXP void GreedyTriangulateBetweenLinestringsWithIndices
(
bvector<DPoint2d> &pointA,
bvector<int> &indexA,
bvector<DPoint2d> pointB,
bvector<int> &indexB,
bvector<Point3d> *triangleIndices,
bvector<DTriangle3d> *trianglePoints
);

//! Test if an array of points is colinear with the bounded line segment from first to last point.
//! @param points => polyline to test
//! @param numPoints => point count of polyline
//! @param absTol => absolute tolerance for distance test, or nonpositive to use DoubleOps::SmallCoordinateRelTol().
//! @param relTol => relative tolerance for distance test, or nonpositive to use DoubleOps::SmallCoordinateRelTol().
//! @return true if all points are within tolerance of the bounded line segment from first to last point.
static GEOMDLLIMPEXP bool IsColinear(DPoint3dCP points, size_t numPoints, double absTol = 0, double relTol = 0);
...
//! @param relTol => relative tolerance for distance test, or nonpositive to use DoubleOps::SmallCoordinateRelTol().
//! @return true if all points are within tolerance of the bounded line segment from first to last point.
static GEOMDLLIMPEXP bool IsColinear(bvector<DPoint3d> const& points, double absTol = 0, double relTol = 0);
};
...
//! Compute the symmetry axes of a polygon in the XY plane.
//! @return false if no symmetry axes are found.
//! @param [out] axes symmetry axes
//! @param [in] points Ordered polygon
//! @param [in] tol tolerance
public: static GEOMDLLIMPEXP bool SymmetryAxesXY (bvector<DRay3d>& axes, bvector<DPoint3d> const& points, double const& tol = 1.0e-3);

//! Compute the rotation matrix according to symmetry axes, if they exist.
//! @return false if no symmetry axes are found.
//! @param [out] matrix Smallest angle rotation matrix transforming the reference axis to one of the symmetry axes
//! @param [in] points Ordered polygon
//! @param [in] referenceAxis reference axis
//! @param [in] tol tolerance
public: static GEOMDLLIMPEXP bool SymmetryAxesXY (RotMatrixR matrix, bvector<DPoint3d> const& points, DVec3dCR referenceAxis, double const& tol = 1.0e-3);
...
//! Structure to accumulate weighted sum of DPoint3ds.
//!
//! Usage cycle:
//!  <ul>
...
//!     </ul>
//!  <li> (after all components area added) GetWeightedAverage (
//!  </ul>
struct DPoint3dWeightedSum
{
private:
    DPoint3d m_xyz;
    double m_w;
public:
    // Constructor ...
    DPoint3dWeightedSum (){Zero(); }
    //! Clear the coordinates and weights to zero.
    void Zero()
        {
        m_xyz.Zero();
        m_w = 0.0;
        }
    //! Accumulate a single point and weight (point coordinates are multiplied by given w)
    void AddWeighted(DPoint3dCR xyz, double w = 1.0)
        {
        m_xyz.x += w * xyz.x;
        m_xyz.y += w * xyz.y;
        m_xyz.z += w * xyz.z;
    ...
    //! Accumulate many points with unit weight.
    void AddUnitWeight(bvector<DPoint3d> const &data)
        {
        for (auto &xyz : data)
            {
            m_xyz.x += xyz.x;
    ...
    //! Accumulate one point with unit weight.
    void AddUnitWeight(DPoint3dCR xyz)
        {
        m_xyz.x += xyz.x;
        m_xyz.y += xyz.y;
        m_xyz.z += xyz.z;
    ...
    //! Copy the weighted point to xyz.
    //! If weight is nonzero, divide xyz by the weight and return true.
    //! If weight is zero, let xyz as is and return false.
    bool GetWeightedAverage(DPoint3dR xyz)
        {
        xyz = m_xyz;
        if (m_w == 0.0)
            return false;
    ...
    //! Test if the weight is strictly non-zero.
    //! Note: This is a simple bitwise nonzero test.
    bool HasNonZeroWeight() const { return m_w != 0.0; }
    //! Return the current weight sum.
    double GetW () const {return m_w;}
    //! Return the current weighted xyz sum. (WITHOUT deweighting!!)
