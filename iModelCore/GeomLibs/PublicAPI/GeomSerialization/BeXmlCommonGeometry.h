/*--------------------------------------------------------------------------------------+
|
|     $Source: PublicAPI/GeomSerialization/BeXmlCommonGeometry.h $
|
...
public:
    //! Try to read a curve primitive
    //! @return false if this node is not a curve primitive.
    static GEOMLIBS_SERIALIZATION_EXPORT bool TryParse (Utf8CP beXmlCGString, ICurvePrimitivePtr &result);
    
    //! Try to read a solid primitive
    //! @return false if this node is not a solid primitive.
    static GEOMLIBS_SERIALIZATION_EXPORT bool TryParse (Utf8CP beXmlCGString, ISolidPrimitivePtr &result);

    //! Try to read a bspline surface
    //! @return false if this node is not a bspline surface
    static GEOMLIBS_SERIALIZATION_EXPORT bool TryParse (Utf8CP beXmlCGString, MSBsplineSurfacePtr &result);

    //! Try to read any geometry types, optionally recursing through the xml tree
    //!    to find geometry items.
    //! @param [in] beXmlCGString string containing the serialized IGeometry object in BeXml.
    //! @param [in,out] geometry to receive geometry.
    //! @param [in] maxDepth max number of recursions for search.  0 means only look at the node itself.
    //! @return true if any geometry found
    static GEOMLIBS_SERIALIZATION_EXPORT bool TryParse (Utf8CP beXmlCGString, bvector<IGeometryPtr> &geometry, size_t maxDepth = INT_MAX);
...
public: static GEOMLIBS_SERIALIZATION_EXPORT bool TryParse (Utf8CP beXmlCGString, bvector<IGeometryPtr> &geometry, bmap<OrderedIGeometryPtr, BeExtendedData> &extendedData, size_t maxDepth = INT_MAX);
        static GEOMLIBS_SERIALIZATION_EXPORT bool TryParse (::Byte* buffer, int bufferLength, bvector<IGeometryPtr> &geometry, BeExtendedDataGeometryMap &extendedDataMap, size_t maxDepth = INT_MAX);
};
...
public:
    ...
    GEOMLIBS_SERIALIZATION_EXPORT static void Write (Utf8StringR cgBeXml, IGeometryCR data, bmap<OrderedIGeometryPtr, BeExtendedData> *extendedData = NULL, bool preferCGSweeps = false);

    //! Write common geometry binary xml (Microsoft .net) for an IGeometryPtr object
    GEOMLIBS_SERIALIZATION_EXPORT static void WriteBytes (bvector<::Byte>& bytes, IGeometryCR data, bmap<OrderedIGeometryPtr, BeExtendedData> *extendedData = NULL, bool preferCGSweeps = false);
