...
struct ScopedMask
    ...
    void ClearInGraph (){m_graph.ClearMask (m_mask); }
...
// delete all nodes and associated coordinate data.
// PRESERVE label setup and normal mode.
void ClearGraphAndArrays ();
...
#define MTG_NODE_FACE_TAG           (-10005)
...
// MTGFacets used for "two sided" cellular topology.
struct MTGDoubleFaceFacets : MTGFacets
    {
    int m_partnerLabel;
    double m_newFaceVertexTolerance;
    int m_modalFaceValue;
    int m_modalFaceLabel;
    int m_defaultModalFaceValue;
    // Constructor sets up label data immediately ...
    GEOMDLLIMPEXP MTGDoubleFaceFacets(double newFaceVertexTolerance = DoubleOps::SmallMetricDistance ());
    ...
    // return prior modal value (e.g. to restore if needed);
    GEOMDLLIMPEXP int SetModalFaceValue (int value);
    // Add a face with distinct loops of nodes on the two sides (joined through the partner label)
    GEOMDLLIMPEXP MTGNodeId AddDoubleSidedFace (bvector<DPoint3d> const &xyz);

    // Access the partner node "on the other side of the face"
    GEOMDLLIMPEXP MTGNodeId GetPartnerNode (MTGNodeId node) const;
    // Access the modal face label of this node
    GEOMDLLIMPEXP int GetFaceValueInNode(MTGNodeId node) const;
