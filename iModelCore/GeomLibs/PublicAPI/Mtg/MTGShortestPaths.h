...
#define Compile_MarkShortestPathBetweenLoops
    ...
    public:
        ...
        void TraceAndUnmarkEdges (MTGNodeId seedNode, MTGMask pathMask, MTGMask edgeClearMask, bvector<MTGNodeId> &reversePath, bvector<MTGNodeId> &growingForwardPath, bool pushMate = true)
            {
            reversePath.clear ();
            MTGNodeId nodeAlongPath = seedNode;
            for (; nodeAlongPath != MTG_NULL_NODEID;)
                {
                nodeAlongPath = m_graph->FindMaskAroundVertex(nodeAlongPath, pathMask);
                if (nodeAlongPath != MTG_NULL_NODEID)
                    {
                    reversePath.push_back (nodeAlongPath);
                    m_graph->ClearMaskAt (nodeAlongPath, pathMask);
                    nodeAlongPath = m_graph->FSucc (nodeAlongPath);
                    }
                }
