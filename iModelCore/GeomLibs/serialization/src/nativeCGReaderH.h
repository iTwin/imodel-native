/*--------------------------------------------------------------------------------------+
|
|  $Source: serialization/src/nativeCGReaderH.h $
|
...
/// </summary>
/// <returns></returns>
bool ReadIPartialCurve (IGeometryPtr &result)
    {
    result = nullptr;
    if (CurrentElementNameMatch ("PartialCurve")
        && ReadToChildOrEnd ())
        {
        ...
        for (;IsStartElement ();)
            {
            if (ReadTagdouble ("fraction0", detail.fraction0))
                continue;
            if (ReadTagdouble ("fraction1", detail.fraction1))
                continue;
            if (ReadTag_AnyICurvePrimitive ("parentCurve", detail.parentCurve))
                continue;
...
/// </summary>
/// <returns></returns>
bool ReadIDgnCurveVector (IGeometryPtr &result)
    {
    result = nullptr;
    if (CurrentElementNameMatch ("DgnCurveVector")
        && ReadToChildOrEnd ())
        {
        ...
        for (;IsStartElement ();)
            {
            if (ReadTagint ("boundaryType", detail.boundaryType))
                continue;
