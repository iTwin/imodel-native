...
int32_t MSXmlBinaryReader::ValueHandle::GetInt8()
    {
    return (char) (m_buffer[m_offset]);
    }

int32_t MSXmlBinaryReader::ValueHandle::GetInt16()
    {
    return (int16_t)(m_buffer[m_offset] + (m_buffer[m_offset + 1] << 8));
    }

int32_t MSXmlBinaryReader::ValueHandle::GetInt32()
    {
...
int64_t MSXmlBinaryReader::ValueHandle::GetInt64()
    ...
    int64_t lo = (uint32_t)(((((b4 << 8) + b3) << 8) + b2) << 8) + b1;
...
int32_t MSXmlBinaryReader::ValueHandle::ToInt()
    {
...
void MSXmlBinaryReader::MoveToEndElement()
    {
...
MSXmlBinaryReader::XmlNode::XmlNode(XmlNodeType nodeType, Byte* buffer, uint32_t nodeFlags) : m_value(buffer), m_nodeType(nodeType)/*, unused m_isEmptyElement(false)*/
    ...
    m_canGetAttribute = ((nodeFlags & (uint32_t) XmlNodeFlags::CanGetAttribute) != 0);
    m_canMoveToElement = ((nodeFlags & (uint32_t)XmlNodeFlags::CanMoveToElement) != 0);
