...
void BeCGWriter::WriteSpiral (struct DSpiral2dPlacement const &spiralPlacement)
    ...
    _Analysis_assume_(spiralPlacement.spiral != nullptr);
...
void BeCGWriter::WriteTextPlacement
    ...
    Indent (dest); WriteText ("textString", data);
    Indent (dest); WriteText ("fontName", "ARIAL");
    Indent (dest); WriteDouble ("characterXSize", charSize);
    Indent (dest); WriteDouble ("characterYSize", charSize);
    Indent (dest); WriteDouble ("slantAngle", 0.0);
            ...
            if (m_compactCurveVectors)
                ...
                for(ICurvePrimitivePtr curve : curveVector)
                    ...
                    if (nullptr != child && child->size () > 0)
                        ...
                        for(ICurvePrimitivePtr childCurve : *child)
                            {
                            m_dest.WriteArrayMemberStart ();
                            Write (*childCurve);
                            m_dest.WriteArrayMemberEnd ();
