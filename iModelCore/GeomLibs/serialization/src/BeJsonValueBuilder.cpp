...
struct BeCGJsonValueWriter : IBeStructuredDataWriter
    ...
    struct StackFrame
        {
        ElementType m_type;
        // Held by pointer because BeJsDocument is move-only and owns the allocator its views point into.
        std::unique_ptr<BeJsDocument> m_value;
        Utf8CP m_name;  // only for debugging?
        // A new stack frame starts with an intended type and a blank value
        StackFrame (ElementType type, Utf8CP name) : m_type (type), m_value (std::make_unique<BeJsDocument> ()), m_name (name) {}
        };
    bvector<StackFrame> m_stack;
    bvector<std::unique_ptr<BeJsDocument>> m_values;
    BeJsDocument m_discard; // sink for values that have nowhere to go
    ...
    //! Returns the slot in the top-of-stack container into which a named value should be written.
    BeJsValue SlotForNamedValue (Utf8CP name, bool nameOptional)
        {
        if (m_stack.empty ())
            {
            m_values.push_back (std::make_unique<BeJsDocument> ());
            return (*m_values.back ())[name];
            }
        StackFrame &tos = m_stack.back ();
        if ((nullptr == name || nameOptional) && tos.m_type == ElementType::Array)
            return tos.m_value->appendValue ();
        if (tos.m_type == ElementType::Set)
            return (*tos.m_value)[name];
        if (tos.m_type == ElementType::Array)
            return tos.m_value->appendValue ()[name]; // put the named value in a singleton set to push as a positional array member.
        Error ("No container for named value");
        m_discard.SetNull ();
        return m_discard;
        }

    void AddNamedValueToTOS (Utf8CP name, BeJsConst value, bool nameOptional)
        {
        SlotForNamedValue (name, nameOptional).From (value);
        }
    ...
    public: BeJsonStatus GrabValue (BeJsValue value)
        {
        if (m_stack.size () != 0)
        ...
        if (m_values.size () == 0)
            ...
            for (size_t i = 0; i < m_values.size (); i++)
                value.appendValue ().From (*m_values[i]);
            }
        return Status ();
        }
    //! Pops the top of stack and returns it, or nullptr if the stack is empty or the name does not match.
    public: std::unique_ptr<BeJsDocument> TryPopStack (Utf8CP name = nullptr)
        {
        if (m_stack.size () == 0)
            {
        ...
        std::unique_ptr<BeJsDocument> value = std::move (m_stack.back ().m_value);
        m_stack.pop_back ();
        return value;
        }
    ...
    public: BeJsonStatus _WriteSetElementEnd (Utf8CP name) override
        {
        std::unique_ptr<BeJsDocument> topValue = TryPopStack (name);
        if (topValue != nullptr)
            {
    ...
    public: BeJsonStatus _WriteNamedSetEnd (Utf8CP name) override
        ...
        if (n >= 3
            ...
            {
            std::unique_ptr<BeJsDocument> topValue = TryPopStack ();
            TryPopStack (); // discard the empty placeholder
            if (topValue != nullptr)
                AddNamedValueToTOS (name, *topValue, false);
            }
    ...
    public: BeJsonStatus _WriteArrayElementEnd (Utf8CP longName, Utf8CP shortName) override
        ...
        std::unique_ptr<BeJsDocument> topValue = TryPopStack (longName);
        if (topValue != nullptr)
            {
    ...
    public: BeJsonStatus _WriteBlockedDoubles (Utf8CP itemName, bool itemNameOptional, double const *data, size_t n) override
        {
        BeJsDocument itemValue;
        for (size_t i = 0; i < n; i++)
            itemValue.appendValue () = data[i];
    ...
    public: BeJsonStatus _WriteNamedDouble (Utf8CP itemName, double data, bool optionalName) override
        {
        SlotForNamedValue (itemName, optionalName) = data;
        return Status ();
        }
...
bool BentleyGeometryJson::TryGeometryToJsonValue(BeJsValue value, bvector<IGeometryPtr> const &data, bool preferNativeDgnTypes)
    {
...
bool BentleyGeometryJson::TryGeometryToJsonValue (BeJsValue value, IGeometryCR data, bool preferNativeDgnTypes)
    {
    BeCGJsonValueWriter builder;
    BeExtendedDataGeometryMap extendedData;
    static bool s_compactCurvePrimitives = false;
