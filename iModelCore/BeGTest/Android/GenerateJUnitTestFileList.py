...
def GenJUnitTestFilesForDir (rootDir, dirNameIn, listfilename, javaTemplate, jniTemplate):
    ...
    if utPackageName == '' or len(utPackageName) == 0:
        utPackageName = 'Tests'
    ...
    if os.path.exists (tcFilePath):
        ...
        for utClassName in tcFile.readlines ():
            ...
            print ('     @"' + sys.executable + '" $(baseDir)SearchAndReplace.py  ' + junitTestFilePath + ' __REPO__ ' + utPackageName)
            print ('     @"' + sys.executable + '" $(baseDir)SearchAndReplace.py  ' + junitTestFilePath + ' __FIXTURE__ ' + utClassName)
            print ('     @"' + sys.executable + '" $(baseDir)SearchAndReplace.py  ' + junitTestFilePath + ' __ANDROIDJUNITTEST_SHARED_LIBRARIES__ "$(__ANDROIDJUNITTEST_SHARED_LIBRARIES__)"')
            print ('     ~time')
            print ('\n')
    ...
    print ('     @"' + sys.executable + '" $(baseDir)SearchAndReplace.py $(outputDir)' + unitTestsJniFileName   + ' __REPO__ ' + utPackageName)
    print ('     @"' + sys.executable + '" $(baseDir)SearchAndReplace.py $(outputDir)' + unitTestsJniFileName   + ' __FIXTURE__ ' + convertToJniName (utPackageName))
