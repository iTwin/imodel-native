...
def GenJUnitTestFilesForDir (dirNameIn, listfilename, javaTemplate, jniTemplate):
    ...
    if os.path.exists (tcFilePath):
        ...
        for utClassName in tcFile.readlines ():
            ...
            print '         @$(baseDir)SearchAndReplace.py  ' + junitTestFilePath + ' __BE_TEST_LOAD_SHARED_LIBRARIES__ "$(shared_libraries)"'
            print '         @$(baseDir)SearchAndReplace.py  ' + junitTestFilePath + ' __BE_TEST_LOAD_SHARED_LIBRARIES_PREFIX__ ""'
            print '     %else'
            print '         @$(baseDir)SearchAndReplace.py  ' + junitTestFilePath + ' __BE_TEST_LOAD_SHARED_LIBRARIES_PREFIX__ "\/\/"'
            print '     %endif'
            print '     ~time'
