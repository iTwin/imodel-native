...
- **`x64-windows-static*-clang.cmake`** — the clang variant. It sets `VCPKG_CHAINLOAD_TOOLCHAIN_FILE` to the shared [`windows-clang-cl.toolchain.cmake`](windows-clang-cl.toolchain.cmake) (referenced as `${CMAKE_CURRENT_LIST_DIR}/../../windows-clang-cl.toolchain.cmake`), which makes vcpkg build the package with BentleyBuild's own LLVM clang-cl (from `LLVM_DIR`). Because the chainload toolchain **replaces** vcpkg's default Windows toolchain, it also selects the CRT itself; `/RTC*` is never used in a clang variant (clang-cl does not implement it).
