...
StyledStreamWriter::StyledStreamWriter( Utf8String indentation )
   : document_(NULL)
   , rightMargin_( 74 )
...
void 
StyledStreamWriter::pushValue( const Utf8String &value )
{
   if ( addChildValues_ )
      childValues_.push_back( value );
   else
...
void 
StyledStreamWriter::writeWithIndent( const Utf8String &value )
{
   writeIndent();
   *document_ << value;
}
...
{
   BeAssert( indentString_.size() >= indentation_.size() );
...
Utf8String 
StyledStreamWriter::normalizeEOL( const Utf8String &text )
{
   Utf8String normalized;
   normalized.reserve( text.length() );
   const char *begin = text.c_str();
