...
Reader::decodeNumber( Token &token )
{
   ...
   while ( current < token.end_ )
   {
      Char c = *current++;
      if ( c < '0'  ||  c > '9' )
         return addError( "'" + Utf8String( token.start_, token.end_ ) + "' is not a number.", token );
...
Reader::decodeDouble( Token &token )
{
   ...
   {
      Utf8String buffer( token.start_, token.end_ );
...
bool 
Reader::decodeString( Token &token, Utf8String &decoded )
{
...
bool 
Reader::addError( const Utf8String &message, 
                  Token &token,
                  Location extra )
{
   ErrorInfo info;
...
bool 
Reader::addErrorAndRecover( const Utf8String &message, 
                            Token &token,
