...
#ifdef BENTLEY_MODIFICATIONS
    // Generate has methods of the form:
    // bool has_name() const { return CheckField(offset); }
    for (auto const& fieldP : struct_def.fields.vec)
        {
        auto& field = *fieldP;
        if (field.deprecated)
            continue;
...
#ifdef BENTLEY_MODIFICATIONS
    // Detect and generate convenience typedefs for bvectors.
    if (NULL != struct_def.attributes.Lookup("emitVectorTypedefs"))
        {
        code += "typedef flatbuffers::Offset<" + struct_def.name + "> " + struct_def.name + "Offset;\n";
        code += "typedef bvector<" + struct_def.name + "Offset> " + struct_def.name + "Offsets;\n\n";
        code += "typedef flatbuffers::Vector<" + struct_def.name + "Offset> " + struct_def.name + "OffsetVector;\n";
        code += "typedef flatbuffers::Offset<" + struct_def.name + "OffsetVector> " + struct_def.name + "OffsetVectorOffset;\n\n";
        }
#endif
...
static void GenStruct(const Parser &parser, StructDef &struct_def,
  ...
  for (auto it = struct_def.fields.vec.begin();
    ...
    code += "#pragma clang diagnostic ignored \"-Wunused-private-field\"\n";
    code += "#endif\n";
#endif
  ...
  code += NumToString(struct_def.bytesize) + ");\n\n";
    ...
    if (NULL != struct_def.attributes.Lookup("emitVectorTypedefs"))
        {
        code += "typedef bvector<" + struct_def.name + "> " + struct_def.name + "s;\n";
        code += "typedef flatbuffers::Vector<" + struct_def.name + " const*> " + struct_def.name + "Vector;\n";
        code += "typedef flatbuffers::Offset<" + struct_def.name + "Vector> " + struct_def.name + "VectorOffset;\n\n";
        }
#endif
}
