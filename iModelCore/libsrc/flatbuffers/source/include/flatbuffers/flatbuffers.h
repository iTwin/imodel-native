...
#ifndef EXCLUDE_BENTLEY_MODIFICATIONS
  typedef return_type iterable_type;
  static iterable_type ReadIterableType(const uint8_t *p, uoffset_t i) { return Read(p, i); }
#endif
};
...
struct VectorIterator {
    ...
    using value_type=typename std::conditional < bConst,
  const typename IndirectHelper<T>::return_type,
  typename IndirectHelper<T>::return_type > ::type;
    using difference_type=uoffset_t;
    using pointer=typename std::conditional < bConst,
  const typename IndirectHelper<T>::return_type,
  typename IndirectHelper<T>::return_type > ::type*;
    using reference=typename std::conditional < bConst,
  const typename IndirectHelper<T>::return_type,
  typename IndirectHelper<T>::return_type > ::type&;
...
#ifndef EXCLUDE_BENTLEY_MODIFICATIONS
    typedef typename IndirectHelper<T>::iterable_type iterable_type;

    template<typename vector_type, typename iterable_type>
    struct Iterator
        {
    private:
        Vector<vector_type> const& m_data;
        uoffset_t m_index;

    public:
        Iterator(Vector<vector_type> const& data, uoffset_t index) : m_data(data), m_index(index) {}
        bool operator!=(Iterator const& rhs) const { return m_index != rhs.m_index; }
        iterable_type operator*() const { assert(m_index < m_data.Length()); return IndirectHelper<T>::ReadIterableType(m_data.Data(), m_index); }
        Iterator const& operator++() { ++m_index; return *this; }
        }; // Iterator

    Iterator<T,iterable_type> begin() const { return Iterator<T,iterable_type>(*this, 0); }
    Iterator<T,iterable_type> end() const { return Iterator<T,iterable_type>(*this, Length()); }
#else
...
#ifndef EXCLUDE_BENTLEY_MODIFICATIONS
    bool IsForcingDefaults() const { return force_defaults_; }
#endif
...
#ifndef EXCLUDE_BENTLEY_MODIFICATIONS
  Offset<String> CreateString(Utf8StringCR str) {
    return CreateString(str.c_str(), str.length());
  }
#endif
...
#ifndef EXCLUDE_BENTLEY_MODIFICATIONS
struct TemporaryForceDefaults
{
private:
    FlatBufferBuilder& m_encoder;
    bool m_oldValue;
    bool m_newValue;

    TemporaryForceDefaults(TemporaryForceDefaults const&);
    TemporaryForceDefaults& operator= (TemporaryForceDefaults const&);

public:
    TemporaryForceDefaults(FlatBufferBuilder& encoder, bool newValue) :
        m_encoder(encoder),
        m_oldValue(encoder.IsForcingDefaults()),
        m_newValue(newValue)
        {
        if (m_newValue != m_oldValue)
            m_encoder.ForceDefaults(m_newValue);
        }

    ~TemporaryForceDefaults()
        {
        if (m_newValue != m_oldValue)
            m_encoder.ForceDefaults(m_oldValue);
        }
