...
#include <folly/intrusive/list.hpp>
#endif
...
struct EmulatedFutexBucket {
  std::mutex mutex_;
  std::list<EmulatedFutexWaitNode*> waiters_;

#if defined (BENTLEY_CHANGE)
  static const size_t kNumBuckets = 4096;
#endif
  static const size_t kNumBuckets = 128;
...
FutexResult emulatedFutexWaitImpl(
  ...
  if (status == std::cv_status::timeout) {
    ...
    std::unique_lock<std::mutex> bucketLock(bucket.mutex_);
    if (!node.signaled_) {
      for (auto it=bucket.waiters_.begin(); it!=bucket.waiters_.end(); ++it){
        if (*it == &node) {
          bucket.waiters_.erase(it);
          return FutexResult::TIMEDOUT;
        }  
      assert(false);
      }
  ...
  assert(bucket.waiters_.end() == std::find(bucket.waiters_.begin(), bucket.waiters_.end(), &node));
  return FutexResult::AWOKEN;
}
