...
// Note: exclude clang-cl on Windows (_MSC_VER) which uses MSVC STL, not libc++.
#if defined(FOLLY_USE_LIBCPP) || (defined(__clang__) && !defined(__GLIBCXX__) && !defined(_MSC_VER))
