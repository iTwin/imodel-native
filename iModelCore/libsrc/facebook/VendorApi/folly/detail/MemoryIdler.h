...
struct MemoryIdler {
  ...
  BE_FOLLY_EXPORT static void flushLocalMallocCaches();
  ...
  BE_FOLLY_EXPORT static void unmapUnusedStack(size_t retain = kDefaultStackToRetain);
  ...
  static std::chrono::steady_clock::duration defaultIdleTimeout() {return std::chrono::seconds(5);}
...
// BENTLEY_CHANGE = defaultIdleTimeout.load(std::memory_order_acquire),
    ...
    if (idleTimeout.count() > 0 && timeoutVariationFrac > 0) {
      ...
      uint64_t tics = (uint64_t) (idleTimeout.count() * (1 + extraFrac));
