...
The version pinned is **3.6.4**, which the vcpkg registry now packages natively. Because of that,
every other file in `openssl/` (portfile, registry patches, helper scripts, `vcpkg.json`) is a
verbatim copy of the registry's 3.6.4 `ports/openssl`; only the two Bentley patches above and
...
All patches in the `PATCHES` list (the ten carried over from the registry's 3.6.4 port plus the
two Bentley patches above) have been verified to apply cleanly to fresh 3.6.4 source with
`git apply --check`.
