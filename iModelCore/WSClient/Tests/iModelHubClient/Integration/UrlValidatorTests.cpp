...
Utf8String extractUrl(Utf8String url)
    ...
    std::regex regex(R"(> HTTP #(\d+) (\w+) https://(dev|qa)-imodelhubapi(-eus|-uks|-sea)?.bentley.com/s?v(\d+).(\d+)/Repositories/(iModel|Project)--\w{8}-\w{4}-\w{4}-\w{4}-\w{12}/)", std::regex::ECMAScript);
    std::cmatch matches;
