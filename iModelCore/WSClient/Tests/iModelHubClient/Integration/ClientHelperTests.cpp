...
TEST_F(ClientHelperTests, SetsStaticAuthorizationHeader)
    ...
    s_handler->ExpectRequest([&](Http::RequestCR req) {
        return Http::Response(HttpStatus::OK, "https://test/foo", "Mas-Server:Bentley-WSG/02.06.00.00,Bentley-WebAPI/2.6", "");
    });
    s_handler->ExpectRequest([&](Http::RequestCR req) {
        request = req;
        return Http::Response(HttpStatus::OK, "https://test/foo", "", "{}");
    });
