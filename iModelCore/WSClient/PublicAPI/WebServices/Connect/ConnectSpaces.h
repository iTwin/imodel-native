...
struct ConnectSpaces
    {
public:
    WS_EXPORT static void SetUserAgent(Utf8StringCR userAgent); // TODO WIP TEMPORARY SOLUTION

    WS_EXPORT static void Initialize();
    WS_EXPORT static bool IsInitialized();
    WS_EXPORT static void Uninitialize();
    WS_EXPORT static bool ParseLastModified(const std::string &lastModified, Bentley::DateTime &dateTime);

    WS_EXPORT bool OnMessageReceived(Utf8CP messageType, JsonValueCR messageObj);
...
public:
    ...
    WS_EXPORT ConnectSpaces();
    WS_EXPORT ConnectSpaces(const ConnectSpaces& other);
    WS_EXPORT ~ConnectSpaces();
