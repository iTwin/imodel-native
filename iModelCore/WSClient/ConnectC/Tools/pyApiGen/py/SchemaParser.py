from xml.dom import minidom

from py.HeaderWriters.CBufferHeaderWriter import CBufferHeaderWriter
from py.HeaderWriters.CPublicApiHeaderWriter import CallStatus
from py.HeaderWriters.CPublicApiHeaderWriter import CPublicApiHeaderWriter
from py.HeaderWriters.CPublicBufferHeaderWriter import CPublicBufferHeaderWriter
from py.HeaderWriters.CppCliClassesHeaderWriter import CppCliClassesHeaderWriter
from py.HeaderWriters.CppCliApiHeaderWriter import CppCliApiHeaderWriter
from py.SourceWriters.CApiSourceWriter import CApiSourceWriter
from py.SourceWriters.CBufferSourceWriter import CBufferSourceWriter
from py.SourceWriters.CppCliClassesSourceWriter import CppCliClassesSourceWriter
from py.SourceWriters.CppCliApiSourceWriter import CppCliApiSourceWriter
from py.Writer import Api
from py.FileMover import FileLink
from py.FileMover import FileMover


xmldoc = minidom.parse('N~3AGlobalSchema.01.00.xml')
ecclasses = xmldoc.getElementsByTagName('ECClass')
api = Api('CWSCC', 'ConnectWebServicesClientC')
excluded_classes = ['Organization_SQLAzure_PoC', 'Project_SQLAzure_PoC',
                    'ProjectFavorite_SQLAzure_PoC', 'ProjectMRU_SQLAzure_Poc',
                    'ProjectMRUDetail_SQLAzure_PoC', 'ProjectProperties']

status_codes = {
    'ERROR400'.format(api.get_upper_api_name()):
        CallStatus(400, 'The response from the server contained a 400, Bad Request, http error'),
    'ERROR401'.format(api.get_upper_api_name()):
        CallStatus(401, 'The response from the server contained a 401, Unauthorized, http error'),
    'ERROR403'.format(api.get_upper_api_name()):
        CallStatus(403, 'The response from the server contained a 403, Forbidden, http error'),
    'ERROR404'.format(api.get_upper_api_name()):
        CallStatus(404, 'The response from the server contained a 404, Not Found, http error'),
    'ERROR409'.format(api.get_upper_api_name()):
        CallStatus(409, 'The response from the server contained a 409, Conflict, http error'),
    'ERROR500'.format(api.get_upper_api_name()):
        CallStatus(500, 'The response from the server contained a 500, Internal Server, http error'),
    'SUCCESS'.format(api.get_upper_api_name()):
        CallStatus(0, 'Successful operation'),
    'INVALID_PARAMETER'.format(api.get_upper_api_name()):
        CallStatus(-100, 'Invalid parameter passed to function'),
    'PROPERTY_HAS_NOT_BEEN_SET'.format(api.get_upper_api_name()):
        CallStatus(-101, 'The buffer property passed to function has not been set in the buffer'),
    'INTERNAL_MEMORY_ERROR'.format(api.get_upper_api_name()):
        CallStatus(-102, 'Memory failed to initialize interally.')
}

# -----------------------------------------------------------------#
# ---------------------Initialize file links-----------------------#
# -----------------------------------------------------------------#
filelinks = []
outCpp = "D:\dev\dgndb0601dev\src\WSClient\ConnectC\\"
outH = "D:\dev\dgndb0601dev\src\WSClient\PublicAPI\WebServices\ConnectC\\"


# -----------------------------------------------------------------#
# ---------------------------C-API---------------------------------#
# -----------------------------------------------------------------#
publicApiHeaderWriter = CPublicApiHeaderWriter('GlobalSchema',
                                               ecclasses,
                                               '../Cpp/Headers/{0}GenPublic.h'.format(api.get_api_acronym()),
                                               api,
                                               status_codes,
                                               excluded_classes)
publicApiHeaderWriter.write_header()
apiSourceWriter = CApiSourceWriter('GlobalSchema',
                                   ecclasses,
                                   '../Cpp/Source/{0}Gen.cpp'.format(api.get_api_acronym()),
                                   api,
                                   status_codes,
                                   excluded_classes)
apiSourceWriter.write_source()

filelinks.append(FileLink(outH,
                          '../Cpp/Headers/{0}GenPublic.h'.format(api.get_api_acronym())))
filelinks.append(FileLink(outCpp,
                          '../Cpp/Source/{0}Gen.cpp'.format(api.get_api_acronym())))

# -----------------------------------------------------------------#
# --------------------------C-Buffer-------------------------------#
# -----------------------------------------------------------------#
publicBufferHeaderWriter = CPublicBufferHeaderWriter(ecclasses,
                                                     '../Cpp/Headers/{0}GenBufferPublic.h'.format(api.get_api_acronym()),
                                                     api,
                                                     status_codes,
                                                     excluded_classes)
publicBufferHeaderWriter.write_header()
headerWriter = CBufferHeaderWriter(ecclasses,
                                   '../Cpp/Headers/{0}GenBuffer.h'.format(api.get_api_acronym()),
                                   api,
                                   status_codes,
                                   excluded_classes)
headerWriter.write_header()
sourceWriter = CBufferSourceWriter(ecclasses,
                                   '../Cpp/Source/{0}GenBuffer.cpp'.format(api.get_api_acronym()),
                                   api,
                                   status_codes,
                                   excluded_classes)
sourceWriter.write_source()

filelinks.append(FileLink(outH,
                          '../Cpp/Headers/{0}GenBufferPublic.h'.format(api.get_api_acronym())))
filelinks.append(FileLink(outCpp,
                          '../Cpp/Headers/{0}GenBuffer.h'.format(api.get_api_acronym())))
filelinks.append(FileLink(outCpp,
                          '../Cpp/Source/{0}GenBuffer.cpp'.format(api.get_api_acronym())))

# -----------------------------------------------------------------#
# --------------------------CPP/CLI--------------------------------#
# -----------------------------------------------------------------#
cppcliClassesHeaderWriter = CppCliClassesHeaderWriter(ecclasses,
                                                      '../CSharp/Headers/{0}GenClasses.h'.format(api.get_api_acronym()),
                                                      api,
                                                      status_codes,
                                                      excluded_classes)
cppcliClassesHeaderWriter.write_header()
cppcliClassesSourceWriter = CppCliClassesSourceWriter(ecclasses,
                                                      '../CSharp/Source/{0}GenClasses.cpp'.format(api.get_api_acronym()),
                                                      api,
                                                      status_codes,
                                                      excluded_classes)
cppcliClassesSourceWriter.write_source()
cppcliapiHeaderWriter = CppCliApiHeaderWriter(ecclasses,
                                              '../CSharp/Headers/{0}GenSharp.h'.format(api.get_api_acronym()),
                                              api,
                                              status_codes,
                                              excluded_classes)
cppcliapiHeaderWriter.write_header()
cppcliapiSourceWriter = CppCliApiSourceWriter(ecclasses,
                                              '../CSharp/Source/{0}GenSharp.cpp'.format(api.get_api_acronym()),
                                              api,
                                              status_codes,
                                              excluded_classes)
cppcliapiSourceWriter.write_source()

# -----------------------------------------------------------------#
# --------------------------Move Files-----------------------------#
# -----------------------------------------------------------------#

filelinks.append(FileLink(outH,
                          'C:\Users\David.Jones\Documents\Connect_Stuff\WSApi\WSApiDll\{0}Public.h'.format(api.get_api_acronym())))
filelinks.append(FileLink(outCpp,
                          'C:\Users\David.Jones\Documents\Connect_Stuff\WSApi\WSApiDll\{0}Private.h'.format(api.get_api_acronym())))
filelinks.append(FileLink(outCpp,
                          'C:\Users\David.Jones\Documents\Connect_Stuff\WSApi\WSApiDll\WSLocalState.h'.format(api.get_api_acronym())))
filelinks.append(FileLink(outCpp,
                          'C:\Users\David.Jones\Documents\Connect_Stuff\WSApi\WSApiDll\{0}Internal.h'.format(api.get_api_acronym())))
filelinks.append(FileLink(outCpp,
                          'C:\Users\David.Jones\Documents\Connect_Stuff\WSApi\WSApiDll\{0}Internal.cpp'.format(api.get_api_acronym())))
filelinks.append(FileLink(outCpp,
                          'C:\Users\David.Jones\Documents\Connect_Stuff\WSApi\WSApiDll\{0}.cpp'.format(api.get_api_acronym())))
filemover = FileMover(filelinks)
filemover.copyfiles()




