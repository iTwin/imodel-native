...
+---------------+---------------+---------------+---------------+---------------+------*/
struct PresentationKey /*__PUBLISH_ABSTRACT__*/
    {
...
struct PresentationRule : public PresentationKey
    ...
    private:
        WString   m_condition;
        bool      m_onlyIfNotHadled;

    protected:
    /*__PUBLISH_SECTION_START__*/
        ECOBJECTS_EXPORT PresentationRule ()
            : PresentationKey (), m_condition (L""), m_onlyIfNotHadled (false)
            {
            }

        ECOBJECTS_EXPORT PresentationRule (WStringCR condition, int priority, bool onlyIfNotHadled)
            : PresentationKey (priority), m_condition (condition), m_onlyIfNotHadled (onlyIfNotHadled)
            {
            }
    ...
    public:
        ...
        ECOBJECTS_EXPORT bool                   GetOnlyIfNotHandled (void) const     { return m_onlyIfNotHadled; }
