...
struct MethodReference : RefCountedBase {
   ...
   public:
    ...
    ExpressionStatus InvokeStaticMethod(EvaluationResult& evalResult, EvaluationResultVector& arguments) { return _InvokeStaticMethod(evalResult, arguments); }
    ExpressionStatus InvokeInstanceMethod(EvaluationResult& evalResult, ECInstanceListCR instanceData, EvaluationResultVector& arguments) { return _InvokeInstanceMethod(evalResult, instanceData, arguments); }
    ExpressionStatus InvokeValueListMethod(EvaluationResult& evalResult, IValueListResultCR valueList, EvaluationResultVector& arguments) { return _InvokeValueListMethod(evalResult, valueList, arguments); }
};  // MethodReference
...
struct ExpressionContext : RefCountedBase {
   ...
   public:
    ...
    ExpressionStatus ResolveMethod(MethodReferencePtr& result, Utf8CP ident, bool useOuterIfNecessary, ExpressionMethodType methodType) { return _ResolveMethod(result, ident, useOuterIfNecessary, methodType); }

    ExpressionStatus GetValue(EvaluationResultR evalResult, PrimaryListNodeR primaryList, bvector<ExpressionContextP> const& contextsStack, ::uint32_t startIndex = 0) { return _GetValue(evalResult, primaryList, contextsStack, startIndex); }
...
struct InstanceListExpressionContext : ExpressionContext {
   ...
   protected:
    ...
    void SetInstanceList(ECInstanceListCR instances) {
...
struct Symbol : RefCountedBase {
   ...
   public:
    ...
    ExpressionStatus GetValue(EvaluationResultR evalResult, PrimaryListNodeR primaryList, bvector<ExpressionContextP> const& contextsStack, ::uint32_t startIndex) { return _GetValue(evalResult, primaryList, contextsStack, startIndex); }

    ExpressionStatus GetReference(EvaluationResultR evalResult, ReferenceResult& refResult, PrimaryListNodeR primaryList, bvector<ExpressionContextP> const& contextsStack, ::uint32_t startIndex) { return _GetReference(evalResult, refResult, primaryList, contextsStack, startIndex); }
#endif
...
struct EXPORT_VTABLE_ATTRIBUTE IECSymbolProvider {
   ...
   public:
    ...
    ECOBJECTS_EXPORT Utf8CP GetName() const { return _GetName(); }
    ECOBJECTS_EXPORT void PublishSymbols(SymbolExpressionContextR context, bvector<Utf8String> const& requestedSymbolSets) const { return _PublishSymbols(context, requestedSymbolSets); }
    //!<@private
...
struct Node : RefCountedBase {
   ...
   protected:
    ...
    virtual ExpressionStatus _GetValue(EvaluationResult& evalResult, ExpressionContextR context) { return ExpressionStatus::NotImpl; }
