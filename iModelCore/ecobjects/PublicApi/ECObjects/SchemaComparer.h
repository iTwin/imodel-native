...
struct PrimitiveChange final : public ECChange {
   private:
    ...
    void _WriteToString(Utf8StringR str, int currentIndex, int) const override {
        if (!IsChanged())
            return;
...
struct PropertyValueChange final : public ECChange {
   ...
   public:
    ...
    Utf8CP GetAccessString() const {
        BeAssert(HasChangeName());
        return GetChangeName();
    }
    bool HasValue() const { return m_value != nullptr; }
    ECN::PrimitiveType GetValueType() const { return m_primType; }
    StringChange* GetString() const {
        BeAssert(m_primType == ECN::PRIMITIVETYPE_String);
        if (m_primType != ECN::PRIMITIVETYPE_String) return nullptr;
        return static_cast<StringChange*>(m_value.get());
    }
    BooleanChange* GetBoolean() const {
        BeAssert(m_primType == ECN::PRIMITIVETYPE_Boolean);
        if (m_primType != ECN::PRIMITIVETYPE_Boolean) return nullptr;
        return static_cast<BooleanChange*>(m_value.get());
    }
    DateTimeChange* GetDateTime() const {
        BeAssert(m_primType == ECN::PRIMITIVETYPE_DateTime);
        if (m_primType != ECN::PRIMITIVETYPE_DateTime) return nullptr;
        return static_cast<DateTimeChange*>(m_value.get());
    }
    DoubleChange* GetDouble() const {
        BeAssert(m_primType == ECN::PRIMITIVETYPE_Double);
        if (m_primType != ECN::PRIMITIVETYPE_Double) return nullptr;
        return static_cast<DoubleChange*>(m_value.get());
    }
    Int32Change* GetInteger() const {
        BeAssert(m_primType == ECN::PRIMITIVETYPE_Integer);
        if (m_primType != ECN::PRIMITIVETYPE_Integer) return nullptr;
        return static_cast<Int32Change*>(m_value.get());
    }
    Int64Change* GetLong() const {
        BeAssert(m_primType == ECN::PRIMITIVETYPE_Long);
        if (m_primType != ECN::PRIMITIVETYPE_Long) return nullptr;
        return static_cast<Int64Change*>(m_value.get());
    }
    Point2dChange* GetPoint2d() const {
        BeAssert(m_primType == ECN::PRIMITIVETYPE_Point2d);
        if (m_primType != ECN::PRIMITIVETYPE_Point2d) return nullptr;
        return static_cast<Point2dChange*>(m_value.get());
    }
    Point3dChange* GetPoint3d() const {
        BeAssert(m_primType == ECN::PRIMITIVETYPE_Point3d);
        if (m_primType != ECN::PRIMITIVETYPE_Point3d) return nullptr;
        return static_cast<Point3dChange*>(m_value.get());
    }
    BinaryChange* GetBinary() const {
        BeAssert(m_primType == ECN::PRIMITIVETYPE_Binary);
        if (m_primType != ECN::PRIMITIVETYPE_Binary) return nullptr;
        return static_cast<BinaryChange*>(m_value.get());
    }
...
struct RelationshipConstraintChange final : public CompositeECChange {
   public:
    ...
    StringChange& AbstractConstraint() { return Get<StringChange>(Type::AbstractConstraint); }
