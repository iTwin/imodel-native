...
struct UnitSystem : Units::UnitSystem, NonCopyableClass {
   ...
   public:
    ...
    void SetId(UnitSystemId id) {
        BeAssert(!m_unitSystemId.IsValid());
        m_unitSystemId = id;
    }  //!< Intended to be called by ECDb or a similar system.
    UnitSystemId GetId() const {
        BeAssert(HasId());
        return m_unitSystemId;
    ...
    void ResetId() { m_unitSystemId.Invalidate(); }
    ...
    ECOBJECTS_EXPORT bool ToJson(BeJsValue outValue, bool includeSchemaVersion = false) const;
};
...
struct Phenomenon : Units::Phenomenon, NonCopyableClass {
   ...
   public:
    ...
    void SetId(PhenomenonId id) {
        BeAssert(!m_phenomenonId.IsValid());
        m_phenomenonId = id;
    }  //!< Intended to be called by ECDb or a similar system.
    PhenomenonId GetId() const {
        BeAssert(HasId());
        return m_phenomenonId;
    }  //!< Return unique id (May return 0 until it has been explicitly set by ECDb or a similar system).
    bool HasId() const { return m_phenomenonId.IsValid(); }
    void ResetId() { m_phenomenonId.Invalidate(); }
...
struct ECUnit : Units::Unit, NonCopyableClass {
   ...
   private:
    ...
    ECObjectsStatus SetDisplayLabel(Utf8StringCR value) {
        T_Super::SetDisplayLabel(value.c_str());
        return ECObjectsStatus::Success;
    }
    ECObjectsStatus SetDescription(Utf8StringCR value) {
        m_description = value;
    ...
    ECObjectsStatus SetConstant(bool isConstant) {
        T_Super::SetConstant(isConstant);
        return ECObjectsStatus::Success;
    }
    BentleyStatus SetNumerator(double value) override {
        m_isNumeratorExplicitlyDefined = true;
        return Units::Unit::SetNumerator(value);
    }
    BentleyStatus SetDenominator(double value) override {
        m_isDenominatorExplicitlyDefined = true;
        return Units::Unit::SetDenominator(value);
    }
    BentleyStatus SetOffset(double value) override {
        m_isOffsetExplicitlyDefined = true;
        return Units::Unit::SetOffset(value);
    }

    SchemaReadStatus ReadXml(pugi::xml_node unitNode, ECSchemaReadContextR context);
    SchemaReadStatus ReadStandardUnitXml(pugi::xml_node, ECSchemaReadContextR context);
    ...
    bool ConstantToJson(BeJsValue outValue, bool standalone, bool includeSchemaVersion) const;
   ...
   public:
    ...
    void ResetId() { m_unitId.Invalidate(); }
    ...
    ECUnitCP GetInvertingUnit() const {
        BeAssert(IsInvertedUnit());
        return (ECUnitCP)GetParent();
    }

    //! Write the ECUnit as a standalone schema child in the ECSchemaJSON format.
    //! @param[out] outValue                Json object containing the schema child Json if successfully written.
    //! @param[in]  includeSchemaVersion    If true the schema version will be included in the Json object.
    bool ToJson(BeJsValue outValue, bool includeSchemaVersion = false) const { return ToJson(outValue, true, includeSchemaVersion); }
};
...
struct NamedFormat : Formatting::Format {
   ...
   protected:
    ECOBJECTS_EXPORT bool _ToJson(BeJsValue out, bool verbose) const override;
   ...
   public:
    ...
    ECObjectsStatus SetName(Utf8StringCR nameOrFormat) {
        m_nameOrFormatString = nameOrFormat;
        return ECObjectsStatus::Success;
    }
    ...
    //! If true this format is an override of a parent ECFormat, if false it is an ECFormat
    bool IsOverride() const { return this != (NamedFormatCP)m_ecFormat; }
    //! If IsOverride this returns the ECFormat that is overriden, else it returns this format as an ECFormat
...
struct ECFormat : NamedFormat, NonCopyableClass {
    ...
    friend struct SchemaXmlReaderImpl;  // needed for ReadXml() method
    friend struct SchemaJsonWriter;     // needed for the ToJson() method
    friend struct SchemaMerger;         // needed to update description/label
   ...
   private:
    ...
    SchemaReadStatus ReadXml(pugi::xml_node unitFormatNode, ECSchemaReadContextR context);
    ...
    bool _ToJson(BeJsValue out, bool verbose) const override { return ToJsonInternal(out, true, verbose); }

   public:
    ECSchemaCR GetSchema() const { return *m_schema; }  //!< The ECSchema that this Format is defined in.

    FormatId GetId() const {
        BeAssert(HasId());
        return m_formatId;
    }  //!< Return unique id (May return 0 until it has been explicitly set by ECDb or a similar system).
    void SetId(FormatId id) {
        BeAssert(!m_formatId.IsValid());
        m_formatId = id;
    }  //!< Intended to be called by ECDb or a similar system.
    bool HasId() const { return m_formatId.IsValid(); }
    void ResetId() { m_formatId.Invalidate(); }
