...
struct ECNameValidation {
   ...
   public:
    ...
    //! Removes control characters from a given string
    //!  @param[out] cntrlCharsRemoved   Output string with control characters removed
    //!  @param[in]  cntrlCharsPresent   Input string from which control characters will be removed
    //!  @return true if control characters were removed
    ECOBJECTS_EXPORT static bool RemoveControlCharacters(Utf8StringR cntrlCharsRemoved, Utf8StringCR cntrlCharsPresent);
};
...
struct CachedSchemaQualifiedName : CachedUtf8String {
   private:
    ...
    static Utf8String ComputeName(NamedItem const& item) {
        return item.GetSchema().GetName() + ":" + item.GetName();
    }

   public:
    template <typename NamedItem>
    Utf8StringCR GetName(NamedItem const& item) const {
        return Get([&]() { return ComputeName(item); });
    }

    template <typename NamedItem>
    void RecomputeName(NamedItem const& item) const {
