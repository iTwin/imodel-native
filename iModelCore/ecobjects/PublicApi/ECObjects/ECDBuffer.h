...
struct PropertyLayout {
   ...
   public:
    PropertyLayout(Utf8CP accessString, uint32_t psi, ECTypeDescriptor typeDescriptor, uint32_t offset, uint32_t nullflagsOffset, uint32_t nullflagsBitmask, uint32_t modifierFlags = 0, uint32_t modifierData = 0) :  //, ECPropertyCP property) :
                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                      m_parentStructIndex(psi),
                                                                                                                                                                                                                      m_typeDescriptor(typeDescriptor),
                                                                                                                                                                                                                      m_offset(offset),
                                                                                                                                                                                                                      m_nullflagsOffset(nullflagsOffset),
                                                                                                                                                                                                                      m_nullflagsBitmask(nullflagsBitmask),
                                                                                                                                                                                                                      m_modifierFlags(modifierFlags),
                                                                                                                                                                                                                      m_modifierData(modifierData) {};  //, m_property(property) {};
...
struct EXPORT_VTABLE_ATTRIBUTE ECDBuffer {
   ...
   protected:
    ...
    //! @precondition the src ECDBuffer must have the same class layout as this, check before calling!
    //! @param[in] source The ECDBuffer to copy instances from
    ECOBJECTS_EXPORT ECObjectsStatus CopyStructInstancesFromBuffer(ECDBufferCR src);
    ...
    // internal routine that copies the instances for a particular struct array property
    // the property layouts must point to struct properties
    ECOBJECTS_EXPORT ECObjectsStatus CopyStructInstancesData(ECDBufferCR src, PropertyLayoutCR srcPropLayout, PropertyLayoutCR dstPropLayout, uint32_t dstPropIdx);
   ...
   public:
    ...
    // In general CopyFromBuffer() should be used instead.
    ECOBJECTS_EXPORT ECObjectsStatus CopyDataBuffer(ECDBufferCR src, bool allowClassLayoutConversion);
    ECOBJECTS_EXPORT ECObjectsStatus CopyStructInstancesData(ECDBufferCR src);
