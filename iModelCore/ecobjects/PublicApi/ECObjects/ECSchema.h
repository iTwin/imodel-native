...
struct EXPORT_VTABLE_ATTRIBUTE IECCustomAttributeContainer {
   private:
    friend struct SchemaXmlReaderImpl;  // For access to ReadCustomAttributes()
    ...
    IECInstancePtr GetCustomAttributeInternal(Utf8StringCR schemaName, Utf8StringCR className, bool includeBaseClasses) const;
    ...
    //! LEGACY METHOD, use GetCustomAttributeInternal(Utf8StringCR schemaName, Utf8StringCR className, bool includeBaseClasses)
    IECInstancePtr GetCustomAttributeInternal(Utf8StringCR className, bool includeBaseClasses) const;

    template <typename ITERATOR_TYPE>
    void _WriteCustomAttributes(BeJsValue& parentNode, ITERATOR_TYPE& iterator) const;
   ...
   public:
    struct IteratorState : RefCountedBase {
       ...
       public:
        ...
        static RefCountedPtr<IteratorState> Create(IECCustomAttributeContainerCR container, bool includeBase) { return new IteratorState(container, includeBase); };
    };

    //! Iterator for the custom attribute instances
    struct const_iterator {
        using iterator_category = std::forward_iterator_tag;
        using value_type = IECInstancePtr const;
        using difference_type = std::ptrdiff_t;
        using pointer = IECInstancePtr const*;
        using reference = IECInstancePtr const&;
...
struct EXPORT_VTABLE_ATTRIBUTE ECProperty /*abstract*/ : public IECCustomAttributeContainer {
    ...
    friend struct SchemaConflictHelper;

   private:
    int32_t m_priority;
    Utf8String m_description;
    ECValue m_minimumValueOrLength;
    ECValue m_maximumValueOrLength;
   ...
   protected:
    struct Flags {
        Flags() : prioritySet(false), readOnly(false), forSupplementation(false) {}
        bool prioritySet : 1;         // Property priority is set
        bool readOnly : 1;            // Property is readonly
        bool forSupplementation : 1;  // If when supplementing the schema, a local property had to be created, currently set but never checked but leaving for future
    };
    Flags m_flags;
   ...
   public:
    ...
    bool IsReadOnlyFlagSet() const { return m_flags.readOnly; }
    ...
    void ResetId() { m_ecPropertyId.Invalidate(); }
    ...
    //! @remarks Supports only primitive types ::PRIMITIVETYPE_String and ::PRIMITIVETYPE_Binary.
    ECOBJECTS_EXPORT uint32_t GetMaximumLength() const;
    ...
    bool IsPriorityLocallyDefined() const { return m_flags.prioritySet; }
    ...
    bool GetIsReadOnly() const { return m_flags.readOnly; };
    ...
    //! Returns true if this property overrides from another property.  If true GetBaseProperty returns the property this one inherits from.
    bool HasBaseProperty() const { return nullptr != m_baseProperty; }
...
struct StructECProperty : public ECProperty {
   ...
   private:
    ...
    StructECProperty(ECClassCR ecClass) : m_structType(nullptr), ECProperty(ecClass) {};
...
struct StructArrayECProperty : ArrayECProperty {
   ...
   private:
    friend struct ECClass;
    ECStructClassCP m_structType;
    StructArrayECProperty(ECClassCR ecClass) : m_structType(nullptr), ArrayECProperty(ecClass) {
   ...
   public:
    struct IteratorState : RefCountedBase {
       ...
       public:
        ...
        ~IteratorState();
        static RefCountedPtr<IteratorState> Create(ECClassCR ecClass, bool includeBaseProperties) { return new IteratorState(ecClass, includeBaseProperties); };
    };

    //! Iterator over the properties
    struct const_iterator {
        using iterator_category = std::forward_iterator_tag;
        using value_type = const ECPropertyP;
        using difference_type = std::ptrdiff_t;
        using pointer = const ECPropertyP*;
        using reference = const ECPropertyP&;
...
struct ECEnumerator : NonCopyableClass {
   ...
   public:
    ...
    ECObjectsStatus SetDescription(Utf8StringCR value) {
        m_description = value;
        return ECObjectsStatus::Success;
    }
    ...
    ECOBJECTS_EXPORT ECObjectsStatus SetString(Utf8StringCR value);
...
struct ECEnumeration : NonCopyableClass {
    ...
    friend struct SchemaMerger;         // needed to update values
   ...
   private:
    ...
    explicit ECEnumeration(ECSchemaCR schema) : m_schema(schema), m_primitiveType(PrimitiveType::PRIMITIVETYPE_Integer), m_isStrict(true), m_enumeratorList(), m_enumeratorIterable(m_enumeratorList) {}
    ~ECEnumeration() {
   ...
   public:
    ...
    void SetId(ECEnumerationId id) {
        BeAssert(!m_ecEnumerationId.IsValid());
        m_ecEnumerationId = id;
    }  //!< Intended to be called by ECDb or a similar system
    ECEnumerationId GetId() const {
        BeAssert(HasId());
        return m_ecEnumerationId;
    }  //!< Return unique id (May return 0 until it has been explicitly set by ECDb or a similar system)
    bool HasId() const { return m_ecEnumerationId.IsValid(); }
    void ResetId() { m_ecEnumerationId.Invalidate(); }
    ...
    ECObjectsStatus SetIsStrict(bool value) {
        m_isStrict = value;
        return ECObjectsStatus::Success;
    }
...
struct KindOfQuantity : NonCopyableClass {
   ...
   public:
    const bpair<Utf8String, bvector<Utf8String>>& GetDescriptorCache() const {
        ValidateDescriptorCache();
        return m_descriptorCache;
    }                                                  //!< Get the cache of FUS descriptors
    ECSchemaCR GetSchema() const { return m_schema; }  //!< The ECSchema that this kind of quantity is defined in.

    void SetId(KindOfQuantityId id) {
        BeAssert(!m_kindOfQuantityId.IsValid());
        m_kindOfQuantityId = id;
    }  //!< Intended to be called by ECDb or a similar system
    KindOfQuantityId GetId() const {
        BeAssert(HasId());
        return m_kindOfQuantityId;
    }  //!< Return unique id (May return 0 until it has been explicitly set by ECDb or a similar system)
    bool HasId() const { return m_kindOfQuantityId.IsValid(); }
    void ResetId() { m_kindOfQuantityId.Invalidate(); }
    ...
    //! @param[in]  value  The new value to apply
    ECObjectsStatus SetRelativeError(double value) {
        m_relativeError = value;
        return ECObjectsStatus::Success;
    }
    ...
    ECObjectsStatus SetDefaultPresentationFormat(ECFormatCR parent, Nullable<int32_t> precisionOverride = nullptr, ECUnitCP inputUnitOverride = nullptr, Utf8CP labelOverride = nullptr) { return AddPresentationFormatSingleUnitOverride(parent, precisionOverride, inputUnitOverride, labelOverride, true); }
    ...
    void RemoveAllPresentationFormats() {
        m_presentationFormats.clear();
        m_descriptorCacheValid = false;
...
struct PropertyCategory : NonCopyableClass {
   ...
   public:
    ...
    void SetId(PropertyCategoryId id) {
        BeAssert(!m_propertyCategoryId.IsValid());
        m_propertyCategoryId = id;
    }  //!< Intended to be called by ECDb or a similar system
    PropertyCategoryId GetId() const {
        BeAssert(HasId());
        return m_propertyCategoryId;
    }  //!< Return unique id (May return 0 until it has been explicitly set by ECDb or a similar system)
    bool HasId() const { return m_propertyCategoryId.IsValid(); }
    void ResetId() { m_propertyCategoryId.Invalidate(); }
    ...
    ECObjectsStatus SetPriority(uint32_t priority) {
        m_priority = priority;
        return ECObjectsStatus::Success;
    }
...
/* Store transient application data that is never serialized to disk. */
struct ECAppData : RefCountedBase {
    struct Data : RefCountedBase {};

   private:
    mutable bmap<Utf8String, RefCountedPtr<Data>> m_appData;

   public:
    RefCountedPtr<Data> GetData(Utf8String key) const {
        auto it = m_appData.find(key);
        if (it == m_appData.end())
            return nullptr;
        return it->second.get();
    }
    bool Contains(Utf8String key) const {
        return m_appData.find(key) != m_appData.end();
    }
    void SetData(Utf8String key, Data& data) {
        m_appData[key] = &data;
    }
    void RemoveData(Utf8String key) {
        auto it = m_appData.find(key);
        if (it != m_appData.end())
            m_appData.erase(it);
    }
    static RefCountedPtr<ECAppData> Create() {
        return new ECAppData();
    }
};
//=======================================================================================
...
struct EXPORT_VTABLE_ATTRIBUTE ECClass /*abstract*/ : IECCustomAttributeContainer {
    ...
    friend struct SchemaMerger;  // uses copyProperty methods
   ...
   private:
    ...
    mutable std::unique_ptr<PropertyList> m_cachedProperties;
    ...
    ECObjectsStatus ValidateBaseProperty(ECPropertyCR baseProperty, ECPropertyP& derivedProperty, bool resolveConflicts);
    ECObjectsStatus ResolvePropertyOverrideIssue(ECPropertyCR baseProperty, ECPropertyP& derivedProperty, ECObjectsStatus overrideFailure, Utf8StringR failureMessage, bool resolveConflicts);
    ...
    ECPropertyP RenameUntilCompatibleBasePropertyFound(ECPropertyCP prop, Utf8StringR newName);
    ...
    bool HasPropertyMapping(Utf8CP oldPropertyName);
   ...
   protected:
    ...
    //! @return   Status code
    virtual SchemaReadStatus _ReadXmlContents(pugi::xml_node classNode, ECSchemaReadContextR context, ECSchemaCP conversionSchema, bvector<NavigationECPropertyP>& navigationProperties);

    void _ReadCommentsInSameLine(pugi::xml_node childNode, bvector<Utf8String>& comments);
   ...
   public:
    ...
    void SetId(ECClassId id) {
        BeAssert(!m_ecClassId.IsValid());
        m_ecClassId = id;
    }  //!< Intended to be called by ECDb or a similar system.
    ...
    ECObjectsStatus SetClassModifier(ECClassModifier modifier) {
        m_modifier = modifier;
        return ECObjectsStatus::Success;
    ...
    ECOBJECTS_EXPORT bool ToJson(BeJsValue outValue, bool includeSchemaVersion = false, bool includeInheritedProperties = false) { return _ToJson(outValue, true, includeSchemaVersion, includeInheritedProperties); }
...
struct EXPORT_VTABLE_ATTRIBUTE ECEntityClass : ECClass {
   ...
   public:
    ...
    //! If the class is a mixin, sets the ECEntityClass which restricts where the mixin can be applied
    //! if nullptr, makes the applies class invalid
    ECOBJECTS_EXPORT ECObjectsStatus SetAppliesToClass(ECEntityClassCP entityClass) const;
...
struct RelationshipMultiplicity {
   ...
   public:
    ...
    RelationshipMultiplicity(uint32_t lowerLimit, uint32_t upperLimit) : m_lowerLimit(lowerLimit), m_upperLimit(upperLimit) {
        BeAssert(lowerLimit <= upperLimit);
        BeAssert(upperLimit > 0);
        BeAssert(upperLimit <= INT_MAX);
    }
    ...
    ECOBJECTS_EXPORT bool IsEqualToOrMoreRestrictiveThan(RelationshipMultiplicityCR other) const;

    //! Checks of the current multiplicity matches the other.
    ECOBJECTS_EXPORT bool Equals(RelationshipMultiplicityCR other) const;
...
struct EXPORT_VTABLE_ATTRIBUTE ECRelationshipConstraint : IECCustomAttributeContainer {
   ...
   public:
    ...
    ECRelationshipClassCR GetRelationshipClass() const {
    ...
    ECOBJECTS_EXPORT ECObjectsStatus SetRoleLabel(Utf8StringCR roleLabel);
    ...
    ECObjectsStatus SetIsPolymorphic(bool isPolymorphic) {
    ...
    ECOBJECTS_EXPORT ECObjectsStatus SetMultiplicity(Utf8StringCR multiplicity);
    ...
    //! @param[in] classConstraint  The ECClassCR to remove from the constraint class list
...
struct EXPORT_VTABLE_ATTRIBUTE ECRelationshipClass : public ECClass {
   ...
   private:
    ...
    mutable ECRelationshipConstraint m_target;
    mutable ECRelationshipConstraint m_source;

    //  Lifecycle management:  For now, to keep it simple, the class constructor is private.  The schema implementation will
    //  serve as a factory for classes and will manage their lifecycle.  We'll reconsider if we identify a real-world story for constructing a class outside
    //  of a schema.
    ECRelationshipClass(ECSchemaCR schema, bool verify = true) : ECClass(ECClassType::Relationship, schema), m_strength(StrengthType::Referencing), m_strengthDirection(ECRelatedInstanceDirection::Forward), m_verify(verify), m_verified(false), m_source(this, true, verify), m_target(this, false, verify) {}
...
struct SchemaKey {
   ...
   public:
    ...
    //! read by older software.
    void SetVersionRead(uint32_t versionRead) {
        m_versionRead = versionRead;
        m_schemaFullName.Invalidate();
        m_checksum.clear();
    }
    ...
    //! that guarantees that newer schemas can be written by older software.
    void SetVersionWrite(uint32_t versionWrite) {
        m_versionWrite = versionWrite;
        m_schemaFullName.Invalidate();
        m_checksum.clear();
    }

    //! Sets the least significant version number that increments with read/write compatible additions.
    void SetVersionMinor(uint32_t versionMinor) {
    ...
    void SetName(Utf8StringCR name) {
        m_schemaName = name;
        m_schemaFullName.Invalidate();
        m_checksum.clear();
    }
    ...
    Utf8StringCR GetFullSchemaName() const {
        return m_schemaFullName.Get([&]() { return FormatFullSchemaName(m_schemaName.c_str(), m_versionRead, m_versionWrite, m_versionMinor); });
    }
...
struct SchemaItemContainer {
   ...
   public:
    ...
    struct const_iterator {
        using iterator_category = std::forward_iterator_tag;
        using value_type = MAP_TYPE const*;
        using difference_type = std::ptrdiff_t;
        using pointer = MAP_TYPE const**;
        using reference = MAP_TYPE const*&;
...
struct SchemaUnitContext : Units::IUnitsContext {
   ...
   private:
    ...
    template <typename T>
    ECObjectsStatus Add(T* toAdd, ECSchemaElementType unitType);
    ...
    template <typename T>
    ECObjectsStatus Delete(T& toDelete);
    ...
    template <typename T>
    T* Get(Utf8CP name) const;

   protected:
    SchemaUnitContext(ECSchemaCR schema) : m_schema(schema), m_unitSystemContainer(m_unitSystemMap), m_phenomenonContainer(m_phenomenonMap), m_unitContainer(m_unitMap) {}
   ...
   public:
    ...
    //! Looks up an ECUnit within this context. If the name is fully qualified it will search reference
    //! schemas.
    ...
    //! Looks up an EC Phenomenon within this context. If the name is fully qualified it will search reference
    //! schemas.
    //! @param[in]  name     The name of the phenomenon to lookup.  Can be either an unqualified (short) name or a qualified name.
    //! @param[in]  useFullName If true, name must be fully qualified ([schema name]:[item name]). The lookup will treat the part to the
    //!                         the left of the separator (:) as a schema name and not an alias
    //! @return   A pointer to the an ECN::Phenomenon if the named phenomenon exists within the current schema or one of its reference schemas; otherwise, nullptr.
    PhenomenonCP LookupPhenomenon(Utf8CP name, bool useFullName = false) const override { return _LookupPhenomenonP(name, useFullName); }

    //! Looks up an EC UnitSystem within this context. If the name is fully qualified it will search reference
    //! schemas.
    //! @param[in]  name     The name of the unit system to lookup.  Can be either an unqualified (short) name or a qualified name.
    //! @param[in]  useFullName If true, name must be fully qualified ([schema name]:[item name]). The lookup will treat the part to the
    //!                         the left of the separator (:) as a schema name and not an alias
    //! @return   A pointer to the an ECN::UnitSystem if the named unit system exists within the current schema or one of its reference schemas; otherwise, nullptr.
    UnitSystemCP LookupUnitSystem(Utf8CP name, bool useFullName = false) const override { return _LookupUnitSystemP(name, useFullName); }
};
...
struct ECSchema : RefCountedBase, IECCustomAttributeContainer {
   ...
   private:
    ...
    mutable RefCountedPtr<ECAppData> m_appData;
    bmap<ECSchemaP, Utf8String> m_referencedSchemaAliasMap;

    ECSchema() : m_classContainer(m_classMap), m_enumerationContainer(m_enumerationMap), m_isSupplemented(false), m_hasExplicitDisplayLabel(false), m_immutable(false), m_kindOfQuantityContainer(m_kindOfQuantityMap), m_propertyCategoryContainer(m_propertyCategoryMap), m_formatContainer(m_formatMap), m_unitsContext(*this), m_schemaOrigin() {}
    virtual ~ECSchema();
    ...
    bool ItemExists(ECClassCP schemaItem) const { return nullptr != GetClassCP(schemaItem->GetName().c_str()); }
    bool ItemExists(ECEnumerationCP schemaItem) const { return nullptr != GetEnumerationCP(schemaItem->GetName().c_str()); }
    bool ItemExists(KindOfQuantityCP schemaItem) const { return nullptr != GetKindOfQuantityCP(schemaItem->GetName().c_str()); }
    bool ItemExists(PropertyCategoryCP schemaItem) const { return nullptr != GetPropertyCategoryCP(schemaItem->GetName().c_str()); }
    bool ItemExists(UnitSystemCP schemaItem) const { return nullptr != GetUnitSystemCP(schemaItem->GetName().c_str()); }
    bool ItemExists(PhenomenonCP schemaItem) const { return nullptr != GetPhenomenonCP(schemaItem->GetName().c_str()); }
    bool ItemExists(ECUnitCP schemaItem) const { return nullptr != GetUnitCP(schemaItem->GetName().c_str()); }
    bool ItemExists(ECFormatCP schemaItem) const { return nullptr != GetFormatCP(schemaItem->GetName().c_str()); }
   ...
   public:
    ...
    ECObjectsStatus SetOriginalECXmlVersion(uint32_t major, uint32_t minor) {
        m_originalECXmlVersionMajor = major;
        m_originalECXmlVersionMinor = minor;
        return ECObjectsStatus::Success;
    }
    ...
    //! @param[in] label        Display label of the unit
    //! @param[in] description  Description of the unit
    ECObjectsStatus CreateUnit(ECUnitP& unit, Utf8CP name, Utf8CP definition, PhenomenonCR phenom, UnitSystemCR unitSystem, Utf8CP label = nullptr, Utf8CP description = nullptr) { return CreateUnit(unit, name, definition, phenom, unitSystem, nullptr, nullptr, nullptr, label, description); }
    ...
    //! @param[in] label        Display label of the unit
    //! @param[in] description  Description of the unit
    ECObjectsStatus CreateUnit(ECUnitP& unit, Utf8CP name, Utf8CP definition, PhenomenonCR phenom, UnitSystemCR unitSystem, Nullable<double> numerator, Utf8CP label = nullptr, Utf8CP description = nullptr) { return CreateUnit(unit, name, definition, phenom, unitSystem, numerator, nullptr, nullptr, label, description); }
    ...
    //! @param[in] label        Display label of the constant
    //! @param[in] description  Description of the constant
    ECOBJECTS_EXPORT ECObjectsStatus CreateConstant(ECUnitP& constant, Utf8CP name, Utf8CP definition, PhenomenonCR phenom, double numerator, Utf8CP label = nullptr, Utf8CP description = nullptr) { return CreateConstant(constant, name, definition, phenom, numerator, nullptr, label, description); }
    ...
    //! Returns the startingName or the startingName with '_' appended until it is unique
    ECOBJECTS_EXPORT Utf8String FindUniqueSchemaItemName(Utf8CP startingName) const;

    //! Returns the name of the item if an item of the same type already exists or no item with the name exists.  Else return a new unique name.
    //! @param[in]  item     The item schema to look for.
    //! @return   Success if the schema is either the current schema or a referenced schema;  ECObjectsStatus::SchemaNotFound if the schema is not found in the list of referenced schemas
    template <typename Item>
    Utf8String FindNameForSchemaItem(const Item* item) const {
        if (nullptr == item) return "_";
        if (ItemExists(item)) return item->GetName().c_str();

        return FindUniqueSchemaItemName(item->GetName().c_str());
    }

    //! Check if there is any named element in the schema with the given name
    //! @return True if there is any named element with the given name
    ECOBJECTS_EXPORT bool NamedElementExists(Utf8CP name) const;
    ...
    ECOBJECTS_EXPORT UnitSystemP GetUnitSystemP(Utf8CP name);
    ...
    ECOBJECTS_EXPORT PhenomenonP GetPhenomenonP(Utf8CP name);
    ...
    //! @param[in]  dropReferences  Whether to force removing even if it is referenced, by dropping all referencing items. Defaults to false
    ECOBJECTS_EXPORT ECObjectsStatus RemoveReferencedSchema(ECSchemaR refSchema, bool dropReferences = false);
    ...
    //! @param[in]  dropReferences  Whether to force removing even if it is referenced, by dropping all referencing items. Defaults to false
    ECOBJECTS_EXPORT ECObjectsStatus RemoveReferencedSchema(SchemaKeyCR schemaKey, SchemaMatchType matchType = SchemaMatchType::Exact, bool dropReferences = false);
    ...
    //! @param[in]  itemWithRef     the schema item which has the reference to the startingRef
    //! @param[out] refForCopy      the correct object reference copy of the input startingRef
    //! @param[in]  startingRef     the schema item we're looking to find the correct reference to
    //! @param[in]  copyReferences  if true the startingRef will be copied into this schema if classWithRef and startingRef come from the same schema but no copy of startingRef can be found in the references of this schema.
    ...
    //! @param[in]  copyReferences  If true any things referenced by the sourceEnumeration and in the same schema as the source phenomenon are copied into this schema if needed.
    //! @param[in]  newName  If not nullptr, this name will be used as the new name instead of the original name
    ECOBJECTS_EXPORT ECObjectsStatus CopyEnumeration(ECEnumerationP& targetEnumeration, ECEnumerationCR sourceEnumeration, bool copyReferences = false, Utf8CP newName = nullptr);
    ...
    //! @param[in]  newName  If not nullptr, this name will be used as the new name instead of the original name
    ECOBJECTS_EXPORT ECObjectsStatus CopyKindOfQuantity(KindOfQuantityP& targetKOQ, KindOfQuantityCR sourceKOQ, bool copyReferences = false, Utf8CP newName = nullptr);
    ...
    //! @param[in]  copyReferences  If true any things referenced by the sourcePropCategory and in the same schema as the source phenomenon are copied into this schema if needed.
    //! @param[in]  newName  If not nullptr, this name will be used as the new name instead of the original name
    ECOBJECTS_EXPORT ECObjectsStatus CopyPropertyCategory(PropertyCategoryP& targetPropCategory, PropertyCategoryCR sourcePropCategory, bool copyReferences = false, Utf8CP newName = nullptr);
    ...
    //! @param[in]  copyReferences  If true any things referenced by the sourceUnitSystem and in the same schema as the source phenomenon are copied into this schema if needed.
    //! @param[in]  newName  If not nullptr, this name will be used as the new name instead of the original name
    ECOBJECTS_EXPORT ECObjectsStatus CopyUnitSystem(UnitSystemP& targetUnitSystem, UnitSystemCR sourceUnitSystem, bool copyReferences = false, Utf8CP newName = nullptr);
    ...
    //! @param[in]  copyReferences  If true any things referenced by the sourcePhenom and in the same schema as the source phenomenon are copied into this schema if needed.
    //! @param[in]  newName  If not nullptr, this name will be used as the new name instead of the original name
    ECOBJECTS_EXPORT ECObjectsStatus CopyPhenomenon(PhenomenonP& targetPhenom, PhenomenonCR sourcePhenom, bool copyReferences, Utf8CP newName = nullptr);
    ...
    //! @param[in]  copyReferences  If true any things referenced by the sourceUnit and in the same schema as the source unit are copied into this schema if needed.
    //! @param[in]  newName  If not nullptr, this name will be used as the new name instead of the original name
    ECOBJECTS_EXPORT ECObjectsStatus CopyUnit(ECUnitP& targetUnit, ECUnitCR sourceUnit, bool copyReferences, Utf8CP newName = nullptr);
    ...
    //! @param[in]  copyReferences  If true any things referenced by the sourceFormat and in the same schema as the source format are copied into this schema if needed.
    //! @param[in]  newName  If not nullptr, this name will be used as the new name instead of the original name
    ...
    //! @param[in]    schemaContext       Required to create schemas
    //! @param[in]    matchType           The match type to use when searching
    //! @param[out]   readStatus          If valid set to the status of reading the schema if we had couldn't locate it. Set to success if it was located.
    ...
    ECOBJECTS_EXPORT static ECSchemaPtr LocateSchema(WCharCP schemaXmlFile, ECSchemaReadContextR schemaContext,
                                                     const SchemaMatchType matchType = SchemaMatchType::LatestWriteCompatible,
                                                     SchemaReadStatus* readStatus = nullptr);
    ...
    //! @param[out]   readStatus          If valid set to the status of reading the schema if we had couldn't locate it. Set to success if it was located.
    //! @return A valid ECSchemaPtr, IsValid() will be true, if the schema can be found or deserialized within the provided ECSchemaReadContext
    ECOBJECTS_EXPORT static ECSchemaPtr LocateSchema(Utf8CP schemaXmlFile, ECSchemaReadContextR schemaContext,
                                                     const SchemaMatchType matchType = SchemaMatchType::LatestWriteCompatible,
                                                     SchemaReadStatus* readStatus = nullptr);
...
struct EXPORT_VTABLE_ATTRIBUTE IECClassLocater {
   protected:
    ...
    virtual ECClassCP _LocateClass(ECClassId const& classId) {
        BeAssert(false && "_LocateClass is not implemented for searching by ECClassId.");
        return nullptr;
    }
