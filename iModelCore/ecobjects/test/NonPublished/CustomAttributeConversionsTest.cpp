        ...
        for (const auto & ecClass : schema.GetClasses())
            ...
            for (const auto & ecProperty : ecClass->GetProperties(false))
                ...
                for (const auto & caName : caToLeave)
                    EXPECT_TRUE(ecProperty->GetCustomAttribute("BuildingDataGroup", caName).IsValid()) << 
                        "CA '" << caName.c_str() << " on " << ecProperty->GetName().c_str() << " removed.";
                for (const auto & caName : caToRemove)
                    EXPECT_FALSE(ecProperty->GetCustomAttribute("BuildingDataGroup", caName).IsValid()) << 
                        "CA " << caName.c_str() << " on " << ecProperty->GetName().c_str() << " not removed.";
                }
            }
        };
