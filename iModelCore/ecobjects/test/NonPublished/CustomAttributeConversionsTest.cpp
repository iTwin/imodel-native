...
struct ECSchemaDownConverterTests : ECTestFixture {};
...
struct CustomAttributeRemovalTest : ECTestFixture {
    ...
    CustomAttributeRemovalTest()
        : m_customAttributeName(""), m_customAttributeSchemaName("") {}
    ...
    bool HasCustomAttribute(ECSchemaR schema, Utf8StringCR schemaName, Utf8StringCR customAttributeName) {
        for (auto ecClass : schema.GetClasses()) {
            for (auto ecProp : ecClass->GetProperties(false)) {
                auto instance = ecProp->GetCustomAttributeLocal(schemaName, customAttributeName);
                if (instance.IsValid())
                    return true;
            }
        }
...
struct StandardValueToEnumConversionTest : CustomAttributeRemovalTest {
    ...
    void ValidateConversion(ECSchemaR origSchema, ECSchemaR convSchema) {
        for (auto const& origClass : origSchema.GetClasses()) {
            ...
            for (auto const& origProp : origClass->GetProperties(false)) {
                ...
                ECValue value;
                if (ECObjectsStatus::Success == customAttr->GetValue(value, "MustBeFromList") && !value.IsNull() && value.IsBoolean()) {
    ...
    void CheckTypeName(Utf8CP typeName, ECSchemaR schema, Utf8CP propertyName, bvector<Utf8CP> classes) {
        for (auto* className : classes) {
            ECPropertyCP prop = schema.GetClassCP(className)->GetPropertyP(propertyName, false);
            ASSERT_TRUE(nullptr != prop) << "Class '" << className << "' does not have property '" << propertyName << "'.";
            EXPECT_STREQ(typeName, prop->GetTypeName().c_str()) << "Property Type for property " << className << "." << propertyName << " should have been " << typeName;
        }
    }
...
void validateEnumeration(ECEnumerationCP ecEnum, std::map<int, Utf8String>& expectedValues, bool shouldBeStrict) {
    EXPECT_EQ(expectedValues.size(), ecEnum->GetEnumeratorCount());
    for (auto enumerator : ecEnum->GetEnumerators()) {
        int i = enumerator->GetInteger();
        EXPECT_STREQ(enumerator->GetDisplayLabel().c_str(), expectedValues[i].c_str()) << "Enumerator displaylabel does not match StandardValue's Display String";
    }
    EXPECT_EQ(shouldBeStrict, ecEnum->GetIsStrict());
}

void validateStandardValuesCA(ECPropertyCP prop, std::map<int, Utf8String>& expectedValues, bool shouldBeStrict) {
    IECInstancePtr stdValuesCA = prop->GetCustomAttributeLocal("StandardValues");
    ASSERT_TRUE(stdValuesCA.IsValid());
    ECValue valueMap;
    ASSERT_EQ(ECObjectsStatus::Success, stdValuesCA->GetValue(valueMap, "ValueMap")) << prop->GetName().c_str();
    ASSERT_FALSE(valueMap.IsNull()) << "ValueMap not set in StandardValues CA on " << prop->GetName().c_str();
    auto arraySize = valueMap.GetArrayInfo().GetCount();
    EXPECT_EQ(expectedValues.size(), arraySize) << "ValueMap not have the expected number of values on property " << prop->GetName().c_str();
    for (uint32_t i = 0; i < arraySize; i++) {
        ECValue value;
        ASSERT_TRUE(ECObjectsStatus::Success == stdValuesCA->GetValue(value, "ValueMap", i));
        ASSERT_TRUE(value.IsStruct());

        IECInstancePtr structInstance = value.GetStruct();
        ASSERT_TRUE(structInstance.IsValid());

        ASSERT_TRUE(ECObjectsStatus::Success == structInstance->GetValue(value, "Value") && value.IsLoaded() && !value.IsNull());
        int index = value.GetInteger();

        ASSERT_TRUE(ECObjectsStatus::Success == structInstance->GetValue(value, "DisplayString") && value.IsLoaded() && !value.IsNull());
        EXPECT_STREQ(expectedValues[index].c_str(), value.GetUtf8CP()) << "Display string for index " << index << " not as expected on property " << prop->GetName().c_str();
    }
    ECValue mustBeFromList;
    EXPECT_EQ(ECObjectsStatus::Success, stdValuesCA->GetValue(mustBeFromList, "MustBeFromList")) << prop->GetName().c_str();
    ASSERT_FALSE(mustBeFromList.IsNull());
    EXPECT_EQ(shouldBeStrict, mustBeFromList.GetBoolean()) << prop->GetName().c_str() << prop->GetName().c_str();
}
...
TEST_F(StandardValueToEnumConversionTest, StandardValuesTest) {
    ...
    std::map<int, Utf8String> stdValues = {{1, "Mr."}, {0, "Ms."}, {-10, "Mr."}, {11, "Ms."}, {2, "Sensei"}};
    bool shouldBeStrict = true;
    validateEnumeration(ecEnum, stdValues, shouldBeStrict);

    // Convert back to EC2 and check standard values
    ECSchemaPtr schemaCopy;
    ASSERT_EQ(ECObjectsStatus::Success, m_schema->CopySchema(schemaCopy));
    ASSERT_TRUE(ECSchemaDownConverter::Convert(*schemaCopy.get())) << "Failed to convert back to EC2";
    validateStandardValuesCA(schemaCopy->GetClassCP("Name")->GetPropertyP("Title"), stdValues, shouldBeStrict);
}
...
TEST_F(StandardValueToEnumConversionTest, StrictTestSimple) {
    ...
    std::map<int, Utf8String> title1ExpectedValues = {{0, "Mr."}};
    bool title1ShouldBeStrict = false;  // Name_Title1 has MustBeFromList set to false so GetIsStrict() should return false
    validateEnumeration(ecEnum, title1ExpectedValues, title1ShouldBeStrict);

    ASSERT_NE(nullptr, ecEnum = m_schema->GetEnumerationCP("Name_Title2")) << "Enumeration Name_Title2 should have been created";
    std::map<int, Utf8String> title2ExpectedValues = {{1, "Ms."}};
    bool title2ShouldBeStrict = true;  // Name_Title2 has MustBeFromList set to true so Enumeration should be strict
    validateEnumeration(ecEnum, title2ExpectedValues, title2ShouldBeStrict);

    ASSERT_NE(nullptr, ecEnum = m_schema->GetEnumerationCP("Name_Title3")) << "Enumeration Name_Title3 should have been created";
    std::map<int, Utf8String> title3ExpectedValues = {{1, "Miss"}};
    bool title3ShouldBeStrict = true;  // Name_Title3 has not set MustBeFromList. Default is true so GetIsStrict() should return true
    validateEnumeration(ecEnum, title3ExpectedValues, title3ShouldBeStrict);

    // Convert back to EC2 and check standard values
    ECSchemaPtr schemaCopy;
    ASSERT_EQ(ECObjectsStatus::Success, m_schema->CopySchema(schemaCopy));
    ASSERT_TRUE(ECSchemaDownConverter::Convert(*schemaCopy.get())) << "Failed to convert back to EC2";
    validateStandardValuesCA(schemaCopy->GetClassCP("Name")->GetPropertyP("Title1"), title1ExpectedValues, title1ShouldBeStrict);
    validateStandardValuesCA(schemaCopy->GetClassCP("Name")->GetPropertyP("Title2"), title2ExpectedValues, title2ShouldBeStrict);
    validateStandardValuesCA(schemaCopy->GetClassCP("Name")->GetPropertyP("Title3"), title3ExpectedValues, title3ShouldBeStrict);
}
...
TEST_F(StandardValueToEnumConversionTest, StrictTestInheritence_ParentPropertyStrictnessWins) {
    ...
    std::map<int, Utf8String> title1ExpectedValues = {{0, "Mr."}};
    bool title1ShouldBeStrict = true;
    validateEnumeration(ecEnum, title1ExpectedValues, title1ShouldBeStrict);
    ...
    ASSERT_EQ(ECObjectsStatus::Success, m_schema->CopySchema(schemaCopy));
    ASSERT_TRUE(ECSchemaDownConverter::Convert(*schemaCopy.get())) << "Failed to convert back to EC2";
    validateStandardValuesCA(schemaCopy->GetClassCP("BaseName")->GetPropertyP("Title1"), title1ExpectedValues, title1ShouldBeStrict);
    EXPECT_TRUE(schemaCopy->GetClassCP("Name")->GetPropertyP("Title1")->GetCustomAttributeLocal("StandardValues").IsNull());
}
...
TEST_F(StandardValueToEnumConversionTest, StrictTestInheritence_ParentPropertyWithNoCAMeansNotStrict) {
    ...
    ASSERT_NE(nullptr, ecEnum = m_schema->GetEnumerationCP(enumName.c_str())) << "Enumeration " << enumName << " should have been created";
    std::map<int, Utf8String> title1ExpectedValues = {{0, "Mr."}};
    bool title1ShouldBeStrict = false;  // Title1 is derived from base class property which has no StandardValues CA so GetIsStrict() should return false
    validateEnumeration(ecEnum, title1ExpectedValues, title1ShouldBeStrict);
    ...
    validateStandardValuesCA(schemaCopy->GetClassCP("BaseName")->GetPropertyP("Title1"), title1ExpectedValues, title1ShouldBeStrict);
    EXPECT_TRUE(schemaCopy->GetClassCP("Name")->GetPropertyP("Title1")->GetCustomAttributeLocal("StandardValues").IsNull());
    // NOTE: On round trip the CA is applied to the base class even though it was originally applied to the derived class ... This is not perfect but the best we can do.
}
...
TEST_F(StandardValueToEnumConversionTest, DuplicateSDValues) {
    ...
    EXPECT_EQ(nullptr, m_schema->GetEnumerationCP("C_TitleC")) << "Enumeration C_TitleC should NOT have been created";
    EXPECT_EQ(enumName1, m_schema->GetClassCP("C")->GetPropertyP("TitleC")->GetTypeName()) << "Enum type should have been " << enumName1;
    std::map<int, Utf8String> cTitleBExpectedValues = {{0, "Sensei"}};
    bool cTitleBShouldBeStrict = false;
    validateEnumeration(ecEnum, cTitleBExpectedValues, cTitleBShouldBeStrict);
    ...
    EXPECT_EQ(nullptr, m_schema->GetEnumerationCP("B_TitleA")) << "Enumeration B_TitleA should NOT have been created";
    std::map<int, Utf8String> cTitleAExpectedValues = {{0, "Mr."}, {1, "Ms."}};
    bool cTitleAShouldBeStrict = false;
    validateEnumeration(ecEnum, cTitleAExpectedValues, cTitleAShouldBeStrict);
    ...
    EXPECT_EQ(enumName2, m_schema->GetClassCP("D")->GetPropertyP("TitleA")->GetTypeName());
    ...
    ASSERT_EQ(ECObjectsStatus::Success, m_schema->CopySchema(schemaCopy));
    ASSERT_TRUE(ECSchemaDownConverter::Convert(*schemaCopy.get())) << "Failed to convert back to EC2";
    ECClassCP classC = schemaCopy->GetClassCP("C");
    validateStandardValuesCA(classC->GetPropertyP("TitleA"), cTitleAExpectedValues, cTitleAShouldBeStrict);
    validateStandardValuesCA(classC->GetPropertyP("TitleB"), cTitleBExpectedValues, cTitleBShouldBeStrict);
    validateStandardValuesCA(classC->GetPropertyP("TitleC"), cTitleBExpectedValues, cTitleBShouldBeStrict);
    validateStandardValuesCA(schemaCopy->GetClassCP("D")->GetPropertyP("TitleA"), cTitleAExpectedValues, cTitleAShouldBeStrict);
    EXPECT_TRUE(schemaCopy->GetClassCP("A")->GetPropertyP("TitleB")->GetCustomAttributeLocal("StandardValues").IsNull());
    EXPECT_TRUE(schemaCopy->GetClassCP("B")->GetPropertyP("TitleA")->GetCustomAttributeLocal("StandardValues").IsNull());
    EXPECT_TRUE(schemaCopy->GetClassCP("E")->GetPropertyP("TitleA")->GetCustomAttributeLocal("StandardValues").IsNull());
}
...
TEST_F(StandardValueToEnumConversionTest, Strict_Duplicate_Inherited_Combo_ExpectTheUnexpected1) {
    ...
    std::map<int, Utf8String> expectedValues = {{0, "Mr."}};
    ECSchemaPtr schemaCopy;
    ASSERT_EQ(ECObjectsStatus::Success, m_schema->CopySchema(schemaCopy));
    ASSERT_TRUE(ECSchemaDownConverter::Convert(*schemaCopy.get()));
    validateStandardValuesCA(schemaCopy->GetClassCP("BaseName")->GetPropertyP("Title1"), expectedValues, false);
    validateStandardValuesCA(schemaCopy->GetClassCP("A")->GetPropertyP("Title1"), expectedValues, true);
    EXPECT_TRUE(schemaCopy->GetClassCP("Name")->GetPropertyP("Title1")->GetCustomAttributeLocal("StandardValues").IsNull());
}
...
TEST_F(StandardValueToEnumConversionTest, Strict_Duplicate_Inherited_Combo_ExpectTheUnexpected2) {
    ...
    ASSERT_TRUE(ECSchemaDownConverter::Convert(*schemaCopy.get()));
    validateStandardValuesCA(schemaCopy->GetClassCP("BaseName")->GetPropertyP("Title1"), expectedValues, false);
    validateStandardValuesCA(schemaCopy->GetClassCP("A")->GetPropertyP("Title1"), expectedValues, false);
    EXPECT_TRUE(schemaCopy->GetClassCP("Name")->GetPropertyP("Title1")->GetCustomAttributeLocal("StandardValues").IsNull());
}
...
TEST_F(StandardValueToEnumConversionTest, MultipleInheritedSDValues_Supplemental_ConversionSuccess) {
    Utf8CP schemaXML =
        ...
        "       <BaseClass>C</BaseClass>"
        "       <ECProperty propertyName='TitleA' typeName='int' displayLabel='TitleA' />"  // NOTE: changed display label so this property is not removed during deserialization
        "   </ECClass>"
        "   <ECClass typeName='B' isStruct='false' isCustomAttributeClass='false' isDomainClass='true'>"
        "       <ECProperty propertyName='TitleA' typeName='int' displayLabel='Title' />"
        "   </ECClass>"
    ...
    std::map<int, Utf8String> expectedValues = {{0, "Sensei"}};
    ECSchemaPtr schemaCopy;
    ASSERT_EQ(ECObjectsStatus::Success, m_schema->CopySchema(schemaCopy));
    ASSERT_TRUE(ECSchemaDownConverter::Convert(*schemaCopy.get()));
    validateStandardValuesCA(schemaCopy->GetClassCP("B")->GetPropertyP("TitleA"), expectedValues, false);
    validateStandardValuesCA(schemaCopy->GetClassCP("C")->GetPropertyP("TitleA"), expectedValues, false);
    EXPECT_TRUE(schemaCopy->GetClassCP("A")->GetPropertyP("TitleA")->GetCustomAttributeLocal("StandardValues").IsNull());
}
...
TEST_F(StandardValueToEnumConversionTest, TestRootClassInGrandparentSchemaSameAlias) {
    ...
    CheckTypeName("base:TestClass_testProp", *middleSchema, "testProp", {"TestClass"});
    CheckTypeName("TestClass_testProp", *baseSchema, "testProp", {"TestClass"});
}

TEST_F(StandardValueToEnumConversionTest, TestRootClassInGrandparentSchemaDifferentAlias) {
    Utf8CP baseSchemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName='baseTest' version='1.0' nameSpacePrefix='base' xmlns='http://www.bentley.com/schemas/Bentley.ECXML.2.0'>
            <ECSchemaReference name='EditorCustomAttributes' version='01.00' prefix='beca' />
            <ECClass typeName='TestClass' isDomainClass='True'>
                <ECProperty propertyName='testProp' typeName='int'/>
            </ECClass>
...
TEST_F(StandardValueToEnumConversionTest, TestRootClassOnlyInGrandparentSchema) {
    Utf8CP baseSchemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName='baseTest' version='1.0' nameSpacePrefix='ts' xmlns='http://www.bentley.com/schemas/Bentley.ECXML.2.0'>
            <ECSchemaReference name='EditorCustomAttributes' version='01.00' prefix='beca' />
            <ECClass typeName='TestClass' isDomainClass='True'>
...
//@bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(StandardValueToEnumConversionTest, DroppedWhenAppliedToUnsupportedType) {
    Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="test" version="1.0" nameSpacePrefix="ts" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.2.0">
            <ECSchemaReference name="EditorCustomAttributes" version="01.00" prefix="beca" />
            <ECClass typeName="aStruct" isStruct="True" isDomainClass="False">
                <ECProperty propertyName="aProp" typeName="int" />
            </ECClass>
            <ECClass typeName="TestClass" isDomainClass="True">
                <ECStructProperty propertyName="structProp" typeName="aStruct">
                    <ECCustomAttributes>
                        <StandardValues xmlns="EditorCustomAttributes.01.00">
                           <ValueMap>
                                <ValueMap>
                ...
                <ECArrayProperty propertyName="structArrayProp" typeName="aStruct" minOccurs="0" maxOccurs="unbounded" isStruct="True">
                    <ECCustomAttributes>
                        <StandardValues xmlns="EditorCustomAttributes.01.00">
                           <ValueMap>
                                <ValueMap>
                ...
                <ECProperty propertyName="geomProp" typeName="Bentley.Geometry.Common.IGeometry">
                    <ECCustomAttributes>
                        <StandardValues xmlns="EditorCustomAttributes.01.00">
                           <ValueMap>
                                <ValueMap>
                ...
                <ECArrayProperty propertyName="point3dArrayProp" typeName="point3d">
                    <ECCustomAttributes>
                        <StandardValues xmlns="EditorCustomAttributes.01.00">
                           <ValueMap>
                                <ValueMap>
                ...
                <ECArrayProperty propertyName="point2dArrayProp" typeName="point2d">
                    <ECCustomAttributes>
                        <StandardValues xmlns="EditorCustomAttributes.01.00">
                           <ValueMap>
                                <ValueMap>
                ...
                <ECArrayProperty propertyName="binaryArrayProp" typeName="binary">
                    <ECCustomAttributes>
                        <StandardValues xmlns="EditorCustomAttributes.01.00">
                           <ValueMap>
                                <ValueMap>
                ...
                <ECArrayProperty propertyName="dateTimeArrayProp" typeName="dateTime">
                    <ECCustomAttributes>
                        <StandardValues xmlns="EditorCustomAttributes.01.00">
                           <ValueMap>
                                <ValueMap>
                ...
                <ECArrayProperty propertyName="geomArrayProp" typeName="Bentley.Geometry.Common.IGeometry">
                    <ECCustomAttributes>
                        <StandardValues xmlns="EditorCustomAttributes.01.00">
                           <ValueMap>
                                <ValueMap>
                ...
                <ECArrayProperty propertyName="boolArrayProp" typeName="boolean">
                    <ECCustomAttributes>
                        <StandardValues xmlns="EditorCustomAttributes.01.00">
                           <ValueMap>
                                <ValueMap>
                ...
                <ECArrayProperty propertyName="longArrayProp" typeName="long">
                    <ECCustomAttributes>
                        <StandardValues xmlns="EditorCustomAttributes.01.00">
                           <ValueMap>
                                <ValueMap>
...
TEST_F(StandardCustomAttributeConversionTests, TestDateTimeAndClassHasCurrentTimeStampPropertyConversion) {
    ...
    ASSERT_EQ(true, ECSchemaConverter::Convert(*schema, *m_readContext)) << "Failed to convert " << schema->GetFullSchemaName().c_str() << ".";
...
TEST_F(StandardCustomAttributeConversionTests, TestDynamicSchemaCAConversion) {
    ...
    ASSERT_TRUE(ECSchemaConverter::Convert(*dynamicSchema, *m_readContext));
...
TEST_F(StandardCustomAttributeConversionTests, TestSupplementedSchemaConversion) {
    ...
    supSchemas.push_back(supSchema.get());
    SupplementedSchemaBuilder builder;
    ASSERT_EQ(SupplementedSchemaStatus::Success, builder.UpdateSchema(*schema.get(), supSchemas, *context.get(), false)) << "Failed to supplement schema";
    EXPECT_TRUE(ECSchemaConverter::Convert(*schema.get(), *context.get())) << "Schema conversion failed";
...
void verifyHidden(ECPropertyCP ecProperty) {
    ...
    EXPECT_TRUE(hiddenPropertyCA.IsValid()) << "Expected to find the 'HiddenProperty' Custom Attribute on " << ecProperty->GetClass().GetFullName() << "." << ecProperty->GetName().c_str();
    ...
    EXPECT_TRUE(value.IsNull() || !value.GetBoolean()) << "Expected 'HiddenProperty.Show' CA value on " << ecProperty->GetClass().GetFullName() << "." << ecProperty->GetName().c_str() << " to be unset or set to false to be considered hidden";
}
...
void verifyShown(ECPropertyCP ecProperty) {
    ...
    EXPECT_TRUE(!value.IsNull() || value.GetBoolean()) << "Expected 'HiddenProperty.Show' CA value on " << ecProperty->GetClass().GetFullName() << "." << ecProperty->GetName().c_str() << " to be set to true to be considered shown";
}
...
void verifySchemaReferencesOnlyCoreCAs(ECSchemaCP convertedSchema) {
    ...
    EXPECT_STREQ("CoreCustomAttributes", convertedSchema->GetReferencedSchemas().begin()->second->GetName().c_str()) << "The only referenced schema does not have the correct name.";
}
...
void verifyHiddenSchemaAppliedCorrectly(Utf8CP schemaXml) {
    ...
    if (schema->GetDescription().Equals("Hide")) {
        ...
        EXPECT_TRUE(showClassesValue.IsNull() || !showClassesValue.GetBoolean()) << "Schema should be hidden based on the legacy DisplayOptions CA, we lack the information to ever set 'ShowClasses' to true so it should be unset or set to false";
...
// Expected conversion behavior of DisplayOptions for class and schema
// EC2 Hidden   | EC2 HideInstances | EC3 Hidden
//  True        |   False           |   False
//  unset       |   False           |   False
//  False       |   False           |   False
//  True        |   unset           |   True
...
TEST_F(StandardCustomAttributeConversionTests, DisplayOptionsCustomAttribute_AppliedToSchema) {
    ...
    // This is imperfect but follows how EC2 presentation rules handles the CA.
...
void verifyClassHidden(ECClassCP ecClass) {
    ...
    EXPECT_TRUE(showValue.IsNull() || !showValue.GetBoolean()) << "Expected class '" << ecClass->GetName().c_str() << "' to be hidden but the HiddenClass.Show value was set to true.";
}
...
void verifyClassNotHidden(ECClassCP ecClass) {
    ...
    ECValue showValue;
    EXPECT_EQ(ECObjectsStatus::Success, hiddenClassCA->GetValue(showValue, "Show"));
    EXPECT_TRUE(!showValue.IsNull() && showValue.GetBoolean()) << "Expected class '" << ecClass->GetName().c_str() << "' to be shown but the HiddenClass.Show value was not set or set to false.";
}
...
TEST_F(StandardCustomAttributeConversionTests, DisplayOptionsCustomAttribute_AppliedToClass) {
    ...
    Utf8CP schemaXml = R"xml(<?xml version="1.0" encoding="UTF-8"?>
        <ECSchema schemaName="TestSchema" namespacePrefix="ts" version="1.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.2.0">
            ...
            <ECClass typeName="F" isDomainClass="true" description="Hide">
                <ECCustomAttributes>
                    <DisplayOptions xmlns="Bentley_Standard_CustomAttributes.01.13">
                        <Hidden>true</Hidden>
                    </DisplayOptions>
            ...
            <ECClass typeName="G" isDomainClass="true" description="Hide">
                <ECCustomAttributes>
                    <DisplayOptions xmlns="Bentley_Standard_CustomAttributes.01.13">
                        <HideInstances>true</HideInstances>
                    </DisplayOptions>
            ...
            <ECClass typeName="H" isDomainClass="true" description="Show">
                <ECCustomAttributes>
                    <DisplayOptions xmlns="Bentley_Standard_CustomAttributes.01.13">
                        <Hidden>true</Hidden>
                        <HideInstances>false</HideInstances>
            ...
            <ECClass typeName="I" isDomainClass="true" description="Hide">
                <ECCustomAttributes>
                    <DisplayOptions xmlns="Bentley_Standard_CustomAttributes.01.13">
                        <Hidden>false</Hidden>
                        <HideInstances>true</HideInstances>
            ...
            <ECClass typeName="J" isDomainClass="true" description="Show">
                <ECCustomAttributes>
                    <DisplayOptions xmlns="Bentley_Standard_CustomAttributes.01.13">
                    </DisplayOptions>
                </ECCustomAttributes>
    ...
    for (const auto& ecClass : schema->GetClasses()) {
        if (ecClass->GetDescription().Equals("Hide"))
            ...
            EXPECT_TRUE(false) << ecClass->GetName().c_str() << " class must specify 'Hide' or 'Show' in its description to identify it as a class that should be hidden or shown";
    }
}
...
void validateClassMapConvertedCorrectly(Utf8CP schemaXml, bool expectSuccess, Utf8CP expectedMappingStrategy) {
    ...
    if (expectSuccess) {
        ...
        EXPECT_STREQ("ECDbMap.02.00.00", schemaReferenceFullName.c_str()) << "The only referenced schema does not have the correct name and version.";
    } else
...
//@bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(ECSchemaDownConverterTests, AddStandardValuesCAForEnumeration) {
    ECSchemaReadContextPtr schemaContext = ECSchemaReadContext::CreateContext();
    ...
    Utf8CP schemaXML = R"xml(<?xml version="1.0" encoding="utf-8"?>
        <ECSchema schemaName="TestSchema" version="01.00.00" displayLabel="Display Label" description="Description" alias="ts" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.2">
            <ECStructClass typeName="DistanceExpression" >
                <ECProperty propertyName="DistanceAlongFromStart" typeName="double" />
            </ECStructClass>
            <ECEnumeration typeName="RevisionStatus" backingTypeName="int" description="..." displayLabel="Revision Status" isStrict="false">
                <ECEnumerator value="200" name="NotApproved" displayLabel="Not Approved" />
                <ECEnumerator value="42" name="Approved" displayLabel="Approved" />
            </ECEnumeration>
            <ECEntityClass typeName="Document" modifier="None" >
                ...
                <ECProperty propertyName="RevisionStatus" typeName="RevisionStatus" displayLabel="Revision Status" />
                <ECStructProperty propertyName="AtPosition" typeName="DistanceExpression" />
            </ECEntityClass>
            <ECEntityClass typeName="MyDoc" modifier="None" >
    ...
    ASSERT_TRUE(ECSchemaDownConverter::Convert(*schema.get()));

    ASSERT_EQ(1, schema->GetReferencedSchemas().size());
    auto schemaRefs = schema->GetReferencedSchemas();
    SchemaKey ecaKey("EditorCustomAttributes", 1, 0, 0);
    ASSERT_TRUE(schemaRefs.end() != schemaRefs.Find(ecaKey, SchemaMatchType::Latest));
    std::map<int, Utf8String> expectedValues = {{200, "Not Approved"}, {42, "Approved"}};
    validateStandardValuesCA(schema->GetClassCP("Document")->GetPropertyP("RevisionStatus"), expectedValues, false);
    EXPECT_TRUE(schema->GetClassCP("MyDoc")->GetPropertyP("RevisionStatus")->GetCustomAttributeLocal("StandardValues").IsNull());
}

TEST_F(CustomAttributeRemovalTest, RemoveCustomAttrsInSchemaConversionWithDiffProps) {
    const Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="BuildingDataGroup" alias="bdg" version="1.0.0" displayLabel="BuildingDataGroup" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.2">

            <ECCustomAttributeClass typeName="XpathDGMapperProperty" appliesTo="PrimitiveProperty"/>
            <ECCustomAttributeClass typeName="DGPropTypeMapperProperty" appliesTo="PrimitiveProperty"/>
            <ECCustomAttributeClass typeName="DGExtendedType" appliesTo="PrimitiveProperty"/>
            <ECCustomAttributeClass typeName="DataGroupCatalogInstanceNameProperty" appliesTo="PrimitiveProperty"/>
            <ECCustomAttributeClass typeName="DataGroupClassNameProperty" appliesTo="PrimitiveProperty"/>

            <ECCustomAttributeClass typeName="OtherCA" appliesTo="PrimitiveProperty" />

            <ECEntityClass typeName="StripCATestClass" displayLabel="StripCATestClass">
                <ECProperty propertyName="A" typeName="string" displayLabel="TestProp">
                    <ECCustomAttributes>
                        <DGPropTypeMapperProperty xmlns="BuildingDataGroup.01.00.01"/>
                        <DGExtendedType xmlns="BuildingDataGroup.1.0.0"/>
                        <DataGroupClassNameProperty xmlns="BuildingDataGroup.01.00.01"/>
                        <XpathDGMapperProperty xmlns="BuildingDataGroup.1.0.0"/>
                        <DataGroupCatalogInstanceNameProperty xmlns="BuildingDataGroup.01.00.01"/>
                        <OtherCA xmlns="BuildingDataGroup.01.00.01" />
                    </ECCustomAttributes>
                </ECProperty>

                <ECProperty propertyName="B" typeName="string" displayLabel="TestProp">
                    <ECCustomAttributes>
                        <OtherCA xmlns="BuildingDataGroup.01.00.01" />
                        <XpathDGMapperProperty xmlns="BuildingDataGroup.1.0.0"/>
                        <DGPropTypeMapperProperty xmlns="BuildingDataGroup.01.00.01"/>
                ...
                <ECProperty propertyName="C" typeName="string" displayLabel="TestProp">
                    <ECCustomAttributes>
                        <XpathDGMapperProperty xmlns="BuildingDataGroup.1.0.0"/>
                        <DataGroupCatalogInstanceNameProperty xmlns="BuildingDataGroup.01.00.01"/>
                        <DGPropTypeMapperProperty xmlns="BuildingDataGroup.01.00.01"/>
    ...
    auto verifyCAs = [](ECSchemaR schema, std::vector<Utf8String> caToRemove, std::vector<Utf8String> caToLeave) {
        for (const auto& ecClass : schema.GetClasses()) {
            for (const auto& ecProperty : ecClass->GetProperties(false)) {
                for (const auto& caName : caToLeave)
                    EXPECT_TRUE(ecProperty->GetCustomAttribute("BuildingDataGroup", caName).IsValid()) << "CA '" << caName.c_str() << " on " << ecProperty->GetName().c_str() << " removed.";
                for (const auto& caName : caToRemove)
                    EXPECT_FALSE(ecProperty->GetCustomAttribute("BuildingDataGroup", caName).IsValid()) << "CA " << caName.c_str() << " on " << ecProperty->GetName().c_str() << " not removed.";
            }
        }
    };
    ...
    // Test Removal works
    {
        ...
        auto converter = CustomECSchemaConverter::Create();
        converter->AddCustomAttributeNameToRemove("BuildingDataGroup:XpathDGMapperProperty");
