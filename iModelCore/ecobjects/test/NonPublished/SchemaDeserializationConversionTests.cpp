...
void TestOverriding(Utf8CP schemaName, int readVersion, bool allowOverriding)
    ...
    if (!allowOverriding)
        ...
        ASSERT_NE(nullptr, child) << "Couldn't find 'child' class in " << schemaName;
        for (Utf8CP renamedPropName : { "TestSchema_IntegerProperty_", "TestSchema_IntArrayProperty_"})
            {
            ECPropertyP renamedProp = child->GetPropertyP(renamedPropName);
            ASSERT_NE(nullptr, renamedProp) << "Couldn't find renamed property " << renamedPropName << " in schema " << schemaName;
            EXPECT_EQ(renamedPropName, "TestSchema_" + renamedProp->GetDisplayLabel() + "_") << "Display label not set when prop renamed " << schemaName;
            }
        return;
        }
