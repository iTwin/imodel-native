...
struct ExtendedTypeTests : ECTestFixture {
    ...
    static void VerifyExtendedTypeOnProperty(
        ...
        EXPECT_EQ(expectedExtendedTypeName, extendedTypeName) << contextMessage << "Unexpected extended type name '" + extendedTypeName + "' expected: '" + expectedExtendedTypeName + "' on property " << ecProperty->GetClass().GetName() + "." + ecProperty->GetName();
    }
...
TEST_F(ExtendedTypeTests, WriteSchemaWithExtendedTypes) {
    ...
    classPtr->CreatePrimitiveProperty(propertyPtr, "knownExtendedType", PRIMITIVETYPE_String);
