...
TEST_F(SchemaTest, FindNameForSchemaItem)
    ...
    KindOfQuantityP koqA, koqAp;
    PropertyCategoryP catA, catAp;
    UnitSystemP systemA, systemAp;
    PhenomenonP phenA, phenAp;
    ECUnitP unitA, unitAp;
    ECFormatP formatA, formatAp;

    ECSchema::CreateSchema(schema, "TestSchema", "ts", 5, 5, 5);

    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateEntityClass(classA, "ClassA"));
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateEnumeration(enumA, "EnumA", PrimitiveType::PRIMITIVETYPE_Integer));
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateKindOfQuantity(koqA, "KoqA"));
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreatePropertyCategory(catA, "CatA"));
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateUnitSystem(systemA, "SystemA"));
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreatePhenomenon(phenA, "PhenA", "PhenA"));
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateUnit(unitA, "UnitA", "UnitA", *phenA, *systemA));
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateFormat(formatA, "FormatA"));

    ECSchemaPtr schemaP;
    ECSchema::CreateSchema(schemaP, "TestSchema2", "ts2", 1, 1, 1);
    EXPECT_EQ(ECObjectsStatus::Success, schemaP->CreateEntityClass(classAp, "EnumA"));
    EXPECT_EQ(ECObjectsStatus::Success, schemaP->CreateEnumeration(enumAp, "KoqA", PrimitiveType::PRIMITIVETYPE_Integer));
    EXPECT_EQ(ECObjectsStatus::Success, schemaP->CreateKindOfQuantity(koqAp, "CatA"));
    EXPECT_EQ(ECObjectsStatus::Success, schemaP->CreatePropertyCategory(catAp, "SystemA"));
    EXPECT_EQ(ECObjectsStatus::Success, schemaP->CreateUnitSystem(systemAp, "PhenA"));
    EXPECT_EQ(ECObjectsStatus::Success, schemaP->CreatePhenomenon(phenAp, "UnitA", "UnitA"));
    EXPECT_EQ(ECObjectsStatus::Success, schemaP->CreateUnit(unitAp, "FormatA", "FormatA", *phenAp, *systemAp));
    EXPECT_EQ(ECObjectsStatus::Success, schemaP->CreateFormat(formatAp, "ClassA"));

    // Get New name when type is different than item with same name
    EXPECT_STREQ("EnumA_", schema->FindNameForSchemaItem(classAp).c_str());
    EXPECT_STREQ("KoqA_", schema->FindNameForSchemaItem(enumAp).c_str());
    EXPECT_STREQ("CatA_", schema->FindNameForSchemaItem(koqAp).c_str());
    EXPECT_STREQ("SystemA_", schema->FindNameForSchemaItem(catAp).c_str());
    EXPECT_STREQ("PhenA_", schema->FindNameForSchemaItem(systemAp).c_str());
    EXPECT_STREQ("UnitA_", schema->FindNameForSchemaItem(phenAp).c_str());
    EXPECT_STREQ("FormatA_", schema->FindNameForSchemaItem(unitAp).c_str());
    EXPECT_STREQ("ClassA_", schema->FindNameForSchemaItem(formatAp).c_str());

    // Return name when type is the same as item with same name
    EXPECT_STREQ("ClassA", schema->FindNameForSchemaItem(classA).c_str());
    EXPECT_STREQ("EnumA", schema->FindNameForSchemaItem(enumA).c_str());
    EXPECT_STREQ("KoqA", schema->FindNameForSchemaItem(koqA).c_str());
    EXPECT_STREQ("CatA", schema->FindNameForSchemaItem(catA).c_str());
    EXPECT_STREQ("SystemA", schema->FindNameForSchemaItem(systemA).c_str());
    EXPECT_STREQ("PhenA", schema->FindNameForSchemaItem(phenA).c_str());
    EXPECT_STREQ("UnitA", schema->FindNameForSchemaItem(unitA).c_str());
    EXPECT_STREQ("FormatA", schema->FindNameForSchemaItem(formatA).c_str());

    // Check that we'll keep adding dashes
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateEntityClass(classApp, "FormatA_"));
    EXPECT_EQ(ECObjectsStatus::Success, schemaP->CreateEnumeration(enumApp, "FormatA_", PrimitiveType::PRIMITIVETYPE_Integer));
    EXPECT_STREQ("FormatA__", schema->FindNameForSchemaItem(enumApp).c_str());
...
TEST_F(SchemaReferenceTest, TryRemoveSchemaInUse)
    ...
    schema->CreateEntityClass(class1, "TestClass1");
    ...
    PhenomenonP phenom;
    UnitSystemP system;
    unitSchema->CreatePhenomenon(phenom, "ExamplePhenomenon", "LENGTH");
    unitSchema->CreateUnitSystem(system, "ExampleUnitSystem");
    ECUnitP unit;
    unitSchema->CreateUnit(unit, "CM", "CM", *phenom, *system);
    EXPECT_NE(nullptr, unit);

    KindOfQuantityP koq1;
    schema->CreateKindOfQuantity(koq1, "KindOfQuantity1");
    koq1->SetPersistenceUnit(*unit);
    ...
    schema->CreateEntityClass(class2, "TestClass2");
    PrimitiveECPropertyP prop1;
    class2->CreatePrimitiveProperty(prop1, "Length", PRIMITIVETYPE_String);
    ...
    EXPECT_EQ(1, schema->GetKindOfQuantityCount());
    EXPECT_EQ(ECObjectsStatus::Success, schema->RemoveReferencedSchema(*unitSchema, true));
    EXPECT_EQ(false, prop1->IsKindOfQuantityDefinedLocally());  // dropReferences=true above removed kindofquantity from property
    EXPECT_EQ(0, schema->GetKindOfQuantityCount());             // dropReferences=true above removed kindofquantity from schema

    // Test format in referenced schema
    ECUnitCP meter = ECTestFixture::GetUnitsSchema()->GetUnitCP("M");
    ASSERT_NE(nullptr, meter);

    ECSchemaPtr formatSchema = ECTestFixture::GetFormatsSchema();
    ECFormatCP format = formatSchema->GetFormatCP("AmerI");
    ASSERT_NE(nullptr, format);
    ...
    EXPECT_EQ(ECObjectsStatus::Success, schema->AddReferencedSchema(*refSchema));
    refSchema->CreatePropertyCategory(proCategory, "PropertyCategory");
    class2->CreatePrimitiveProperty(prop2, "PrimProp1", PRIMITIVETYPE_String);
    prop2->SetCategory(proCategory);
    ...
    schema->CreateEntityClass(class3, "TestClass3");
    PrimitiveECPropertyP prop3;
    class3->CreatePrimitiveProperty(prop3, "PrimProp2", PRIMITIVETYPE_String);
    KindOfQuantityP koq4;
    refSchema->CreateKindOfQuantity(koq4, "KindOfQuantity4");
    EXPECT_NE(nullptr, koq4);
    koq4->SetPersistenceUnit(*unit);
    EXPECT_EQ(ECObjectsStatus::Success, schema->AddReferencedSchema(*refSchema));
    EXPECT_EQ(ECObjectsStatus::Success, prop3->SetKindOfQuantity(koq4));
...
TEST_F(SchemaReferenceTest, TryRemoveSchemaInUseWithCustomAttributes)
    ...
    schema->CreateRelationshipClass(relClass, "RelClass");
    schema->CreateEntityClass(entityClass, "EntityClass");
    entityClass->CreatePrimitiveProperty(prop, "Prop", PRIMITIVETYPE_String);
...
TEST_F(SchemaCreationTest, CanFullyCreateASchema)
    ...
    class1->CreatePrimitiveProperty(stringProp, "StringMember", PRIMITIVETYPE_String);
    class1->CreateStructProperty(structProp, "StructMember", *structClass);
    class1->CreateStructArrayProperty(nestedArrayProp, "NestedArray", *structClass);
    ...
    class1->CreatePrimitiveProperty(binaryProperty, "BinaryProp", PRIMITIVETYPE_String);
    class1->CreatePrimitiveProperty(booleanProperty, "BooleanProp", PRIMITIVETYPE_String);
    class1->CreatePrimitiveProperty(dateTimeProperty, "DateTimeProp", PRIMITIVETYPE_String);
    class1->CreatePrimitiveProperty(doubleProperty, "DoubleProp", PRIMITIVETYPE_String);
    class1->CreatePrimitiveProperty(integerProperty, "IntProp", PRIMITIVETYPE_String);
    class1->CreatePrimitiveProperty(longProperty, "LongProp", PRIMITIVETYPE_String);
    class1->CreatePrimitiveProperty(point2DProperty, "Point2dProp", PRIMITIVETYPE_String);
    class1->CreatePrimitiveProperty(point3DProperty, "Point3dProp", PRIMITIVETYPE_String);
    ...
    PrimitiveECPropertyP conflictProp;
    EXPECT_EQ(ECObjectsStatus::NamedItemAlreadyExists, class1->CreatePrimitiveProperty(conflictProp, "IntProp2", PRIMITIVETYPE_Double));
    EXPECT_EQ(ECObjectsStatus::Success, class1->CreatePrimitiveProperty(conflictProp, "IntProp2", PRIMITIVETYPE_Double, true));
    EXPECT_TRUE(nullptr != class1->GetPropertyP("ts_IntProp2_"));
...
TEST_F(SchemaCreationTest, OriginalXmlVersionEqualsInputVersion_UnlessInputVersionIsLowerThan31)
    ...
    ECSchema::CreateSchema(testSchema, "TestSchema", "ts", 1, 0, 2, ECVersion::V2_0);
    ...
    ECSchema::CreateSchema(testSchema, "TestSchema", "ts", 1, 0, 2, ECVersion::V3_1);
    EXPECT_EQ(3, testSchema->GetOriginalECXmlVersionMajor());
    EXPECT_EQ(1, testSchema->GetOriginalECXmlVersionMinor());
    ECSchema::ParseECVersion(testSchemaMajor, testSchemaMinor, testSchema->GetECVersion());
    EXPECT_EQ(3, testSchemaMajor);
    EXPECT_EQ(1, testSchemaMinor);

    ECSchema::CreateSchema(testSchema, "TestSchema", "ts", 1, 0, 2, ECVersion::V3_2);
    EXPECT_EQ(3, testSchema->GetOriginalECXmlVersionMajor());
    EXPECT_EQ(2, testSchema->GetOriginalECXmlVersionMinor());
    ECSchema::ParseECVersion(testSchemaMajor, testSchemaMinor, testSchema->GetECVersion());
    EXPECT_EQ(3, testSchemaMajor);
    EXPECT_EQ(2, testSchemaMinor);
...
TEST_F(SchemaCreationTest, AddBaseClassWithPropertyCaseDifferencesInDerivedClass)
    ...
    testSchema->CreateEntityClass(derived3, "Derived3");
    PrimitiveECPropertyP d3PropA;
    derived3->CreatePrimitiveProperty(d3PropA, "PropertyA", PRIMITIVETYPE_Double);
    PrimitiveECPropertyP d3PropB;
    derived3->CreatePrimitiveProperty(d3PropB, "PropertyB", PRIMITIVETYPE_Double);

    ECEntityClassP derived2;
    testSchema->CreateEntityClass(derived2, "Derived2");
    PrimitiveECPropertyP d2PropA;
    derived2->CreatePrimitiveProperty(d2PropA, "PropertyA", PRIMITIVETYPE_Double);
    PrimitiveECPropertyP d2PropB;
    derived2->CreatePrimitiveProperty(d2PropB, "PropertyB", PRIMITIVETYPE_Double);
    derived3->AddBaseClass(*derived2);

    ECEntityClassP derived1;
    testSchema->CreateEntityClass(derived1, "Derived1");
    PrimitiveECPropertyP d1PropB;
    derived1->CreatePrimitiveProperty(d1PropB, "PropertyB", PRIMITIVETYPE_Double);
    derived2->AddBaseClass(*derived1);

    ECEntityClassP base;
    testSchema->CreateEntityClass(base, "Base");
    PrimitiveECPropertyP bPropA;
    base->CreatePrimitiveProperty(bPropA, "propertya", PRIMITIVETYPE_Double);
    PrimitiveECPropertyP bPropB;
    ...
    validateProperty(derived2, "propertya", "Base");
    validateProperty(derived2, "propertyb", "Derived1");

    EXPECT_EQ(nullptr, derived1->GetPropertyP("PropertyA", false));
    validateProperty(derived1, "propertyb", "Base");
...
TEST_F(SchemaVersionTest, CreateECVersionTest)
    ...
    EXPECT_STREQ("3.3", ECSchema::GetECVersionString(ECVersion::Latest)) << "ECVersion Latest should be equal to 3.3, therefore the string of it should be equal to 3.3.";
    ...
    EXPECT_EQ(ECVersion::Latest, ecVersion3_3) << "ECVersion Latest should be equal to 3.3, therefore the comparsion should succeed.";
    }
...
TEST_F(SchemaVersionTest, CreateSchemaECVersionTest)
    ...
    ECSchemaPtr schema;
    ECSchema::CreateSchema(schema, "TestSchema", "ts", 5, 0, 5);
    EXPECT_TRUE(schema->IsECVersion(ECVersion::V3_3)) << "The default schema ECVersion should be EC3.3 so this should validate to true.";
    ...
    EXPECT_TRUE(schema->IsECVersion(ECVersion::V3_3)) << "The schema was created as the Latest version which is EC3.3 so this should validate to true.";
    ...
    {
    ECSchemaPtr schema;
    ECSchema::CreateSchema(schema, "TestSchema", "ts", 5, 0, 5, ECVersion::V3_3);
    EXPECT_TRUE(schema->IsECVersion(ECVersion::V3_3)) << "The schema was created as an EC3.3 schema.";
    EXPECT_TRUE(schema->IsECVersion(ECVersion::Latest)) << "The schema was created as an EC3.3 schema so Latest should return true.";
    }
    {
    ECSchemaPtr schema;
    ...
    EXPECT_FALSE(schema->IsECVersion(ECVersion::Latest)) << "The schema was created as an EC3.2 schema so Latest should return true.";
    }
    {
    ECSchemaPtr schema;
...
TEST_F(SchemaCreationTest, CodifyAllowedNamelessItems)
    ...
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateUnitSystem(unitSystem, "", "ExampleUnitSystemLabel", "ExampleUnitSystemDescription"))
        << "can create a unit system with an empty name";
    schema->DeleteUnitSystem(*unitSystem);
    ...
    PhenomenonP phenom;
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreatePhenomenon(phenom, "", "LENGTH", "ExamplePhenomenonLabel", "ExamplePhenomenonDescription"))
        << "can create a phenomenon with an empty name";
    schema->DeletePhenomenon(*phenom);
    ...
    ECUnitP unit;
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateUnit(unit, "", "[MILLI]M", *phenom, *unitSystem, 10.0, 1.0, 1.0, "ExampleUnitLabel", "ExampleUnitDescription"))
        << "can create a unit with an empty name";
    schema->DeleteUnit(*unit);
    ...
    ECUnitP constant;
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateConstant(constant, "", "ONE", *phenom, 10.0, "ExampleConstantLabel", "ExampleConstantDescription"))
        << "can create a constant with an empty name";
    schema->DeleteUnit(*constant);
    ...
    ECUnitP invertedUnit;
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateInvertedUnit(invertedUnit, *unit, "", *unitSystem, "ExampleInvertedUnitLabel", "ExampleInvertedUnitDescription"))
        << "can create an inverted unit with an empty name";
    schema->DeleteUnit(*invertedUnit);
    ...
    ECFormatP format;
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateFormat(format, "", "ExampleFormatLabel", "ExampleFormatDescription"))
        << "can create a format with an empty name";
    schema->DeleteFormat(*format);
    ...
    KindOfQuantityP koq2;
    EXPECT_EQ(ECObjectsStatus::Success, schema->CreateKindOfQuantity(koq2, ""))
        << "can create a kind of quantity with an empty name";
    schema->DeleteKindOfQuantity(*koq2);
