...
struct NavigationPropertyValueTests : ECTestFixture {
    ...
    void TestSettingNavPropLongValuesWithRel(IECInstanceR instance, const std::array<ECRelationshipClassP, 3>& expectedRelClasses);
...
//@bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(NavigationECPropertyTests, ValidNavigationPropertyOverrides) {
    {
        Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.1">
            <ECEntityClass typeName="SourceClass" >
        ...
        ECSchemaPtr schema;
        ECSchemaReadContextPtr context = ECSchemaReadContext::CreateContext();
        EXPECT_EQ(SchemaReadStatus::Success, ECSchema::ReadFromXmlString(schema, schemaXml, *context)) << "Should deserialize successfully because the class with the overriding navigation property is also listed as a relationship constraint.";
    }
    {
        Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.1">
        ...
        ECSchemaPtr schema;
        ECSchemaReadContextPtr context = ECSchemaReadContext::CreateContext();
        EXPECT_EQ(SchemaReadStatus::Success, ECSchema::ReadFromXmlString(schema, schemaXml, *context)) << "Should deserialize successfully because the class with the overriding navigation property is a valid constraint class.";
    }
    // !!These two cases encode bad behavior that exists in the implementation!!

    // Relationship Constraint has polymorphic set to false but class definition is not sealed.
    // So derived classes can inherit nav prop for relationship that should only be valid for the base class.
    // We have always supported this so we cannot make it a schema load failure.
    // If the code were fixed SourceClass would have to be marked sealed and DerivedSource would have to be removed.
    {
        Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.1">
            <ECEntityClass typeName="SourceClass" >
            ...
            <ECEntityClass typeName="TargetClass" />
            <ECRelationshipClass typeName="RelClass" strength="Referencing" modifier="None">
                <Source multiplicity="(0..1)" roleLabel="source" polymorphic="false">
                    <Class class="SourceClass"/>
                </Source>
                <Target multiplicity="(1..1)" roleLabel="target" polymorphic="true">
                    <Class class="TargetClass"/>
        ...
        ECSchemaPtr schema;
        ECSchemaReadContextPtr context = ECSchemaReadContext::CreateContext();
        EXPECT_EQ(SchemaReadStatus::Success, ECSchema::ReadFromXmlString(schema, schemaXml, *context)) << "This is wrong but we have always allowed it so we will continue to allow it";
    }
    // Same as the case above but a property override for the navigation property is applied in the derived class.
    // It never used to be possible to override a navigation property so this case can be safely turned into a schema load failure.
    // If the code were fixed this test case would continue to fail
    {
        Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.1">
            <ECEntityClass typeName="SourceClass" >
        ...
        ECSchemaPtr schema;
        ECSchemaReadContextPtr context = ECSchemaReadContext::CreateContext();
        EXPECT_NE(SchemaReadStatus::Success, ECSchema::ReadFromXmlString(schema, schemaXml, *context)) << "Should not deserialize successfully because the class with the overriding navigation property is not a valid constraint class.";
    }
}
...
//@bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(NavigationECPropertyTests, NavigationPropertiesWithAbstractConstraints) {
    // It would be better if you had to define the navigation property on the abstract constraint to ensure that it is consistent in
    // all concrete constraints.  Doing that would mean the FK might show up in classes where it is not actually supported.
    // Ideally the Nav prop would be defined or inherited into every constraint class and have the same name.  This is not enforced
    // in EC but is enforced in ECDb.

    // Navigation properties defined on concrete constraints but not on abstract constraint
    {
        Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.2">
            ...
            <ECEntityClass typeName="DerivedSource2">
                <BaseClass>SourceClass</BaseClass>
                <ECNavigationProperty propertyName="NavProp" relationshipName="RelClass" direction="forward"/>
            </ECEntityClass>
            <ECEntityClass typeName="TargetClass" />
        ...
        ECSchemaPtr schema;
        ECSchemaReadContextPtr context = ECSchemaReadContext::CreateContext();
        EXPECT_EQ(SchemaReadStatus::Success, ECSchema::ReadFromXmlString(schema, schemaXml, *context)) << "Should deserialize Navigation properties defined on concrete constraints but not on abstract constraint.";
    }
    // Navigation property cannot be defined on the abstract constraint
    {
        Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.2">
            <ECEntityClass typeName="SourceClass" >
        ...
        ECSchemaPtr schema;
        ECSchemaReadContextPtr context = ECSchemaReadContext::CreateContext();
        EXPECT_NE(SchemaReadStatus::Success, ECSchema::ReadFromXmlString(schema, schemaXml, *context)) << "Should not deserialize Navigation property defined on abstract constraint.";
    }
    // Navigation properties defined on multiple concrete constraints that are parent/children
    {
        Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.2">
            <ECEntityClass typeName="SourceClass" >
        ...
        ECSchemaPtr schema;
        ECSchemaReadContextPtr context = ECSchemaReadContext::CreateContext();
        EXPECT_EQ(SchemaReadStatus::Success, ECSchema::ReadFromXmlString(schema, schemaXml, *context)) << "Should deserialize Navigation properties defined on multiple concrete constraints that are parent/children.";
    }
    // Navigation properties defined on concrete constraints but not on abstract constraint, concrete constraints use different names
    // This is bad and not supported in ECDb
    {
        Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.2">
            <ECEntityClass typeName="SourceClass" >
            ...
            <ECEntityClass typeName="DerivedSource2">
                <BaseClass>SourceClass</BaseClass>
                <ECNavigationProperty propertyName="NavPropDiff" relationshipName="RelClass" direction="forward"/>
            </ECEntityClass>
            <ECEntityClass typeName="TargetClass" />
            <ECRelationshipClass typeName="RelClass" strength="Referencing" modifier="None">
                <Source multiplicity="(0..1)" roleLabel="source" polymorphic="true" abstractConstraint="SourceClass">
        ...
        ECSchemaPtr schema;
        ECSchemaReadContextPtr context = ECSchemaReadContext::CreateContext();
        EXPECT_EQ(SchemaReadStatus::Success, ECSchema::ReadFromXmlString(schema, schemaXml, *context)) << "Should deserialize Navigation properties defined on concrete constraints but not on abstract constraint, concrete constraints use different names.";
    }
    // Navigation properties defined on multiple concrete constraints that are parent/children, base constraint uses different name
    // This is bad and is not supported in ECDb
    {
        Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.2">
            <ECEntityClass typeName="SourceClass" >
                <ECNavigationProperty propertyName="NavPropBase" relationshipName="RelClass" direction="forward"/>
            </ECEntityClass>
            <ECEntityClass typeName="DerivedSource">
                <BaseClass>SourceClass</BaseClass>
                <ECNavigationProperty propertyName="NavProp" relationshipName="RelClass" direction="forward"/>
        ...
        ECSchemaPtr schema;
        ECSchemaReadContextPtr context = ECSchemaReadContext::CreateContext();
        EXPECT_EQ(SchemaReadStatus::Success, ECSchema::ReadFromXmlString(schema, schemaXml, *context)) << "Should deserialize Navigation properties defined on multiple concrete constraints that are parent/children, base constraint uses different name.";
    }
}
...
//@bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
TEST_F(NavigationECPropertyTests, NavigationPropertiesOnBothEndpoints) {
    // This is supported by ECObjects but not by ECDb
    {
        Utf8CP schemaXml = R"xml(<?xml version='1.0' encoding='UTF-8'?>
        <ECSchema schemaName="TestSchema" alias="ts" version="1.0.0" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.2">
            <ECEntityClass typeName="SourceClass" >
                <ECNavigationProperty propertyName="SourceToTarget" relationshipName="RelClass" direction="forward"/>
            </ECEntityClass>
            <ECEntityClass typeName="TargetClass" >
                <ECNavigationProperty propertyName="TargetToSource" relationshipName="RelClass" direction="backward"/>
            </ECEntityClass>
            <ECRelationshipClass typeName="RelClass" strength="Referencing" modifier="None">
                <Source multiplicity="(0..1)" roleLabel="source" polymorphic="true">
                    <Class class="SourceClass"/>
    ...
    if (!expectedRelClassId.IsValid()) {
        ...
        if (nullptr != expectedRelClass) {
            ...
            EXPECT_TRUE(ECClass::ClassesAreEqualByName(expectedRelClass, relClass)) << "The relationship for '" << propertyAccessor << "' was not the expected relationship."
                                                                                    << " Got " << relClass->GetName() << " but expected " << expectedRelClass->GetName();
        } else {
...
//@bsimethod
//+---------------+---------------+---------------+---------------+---------------+------
void NavigationPropertyValueTests::TestSettingNavPropLongValuesWithRel(IECInstanceR instance, const std::array<ECRelationshipClassP, 3>& expectedRelClasses) {
    BeInt64Id navId(42);
    ECValue myTargetValue(navId, expectedRelClasses[0]);
    ...
    EXPECT_EQ(expectedRelClasses[0], myTargetValue.GetNavigationInfo().GetRelationshipClass()) << "Relationship Class of MyTarget nav property not as expected";

    VerifyNavPropLongValue(instance, "MyTarget", navId, expectedRelClasses[0]);
...
void NavigationPropertyValueTests::VerifyInstanceXml(ECSchemaPtr schema, IECInstanceR sourceInstance, IECInstanceR sourceDeserialized) {
    ...
    VerifyNavPropLongValue(sourceDeserialized, "MyTarget", BeInt64Id(42), schema->GetClassCP("DerivedRelClass0")->GetRelationshipClassCP());
}
...
void NavigationPropertyValueTests::InstanceWithNavProp() {
    ...
    // we can't put multiple navprops of the same relation in one class so we use a different relationship class for each navprop
    std::array<ECRelationshipClassP, 3> relClasses;
    std::array<ECRelationshipClassP, 3> derivedRelClasses;
    std::array<ECRelationshipClassP, 3> relClass2s;

    ECSchema::CreateSchema(schema, "NavTest", "ts", 4, 0, 2);
    for (int i = 0; i < relClasses.size(); ++i)
        schema->CreateRelationshipClass(relClasses[i], Utf8PrintfString("RelClass_%d", i));
    for (int i = 0; i < derivedRelClasses.size(); ++i)
        schema->CreateRelationshipClass(derivedRelClasses[i], Utf8PrintfString("DerivedRelClass%d", i));
    for (int i = 0; i < relClass2s.size(); ++i)
        schema->CreateRelationshipClass(relClass2s[i], Utf8PrintfString("RelClass2_%d", i));
    schema->CreateEntityClass(sourceClass, "Source");
    schema->CreateEntityClass(targetClass, "Target");
    ...
    for (int i = 0; i < derivedRelClasses.size(); ++i) {
        auto& derivedRelClass = derivedRelClasses[i];
        derivedRelClass->GetSource().SetMultiplicity(RelationshipMultiplicity::ZeroOne());
        derivedRelClass->GetSource().AddClass(*sourceClass);
        derivedRelClass->GetTarget().SetMultiplicity(RelationshipMultiplicity::OneOne());
        derivedRelClass->GetTarget().AddClass(*derivedTargetClass);
        derivedRelClass->AddBaseClass(*relClasses[0]);
        derivedRelClass->SetId(ECClassId((uint64_t)25));
    }

    for (auto& relClass2 : relClass2s) {
        relClass2->GetSource().AddClass(*sourceClass);
        relClass2->GetSource().SetMultiplicity(RelationshipMultiplicity::ZeroMany());
        relClass2->GetTarget().AddClass(*targetClass);
    ...
    NavigationECPropertyP navPropSource;
    CreateNavProp(sourceClass, "MyTarget", *relClasses[0], ECRelatedInstanceDirection::Forward, navPropSource);
    NavigationECPropertyP navPropSourceNoRel;
    CreateNavProp(sourceClass, "MyTargetNoRel", *relClasses[1], ECRelatedInstanceDirection::Forward, navPropSourceNoRel);
    NavigationECPropertyP navPropSourceRelId;
    CreateNavProp(sourceClass, "MyTargetWithRelId", *relClasses[2], ECRelatedInstanceDirection::Forward, navPropSourceRelId);
    ...
    TestSettingNavPropLongValuesWithRel(*sourceInstance, derivedRelClasses);
...
TEST_F(NavigationPropertyValueTests, TestRelClassNameJsonNotSerializedForRelationshipClassesWithSealedModifier) {
    ...
    static auto const ValidateTestNavProp = [&](ECEntityClassP const& sourceClass, ECRelationshipClassP const& navPropRelClass, bool shouldRelClassNameBeSerialized) -> void {
        ...
        EXPECT_TRUE((membs.end() == std::find_if(
                                        membs.begin(),
                                        membs.end(),
                                        [](Utf8StringCR _) -> bool { return _ == ECJsonSystemNames::Navigation::RelClassName(); })) == !shouldRelClassNameBeSerialized);
    };
