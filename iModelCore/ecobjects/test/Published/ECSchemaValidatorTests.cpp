...
TEST_F(SchemaValidatorTests, EntityClassesMayNotHaveTheSameDisplayLabel)
    ...
    ASSERT_TRUE(ECSchemaValidator::Validate(*schema)) << "Should succeed validation as no display labels are defined";

    ASSERT_EQ(ECObjectsStatus::Success, entity1->SetDisplayLabel("E0"));
    ASSERT_FALSE(ECSchemaValidator::Validate(*schema)) << "Should fail validation as defined display label conflicts with a class name without a defined display label";
    ...
    ASSERT_TRUE(ECSchemaValidator::Validate(*schema)) << "Should succeed validation as defined display labels are different";
    ...
    ASSERT_FALSE(ECSchemaValidator::Validate(*schema)) << "Should fail validation as defined display labels are the same";
    }
...
TEST_F(SchemaValidatorTests, PropertiesMayNotHaveTheSameDisplayLabelAndCategory)
    ...
    ASSERT_TRUE(ECSchemaValidator::Validate(*schema)) << "Entity class has two properties with different names so validation should succeed";

    ASSERT_EQ(ECObjectsStatus::Success, prop2->SetDisplayLabel("P1"));
    ASSERT_FALSE(ECSchemaValidator::Validate(*schema)) << "Entity class has two properties with the same display label and no defined category so validation should fail";
    ...
    ASSERT_TRUE(ECSchemaValidator::Validate(*schema)) << "Entity class has two properties with different display labels so validation should succeed";
    ...
    ASSERT_TRUE(ECSchemaValidator::Validate(*schema)) << "Entity class has two properties with the same display label but different categories so validation should succeed";

    ASSERT_EQ(ECObjectsStatus::Success, prop2->SetCategory(cat1));
    ASSERT_FALSE(ECSchemaValidator::Validate(*schema)) << "Entity class has two properties with the same display label and category so validation should fail";
...
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F(SchemaValidatorTests, NoClassShouldSubclassPhysicalModel)
    {
    // Test that an entity class may not subclass bis:PhysicalModel
    ECSchemaPtr bisSchema;
    ECEntityClassP bisEntity, bisPhysicalModel;
    ECSchemaPtr schema;
    ECEntityClassP entity0, entity1;

    ASSERT_EQ(ECObjectsStatus::Success, ECSchema::CreateSchema(bisSchema, "BisCore", "bis", 1, 1, 1));
    ASSERT_EQ(ECObjectsStatus::Success, bisSchema->CreateEntityClass(bisEntity, "BisEntity"));
    ASSERT_EQ(ECObjectsStatus::Success, bisSchema->CreateEntityClass(bisPhysicalModel, "PhysicalModel"));
    ASSERT_EQ(ECObjectsStatus::Success, ECSchema::CreateSchema(schema, "EntityClassSchema", "ECC", 1, 1, 1));
    ASSERT_EQ(ECObjectsStatus::Success, schema->AddReferencedSchema(*bisSchema));
    ASSERT_EQ(ECObjectsStatus::Success, schema->CreateEntityClass(entity0, "GoodEntity"));
    ASSERT_EQ(ECObjectsStatus::Success, entity0->AddBaseClass(*bisEntity));

    ASSERT_TRUE(ECSchemaValidator::Validate(*schema)) << "Entity class does not subclass bis:PhysicalModel, so validation should succeed";
    ASSERT_EQ(ECObjectsStatus::Success, schema->CreateEntityClass(entity1, "BadEntity"));
    ASSERT_EQ(ECObjectsStatus::Success, entity1->AddBaseClass(*bisPhysicalModel));
    ASSERT_FALSE(ECSchemaValidator::Validate(*schema)) << "Entity class subclasses bis:PhysicalModel, so validation should fail";
    }
