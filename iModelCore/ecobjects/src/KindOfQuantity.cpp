...
// @bsimethod
//--------------------------------------------------------------------------------------
ECObjectsStatus KindOfQuantity::SetDisplayLabel(Utf8StringCR value) {m_validatedName.SetDisplayLabel(value.c_str()); return ECObjectsStatus::Success;}
...
SchemaWriteStatus KindOfQuantity::WriteXml(BeXmlWriterR xmlWriter, ECVersion ecXmlVersion) const
    ...
    if (!hasValidUnitForVersion)
        {
        LOG.errorv("Failed to write KindOfQuantity %s because its persistence unit (%s) is not defined in the EC Xml version it is being serialized to (%s)", 
            GetName().c_str(), GetPersistenceUnit()->GetFullName().c_str(), ECSchema::GetECVersionString(ecXmlVersion));
        return SchemaWriteStatus::FailedToSaveXml;
        }
...
// @bsimethod
//---------------+---------------+---------------+---------------+---------------+-------
bool KindOfQuantity::ToJson(BeJsValue outValue, bool standalone, bool includeSchemaVersion) const
    {
    ...
    if (0 != presentationUnits.size())
        {
        auto presentationUnitArr = outValue[PRESENTATION_UNITS_ATTRIBUTE];
        for (auto const& format : presentationUnits)
            ...
            presentationUnitArr.appendValue() = SchemaParseUtils::GetJsonFormatString(format, GetSchema());
            }
        }
...
/*---------------------------------------------------------------------------------**//**
 @bsimethod
+---------------+---------------+---------------+---------------+---------------+------*/
SchemaReadStatus KindOfQuantity::ReadXml(pugi::xml_node kindOfQuantityNode, ECSchemaReadContextR context)
    {
    Utf8String value;      // used by the macros.

    if (GetName().length() == 0)
        READ_REQUIRED_XML_ATTRIBUTE(kindOfQuantityNode, TYPE_NAME_ATTRIBUTE, this, Name, kindOfQuantityNode.name())
    ...
    double relError;
    if (BePugiXmlHelper::GetAttributeDoubleValue(kindOfQuantityNode, relError, RELATIVE_ERROR) != BePugiXmlValueResult::Success)
        {
    ...
    Utf8String persUnit = kindOfQuantityNode.attribute(PERSISTENCE_UNIT_ATTRIBUTE).as_string();
    if (Utf8String::IsNullOrEmpty(persUnit.c_str()))
        {
    ...
    auto presentationFormatsAttr = kindOfQuantityNode.attribute(PRESENTATION_UNITS_ATTRIBUTE);
    if (presentationFormatsAttr)
        {
        value = presentationFormatsAttr.as_string();
        // NOTE: The format string is a ';' seperated list of formats and format overrides.  
        // In EC 3.1 units and formats have *, ^, ., (, ), /, -, @, and $ in their names, in EC 3.2 units and formats must have valid EC names 
        // but the label override may include a ';' so a regex is needed to split both formats safely
        static std::regex const rgx(R"REGEX(([^[;]+(\[[^|\]]+[|]?([^\]]+)?\]){0,4}))REGEX", std::regex::optimize);
        auto firstFormat = std::sregex_iterator(value.begin(), value.end(), rgx);
        auto end = std::sregex_iterator();
        for (auto i = firstFormat; i != end; ++i)
            {
            auto match = *i;
            presentationFormats.push_back(match.str());
            }
        }
...
bool fusAliasFromFormatString (Utf8String formatString, Utf8CP& mapped)
    {
    mapped = Formatting::LegacyNameMappings::TryGetLegacyNameFromFormatString(("FORMATS:" + formatString).c_str());
    mapped = Formatting::AliasMappings::TryGetAliasFromName(mapped);
    return nullptr != mapped;
    }
