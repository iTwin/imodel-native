...
/*---------------------------------------------------------------------------------**//**
*  Construct a Null ECObject::ECValue (of a specific type, but with IsNull = true)
* @bsimethod                                                    CaseyMullen     09/09
+---------------+---------------+---------------+---------------+---------------+------*/
...
/*---------------------------------------------------------------------------------**//**
* @param holdADuplicate     If true, ECObject::ECValue will make a duplicate, otherwise 
* ECObject::ECValue holds the original pointer. Intended only for use when initializing arrays of strings, to avoid duplicating them twice.
* @bsimethod                                                    CaseyMullen     09/09
+---------------+---------------+---------------+---------------+---------------+------*/
...
PrimitiveType   ECValue::GetPrimitiveType() const
    {
    PRECONDITION (IsPrimitive() && "Tried to get the primitive type of an ECObject::ECValue that is not classified as a primitive.", (PrimitiveType)0);    
...
::Int32         ECValue::GetInteger() const
    {
    PRECONDITION (IsInteger() && "Tried to get integer value from an ECObject::ECValue that is not an integer.", 0);
...
::Int64         ECValue::GetLong() const
    {
    PRECONDITION (IsLong() && "Tried to get long64 value from an ECObject::ECValue that is not an long64.", 0);
    PRECONDITION (!IsNull() && "Getting the value of a NULL non-string primitive is ill-defined", 0);
    return m_long64;
    };
...
double          ECValue::GetDouble() const
    {
    PRECONDITION (IsDouble() && "Tried to get double value from an ECObject::ECValue that is not an double.", std::numeric_limits<double>::quiet_NaN());
...
bool          ECValue::GetBoolean() const
    {
    PRECONDITION (IsBoolean() && "Tried to get boolean value from an ECObject::ECValue that is not a boolean.", 0);
    PRECONDITION (!IsNull() && "Getting the value of a NULL non-string primitive is ill-defined", 0);
    return m_boolean;
    };
...
Int64          ECValue::GetDateTimeTicks() const
    {
    PRECONDITION (IsDateTime() && "Tried to get DateTime value from an ECObject::ECValue that is not a DateTime.", 0);
    PRECONDITION (!IsNull() && "Getting the value of a NULL non-string primitive is ill-defined", 0);
    return m_dateTime;
    }
...
DPoint2d          ECValue::GetPoint2D() const
    ...
    PRECONDITION (IsPoint2D() && "Tried to get Point2D value from an ECObject::ECValue that is not a Point2D.", badValue);
...
DPoint3d          ECValue::GetPoint3D() const
    ...
    PRECONDITION (IsPoint3D() && "Tried to get Point3D value from an ECObject::ECValue that is not a Point3D.", badValue);
    PRECONDITION (!IsNull() && "Getting the value of a NULL non-string primitive is ill-defined", badValue);
    return m_dPoint3d;
    };
...
WCharCP ECValue::GetString() const
    {
    PRECONDITION (IsString() && "Tried to get string value from an ECObject::ECValue that is not a string.", L"<Programmer Error: Attempted to get string value from ECObject::ECValue that is not a string.>");
    return m_string;
    };

/*---------------------------------------------------------------------------------**//**
* @param[in] holdADuplicate     If true, ECObject::ECValue will make a duplicate, otherwise 
*                               ECObject::ECValue holds the original pointer
* @bsimethod                                                    CaseyMullen     09/09
+---------------+---------------+---------------+---------------+---------------+------*/
...
const byte * ECValue::GetBinary(size_t& size) const
    {
    PRECONDITION (IsBinary() && "Tried to get binarydata from an ECObject::ECValue that is not binary.", NULL);
...
IECInstancePtr  ECValue::GetStruct() const
    {
    PRECONDITION (IsStruct() && "Tried to get struct value from an ECObject::ECValue that is not a struct.", 0);
...
WString    ECValue::ToString () const
    ...
    if (IsArray())
        ...
        switch (valueKind)
            ...
            default:
                {
                return L"ECObject::ECValue::ToString needs work... unsupported data type";
                }            
            }
        }
...
static ECObjectsStatus getECValueAccessorUsingManagedAccessString (wchar_t* asBuffer, wchar_t* indexBuffer, ECValueAccessor& va, ECEnablerCR  enabler, WCharCP managedPropertyAccessor)
    ...
    ECObject::ECEnablerP enablerP = const_cast<ECObject::ECEnablerP>(&enabler);
