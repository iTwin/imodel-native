...
bool ECValue::ConvertPrimitiveToECExpressionLiteral(Utf8StringR expr) const {
    ...
    switch (GetPrimitiveType()) {
        case PRIMITIVETYPE_Boolean:
            expr = GetBoolean() ? "True" : "False";
            return true;
        case PRIMITIVETYPE_DateTime:
            expr.Sprintf("@%" PRId64, GetDateTimeTicks());
            return true;
        case PRIMITIVETYPE_Double:
            expr = formatDouble(GetDouble());
            return true;
        case PRIMITIVETYPE_Integer:
            expr.Sprintf("%" PRId32, GetInteger());
            return true;
        case PRIMITIVETYPE_Long:
            expr.Sprintf("%" PRId64, GetLong());
            return true;
        case PRIMITIVETYPE_Point2d:
            expr.Sprintf("{%s,%s}", formatDouble(GetPoint2d().x).c_str(), formatDouble(GetPoint2d().y).c_str());
            return true;
        case PRIMITIVETYPE_Point3d: {
            DPoint3d pt = GetPoint3d();
    ...
    switch (newType) {
        case PRIMITIVETYPE_Integer: {
            ...
            if (PRIMITIVETYPE_Double == curType) {
                double roundingTerm = DoubleOps::AlmostEqual(GetDouble(), 0.0) ? 0.0 : GetDouble() > 0.0 ? 0.5
                                                                                                         : -0.5;
...
bool ECValueAccessor::operator!=(ECValueAccessorCR accessor) const {
    if (GetDepth() != accessor.GetDepth())
        return true;
    for (uint32_t depth = 0; depth < GetDepth(); depth++) {
        if ((*this)[depth].GetEnabler() != accessor[depth].GetEnabler() || (*this)[depth].GetPropertyIndex() != accessor[depth].GetPropertyIndex() || (*this)[depth].GetArrayIndex() != accessor[depth].GetArrayIndex())
            return true;
    }
    return false;
}
...
ECValueCR ECPropertyValue::GetValue() const {
    EvaluateValue();
    return m_ecValue;
}
...
bool AdHocPropertyMetadata::PrimitiveTypeForCode(PrimitiveType& type, int32_t code) {
    switch (code) {
        case (int32_t)PrimitiveTypeCode::String:
            type = PRIMITIVETYPE_String;
            return true;
        case (int32_t)PrimitiveTypeCode::Integer:
            type = PRIMITIVETYPE_Integer;
            return true;
        case (int32_t)PrimitiveTypeCode::Long:
            type = PRIMITIVETYPE_Long;
            return true;
        case (int32_t)PrimitiveTypeCode::Double:
            type = PRIMITIVETYPE_Double;
            return true;
        case (int32_t)PrimitiveTypeCode::DateTime:
            type = PRIMITIVETYPE_DateTime;
            return true;
        case (int32_t)PrimitiveTypeCode::Boolean:
            type = PRIMITIVETYPE_Boolean;
            return true;
        case (int32_t)PrimitiveTypeCode::Point2d:
            type = PRIMITIVETYPE_Point2d;
            return true;
        case (int32_t)PrimitiveTypeCode::Point3d:
            type = PRIMITIVETYPE_Point3d;
            return true;
        default:
            return false;
    }
...
bool AdHocPropertyMetadata::CodeForPrimitiveType(int32_t& code, PrimitiveType type) {
    switch (type) {
        case PRIMITIVETYPE_String:
            code = static_cast<int32_t>(PrimitiveTypeCode::String);
            return true;
        case PRIMITIVETYPE_Integer:
            code = static_cast<int32_t>(PrimitiveTypeCode::Integer);
            return true;
        case PRIMITIVETYPE_Long:
            code = static_cast<int32_t>(PrimitiveTypeCode::Long);
            return true;
        case PRIMITIVETYPE_Double:
            code = static_cast<int32_t>(PrimitiveTypeCode::Double);
            return true;
        case PRIMITIVETYPE_DateTime:
            code = static_cast<int32_t>(PrimitiveTypeCode::DateTime);
            return true;
        case PRIMITIVETYPE_Boolean:
            code = static_cast<int32_t>(PrimitiveTypeCode::Boolean);
            return true;
        case PRIMITIVETYPE_Point2d:
            code = static_cast<int32_t>(PrimitiveTypeCode::Point2d);
            return true;
        case PRIMITIVETYPE_Point3d:
            code = static_cast<int32_t>(PrimitiveTypeCode::Point3d);
            return true;
        default:
            return false;
    }
