...
Utf8String NamedFormat::GetQualifiedFormatString(ECSchemaCR primarySchema) const {
    ...
    if (!EXPECTED_CONDITION(ECObjectsStatus::Success == primarySchema.ResolveAlias(m_ecFormat->GetSchema(), alias))) {
        LOG.warningv(
            "warning: Cannot qualify a NamedFormat name with an alias unless the schema containing the ECFormat is referenced by the primary schema."
            "The name will remain unqualified.\n  Primary ECSchema: %s\n  NamedFormat: %s\n ECSchema containing the parent ECFormat: %s",
            primarySchema.GetName().c_str(), name.c_str(), m_ecFormat->GetSchema().GetName().c_str());
        return name;
    }
...
SchemaWriteStatus ECFormat::WriteXml(BeXmlWriterR xmlWriter, ECVersion ecXmlVersion) const {
    ...
    if (HasNumeric()) {
        ...
        xmlWriter.WriteAttribute(FORMAT_PRECISION_ATTRIBUTE, GetPresentationType() == Formatting::PresentationType::Fractional ? static_cast<uint32_t>(pow(2u, static_cast<uint32_t>(nfs->GetFractionalPrecision()))) : static_cast<uint32_t>(nfs->GetDecimalPrecision()));
        if (nfs->HasMinWidth())
