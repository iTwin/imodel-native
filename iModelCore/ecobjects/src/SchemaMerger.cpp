...
ECObjectsStatus SchemaMerger::MergePrimitive(PrimitiveChange<T>& change, TParent* parent, ECObjectsStatus (TParent::*setPrimitive)(TSetter), Utf8CP parentKey, SchemaMergeResult& result, SchemaMergeOptions const& options, bool preferLeftValue) {
    ...
    if (!newValue.IsValid()) {  // IsValid() implicitly also checks IsNull()
...
ECObjectsStatus SchemaMerger::MergeItems(SchemaMergeResult& result, ECSchemaP left, ECSchemaCP right, SchemaMergeOptions const& options, ECChangeArray<TItemChange>& changes,
    ...
    for (const auto& schemaItemChange : changes) {
        ...
        if (opCode == ECChange::OpCode::New) {  // This may actually work
            ...
            if (left->NamedElementExists(itemName) &&
                ((left->*getItemP)(itemName) == nullptr)) {  // An item of another type exists with the same name
...
ECObjectsStatus SchemaMerger::MergeSchema(SchemaMergeResult& result, ECSchemaP left, ECSchemaCP right, RefCountedPtr<SchemaChange> schemaChange, SchemaMergeOptions const& options) {
    ...
    if (schemaChange->OriginalECXmlVersionMajor().IsChanged() || schemaChange->OriginalECXmlVersionMinor().IsChanged()) {  // Apply the highest version being used
    ...
    for (auto classChange : schemaChange->Classes()) {
        ...
        if (left->NamedElementExists(className.c_str()) &&
            (left->GetClassCP(className.c_str()) == nullptr)) {  // An item of another type exists with the same name
            if (!options.GetRenameSchemaItemOnConflict()) {
                result.Issues().ReportV(IssueSeverity::Error, IssueCategory::BusinessProperties, IssueType::ECSchema, ECIssueId::EC_0026,
...
ECObjectsStatus SchemaMerger::MergeRelationshipConstraint(SchemaMergeResult& result, ECRelationshipClassP left, ECClassCP right, RelationshipConstraintChange& change, SchemaMergeOptions const& options, bool isSource) {
    ...
    status = MergeReferencedSchemaItem<ECClassCP>(result, change.AbstractConstraint(), [&](ECClassCP value) { return constraint.SetAbstractConstraint(*value); }, [&](ECSchemaP schema, Utf8StringCR name) { return schema->GetClassCP(name.c_str()); }, left->GetFullName(), options);
    if (status != ECObjectsStatus::Success)
        return status;
    ...
    for (auto constraintClassChange : constraintClassChanges) {
        ...
        status = MergeReferencedSchemaItem<ECClassCP>(result, *constraintClassChange, [&](ECClassCP value) {
                ...
                return constraint.AddClass(*value); }, [&](ECSchemaP schema, Utf8StringCR name) { return schema->GetClassCP(name.c_str()); }, left->GetFullName(), options);
        if (status != ECObjectsStatus::Success)
            return status;
    }
...
ECObjectsStatus SchemaMerger::MergeClass(SchemaMergeResult& result, ECClassP left, ECClassCP right, RefCountedPtr<ClassChange> classChange, SchemaMergeOptions const& options) {
    ...
    for (auto propertyChange : classChange->Properties()) {
        ...
        if (opCode == ECChange::OpCode::New) {
            if (!SchemaConflictHelper::CanPropertyBeAdded(*left, *rightProperty)) {  // conflict
...
ECObjectsStatus SchemaMerger::MergeProperty(SchemaMergeResult& result, ECPropertyP left, ECPropertyCP right, RefCountedPtr<PropertyChange> propertyChange, SchemaMergeOptions const& options) {
    ...
    status = MergeReferencedSchemaItem<PropertyCategoryCP>(result, propertyChange->Category(), [&](PropertyCategoryCP value) { return left->SetCategory(value); }, [&](ECSchemaP schema, Utf8StringCR name) { return schema->GetPropertyCategoryCP(name.c_str()); }, key.c_str(), options);
    if (status != ECObjectsStatus::Success)
        return status;

    status = MergeReferencedSchemaItem<KindOfQuantityCP>(result, propertyChange->KindOfQuantity(), [&](KindOfQuantityCP value) { return left->SetKindOfQuantity(value); }, [&](ECSchemaP schema, Utf8StringCR name) { return schema->GetKindOfQuantityCP(name.c_str()); }, key.c_str(), options);
    if (status != ECObjectsStatus::Success)
        return status;
...
ECObjectsStatus SchemaMerger::MergeKindOfQuantity(SchemaMergeResult& result, KindOfQuantityP left, KindOfQuantityCP right, RefCountedPtr<KindOfQuantityChange> change, SchemaMergeOptions const& options) {
    ...
    status = MergeReferencedSchemaItem<ECUnitCP>(result, change->PersistenceUnit(), [&](ECUnitCP value) { return left->SetPersistenceUnit(*value); }, [&](ECSchemaP schema, Utf8StringCR name) { return schema->GetUnitCP(name.c_str()); }, key, options);
    if (status != ECObjectsStatus::Success)
        return status;

    auto& presentationFormatsChange = change->PresentationFormats();
    if (presentationFormatsChange.IsChanged()) {  // SchemaComparer compares these by index and returns modified or deleted/new based on which side has more items
