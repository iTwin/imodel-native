...
CustomAttributeReadStatus SchemaXmlReaderImpl::ReadContainerCustomAttributes(ECSchemaR parentSchema, pugi::xml_node xmlNode, ECClassR container) {
    ...
    if (container.IsRelationshipClass()) {
        ...
        status = ReadChildrenCustomAttributes(parentSchema, xmlNode, nullptr, [](pugi::xml_node node) { return 0 == strcmp(node.name(), ECXML_TARGETCONSTRAINT_ELEMENT); }, [&](Utf8CP name) { return &container.GetRelationshipClassP()->GetTarget(); });
        if (CustomAttributeReadStatus::InvalidCustomAttributes == status) return status;

        status = ReadChildrenCustomAttributes(parentSchema, xmlNode, nullptr, [](pugi::xml_node node) { return 0 == strcmp(node.name(), ECXML_SOURCECONSTRAINT_ELEMENT); }, [&](Utf8CP name) { return &container.GetRelationshipClassP()->GetSource(); });
        if (CustomAttributeReadStatus::InvalidCustomAttributes == status) return status;
    }

    return ReadChildrenCustomAttributes(parentSchema, xmlNode, PROPERTY_NAME_ATTRIBUTE, [](pugi::xml_node node) { return 0 == strcmp(node.name(), EC_PROPERTY_ELEMENT) ||
                                                                                                                         ...
                                                                                                                         0 == strcmp(node.name(), EC_NAVIGATIONPROPERTY_ELEMENT); }, [&](Utf8CP name) { return container.GetPropertyP(name); });
}
...
CustomAttributeReadStatus SchemaXmlReaderImpl::DeserializeChildCustomAttributes(ECSchemaR contextSchema, pugi::xml_node schemaNode) {
    return ReadChildrenCustomAttributes(contextSchema, schemaNode, TYPE_NAME_ATTRIBUTE, [&](pugi::xml_node node) { return IsSchemaChildElementNode(node, ECSchemaElementType::ECClass); }, [&](Utf8CP name) { return contextSchema.GetClassP(name); });
}
