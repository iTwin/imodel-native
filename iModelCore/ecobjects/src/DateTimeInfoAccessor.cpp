...
ECObjectsStatus DateTimeInfoAccessor::GetFrom(DateTime::Info& dateTimeInfo, ECPropertyCR dateTimeProperty) {
    ...
    PRECONDITION((dateTimeProperty.GetIsPrimitive() && dateTimeProperty.GetAsPrimitiveProperty()->GetType() == PRIMITIVETYPE_DateTime) ||
                     ((arrayDateTimeProp = dateTimeProperty.GetAsPrimitiveArrayProperty()) != nullptr && arrayDateTimeProp->GetKind() == ARRAYKIND_Primitive && arrayDateTimeProp->GetPrimitiveElementType() == PRIMITIVETYPE_DateTime),
                 ECObjectsStatus::DataTypeNotSupported);
