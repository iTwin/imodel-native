...
//--------------------------------------------------------------------------------------
// static
ECUnitP ECUnit::_Create(Units::UnitSystemCR unitSystem, Units::PhenomenonCR phenomenon, Utf8CP name, uint32_t id, Utf8CP definition, Utf8Char dimensionSymbol, double factor, double offset, bool isConstant)
    {
    ...
    auto ptrUnit = new ECUnit(unitSystem, phenomenon, name, id, definition, dimensionSymbol, factor, offset, isConstant);
    if (nullptr == ptrUnit)
        return nullptr;
...
SchemaReadStatus ECUnit::ReadXml(ECUnitP& unit, BeXmlNodeR unitNode, ECSchemaCR schema, ECSchemaReadContextR context)
    ...
    if (BEXML_Success != xmlStatus) factor = 1.0;
