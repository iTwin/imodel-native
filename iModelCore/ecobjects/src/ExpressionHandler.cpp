...
ExpressionToken Lexer::GetIdentifier() {
    ...
    while (0 != isalnum(current = PeekChar()) || current == '_')
        GetNextChar();
...
NodePtr ECEvaluator::ParsePrimary() {
    ...
    switch (m_lexer->GetTokenType()) {
        ...
        case TOKEN_IntegerConstant: {
            int64_t value = BeStringUtilities::ParseUInt64(m_lexer->GetTokenStringCP());
...
NodePtr ECEvaluator::ParseShift() {
    ...
    while (m_lexer->GetTokenType() == TOKEN_ShiftLeft || m_lexer->GetTokenType() == TOKEN_ShiftRight || m_lexer->GetTokenType() == TOKEN_UnsignedShiftRight) {
        op = m_lexer->GetTokenType();
        m_lexer->Advance();
        NodePtr left = result;
...
NodePtr ECEvaluator::ParseComparison() {
    ...
    while (m_lexer->GetTokenType() == TOKEN_Equal || m_lexer->GetTokenType() == TOKEN_NotEqual || m_lexer->GetTokenType() == TOKEN_Less || m_lexer->GetTokenType() == TOKEN_LessEqual || m_lexer->GetTokenType() == TOKEN_Greater || m_lexer->GetTokenType() == TOKEN_GreaterEqual || m_lexer->GetTokenType() == TOKEN_Like
