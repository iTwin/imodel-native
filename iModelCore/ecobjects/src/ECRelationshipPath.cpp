...
bool ECRelationshipPath::Validate() const {
    ...
    for (const ECRelatedClassSpecifier& specifier : m_relatedClassSpecifiers) {
        ...
        ECRelationshipConstraintCR previousConstraint = (specifier.m_direction == ECRelatedInstanceDirection::Forward) ? specifier.m_relationshipClass->GetSource() : specifier.m_relationshipClass->GetTarget();
        if (!ValidateConstraint(previousConstraint, previousClass))
            return false;

        // Validate next class
        ECRelationshipConstraintCR nextConstraint = (specifier.m_direction == ECRelatedInstanceDirection::Forward) ? specifier.m_relationshipClass->GetTarget() : specifier.m_relationshipClass->GetSource();
...
void ECRelationshipPath::Reverse(ECRelationshipPath& reversedPath) const {
    ...
    while (iter != m_relatedClassSpecifiers.rend()) {
        ...
        ECRelatedInstanceDirection reversedDirection = (iter->GetDirection() == ECRelatedInstanceDirection::Forward) ? ECRelatedInstanceDirection::Backward : ECRelatedInstanceDirection::Forward;
