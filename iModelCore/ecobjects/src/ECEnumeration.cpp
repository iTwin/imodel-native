...
ECObjectsStatus ECEnumeration::SetType(PrimitiveType value) {
    if (value != PRIMITIVETYPE_Integer && value != PRIMITIVETYPE_String && !GetSchema().OriginalECXmlVersionGreaterThan(ECVersion::Latest))  // If is it greater than we know about allow the parsed primitive type to be set.
