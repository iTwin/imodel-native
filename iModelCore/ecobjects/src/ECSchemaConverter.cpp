...
CustomECSchemaConverterP ECSchemaConverter::GetSingleton() {
    ...
    if (nullptr == converterSingleton) {
        ...
        converterSingleton->AddCustomAttributeNameToRemove("BuildingDataGroup:XpathDGMapperProperty");
        converterSingleton->AddCustomAttributeNameToRemove("BuildingDataGroup:DGPropTypeMapperProperty");
        converterSingleton->AddCustomAttributeNameToRemove("BuildingDataGroup:DGExtendedType");
        converterSingleton->AddCustomAttributeNameToRemove("BuildingDataGroup:DataGroupCatalogInstanceNameProperty");
        converterSingleton->AddCustomAttributeNameToRemove("BuildingDataGroup:DataGroupClassNameProperty");
...
void CustomECSchemaConverter::ProcessCustomAttributeInstances(IECCustomAttributeContainerR container, Utf8String containerName, ECSchemaReadContextR contxt) {
    ...
    for (auto const& attr : container.GetCustomAttributes(false)) {
        ...
        if (nullptr != converter) {
            ...
            m_convertedOK = m_convertedOK && container.RemoveCustomAttribute(attr->GetClass());
        else if (ShouldRemoveCustomAttribute(fullName))
            m_convertedOK = m_convertedOK && container.RemoveCustomAttribute(attr->GetClass());
    }
}
...
void CustomECSchemaConverter::ProcessRelationshipConstraint(ECRelationshipConstraintR constraint, bool isSource) {
    ...
    for (ECClassCP baseConstraintClass : baseConstraint.GetConstraintClasses()) {
        (baseConstraintClass->IsEntityClass()) ? constraint.AddClass(*baseConstraintClass->GetEntityClassCP()) : constraint.AddClass(*baseConstraintClass->GetRelationshipClassCP());
    }
}
...
void CustomECSchemaConverter::ConvertPropertyLevel(bvector<ECClassP>& classes, ECSchemaReadContextR context) {
    for (auto const& ecClass : classes) {
        ...
        for (auto const& ecProp : ecClass->GetProperties(false)) {
            // Checking for empty struct properties, and removing them if found
            if (ecProp->GetIsStruct() || ecProp->GetIsStructArray()) {
                // Getting ecProp's struct class
                ECStructClassCR structType = ecProp->GetIsStruct() ? ecProp->GetAsStructProperty()->GetType() : ecProp->GetAsStructArrayProperty()->GetStructElementType();
                // if the class is empty, removing ecProp
                if (!structType.GetPropertyCount()) {
                    ecClass->DeleteProperty(*ecProp);
                    continue;
                }
            }
...
ECObjectsStatus ECDbClassMapConverter::Convert(ECSchemaR schema, IECCustomAttributeContainerR container, IECInstanceR instance, ECSchemaReadContextP context) {
    ...
    schema.RemoveUnusedSchemaReferences();
    if (nullptr == ecClass) {
        LOG.warningv("ECDbMap:ClassMap custom attribute applied to a container which is not a class.  Removing Custom Attribute from %s and skipping.", container.GetContainerName().c_str());
        return ECObjectsStatus::Success;
    }
    ...
    if (ecdbMapSchema.IsNull() || !ecdbMapSchema.IsValid()) {
        LOG.errorv("Failed to convert ECDbMap::ClassMap on %s because the ECDbMap 2.0.0 schema could not be found.  Removing and skipping.", container.GetContainerName().c_str());
        return ECObjectsStatus::Error;
    }
...
ECObjectsStatus StandardValueInfo::ExtractInstanceData(IECInstanceR instance, StandardValueInfo& sdInfo) {
    ...
    ECValue value;
    if (ECObjectsStatus::Success == instance.GetValue(value, "MustBeFromList") && !value.IsNull() && value.IsBoolean()) {
...
bool isIntOrIntArray(ECPropertyP prop) {
    ...
    PrimitiveType primType = (nullptr == primArrProp) ? primProp->GetType() : primArrProp->GetType();
    return PrimitiveType::PRIMITIVETYPE_Integer == primType;
}
...
ECObjectsStatus StandardValuesConverter::Convert(ECSchemaR schema, IECCustomAttributeContainerR container, IECInstanceR instance, ECSchemaReadContextP context) {
    ...
    if (nullptr == prop || !isIntOrIntArray(prop)) {
        LOG.warningv("StandardValues custom attribute applied to a container which is not an int or int array property.  Removing Custom Attribute from %s and skipping.", container.GetContainerName().c_str());
...
ECObjectsStatus StandardValuesConverter::FindEnumeration(ECSchemaR schema, ECEnumerationP& enumeration, StandardValueInfo& sdInfo) {
    for (auto ecEnum : schema.GetEnumerations()) {
        if ((PrimitiveType::PRIMITIVETYPE_Integer != ecEnum->GetType()) || (sdInfo.m_mustBeFromList != ecEnum->GetIsStrict()))
            continue;
...
bool kindOfQuantityHasMatchingPersistenceUnit(KindOfQuantityCP koq, Units::UnitCP unit) {
    if (nullptr == koq)
        return true;
...
ECObjectsStatus createNewKindOfQuantity(ECSchemaR schema, KindOfQuantityP& newKOQ, KindOfQuantityCP baseKOQ, ECUnitCP newUnit, ECUnitCP newDisplayUnit, bool& persistenceUnitChanged, Utf8CP newKoqName, bool resolveConflicts = false) {
    Utf8String koqNameToUse = resolveConflicts ? schema.FindUniqueSchemaItemName(newKoqName) : newKoqName;

    ECObjectsStatus status = schema.CreateKindOfQuantity(newKOQ, koqNameToUse.c_str());
    if (ECObjectsStatus::Success != status) {
    ...
    if (kindOfQuantityHasMatchingPersistenceUnit(baseKOQ, newUnit)) {
...
bool canGetOrCreateAcceptableKindOfQuantity(ECSchemaR schema, Utf8CP newKoqName, KindOfQuantityP& newKOQ, KindOfQuantityCP baseKOQ, ECUnitCP newUnit, ECUnitCP newDisplayUnit, bool& persistenceUnitChanged) {
    newKOQ = schema.GetKindOfQuantityP(newKoqName);
    if (nullptr == newKOQ)
        return ECObjectsStatus::Success == createNewKindOfQuantity(schema, newKOQ, baseKOQ, newUnit, newDisplayUnit, persistenceUnitChanged, newKoqName);

    if (!kindOfQuantityHasMatchingPersistenceUnit(baseKOQ, newKOQ->GetPersistenceUnit()))
        return false;
    ...
    if (!kindOfQuantityHasMatchingPersistenceUnit(newKOQ, newUnit))
        return false;
...
ECObjectsStatus obtainKindOfQuantity(ECSchemaR schema, ECPropertyP prop, KindOfQuantityP& newKOQ, IECInstanceR unitSpecCA, ECUnitCP newUnit, ECUnitCP newDisplayUnit, bool& persistenceUnitChanged, Utf8CP newKoqName, ECSchemaReadContextP context) {
    ...
    if (canGetOrCreateAcceptableKindOfQuantity(schema, newKoqName, newKOQ, baseKOQ, newUnit, newDisplayUnit, persistenceUnitChanged))
        return ECObjectsStatus::Success;

    Utf8PrintfString newKoqNameWithClass("%s_%s", newKoqName, prop->GetClass().GetName().c_str());
    if (canGetOrCreateAcceptableKindOfQuantity(schema, newKoqNameWithClass.c_str(), newKOQ, baseKOQ, newUnit, newDisplayUnit, persistenceUnitChanged))
        return ECObjectsStatus::Success;

    Utf8PrintfString newKoqNameWithClassAndProperty("%s_%s", newKoqNameWithClass.c_str(), prop->GetName().c_str());
    if (canGetOrCreateAcceptableKindOfQuantity(schema, newKoqNameWithClassAndProperty.c_str(), newKOQ, baseKOQ, newUnit, newDisplayUnit, persistenceUnitChanged))
        return ECObjectsStatus::Success;

    bool resolveConflicts = true;
    return createNewKindOfQuantity(schema, newKOQ, baseKOQ, newUnit, newDisplayUnit, persistenceUnitChanged, newKoqNameWithClassAndProperty.c_str(), resolveConflicts);
}
...
ECObjectsStatus UnitSpecificationConverter::Convert(ECSchemaR schema, IECCustomAttributeContainerR container, IECInstanceR instance, ECSchemaReadContextP context) {
    ECPropertyP prop = dynamic_cast<ECPropertyP>(&container);
    if (prop == nullptr) {
        LOG.infov("Found UnitSpecification or DisplayUnitSpecification custom attribute on a container which is not a property, removing.  Container is %s.", container.GetContainerName().c_str());
    ...
    if (nullptr != koq && prop->IsKindOfQuantityDefinedLocally()) {
        LOG.infov("Found DisplayUnitSpecification on property %s:%s.%s with UnitSpecification defined locally. Skipping conversion since the DisplayUnitSpecificaiton was already processed.", schema.GetFullSchemaName().c_str(), prop->GetClass().GetName().c_str(), prop->GetName().c_str());
        container.RemoveCustomAttribute(instance.GetClass().GetSchema().GetName(), instance.GetClass().GetName());
        return ECObjectsStatus::Success;
    }
    ...
    Utf8String koqName;
    if (!LegacyUnits::GetUnitForECProperty(oldUnit, *prop)) {
        if (nullptr == koq) {
            Utf8String fullName = schema.GetFullSchemaName();
    ...
    if (nullptr == newUnit) {
        Utf8CP ecName = Units::UnitNameMappings::TryGetECNameFromOldName(oldUnit.c_str());
    ...
    if (nullptr == newUnit) {
        Utf8String fullName = schema.GetFullSchemaName();
        LOG.warningv("The property %s:%s.%s has an old unit '%s' that does not resolve to a new unit.  Dropping unit to continue", fullName.c_str(), prop->GetClass().GetName().c_str(), prop->GetName().c_str(), oldUnit.c_str());
        container.RemoveCustomAttribute(instance.GetClass().GetSchema().GetName(), instance.GetClass().GetName());
        return ECObjectsStatus::Success;
    }
    ...
    Utf8String oldDisplayUnit;
    Utf8String oldFormatString;
    if (LegacyUnits::GetDisplayUnitAndFormatForECProperty(oldDisplayUnit, oldFormatString, oldUnit, *prop) && (0 != strcmp(oldDisplayUnit.c_str(), oldUnit.c_str()))) {
        Utf8CP ecName = Units::UnitNameMappings::TryGetECNameFromOldName(oldDisplayUnit.c_str());
        ...
        if (nullptr == newDisplayUnit)
            LOG.warningv("The property %s:%s.%s has an old display unit '%s' that does not resolve to a new unit.", schema.GetFullSchemaName().c_str(), prop->GetClass().GetName().c_str(), prop->GetName().c_str(), oldDisplayUnit.c_str());
    }
    ...
    if (persistenceUnitChanged)
        addOldUnitCustomAttribute(schema, prop, oldUnit.c_str());
...
ECObjectsStatus UnitSpecificationsConverter::Convert(ECSchemaR schema, IECCustomAttributeContainerR container, IECInstanceR instance, ECSchemaReadContextP context) {
    ...
    if (containerAsSchema == nullptr) {
        LOG.warningv("Found a 'UnitSpecifications' custom attribute on a container which is not an ECSchema, removing.  Container is %s", container.GetContainerName().c_str());
...
ECObjectsStatus PropertyPriorityConverter::Convert(ECSchemaR schema, IECCustomAttributeContainerR container, IECInstanceR instance, ECSchemaReadContextP context) {
    ECPropertyP prop = dynamic_cast<ECPropertyP>(&container);
    if (prop == nullptr) {
        LOG.infov("Found PropertyPriority custom attribute on a container which is not a property, removing.  Container is %s", container.GetContainerName().c_str());
    ...
    if (ECObjectsStatus::Success != status || previousPriority.IsNull()) {
        LOG.infov("Found a PropertyPriority custom attribute on an the ECProperty, '%s.%s', but it did not contain a Priority value. Dropping custom attribute....",
...
PropertyCategoryP getExistingCategory(ECSchemaR schema, ECPropertyP prop, IECInstanceR categoryCA, Utf8CP existingName) {
    ...
    if (nullptr != existingCategory) {
        ...
        if (ECObjectsStatus::Success == categoryCA.GetValue(value, "DisplayLabel") && !value.IsNull() && value.IsString() &&
            ...
            LOG.infov("Found a Category custom attribute on '%s.%s' with a name that matches an existing category '%s' but the attributes of the existing category and the custom attribute don't match, using the existing category anyway.",
...
ECObjectsStatus CategoryConverter::Convert(ECSchemaR schema, IECCustomAttributeContainerR container, IECInstanceR instance, ECSchemaReadContextP context) {
    ECPropertyP prop = dynamic_cast<ECPropertyP>(&container);
    if (prop == nullptr) {
        LOG.infov("Found Category custom attribute on a container which is not a property, removing.  Container is %s", container.GetContainerName().c_str());
    ...
    if (ECObjectsStatus::Success != status || existingName.IsNull() || Utf8String::IsNullOrEmpty(existingName.GetUtf8CP())) {
        LOG.warningv("Found a Category custom attribute on an ECProperty, '%s.%s', but it did not contain a Name value. Dropping custom attribute....",
                     prop->GetClass().GetFullName(), prop->GetName().c_str());
...
ECObjectsStatus HidePropertyConverter::Convert(ECSchemaR schema, IECCustomAttributeContainerR container, IECInstanceR instance, ECSchemaReadContextP context) {
    ECPropertyP prop = dynamic_cast<ECPropertyP>(&container);
    if (prop == nullptr) {
        LOG.infov("Found HideProperty custom attribute on a container which is not a property, removing.  Container is %s", container.GetContainerName().c_str());
...
bool shouldHide(IECInstanceR instance) {
    return getBoolValue(instance, HIDE_INSTANCES, false) || (getBoolValue(instance, HIDDEN, false) && getBoolValue(instance, HIDE_INSTANCES, true));
}
