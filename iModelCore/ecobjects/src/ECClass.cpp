...
ECObjectsStatus ECClass::ReplaceProperty(ECPropertyP& newProperty, ValueKind kind, ECPropertyR propertyToRemove) {
    ...
    switch (kind) {
        case VALUEKIND_Primitive:
            newProperty = new PrimitiveECProperty(*this);
            break;
        case VALUEKIND_Array:
            newProperty = new PrimitiveArrayECProperty(*this);
            break;
        case VALUEKIND_Struct:
            newProperty = new StructECProperty(*this);
            break;
        case VALUEKIND_Navigation:
            newProperty = new NavigationECProperty(*this);
            break;
        default:
...
ECObjectsStatus ECClass::AddProperty(ECPropertyP& pProperty, bool resolveConflicts) {
    ...
    if (nullptr != localProperty) {
        if (!resolveConflicts) {  // Even though we may be able to return the found property, if resolveConflicts is false we need to return an error due to contract.
...
ECRelationshipConstraint::ECRelationshipConstraint(ECRelationshipClassP relationshipClass, bool isSource, bool verify)
    : m_isSource(isSource), m_verify(verify), m_relClass(relationshipClass), m_multiplicity(&s_zeroOneMultiplicity), m_isPolymorphic(true), m_abstractConstraint(nullptr), m_verified(false) {}
...
SchemaWriteStatus ECRelationshipClass::_WriteXml(BeXmlWriterR xmlWriter, ECVersion ecXmlVersion) const {
    ...
    if (m_strengthDirection != ECRelatedInstanceDirection::Forward) {  // skip the attribute for "forward" as it is the default value.
