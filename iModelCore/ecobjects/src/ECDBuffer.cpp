...
uint32_t PropertyLayout::GetOffset() const {
    BeAssert(!m_typeDescriptor.IsStruct());
    return m_offset;
}
uint32_t PropertyLayout::GetNullflagsOffset() const {
    BeAssert(!m_typeDescriptor.IsStruct());
    return m_nullflagsOffset;
}
NullflagsBitmask PropertyLayout::GetNullflagsBitmask() const {
    BeAssert(!m_typeDescriptor.IsStruct());
    return m_nullflagsBitmask;
}
 ...
 * @bsimethod
 +---------------+---------------+---------------+---------------+---------------+------*/
Utf8String PropertyLayout::ToString() const {
    Utf8String typeName;
...
static uint32_t ComputeFnvHashForString(Utf8CP buf, uint32_t hashValue) {
    ...
    unsigned char const* bp = (unsigned char const*)buf; /* start of buffer */
...
uint32_t ClassLayout::CalculateBytesUsed(Byte const* propertyData) const {
    ...
    SecondaryOffset const* pLast = (SecondaryOffset const*)(propertyData + m_sizeOfFixedSection - sizeof(SecondaryOffset));
 ...
 * @bsimethod
 +---------------+---------------+---------------+---------------+---------------+------*/
ClassLayoutPtr ClassLayout::Factory::DoBuildClassLayout() {                            // ECLogger::Log->debugv (L"Building ClassLayout for ECClass %ls", m_ecClass->GetName().c_str());
...
uint32_t ECDBuffer::GetPropertyValueSize(PropertyLayoutCR propertyLayout, uint32_t index) const {
    if (IsArrayOfFixedSizeElements(propertyLayout)) {
        ...
        SecondaryOffset const* pIndexValueOffset = (SecondaryOffset const*)(data + GetOffsetOfArrayIndex(arrayOffset, propertyLayout, index));
        ...
        SecondaryOffset const* pNextPropertyValueOffset = (SecondaryOffset const*)(data + propertyLayout.GetOffset()) + 1;
...
ArrayCount ECDBuffer::GetAllocatedArrayCount(PropertyLayoutCR propertyLayout) const {
    if (propertyLayout.IsFixedSized()) {
        ...
        SecondaryOffset const* pSecondaryOffset = (SecondaryOffset const*)(GetPropertyData() + propertyLayout.GetOffset());
        SecondaryOffset const* pNextOffset = pSecondaryOffset + 1;
        ...
        return *((ArrayCount const*)pCount);
    }
}
...
static void PrepareToAccessNullFlags(
    ...
    if (propertyLayout.IsFixedSized())
        ...
        nullflagsOffset = *((SecondaryOffset const*)(data + propertyLayout.GetOffset())) + sizeof(ArrayCount);
...
void ECDBuffer::SetPropertyValueNull(PropertyLayoutCR propertyLayout, bool useIndex, uint32_t index, bool isNull) {
    ...
    NullflagsBitmask const* nullflagsP = (NullflagsBitmask const*)(data + nullflagsOffset);
...
ECObjectsStatus ECDBuffer::EnsureSpaceIsAvailable(uint32_t& offset, PropertyLayoutCR propertyLayout, uint32_t bytesNeeded) {
    Byte* data = const_cast<Byte*>(GetPropertyData());
    ...
    if (0 == *pSecondaryOffset) {
        ...
        for (pPriorOffset = pSecondaryOffset - 1; 0 == *pPriorOffset; --pPriorOffset);  // We have found the first non-zero secondaryOffset before this one
...
ECObjectsStatus ECDBuffer::RemoveArrayElementsFromMemory(PropertyLayoutCR propertyLayout, uint32_t removeIndex, uint32_t removeCount) {
    ...
    Byte const* currentData = (Byte const*)_GetData();
    ...
    if (!hasFixedSizedElements) {
        ...
        SecondaryOffset beginSecondaryIndexValueOffset = GetOffsetOfArrayIndex(arrayOffset, propertyLayout, removeIndex);
        Byte* secondaryDestination = propertyData + beginSecondaryIndexValueOffset;
        uint32_t offsetDelta = removeCount * sizeof(SecondaryOffset);

        source = secondaryDestination + offsetDelta;
        bytesToMove = propertyBytesAllocated - (beginSecondaryIndexValueOffset + offsetDelta);

        if (bytesToMove > 0)
            memmove(secondaryDestination, source, bytesToMove);

        totalBytesAdjusted += (uint32_t)(source - secondaryDestination);
    ...
    if (firstMoveIndex >= preArrayCount)  // removed last entries
    {
        for (uint32_t iRemove = removeIndex; iRemove < removeIndex + removeCount; iRemove++) {
            ...
            auto pNullflagsCurrent = preNullflagsStart + group;
    ...
    } else  // removing entries from middle of array
    {
        ...
        for (uint32_t iRemove = removeIndex; iRemove < postArrayCount; iRemove++) {
            ...
            auto pOldNullflagsCurrent = preNullflagsStart + oldgroup;
...
ECObjectsStatus ECDBuffer::EnsureSpaceIsAvailableForArrayIndexValue(PropertyLayoutCR propertyLayout, uint32_t arrayIndex, uint32_t bytesNeeded) {
    ...
    uint32_t endOfValueDataPreGrow = *((SecondaryOffset const*)(GetPropertyData() + propertyLayout.GetOffset()) + 1);
...
ECObjectsStatus ECDBuffer::GrowPropertyValue(PropertyLayoutCR propertyLayout, uint32_t additionalBytesNeeded) {
    ...
    if (bytesUsed > bytesAllocated) {
        BeAssert(false);
        return ECObjectsStatus::MemoryBoundsOverrun;
    }
    ...
    Byte* writeableData = const_cast<Byte*>(GetPropertyData());
...
ECObjectsStatus ECDBuffer::ShiftArrayIndexValueData(PropertyLayoutCR propertyLayout, uint32_t arrayIndex, uint32_t arrayCount, uint32_t endOfValueDataOffset, int32_t shiftBy) {
    ...
    Byte const* data = (Byte const*)GetPropertyData();
    SecondaryOffset const* pNextProperty = (SecondaryOffset const*)(data + propertyLayout.GetOffset()) + 1;
    uint32_t arrayOffset = GetOffsetOfPropertyValue(propertyLayout);
    ...
    if (bytesToMove > 0) {
        Byte* source = const_cast<Byte*>(data + indexValueOffset);
        Byte* destination = source + shiftBy;
    ...
    SecondaryOffset const* pCurrent = (SecondaryOffset const*)(data + GetOffsetOfArrayIndex(arrayOffset, propertyLayout, arrayIndex + 1));
...
static ECObjectsStatus duplicateProperties(IECInstanceR target, ECValuesCollectionCR source, bool ignoreErrors) {
    ...
    for (ECValuesCollection::const_iterator it = source.begin(); it != source.end(); ++it) {
        ...
        if (prop.HasChildValues()) {
            // PropertyValueMatchesNoChange return value is stupid.
...
ECObjectsStatus ECDBuffer::CopyPropertiesFromBuffer(ECDBufferCR srcBuffer) {
    ...
    return CopyStructInstancesFromBuffer(srcBuffer);
}

/*---------------------------------------------------------------------------------**/ /**
 * @bsimethod
 +---------------+---------------+---------------+---------------+---------------+------*/
ECObjectsStatus ECDBuffer::CopyStructInstancesFromBuffer(ECDBufferCR srcBuffer) {
    ClassLayoutCR classLayout = GetClassLayout();
    PRECONDITION(classLayout.Equals(srcBuffer.GetClassLayout()) && "ClassLayout of buffers must be equal", ECObjectsStatus::OperationNotSupported);
 ...
 * @bsimethod
 +---------------+---------------+---------------+---------------+---------------+------*/
ECObjectsStatus ECDBuffer::CopyStructInstancesData(ECDBufferCR src, PropertyLayoutCR srcPropLayout, PropertyLayoutCR myPropLayout, uint32_t dstPropIdx) {
    ECObjectsStatus status = ECObjectsStatus::Success;
    ArrayCount nEntries = src.GetReservedArrayCount(srcPropLayout);
    for (ArrayCount arrayIdx = 0; arrayIdx < nEntries; arrayIdx++) {
        ECValue srcStructVal, dstStructVal;
        dstStructVal.SetIsNull(true);
        if (ECObjectsStatus::Success == src._GetStructArrayValueFromMemory(srcStructVal, srcPropLayout, arrayIdx) && !srcStructVal.IsNull()) {
            // The ECDBuffer we copied from source to 'this' contains struct ID values relevant only to source buffer
            // So clear out the struct ID entry in 'this' first.
            // WIP_FUSION: Assumption that struct IDs are always integers and 0 == null struct
            //  There seems to be no reason not to enforce that.
            ...
            IECInstanceP asInstance = nullptr;
            ECPropertyCP ecprop = nullptr;
            StructArrayECPropertyCP structProp = nullptr;
            if (nullptr == (asInstance = _GetAsIECInstance()) || nullptr == (ecprop = asInstance->GetEnabler().LookupECProperty(dstPropIdx)) || nullptr == (structProp = ecprop->GetAsStructArrayProperty())) {
                BeAssert(asInstance != nullptr && "programmer error: should only be called on buffers with a valid IECInstance");
                BeAssert(structProp != nullptr && "programmer error: should only be called for struct properties");
                continue;
            }

            IECInstancePtr dstStruct = structProp->GetStructElementType().GetDefaultStandaloneEnabler()->CreateInstance();
            dstStruct->GetECDBufferP()->CopyDataBuffer(*srcStructVal.GetStruct()->GetECDBuffer(), true);
            dstStructVal.SetStruct(dstStruct.get());

            if (ECObjectsStatus::Success != (status = _SetStructArrayValueToMemory(dstStructVal, myPropLayout, arrayIdx))) {
                // This useless return value is a constant pain in the...
                if (ECObjectsStatus::PropertyValueMatchesNoChange == status)
                    status = ECObjectsStatus::Success;
                else
...
ECObjectsStatus ECDBuffer::SetPrimitiveValueToMemory(ECValueCR v, PropertyLayoutCR propertyLayout, bool useIndex, uint32_t index, bool alreadyCalculated) {
    ...
    if (!alreadyCalculated && propertyLayout.HoldsCalculatedProperty()) {
        ...
        switch (calcStatus) {  // needswork: if it failed to parse the value with a regexp, maybe we still allow them to set it... just not propagate to dependents?
    ...
    switch (primitiveType) {
        case PRIMITIVETYPE_Integer: {
            ...
            uint32_t const* valueP = (uint32_t const*)(GetPropertyData() + offset);
        ...
        case PRIMITIVETYPE_Binary: {
            ...
            _Analysis_assume_(dataBuffer != nullptr);
...
Utf8String ECDBuffer::InstanceDataToString(Utf8CP indent) const {
    ...
    uint32_t nProperties = classLayout.GetPropertyCount();
    uint32_t nClassNullflagsBitmasks = CalculateNumberNullFlagsBitmasks(classLayout.GetPropertyCountExcludingEmbeddedStructs());

    for (uint32_t i = 0; i < nClassNullflagsBitmasks; i++) {
        uint32_t offset = i * sizeof(NullflagsBitmask);
        Byte const* address = offset + data;
        appendFormattedString(oss, "%s  [0x%" PRIx64 "][%4" PRIu32 "] Nullflags[%" PRIu32 "] = 0x%" PRIx32 "\n", indent, (uint64_t)address, offset, i, *(NullflagsBitmask const*)(data + offset));
    }
    ...
    for (uint32_t i = 0; i < nProperties; i++) {
        ...
        if (propertyLayout->IsFixedSized())
            ...
            SecondaryOffset secondaryOffset = *(SecondaryOffset const*)address;
        ...
        if (propertyLayout->GetTypeDescriptor().IsArray()) {
            ...
            if (count != GetReservedArrayCount(*propertyLayout))
                ...
                if (count > 0) {
                    uint32_t arrayNullflagsOffset;
                    uint32_t nArrayNullflagsBitmasks = CalculateNumberNullFlagsBitmasks(count);

                    if (propertyLayout->IsFixedSized())
                        arrayNullflagsOffset = propertyLayout->GetOffset();
                    else
                        arrayNullflagsOffset = *((SecondaryOffset const*)(data + propertyLayout->GetOffset())) + sizeof(ArrayCount);

                    for (uint32_t j = 0; j < nArrayNullflagsBitmasks; j++) {
                        Byte const* bitAddress = arrayNullflagsOffset + data;

                        appendFormattedString(oss, "%s  [0x%x][%4." PRIu32 "] Nullflags[%" PRIu32 "] = 0x%x\n", indent, bitAddress, arrayNullflagsOffset, j, *(NullflagsBitmask const*)(bitAddress));
                        arrayNullflagsOffset += sizeof(NullflagsBitmask);
                    }
                }

                for (uint32_t j = 0; j < count; j++) {
                    offset = GetOffsetOfArrayIndex(GetOffsetOfPropertyValue(*propertyLayout), *propertyLayout, j);
                    address = data + offset;
                    status = GetValueFromMemory(v, *propertyLayout, j);
                    if (ECObjectsStatus::Success == status)
                        valueAsString = v.ToString();
                    else {
                        Utf8Char temp[1024];
                        BeStringUtilities::Snprintf(temp, "Error (%d) returned while obtaining array index value", status, j);
                        valueAsString = Utf8String(temp);
                    }

                    if (IsArrayOfFixedSizeElements(*propertyLayout))
                        appendFormattedString(oss, "%s      [0x%x][%4." PRIu32 "] %" PRIu32 " = %s\n", indent, address, offset, j, valueAsString.c_str());
                    else {
                        SecondaryOffset secondaryOffset = GetOffsetOfArrayIndexValue(GetOffsetOfPropertyValue(*propertyLayout), *propertyLayout, j);
                        Byte const* realAddress = data + secondaryOffset;

                        appendFormattedString(oss, "%s      [0x%x][%4." PRIu32 "] -> [0x%x][%4." PRIu32 "] %" PRIu32 " = %s\n", indent, address, offset, realAddress, secondaryOffset, j, valueAsString.c_str());
                    }
    ...
    if (1 == nProperties) {
        ...
        SecondaryOffset const* pLast = (SecondaryOffset const*)(data + offsetOfLast);
...
ArrayResizer::ArrayResizer(ClassLayoutCR classLayout, PropertyLayoutCR propertyLayout, ECDBufferR instance, uint32_t resizeIndex, uint32_t resizeElementCount) : m_classLayout(classLayout),
                                                                                                                                                                 ...
                                                                                                                                                                 m_instance(instance),
                                                                                                                                                                 m_resizeIndex(resizeIndex),
                                                                                                                                                                 m_resizeElementCount(resizeElementCount) {
...
ECObjectsStatus ArrayResizer::ShiftDataFollowingResizeIndex() {
    ...
    DEBUG_EXPECT(pNextProperty == m_propertyData + *((SecondaryOffset const*)(m_propertyData + m_propertyLayout.GetOffset()) + 1));
#endif
    if (m_preArrayByteCount > offsetOfResizePoint) {
        ...
        m_instance.MoveData(const_cast<Byte*>(m_pResizeIndexPostShift), m_pResizeIndexPreShift, byteCountToShift);
    }
...
ECObjectsStatus ArrayResizer::SetSecondaryOffsetsFollowingResizeIndex() {
    ...
    if (m_preAllocatedArrayCount > m_resizeIndex) {
        ...
        m_postSecondaryOffsetOfResizeIndex = *((SecondaryOffset const*)m_pResizeIndexPostShift) + m_resizeFixedSectionByteCount;
    } else {
    ...
    SecondaryOffset* pSecondaryOffset = const_cast<SecondaryOffset*>((SecondaryOffset const*)(m_pResizeIndexPostShift - insertedSecondaryOffsetByteCount));
...
ECObjectsStatus ArrayResizer::ShiftDataPreceedingResizeIndex() {
    ...
    Byte* pShiftTo = const_cast<Byte*>(m_propertyData + m_arrayOffset + m_postHeaderByteCount);
...
ECObjectsStatus ArrayResizer::WriteArrayHeader() {
    ...
    ECObjectsStatus status = ECObjectsStatus::Success;
    ScopedWriteBuffer writeBuffer(m_postHeaderByteCount, m_instance._AllowWritingDirectlyToInstanceMemory(), const_cast<Byte*>(m_propertyData + m_arrayOffset));
    Byte* pWriteBuffer = writeBuffer.GetData();
    if (!m_instance._AllowWritingDirectlyToInstanceMemory())
...
bool ECDHeader_v0::ReadHeader(ECDHeader_v0& hdrOut, Byte const* data) {
    ...
    ECDHeader_v0 const* hdr = (ECDHeader_v0 const*)data;
...
uint32_t ECDBuffer::GetOffsetToPropertyData() const {
    uint8_t const* hdrSizeP = (uint8_t const*)(_GetData() + ECDOFFSET_HeaderSize);
    return *hdrSizeP;
}
...
ECObjectsStatus ECDBuffer::CopyDataBuffer(ECDBufferCR src, bool allowClassLayoutConversion) {
    ...
    /*
    CopyPropertiesFromBuffer returns OperationNotSupported if class layouts are not compatible,
    using it here, we could get a nice optimization by copying directly without conversion conditions.
    However, class layout equality does not guarantee equality of referenced struct types in the layout
    for an illustration, see the following

    schema SchemaA {
        struct MyStruct { radius: string }
        class MyClass { elements: MyStruct[] }
    }
    schema SchemaB {
        struct MyStruct { radius: double }
        class MyClass { elements: MyStruct[] }
    }

    Then SchemaA.MyClass and SchemaB.MyClass would be considered to have the same class layout, which would
    incorrectly suggest to CopyPropertiesFromBuffer to copy directly which would mix up double and string properties.

    To support this in ECDBuffer, either the ClassLayout checksum needs to take struct types of property layouts
    into consideration, or we need a separate checksum for that case.
    */

    ECObjectsStatus status = ECObjectsStatus::Success;

    ClassLayoutCR srcLayout = src.GetClassLayout(),
                  dstLayout = GetClassLayout();

    if (!allowClassLayoutConversion)
    ...
    for (PropertyLayoutCP srcPropLayout : srcLayout.m_propertyLayouts) {
        ...
        if (ECObjectsStatus::Success == dstLayout.GetPropertyLayout(dstPropLayout, srcPropLayout->GetAccessString())) {
            uint32_t dstPropIdx;
            if (ECObjectsStatus::Success != dstLayout.GetPropertyLayoutIndex(dstPropIdx, *dstPropLayout)) {
                BeAssert(false && "couldn't get destination property index at known index");
                continue;
            }
            ...
            if (dstType.GetTypeKind() != srcType.GetTypeKind()) {
                continue;  // mismatched types always null
            ...
            } else  // array
            {
                ...
                if (ECObjectsStatus::Success == src.GetValueFromMemory(v, *srcPropLayout) && v.IsArray() && v.GetArrayInfo().GetCount() > 0) {
                    ...
                    // copy primitive array contents, which is used as an index for struct arrays
                    for (uint32_t i = 0; i < count; i++) {
                        if (ECObjectsStatus::Success == src.GetPrimitiveValueFromMemory(v, *srcPropLayout, true, i)) {
                            ...
                            if (!srcType.IsPrimitiveArray() || v.ConvertToPrimitiveType(dstType.IsStructArray() ? GetStructArrayPrimitiveType() : dstType.GetPrimitiveType()))
                                SetPrimitiveValueToMemory(v, *dstPropLayout, true, i, true);
                        }
                    }

                    if (srcType.IsStructArray() && dstType.IsStructArray()) {
                        status = CopyStructInstancesData(src, *srcPropLayout, *dstPropLayout, dstPropIdx);
                        if (ECObjectsStatus::Success != status)
                            return status;
                    }
                }
