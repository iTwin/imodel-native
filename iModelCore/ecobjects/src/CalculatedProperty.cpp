...
CalculatedPropertySpecificationPtr CalculatedPropertySpecification::Create(ECPropertyCR ecprop, PrimitiveType primitiveType) {
    ...
    if (ECObjectsStatus::Success != customAttr->GetValue(v, "ECExpression") || v.IsNull()) {
        BeAssert(false && "CalculatedECPropertySpecification must contain an ECExpression");
        return NULL;
    }
    ...
    if (node.IsNull()) {
        BeAssert(false && "Could not parse ECExpression for CalculatedECPropertySpecification");
        return NULL;
    }
