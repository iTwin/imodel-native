...
ExpressionStatus AssignmentNode::_GetValue(EvaluationResult& evalResult, ExpressionContextR context) {
     ...
     * @bsimethod
     +---------------+---------------+---------------+---------------+---------------+------*/
    template <>
    /*static*/ bool PerformCompare<double>(double l, ExpressionToken op, double r)  // Android compiler gives error "explicit template specialization cannot have a storage class" with the static keyword
    {
    ...
    ExpressionStatus ComparisonNode::_GetValue(EvaluationResult & evalResult, ExpressionContextR context) {
        ...
        if (ecLeft.IsNull() || ecRight.IsNull()) {
            ...
            switch (m_operatorCode) {
                case TOKEN_Equal:
                    boolResult = leftnull && rightnull;
                    break;
                case TOKEN_NotEqual:
                    boolResult = leftnull != rightnull;
                    break;
                default:
                    break;  // less/greater operators nonsensical for null values
            }
    ...
    struct ExpressionRemapper : NodeVisitor {
       private:
        ...
        ExpressionRemapper(ECSchemaCR pre, ECSchemaCR post, IECSchemaRemapperCR remapper)
            : m_preSchema(pre), m_postSchema(post), m_remapper(remapper), m_currentClass(nullptr), m_state(kState_AwaitingAccessor), m_anyRemapped(false), m_schemaRenamed(!pre.GetName().Equals(post.GetName())) {
            //
        }
        ...
        template <size_t N>
        bool ExtractArguments(Utf8StringP (&args)[N], CallNodeCR callNode);
        template <size_t N>
        void ReplaceArguments(Utf8StringP (&args)[N], CallNodeCR callNode);
     ...
     * @bsimethod
     +---------------+---------------+---------------+---------------+---------------+------*/
    template <size_t N>
    bool ExpressionRemapper::ExtractArguments(Utf8StringP(&args)[N], CallNodeCR callNode) {
     ...
     * @bsimethod
     +---------------+---------------+---------------+---------------+---------------+------*/
    template <size_t N>
    void ExpressionRemapper::ReplaceArguments(Utf8StringP(&args)[N], CallNodeCR callNode) {
    ...
    // @bsimethod
    //---------------------------------------------------------------------------------------
    ::int32_t ResolvedIIfNode::_GetIntegerValue(ExpressionStatus & status, ExpressionContextR context) { return m_condition->_GetBooleanValue(status, context) ? m_true->_GetIntegerValue(status, context) : m_false->_GetIntegerValue(status, context); }

    ::int64_t ResolvedIIfNode::_GetLongValue(ExpressionStatus & status, ExpressionContextR context) { return m_condition->_GetBooleanValue(status, context) ? m_true->_GetLongValue(status, context) : m_false->_GetLongValue(status, context); }

    double ResolvedIIfNode::_GetDoubleValue(ExpressionStatus & status, ExpressionContextR context) { return m_condition->_GetBooleanValue(status, context) ? m_true->_GetDoubleValue(status, context) : m_false->_GetDoubleValue(status, context); }

    bool ResolvedIIfNode::_GetBooleanValue(ExpressionStatus & status, ExpressionContextR context) { return m_condition->_GetBooleanValue(status, context) ? m_true->_GetBooleanValue(status, context) : m_false->_GetBooleanValue(status, context); }
