...
public:
    ...
    static Utf8String GetQualifiedName(ECSchemaCR primarySchema, T const& schemaItem)
        ...
        if (!EXPECTED_CONDITION (ECObjectsStatus::Success == primarySchema.ResolveAlias(schemaItem.GetSchema(), alias)))
            {
            NativeLogging::CategoryLogger("ECObjectsNative").warningv ("warning: Cannot qualify a SchemaItem name with an alias unless the schema containing the SchemaItem is referenced by the primary schema."
