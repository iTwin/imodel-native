...
#define READ_OPTIONAL_XML_ATTRIBUTE(_nodeVar, _xmlAttributeName, _setInPointer, _setInPropertyName)                 \
    {                                                                                                               \
        auto attrNode = _nodeVar.attribute(_xmlAttributeName);                                                      \
        if (attrNode && (ECObjectsStatus::Success != _setInPointer->Set##_setInPropertyName(attrNode.as_string()))) \
...
#define READ_OPTIONAL_XML_ATTRIBUTE_IGNORING_SET_ERRORS(_nodeVar, _xmlAttributeName, _setInPointer, _setInPropertyName) \
    {                                                                                                                   \
        ...
        if (!attrNode)                                                                                                  \
            ...
            setterStatus = _setInPointer->Set##_setInPropertyName(attrNode.as_string());                                \
    }
...
#define READ_REQUIRED_XML_ATTRIBUTE(_nodeVar, _xmlAttributeName, _setInPointer, _setInPropertyName, _elementName)       \
    {                                                                                                                   \
        ...
        if (_setInPointer->Set##_setInPropertyName(attrNode.as_string()) != ECObjectsStatus::Success)                   \
            return SchemaReadStatus::InvalidECSchemaXml;                                                                \
    }
...
#define READ_REQUIRED_XML_ATTRIBUTE_IGNORING_SET_ERRORS(_nodeVar, _xmlAttributeName, _setInPointer, _setInPropertyName, _elementName) \
    {                                                                                                                                 \
        ...
        } else if (_setInPointer->Set##_setInPropertyName(attrNode.as_string()) == ECObjectsStatus::ParseError)                       \
   ...
   public:
    ...
    static Utf8String GetQualifiedName(ECSchemaCR primarySchema, T const& schemaItem) {
        ...
        if (!EXPECTED_CONDITION(ECObjectsStatus::Success == primarySchema.ResolveAlias(schemaItem.GetSchema(), alias))) {
            NativeLogging::CategoryLogger("ECObjectsNative").warningv(
                "warning: Cannot qualify a SchemaItem name with an alias unless the schema containing the SchemaItem is referenced by the primary schema."
                "The name will remain unqualified.\n  Primary ECSchema: %s\n  SchemaItem: %s\n ECSchema containing SchemaItem: %s",
                primarySchema.GetName().c_str(), name.c_str(), schemaItem.GetSchema().GetName().c_str());
            return name;
        }
