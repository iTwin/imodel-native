...
public:
    ...
    struct Options
    {
        private:
            ...
            uint32_t m_maxConnectionsPerHost = 0;
            uint32_t m_maxTotalConnections = 0;

        public:
            Options() {};
            Options(BeFileNameCR assetDir, uint32_t maxConnectionsPerHost = 0, uint32_t maxTotalConnections = 0)
    ...
    BEHTTP_EXPORT static void Initialize(Options const& options);
    /*DEPRECATED*/ BEHTTP_EXPORT static void Initialize(BeFileNameCR assetsDirectoryPath);
