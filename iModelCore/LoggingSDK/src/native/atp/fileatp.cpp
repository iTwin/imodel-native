/*--------------------------------------------------------------------------------------+
|
|     $Source: src/native/atp/fileatp.cpp $
|
...
static wchar_t      s_logFileName[1024];

wchar_t* getlogfilename ( LPCWSTR name )
    {
    swprintf ( s_logFileName, L"%s\\%s.log", _wgetenv(L"TEMP"), name );
...
TEST (SimpleFileLogger, configSUCCESS)
    ...
    CHECK ( SUCCESS == LoggingConfig::ActivateProvider(SIMPLEFILE_LOGGING_PROVIDER) );
...
TEST (SimpleFileLogger, configFAILURE)
        ...
        CHECK ( ERROR == LoggingConfig::ActivateProvider(SIMPLEFILE_LOGGING_PROVIDER) );
