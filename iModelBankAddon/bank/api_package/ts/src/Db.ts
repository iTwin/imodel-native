...
export interface ChangesetStats {
  changes: {
    element: ChangeSetCounts,
    model: ChangeSetCounts,
    aspect: ChangeSetCounts,
  };
  total: {
    elements: number;
  };
...
export interface ChangesetStatsAndInfo {
...
export class Db implements IDisposable {
  private _nativeDb: IModelBankNative.NativeSQLiteDb;
  private _statementCache: StatementCache;

  public constructor() {
    this._nativeDb = new (IModelBankNativeHost.addon).NativeSQLiteDb();
  ...
  public get nativeDb(): IModelBankNative.NativeSQLiteDb { return this._nativeDb; }
  ...
  public create(path: string, defaultTxn: DefaultTxn = DefaultTxn.Deferred): void {
    ...
    if (DbResult.BE_SQLITE_OK !== res)
      throw IModelBankNativeHost.makeDbError(res, "Creating database failed.");
  }
  ...
  public open(path: string, mode: OpenMode, defaultTxn: DefaultTxn = DefaultTxn.Deferred): void {
    ...
    if (DbResult.BE_SQLITE_OK !== res)
      throw IModelBankNativeHost.makeDbError(res, "Opening database failed.");
  }
  ...
  public saveChanges(): void {
    ...
    if (DbResult.BE_SQLITE_OK !== res)
      throw IModelBankNativeHost.makeDbError(res, "Saving changes failed.");
  }

  public abandonChanges(): void {
    const res = this._nativeDb.abandonChanges();
    if (DbResult.BE_SQLITE_OK !== res)
      throw IModelBankNativeHost.makeDbError(res, "Abandoning changes failed.");
  }
  ...
  public createTable(tableName: string, ddl: string): void {
    ...
    if (DbResult.BE_SQLITE_OK !== res)
      throw IModelBankNativeHost.makeDbError(res, "Create table failed.");
  }
  ...
  public applyChangeSet(blockFileNames: string[]): ChangesetStatsAndInfo {
    const applyResult: IModelBankNative.NativeApplyResult = this._nativeDb.applyChangeSet(blockFileNames);
    if (applyResult.status !== DbResult.BE_SQLITE_OK) {
      Logger.logError(loggingCategory, `applyChangeSet failed with error ${applyResult.status}`);
      throw new Error(`applyChangeSet failed with error ${applyResult.status}`);
    }

    assert(applyResult.element !== undefined);
    assert(applyResult.element.inserts !== undefined);
    assert(applyResult.element.deletes !== undefined);
    assert(applyResult.element.updates !== undefined);

    const changesetStats: ChangesetStats = {
      total: { elements: 0 },
      changes: {
        element: applyResult.element,
        model: applyResult.model,
        aspect: applyResult.aspect,
      },
    };

    const changesetInfo: ChangesetInfo = {
      containsSchemaChanges: applyResult.containsSchemaChanges,
    };

    this.withPreparedStatement("select count(*) as count from bis_Element", (stmt) => { stmt.step(); changesetStats.total.elements = stmt.getRow().count; });

    return { changesetStats, changesetInfo };
  }
}
