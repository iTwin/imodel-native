...
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F(C3dBridgeTests, TestDenySchemaLock)
    {
    ...
    argvMaker.SetUpBridgeProcessingArgs(testDir.c_str(), C3dBridgeTestsFixture::GetC3dBridgeRegSubKey(), GetC3dBridgeDllName(), DEFAULT_IMODEL_NAME, true, L"imodel-bank.rsp");
...
* @bsimethod                                    Sam.Wilson                      03/19
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F(C3dBridgeTests, MultiBridgeSequencing)
    {
    if (nullptr == GetIModelBankServerJs())
        {
        fprintf(stderr, "Lock tests are supported only when using imodel-bank as the server\n.");
    ...
    std::thread bridge_a([&] 
        ...
        FwkArgvMaker argvMaker;
        argvMaker.SetUpBridgeProcessingArgs(aDir.c_str(), L"bridge_a", GetC3dBridgeDllName(), DEFAULT_IMODEL_NAME, true, L"imodel-bank.rsp");
        argvMaker.ReplaceArgValue(L"--imodel-bank-access-token", ComputeAccessTokenW("A").c_str());

        BentleyApi::BeFileName inputFile;
        MakeCopyOfFile(inputFile, L"c3dtest1.dwg", L"_A");
    ...
    // That's why we have to run the second bridge in a separate process.
        ...
        FwkArgvMaker argvMaker;
        argvMaker.SetUpBridgeProcessingArgs(bDir.c_str(), L"bridge_b", GetC3dBridgeDllName(), DEFAULT_IMODEL_NAME, true, L"imodel-bank.rsp");
        argvMaker.ReplaceArgValue(L"--imodel-bank-access-token", ComputeAccessTokenW("B").c_str());

        BentleyApi::BeFileName inputFile;
        MakeCopyOfFile(inputFile, L"c3dtest1.dwg", L"_B");
...
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F(C3dBridgeTests, ConvertLinesUsingBridgeFwk)
    {
    auto testDir = CreateTestDir();
...
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F(C3dBridgeTests, TestSourceElementIdAspect)
    {
    auto testDir = CreateTestDir();
...
* @bsimethod                                    Abeesh.Basheer                  10/2018
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F(C3dBridgeTests, ConvertAttachmentSingleBridge)
    {
    auto testDir = CreateTestDir();
    ...
    MakeCopyOfFile(refFile, L"c3dtest1.dwg", L"-Ref-1");
    AddLine(refFile);
    ...
    testRegistry.AddBridge(C3dBridgeTestsFixture::GetC3dBridgeRegSubKey(), iModelBridge_getAffinity);
...
TEST_F(C3dBridgeTests, ConvertAttachmentSingleBridgeAlternateRegistry)
    ...
    auto regsubKey = C3dBridgeTestsFixture::GetC3dBridgeRegSubKey();
    bvector<WString> args;
...
+---------------+---------------+---------------+---------------+---------------+------*/
void C3dBridgeTests::SetupTwoRefs(bvector<WString>& args, BentleyApi::BeFileName& masterFile, BentleyApi::BeFileName& refFile1, BentleyApi::BeFileName& refFile2, BentleyApi::BeFileName const& testDir, FakeRegistry& testRegistry)
    {
    MakeCopyOfFile(masterFile, L"c3dtest1.dwg", NULL);
    AddLine(masterFile);

    MakeCopyOfFile(refFile1, L"c3dtest1.dwg", L"-Ref-1");
    AddLine(refFile1);

    MakeCopyOfFile(refFile2, L"c3dtest1.dwg", L"-Ref-2");
    AddLine(refFile2);
...
* @bsimethod                                    Sam.Wilson                      11/2018
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F(C3dBridgeTests, DISABLED_PushAfterEachModel)
    {
    auto testDir = CreateTestDir();
    ...
    // Set up to process a master file and two reference files, all mapped to C3dBridge
    bvector<WString> args;
    SetUpBridgeProcessingArgs(args, testDir.c_str(), C3dBridgeTestsFixture::GetC3dBridgeRegSubKey(), DEFAULT_IMODEL_NAME);
    ...
    std::function<T_iModelBridge_getAffinity> lambda = [=](BentleyApi::WCharP buffer, const size_t bufferSize, iModelBridgeAffinityLevel& affinityLevel,
        ...
        {
        affinityLevel = iModelBridgeAffinityLevel::Medium;
        wcscpy(buffer, C3dBridgeTestsFixture::GetC3dBridgeRegSubKey());
        };
    testRegistry.AddBridge(C3dBridgeTestsFixture::GetC3dBridgeRegSubKey(), lambda);
    ...
    if (true)
        ...
        EXPECT_TRUE(revstats.descriptions.find("c3dtest1-Ref-1") != revstats.descriptions.end()) << "first ref should have been pushed in its own revision";
        EXPECT_TRUE(revstats.descriptions.find("c3dtest1-Ref-2") != revstats.descriptions.end()) << "second ref should have been pushed in its own revision";
        }
    }
...
* @bsimethod                                    Abeesh.Basheer                  11/2018
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F(C3dBridgeTests, DISABLED_OidcTest)
    {
    auto testDir = CreateTestDir();
...
+---------------+---------------+---------------+---------------+---------------+------*/
TEST_F(C3dBridgeTests, DISABLED_TestCodeRemovalPerformance)
    {
    auto testDir = CreateTestDir();
    ...
    if (true)
        ...
        RunTheBridge(args);
        watch.Stop();
        LOGTODB("C3dBridgeTests", "TestCodeRemovalPerformance,", watch.GetElapsedSeconds() * 1000.0);
